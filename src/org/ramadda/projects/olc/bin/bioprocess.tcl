...
#	puts "$p= [set $p]"
	...
	regsub -all {  +} $n { } n
	set toks [split $n { }]
	foreach {taxa v tsn}  $toks break
	set taxa [string tolower $taxa]
	regsub -all {__} $v {} v
	regsub -all {__} $tsn {} tsn	
	set taxa [string trim $taxa]
	set v [string trim $v]
	set tsn [string trim $tsn]
##	puts stderr $taxa
	append ::entries [col taxon_${taxa} $v]
	puts stderr "TAX:$taxa $name $v"
#	puts stderr "taxa: $taxa v: $v tsn:$tsn"
    }
    exit
    ...
    if {$lakota!=""} {
	append ::entries [mtd2 archive_bio_alternate_name {Lakota Name} $lakota]
    }
    if {$lakota_names!=""} {
	append ::entries [mtd2 archive_bio_alternate_name {Lakota Name} $lakota_names]
    }    
