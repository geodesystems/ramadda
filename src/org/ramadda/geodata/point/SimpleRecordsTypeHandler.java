...
public class SimpleRecordsTypeHandler extends PointTypeHandler {
    ...
    public Result processEntryAccess(Request request, Entry entry)
        ...
        List<RecordField> fields = new SimpleRecordsRecordFile(getRepository(), entry).getFields(false);
        ...
        if(action.equals("new"))  {
            ...
            for(RecordField field: fields) {
                ...
                if(Misc.equals(field.getType(),"date")) {
                    value +="T" + request.getString(field.getName()+".time","");
                }
            ...
            getPageHandler().entrySectionOpen(request, entry,  sb,"New Record");
            sb.append(HtmlUtils.form(getRepository().getUrlBase()+"/entry/access"));
            ...
            for(RecordField field: fields) {
                ...
                if(desc==null) desc = "";
                if(field.isTypeNumeric()) extra+=" size=5 ";
                ...
                if(values!=null) {
                    ...
                    widget  = HtmlUtils.textArea(field.getName(),dflt,Integer.parseInt(rows),60);
                } else if(Misc.equals(field.getType(),"date")) {
                    widget = getDateHandler().makeDateInput(request, field.getName(),
                                                            "flexiform", new Date());
                } else {
                    widget  = HtmlUtils.input(field.getName(),dflt,extra);
                }
                sb.append(HtmlUtils.formEntry(field.getLabel()+":", widget+" " + desc));
            }
            ...
            sb.append(HtmlUtils.formPost(getRepository().getUrlBase()+"/entry/access"));
            sb.append(HtmlUtils.hidden(ARG_ENTRYID,entry.getId()));
            ...
            for(List<String> cols: rows) {
                sb.append("<tr valign=top " + (odd?" class=ramadda-row-odd ":" class=ramadda-row-even ") +">");
                odd=!odd;
                sb.append(HtmlUtils.td(HtmlUtils.checkbox("del_" + cnt,"true",false)));
                ...
                for(int i=0;i<cols.size();i++) {
                    ...
                    if(values!=null) {
                        ...
                        try {
                            widget = getDateHandler().makeDateInput(request, arg,
                                                                    "flexiform", Utils.parseDate(col));
                        } catch(Exception exc) {
                            widget = "Bad date: " + HtmlUtils.input(arg,col,"");
                        }
                    } else {
            ...
            getEntryManager().updateEntry(request, entry);
            return new Result(getRepository().getUrlBase()+"/entry/access?action=edit&entryid=" + entry.getId());
        } else {
            return new Result("Error",
    ...
    public String getWikiInclude(WikiUtil wikiUtil, Request request,
        ...
        if (tag.equals("flexi.add")) {
            if(!getAccessManager().canEditEntry(request,  entry)) {
                return "";
            }
            return HtmlUtils.href(getRepository().getUrlBase()+"/entry/access?action=newform&entryid=" + entry.getId(),"Add Record"," class=ramadda-button") +" " +
                HtmlUtils.href(getRepository().getUrlBase()+"/entry/access?action=edit&entryid=" + entry.getId(),"Edit Records"," class=ramadda-button") +"<p>";
        }
        return null;
    }

    public List<List<String>> getRows(Entry entry,List<RecordField> recordFields) {
        List<List<String>> rows = new ArrayList<List<String>>();
        for(String line: StringUtil.split(entry.getValue(IDX_DATA,""), "\n", true, true)) {
            List<String> toks = StringUtil.split(line,",");
            Hashtable<String,String> map = new Hashtable<String,String>();
            for(String tuple: toks) {
                List<String> toks2 = StringUtil.splitUpTo(tuple,":",2);
                String value =toks2.size()==1?"":toks2.get(1);
                value = value.replaceAll("_colon_",":");
    ...
    public class SimpleRecordsRecordFile extends CsvFile {
        ...
        public List<RecordField> getFields(boolean failureOk) {
            if(recordFields!=null) return recordFields;
            recordFields = new ArrayList<RecordField>();
            int cnt=1;
            for(String line: StringUtil.split(entry.getValue(IDX_FIELDS,""), "\n", true, true)) {
                ...
                RecordField field =new RecordField(id, Utils.makeLabel(id),desc,cnt,null);
                if(type.equals("date"))
                    field.setDateFormat(new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss Z"));
                field.setType(type);
                if(type.equals("date") || field.isTypeNumeric())
                    field.setChartable(true);
                if(values!=null) {
                    field.setProperty("values",StringUtil.split(values,",",true,true));
                }
                if(rows!=null) {
