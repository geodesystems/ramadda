    ...
    for (var col=col0; col<cols+col0; col++) {
	  if (view=="values") {
	    if (!marks[row] || !marks[row][col]) {
          ...
          out += "<td class=\"sstd\"  "+(rowSpan?"rowspan='"+rowSpan+"'":"")+" "+(colSpan?"colspan='"+colSpan+"'":"")+" id='"+row+"_"+col+"' onmousedown='mousedown("+row+","+col+");' onmouseup='mouseup();' onmouseover='buildStatus("+row+","+col+");' onclick='mouseoverCell("+row+","+col+");' onclick='mouseoverCell("+row+","+col+");' ondblclick='editCell("+row+","+col+",0);'><div style='"+style+"'>"+htmlEscape(value,true)+"</div></td>";
		}
	  } else if (view=="formulas") {
        out += "<td class=\"sstd\"  id='"+row+"_"+col+"' onmouseover='buildStatus("+row+","+col+");' onclick='mouseoverCell("+row+","+col+");' onclick='mouseoverCell("+row+","+col+");' ondblclick='editCell("+row+","+col+",0);'>"+htmlEscape(getCells(row,col,0),true)+"</td>";
	  } else {
        out += "<td class=\"sstd\"  id='"+row+"_"+col+"' onmouseover='buildStatus("+row+","+col+");' onclick='mouseoverCell("+row+","+col+");' onclick='mouseoverCell("+row+","+col+");' ondblclick='editCell("+row+","+col+",0);'>"+htmlEscape(getCells(row,col,1),true)+"</td>";
	  }
    }
    out += "</tr>";
  }
...
function makeHeader() {
  ...
  if (allowPaging) {
    ...
    out += "<input class='ssinput' type='text' value='"+rows+"' style='width:28px; text-align:center;' id='rows' onfocus='active=\"dimensions\";' onblur='active=\"content\";'> - ";
  }
  ...
  out += "<input class='ssinput' type='text' value='' title='"+trans("Formula")+"' id='value' style='width:400;' disabled onmouseover='previewValue();' onkeyup='previewValue();'> ";
  out += "<input class='ssinput'  type='text' title='"+trans("Style")+"' value='' id='styling' style='width:100;' disabled onmouseover='previewValue();' onkeyup='previewValue();'> ";
  out += "</td><td class=\"sstd\" nowrap>";
  if (isWriteable) {
...
function showHeaderFooter(show) {
  ...
  if (show) {
    ...
    getObj("ssheader").style.display="";
    getObj("ssfooter").style.display="";
    getObj("status").style.display="";
  } else {
    getObj("content").style.top="0px";
    getObj("content").style.bottom="0px";
    getObj("ssheader").style.display="none";
    getObj("ssfooter").style.display="none";
...
function getCells2(col_str,row,item,calls,returnObj) {
  alert("getCells2");
...
function ValuesObject (theValues, colIndices, rowIndices) {
...
function getCells3(col_str,row,col_str2,row2,item,calls,returnObj) {
  ...
  for (var i_row=row; i_row<=row2; i_row++) {
    for (var i_col=col_num; i_col<=col_num2; i_col++) {
	  ...
	  if (val == val_data) result[result.length] = val; else result[result.length] = val_data;
          colIndices[colIndices.length] = i_col;
          rowIndices[rowIndices.length] = i_row;
	}
  }
  if(returnObj) {
      return  new ValuesObject(result, colIndices, rowIndices);
  }
  return result;
}
...
function linechart(valuesObject1,valuesObject2,title,width,height) {
    values1 = valuesObject1.values;
    values2 = valuesObject2.values;
    if(!width) width=250;
    if(!height) height=100;
    if(!title) title = "";
    var v1 = comma(values1);    
    var v2 = comma(values2);
    
   return "html:<img src=\"http://chart.apis.google.com/chart?chs=" + width +"x" + height +"&chtt=" + title +"&chd=t:" + v1+"|" + v2 +"&cht=lc&chl=" + title +"&chco=" + chartColors +"\">"
}


function barchart(valuesObject,title,width,height) {
    if(!width) width=250;
    if(!height) height=100;
    if(!title) title = "";
    values = valuesObject.values;
    var v = comma(values);
    var labels = getColLabels(valuesObject.colIndices);

   return "html:<img src=\"http://chart.apis.google.com/chart?chs=" + width +"x" + height +"&chtt=" + title +"&chd=t:" + v+"&chl=" + labels +"&cht=bvs\">"
}


function piechart(valuesObject,title,width,height) {
    values = valuesObject.values;
    if(!width) width=250;
    if(!height) height=100;
    if(!title) title = "";
    var v = comma(values);
    var labels = getColLabels(valuesObject.colIndices);
    return "html:<img src=\"http://chart.apis.google.com/chart?chs=" + width +"x" + height +"&chtt=" + title +"&chd=t:" + v+"&cht=p3&chl=" + labels +"&chtt=" + title + "\">"
}

function getColLabels(indices) {
    var v = "";
...
function loadSheetFromUrl(location) {
  ...
  if (window.addEventListener) {
    window.addEventListener("load",function() { loadSpreadsheet("// dbCells"); },false);
  } else {
    script.onreadystatechange = function() { if (this.readyState == 'loaded') loadSpreadsheet("// dbCells"); }
  }
  script.src = location;
  document.getElementsByTagName("head").item(0).appendChild(script);
}
...
function showCell(row,col,calls) {
  ...
  if (calls>25) { // avoid endless recursion
    ...
    var forChart  = value.indexOf("chart")>=0;
    var cmd = "";
    var token = "";
    var openToken = "";
    var sequence = "";
    for (var i=0; i<value.length; i++) {
      ...
      if (((token=="'" || token=="\"") && openToken=="") || i==value.length-1) {
	    if (openToken=="") {
		  sequence = sequence.replace(/([A-Z]+)([0-9]+):([A-Z]+)([0-9]+)/g,"getCells3('$1',$2,'$3',$4,0,"+(calls+1)+"," + forChart+")");
		  sequence = sequence.replace(/([A-Z]+)([0-9]+)/g,"getCells2('$1',$2,0,"+(calls+1)+"," +forChart+")");
	    }
	    openToken = token;
	    cmd += sequence;
	    sequence = "";
