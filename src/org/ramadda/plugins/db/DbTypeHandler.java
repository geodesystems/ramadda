...
public class DbTypeHandler extends PointTypeHandler implements DbConstants /* BlobTypeHandler*/ {
    ...
    public void addToEntryNode(Request request, Entry entry,
        ...
        readValues(request, Clause.eq(COL_ID, entry.getId()), "", -1, pw,null);
        pw.flush();
        zos.closeEntry();
    ...
    public String getInlineHtml(Request request, Entry entry)
        ...
        List<Object[]> valueList = readValues(request, entry,
					      Clause.eq(COL_ID, entry.getId()),null);
	...
	addStyleSheet(sb);
        ...
        if (headerToks.size() > 1) {
            HtmlUtils.div(sb,StringUtil.join("&nbsp;|&nbsp;",
	...
	String formId = null;
	    ...
	    System.err.println("search form:" + searchForm.length() +" memory:" + Utils.decimals(mem2-mem1,1));
            sb.append(HtmlUtils.makeShowHideBlock(msg("Search again"), searchForm, false));
        }
	...
	ValueIterator iterator =  makeValueIterator(request, entry, view, action,  fromSearch);
        ...
        if (valueList == null) {
            ...
            if (doCache) {
                if (debugTimes) {
                    System.err.println("Writing to cache:" + valueList.size());
                }
     ...
     * @throws Exception _more_
     */
    public ValueIterator makeValueIterator(Request request, Entry entry, String view,
					   String action, boolean fromSearch)
	throws Exception {
        boolean doGroupBy = isGroupBy(request);
	...
	if (view.equals(VIEW_TABLE) || view.equals("")) {
	    return new ValueIterator.TableIterator(request,this,entry,fromSearch);
	}
	...
	*/
	...
	//	if(true) return;
		...
		buffers.add(buffer = new MyNamedBuffer("Summary",formHeader,"summary"));
                ...
                aggSB.append(HtmlUtils.labeledCheckbox(ARG_AGG_PERCENT, "true",
	...
	buffers.add(buffer = new MyNamedBuffer("Order By/Display",formHeader,"orderby"));
        ...
        viewSB.append(HtmlUtils.labeledCheckbox(ARG_DB_SHOW + "toggleall", "true",false,"Toggle All"));
        viewSB.append("<br>");
        for (Column column : dbInfo.getColumnsToUse()) {
            String arg = ARG_DB_SHOW + "_" + column.getName();
            viewSB.append(HtmlUtils.labeledCheckbox(arg, "true",request.get(arg, defaultShow), column.getLabel()));
            viewSB.append("<br>");
        }
        ...
        for (Column column : dbInfo.getColumnsToUse()) {
            ...
            uniqueSB.append(HtmlUtils.labeledCheckbox(arg, "true",                      request.get(arg,
												    false), column.getLabel()));
            uniqueSB.append("<br>");
        }
	...
	buffers.add(buffer = new MyNamedBuffer("Advanced Options", formHeader,"advanced"));
            ...
            buffer.append(
                HtmlUtils.formEntry(
                    ...
                    HtmlUtils.labeledCheckbox(
                        ARG_DB_OR, "true",
                        request.get(ARG_DB_OR, false), "Match any of the above search criteria (OR logic)")));


            String suffix = getAccessManager().canEditEntry(request, entry)
                            ? HtmlUtils.space(2)
                              + HtmlUtils.labeledCheckbox(ARG_DB_DOSAVESEARCH,
                                  "true",
                                  request.get(ARG_DB_DOSAVESEARCH,
					      false), "Save Search")  : "";

            buffer.append(HU.formEntryTop("", "Add extra columns in output, one per line:"));
            buffer.append(HU.formEntry(msgLabel("Extra Columns"),
					  HtmlUtils.textArea(ARG_EXTRA_COLUMNS,
							     request.getString(ARG_EXTRA_COLUMNS, ""),
							     5,40)));
	    ...
	    String print = 
		HtmlUtils.labeledCheckbox(ARG_FOR_PRINT, "true", request.get(ARG_FOR_PRINT, false),"Printable") +
		HU.space(4) +
		"Entries per page: " +
	...
	double mem1 = Utils.getUsedMemory();
	Result result =  handleList(request, entry, mainClause, "", true);
	double mem2 = Utils.getUsedMemory();
	System.err.println("handleList:" + Utils.decimals(mem2-mem1,1));
	return result;
    }
	...
	List<String> extraCols = Utils.split(request.getString(ARG_EXTRA_COLUMNS,""),"\n",true,true);
	...
	int lineCnt = -1;
		...
		hb.append(tableHeader);
            ...
            String   event  = getEventJS(request, entry, values[IDX_DBID], rowId,
                                         divId);
            hb.append("\n");
    ...
    public Result handleListGrid(Request request, Entry entry,
        ...
        for (Object[] valuesArray : valueList) {
            ...
            String event = getEventJS(request, entry, valuesArray[IDX_DBID], rowId,
                                      rowId);
            String href = HtmlUtils.href(url,
    ...
    public Result handleListCalendar(Request request, Entry entry,
        ...
        for (Object[] values : valueList) {
            ...
            */
            //            html.append(href);
            String rowId = "row_" + values[IDX_DBID];
            String event = getEventJS(request, entry, values[IDX_DBID], rowId, rowId);
	...
	if(iterator!=null) iterator.initialize(request, doGroupBy);
