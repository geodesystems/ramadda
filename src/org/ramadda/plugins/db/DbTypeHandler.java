...
public class DbTypeHandler extends PointTypeHandler implements DbConstants /* BlobTypeHandler*/ {
    ...
    public String addViewHeader(Request request, Entry entry, Appendable sb,
        ...
        if (headerToks.size() > 1) {
            HtmlUtils.div(sb, StringUtil.join("&nbsp;|&nbsp;", headerToks),
        ...
        if (extraLinks != null) {
            HtmlUtils.div(sb, extraLinks, HtmlUtils.cssClass(CSS_DB_HEADER));
        }
        ...
        if (fromSearch) {
            ...
            String searchForm = getSearchForm(request, entry,
                                    formId).toString();
            double mem2 = Utils.getUsedMemory();
            Utils.printMemory("search form:" + searchForm.length() 
			      + " memory:",mem1,mem2);
     ...
     * @throws Exception _more_
     */
    private Result handleQuery(Request request, Entry entry,
                                  String action, Clause clause,
                                  boolean fromSearch)
            throws Exception {
    ...
    private void getSearchFormInner(Request request, Entry entry,
        ...
        if (normalForm) {
            if (tfos.size() > 0) {
                ...
                aggSB.append(HtmlUtils.labeledCheckbox(ARG_AGG_PERCENT,
                    ...
                    buffer.append(
                        formEntry(
                            ...
                            HtmlUtils.select(
                                ...
                                HtmlUtils.cssClass(
                                    "search-select")) + HtmlUtils.space(1)
        ...
        viewSB.append(HtmlUtils.labeledCheckbox(ARG_DB_SHOW + "toggleall",
                "true", false, "Toggle All"));
        viewSB.append("<br>");
        for (Column column : dbInfo.getColumnsToUse()) {
            String arg = ARG_DB_SHOW + "_" + column.getName();
            viewSB.append(HtmlUtils.labeledCheckbox(arg, "true",
        ...
        for (Column column : dbInfo.getColumnsToUse()) {
            ...
            uniqueSB.append(HtmlUtils.labeledCheckbox(arg, "true",
        ...
        buffers.add(buffer = new DbNamedBuffer("Advanced Options",
                formHeader, "advanced"));
        if (normalForm) {
            buffer.append(HtmlUtils.formEntry(msgLabel("Search Type"),
            ...
            String print = HtmlUtils.labeledCheckbox(
                               ARG_FOR_PRINT, "true",
            ...
            if (addressTemplate != null) {
                print += "<br>" + HU.b("Address label skip: ")
                         ...
                         .input("addresslabelskip", request
                             .getString("addresslabelskip", ""), HtmlUtils
        ...
        for (DbNamedBuffer b : buffers) {
            ...
            HU.div(sb,
                   HtmlUtils.makeShowHideBlock(b.getName(), contents, show),
    ...
    public Result handleBulkUpload(final Request request, final Entry entry,
        ...
        DataProvider.CsvDataProvider provider =
            new DataProvider.CsvDataProvider(textReader);
    ...
    public void makeTable(Request request, Entry entry,
        ...
        if (valueList.size() > 0) {
            ...
            HtmlUtils.open(tableHeader, "table", "class", "dbtable",
        ...
        for (int cnt = 0; cnt < valueList.size(); cnt++) {
            ...
            if ( !forPrint) {
                HtmlUtils.open(hb, "td", "width", "10", "style",
                               "white-space:nowrap;");
                HtmlUtils.open(hb, "div", "class", "ramadda-db-div", "id",
                               divId);
                if (doForm) {
                    String call =
                        HtmlUtils.attr(
                    ...
                    hb.append(HtmlUtils.checkbox(ARG_DBID_SELECTED, dbid,
                            false, HtmlUtils.id(cbxId) + call));
                }
                if (canEdit) {
                ...
                for (int i = 0; i < values.length; i++) {
                    ...
                    if (obj instanceof Double) {
                        ...
                        if (s.length() == 0) {
                            ...
                            s = HtmlUtils.href(
                                url, s,
                                HtmlUtils.attr("title", "Click to search")
    ...
    public Result handleListMap(Request request, Entry entry,
        ...
        for (List listValues : lists) {
            ...
            for (Object obj : listValues) {
                ...
                if (bbox) {
                    map.addBox("", "", "",
                               new MapBoxProperties("red", false), north,
                               west, south, east);
                }
                if (getDbInfo().getMapCategoryColumn() != null) {
                    String cat =
            ...
            if (catMap != null) {
                ...
                for (String cat : cats) {
                    ...
                    String content = HtmlUtils.insetLeft(catSB.toString(),
                                         20);
                    entryList.append(HtmlUtils.makeShowHideBlock(cat,
                            content, open));
                    open = false;
                }
            }
     ...
     * @return _more_
     */
    protected List<TwoFacedObject> getEnumValues(Request request, Entry entry,
            Column column) {
        try {
    ...
    public Result handleListCategory(Request request, Entry entry,
        ...
        for (Object[] valuesArray : valueList) {
            ...
            String event = getEventJS(request, entry, valuesArray[IDX_DBID],
                                      rowId, rowId);
    ...
    public Result handleListCalendar(Request request, Entry entry,
        ...
        boolean canEdit      = getAccessManager().canEditEntry(request,
                                   entry);
        StringBuilder sb     = new StringBuilder();
        String links =  getHref(request, entry, VIEW_ICAL, msg("ICAL"));
    ...
    public void createBulkForm(Request request, Entry entry, Appendable sb,
        ...
        bulkSB.append(HtmlUtils
            .formEntry("", HtmlUtils
                .labeledCheckbox(ARG_DB_BULK_NUKEIT, "true", false, "Delete existing database") + HtmlUtils
                .space(2) + HU
