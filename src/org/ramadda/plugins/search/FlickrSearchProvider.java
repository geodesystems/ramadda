...
public class FlickrSearchProvider extends SearchProvider {
    ...
    private static final String URL ="https://www.flickr.com/services/rest/?method=flickr.photos.search&format=rest";
	...
	url += "&" + HtmlUtils.arg("extras", "description,date_taken,geo,tags");
	url += "&" + HtmlUtils.arg("per_page", ""+max);
	url += "&" + HtmlUtils.arg("page", ""+page);
	SelectionRectangle rect = request.getSelectionBounds();
	if(rect.anyDefined()) {
	    String bbox = rect.getWest(-180) +"," + rect.getSouth(-90) +","  + rect.getEast(180) +"," + rect.getNorth(90);
	...
	System.err.println(getName() + " url:" + url);
        ...
        for (int childIdx = 0; childIdx < children.getLength(); childIdx++) {
            ...
            Date    dttm     = null;
	    String date = XmlUtil.getAttribute(item,"datetaken",(String)null);
	    if(date!=null) dttm  = Utils.parseDate(date);
	    if(dttm==null) dttm = new Date();
            double  latitude  = XmlUtil.getAttribute(item, "latitude", Double.NaN);
            double  longitude  = XmlUtil.getAttribute(item, "longitude", Double.NaN);	    
	    ...
	    if(desc2.length()>0) desc = desc +"<br>" + desc2;

            Entry newEntry = new Entry(Repository.ID_PREFIX_SYNTH + getId()
                                       + TypeHandler.ID_DELIMITER
                                       + id, typeHandler);
	    if(!Double.isNaN(latitude) && latitude!=0 && longitude!=0) {
