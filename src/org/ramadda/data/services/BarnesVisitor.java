...
public class BarnesVisitor extends BridgeRecordVisitor {
    ...
    public BarnesVisitor(RecordOutputHandler handler, Request request,
        ...
        for(String tok: (List<String>)request.get(RecordOutputHandler.ARG_DIVISOR,new ArrayList<String>())) {
    ...
    public boolean doVisitRecord(RecordFile file, VisitInfo visitInfo,
        ...
        if(divisors.size()>0) {
            ...
            float o = value;
            if(values==0) value = Float.NaN;
            else value = value/values;
        ...
        if(points == null) {
            ...
            minLat = Float.isNaN(lat)?minLat:(float)Math.min(minLat,lat);
            maxLat = Float.isNaN(lat)?maxLat:(float)Math.max(maxLat,lat);
            minLon = Float.isNaN(lon)?minLon:(float)Math.min(minLon,lon);
            maxLon = Float.isNaN(lon)?maxLon:(float)Math.max(maxLon,lon);
        }
    ...
    public void finishedWithAllFiles() throws Exception {
        ...
        Barnes.AnalysisParameters ap =
            Barnes.getRecommendedParameters(minLon, minLat, maxLon,
                                     maxLat, data3D);

        float[] lons = ap.getGridXArray();
        float[] lats = ap.getGridYArray();
        float[][] faaGrid =
            Barnes.point2grid(lons,lats,
                              data3D, (float) ap.getScaleLengthGU(), gain,
                              numPasses);


        grid = new  IdwGrid(faaGrid.length,faaGrid[0].length, maxLat, minLon, minLat,maxLon);
