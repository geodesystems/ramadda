...
//            let garea = MapUtils.squareMetersToSquareFeet(geometry.getGeodesicArea(this.map.getMap().getProjectionObject()));
	...
	menuItem: function(id,label,cmd) {
	    if(cmd) {
		let prefix = "";
		if(cmd!="Esc") prefix = HU.image(icon_command,['width','12px']);
		label = HU.leftRightTable(label,HU.div(['style','margin-left:8px;'], prefix+" " +HU.span(['style','color:#ccc'], cmd)));
	    }
...
//	    this.clearCommands();
	...
	loadMap: function(entryId) {
	    ...
	    let _this = this;
		    ...
		    if(data=="") data="[]";
		    try {
		    this.loadAnnotationJson(JSON.parse(data),_this.map,_this.myLayer,_this.glyphMap);
			this.featureHasBeenChanged = false;
			this.showLegend();
