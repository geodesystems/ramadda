...
function RamaddaEntryDisplay(displayManager, id, type, properties) {
	...
	getEntriesMetadata:function(entries) {
	    let mdtsFromEntries = [];
            ...
            let headerRow = HU.tr([ATTR_VALIGN, ALIGN_BOTTOM], HU.join(headerItems, ""));
            html += headerRow;
		...
		this.galleryId = HU.getUniqueId("gallery_");
                let newHtml = HU.open(TAG_DIV,[ATTR_ID, this.galleryId,
					       ATTR_CLASS,"ramadda-grid"]);
		let itemWidth = this.getProperty("galleryItemWidth",HU.px(200));
		    ...
		    let imageUrl =entry.getImageUrl();
                    ...
                    newHtml += HU.div([ATTR_CLASS, "image-outer"],
				      HU.div([ATTR_CLASS, "image-inner"], img) +
...
function RamaddaSearcherDisplay(displayManager, id,  type, properties) {
	...
	*/
	    ...
	    label = HU.div([ATTR_CLASS,'display-search-label-toggle',
			   ...
			   HU.span([ATTR_ID,imageId],
				   HU.getIconImage(toggleClose?'fa-plus':'fa-minus', [],
						   [ATTR_STYLE,HU.css(CSS_COLOR,COLOR_WHITE)])) +
			   ' ' + HU.span([],label));
	    setTimeout(()=>{
	...
	addWidget:function(label,widget,args) {
	    ...
	    html += HU.tag(TAG_TD, [ATTR_ID,this.getDomId(ID_SEARCH_FORM),
				    ATTR_WIDTH,HU.perc(1)], form);
	    this.formShown  = true;
	    ...
	    let _this=this;
            let theDisplay = this;
	    $('.'+this.getUniqueClass('searchbutton')).button().click(function(event) {
...
//			title='Entries';
	...
	makeSearchSettings: function() {
		...
		if(id==OUTPUT_EXPORT && !this.canExport()) return false;
	    ...
	    this.getContents().find(HU.dotClass(CLASS_SEARCH_TEXTINPUT)).each(function() {
		...
		if(col.isNumeric()) {
		    ...
		    if(col.getType()==COLUMN_TYPE_STRING ||
	...
	getSearchValue:function(key,dflt) {
	    ...
	    let html = HU.select('',[ATTR_MULTIPLE,true,
				     ATTR_SIZE,4,
	    ...
	    let buttons = [];
	    let searchButton = HU.span([ATTR_ID, this.getDomId(ID_SEARCH),
				  ATTR_CLASS,
				  HU.classes(CLASS_BUTTON,
					     this.getUniqueClass('searchbutton'),
					     'display-search-button',CLASS_CLICKABLE)], buttonLabel);
		...
		buttons.push(HU.span([ATTR_ID, this.getDomId(ID_DOWNLOAD_XLSX),
            ...
            if (this.ramaddas.length > 0) {
                ...
                for (let i = 0; i < this.ramaddas.length; i++) {
                    ...
                    let attrs = [ATTR_TITLE, "",
				 ATTR_VALUE, ramadda.getId(),
				 "data-iconurl", icon
				];
		...
		if(providers.length==1) {
			...
			this.getPropertyProviders().forEach(provider=>{
			    ...
			    if(img) {
				img = img.replace(/\${urlroot}/g, RamaddaUtil.getBaseUrl());
				img = img.replace(/\${root}/g, RamaddaUtil.getBaseUrl());
		...
		if(this.typeList == null || this.typeList.length==0) {
                    widgetMap['types'] = HU.div([ATTR_STYLE,HU.css(CSS_MARGIN_BOTTOM,HU.px(4)),
						ATTR_ID, this.getDomId(ID_TYPE_DIV)],
					       HU.span([ATTR_CLASS, "display-loading"], "Loading types..."));
		} else {
		    widgetMap['types'] =
			HU.div([ATTR_STYLE,HU.css(CSS_MARGIN_BOTTOM,HU.px(4)),
				ATTR_ID, this.getDomId(ID_TYPE_DIV)]);
		}
            }
	    ...
	    let textFields = [];
            let textField = HU.input('', text,
				     Utils.mergeLists([ATTR_TEXT_INPUT,'Text'],attrs,inputAttrs));
            if (this.getShowText()) {
		...
		widgetMap['text'] = this.addWidget('Text',Utils.join(textFields,HU.br()),{
		    toggleClose:this.getTextToggleClose(toggleClose)});
	    }
		...
		if(Utils.stringDefined(label))
		    ...
		    dateWidget+=HU.div([ATTR_STYLE,HU.css(CSS_MARGIN_TOP,HU.px(4))],
				       this.changedateRangeWidget.getHtml());
            }


	    if(Utils.stringDefined(dateWidget)) {
		widgetMap['date'] = this.addWidget('Date',dateWidget,{
		    toggleClose:this.getDateToggleClose(toggleClose)
		});
	    }
            if (this.getShowArea()) {
		...
		let areaWidget =new AreaWidget(this);
                ...
                widgetMap['area'] =  this.addWidget(label, HU.div([ATTR_ID,this.domId(ID_SEARCH_AREA)],
								  areaWidget.getHtml()),
	    ...
	    Object.keys(widgetMap).forEach(type=>{
		...
		contents = HU.div([ATTR_STYLE,
				   HU.css(CSS_OVERFLOW_Y,OVERFLOW_AUTO,
	...
	getFormText:function() {
	    ...
	    let dest =     jqid(this.getMetadataFieldId(metadataType));
	    dest.html('');
	    this.idToElement = {};
...
*/		
		...
		let toggleClose = this.getTypesToggleClose(this.getToggleClose());
...
function RamaddaSearchDisplay(displayManager, id, properties, theType) {
	...
	doDownload:function(url,extra,size) {
		...
		HU.initPageSearch('#' + id +' .search-component',
				  null,'Filter',false,{addToUrl:false,target:$(this),inputSize:'10'});
	    ...
	    if(this.galleryId) {
		HU.createFancyBox(jqid(this.galleryId).find("a.popup_image"),{helpers:{title:{type:'over'}}});
	    }
		...
		jqid(this.tabId).tabs({activate: tabbed,active: index});
	    }	
		...
		let fields = [new RecordField({type: "string", index: (index++),
			      ...
			      new RecordField({type: "string", index: (index++),
					       id: "tags",label: "Tags"}),
			      new RecordField({type: "string", index: (index++),
					       id: "display_html",label: "Display Html"}),
			      new RecordField({type: "string", index: (index++),
					       id: 'id', label: "Entry ID"}),			      
			      new RecordField({index: (index++),
		...
		if(this.entryTypes && this.entryTypes.length) {
			...
			fields.push(new RecordField({index: (index++),
						     id: column.getName(),label: column.getLabel()}));
		    });
		}
		let records = [];
		let defaultImage = this.getDefaultImage();
		if(defaultImage) {
			...
			defaultImage = RamaddaUtil.getUrl(defaultImage);
		}
		...
		this.myDisplays.forEach(info=> {
			...
			dialog.find(HU.dotClass(CLASS_SEARCH_TAG)).click(function() {
			    let type = $(this).attr(ATTR_METADATA_TYPE);
			    let value = $(this).attr(ATTR_METADATA_VALUE);			    
		...
		this.areaEntries.forEach(entry=>{
		    ...
		    let text = link;
		    if(entry.getIsImage()) {
			text = HU.image(entry.getResourceUrl(),
					[ATTR_WIDTH, 400,
					 ATTR_ID, this.getDomId("entry_" + entry.getIdForDom()),
					 ATTR_ENTRYID, entry.getId(),
					 ATTR_CLASS, "display-entrygallery-entry"
...
function RamaddaEntrygridDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
        ...
        initDisplay: function() {
            let _this = this;
            this.createUI();
            let html = HU.div([ATTR_ID, this.getDomId(ID_CONTENTS)],
			      this.getLoadingMessage("Loading entries..."));
            ...
            let myCallback = {
                entryListChanged: function(list) {
                    ...
                    _this.canvas = jqid(_this.getDomId(ID_CANVAS));
                    _this.gridPopup = jqid(_this.getDomId(ID_GRID) + " .display-grid-popup");
                    ...
                    jqid(_this.getDomId(ID_GRID) + " .display-grid-action").click(function() {
        ...
        initGrid: function(entries) {
            ...
            items.mouseover(function(evt) {
                ...
                if (thumb) {
                    ...
                    html += HU.image(entry.getResourceUrl(),
				     [ATTR_WIDTH, 300]) + HU.br();
                }
        ...
        makeFramework: function(entries) {
            ...
            html += HU.openTag(TAG_TD,
			       [ATTR_STYLE, HU.css(CSS_HEIGHT,HU.px(this.getProperty("height", "400")))]);
            ...
            html += HU.div([ATTR_CLASS, "display-grid-axis-bottom ramadda-noselect",
			    ATTR_TITLE, mouseInfo,
			    ATTR_ID, this.getDomId(ID_AXIS_BOTTOM)], "");
        ...
        makeGrid: function(entries) {
            ...
            for (let i = 0; i < axis.Y.ticks.length; i++) {
                ...
                axis.Y.html += HU.div([ATTR_STYLE, style,
				      ...
				      tick.label + " " +
				      HU.div([ATTR_CLASS, "display-grid-htick"], ""));
            }
    ...
    RamaddaUtil.defineMembers(this, {
        ...
        displayRepositories: function() {
                ...
                for (let i = 0; i < this.ramaddas.length; i++) {
                    ...
                    if (repoType == null) {
					   ...
					   ATTR_TARGET, "_blank"],
                        ...
                        row += HU.td([ATTR_ALIGN, ALIGN_RIGHT,
            ...
            for (let i = 0; i < cats.length; i++) {
                ...
                let rows = catMap[cat];
                html += HU.open(TAG_TR);
                html += HU.th([ATTR_COLSPAN, "" + (1 + this.ramaddas.length)], cat);
                html += HU.close(TAG_TR);
...
function DisplayEntryMetadataElement(display,metadata,element) {
	...
	makeInput:function() {
	    this.inputId = this.display.getMetadataFieldId(this.metadata.getType())+
		'_element_' +
		this.getIndex()+
		'_input';
	    let input = HU.input('','',[ATTR_CLASS,CLASS_SIMPLESEARCH_INPUT,
	...
	makeCheckboxes:function(idToElementMap,multiples) {
	    ...
	    this.selectId = this.display.getMetadataFieldId(this.metadata.getType())+"_select_" + this.getIndex();
            let select =multiples?'': HU.tag(TAG_OPTION,
					     [ATTR_TITLE, "",
					      ATTR_VALUE, ""]);
