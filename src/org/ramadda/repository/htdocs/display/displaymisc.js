...
function RamaddaGraphDisplay(displayManager, id, properties) {
	...
	{p:'targetField',ex:''},
	{p:'labelField'},
	{p:'nodeBackground',ex:COLOR_LIGHT_GRAY},
	{p:'drawCircle',ex:'true'},
	{p:'nodeWidth',d:'10'},
	{p:'linkColor',d:COLOR_LIGHT_GRAY},
	{p:'linkDash',ex:'5'},
	{p:'linkWidth',d:'1'},
	{p:'arrowLength',ex:'6'},
...
function RamaddaTreeDisplay(displayManager, id, properties) {
	...
	countToRecord: {},
		...
		let block = jqid(baseId+"_block"+cnt);
		let img = jqid(baseId+"_toggle_image"+cnt);
		$(this).attr("toggle-state",state);
		if(state)  {
		    block.css(CSS_DISPLAY,DISPLAY_BLOCK);
		    img.attr(ATTR_SRC,icon_downdart);
		} else {
		    block.css(CSS_DISPLAY,DISPLAY_NONE);
		...
		state = !state;
		let cnt = $(this).attr("block-count");
		let block = jqid(baseId+"_block_details"+cnt);
		let img = jqid(baseId+"_toggle_details_image"+cnt);
		$(this).attr("toggle-state",state);
		if(state)  {
		    block.css(CSS_DISPLAY,DISPLAY_BLOCK);
		    img.attr(ATTR_SRC,icon_downdart);
...
function RamaddaTimelineDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
		...
		this.jq(ID_TIMELINE).find(".tl-storyslider").css(CSS_DISPLAY,DISPLAY_NONE);
		this.jq(ID_TIMELINE).find(".tl-menubar").css(CSS_DISPLAY,DISPLAY_NONE);		
	    } 
	    ...
	    this.jq(ID_TIMELINE).find(".tl-slidenav-description").css(CSS_DISPLAY,DISPLAY_NONE);
...
function RamaddaHoursDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
	    ...
	    html = HU.open(TAG_DIV,[ATTR_STYLE,HU.css(CSS_POSITION,POSITION_RELATIVE)]) +
		HU.open(TAG_TABLE,[ATTR_WIDTH,HU.perc(100)]);
		...
		html +=  HU.tr([ATTR_STYLE,HU.css(CSS_BORDER_BOTTOM,HU.border(1,COLOR_LIGHT_GRAY))],
			       HU.tds([],["",HU.div([ATTR_CLASS,"display-hours-label"], dayLabel),"#"]));
		let multiCount = 0;
		Utils.sortNumbers(dayInfo.hours).forEach(hour=>{
		    let row = HU.open(TAG_TR,[ATTR_STYLE,HU.css(CSS_BORDER_TOP,HU.border(1,COLOR_LIGHT_GRAY))]);
		    //		    if(hour!=9) return
		    let hourLabel  = HU.div([ATTR_STYLE,this.getPropertyDayLabelStyle("")], Utils.formatHour(hour));
		    row += HU.td([ATTR_WIDTH,10,
				  ATTR_ALIGN,ALIGN_RIGHT],hourLabel);
		    row += HU.open(TAG_TD,[ATTR_STYLE,HU.css(CSS_BACKGROUND,'#efefef'),
			...
			let left =  HU.perc(Math.round(minutes/61.0*100));
			let key = hour+"_"+minutes;
			if(dayInfo.minutesCount[key]>1) {
				...
				HU.div([MULTI_ID,displayed[minutes].multiid,
					ATTR_RECORD_ID, record.getId(),
					ATTR_RECORD_INDEX,_this.recordToIndex[record.getId()],
					ATTR_TITLE,"",
			    ...
			    row+= HU.div([ATTR_RECORD_ID, record.getId(),
					  ATTR_RECORD_INDEX,_this.recordToIndex[record.getId()],
					  ATTR_TITLE,"",
					  ATTR_STYLE, css,
					  ATTR_CLASS,'display-hours-box']);
			...
			extra+=HU.div([ATTR_ID,this.domId(id),
				       ATTR_CLASS,"display-hours-box-extra"],displayed[minute].contents);
		    }
		...
		if($(this).attr("showing")=="true") {
		    div.hide();
		    $(this).css(CSS_BORDER,HU.border(1,COLOR_LIGHT_GRAY));
        ...
        handleEventRecordSelection: function(source, args) {
	    if(!args.record) return;
	    let select = ".display-hours-box[" + ATTR_RECORD_ID +"='" + args.record.getId()+"']";
...
function RamaddaBlankDisplay(displayManager, id, properties,type) {
    if(!properties.width) properties.width=HU.perc(100);
...
function RamaddaHtmltableDisplay(displayManager, id, properties,type) {
	...
	{p:'tableStyle',d:HU.css(CSS_BORDER_LEFT,CSS_BASIC_BORDER,CSS_BORDER_RIGHT,CSS_BASIC_BORDER)},
	{p:'tableHeaderStyle'},
 	{p:'tableCellStyle'},	
	...
	handleColumn:function(fields,aggByField,field,record,v,tdAttrs,matchers) {
	    ...
	    let span = HU.span([ATTR_ID,aggId+"_toggle",
				"toggleopen","false",
				ATTR_CLASS,CLASS_CLICKABLE],
			       HU.span([ATTR_ID,aggId+"_toggleimage"],
				       HU.getIconImage("fas fa-chevron-right"))+SPACE + v);
	    tdAttrs = Utils.mergeLists(tdAttrs,[ATTR_NOWRAP,null]);
	...
	updateHtmlTable: function(records) {
	    ...
	    html +=HU.openTag(TAG_TABLE,[ATTR_CLASS,HU.classes(CLASS_TABLE,CLASS_TABLE_STRIPE),
					 ATTR_WIDTH,HU.perc(100),
					 ATTR_ID,this.domId(ID_TABLE),
	    ...
	    if(anyGroups) {
		let attrs = [ATTR_STYLE,HU.css(CSS_BACKGROUND,COLOR_WHITE,
					       CSS_WIDTH,HU.perc(100))];
		html+=HU.open(TAG_TR,[ATTR_STYLE,HU.css(CSS_BACKGROUND,COLOR_TRANSPARENT),
		...
		fields.forEach((f,idx)=>{
			...
			if(group!=f.getGroup()) {
				...
				html+=HU.th([ATTR_CLASS,"display-table-group-header-th",
					     ATTR_STYLE,HU.css(CSS_BORDER_BOTTOM,HU.border(0,COLOR_TRANSPARENT),
							       CSS_BACKGROUND,COLOR_TRANSPARENT),
					     ATTR_COLSPAN,groupCnt[group]],
					    HU.div([ATTR_CLASS,"display-table-group-header"], group))+"\n";
			    }
			}
			return;
		    }
	    ...
	    let header1=HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_TOP]);
	    let header2=HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_TOP]);
	    if(includeDate) {
	    ...
	    let headerStyle = this.getTableHeaderStyle("")+HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER);
	    let fieldMap = {}
		...
		let attrs = [ATTR_TITLE,title,
			     ATTR_CLASS,HU.classes(CLASS_CLICKABLE,'display-table-header'),
		...
		if(fancy) {
		    ...
		    if(sort) label = HU.getIconImage(sortAscending?"fas fa-arrow-down":"fas fa-arrow-up",null,
						     [ATTR_STYLE,HU.css(CSS_FONT_SIZE,HU.important(HU.pt(8)))]) +" " + label;
	    ...
	    header1+=HU.close(TAG_TR);
	    header2+=HU.close(TAG_TR);
	    html+=header1;
		...
		fields.forEach((f,idx)=>{
		    ...
		    let color = null;
		    let foreground=COLOR_BLACK;
		    let tdAttrs = [];
		    let props = fieldProps[f.getId()]??{};
		    if(props.isNumeric)
			tdAttrs = [ATTR_ALIGN,ALIGN_RIGHT];
			...
			addColumn(HU.td(Utils.mergeLists(tdAttrs,
							 [ATTR_STYLE,HU.css(CSS_BACKGROUND, color,
									    CSS_COLOR,HU.important(fg))]),sv),null,f);
		    } else if(props.showBar) {
			let percent = 1-(value-props.barMin)/(props.barMax-props.barMin);
			percent = HU.perc((percent*100));
			let contents = "";
			sv = HU.perc(Utils.formatNumberComma(value));
			...
			let outer = HU.div([ATTR_CLASS,"ramadda-bar-outer",
					    ATTR_STYLE, (width?HU.css(CSS_WIDTH,HU.getDimension(width)):"")+
					    HU.css(CSS_MIN_WIDTH,HU.px(100))+(props.barLabelInside?HU.css(CSS_HEIGHT,"1.5em"):"")],bar);
			if(props.barLabelInside) {
			    addColumn(HU.td([],outer),null,f);
			} else {
			    addColumn(HU.td([],HU.row([[ATTR_ALIGN,ALIGN_RIGHT],sv],outer)),null,f);
			}
		...
		if(categoryField) {
			...
			for(let j=0;j<columns.length-1;j++)
			    extra+=HU.td([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_NONE),
					  ATTR_CLASS,'display-htmltable-category-cell',],'');
			let row = HU.tr([ATTR_CLASS,'display-htmltable-category-row'],
					HU.td([ATTR_CLASS,'display-htmltable-category-cell',
					       ATTR_COLSPAN,columns.length],
					      HU.div([ATTR_CLASS,'display-htmltable-category'],c))+extra);
			html+=row;
		    }
		}		
		...
		if(record.isAggregate)
		    html+=HU.openTag(TAG_TR,[ATTR_STYLE,HU.css(CSS_FONT_WEIGHT,"550")+rowStyle,
					     'aggregateRow',aggId,
					     ATTR_TITLE,'',
					     ATTR_VALIGN,ALIGN_TOP,
					     ATTR_CLASS,clazz,
		    ...
		    html+=HU.openTag(TAG_TR,[ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_NONE),
					     "aggregateId", aggId,
					     ATTR_TITLE,'',
					     ATTR_VALIGN,ALIGN_TOP,
					     ATTR_CLASS,clazz,
		    ...
		    html+=HU.openTag(TAG_TR,[ATTR_STYLE,rowStyle,"aggregateId", aggId,
					     ATTR_TITLE,'',
					     ATTR_VALIGN,ALIGN_TOP,
					     ATTR_CLASS,clazz +' display-htmltable-row-' +(even?'even':'odd'),
					     ATTR_RECORD_ID,record.getId()]);
		...
		html+=HU.div([ATTR_ID,this.domId("addrow"),
			      ATTR_CLASS,CLASS_CLICKABLE],
	    ...
	    this.setContents(html);
	    aggIds.forEach(id=>{
		jqid(id+"_toggle").click(function() {
			...
			row.find(".display-td").css(CSS_FONT_WEIGHT,FONT_BOLD).css(CSS_BORDER_BOTTOM,HU.border(1,'#888'));
		...
		colorRowBy.forEach((cb,idx)=>{
		    if(idx>0)
			dom.append(HU.div([ATTR_STYLE,HU.css(CSS_BORDER_BOTTOM,HU.border(1,COLOR_LIGHT_GRAY))]));
		    idx = (colorBarCnt++);
		    let id = this.domId(ID_COLORTABLE+idx);
		    dom.append(HU.div([ATTR_ID,id]));
		...
		let field = fieldMap[$(this).attr(ATTR_FIELD_ID)];
	    ...
	    rows.click(function() {
		let record = _this.recordMap[$(this).attr(ATTR_RECORD_ID)];
		if(!record) return;
		_this.propagateEventRecordSelection({record:record});
	    });
		...
		let recordIndex = dom.attr(ATTR_RECORD_INDEX); 
...
function RamaddaPolltableDisplay(displayManager, id, properties) {
	...
	makeColumn:function(record,field,attrs,v) {
	    ...
	    v = HU.div([ATTR_STYLE,HU.css(CSS_OVERFLOW_Y,OVERFLOW_AUTO,CSS_MAX_HEIGHT,this.cellsToggled?TOGGLED_CSS:HU.px(500)),
			ATTR_CLASS,'display-polltable-field',
			'data-record-id',record.getId()],v);
	    return HU.td(attrs,v);
	},
	...
	updateUI: function() {
	    ...
	    let buttons = HU.buttons([HU.div([ATTR_STYLE,HU.css(CSS_MIN_WIDTH,HU.px(130)),
					      ATTR_ID,this.domId(ID_TOGGLE_CELLS)],
					     this.cellsToggled?'Expand Cells':'Toggle Cells'),
	...
	addVotes:function() {
		...
		if(!$(this).attr(ATTR_FIELD_ID))return;
	...
	applyShowVotes:function () {
		...
		stats+=HU.space(2)+'('+HU.span([ATTR_STYLE,HU.css(CSS_FONT_WEIGHT,FONT_BOLD,
								  CSS_COLOR,'#D85F48')],'* controversial')+')';
		...
		blocks.each(function() {
		    ...
		    if(c) {
			style+=HU.css(CSS_COLOR,c,CSS_FONT_WEIGHT,FONT_BOLD);
		    }
		    let icon = i=>{
			let attrs=[];
...
function RamaddaTsneDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, [], {
        ...
        updateUI: async function(pointData) {
            ...
            this.setContents(HU.table([ATTR_WIDTH,HU.perc(100)], HU.tr([ATTR_VALIGN,ALIGN_TOP],
								       HU.td([ATTR_WIDTH,HU.perc(80)], canvas) +
								       HU.td([ATTR_WIDTH,HU.perc(20)], details))));
        ...
        takeStep: function() {
            ...
            if (!this.haveStepped) {
                this.canvas.find(".display-tnse-mark").click(function(e) {
                    ...
                    let details = HU.open(TAG_TABLE,[ATTR_CLASS,CLASS_FORMTABLE,
						     ATTR_WIDTH,HU.perc(100)]);
...
function RamaddaHeatmapDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, [], {
        ...
        updateUI: function(pointData) {
            ...
            html += HU.open(TAG_TABLE, [ATTR_BORDER, 0,
					ATTR_CLASS, "display-heatmap"]);
            html += HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_BOTTOM]);
            if (showIndex) {
                html += HU.td([ATTR_ALIGN,ALIGN_CENTER],
			      HU.div([ATTR_CLASS, "display-heatmap-heading-top"], header[0]));
            }
            for (let fieldIdx = 0; fieldIdx < fields.length; fieldIdx++) {
                let field = fields[fieldIdx];
                if ((!field.isFieldNumeric() || field.isFieldGeo())) continue;
                html += HU.td([ATTR_ALIGN,ALIGN_CENTER],
			      HU.div([ATTR_CLASS, "display-heatmap-heading-top"], this.getFieldLabel(field)));
            }
            html += HU.close(TAG_TR);
            ...
            for (let rowIdx = 1; rowIdx < dataList.length; rowIdx++) {
                ...
                if (showIndex) {
                    html += HU.td([ATTR_CLASS, "display-heatmap-heading-side",
				   ATTR_STYLE, extraCellStyle + extraTdStyle], rowLabel);
                }
                let colCnt = 0;
                for (let colIdx = 0; colIdx < fields.length; colIdx++) {
                    let field = fields[colIdx];
                    ...
                    if (!showValue) number = ""; 
                    html += HU.td([ATTR_VALIGN, ALIGN_CENTER,
				   ATTR_ALIGN, ALIGN_RIGHT,
				   ATTR_STYLE, style + extraCellStyle + extraTdStyle,
				   ATTR_CLASS, "display-heatmap-cell"],
				  HU.div([ATTR_TITLE, title,
					  ATTR_STYLE, extraCellStyle + HU.css(CSS_COLOR,textColor)], number));
                    colCnt++;
                }
                html += HU.close(TAG_TR);
            }
...
function RamaddaRankingDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
		...
		if(stringField) stringFields.push(stringField);
            ...
            let menu = HU.open(TAG_SELECT,[ATTR_CLASS,CLASS_PULLDOWN,
					   ATTR_ID, this.domId("sortfields")]);
            for (let i = 0; i < numericFields.length; i++) {
                let field = numericFields[i];
                let extra = "";
                if (field.getId() == sortField.getId()) extra = " selected ";
                menu += HU.tag(TAG_OPTION,[ATTR_VALUE, field.getId(), extra,null], this.getFieldLabel(field));
            }
	    ...
	    top += HU.span([ATTR_ID,this.domId("sort")],
            ...
            if (this.getProperty("showRankingMenu", true)) {
                top+= " " + HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK),
				    ATTR_CLASS,"display-filterby"],menu);
            }
	    this.jq(ID_TOP_LEFT).html(top);
	    this.jq("sort").click(()=>{
		this.sortAscending= !this.sortAscending;
		if(this.sortAscending) 
		    this.jq("sort").html(HU.getIconImage("fa-sort-up",
							 [ATTR_STYLE,HU.css(CSS_CURSOR,CURSOR_POINTER)]));
		else
		    this.jq("sort").html(HU.getIconImage("fa-sort-down",
							 [ATTR_STYLE,HU.css(CSS_CURSOR,CURSOR_POINTER)]));
		this.updateUI();
	    });
            let html = "";
            html += HU.open(TAG_DIV, [ATTR_STYLE, HU.css(CSS_MAX_HEIGHT,HU.perc(100),
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
		...
		records.forEach((r,idx)=>{
		    ...
		    let box = HU.open(TAG_TABLE,[ATTR_CELLPADDING,0,ATTR_CELLSPACING,0,
						 ATTR_STYLE,HU.css(CSS_BORDER,HU.border(1,COLOR_LIGHT_GRAY),
...
function RamaddaCrosstabDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, [], {
	    ...
	    let select = HU.span([ATTR_CLASS,"display-filterby"],
				 "Display: " + HU.select("",[ATTR_STYLE,"",
							     ATTR_ID,this.domId("crosstabselect")],enums,
	...
	makeTable: function() {
	    ...
	    let rows =  this.getFieldsByIds(null, this.getProperty(ATTR_ROWS, null, true));
	    if(!col) col  = allFields[0];
	    if(rows.length==0) rows  = allFields;

            let html = HU.open(TAG_TABLE, [ATTR_BORDER, HU.px(1),
					   "bordercolor", COLOR_LIGHT_GRAY,
					   ATTR_CLASS, "display-crosstab",
					   ATTR_CELLSPACING, 1,
					   ATTR_CELLPADDING, 2]);
		...
		rowValues.sort();
		if(cnt==1)
		    html+=HU.tr([],HU.td()+ HU.td([ATTR_ALIGN,ALIGN_CENTER,
						   ATTR_CLASS,'display-crosstab-header',
						   ATTR_COLSPAN,colValues.length], col.getLabel()) +
				HU.td([],SPACE));
		html+=HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_BOTTOM,
				      ATTR_CLASS,'display-crosstab-header-row'],
		...
		for(let i=0;i<rowValues.length;i++) {
			...
			if(Utils.isDefined(count[key])) {
			    let perc = HU.perc(Math.round(count[key]/total*100));
			    html+=HU.td([ATTR_ALIGN,ALIGN_RIGHT], count[key] +SPACE+"(" + perc+")");
			} else {
			    html+=HU.td([],SPACE);
			}
		    }
		    let perc = Math.round(rowcount[rowValue]/total*100) +"%";
		    html+=HU.td([ATTR_ALIGN,ALIGN_RIGHT], rowcount[rowValue] +SPACE +'(' + perc+')');
		    html+=HU.close(TAG_TR);
		}
	    });
            html += HU.close(TAG_TABLE);
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
		...
		let lowSlider = HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK)],HU.div([],"Negative Correlation") +  
				       ...
				       HU.div([ATTR_STYLE,HU.css(CSS_HEIGHT,HU.px(20),
								 CSS_DISPLAY,DISPLAY_INLINE_BLOCK,
								 CSS_WIDTH,HU.px(200),
								 CSS_BACKGROUND,this.getProperty('lowSliderBackground','#FD9596')),
					       ATTR_ID,this.gid(ID_SLIDER_LOW)]) +
				       HU.div([ATTR_ID,this.gid(ID_SLIDER_LOW_MAX),
					       ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_LEFT,
								 CSS_WIDTH,HU.px(50),
								 CSS_DISPLAY,DISPLAY_INLINE_BLOCK,
		...
		let highSlider = HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK)], HU.div([],"Positive Correlation") +
					...
					HU.div([ATTR_STYLE,HU.css(CSS_HEIGHT,HU.px(20),
								  CSS_DISPLAY,DISPLAY_INLINE_BLOCK,
								  CSS_WIDTH,HU.px(200),
								  CSS_BACKGROUND,this.getProperty('highSliderBackground','#64A982')),
						ATTR_ID,this.gid(ID_SLIDER_HIGH)]) +
	...
	makeTable: function() {
	    ...
	    this.csv =[];
            html += HU.open(TAG_TABLE, [ATTR_CELLSPACING,0,
					ATTR_CELLPADDING, 0,
					ATTR_BORDER, 0,
					ATTR_CLASS, "display-correlation",
					ATTR_WIDTH, "100%"]);
            let col1Width = 10 + "%";
            let width = 90 / fieldCnt + "%";
            html += HU.open(TAG_TR,[ATTR_VALIGN,CSS_BOTTOM]) +
		HU.td([ATTR_CLASS,"display-heading",
		       ATTR_WIDTH, col1Width],SPACE);
		...
		html += HU.td([ATTR_ROWFIELD,field1.getId(),
            ...
            html += HU.tr([],HU.td([]) + HU.td([ATTR_COLSPAN,(fieldCnt + 1)],
					       HU.div([ATTR_ID, this.domId(ID_LASTROW)], "")));
            html += HU.close(TAG_TABLE);
	    this.jq(ID_TABLE).html(html);
	    let _this = this;
	    let selectedRow;
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
		...
		let div = "";
		    ...
		    if(v.getTime) v = this.formatDate(v);
                    div += HU.boldLabel(this.getFieldLabel(field)) + v + HU.br() +"\n";
                }
                let box = HU.div([ATTR_CLASS,showCards?"":"display-records-record",
				  ATTR_RECORD_INDEX,rowIdx,
				  ATTR_RECORD_ID, records[rowIdx].getId()], div);
		...
		let record = _this.records[$(this).attr(ATTR_RECORD_INDEX)];
		if(record) {
...
function RamaddaStatsDisplay(displayManager, id, properties, type) {
    ...
    defineDisplay(this, SUPER, myProps, {
	    ...
	    let attrs = [ATTR_ID,this.getDomId("statstable"),
			 ATTR_CLASS, HU.classes(CLASS_TABLE_ROWBORDER,CLASS_TABLE_STRIPE,'display-stats')];
	    ...
	    html+=HU.open(TAG_THEAD);
            if (!justOne) {
                ...
                if(showTableHeader)
		    html += HU.tr([ATTR_VALIGN, ALIGN_BOTTOM],
				  HU.ths([ATTR_CLASS, "display-stats-header",
					  ATTR_ALIGN, ALIGN_CENTER], header));  
            }
	    html+=HU.close(TAG_THEAD);
		...
		let addValue=(v,label) =>{
			...
			let tmp =p=> {
			    ...
			    if(doValueSelection) {
				s = HU.span([ATTR_CLASS,"display-stats-value",
					     ATTR_DATA_TYPE, "percentile",
					     ATTR_DATA_VALUE, p],s);
			    }
			...
			addValue(stat.numMissing,'Missing');
                ...
                right = HU.tds([ATTR_ALIGN, ALIGN_RIGHT], values);
                let align = (justOne ? ALIGN_RIGHT : ALIGN_LEFT);
                let label = this.getFieldLabel(field);
		...
		if(!showTableHeader) label=label+":";
                let row = HU.tr([ATTR_VALIGN,ALIGN_TOP],
				HU.td([ATTR_NOWRAP,"true",ATTR_ALIGN, align],
				      type + HU.b(HU.span([ATTR_TITLE, tooltip], label))) + right);
                html += row;
            });
		...
		values.each(function() {
		    ...
		    let links = SPACE + HU.getIconImage("fa-less-than",[ATTR_TITLE,"Filter other displays",
									ATTR_CLASS,"display-stats-value-link",
									ATTR_DATA_TYPE,type,
...
function RamaddaCooccurenceDisplay(displayManager, id, properties) {
	...
	getHeader2:function() {
		...
		html +=  HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK)],
				"Sort by: " + HU.select("",[ATTR_ID,this.domId(ID_SORTBY)],
							enums,this.getProperty("sortBy","")))+SPACE2;
	...
	initHeader2:function() {
		...
		HU.open(TAG_TABLE,[ATTR_STYLE,HU.css(CSS_HEIGHT,'100%'),
				   ATTR_CLASS,'display-cooc-table', 'order',0]);
	    table += HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_BOTTOM]) +
		HU.td([ATTR_BORDER,'none']);
	    targets.map(target=>{
		target = target.replace(/ /g,SPACE).replace(/-/g,SPACE);
		table += HU.td([ATTR_STYLE,HU.css(CSS_BORDER,'none'),
				ATTR_WIDTH,6],
		...
		table += HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_BOTTOM]) +
		    HU.td([ATTR_STYLE,HU.css(CSS_BORDER,'none'),
			   ATTR_ALIGN,ALIGN_RIGHT],
		...
		targets.map(target=>{
			...
			if(weight == missing || maxWeight == 0) 
			    style = HU.css(CSS_BACKGROUND,COLOR_LIGHT_GRAY);
			else {
			    if(colorBy.index>=0) {
...
function RamaddaBoxtableDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
	    ...
	    let html = HU.open(TAG_TABLE,[ATTR_CLASS,'display-colorboxes-table',
					  ATTR_CELLPADDING,5]);
		...
		let label = HU.span([ATTR_FIELD_ID,categoryField.getId(),
				     ATTR_FIELD_VALUE,cat], cat);
		let tdAttrs = [ATTR_ALIGN,ALIGN_RIGHT,
			       ATTR_CLASS,'display-colorboxes-header'];
		if(labelColumnWidth)
		    tdAttrs.push(ATTR_WIDTH,labelColumnWidth);
		let row = HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_CENTER],
				  HU.td(tdAttrs,label+ " ("+length+")"));
		row+=	  HU.open(TAG_TD);
		...
		catMap[cat].list.forEach((record,idx)=>{
			...
			contents  = HU.div([ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER)],
					   HU.image(url,[ATTR_WIDTH,imageWidth]));
			...
			contents +=HU.div([ATTR_CLASS,'display-colorboxes-label',
					   ATTR_STYLE,labelStyle],label);
		    }
		    row +=HU.div([ATTR_TITLE,"",
				  ATTR_RECORD_ID, record.getId(),
				  ATTR_CLASS,HU.classes(CLASS_CLICKABLE,clazz),
				  ATTR_STYLE,style],contents);
		});
...
function RamaddaPercentchangeDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
		...
		html += HU.tr([], HU.th([ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER)],
					this.getProperty("fieldLabel", "Field")) +
			      HU.th([ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER)], label1) +
			      HU.th([ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER)], label2)
			      + HU.th([ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER)], "Percent Change"));
		html += HU.close(TAG_THEAD);
		html += HU.open(TAG_TBODY, []);
	    }
	    let tuples= [];
	    ...
	    let highlightColor = this.getProperty("highlightColor",COLOR_LIGHT_GRAY);
		...
		if(template) {
		    ...
		    html += HU.tr([ATTR_STYLE,style], HU.td([], this.getFieldLabel(t.field)) + 
				  HU.td([ATTR_ALIGN,ALIGN_RIGHT], this.formatNumber(t.val1)) +
				  HU.td([ATTR_ALIGN,ALIGN_RIGHT], this.formatNumber(t.val2))
				  + HU.td([ATTR_ALIGN,ALIGN_RIGHT], HU.perc(t.percent)));
		}
	    });
...
function RamaddaDatatableDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
	    ...
	    let table = "";
	    table+=HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_BOTTOM]) + HU.td([],"");
		...
		if(needToRotate) {
		    ...
		    label = HU.div([ATTR_TITLE,column.label,
				    ATTR_CLASS,"display-datatable-header-slant"],label);
		}		    
		table+=HU.td([ATTR_CLASS,'display-datatable-header',
			      ATTR_ALIGN,ALIGN_CENTER],label);
	    });
	    table+=HU.close(TAG_TR);
		...
		columns.map(column=>{
			...
			extra2 = HU.div(["data-key",key,
					 ATTR_CLASS,"display-datatable-counts"]);
			if(cell.checked.length) {
			    extra1= HU.getIconImage(this.getProperty("checkedIcon","fa-check"),
						    [ATTR_TITLE,"",
						     "data-key",key,
		    ...
		    table += HU.td([ATTR_CLASS,'display-datatable-cell',
				    ATTR_ALIGN,ALIGN_RIGHT,
				    ATTR_STYLE,style,
				    ATTR_WIDTH,HU.perc(width)], cellHtml);
		});
		if(showRowTotals) {
		    ...
		    table += HU.td([ATTR_WIDTH,100,
				    ATTR_VALIGN,ALIGN_TOP],bar);
		}
		table += HU.close(TAG_TR);
	    });
	    if(showColumnTotals) {
		table+=HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_TOP]) + HU.td();
		columns.map(column=>{
	    ...
	    table+=HU.td([ATTR_COLSPAN,cellCount,
			  ATTR_CLASS,'display-datatable-footer',
			  ATTR_ALIGN,ALIGN_CENTER,
			  ATTR_ID,this.domId(ID_COLORTABLE)]);
	    table+=HU.close(TAG_TD);
	    table+=HU.open(TAG_TR,[],HU.td());
	    table+=HU.td([ATTR_COLSPAN,cellCount,
			  ATTR_CLASS,'display-datatable-footer',
			  ATTR_ALIGN,ALIGN_CENTER,
			  ATTR_ID,this.domId(ID_PIECOLORS)]);
		...
		headerRow+=  HU.td([ATTR_CLASS,"display-datatable-selector",
				    ATTR_ALIGN,ALIGN_CENTER],
				   HU.select("",[ATTR_ID,this.domId("rowSelector")],
					     selectors,
					     rowSelector,15));
	    }
	    if(this.getProperty("showColumnSelector",true)) {
		headerRow+=  HU.td([ATTR_COLSPAN,columns.length,
				    ATTR_CLASS,"display-datatable-selector",
				    ATTR_WIDTH,HU.perc(90),
				    ATTR_ALIGN,ALIGN_CENTER],
				   HU.select("",[ATTR_ID,this.domId("columnSelector")],
					     selectors,
					     columnSelector));
	    }

	    let mainTable = HU.open(TAG_TABLE,[ATTR_STYLE,HU.css(CSS_FONT_SIZE, this.getProperty("fontSize",'8pt')),
					       ATTR_CLASS,'display-colorboxes-table',
					       ATTR_CELLPADDING,0,
					       ATTR_CELLSPACING,0,
					       ATTR_WIDTH,'100%']);
		...
		countFields.forEach((f,idx)=>{
		    ...
		    $(this).append(HU.div([ATTR_CLASS,"display-datatable-piechart",
					   ATTR_ID,id,
					   ATTR_TITLE,"",
...
function RamaddaSparklineDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
		...
		groups.values.forEach((value,idx)=>{
		    ...
		    let gid = id+"_"+ +idx;
		    let c = HU.div([ATTR_CLASS,"display-sparkline-sparkline",
				    ATTR_ID,gid,
		    ...
		    jqid(id).append(HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK,CSS_MARGIN,HU.px(4))],c));
		    let gcol = this.getColumnValues(grecords, field);
		    drawSparkline(this, "#"+gid,w,h,gcol.values,grecords,min,max,colorBy);
		});		
	    } else {
		html = HU.div([ATTR_CLASS,"display-sparkline-sparkline",
			       ATTR_ID,this.domId(ID_INNER),
			       ATTR_STYLE,HU.css(CSS_WIDTH, HU.px(w),CSS_HEIGHT, HU.px(h))]);
		if(showDate) {
		...
		let left = this.getProperty("showMin")? HU.div([ATTR_CLASS,"display-sparkline-value",
								ATTR_STYLE, this.getPropertyLabelStyle("")],
							       this.formatNumber(col.values[0])):"";
		let right = this.getProperty("showMax",true)? HU.div([ATTR_CLASS,"display-sparkline-value",
								      ATTR_STYLE, this.getPropertyLabelStyle("")],
								     this.formatNumber(col.values[col.values.length-1])):"";
		if(left!=""  || right!="")
...
function RamaddaPointimageDisplay(displayManager, id, properties) {
	...
	{p:'doHeatmap',ex:'true'},
	{p:'padding',ex:'5'},
	{p:'borderColor',ex:COLOR_LIGHT_GRAY},
	...
	updateUI: function() {
		...
		jqid(this.getProperty(PROP_DIVID)).css(CSS_BORDER,HU.border(1,this.getProperty("borderColor")));
	    }
	    let bounds ={};
	    ...
	    let html = HU.div([ATTR_ID,this.domId("inner"),
			       ATTR_STYLE,style]);
	    this.setContents(html); 
	    let pad = 10;
...
function RamaddaCanvasDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
		...
		let c = HU.tag(TAG_CANVAS,[ATTR_CLASS,canvasClass,
					   ATTR_STYLE,canvasStyle, 	
					   ATTR_WIDTH,canvasWidth,
					   ATTR_HEIGHT,canvasHeight,
					   ATTR_ID,cid]);
		let icon = iconField? HU.image(record.getValue(iconField.getIndex()))+SPACE:"";
		...
		let div =  HU.div([ATTR_TITLE,"",
				   ATTR_CLASS,"display-canvas-block",
				   ATTR_RECORD_INDEX,idx,
				   ATTR_RECORD_ID, record.getId()], topTitle+c+title);
		if(urlField) {
		    let url = record.getValue(urlField.getIndex());
...
function RamaddaFieldtableDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
	    ...
	    let html = HU.open(TAG_TABLE,[ATTR_CLASS, "",
					  ATTR_BORDER,0,
					  ATTR_ID,this.domId(ID_TABLE)]);
	    html += HU.open(TAG_THEAD);
		...
		if(labelField) {
		    hdrAttrs.push(ATTR_FIELD_ID);
		    hdrAttrs.push(labelField.getId());
		    hdrAttrs.push(ATTR_FIELD_VALUE);
		    hdrAttrs.push(r.getValue(labelField.getIndex()));
		}
		html += HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_CENTER,
					ATTR_RECORD_INDEX,idx,
					ATTR_RECORD_ID, r.getId(),
					ATTR_CLASS,"display-fieldtable-row"]);
		html+=HU.td([ATTR_STYLE,HU.css(CSS_VERTICAL_ALIGN,ALIGN_CENTER),
			     ATTR_ALIGN,ALIGN_RIGHT],HU.div(hdrAttrs,label));
		fields.forEach(f=>{
		    let v = r.getValue(f.getIndex());
		    ...
		    let inner = HU.tag(TAG_CANVAS,[ATTR_TITLE,"Value:" + v +"   Range:" + c.min +" - " + c.max,
						   ATTR_STYLE,cstyle, 
						   ATTR_WIDTH,canvasWidth,
						   ATTR_HEIGHT,cw,
						   ATTR_ID,cid]);
		    contents +=HU.div([ATTR_STYLE,HU.css(CSS_POSITION,POSITION_ABSOLUTE,CSS_LEFT,HU.px(0),
							 CSS_RIGHT, HU.px(cw))],
				      inner);
		    html+=HU.td(["data-order", v,
				 ATTR_STYLE,HU.css(CSS_VERTICAL_ALIGN,'middle'),
				 ATTR_ALIGN,ALIGN_RIGHT,
				 ATTR_TITLE, "Range:" + c.min +" - " + c.max],
				HU.div([ATTR_STYLE,HU.css(CSS_POSITION,POSITION_RELATIVE,
							  CSS_WIDTH,HU.px(width),
							  CSS_HEIGHT,HU.px(1),
							  CSS_MARGIN_LEFT,HU.px(10),
							  CSS_MARGIN_RIGHT,HU.px(10),
							  CSS_BORDER,HU.border(1,COLOR_LIGHT_GRAY))],contents));
	    ...
	    let markerStroke = this.getProperty("markerStroke",COLOR_BLACK);
...
function RamaddaSelectedrecordsDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
	    ...
	    html+=HU.openTag(TAG_TABLE,[ATTR_CLASS,HU.classes(CLASS_TABLE,CLASS_TABLE_STRIPE,CLASS_TABLE_ROWBORDER),
					ATTR_ID,this.domId(ID_RECORDS)]);
	    this.idToRecord = {};
	    this.csv =[];
	    if(labelField) {
		let row = [];
		...
		html+=HU.open(TAG_THEAD);
		row.push("Field");
		html+=HU.th([ATTR_ALIGN,ALIGN_CENTER],'Field');

		records.forEach(record=>{
		    this.idToRecord[record.getId()] = record;
		    let v = labelField.getValue(record);
		    row.push(v);
		    let div = HU.div([ATTR_CLASS,HU.classes(CLASS_CLICKABLE,'display-selectedrecords-header'),
				      ATTR_TITLE,'Click to remove',
				      'data-record-id',record.getId()],
				     v);
		    html+=HU.th([ATTR_ALIGN,ALIGN_CENTER],div);
		});
		html+=HU.close(TAG_THEAD);
	    }
	    html+=HU.open(TAG_TBODY);
		...
		records.forEach((record,idx) =>{
		    ...
		    row.push(v);
		    if(!isNaN(v)) attrs.push(ATTR_ALIGN,ALIGN_RIGHT);
		    else if(String(v)==="NaN") {
			v="--";
			attrs.push(ATTR_ALIGN,ALIGN_RIGHT);			
		    }			
		    html+=HU.td(attrs,v);
		});
		html+=HU.close(TAG_TR);		
...
function RamaddaDotstackDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
		...
		grecords.forEach(r=>{
			...
			[ATTR_TITLE,"",
			 ATTR_RECORD_ID, r.getId(),
			 ATTR_RECORD_INDEX,idToIndex[r.getId()],
			 ATTR_CLASS, "display-dotstack-dot",
...
function RamaddaDotbarDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
		...
		html+=HU.div([ATTR_STYLE,HU.css(CSS_POSITION,POSITION_ABSOLUTE,
						...
						CSS_RIGHT,HU.px(0),
						CSS_TOP,HU.perc(50),
						CSS_BORDER_TOP,HU.border(1,COLOR_LIGHT_GRAY))]);
		html+=SPACE;
		records.forEach((r,idx2)=>{
			...
			if(!selected) {
			    ...
			    dotBorder = HU.border(1,COLOR_BLACK);
			}
		    }
		    if(selected) {
			selectedRecord = r;
		    ...
		    html +=  HU.span([ATTR_RECORD_INDEX,idx2,
				      ATTR_RECORD_ID, r.getId(),
				      ATTR_CLASS,clazz,
		...
		let idx = $(this).attr(ATTR_RECORD_INDEX);
		dots.removeClass("display-dotbar-dot-highlight");
		_this.find("[" + ATTR_RECORD_INDEX+"=\"" + idx+"\"]").addClass( "display-dotbar-dot-highlight");
	    });
	    dots.click(function() {
		let idx = $(this).attr(ATTR_RECORD_INDEX);
		...
		_this.find("[" + ATTR_RECORD_INDEX+"=\"" + idx+"\"]").addClass( "display-dotbar-dot-select");
		_this.hadClick = true;
		_this.propagateEventRecordSelection({record: record});
		_this.updateUI();
...
function RamaddaDategridDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
	    ...
	    let height = HU.em(1.5);
	    html=HU.open(TAG_DIV,[ATTR_CLASS,'display-dategrid-table']) +
		HU.open(TAG_TABLE,[ATTR_WIDTH,HU.perc(100),
				   ATTR_BORDER,0,
				   ATTR_CELLPADDING,0,
				   ATTR_CELLSPACING,0]);
	    html+=HU.open(TAG_TR) + HU.tag(TAG_TD,[ATTR_WIDTH,leftWidth],
					   HU.div([ATTR_CLASS,"display-dategrid-header"],
						  this.getLeftLabel(this.getFieldLabel(groupField))));
	    ...
	    let dateHeader = HU.open(TAG_DIV,[ATTR_CLASS,'display-dategrid-dateheader',
					      ATTR_STYLE,dateHeaderStyle]) + SPACE;
	    let date  = minDate;
		...
		dateHeader+=HU.div([ATTR_CLASS,HU.classes('display-dategrid-header','display-dategrid-date'),
				    ATTR_STYLE,style],this.formatDate(date))+"\n";
		...
		html+=HU.tag(TAG_TD,[ATTR_WIDTH,rightWidth],
			     HU.div([ATTR_CLASS,HU.classes('display-dategrid-header','display-dategrid-stats')],
				    this.getRightLabel(Utils.join(dflt,"/"))));
	    }
	    html +=HU.close(TAG_TR);
	    Object.keys(cats).sort(v=>{
		let cat = cats[v];
		let row = HU.open(TAG_DIV,[ATTR_CLASS,"display-dategrid-row",
		...
		for(let i=0;i<sorted.length;i++) {
		    ...
		    row+=HU.div(["foo","bar",
				 ATTR_RECORD_ID,r.getId(),
				 ATTR_CLASS,"display-dategrid-box",
				 ATTR_TITLE,cv,
				 ATTR_STYLE,HU.css(CSS_LEFT,perc,
						   CSS_RIGHT,right,
						   CSS_HEIGHT,height,
						   CSS_BACKGROUND,color)+boxStyle],SPACE);
		}
		row+=HU.close(TAG_DIV);
		html+=HU.open(TAG_TR)+HU.tag(TAG_TD,[ATTR_WIDTH,leftWidth],
					     HU.div([ATTR_STYLE,leftStyle,
						     ATTR_CLASS,"display-dategrid-rowlabel"], v))+
		    HU.tag(TAG_TD,[],row);
		if(showStats) {
		    let stats = [];
		    ...
		    html+=HU.td([ATTR_NOWRAP,"true"],HU.div([ATTR_STYLE, rightStyle,
							     ATTR_CLASS,"display-dategrid-stats"],
							    Utils.join(stats,SPACE)));
		}
		html+=HU.close(TAG_TR);
	    });
	    html += HU.close(TAG_TABLE,TAG_DIV);
		...
		boxMap[$(this).attr(ATTR_RECORD_ID)] = $(this);
	    });
...
function RamaddaStripesDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
	    ...
	    let html = HU.open(TAG_CENTER) + HU.open(TAG_DIV,[ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK)]);
	    let colorBys = [];
		...
		if(this.getShowLabel()) {
		    left  = HU.div([ATTR_STYLE,HU.css(CSS_MARGIN_LEFT,HU.em(2))], left);
		}
		let center = HU.div([ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER)],
				    this.formatDate(allRecords[parseInt(allRecords.length/2)].getDate()));
		...
		fields.forEach((field,fidx)=>{
		    ...
		    html+=HU.open(TAG_TABLE,[ATTR_ID,uid,
