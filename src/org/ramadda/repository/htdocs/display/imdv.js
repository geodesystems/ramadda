...
function RamaddaImdvDisplay(displayManager, id, properties) {
	...
	finalize: function(cancel) {
		...
		let box = this.display.getNewFeature();
                box.style.strokeColor = COLOR_TRANSPARENT;
	...
	move: function(evt) {
	    ...
	    if(!this.image) {
		this.image=  this.display.getMap().addImageLayer('IMDV Image','IMDV Image','',this.style.imageUrl,
								 true,  b.top,b.left,b.bottom,b.right);
	    } else {
    ...
    const ID_MAP_TOGGLE_ON  ='toggleon';
    const ID_MAP_TOGGLE_OFF  ='toggleoff';        
	...
	handleNewRoute:function(cmd,pts) {
	    ...
	    let buttons  =HU.div([ATTR_CLASS,HU.classes(CLASS_BUTTON_OK,CLASS_DISPLAY_BUTTON)],
				 LABEL_OK) + SPACE2 +
		HU.div([ATTR_CLASS,HU.classes(CLASS_BUTTON_CANCEL,CLASS_DISPLAY_BUTTON)],
		       LABEL_CANCEL);	    
	    html+=HU.div([ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_RIGHT,CSS_MARGIN_TOP,HU.px(5))],
			 buttons);
	...
	createRouteForm:function(addSequence) {
		...
		html+=HU.formEntryLabel('Provider',
					HU.select('',[ATTR_ID,this.domId('routeprovider')],
						  providers,this.routeProvider));
		html+=HU.formEntryLabel('Route Type' ,
					HU.select('',[ATTR_ID,this.domId('routetype')],
						  [ROUTE_CAR,ROUTE_BICYCLE,ROUTE_PEDESTRIAN],
						  this.routeType));
		if(addSequence) {
	...
	addIsolineAt:function(center,lon) {
	    ...
	    let buttons  =HU.div([ATTR_CLASS, HU.classes(CLASS_BUTTON_OK,CLASS_DISPLAY_BUTTON)], LABEL_OK) + SPACE2 +
		HU.div([ATTR_CLASS, HU.classes(CLASS_BUTTON_CANCEL,CLASS_DISPLAY_BUTTON)], LABEL_CANCEL);	    
	    html+=HU.div([ATTR_STYLE, HU.css(CSS_TEXT_ALIGN,ALIGN_RIGHT,CSS_MARGIN_TOP,HU.px(5))], buttons);
	    html=HU.div([ATTR_STYLE, HU.css(CSS_MARGIN,HU.px(5))],html);
	...
	gotoAddress:function(widget,address) {
		...
		clear();
			...
			html+=HU.div([ATTR_INDEX,idx,
				      ATTR_CLASS,HU.classes(CLASS_CLICKABLE,CLASS_MENU_ITEM)],loc.name);
		    });
	...
	makeRangeRings:function(center,radii,style,angle,ringStyle,labels) {
		...
		let ring = OpenLayers.Geometry.Polygon.createRegularPolygon({x:p1.lon,y:p1.lat},
									    dist, 100,0);
		let _style = $.extend({},style??{strokeWidth:2,strokeColor:'blue',fillColor:COLOR_TRANSPARENT});
		if(ringStyle['*']) {
	...
	setCommandCursor: function(cursor) {
	    this.getMap().setCursor(cursor??CURSOR_POINTER);
	},
	...
	initGlyphCommand:function(glyphType, cmd,args) {
		...
		let wmtHtml = '';
		wmtHtml+=HU.br();
		wmtHtml+=HU.br()+'Or enter either a TMS/WMS server URL with a layer name:';
		let form = (label,name,size)=>{
		    wmtHtml+=HU.formEntryLabel(label,HU.input('',this.cache[name]??'',
							      [ATTR_ID,this.domId(name),ATTR_SIZE,size??60]));
		}
		let args = [
		    ['Name','servername'],
		...
		wmtHtml+=HU.close(TAG_TABLE);
		wmtHtml+=HU.br()+'Or enter Tile JSON URL:';
		wmtHtml +=  HU.formTable();
		...
		let ok = ()=>{
			...
			$.getJSON(tileJson, (data)=> {
				...
				args.legendText = HU.href(data.attribution_link,"Courtesy: "+ data.attribution,
							  [ATTR_TARGET, '_other']);
			    }
			    if(data.bounds) {
	...
	initOSMSearch:function() {
	    ...
	    let buttons =Utils.join([HU.div([ATTR_CLASS,HU.classes(CLASS_BUTTON_OK,CLASS_BUTTON)], 'Search'),
				     ...
				     HU.div([ATTR_CLASS,HU.classes(CLASS_BUTTON_CANCEL,CLASS_BUTTON)], 'Close')],
				    SPACE1);
	    ...
	    this.osm.dialog.find(HU.dotClass(CLASS_BUTTON_CANCEL)).button().click(function() {
	    ...
	    this.osm.dialog.find(HU.dotClass(CLASS_BUTTON_OK)).button().click(() =>{
		this.doOSMSearch();
	    });
	},
	...
	handleOSMResults: function(data,limit) {
		...
		if(e.tags) {
			...
			popup+=HU.formEntryLabel(label,v);
		    });
		}
		popup += HU.close(TAG_TABLE);
	...
	initStac:function(dialog) {
		...
		let plus = HU.span([ATTR_ID,this.domId('stac_add'),
				    ATTR_CLASS,CLASS_CLICKABLE,
				    ATTR_TITLE,'Add a STAC catalog URL'],HU.getIconImage('fas fa-plus'));
		let back  = HU.span([ATTR_ID,this.domId('stac_back'),
				     ATTR_CLASS,CLASS_CLICKABLE,
		...
		let html = '';
		let title = (data.title??baseUrl)+HU.space(1) +
		    HU.href(baseUrl,HU.getIconImage(ICON_LINK,[],[ATTR_STYLE,HU.css(CSS_FONT_SIZE,HU.pt(9))]),
			    [ATTR_TARGET,'_stac']);
		...
		if(data.links) {
			...
			if(!label)
			    label  = url.replace(/.*\/([^\/]+$)/,"$1");
			let href = HU.href(url,HU.getIconImage(ICON_LINK,[],
			...
			if(!isJson) {
			    linksHtml2+=HU.tr(HU.td([ATTR_WIDTH,HU.perc(10),ATTR_NOWRAP,'true'],'')+HU.td(href));
			} else {
			    linksHtml1+=HU.tr(HU.td([ATTR_WIDTH,HU.perc(10),
						     ATTR_NOWRAP,'true'],
						    HU.div([ATTR_TITLE,url,
							    ATTR_CLASS,'imdv-stac-item'],
							   HU.span(['link',url,
								    ATTR_CLASS,'imdv-stac-link'], 'Load'))) +
					      HU.td(href));
			}
		...
		if(data.assets) {
			...
			if(table=='') {
			    table+=HU.b('Assets: ')+'note: the IMDV does not handle all projections. Try the thumbnail' + HU.br();
			...
			let link = HU.href(assetUrl,HU.getIconImage(ICON_LINK,[],
								    [ATTR_STYLE,HU.css(CSS_FONT_SIZE,HU.pt(9))])+
					   ' ' +label +' ('+ asset.type+')',
					   [ATTR_TITLE,assetUrl,
			...
			if(isImage) {
			    images+=HU.tr(HU.td([ATTR_WIDTH,HU.perc(10),
						 ATTR_NOWRAP,'true'],
						HU.div([ATTR_CLASS,'imdv-stac-item'],
						       HU.span(['asset-id',key,ATTR_CLASS,'imdv-stac-asset'],
							       'Add Image')))+
					  HU.td(link));
			} else {
		...
		if(linksHtml!='') {
		    ...
		    if(assetsHtml!='') 
			html+=HU.div([ATTR_STYLE,HU.css(CSS_MAX_HEIGHT,HU.em(5),CSS_OVERFLOW_Y,OVERFLOW_AUTO)],
				     linksHtml);
		    else
			html+= linksHtml;		    
		}
	...
	initGlyphButtons:function(dom) {
		...
		let id  = $(this).attr(ATTR_GLYPH_ID);
		...
		if(command=='toback') _this.changeOrder(false,mapGlyph);
			...
			let dontAsk = HU.checkbox(_this.domId('dontask'),
						  [ATTR_ID,_this.domId('dontask'),
	...
	makeGlyphButtons:function(mapGlyph,includeEdit,debug) {
		...
		buttons.push(HU.span([ATTR_CLASS,CLASS_CLICKABLE,
				      ATTR_TITLE,'Delete',
				      ATTR_GLYPH_ID,mapGlyph.getId(),
				      ATTR_BUTTON_COMMAND,ID_DELETE],icon(ICON_ERASER)));

		if(mapGlyph.isMarker() && this.isIsolineEnabled()) {
		    buttons.push(HU.span([ATTR_CLASS,CLASS_CLICKABLE,
					  ATTR_TITLE,'Add Isoline',
					  ATTR_GLYPH_ID,mapGlyph.getId(),
					  ATTR_BUTTON_COMMAND,"addisoline"],icon('fa-regular fa-circle-dot')));
		}
	    }
	    if(buttons.length==0) return '';
	    let attrs='';
	    let bar =  Utils.wrap(buttons,HU.open(TAG_SPAN,attrs),HU.close(TAG_SPAN));
	    return bar;
	},
	makeListItem:function(mapGlyph,idx) {
	    ...
	    line+= HU.td([ATTR_GLYPH_ID,mapGlyph.getId(),
	...
	createData:function(mapOptions) {
		...
		let msg = "";
		if(!match) {
		    msg = HU.div([],"There doesn't seem to be any map displays defined for this entry");
		}
		if(match) {
		    inner = match[1];
		    inner = inner.replace(/\n/g," ");
		...
		let skip = ['hereRoutingEnabled','googleRoutingEnabled','canEdit',
			    'title','bounds','zoomLevel','mapCenter',
			    'width','height','user','entryIcon','entryId','thisEntryType','fileUrl','popupWidth','popupHeight','divid'];
		let userInput = "";
		for(key in attrs) {
		...
		let widget =  msg+HU.textarea("",userInput,[ATTR_ID,this.domId('displayattrs'),
							    ATTR_ROWS,10,
							    ATTR_COLS, 60]);
		...
		widget = HU.div([ATTR_STYLE,HU.css(CSS_MARGIN,HU.px(4))],
				widget+HU.br()+andZoom + buttons);
		
		let dialog =  HU.makeDialog({content:widget,
					     anchor:this.jq(ID_MENU_FILE),
					     title:"Map Display Attributes",
					     header:true,
					     draggable:true,
					     remove:false});
	...
	listFeatures:function() {
	    ...
	    html+=HU.open(TAG_CENTER);
	    html +=HU.div([ATTR_ID,this.domId(ID_LIST_DELETE),
			   ATTR_CLASS,CLASS_DISPLAY_BUTTON], 'Delete Selected');
	    html += SPACE2;	    
	    html +=HU.div([ATTR_ID,this.domId(ID_LIST_CANCEL),
			   ATTR_CLASS,CLASS_DISPLAY_BUTTON], 'Close');	    
	    html+=HU.close(TAG_CENTER);
	    html  = HU.div([ATTR_CLASS,'wiki-editor-popup'], html);
	    let dialog = this.listDialog  =
	...
	doEdit: function(mapGlyph) {
	    ...
	    let html =HU.div([ATTR_STYLE,HU.css(CSS_MARGIN,HU.px(8))],
			     'Feature: ' + mapGlyph.type); 
	...
	makeStyleForm:function(style,mapGlyph,args) {
	    args = args??{};
	    let html = "";
	    let props;
	    let values = {};
	    ...
	    let isGroup = mapGlyph?mapGlyph.isGroup():args.isGroup;
	    if(style) {
		if(mapGlyph) {
		...
		if(header) {
		    strip=header.strip;
		    html+=HU.tr([],HU.td([ATTR_COLSPAN,2],HU.div([ATTR_CLASS,'imdv-form-header'],header.label)));
		}  else if(strip && !prop.startsWith(strip)) {
		    html+=HU.tr([],HU.td([ATTR_COLSPAN,2],''));
		    strip = null;
		}
		let label = prop;
		...
		if(prop=='externalGraphic' || prop.indexOf('ExternalGraphic')>=0 || prop=='childIcon') {
			...
			HU.open(TAG_TABLE) + HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_TOP]) +
			HU.open(TAG_TD,[ATTR_WIDTH,HU.perc(1)]) +
			HU.image(graphic,[ATTR_WIDTH,HU.px(24),ATTR_ID,this.domId(prop+'_image')]) +
			HU.close(TAG_TD) + HU.open(TAG_TD) +
			div +HU.close(TAG_TD, TAG_TR, TAG_TABLE);
		} else {
		    let v = values[prop];
			...
			widget =  HU.textarea("",v,[ATTR_ID,domId,ATTR_ROWS,3,ATTR_COLS, 60]);
			if(mapGlyph.isEntry()) {
			    widget+=HU.br() +HU.checkbox(this.domId("useentrylabel"),[],mapGlyph.getUseEntryLabel(),"Use label from entry");
			}
			...
			widget =  HU.textarea("",v||"",[ATTR_ID,domId,ATTR_ROWS,5,ATTR_COLS, 60]);
			if(icons.length>0) {
			    let hdr = HU.boldLabel("Add icon");
			    icons.forEach(icon=>{
			...
			if(prop=="textBackgroundFillOpacity" || prop=="textBackgroundPadding" || prop=="strokeWidth" || prop=="pointRadius" || prop=="fontSize" || prop=="imageOpacity" || prop=='dotSize') size="4";
			    ...
			    size='60';
			    rows=5;
			    extra = HU.pre([],'property=value...');
			} else if(prop=='imagefilter') {
			    label = 'Image Filter'
			    size='60';
			    extra=HU.br()+'e.g.:<pre>contrast(200%) grayscale(80%) brightness(0.4)\ninvert(75%) saturate(30%) sepia(60%)</pre>';
			    extra += HU.href('https://developer.mozilla.org/en-US/docs/Web/CSS/filter','Help',
					     [ATTR_TARGET,'_help']);
			} else if(prop=='clippath') {
			    label='Image Clip Path';
			    size='60';
			    extra = HU.br() + HU.span([ATTR_ID,'clippathdraw',
						       ATTR_CLASS,CLASS_CLICKABLE,
						       ATTR_TITLE,'Draw polygon'],HU.getIconImage('fas fa-draw-polygon')+' clip the image') +', e.g.,<pre>polygon(x1 y1,x2 y2,x3 y3,x4 y4)\n'+
				'e.g., to clip 5% from left, 18% from top and 10% from right do:\n'+
				'polygon(5% 18%,95% 18%,95% 90%,5% 90%)'+
				'</pre>';
			    extra += HU.href('https://developer.mozilla.org/en-US/docs/Web/CSS/clip-path','Help',
					     [ATTR_TARGET,'_help']);
			    ...
			    extra += HU.href('https://developer.mozilla.org/en-US/docs/Web/CSS/transform','Help',
					     [ATTR_TARGET,'_help']);
			}
			if(prop.indexOf("Color")>=0) {
			    ...
			    */
			    ...
			    /*			    widget =  HU.div([ATTR_ID,domId+'_display',ATTR_CLASS,'ramadda-dot',
						    ...
						    HU.space(2)+widget;
						    //			    widget  = HU.table([ATTR_CELLPADDING,'0',ATTR_CELLSPACING,'0'],HU.tr([ATTR_VALIGN,ALIGN_TOP],HU.tds([],[widget,bar])));
						    */
			    ...
			    widget =  HU.input("",v,[ATTR_ID,domId,ATTR_SIZE,size])+HU.space(4) +
				
			    HU.div([ATTR_SLIDER_MIN,min,
				    ATTR_SLIDER_MAX,max,
				    ATTR_SLIDER_STEP,1,
				    ATTR_SLIDER_VALUE,v,
				    'slider-id',domId,
			    ...
			    if(!v || v=="") v= 1;
			    widget =  HU.input("",v,[ATTR_ID,domId,ATTR_SIZE,4])+HU.space(4) +
			    ...
			    widget =
				HU.textarea("",v,[ATTR_ID,this.domId(id),ATTR_ROWS,rows,ATTR_COLS,size]);

			} else {
			    widget =  HU.input("",v,[ATTR_ID,this.domId(id),ATTR_SIZE,size]);
			}
		    }
		}
		let suffix='';
		...
		html+=HU.formEntryLabel(label,widget+suffix+extra);
		html+='\n';
	    });
	    html+=HU.close(TAG_TABLE);
	...
	doProperties: function(style, apply,mapGlyph,_anchor) {
		...
		let textarea = HU.textarea("",attrs,[ATTR_ID,this.domId('displayattrs'),
						     ATTR_ROWS,15,
						     ATTR_COLS, 60]);
		...
		if(prop && value) {
			...
			mapGlyph.applyStyleToChildren(prop,value,true);
		    });
		}
	    });
		...
		dialog.find('#clippathdraw').button().click(function(){
		    ...
		    let html = HU.div([ATTR_ID,_this.domId('clippath_container'),
				      ...
				      HU.image(mapGlyph.style.imageUrl,
						...
						ATTR_STYLE,HU.css(CSS_CURSOR,CURSOR_POINTER,
								  CSS_BORDER,HU.border(1,COLOR_LIGHT_GRAY))]));
		    let buttons = HU.buttons([HU.div([ATTR_CLASS,HU.classes(CLASS_BUTTON_OK,CLASS_DISPLAY_BUTTON)], LABEL_OK),
					      HU.div([ATTR_CLASS,HU.classes(CLASS_BUTTON_CANCEL,CLASS_DISPLAY_BUTTON)], LABEL_CANCEL)]);
			...
			HU.span([ATTR_CLASS,CLASS_CLICKABLE,
				...
				'Try') +HU.space(1) +
			HU.input('','',[ATTR_CLASS,'pathoutput',
					ATTR_SIZE,60,
					ATTR_STYLE,HU.css(CSS_MARGIN_BOTTOM,HU.em(0.5))]) +
			HU.br()+html;
		    html = HU.div([ATTR_STYLE,HU.css(CSS_MARGIN,HU.px(10),
						     CSS_TEXT_ALIGN,ALIGN_CENTER)],html);
		    let path='';
	    ...
	    dialog.find(HU.dotClass(CLASS_SLIDER)).each(function() {
		let min = $(this).attr(ATTR_SLIDER_MIN);
		let max = $(this).attr(ATTR_SLIDER_MAX);
		let step = $(this).attr(ATTR_SLIDER_STEP)??0.01;
		$(this).slider({		
		    min: +min,
		    max: +max,
		    step:+step,
	...
	makeBox: function(type) {
	    ...
	    html+=HU.center(HU.input('',r.n,[ATTR_STYLE, style,
					     ATTR_ID,this.domId('_north'),
					     ATTR_SIZE,6,
					     ATTR_PLACEHOLDER,'North']));
	    html+=HU.input('',r.w,[ATTR_STYLE, style,
				   ATTR_ID,this.domId('_west'),
				   ATTR_SIZE,6,
				   ATTR_PLACEHOLDER,'West']);
	    html+=HU.input('',r.e,[ATTR_STYLE, style,
				   ATTR_ID,this.domId('_east'),
				   ATTR_SIZE,6,
				   ATTR_PLACEHOLDER,'East']);
	    html+=HU.center(HU.input('',r.s,[ATTR_STYLE, style,
					     ATTR_ID,this.domId('_south'),
					     ATTR_SIZE,6,
					     ATTR_PLACEHOLDER,'South']));
	...
	doMapProperties:function() {
	    ...
	    right+=HU.formEntryLabel('Legend width',
				     HU.input('',this.getMapProperty('legendWidth',''),
					      [ATTR_ID,this.domId('legendwidth'),
					       ATTR_SIZE,'4']));
	...
	showFileMenu: function(button) {
	    ...
	    html+=div;
	    html+= HU.href(Ramadda.getUrl('/userguide/imdv/index.html'),'Help',[ATTR_TARGET,'_help']);
	...
	initViewMenu:function(suffix,anchor) {
	    ...
	    this.jq(ID_MAP_TOGGLE_OFF+suffix).click(()=>{
		this.glyphs.forEach((mapGlyph,idx)=>{
		    mapGlyph.setVisible(false,true);
		});
	    });
	    this.jq(ID_MAP_TOGGLE_ON+suffix).click(()=>{
		this.glyphs.forEach((mapGlyph,idx)=>{
	...
	getDecoration:function(style) {
	    ...
	    return  HU.div([ATTR_STYLE,HU.css(CSS_MARGIN_BOTTOM,HU.px(5),
					      ...
					      CSS_DISPLAY,DISPLAY_INLINE_BLOCK)+dim + cssStyle]);
	},
	...
	showMapLegend: function() {
		...
		html = HU.div([ATTR_STYLE,HU.css(CSS_PADDING,HU.px(5),
						 ...
						 CSS_BACKGROUND_COLOR,COLOR_MELLOW_YELLOW)], html);
	    }
	...
	showNewMenu: function(button) {
	    ...
	    html+=HU.close(TAG_TD,TAG_TR, TAG_TABLE);
	    html  = this.makeMenu(html);
	    this.dialog = HU.makeDialog({content:html,anchor:button});
	    this.glyphTypes.forEach(g=>{
	...
	handleError:function(err,url) {
	    ...
	    err = message??err;
	    if(url) err="Error loading URL:" + url+HU.br()+err;
	...
	makeLegendDroppable:function(droppedOnGlyph,label,notify) {
		...
		drop: (event,ui)=>{
		    notify();
		    let draggedGlyph = this.findGlyph(ui.draggable.attr(ATTR_GLYPH_ID));
		    if(!draggedGlyph) {
	...
	makeLegend: function() {
	    ...
	    if(glyphs.length)
		html+=HU.div([ATTR_ID,this.domId(ID_DROP_END),
			      ATTR_CLASS,CLASS_LEGEND_ITEM,
			      ATTR_STYLE,HU.css(CSS_WIDTH,HU.perc(100),CSS_HEIGHT,HU.em(1))],'');
		...
		top+=HU.div([ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER,CSS_MARGIN_BOTTOM,HU.px(2))],
			    this.getBaseLayersSelect());
	    }
	    if(showViewInLegend) {
		top +=HU.center(this.makeViewMenu(false,'_legend',(id,lbl)=>{
		    return HU.span([ATTR_STYLE,HU.css(CSS_MARGIN_RIGHT,HU.px(6)),
				    ATTR_ID,id,
				    ATTR_CLASS,CLASS_CLICKABLE], lbl);}));
	    }
	    html=top+html;
		...
		content: function () {
		    ...
		    title=HU.div([ATTR_STYLE,HU.css(CSS_FONT_SIZE,HU.pt(10))],title);
		    return title;
		},
		show: {
		    delay: 1000,
	...
	canChange: function() {
	    ...
	    */
	    ...
	    let message3 = HU.div([ATTR_ID,this.domId(ID_MESSAGE3),
				   ATTR_CLASS,'ramadda-imdv-message3'],'');
	    if(this.getShowMapLegend()) {
		this.jq(ID_MAP_CONTAINER).append(message3);
	    }

	    let labels = HU.div([ATTR_ID,this.domId(ID_GLYPH_LABELS),
				 ATTR_CLASS,'imdv-inmap-labels'],'');
	...
	makeMenuBar:function() {
		...
		HU.span([ATTR_STYLE,HU.css(CSS_POSITION,POSITION_RELATIVE)], 
			HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK),
				...
				ATTR_TITLE,'Clear',
				ATTR_CLASS,CLASS_CLICKABLE],
			       HU.getIconImage(ICON_ERASER,[],[ATTR_STYLE,HU.css(CSS_COLOR,COLOR_LIGHT_GRAY)])) +
			' ' +
		...
		address = address +' ' +HU.checkbox(this.domId(ID_ADDRESS_ADD),
						    [ATTR_ID,this.domId(ID_ADDRESS_ADD),
		...
		menuBar=  HU.table([ATTR_ID,this.domId(ID_MAP_MENUBAR),
				   ...
				   HU.tr([ATTR_VALIGN,ALIGN_BOTTOM],HU.td([],menuBar) +
		...
		menuBar= HU.table([ATTR_ID,this.domId(ID_MAP_MENUBAR),
				   ATTR_WIDTH,HU.perc(100)],
				  HU.tr([ATTR_VALIGN,ALIGN_BOTTOM],HU.td([],'') +
					HU.td([ATTR_WIDTH,HU.perc(50)], message) +
					HU.td([ATTR_ALIGN,ALIGN_RIGHT,
					       ATTR_STYLE,HU.css(CSS_PADDING_RIGHT,HU.px(10)),
					       ATTR_WIDTH,HU.perc(50)],mapHeader+address)));
	...
	createMapLegendWrapper:function() {
	    ...
	    let innerDiv = HU.div([ATTR_ID,this.domId(ID_LEGEND_MAP_WRAPPER),
				   ATTR_CLASS,'imdv-legend-map-wrapper',
				   ATTR_STYLE,legendStyle]);
	    let inner = $(innerDiv);
		...
		stop:function() {
		    ...
		    pos.left = inner.css(CSS_LEFT);		    
		    pos.top = inner.css(CSS_TOP);
		    return
		    /* TODO?
		       let set = (which,v) =>{
	...
	makeControls:function() {
	    ...
	    this.jq(ID_MAP).css('caret-color',COLOR_TRANSPARENT);
		...
		dragVertex: function(vertex, pixel) {
			...
			if(mapGlyph.isImage()) {
			    this.feature.style={strokeColor:COLOR_TRANSPARENT,fillColor:COLOR_TRANSPARENT};
...
window.olGetPatternId = function(ol,p,stroke,fill) {
    ...
    patternNode.setAttributeNS(null, ATTR_WIDTH,p.width);
    patternNode.setAttributeNS(null, ATTR_HEIGHT,p.height);
...
function olCheckLabelBackground(renderer,   style,label,featureId,bbox) {
	...
	let bgStyle = "";
	bgStyle+="fill:" +((style.textBackgroundFillColor=='' || !style.textBackgroundFillColor)?COLOR_TRANSPARENT:style.textBackgroundFillColor)+";";
