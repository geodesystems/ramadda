...
var CLASS_ANIM_BUTTON  = 'display-anim-button';
...
function RamaddaBaseMapDisplay(displayManager, id, type,  properties) {
	...
	{p:'boundsFillColor',d:COLOR_TRANSPARENT},
	...
	extraLayers:[],
		...
		$( document ).ready(()=> {
            ...
            let html = "";
            let extraStyle="";
            let height = this.getProperty("height", this.getProperty("mapHeight", HU.vh(70)));
            if (height < 0) {
	    ...
	    let map =HU.div([ATTR_TABINDEX,1,
			     ...
			     ATTR_ID, this.domId(ID_MAP)]);

	    let mapContainer = HU.div([ATTR_CLASS,"ramadda-map-container",
				       ATTR_ID,this.domId(ID_MAP_CONTAINER)],
				      map+
				      HU.div([ATTR_CLASS,"ramadda-map-slider",
					      ATTR_STYLE,this.getProperty("popupSliderStyle",
									  HU.css(CSS_MAX_HEIGHT,HU.px(400),
	...
	initRegionsSelector:function(button) {
	    ...
	    html = HU.div([ATTR_CLASS, CLASS_POPUP_INNER,
			   ATTR_ID,id,
			   ATTR_STYLE,HU.css(CSS_MARGIN,HU.em(1),
	...
	makeRegionsMenu:function() {
		...
		let item = HU.div([ATTR_CLASS,HU.classes(CLASS_MENU_ITEM,'ramadda-region-item'),
				   ATTR_IDX,idx],name);
		if(!groups[group]) groups[group] = "";
		groups[group] +=item;});
	    let html ='';
	    html+= HU.center(HU.input('','',[ATTR_PLACEHOLDER,'Find Region',
					     ATTR_ID,this.domId('regionsearch'),
					     ATTR_WIDTH,10]));
	    html += HU.open(TAG_TABLE,[ATTR_WIDTH,HU.perc(100)])+HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_TOP]);
	    let keys = Object.keys(groups);
	    let width = HU.perc((100/keys.length));
	    keys.forEach(group=>{
	...
	getBaseLayersSelect:function() {
		...
		if(prefix) prefix=HU.boldLabel(prefix);
	...
	initMap:function(map) {
		...
		this.map.popupListener = (id,text) =>{
			...
			let value = $(this).attr(ATTR_FIELD_VALUE);			
	...
	addExtraLayers:function(extras) {
		...
		if(type=="baselayer") {
		    ...
		    if(!args.fillColor) args.fillColor=COLOR_TRANSPARENT;
...
function RamaddaMapDisplay(displayManager, id, properties) {
	...
	{p:'collisionDotColor',d:COLOR_WHITE,tt:'Color of dot drawn at center'},
	{p:'collisionDotOpacity',d:'0.9',tt:'Opacity of dot drawn at center'},	
	{p:'collisionRingColor',d:COLOR_BLACK,tt:'Color of ring'},
	...
	{p:'collisionLabelColor',d:COLOR_BLACK},
	...
	{p:'labelFontColor',ex:COLOR_BLACK},
	{p:'labelFontSize',ex:HU.px(12)},
	...
	{p:'labelStrokeColor',ex:COLOR_BLACK},
	...
	{p:'hexmapFillColor',d:COLOR_TRANSPARENT},
	...
	{p:'voronoiFillColor',d:COLOR_TRANSPARENT},
	...
	getRecordUrlHtml: function(attrs, field, record) {
	    ...
	    return  HU.span([ATTR_CLASS,CLASS_CLICKABLE,
			     ATTR_ID,this.domId(ID_TRACK_VIEW+"_1")],label);
	},
	removeTrack:function(record) {
	    ...
	    let item = this.jq(ID_LEFT).find(HU.attrSelect(ATTR_RECORD_ID,record.getId()));
	...
	loadTrack: function(record, data) {
		...
		fillColor:this.getProperty("fillColor") ||COLOR_TRANSPARENT,
	    };
	...
	initMap: function(map) {
		...
		this.getDisplayEntry(entry=>{
			...
			entries.forEach(entry=>{
			    ...
			    if(entry.getIsImage()) {
				html+=HU.br() + HU.image(entry.getImageUrl(),[ATTR_WIDTH,HU.perc(100)]);
			    }
	...
	toggleVectorLayer: function() {
	    ...
	    if(debugPopup) this.logMsg("handleLayerSelect");
            ...
            this.getEntryList().showMessage("Searching",
					    HU.div([ATTR_STYLE, HU.css(CSS_MARGIN,HU.px(20))],
						   this.getWaitImage()));
        },
		...
		style+=HU.css(CSS_DISPLAY,DISPLAY_NONE);
            ...
            let html =  HU.div([ATTR_STYLE,style,
				ATTR_CLASS, "display-inner-contents",
	...
	getHeader2:function() {
	    ...
	    html += HU.div([ATTR_CLASS,CLASS_HEADER_SPAN,
			    ATTR_ID,this.domId("locations")]);
	    return html;
	},
	locationMenuCnt:0,
	addLocationMenu:function(url,data) {
		...
		locations.forEach((loc,idx)=>{
			...
			inner+=HU.div([ATTR_CLASS,
				       HU.classes(CLASS_CLICKABLE,CLASS_HOVERABLE,'display-map-location'),
				       ATTR_NORTH,loc.north,ATTR_WEST,loc.west,
				       ATTR_SOUTH,loc.south,ATTR_EAST,loc.east], loc.name);
		...
		dialog.find(HU.dotClass(CLASS_CLICKABLE)).click(function() {
		    ...
		    if(Utils.isDefined($(this).attr(ATTR_LONGITUDE))) {
			let point = MapUtils.createLonLat(+$(this).attr(ATTR_LONGITUDE),+$(this).attr(ATTR_LATITUDE));
			_this.map.zoomTo(9);
			_this.map.setCenter(point);
		    } else if(Utils.isDefined($(this).attr(ATTR_NORTH))) {
			_this.map.setViewToBounds(new RamaddaBounds(+$(this).attr(ATTR_NORTH),
								    +$(this).attr(ATTR_WEST),
								    +$(this).attr(ATTR_SOUTH),
								    +$(this).attr(ATTR_EAST)));
		    } else {
			let geometry = locations[$(this).attr(ATTR_INDEX)].geometry;
...
//		    dialog.remove();
	...
	initHeader2:function() {
		...
		if(!_this.clipToView) {
		    $(this).css(CSS_BORDER,HU.border(1,HU.rgba(0,0,0,0)));
		} else {
		    $(this).css(CSS_BORDER,HU.border(1,'#aaa'));
		}
		_this.haveCalledUpdateUI = false;
		_this.updateUI();
	    });
	...
	makeToc:function(records) {
		...
		if(doTable) {
		    ...
		    fields.forEach(f=>{
			html+=HU.tag(TAG_TD,[ATTR_STYLE,HU.css(CSS_FONT_WEIGHT,FONT_BOLD)],f.getLabel());
		    });
		...
		records.forEach((record,idx)=>{
			...
			if(!iconField) {
			    ...
			    value = HU.getIconImage(iconField.getValue(record,icon_blank16),[ATTR_WIDTH,16]) + SPACE + value;
			}
			if(urlField) {

			    let url = urlField.getValue(record);
			    if(Utils.stringDefined(url)) {
				value=HU.href(url,HU.getIconImage('fas fa-link',null,
								  [ATTR_STYLE,HU.css(CSS_FONT_SIZE,HU.pt(8))]),
					      [ATTR_TARGET,'_link']) + HU.space(1) +value;
			    }
			}
			html+= HU.span([ATTR_TITLE, title,
					ATTR_CLASS,clazz,
					ATTR_RECORD_ID,record.getId(),
					ATTR_RECORD_INDEX,idx], value);
		    }
		});
		if(doTable) {
		...
		let style = HU.css(CSS_HEIGHT,height,CSS_MAX_HEIGHT,height,CSS_OVERFLOW_Y,OVERFLOW_AUTO);
		if(this.getTocWidth()) {
		    style+=HU.css(CSS_MIN_WIDTH,this.getTocWidth());
		}
		html = HU.div([ATTR_CLASS, "display-map-toc",
			       ATTR_STYLE,style,
			       ATTR_ID, this.domId("toc")],html);
	...
	updateUIInner: function(args, pointData, records, debug) {
		...
		header+= HU.span([ATTR_CLASS,HU.classes(CLASS_CLICKABLE,HU.classes('display-header-span',CLASS_MENU_BUTTON)),
	...
	createHeatmap(records, fields, bounds) {
		...
		controls.push(HU.div([ATTR_ID,this.domId(ID_HEATMAP_ANIM_STEP_BACK),
				      ...
				      ATTR_TITLE,"Step back"],
 				     HU.getIconImage("fa-step-backward",
						     [ATTR_CLASS,CLASS_ANIM_BUTTON])));
		...
		controls.push(HU.div([ATTR_ID,this.domId(ID_HEATMAP_ANIM_STEP_FORWARD),
				      ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK),
				      ATTR_TITLE,"Step forward"],
 				     HU.getIconImage("fa-step-forward",
						     [ATTR_CLASS,CLASS_ANIM_BUTTON])));
		...
		controls.push(HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK,
							...
							CSS_MARGIN_RIGHT,HU.px(5))],
				     HU.select("",[ATTR_ID,this.domId(ID_HEATMAP_ANIM_LIST)],labels)));
		this.writeHeader(ID_HEADER2_PREPREFIX, Utils.join(controls,SPACE2));
		let _this = this;
		...
		this.jq(ID_HEATMAP_ANIM_PLAY).click(function() {
		    ...
		    $(this).html(HU.getIconImage(icon,[ATTR_CLASS, CLASS_ANIM_BUTTON]));
		...
		this.jq(ID_HEATMAP_ANIM_STEP_FORWARD).click(function() {
		    ...
		    let icon = _this.heatmapPlayingAnimation?ICON_STOP:ICON_PLAY;
		    _this.jq(ID_HEATMAP_ANIM_PLAY).html(HU.getIconImage(icon,
									[ATTR_CLASS,CLASS_ANIM_BUTTON]));
		    _this.stepHeatmapAnimation(1);
		});
		this.jq(ID_HEATMAP_ANIM_STEP_BACK).click(function() {
		    _this.heatmapPlayingAnimation = false;
		    let icon = _this.heatmapPlayingAnimation?ICON_STOP:ICON_PLAY;
		    _this.jq(ID_HEATMAP_ANIM_PLAY).html(HU.getIconImage(icon,[ATTR_CLASS,CLASS_ANIM_BUTTON]));
		    _this.stepHeatmapAnimation(-1);
		});		
		...
		this.writeHeader(ID_HEADER2_PREFIX,HU.span([ATTR_STYLE,HU.css(CSS_MARGIN_RIGHT,HU.px(8))],toggle));
	...
	updateHtmlLayer:function() {
		...
		$(vp).append(HU.div([ATTR_STYLE,HU.css(CSS_Z_INDEX,10),
				     ATTR_CLASS,'display-map-htmllayer',
				     ATTR_ID,this.htmlLayerId]));
	    }
		...
		let html = 
		    HU.div([ATTR_ID,id,
			    ATTR_CLASS,'display-map-html-item',
		...
		this.htmlPopup +=
		    HU.div([ATTR_ID,hid,
			    ATTR_RECORD_INDEX, idx,
			    ATTR_TITLE,'',
			    ATTR_CLASS,'display-map-html-hitem',
		...
		if(shape == "pie" || shape == "piechart") {
			...
			let pie = HU.tag(TAG_CANVAS,[ATTR_STYLE,HU.css(CSS_CURSOR,CURSOR_POINTER),
						     ATTR_ID,id ,
						     ATTR_WIDTH,cw,
						     ATTR_HEIGHT, ch]);
			if(idx==0)
		...
		fillColor:this.getHexmapFillColor(COLOR_TRANSPARENT),
		...
		fontColor:this.getProperty('hexmapLabelFontColor',COLOR_BLACK),
		fontWeight: this.getProperty('hexmapLabelFontWeight',FONT_BOLD)
	    }
		...
		if(tpoints.length==1) {
		    featuresToAdd.push(this.map.createPoint(ATTR_ID,
...
//				    this.getRecordHtml(f.record, fields, highlightTemplate|| tooltip));
		...
		if(polygonField) {
		    ...
		    polygonProps.fillColor = COLOR_TRANSPARENT;
		...
		let html = "";
		for(a in iconMap) {
		    html+=HU.image(iconMap[a],[ATTR_WIDTH,32]) +" " + a+" ";
		}
		...
		for(v in shapeBy.map) {
		    ...
		    else if(shape=="triangle") shape=HU.getIconImage("/icons/triangle.png",
								     [ATTR_WIDTH,HU.px(16)]);
		    else if(shape=="downtriangle") shape=HU.getIconImage("/icons/downtriangle.png",
									 [ATTR_WIDTH,HU.px(16)]);
		    else if(shape=="lightning") shape=HU.getIconImage("/icons/lightning.png",
								      [ATTR_WIDTH,HU.px(16)]);
		    else if(shape=="cross") shape=HU.getIconImage("/icons/cross.png",
								  [ATTR_WIDTH,HU.px(16)]);		    
	...
	getTextGetter:function(fields,showCount) {
		...
		if(tooltipHeader) {
		    ...
		    text+=HU.boldLabel('Count') + records.length+HU.br();
		}
		...
		records.forEach((record,idx)=>{
		    ...
		    else   if(idx>0) text+=HU.thinLine();
		    text +=   v;
		});
		...
		if(records.length>0 && this.getShowMapInTooltip()) {
		    ...
		    let div  = HU.div([ATTR_ID,mapDiv,
				       ATTR_STYLE,HU.css(CSS_HEIGHT,HU.px(400))]);
		...
		if(!this.legendId) {
		    this.legendId = this.domId(ID_LEGENDID);
		...
		this.jq(ID_LEGENDID).html(keyLegend);
	    }
	    if(this.labelFeatures)
