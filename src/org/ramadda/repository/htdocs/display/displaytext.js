...
function RamaddaWordcloudDisplay(displayManager, id, properties) {
	...
	hasJq:false,
		...
		for(word in wordToWeight) {
		    info.push({
			text: word,
			html:{style:HU.css(CSS_CURSOR,CURSOR_POINTER)},
	    ...
	    let width = HU.perc((100 * 1 / strings.length));
			...
			html:{style:HU.css(CSS_CURSOR,CURSOR_POINTER)},
                ...
                divs += HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK,CSS_WIDTH, width)], 
			       label + HU.div([ATTR_STYLE, HU.css(CSS_BORDER,HU.border(1,COLOR_LIGHT_GRAY),
		...
		field = this.getFieldById(null, this.getProperty("termField"));
			...
			if (tokenize) {
                            v = v.replace(re, HU.span([ATTR_STYLE,HU.css(CSS_BACKGROUND,'yellow')],'$1'));
			}
		    }
                    if (showRecords) {
...
function RamaddaTemplateDisplay(displayManager, id, properties) {
	...
	updateUI: function() {
		...
		if(!this.selectedRecord && !this.selectedRecords) {
		    this.setContents(this.getEmptyMessage(HU.br()));
		    return;
		}
		...
		if(cols>0) {
		    ...
		    contents += HU.open(TAG_DIV,[ATTR_CLASS,'row-tight row']);
		}
		var colCnt = 0;
		...
		for(var rowIdx=0;rowIdx<selected.length;rowIdx++) {
			...
			contents+=HU.open(TAG_DIV,[ATTR_CLASS,colTag]);
			colCnt++;
		    }
			...
			if(this.getColorBackground()) {
			    recordStyle = HU.css(CSS_BACKGROUND,color) + recordStyle;
			}
		...
		this.jq(ID_DISPLAY_CONTENTS).find('.display-template-record').css(CSS_CURSOR,CURSOR_POINTER);
		...
		this.getContents().css(CSS_OVERFLOW_Y,'scroll');
...
function RamaddaBlocksDisplay(displayManager, id, properties) {
	...
	{p:'emptyBlockStyle',d:HU.css(CSS_BACKGROUND,COLOR_TRANSPARENT,CSS_BORDER,HU.border(1,COLOR_LIGHT_GRAY)),
	 tt:'Style for blocks not displayed yet'},	
    ];
	...
	showBlocks: function(initial, step) {
		...
		let style =  iconProp?"":HU.css(CSS_WIDTH,HU.px(dim),CSS_HEIGHT, HU.px(dim));
		let iconStyle = "";
		if(!initial) {
		    if(i<step) {
			if(!iconProp)
			    style += HU.css(CSS_BACKGROUND, ct[i]);
			else
			    iconStyle+=HU.css(CSS_COLOR ,ct[i]);
			let footerIcon =  iconProp?HU.getIconImage(iconProp, null, [ATTR_STYLE, iconStyle]):"";
			footer += HU.div([ATTR_CLASS,clazz,ATTR_STYLE,style],footerIcon) +" " +
		...
		for(let j=0;j<10000 && j<cnt;j++) {
		    contents += HU.div([ATTR_CLASS,clazz,ATTR_STYLE,style,ATTR_TITLE,label],icon);
		}
		blockCnt++;
	    }
	    contents += HU.closeDiv();
	    let header = this.getProperty("header","");
	    header = header.replace("${divider}",divider).replace("${total}",
								  Utils.formatNumberComma(this.total)).replace("${blockValue}",
													       Utils.formatNumberComma(Math.round(this.blockValue)));
            ...
            for (a in fieldInfo) {
                ...
                if (this.getProperty("showSummary", true)) {
				  ...
				  HU.th([], SPACE));
                    ...
                    html += HU.tr([], HU.td([ATTR_ALIGN, ALIGN_RIGHT], "Total words:") + HU.td([], cnt.count));
                    html += HU.tr([], HU.td([ATTR_ALIGN, ALIGN_RIGHT], "Average word length:") + HU.td([], Math.round(cnt.total / cnt.count)));
				  ...
				  HU.th([ATTR_WIDTH, td2Width], "Count") + (showBars ? HU.th([], "") : ""));
                    ...
                    for (let i = 0; i < tmp.length; i++) {
                        ...
                        if (showBars) {
                            ...
                            let color = barColor;
                            let div = HU.div([ATTR_STYLE, HU.css(CSS_HEIGHT,HU.px(10),
...
function RamaddaFrequencyDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
	    ...
	    let bannerHtml = "";
	    for (let col = 0; col < fields.length; col++) {
		bannerHtml += HU.open(TAG_DIV,[ATTR_STYLE,HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER)]);
		let f = fields[col];
		let s = summary[f.getId()];
		//		if(col>0) html+=HU.br();
		if(f.isNumeric()) {
		...
		if(this.getProperty("showHeader",true)) {
		    ...
		    html += HU.tr([], HU.th([],  label+ count+ percent+bars));
		    html += HU.closeTag(TAG_THEAD);
		}
		...
		for(let i=0;i<s.values.length;i++) {
		    ...
		    let tdb = showBars?HU.td([ATTR_VALIGN,ALIGN_CENTER,ATTR_WIDTH,barWidth],
					     HU.div([ATTR_TITLE,HU.perc(Math.round(perc*100)),
						     ATTR_STYLE,HU.css(CSS_BACKGROUND,color,CSS_HEIGHT,HU.px(10),
								       CSS_WIDTH,HU.px(Math.round(bw*barWidth)))],"")):"";
		    html += HU.tr([], tdv + tdc + tdp + tdb);
		}
		html += HU.close(TAG_TBODY,TAG_TABLE,TAG_DIV);
		bannerHtml += HU.close(TAG_TD);
	    }

	    let doBanner = this.getBanner(false);
...
function RamaddaTextanalysisDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, [], {
        ...
        updateUI: function() {
            var includes = HU.tag(TAG_SCRIPT,[ATTR_SRC,RamaddaUtil.getCdnUrl("/lib/compromise.min.js")]);
            this.writeHtml(ID_DISPLAY_TOP, includes);
            let _this = this;
            var func = function() {
		if(window.nlp) {
	    ...
	    var nlp = this.nlp;
            ...
            for (var i = 0; i < cols.length; i += 3) {
                ...
                var style = HU.css(CSS_PADDING,HU.px(5));
                var row = "";
                ...
                html += HU.tag(TAG_TABLE, [ATTR_WIDTH, HU.perc(100)], HU.tr(row));
            }
        ...
        printList: function(title, l) {
            ...
            for (var i = 0; i < l.length; i++) {
                ...
                var row = HU.td([], l[i].normal) +
                    HU.td([], l[i].count + ' (' + HU.perc(l[i].percent) + ')');
                table += HU.tr([], row);
...
function RamaddaTextrawDisplay(displayManager, id, properties) {
	...
	doShrink: properties["initialShrink"],
	    ...
	    if(this.showShrink) {
		input += " " + HU.checkbox("shrink",
					   [ATTR_ID,this.domId(ID_SHRINK)],
					   this.getProperty("initialShrink", true),'Shrink');
	    }
	    ...
	    var style = this.getProperty("displayInnerStyle","");
            var html = HU.div([ATTR_ID, this.domId(ID_TEXT),
			       ATTR_STYLE, HU.css(CSS_PADDING,HU.px(4),
						  ...
						  CSS_OVERFLOW_Y,OVERFLOW_AUTO) + style]);
            this.setContents(html);
	    let t1 = new Date();
            this.showText();
	    let t2 = new Date();
		...
		highlights=[];
		highlightStyles = this.getProperty("highlightStyles",
						   HU.css(CSS_BACKGROUND,'rgb(250_comma_0_comma_0)')).split(",");
	    ...
	    var bubble=this.getProperty("doBubble",false);
            if (labelTemplate) {
                corpus += HU.open(TAG_TABLE,[ATTR_WIDTH,HU.perc(100)]);
            }
            var lineCnt = 0;
		...
		var rowStyle="";
			...
			rowStyle +=HU.css(CSS_BACKGROUND,Utils.addAlphaToColor(color,"0.25"));
		    }
                }
		...
		if(rowScale) {
		    if(!hasMatch) {
			rowStyle += HU.css(CSS_WEBKIT_TRANSFORM,'scale(1," + rowScale +")');
			rowStyle += HU.css(CSS_LINE_HEIGHT,rowScale);
			rowAttrs.push(ATTR_STYLE);
		...
		if(fromField) line+=HU.div([ATTR_CLASS,"ramadda-bubble-from"],
					   ""+row[fromField.getIndex()]);
		...
		if(highlights) {
			...
			line= line.replace(h, HU.span([ATTR_STYLE,s],'$1'));
		    }
		}
		line = HU.div(lineAttrs,line);
			...
			r+= HU.td([ATTR_WIDTH, HU.px(5),
				   ATTR_STYLE,HU.css(CSS_BACKGROUND,COLOR_LIGHT_GRAY)],
		...
		label = displayedLineCnt+"/" +
		    this.allRecords.length+
		    linesWord+
		    " (" + HU.perc(Math.round(displayedLineCnt/this.allRecords.length*100))+")";
	    }
		...
		var open = function() {
		    ...
		    $(this).css(CSS_BORDER_BOTTOM,HU.border(1,COLOR_LIGHT_GRAY));
		    $(this).css(CSS_BORDER_TOP,HU.border(1,COLOR_LIGHT_GRAY));
		};
		var close = function() {
		    var row = this;
		    if(!$(row).attr("matched")) {	
			...
			$(row).css(CSS_BORDER_BOTTOM,HU.border(0,COLOR_LIGHT_GRAY));
			$(row).css(CSS_BORDER_TOP,HU.border(0,COLOR_LIGHT_GRAY));
		    }
		}
		rows.each(close);
...
function RamaddaGlossaryDisplay(displayManager, id, properties) {
    ...
    defineDisplay(addRamaddaDisplay(this), SUPER, myProps, {
		...
		let group =  HU.div([ATTR_CLASS,"display-glossary-group-header"],  _letter) +
		    HU.openTag(TAG_DIV,[ATTR_CLASS,"display-glossary-group-inner"]);
	    ...
	    let height = this.getProperty("glossaryHeight",HU.px(600));
	    header = HU.div([ATTR_ID,this.domId(ID_GLOSSARY_HEADER),
			     ATTR_CLASS,"display-glossary-header"], header);
	    html = HU.div([ATTR_STYLE,HU.css(CSS_MAX_HEIGHT,HU.getDimension(height),
