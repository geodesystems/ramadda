...
$.extend(Utils,{
	...
	for(let i=0;i<arguments.length;i+=3) {
	    rgb.push(HU.rgb(arguments[i],arguments[i+1],arguments[i+2]));
	}
	return rgb;
    },
	...
	let html = HU.div([ATTR_ID,guid]);
        html += HU.open(TAG_TABLE,[ATTR_WIDTH,HU.perc(50)]);
        let code = '';
	let cat = '';
	Utils.AllColorTables.forEach(ct=>{
            if(ct.category) {
		cat = ct.category;
                html += HU.tr([],HU.td([ATTR_COLSPAN,3],HU.h3(ct.category)));
		return
            }
	    ...
	    for(var i=0;i<colors.length;i++) {
            ...
            code += '});\n';
            html += HU.tr([ATTR_CLASS,'ramadda-colortable',ATTR_DATA_CORPUS,cat],
			  HU.td([ATTR_WIDTH,HU.perc(10)],
				HU.b(HU.span([ATTR_CLASS,'colortable-id'],ct.id))) +
			  HU.td([ATTR_ALIGN,ALIGN_RIGHT],HU.space(2)+colors.length+SPACE) +
			  HU.td([], HU.div([ATTR_ID, domId + '_' + cnt,
					    ATTR_STYLE, HU.css(CSS_OVERFLOW_X,OVERFLOW_HIDDEN,CSS_MAX_WIDTH,HU.px(500),
	...
	Utils.AllColorTables.forEach(colortable=>{
	    ...
	    let attrs = [ATTR_STYLE,HU.css(CSS_MARGIN_BOTTOM,HU.px(2),CSS_WIDTH,HU.px(400)),
			 ...
			 ATTR_DATA_CORPUS,colortable.id +' '+ category,
	...
	let colorInfo={};
		...
		let info = colorInfo[v.color];
                ...
                info.label+=HU.div([ATTR_TITLE,v.value,
				    ATTR_STYLE,style],value);
	...
	if(!options.horizontal && Utils.stringDefined(options.title))
        ...
        if(!options.showColorTableDots) {
            html+= HU.open(TAG_TABLE,[ATTR_CELLPADDING,0,
				      ATTR_CELLSPACING,0,
        ...
        if (options.showRange) {
            if(!options.showColorTableDots) {
                if(options.horizontal) 
                    html += HU.tag(TAG_TD,[ATTR_WIDTH,HU.perc(1)],formatter(min) + SPACE);
                else
                    html += formatter(min) + HU.br();
            }   else {
            }
        }
	...
	if(options.tooltips && options.tooltips.length>0 && options.tooltips.length<ct.length)  {
		...
		if(options.stringValues)  {
		    ...
		    color = options.stringValues[idx]?.color??color;
                ...
                let delim = SPACE;
		//		if(!options.horizontal && !options.tooltips)                    delim=HU.br();
		...
		if(options.tooltips) {
                    html += HU.div(['label',label,
				    ATTR_DATA_VALUE,val,
				    ATTR_STYLE,HU.css(CSS_WIDTH,HU.perc(100),
						      CSS_DISPLAY,DISPLAY_INLINE_BLOCK),
				    ATTR_CLASS,"display-colortable-dot-item",
				    ATTR_TITLE,label],
				   HU.div([ATTR_DATA_VALUE,val,
					   ATTR_CLASS, "display-colortable-dot",
					   ATTR_STYLE, dotStyle]) + delim + label);
		} else {
		    let dot = HU.span([ATTR_DATA_VALUE,val,
				       ATTR_CLASS, "display-colortable-dot",
				       ATTR_STYLE, dotStyle]);
		    let item;
		    if(!options.horizontal) {
			...
			item = dot + delim + label;
                    ...
                    html += HU.div([ATTR_CLASS,"display-colortable-dot-item",
				    ATTR_TITLE,label], item);
		}
		//                if(!options.horizontal) html +=HU.br();
		return;
            } 
            let label;
	    let title='';
		...
		attrs.push(ATTR_STYLE,labelStyle);
        ...
        if(!options.showColorTableDots) {
            if (options.showRange) {
                if(options.horizontal) 
                    html += HU.td([ATTR_WIDTH,HU.perc(1)], formatter(max) + SPACE);
                else
                    html += formatter(max) + HU.br();
            }
            if(options.horizontal) 
