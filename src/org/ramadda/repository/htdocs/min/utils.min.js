...
console.log("creating Utils");var Utils={pageLoaded:false,loadFunctions:[],addLoadFunction:function(f){Utils.loadFunctions.push(f);},getIcon:function(icon){return ramaddaBaseUrl+"/icons/"+icon;},imports:{},get:function(list,idx,dflt){if(!list)return dflt;if(idx<list.length)return list[idx];return dflt;},rotate:function(cx,cy,x,y,angle,anticlock_wise=false){if(angle==0){return{x:parseFloat(x),y:parseFloat(y)};}
...
return info.editor;},makeBreadcrumbsInit:function(id){if(!Utils.getPageLoaded()){let theId=id;Utils.addLoadFunction(()=>{HtmlUtils.makeBreadcrumbs(theId);});return;}
...
$("<img>",{style:item.element.attr("data-style"),"class":"ui-icon "+item.element.attr("data-class"),"src":item.element.attr("img-src")}).appendTo(wrapper);wrapper.append(item.label);return li.append(wrapper).appendTo(ul);}});console.log("done creating HtmlUtils");window.onbeforeunload=pageIsUnloading;function Div(contents,clazz){this.id=HtmlUtils.getUniqueId();this.contents=contents||"";this.extra="";this.clazz=clazz;this.setHidden=function(){}
...
$(document).ready(function(){HU.documentReady=true;Utils.checkForResize();});console.log("utils.js done loading");
