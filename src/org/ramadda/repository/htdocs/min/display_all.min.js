...
function RamaddaTemplateDisplay(displayManager,id,properties){if(!Utils.isDefined(properties.showTitle))properties.showTitle=false;if(!Utils.isDefined(properties.showMenu))properties.showMenu=false;if(!Utils.isDefined(properties.displayStyle))properties.displayStyle="background:rgba(0,0,0,0);";let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_TEMPLATE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{updateUI:function(){var pointData=this.getData();if(pointData==null)return;var records=this.filterData();if(!records)return;records=this.sortRecords(records);var fields=pointData.getRecordFields();var template=this.getProperty("template","");var select=this.getProperty("select","all");var selected=[];var summary={};records.map(r=>{r=this.getDataValues(r);for(var i=0;i<fields.length;i++){var f=fields[i];if(!f.isNumeric)continue;var v=f.getValue(r);if(!summary[f.getId()]){summary[f.getId()]={total:0,min:v,max:v,count:0}}
var s=summary[f.getId()];if(!isNaN(v)){s.total+=v;s.min=Math.min(s.min,v);s.max=Math.max(s.max,v);s.count++;}}});for(var i=0;i<fields.length;i++){var f=fields[i];if(!f.isNumeric)continue;var s=summary[f.getId()];if(s&&s.count){s.average=Utils.formatNumber(s.total/s.count);}}
