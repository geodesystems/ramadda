...
for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];if(this.filterBounds&&record.hasLocation()){var b=this.filterBounds;var lat=record.getLatitude();var lon=record.getLongitude();if(lat>b.top||lat<b.bottom||lon<b.left||lon>b.right)
continue;}
...
let cnt=0;let ok=false;fieldsToUse.some(f=>{if(field&&!(field.isFieldLatitude()||f.isFieldLongitude()))
if(f.isFieldLatitude()||f.isFieldLongitude())return true;if(f.isNumeric()){cnt++;let v=r.getValue(f.getIndex());ok=!isNaN(v);}
...
this.makePopups(contents.find(".display-mapgrid-cell"),records);let _this=this;contents.find(".display-mapgrid-cell").click(function(){var record=records[$(this).attr("recordIndex")];if(record){_this.getDisplayManager().notifyEvent("handleEventRecordSelection",_this,{record:record});}});this.makeTooltips(contents.find(".display-mapgrid-cell"),records,null,"${default}");if(colorBy.index>=0){colorBy.displayColorTable();}
