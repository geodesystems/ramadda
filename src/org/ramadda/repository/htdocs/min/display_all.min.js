...
this.cells=cells;this.layoutCells();newCell.focus();},deleteCell:function(cell){cell.jq(ID_CELL).remove();var cells=[];this.cells.map(c=>{if(cell.id!=c.id){cells.push(c);}});this.cells=cells;if(this.cells.length==0){this.addCell("",null);}},runAll:async function(){var ok=true;for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];await cell.run(result=>ok=result);if(!ok)break;}},toggleAll:function(on){this.cells.map(cell=>{cell.showInput=on;cell.applyStyle();});},});}
...
if(this.showBorder){this.cell.css("border-top","1px #ccc solid");}else{this.cell.css("border-top","1px #fff solid");}},askDelete:function(){let _this=this;var menu="";menu+="You sure you want to delete this cell?<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","yes"],"Yes")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","cancel"],"No");var popup=_this.jq(ID_MENU);popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();popup.position({of:this.menuButton,my:"left top",at:"right top",collision:"fit fit"});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");popup.hide();if(what=="yes"){_this.notebook.deleteCell(_this);}});},run:async function(callback){if(this.running)return Utils.call(callback,true);this.running=true;try{await this.runInner();}catch(e){this.writeOutput("An error occurred:"+e);console.log(e.stack);return false;}
