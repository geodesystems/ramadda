...
return colors;},getColorTable:function(justColors,name,dflt){var colorTable=this.getColorTableName(name);if(!colorTable){colorTable=dflt;}
var list;if(colorTable){var ct=Utils.ColorTables[colorTable];if(ct&&justColors)return this.convertColors(ct.colors);if(!ct&&name){return this.convertColors(colorTable.split(","));}
return ct;}
...
if(this.getProperty("excludeRow")){let exclude=[];this.getProperty("excludeRow").split(";").map(tok=>{[col,val]=tok.split(",");exclude.push([this.getFieldById(null,col),new RegExp(val)]);});dataList=dataList.filter(r=>{let ok=true;exclude.map(pair=>{if(String(r.getValue(pair[0].getIndex())).match(pair[1]))ok=false;});return ok;});}
if(this.getProperty("matchRow")){let match=[];this.getProperty("matchRow").split(";").map(tok=>{[col,val]=tok.split(",");match.push([this.getFieldById(null,col),new RegExp(val)]);});dataList=dataList.filter(r=>{let ok=true;match.map(pair=>{if(!String(r.getValue(pair[0].getIndex())).match(pair[1]))ok=false;});return ok;});}
...
chartOptions.vAxis.logScale=true;chartOptions.vAxis.viewWindowMode=this.getProperty("viewWindowMode","pretty");chartOptions.vAxis.viewWindowMode=this.getProperty("viewWindowMode","pretty");if(dataList.length>0&&this.getDataValues(dataList[0]).length>1){if(!chartOptions.vAxis)chartOptions.vAxis={};if(!chartOptions.hAxis)chartOptions.hAxis={};if(this.getProperty("hAxisTitle")){chartOptions.hAxis.title=this.getProperty("hAxisTitle");}
if(this.getProperty("vAxisTitle")){chartOptions.vAxis.title=this.getProperty("vAxisTitle");}
...
this.jq(ID_DISPLAY_CONTENTS).html(html);let categoryField=this.getFieldById(null,this.getProperty("categoryField","category"));let valueField=this.getFieldById(null,this.getProperty("valueField"));let values;let min=0;let max=0;if(valueField){values=this.getColumnValues(records,valueField).values;values.map((v,idx)=>{min=idx==0?v:Math.min(v,min);max=idx==0?v:Math.max(v,max);});}
