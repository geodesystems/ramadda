...
var headerTemplate=this.getProperty("headerTemplate","");var footerTemplate=this.getProperty("footerTemplate","");headerTemplate=headerTemplate.replace("${selectedCount}",selected.length);headerTemplate=headerTemplate.replace("${totalCount}",records.length);footerTemplate=footerTemplate.replace("${selectedCount}",selected.length);footerTemplate=footerTemplate.replace("${totalCount}",records.length);for(var i=0;i<fields.length;i++){var f=fields[i];if(!f.isNumeric)continue;var s=summary[f.getId()];headerTemplate=headerTemplate.replace("${"+f.getId()+"_total}",s.total).replace("${"+f.getId()+"_min}",s.min).replace("${"+f.getId()+"_max}",s.max).replace("${"+f.getId()+"_average}",s.average);}
if(this.filterFields){for(var filterIdx=0;filterIdx<this.filterFields.length;filterIdx++){var f=this.filterFields[filterIdx];if(f.isNumeric){var min=$("#"+this.getDomId("filterby_"+f.getId()+"_min")).val().trim();var max=$("#"+this.getDomId("filterby_"+f.getId()+"_max")).val().trim();headerTemplate=headerTemplate.replace("${filter_"+f.getId()+"_min}",min);headerTemplate=headerTemplate.replace("${filter_"+f.getId()+"_max}",max);footerTemplate=footerTemplate.replace("${filter_"+f.getId()+"_min}",min);footerTemplate=footerTemplate.replace("${filter_"+f.getId()+"_max}",max);}else{var value=$("#"+this.getDomId("filterby_"+f.getId())).val().trim();if(value==""){var regexp=new RegExp("\\${filter_"+f.getId()+"[^}]*\\}",'g');headerTemplate=headerTemplate.replace(regexp,"");footerTemplate=footerTemplate.replace(regexp,"");}else{var regexp=new RegExp("\\${filter_"+f.getId()+" +prefix='([^']*)' +suffix='([^']*)' *\\}",'g');headerTemplate=headerTemplate.replace(regexp,"$1"+value+"$2");footerTemplate=headerTemplate.replace(regexp,"$1"+value+"$2");var regexp=new RegExp("\\${filter_"+f.getId()+" +prefix='([^']*)' *\\}",'g');headerTemplate=headerTemplate.replace(regexp,"$1"+value);footerTemplate=headerTemplate.replace(regexp,"$1"+value);var regexp=new RegExp("\\${filter_"+f.getId()+" +suffix='([^']*)' *\\}",'g');headerTemplate=headerTemplate.replace(regexp,value+"$1");footerTemplate=headerTemplate.replace(regexp,value+"$1");var regexp=new RegExp("\\${filter_"+f.getId()+" *\\}",'g');headerTemplate=headerTemplate.replace(regexp,value);footerTemplate=footerTemplate.replace(regexp,value);}}}}
if(selected.length>0)
contents+=headerTemplate;if(template!=""){var max=parseFloat(this.getProperty("maxNumber",-1));for(var rowIdx=0;rowIdx<selected.length;rowIdx++){if(max!=-1&&rowIdx>=max)break;var row=this.getDataValues(selected[rowIdx]);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);for(var col=0;col<fields.length;col++){var f=fields[col];var value=row[f.getIndex()];if(f.getType()=="image"){if(value&&value.trim().length>1){var attrs=[];if(this.getProperty("imageWidth","")!=""){attrs.push("width");attrs.push(this.getProperty("imageWidth",""));}
