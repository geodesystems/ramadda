...
date2=null;value=[date1,date2];}
values.push(value);}
for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);var ok=true;for(var i=0;i<this.filterFields.length;i++){if(!ok)break;var filterField=this.filterFields[i];var filterValue=values[i];if(filterValue==null||filterValue=="")continue;var value=row[filterField.getIndex()];if(filterField.getType()=="enumeration"){filterValue=""+filterValue;if((""+value)!==filterValue){ok=false;}}else if(filterField.isNumeric){if(!isNaN(filterValue[0])&&value<filterValue[0])ok=false;else if(!isNaN(filterValue[1])&&value>filterValue[1])ok=false;}else if(filterField.getType()=="date"){if(value&&typeof value=="object"){var date1=filterValue[0];var date2=filterValue[1];if(date1&&value.getTime()<date1.getTime())
ok=false;else if(date2&&value.getTime()>date2.getTime())
ok=false;}}else{filterValue=""+filterValue;value=(""+value).toLowerCase();if(value.indexOf(filterValue.toLowerCase())<0){ok=false;}}}
...
widget=HtmlUtils.select("",["style",widgetStyle,"id",widgetId,"fieldId",filterField.getId()],enums,dfltValue);}else if(filterField.isNumeric){var min=0;var max=0;var cnt=0;records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(isNaN(value))return;if(cnt==0){min=value;max=value;}
...
function RamaddaCardsDisplay(displayManager,id,properties){var ID_RESULTS="results";let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_CARDS,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{getContentsStyle:function(){return"";},updateUI:function(){var pointData=this.getData();if(pointData==null)return;var allFields=pointData.getRecordFields();var fields=this.getSelectedFields(allFields);if(fields==null||fields.length==0){fields=allFields;}
...
return null;},});}
var topGroup=new groupNode("");var colorMap={};var colorCnt=0;for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);var contents="";var tooltip="";this.tooltipFields.map(field=>{if(tooltip!="")tooltip+="&#10;";tooltip+=field.getValue(row);});var label="";var caption="";if(this.captionFields.length>0){if(this.captionTemplate)caption=this.captionTemplate;this.captionFields.map(field=>{if(this.captionTemplate)
...
if(this.labelField)label="<br>"+row[this.labelField.getIndex()];var html="";var img=null;if(this.imageField){img=row[this.imageField.getIndex()];if(this.onlyShowImages&&!Utils.stringDefined(img))continue;}
var imgAttrs=["class","display-cards-popup","data-fancybox",this.getDomId("gallery"),"data-caption",caption];if(img)img=img.trim();if(img!=""){img=HtmlUtils.href(img,HtmlUtils.image(img,["width",width]),imgAttrs)+label;html=HtmlUtils.div(["class","display-cards-item","title",tooltip,"style","margin:"+margin+"px;"],img);}else{var style="";if(fontSize){style+=" font-size:"+fontSize+"; ";}
