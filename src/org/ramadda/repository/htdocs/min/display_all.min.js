...
return{colors:colorValues,min:min,max:max};},getColorByMap:function(){var colorByMapProp=this.getProperty("colorByMap");if(colorByMapProp){var toks=colorByMapProp.split(",");var stringMap={};for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){stringMap[toks2[0]]=toks2[1];}}
return stringMap;}
...
this.updateLabels();var windowEnd=this.end.getTime();if(windowEnd<=this.dateMax.getTime()){if(this.running){setTimeout(()=>{if(!this.running)return;this.doNext()},this.speed);}}else{this.running=false;this.inAnimation=false;this.btnRun.html(HtmlUtils.getIconImage("fa-play"));}},updateLabels:function(){if(this.label)
this.label.html(this.formatAnimationDate(this.begin)+" - "+this.formatAnimationDate(this.end));},formatAnimationDate:function(d){if(this.dateFormat=="yyyy"){return Utils.formatDateYYYY(d);}else if(this.dateFormat=="yyyyMMdd"){return Utils.formatDateYYYYMMDD(d);}else{return Utils.formatDate(d);}},});}
...
if(template!=""){var colorBy=this.getProperty("colorBy");var colorByMap=this.getColorByMap();var max=parseFloat(this.getProperty("maxNumber",-1));for(var rowIdx=0;rowIdx<selected.length;rowIdx++){if(max!=-1&&rowIdx>=max)break;var row=this.getDataValues(selected[rowIdx]);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);for(var col=0;col<fields.length;col++){var f=fields[col];var value=row[f.getIndex()];if(f.getType()=="image"){if(value&&value.trim().length>1){var attrs=[];if(this.getProperty("imageWidth","")!=""){attrs.push("width");attrs.push(this.getProperty("imageWidth",""));}
...
this.pointsAreMarkers=true;var iconMap;var iconMapProp=this.getProperty("iconMap");if(iconMapProp){var toks=iconMapProp.split(",");iconMap={};for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){iconMap[toks2[0]]=toks2[1];}}}
var dfltShape=this.getProperty("defaultShape",null);var dfltShapes=["circle","triangle","star","square","cross","x","lightning","rectangle","church"];var dfltShapeIdx=0;var shapeBy={id:this.getDisplayProp(source,"shapeBy",null),field:null,map:{}}
