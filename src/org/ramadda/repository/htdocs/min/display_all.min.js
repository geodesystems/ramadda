...
return{values:values,min:min,max:max};},requiresGrouping:function(){return false;},getFilterFieldValue:function(field){var value;var element=$("#"+this.getDomId("filterby_"+field.getId()));value=element.val();if(!value)
value=element.attr("value");return value;},filterData:function(dataList,fields,doGroup,skipFirst){var t1=new Date();var pointData=this.getData();if(!dataList){if(pointData==null)return null;dataList=pointData.getRecords();}
...
var newData=[];var values=[];var allIsUsed=false;for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];if(filterField.isString()){var value=this.getFilterFieldValue(filterField);if(value==FILTER_ALL){allIsUsed=true;break;}}}
...
if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;var value;var _values=[];var regexps=[];if(filterField.isNumeric()){var minField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_min"));var maxField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_max"));var minValue=parseFloat(minField.val().trim());var maxValue=parseFloat(maxField.val().trim());var dfltMinValue=parseFloat(minField.attr("data-min"));var dfltMaxValue=parseFloat(maxField.attr("data-max"));if(minValue!=dfltMinValue||maxValue!=dfltMaxValue){value=[minValue,maxValue];}else{value=[NaN,NaN];}}else if(filterField.getType()=="date"){var date1=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date1")).val();var date2=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date2")).val();if(date1!=null&&date1.trim()!="")
date1=Utils.parseDate(date1);else
date1=null;if(date2!=null&&date2.trim()!="")
date2=Utils.parseDate(date2);else
date2=null;value=[date1,date2];}else{value=this.getFilterFieldValue(filterField);if(!Array.isArray(value))value=[value];value.map(v=>{_values.push((""+v).toLowerCase());try{regexps.push(new RegExp(v,"i"));}catch(skipIt){}});}
...
continue;}
var row=this.getDataValues(record);var ok=true;for(var i=0;i<this.filterFields.length;i++){if(!ok)break;if(values[i]==null)continue;var filterValue=values[i].value;var filterField=this.filterFields[i];if(filterValue==null||filterValue.length==0||(filterValue.length==1&&filterValue[0]==FILTER_ALL))continue;var value=row[filterField.getIndex()];if(filterField.getType()=="enumeration"){ok=false;for(var j=0;j<filterValue.length;j++){var v=""+filterValue[j];if((""+value)==v){ok=true;break;}}}else if(filterField.isNumeric()){if(isNaN(filterValue[0])&&isNaN(filterValue[0]))continue;if(isNaN(value)){ok=false;}else{if(!isNaN(filterValue[0])&&value<filterValue[0])ok=false;else if(!isNaN(filterValue[1])&&value>filterValue[1])ok=false;}}else if(filterField.getType()=="date"){if(value&&typeof value=="object"){var date1=filterValue[0];var date2=filterValue[1];if(date1&&value.getTime()<date1.getTime())
...
var values=[];parent.find(".display-filterby-button-selected").each(function(){values.push($(this).attr("value"));});var value=Utils.join(values,",");parent.attr("value",value);$("#"+parent.attr("id")+"_label").html(values.includes(FILTER_ALL)?"&nbsp;":value);inputFunc(parent,null,value);});});this.jq(ID_FILTERBAR).find(".display-filter-input").keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){return;}
...
if(i>0){html+=HtmlUtils.closeTag(TAG_DIV);}}}else if(this.layout==LAYOUT_TABLE){if(displaysToLayout.length==1){html+=displaysToLayout[0].layoutDiv;}else{var percent=Math.round((100/this.columns))+"%";var i=0;html+="<table width=100%>\n";var colCnt=100;for(var i=0;i<displaysToLayout.length;i++){colCnt++;if(colCnt>=this.columns){if(i>0){html+=HtmlUtils.closeTag(TAG_TR);}
...
var regexpMap={};var filterFieldMap={};if(this.filterFields){this.filterFields.map(f=>{if(f.isString)filterFieldMap[f.getId()]=true;});}
for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];this.indexToRecord[rowIdx]=record;this.recordToIndex[record.getId()]=rowIdx;var row=this.getDataValues(record);var line="";for(var col=0;col<fields.length;col++){var f=fields[col];if(rowIdx==0){if(filterFieldMap[f.getId()]){var value=this.getFilterFieldValue(f);if(value&&value.length>0){try{regexpMap[f.getId()]=new RegExp("("+value+")","i");}catch(e){}}}}
line+=" ";var value=""+row[f.getIndex()];value=value.replace(/</g,"&lt;").replace(/>/g,"&gt;");if(regexpMap[f.getId()]){value=value.replace(regexpMap[f.getId()],"<span style=background:yellow;>$1</span>");}
line+=value;}
