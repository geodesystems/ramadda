...
var displayDebug=false;var ID_FIELDS="fields";var ID_HEADER="header";var ID_HEADER1="header1";var ID_HEADER2="header2";var ID_HEADER3="header3";var ID_FILTERBAR="filterbar";var ID_TITLE=ATTR_TITLE;var ID_TITLE_EDIT="title_edit";var ID_TOP_RIGHT="topright";var ID_TOP_LEFT="topleft";var ID_DETAILS="details";var ID_DISPLAY_CONTENTS="contents";var ID_DISPLAY_TOP="top";var ID_DISPLAY_BOTTOM="bottom";var ID_GROUP_CONTENTS="group_contents";var ID_DETAILS_MAIN="detailsmain";var ID_GROUPBY_FIELDS="groupdbyfields";var ID_TOOLBAR="toolbar";var ID_TOOLBAR_INNER="toolbarinner";var ID_LIST="list";var ID_DIALOG="dialog";var ID_DIALOG_TABS="dialog_tabs";var ID_DIALOG_BUTTON="dialog_button";var ID_FOOTER="footer";var ID_FOOTER_LEFT="footer_left";var ID_FOOTER_RIGHT="footer_right";var ID_MENU_BUTTON="menu_button";var ID_MENU_OUTER="menu_outer";var ID_MENU_INNER="menu_inner";var ID_REPOSITORY="repository";var PROP_DISPLAY_FILTER="displayFilter";var PROP_EXCLUDE_ZERO="excludeZero";var PROP_DIVID="divid";var PROP_FIELDS="fields";var PROP_LAYOUT_HERE="layoutHere";var PROP_HEIGHT="height";var PROP_WIDTH="width";var FILTER_ALL="-all-";function initRamaddaDisplays(){ramaddaCheckForResize();if(window.globalDisplaysList==null){return;}
...
return this.getProperty(key,dflt);},initTooltip:function(){},formatNumber:function(number){if(!this.getProperty("format",true))return number;return Utils.formatNumber(number);},propertyDefined:function(key){return Utils.isDefined(this.getProperty(key));},setPropertyOn:function(object,myProperty,objectProperty,dflt){var prop=this.getProperty(myProperty,dflt);if(Utils.isDefined(prop)&&prop!=null){object[objectProperty]=prop;}},getProperty:function(key,dflt,skipThis){if(!skipThis&&this[key]){return this[key];}
...
return iconMap;},getColorByMap:function(){var colorByMapProp=this.getProperty("colorByMap");if(colorByMapProp){var toks=colorByMapProp.split(",");var stringMap={};for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){stringMap[toks2[0].trim()]=toks2[1].trim();}}
return stringMap;}
...
return;}
if(prop.property=="filterValue"){if(!this.getProperty("acceptFilterEvent",true))return;this.haveCalledUpdateUI=false;var widgetId="filterby_"+prop.fieldId;this.settingFilterValue=true;if(Utils.isDefined(prop.value2)){this.jq(widgetId+"_min").val(prop.value);this.jq(widgetId+"_min").attr("value",prop.value);this.jq(widgetId+"_max").val(prop.value2);this.jq(widgetId+"_max").attr("value",prop.value2);}else{this.jq(widgetId).val(prop.value);this.jq(widgetId).attr("value",prop.value);}
...
widgetStyle="display:none;";var fieldMap={};for(var i=0;i<filterBy.length;i++){var filterField=this.getFieldById(fields,filterBy[i]);if(!filterField)continue;fieldMap[filterField.getId()]={field:filterField,values:[],};if(!filterField)continue;this.filterFields.push(filterField);var widget;var widgetId=this.getDomId("filterby_"+filterField.getId());if(filterField.getType()=="enumeration"){var dfltValue=this.getProperty(filterField.getId()+".filterValue",FILTER_ALL);var filterValues=this.getProperty(filterField.getId()+".filterValues");var enums=null;if(filterValues){var toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}
enums=toks;}
...
return html;},makeToolbar:function(props){var toolbar="";var get=this.getGet();var addLabel=props.addLabel;var images=[];var calls=[];var labels=[];if(!this.getIsLayoutFixed()){calls.push("removeRamaddaDisplay('"+this.getId()+"')");images.push("fa-cut");labels.push("Delete display");}
...
var nonNullRecords=0;var indexField=this.indexField;var records=this.filterData();var allFields=pointData.getRecordFields();this.hasDate=this.getHasDate(records);var date_formatter=this.getDateFormatter();var rowCnt=-1;for(j=0;j<records.length;j++){var record=records[j];var date=record.getDate();if(!this.dateInRange(date)){continue;}
rowCnt++;this.recordToIndex[record.getId()]=rowCnt;this.indexToRecord[rowCnt]=record;var values=[];if(props&&(props.includeIndex||props.includeIndexIfDate)){var indexName=null;if(indexField>=0){var field=allFields[indexField];values.push(record.getValue(indexField)+offset);indexName=field.getLabel();}else{if(this.hasDate){values.push(this.getDateValue(date,date_formatter));indexName="Date";}else{if(!props.includeIndexIfDate){values.push(j);indexName="Index";}}}
...
var annotationCnt=0;for(var i=1;i<dataList.length;i++){var record=dataList[i];var theRecord=dataList[i].record;var row=this.getDataValues(record);row=row.slice(0);var label="";if(dataList[i].record){for(var j=0;j<tooltipFields.length;j++){label+="<b>"+tooltipFields[j].getLabel()+"</b>: "+
...
tooltip="<div style='padding:8px;'>"+tooltip+"</div>";newRow=[];for(var j=0;j<row.length;j++){var value=row[j];if(j>0&&fixedValueS){newRow.push(fixedValueN);break;}
newRow.push(value);if(j==0&&props.includeIndex){}else{newRow.push(tooltip);}}
...
if(this.hAxis){if(chartOptions.hAxis){chartOptions.hAxis.title=this.hAxis;}else{chartOptions.hAxis={title:this.hAxis}}}
if(this.vAxis){if(chartOptions.vAxis){chartOptions.vAxis.title=this.vAxis;}else{chartOptions.vAxis={title:this.vAxis}}}
...
var showCount=this.getProperty("showCount",true);var showBars=this.getProperty("showBars",false);var barWidth=+this.getProperty("barWidth",200);for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);for(var col=0;col<fields.length;col++){var f=fields[col];var s=summary[f.getId()];var value=row[f.getIndex()];if(!Utils.isDefined(s.min)){s.min=value;s.max=value;}
...
html+=HtmlUtils.openTag("div",["class","display-frequency-table","style",hor?"":"display:block;"]);html+=HtmlUtils.openTag("table",["cellpadding","3","id",this.getDomId("summary"+col),"table-height",this.getProperty("tableHeight","300",true),"class","stripe row-border nowrap ramadda-table"]);if(this.getProperty("showHeader",true)){html+=HtmlUtils.openTag("thead",[]);var label=HtmlUtils.span(["title","Click to reset","class","display-frequency-label","data-field",s.field.getId()],f.getLabel());label=HtmlUtils.div(["style","max-width:500px;overflow-x:auto;"],label);var count=showCount?HtmlUtils.th(["align","right","width","20%"],HtmlUtils.div(["style","text-align:right"],"Count")):"";var percent=HtmlUtils.th(["align","right","width","20%"],HtmlUtils.div(["style","text-align:right"],"Percent"));var bars=showBars?HtmlUtils.th(["align","right","width",barWidth],HtmlUtils.div(["style","text-align:right"],"&nbsp;")):"";html+=HtmlUtils.tr([],HtmlUtils.th(["xxwidth","60%"],label+count+percent+bars));html+=HtmlUtils.closeTag("thead");}
html+=HtmlUtils.openTag("tbody",[]);if(!f.isNumeric){s.values.sort((a,b)=>{if(a.count<b.count)return 1;if(a.count>b.count)return-1;return 0;});}
var colors=this.getColorTable(true);var dfltColor=this.getProperty("barColor","blue");var maxPercent=0;for(var i=0;i<s.values.length;i++){var count=s.values[i].count;if(count==0)continue;var perc=count/s.total;maxPercent=Math.max(maxPercent,perc);}
for(var i=0;i<s.values.length;i++){var value=s.values[i].value;var label=value;if(label=="")label="&lt;blank&gt;";var count=s.values[i].count;if(count==0)continue;value=value.replace(/\'/g,"&apos;");value=HtmlUtils.span(["title","Click to select","class","display-frequency-value","data-field",s.field.getId(),"data-value",value],label);var tdv=HtmlUtils.td([],value);var tdc=(showCount?HtmlUtils.td(["align","right"],count):"");var perc=count/s.total;var tdp=HtmlUtils.td(["align","right"],s.total==0?"0":Math.round(perc*100)+"%");var color=dfltColor;if(colors){if(i<colors.length)
color=colors[i];else
color=colors[colors.length-1];}
var bw=perc/maxPercent;var tdb=showBars?HtmlUtils.td(["valign","center","width",barWidth],HtmlUtils.div(["title",Math.round(perc*100)+"%","style","background:"+color+";height:10px;width:"+(Math.round(bw*barWidth))+"px"],"")):"";html+=HtmlUtils.tr([],tdv+tdc+tdp+tdb);}
html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");html+=HtmlUtils.closeTag("div");}
this.writeHtml(ID_DISPLAY_CONTENTS,html);let _this=this;this.jq(ID_DISPLAY_CONTENTS).find(".display-frequency-value").click(function(){_this.handleEventPropertyChanged(_this,{property:"filterValue",id:"id",fieldId:$(this).attr("data-field"),value:$(this).attr("data-value")});});this.jq(ID_DISPLAY_CONTENTS).find(".display-frequency-label").click(function(){var field=$(this).attr("data-field");_this.handleEventPropertyChanged(_this,{property:"filterValue",id:"id",fieldId:field,value:"-all-"});});if(this.getProperty("showHeader",true)){for(var col=0;col<fields.length;col++){HtmlUtils.formatTable("#"+this.getDomId("summary"+col),{});}}}});}
...
html+=HtmlUtils.div([ATTR_CLASS,"display-map-map","style",extraStyle,ATTR_ID,this.getDomId(ID_MAP)]);html+=HtmlUtils.div([ATTR_CLASS,"",ATTR_ID,this.getDomId(ID_BOTTOM)]);if(this.showLocationReadout){html+=HtmlUtils.openTag(TAG_DIV,[ATTR_CLASS,"display-map-latlon"]);html+=HtmlUtils.openTag("form");html+="Latitude: "+
...
this.getDisplayManager().handleEventEntrySelection(this,{entry:marker.entry,selected:true});this.selectedMarker=marker;},getDisplayProp:function(source,prop,dflt){if(Utils.isDefined(this[prop])){return this[prop];}
prop="map-"+prop;if(Utils.isDefined(source[prop])){return source[prop];}
if(source.getProperty)
return source.getProperty(prop,dflt);return null;},applyVectorMap:function(force){if(!force&&this.vectorMapApplied){return;}
if(!this.doDisplayMap()){return;}
...
var polygonField=this.getFieldById(fields,this.getProperty("polygonField"));var polygonColorTable=this.getColorTable(true,"polygonColorTable",null);var latlon=this.getProperty("latlon",true);var source=this;var radius=parseFloat(this.getDisplayProp(source,"radius",8));var strokeWidth=parseFloat(this.getDisplayProp(source,"strokeWidth","1"));var strokeColor=this.getDisplayProp(source,"strokeColor","#000");var colorByAttr=this.getProperty("colorBy",null);var colors;if(colorByAttr){colors=this.getColorTable(true,colorByAttr+".colorTable");if(!colors){var c=this.getProperty(colorByAttr+".colors");if(c)
colors=c.split(",");}
if(!colors)
colors=this.getColorTable(true);}
var sizeByAttr=this.getDisplayProp(source,"sizeBy",null);var isTrajectory=this.getDisplayProp(source,"isTrajectory",false);if(isTrajectory){var attrs={strokeWidth:2,strokeColor:"blue"}
this.map.addPolygon("id","",points,attrs,null);return;}
if(!colors&&source.colors&&source.colors.length>0){colors=source.colors;if(colors.length==1&&Utils.ColorTables[colors[0]]){colors=Utils.ColorTables[colors[0]].colors;}}
if(colors==null){colors=Utils.ColorTables.grayscale.colors;}
...
var colorBy={id:colorByAttr,minValue:0,maxValue:0,field:null,index:-1,isString:false,stringMap:null};var sizeBy={id:this.getDisplayProp(source,"sizeBy",null),minValue:0,maxValue:0,field:null,index:-1,isString:false,stringMap:{},};var sizeByMap=this.getProperty("sizeByMap");if(sizeByMap){var toks=sizeByMap.split(",");for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){sizeBy.stringMap[toks2[0]]=toks2[1];}}}
...
if(field.getId()==shapeBy.id||("#"+(i+1))==shapeBy.id){shapeBy.field=field;if(field.isString())shapeBy.isString=true;}
...
sizeBy.index=sizeBy.field!=null?sizeBy.field.getIndex():-1;colorBy.index=colorBy.field!=null?colorBy.field.getIndex():-1;shapeBy.index=shapeBy.field!=null?shapeBy.field.getIndex():-1;var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);var dateMin=null;var dateMax=null;colorBy.stringMap=this.getColorByMap();var colorByMap={};var colorByValues=[];var colorByMinPerc=this.getDisplayProp(source,"colorByMinPercentile",-1);var colorByMaxPerc=this.getDisplayProp(source,"colorByMaxPercentile",-1);var dates=[];var justOneMarker=this.getProperty("justOneMarker",false);for(var i=0;i<points.length;i++){var pointRecord=records[i];dates.push(pointRecord.getDate());if(dateMin==null){dateMin=pointRecord.getDate();dateMax=pointRecord.getDate();}else{var date=pointRecord.getDate();if(date){if(date.getTime()<dateMin.getTime())
dateMin=date;if(date.getTime()>dateMax.getTime())
dateMax=date;}}
var tuple=pointRecord.getData();var v=tuple[colorBy.index];if(colorBy.isString){if(!Utils.isDefined(colorByMap[v])){colorByValues.push(v);colorByMap[v]=colorByValues.length;colorBy.minValue=1;colorBy.maxValue=colorByValues.length;}}
...
if(!sizeBy.isString){v=tuple[sizeBy.index];if(!isNaN(v)&&!(v===null)){if(i==0||v>sizeBy.maxValue)sizeBy.maxValue=v;if(i==0||v<sizeBy.minValue)sizeBy.minValue=v;}}}
...
colorBy.minValue=this.getDisplayProp(source,"colorByMin",colorBy.minValue);colorBy.maxValue=this.getDisplayProp(source,"colorByMax",colorBy.maxValue);colorBy.origMinValue=colorBy.minValue;colorBy.origMaxValue=colorBy.maxValue;var colorByOffset=0;var colorByLog=this.getProperty("colorByLog",false);var colorByFunc=Math.log;if(colorByLog){if(colorBy.minValue<1){colorByOffset=1-colorBy.minValue;}
colorBy.minValue=colorByFunc(colorBy.minValue+colorByOffset);colorBy.maxValue=colorByFunc(colorBy.maxValue+colorByOffset);}
colorBy.range=colorBy.maxValue-colorBy.minValue;if(this.points){for(var i=0;i<this.points.length;i++){if(this.pointsAreMarkers)
...
if(isNaN(props.pointRadius)||props.pointRadius==0)props.pointRadius=radius;var hasColorByValue=false;var colorByValue;if(colorBy.index>=0){var value=pointRecord.getData()[colorBy.index];hasColorByValue=true;colorByValue=value;var percent=0;var msg="";var pctFields=null;if(this.percentFields!=null){pctFields=this.percentFields.split(",");}
if(this.showPercent){var total=0;var data=pointRecord.getData();var msg="";for(var j=0;j<data.length;j++){var ok=fields[j].isNumeric&&!fields[j].isFieldGeo();if(ok&&pctFields!=null){ok=pctFields.indexOf(fields[j].getId())>=0||pctFields.indexOf("#"+(j+1))>=0;}
if(ok){total+=data[j];msg+=" "+data[j];}}
if(total!=0){percent0=percent=value/total*100;percent=(percent-colorBy.minValue)/(colorBy.maxValue-colorBy.minValue);}}else{var v=value;if(colorBy.isString){v=colorByMap[v];}
v+=colorByOffset;if(colorByLog){v=colorByFunc(v);}
percent=(v-colorBy.minValue)/colorBy.range;}
var index=parseInt(percent*colors.length);if(index>=colors.length)index=colors.length-1;else if(index<0)index=0;if(colorBy.stringMap){props.fillColor=colorBy.stringMap[value];if(!Utils.isDefined(props.fillColor)){props.fillColor=colorBy.stringMap["default"];}
if(!Utils.isDefined(props.fillColor)){props.fillColor="blue";}}else{props.fillColor=colors[index];}
props.fillOpacity=parseFloat(this.getProperty("fillOpacity",0.8));didColorBy=true;}
...
if(this.map.circles)
this.map.circles.redraw();this.jq(ID_BOTTOM).html(HtmlUtils.div(["id",this.getDomId(ID_COLORTABLE)])+
HtmlUtils.div(["id",this.getDomId(ID_SHAPES)]));if(didColorBy){if(colorBy.stringMap){var colors=[];colorByValues=[];for(var i in colorBy.stringMap){colorByValues.push(i);colors.push(colorBy.stringMap[i]);}
this.displayColorTable(colors,ID_COLORTABLE,colorBy.origMinValue,colorBy.origMaxValue,{stringValues:colorByValues});}else{this.displayColorTable(colors,ID_COLORTABLE,colorBy.origMinValue,colorBy.origMaxValue,{stringValues:colorByValues});}}
