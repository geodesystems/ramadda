...
var showGeo=false;if(Utils.isDefined(this.showGeo)){showGeo=(""+this.showGeo)=="true";}
if(!template)
...
return;}
if(prop.property=="filterValue"){if(!this.getProperty("acceptFilterEvent",true))return;this.haveCalledUpdateUI=false;var widgetId=this.getFilterId(prop.fieldId);if(prop.id&&prop.id.endsWith("date1")){widgetId+="_date1";}else if(prop.id&&prop.id.endsWith("date2")){widgetId+="_date2";}
...
dataList=binned;}
dataList=this.sortRecords(dataList);this.recordToIndex={};this.indexToRecord={};for(var i=0;i<dataList.length;i++){var record=dataList[i];this.recordToIndex[record.getId()]=i;this.indexToRecord[i]=record;}
...
var indexObj=[];var closest=RecordUtil.findClosest(records,lon,lat,indexObj);if(closest!=null){this.propagateEventRecordSelection(mapDisplay,pointData,{index:indexObj.index});}
...
var pointNumber=data.points[0].pointNumber;var pattern=this.patternList[pointNumber];console.log(pattern);var args={property:"filterValue",fieldId:this.textField.getId(),value:pattern};this.propagateEvent("handleEventPropertyChanged",args);},});}
