...
return;}
if(prop.property=="filterValue"){if(!this.getProperty("acceptFilterEvent",true))return;this.haveCalledUpdateUI=false;var widgetId=this.getFilterId(prop.fieldId);this.settingFilterValue=true;if(Utils.isDefined(prop.value2)){$("#"+widgetId+"_min").val(prop.value);$("#"+widgetId+"_min").attr("value",prop.value);$("#"+widgetId+"_max").val(prop.value2);$("#"+widgetId+"_max").attr("value",prop.value2);}else{var widget=$("#"+widgetId);widget.val(prop.value);widget.attr("value",prop.value);if(widget.attr("isButton")){widget.find(".display-filterby-button").removeClass("display-filterby-button-selected");widget.find("[value='"+prop.value+"']").addClass("display-filterby-button-selected");}}
...
return{values:values,min:min,max:max};},requiresGrouping:function(){return false;},getFilterFieldValues:function(field){var element=$("#"+this.getDomId("filterby_"+field.getId()));var value=element.val();if(!value)
value=element.attr("value");if(!Array.isArray(value))value=value.split(",");var tmp=[];value.map(v=>tmp.push(v.trim()));value=tmp;return value;},filterData:function(dataList,fields,doGroup,skipFirst){var t1=new Date();var pointData=this.getData();if(!dataList){if(pointData==null)return null;dataList=pointData.getRecords();}
...
var newData=[];var values=[];var allIsUsed=false;for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];if(filterField.isString()){var values=this.getFilterFieldValues(filterField);if(values.includes(FILTER_ALL)){allIsUsed=true;break;}}}
for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];var prefix=this.getProperty(filterField.getId()+".filterPrefix");var suffix=this.getProperty(filterField.getId()+".filterSuffix");var useIfAll=this.getProperty(filterField.getId()+".filterUseIfAllIsSet",true);if(allIsUsed&&!useIfAll){values.push(null);continue;}
if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;var value;var values=[];var _values=[];var regexps=[];if(filterField.isNumeric()){var minField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_min"));var maxField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_max"));var minValue=parseFloat(minField.val().trim());var maxValue=parseFloat(maxField.val().trim());var dfltMinValue=parseFloat(minField.attr("data-min"));var dfltMaxValue=parseFloat(maxField.attr("data-max"));if(minValue!=dfltMinValue||maxValue!=dfltMaxValue){value=[minValue,maxValue];}else{value=[NaN,NaN];}}else if(filterField.getType()=="date"){var date1=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date1")).val();var date2=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date2")).val();if(date1!=null&&date1.trim()!="")
date1=Utils.parseDate(date1);else
date1=null;if(date2!=null&&date2.trim()!="")
date2=Utils.parseDate(date2);else
date2=null;value=[date1,date2];}else{value=this.getFilterFieldValues(filterField);if(!Array.isArray(value))value=[value];value.map(v=>{_values.push((""+v).toLowerCase());try{regexps.push(new RegExp(v,"i"));}catch(skipIt){}});}
var filterStartsWith=this.getProperty(filterField.getId()+".filterStartsWith",false);var anyValues=false;_values.map(v=>{if(v.length>0)anyValues=true});values.push({value:value,regexps:regexps,_values:_values,anyValues:anyValues,startsWith:filterStartsWith});}
...
continue;}
var row=this.getDataValues(record);var ok=true;for(var i=0;i<this.filterFields.length;i++){if(!ok)break;if(values[i]==null)continue;var filterValue=values[i].value;var filterField=this.filterFields[i];if(filterValue==null||filterValue.length==0||(filterValue.length==1&&filterValue[0]==FILTER_ALL)){continue;}
var value=row[filterField.getIndex()];if(filterField.getType()=="enumeration"){ok=false;for(var j=0;j<filterValue.length;j++){var fv=""+filterValue[j];if((""+value)==fv){ok=true;break;}}}else if(filterField.isNumeric()){if(isNaN(filterValue[0])&&isNaN(filterValue[0]))continue;if(isNaN(value)){ok=false;}else{if(!isNaN(filterValue[0])&&value<filterValue[0])ok=false;else if(!isNaN(filterValue[1])&&value>filterValue[1])ok=false;}}else if(filterField.getType()=="date"){if(value&&typeof value=="object"){var date1=filterValue[0];var date2=filterValue[1];if(date1&&value.getTime()<date1.getTime())
ok=false;else if(date2&&value.getTime()>date2.getTime())
ok=false;}}else{var startsWith=values[i].startsWith;var _values=values[i]._values;if(values[i].anyValues){ok=false;value=(""+value).toLowerCase();for(var j=0;j<_values.length;j++){var fv=_values[j];if(startsWith){if(value.startsWith(fv)){ok=true;break;}}else if(value.indexOf(fv)>=0){ok=true;break;}else{for(ri=0;ri<values[i].regexps.length;ri++){if(value.match(values[i].regexps[ri])){ok=true;break;}}}}}}}
...
if(v==dfltValue)extra=" display-filterby-button-selected ";if(v==FILTER_ALL)extra=" display-filterby-button-all ";if(useImage){var image=null;if(imageMap)image=imageMap[v];if(!image||image=="")image=enums[j].image;if(image){buttons+=HtmlUtils.div(["fieldId",filterField.getId(),"class","display-filterby-button display-filterby-button-image"+extra,"value",v,"title",label],HtmlUtils.image(image,imageAttrs));}else{buttons+=HtmlUtils.div(["fieldId",filterField.getId(),"class","display-filterby-button display-filterby-button-image"+extra,"value",v,"title",label],label);}}else{buttons+=HtmlUtils.div(["fieldId",filterField.getId(),"class","display-filterby-button  display-filterby-button-image"+extra,"value",v],label);}}
buttons+=HtmlUtils.div(["class","display-filterby-button-label","id",this.getDomId("filterby_"+filterField.getId()+"_label")],"&nbsp;");bottom+=HtmlUtils.div(["value",dfltValue,"class","display-filterby-buttons","id",widgetId,"isButton","true","fieldId",filterField.getId()],buttons);continue;}else{var tmp=[];enums.map(e=>tmp.push(e.value));widget=HtmlUtils.select("",attrs,tmp,dfltValue);}}else if(filterField.isNumeric()){var min=0;var max=0;var cnt=0;records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(isNaN(value))return;if(cnt==0){min=value;max=value;}
else{min=Math.min(min,value);max=Math.max(max,value);}
cnt++;});var dfltValueMin=this.getProperty(filterField.getId()+".filterValueMin",min);var dfltValueMax=this.getProperty(filterField.getId()+".filterValueMax",max);widget=HtmlUtils.input("",dfltValueMin,["data-min",min,"class","display-filter-range display-filter-input","style",widgetStyle,"id",widgetId+"_min","size",4,"fieldId",filterField.getId()]);widget+=" - ";widget+=HtmlUtils.input("",dfltValueMax,["data-max",max,"class","display-filter-range display-filter-input","style",widgetStyle,"id",widgetId+"_max","size",4,"fieldId",filterField.getId()]);}else if(filterField.getType()=="date"){widget=HtmlUtils.datePicker("","",["style",widgetStyle,"id",widgetId+"_date1"])+" - "+
...
if(!value)
value=input.val();if(!value||value=="")
value=input.attr("value");if(!value)value=input.val();var fieldId=input.attr("fieldId");_this.checkFilterField(input);_this.haveCalledUpdateUI=false;if(_this.settingFilterValue){return;}
...
var values=[];parent.find(".display-filterby-button-selected").each(function(){values.push($(this).attr("value"));});if(values.length==0){parent.find(".display-filterby-button-all").addClass("display-filterby-button-selected");values.push(FILTER_ALL);}
var value=Utils.join(values,", ");parent.attr("value",value);$("#"+parent.attr("id")+"_label").html(values.includes(FILTER_ALL)?"&nbsp;":value);inputFunc(parent,null,value);});});this.jq(ID_FILTERBAR).find(".display-filter-input").keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){return;}
...
if(template!=""){var props=this.getTemplateProps(fields);var max=parseFloat(this.getProperty("maxNumber",-1));var cols=parseFloat(this.getProperty("templateColumns",-1));var colTag;if(cols>0){colTag="col-md-"+Math.round(12/cols);contents+='<div class="row wiki-row">';}
var colCnt=0;for(var rowIdx=0;rowIdx<selected.length;rowIdx++){if(max!=-1&&rowIdx>=max)break;if(cols>0){if(colCnt>=cols){colCnt=0;contents+='</div>';contents+='<div class="row wiki-row">';}
...
var row=this.getDataValues(record);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);s=this.applyRecordTemplate(row,fields,s,props);s=s.replace(/\${recordIndex}/g,(rowIdx+1));var tag=HtmlUtils.openTag("div",["style",color?"background: "+color:"","id",this.getId()+"-"+record.getId(),"title","","class","display-template-record","recordIndex",rowIdx]);if(s.startsWith("<td")){s=s.replace(/<td([^>]*)>/,"<td $1>"+tag);s=s.replace(/<\/td>$/,"</div></td>");contents+=s;}else if(s.startsWith("<tr")){s=s.replace(/<td([^>]*)>/g,"<td $1>"+tag);s=s.replace(/<\/td>/g,"</div></td>");contents+=s;}else{contents+=tag+s+"</div>"}
if(cols>0){contents+='</div>\n';}}
if(cols>0){contents+='</div>';}}
if(selected.length>0)
contents+=footerTemplate;this.writeHtml(ID_DISPLAY_CONTENTS,contents);this.makeTooltips(this.jq(ID_DISPLAY_CONTENTS).find(".display-template-record"),selected);let _this=this;this.jq(ID_DISPLAY_CONTENTS).find(".display-template-record").click(function(){var record=selected[$(this).attr("recordIndex")];_this.handleEventRecordHighlight(this,{record:record,highlight:true,immediate:true,skipScroll:true});_this.getDisplayManager().notifyEvent("handleEventRecordSelection",_this,{highlight:true,record:record});});},highlightCount:0,handleEventRecordHighlight:function(source,args){let myCount=++this.highlightCount;var id="#"+this.getId()+"-"+args.record.getId();if(this.highlightedElement){var css=this.getProperty("highlightOffCss","").split(",");if(css.length>1){for(var i=0;i<css.length;i+=2)
this.highlightedElement.css(css[i],css[i+1]);}else{this.highlightedElement.removeClass("display-template-record-highlight");}}
if(args.highlight){if(args.immediate){this.highlightElement(args);}else{setTimeout(()=>{if(myCount==this.highlightCount){this.highlightElement(args);}},500);}}else{var css=this.getProperty("highlightOffCss","").split(",");var element=$(id);if(css.length>1){for(var i=0;i<css.length;i+=2)
element.css(css[i],css[i+1]);}else{element.removeClass("display-template-record-highlight");}}},highlightElement:function(args){var id="#"+this.getId()+"-"+args.record.getId();var element=$(id);this.highlightedElement=element;var css=this.getProperty("highlightOnCss","").split(",");if(css.length>1){for(var i=0;i<css.length;i+=2)
element.css(css[i],css[i+1]);}else{element.addClass("display-template-record-highlight");}
