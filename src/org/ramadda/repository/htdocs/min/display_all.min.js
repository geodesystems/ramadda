...
header2+=HtmlUtils.span(["xstyle","display:inline-block;","id",this.getDomId(ID_HEADER2_SUFFIX)]);this.jq(ID_HEADER2).html(header2);this.initHeader2();var theDisplay=this;let macroChange=(macro,value,what)=>{if(this.settingMacroValue)return;console.log("change");if(macro.triggerReload){console.log("reloading");this.macroChanged();this.reloadData();}
this.settingMacroValue=true;var args={entryId:this.entryId,property:"macroValue",id:macro.name,what:what,value:value};this.propagateEvent("handleEventPropertyChanged",args);this.settingMacroValue=false;};macroDateIds.every(id=>{HtmlUtils.datePickerInit(id);return true;});macros.every(macro=>{$("#"+this.getDomId(macro.getId())+","+
