...
if(colorTable){var ct=Utils.ColorTables[colorTable];if(ct&&justColors)return ct.colors;if(!ct&&name){return colorTable.split(",");}
return ct;}
if(this.getProperty("colors")){var colors=this.getProperty("colors");if((typeof colors)=="object")return colors;return colors.split(",");}
...
widgetStyle="display:none;";for(var i=0;i<filterBy.length;i++){var filterField=this.getFieldById(fields,filterBy[i]);if(!filterField)continue;this.filterFields.push(filterField);var widget;var widgetId=this.getDomId("filterby_"+filterField.getId());if(filterField.getType()=="enumeration"){var dfltValue=this.getProperty(filterField.getId()+".filterValue","-all-");var filterValues=this.getProperty(filterField.getId()+".filterValues");var enums=null;if(filterValues){var toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}}
if(enums==null){var includeAll=this.getProperty(filterField.getId()+".includeAll",true);enums=includeAll?[["-all-","All"]]:[];var enumValues=[];var seen={};records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(!seen[value]){seen[value]=true;var label=value;if(label.length>20){label=label.substring(0,19)+"...";}
...
return matchedFeature;},needsData:function(){return true;},animation:new MapAnimation(this),showAllPoints:function(){if(this.lines){for(var i=0;i<this.lines.length;i++){var line=this.lines[i];line.style.display='inline';}
if(this.map.lines)
...
var colorByMap={};var colorByValues=[];var colorByMinPerc=this.getDisplayProp(source,"colorByMinPercentile",-1);var colorByMaxPerc=this.getDisplayProp(source,"colorByMaxPercentile",-1);var justOneMarker=this.getProperty("justOneMarker",false);for(var i=0;i<points.length;i++){var pointRecord=records[i];if(dateMin==null){dateMin=pointRecord.getDate();dateMax=pointRecord.getDate();}else{var date=pointRecord.getDate();if(date){if(date.getTime()<dateMin.getTime())
...
sizeBy.range=sizeBy.maxValue-sizeBy.minValue;if(dateMax){this.animation.init(dateMin,dateMax);}
...
function MapAnimation(display){var ID_RUN="maprun";var ID_NEXT="mapnext";var ID_PREV="mapprev";var ID_BEGIN="mapbegin";var ID_END="mapend";var ID_SLIDER="slider";var ID_SHOWALL="showall";var ID_ANIMATION_LABEL="animationlabel";this.steps=parseFloat(display.getProperty("animationSteps",60));this.windowUnit=display.getProperty("animationWindow","");if(this.windowUnit!=""){if(this.windowUnit=="decade"){this.window=1000*60*60*24*365*10;}else if(this.windowUnit=="halfdecade"){this.window=1000*60*60*24*365*5;}else if(this.windowUnit=="year"){this.window=1000*60*60*24*366;}else if(this.windowUnit=="month"){this.window=1000*60*60*24*32;}else if(this.windowUnit=="day"){this.window=1000*60*60*25;}else if(this.windowUnit=="hour"){this.window=1000*60*61;}else if(this.windowUnit=="minute"){this.window=1000*61;}else{this.window=1001;}}else if(this.steps>0){this.window=this.dateRange/this.steps;}
this.display=display;$.extend(this,{running:false,inAnimation:false,begin:null,end:null,dateMin:null,dateMax:null,dateRange:0,dateFormat:display.getProperty("animationDateFormat","yyyyMMdd"),mode:display.getProperty("animationMode","cumulative"),speed:parseInt(display.getProperty("animationSpeed",250)),toggleAnimation:function(){this.running=!this.running;this.btnRun.html(HtmlUtils.getIconImage(this.running?"fa-stop":"fa-play"));if(this.running)
this.startAnimation();},getDomId:function(id){return this.display.getDomId(id);},jq:function(id){return this.display.jq(id);},init:function(dateMin,dateMax){this.dateMin=dateMin;this.dateMax=dateMax;this.dateRange=this.dateMax.getTime()-this.dateMin.getTime();this.begin=this.dateMin;this.end=this.dateMin;if(this.label)
this.label.html(this.formatAnimationDate(this.dateMin)+" - "+this.formatAnimationDate(this.dateMax));},makeControls:function(){var buttons=HtmlUtils.span(["id",this.getDomId(ID_BEGIN),"title","Go to beginning"],HtmlUtils.getIconImage("fa-fast-backward"))+
...
HtmlUtils.span(["id",this.getDomId(ID_ANIMATION_LABEL),"class","display-map-animation-label"]);buttons=HtmlUtils.div(["class","display-map-animation-buttons"],buttons);this.jq(ID_TOP_LEFT).append(buttons);HtmlUtils.rangeInputInit(this.getDomId(ID_SLIDER));this.btnRun=this.jq(ID_RUN);this.btnPrev=this.jq(ID_PREV);this.btnNext=this.jq(ID_NEXT);this.btnBegin=this.jq(ID_BEGIN);this.btnEnd=this.jq(ID_END);this.btnShowAll=this.jq(ID_SHOWALL);this.label=this.jq(ID_ANIMATION_LABEL);this.btnRun.button().click(()=>{this.toggleAnimation();});this.btnBegin.button().click(()=>{this.begin=this.dateMin;if(this.mode=="sliding"){this.end=new Date(this.dateMin.getTime()+this.window);}else{this.end=new Date(this.dateMin.getTime()+this.window);}
this.stopAnimation();});this.btnEnd.button().click(()=>{this.end=this.dateMax;if(this.mode=="sliding"){this.begin=new Date(this.dateMax.getTime()-this.window);}else{this.end=this.dateMax;}
this.stopAnimation();});this.btnPrev.button().click(()=>{if(this.mode=="sliding"){if(this.begin.getTime()>this.dateMin.getTime()+this.window){this.end=this.begin;this.begin=new Date(this.begin.getTime()-this.window);}}else{if(this.end.getTime()>this.begin.getTime()+this.window){this.end=new Date(this.end.getTime()-this.window);}else{return;}}
this.stopAnimation();});this.btnNext.button().click(()=>{this.doNext();});this.btnShowAll.button().click(()=>{this.begin=this.dateMin;this.end=this.dateMin;this.inAnimation=false;this.label.html(this.formatAnimationDate(this.dateMin)+" - "+this.formatAnimationDate(this.dateMax));this.running=false;this.btnRun.html(HtmlUtils.getIconImage("fa-play"));this.display.showAllPoints();});},doNext:function(){if(this.mode=="sliding"){if(this.end.getTime()<this.dateMax.getTime()-this.window){this.begin=this.end;this.end=new Date(this.end.getTime()+this.window);}}else{if(this.end.getTime()<this.dateMax.getTime()-this.window){this.end=new Date(this.end.getTime()+this.window);}else{return;}}
this.stopAnimation();},startAnimation:function(justOneStep,back){if(!this.display.points){return;}
if(!this.dateMax)return;if(!justOneStep)
this.running=true;if(!this.inAnimation){this.inAnimation=true;this.label.html("");var date=this.dateMin;this.begin=date;var unit=this.windowUnit;if(unit!=""){var tmp=0;if(unit=="decade"){this.begin=new Date(date.getUTCFullYear(),0);}else if(unit=="year"){this.begin=new Date(date.getUTCFullYear(),0);}else if(unit=="month"){this.begin=new Date(date.getUTCFullYear(),date.getMonth());}else if(unit=="day"){this.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay());}else if(unit=="hour"){this.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours());}else if(unit=="minute"){this.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getMinutes());}else{this.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getSeconds());}}
this.end=this.begin;for(var i=0;i<this.display.points.length;i++){var point=this.display.points[i];point.style.display='none';}
if(this.display.map.circles)
this.display.map.circles.redraw();}
this.stepAnimation(back);},stopAnimation:function(){this.btnRun.html(HtmlUtils.getIconImage("fa-play"));this.running=false;this.applyAnimation();},stepAnimation:function(back){if(!this.display.points||!this.dateMax)return;var oldBegin=this.begin;var oldEnd=this.end;var unit=this.windowUnit;var date=back?new Date(this.begin.getTime()-this.window):new Date(this.end.getTime()+this.window);var newDate;if(unit=="decade"){newDate=new Date(date.getUTCFullYear(),0);}else if(unit=="year"){newDate=new Date(date.getUTCFullYear(),0);}else if(unit=="month"){newDate=new Date(date.getUTCFullYear(),date.getMonth());}else if(unit=="day"){newDate=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay());}else if(unit=="hour"){newDate=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours());}else if(unit=="minute"){newDate=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getMinutes());}else{newDate=new Date(this.end.getTime()+this.window);}
if(this.mode=="sliding"){if(back){this.begin=newDate;this.end=oldBegin;}else{this.begin=oldEnd;this.end=newDate;}
if(this.begin.getTime()<this.dateMin.getTime()){this.begin=this.dateMin;this.end=new Date(this.dateMin.getTime()+this.window);}
if(this.begin.getTime()>this.dateMax.getTime()){this.end=this.dateMax;this.begin=new Date(this.dateMax.getTime()-this.window);}
if(this.end.getTime()>this.dateMax.getTime()){}}else{this.end=newDate;}
this.applyAnimation();},applyAnimation:function(){var windowStart=this.begin.getTime();var windowEnd=this.end.getTime();var atLoc={};for(var i=0;i<this.display.lines.length;i++){var line=this.display.lines[i];if(line.date<windowStart||line.date>windowEnd){line.style.display='none';continue;}
line.style.display='inline';}
for(var i=0;i<this.display.points.length;i++){var point=this.display.points[i];if(point.date<windowStart||point.date>windowEnd){point.style.display='none';continue;}
...
if(this.display.map.circles)
this.display.map.circles.redraw();if(this.display.map.lines)
this.display.map.lines.redraw();this.label.html(this.formatAnimationDate(this.begin)+" - "+this.formatAnimationDate(this.end));if(windowEnd<=this.dateMax.getTime()){if(this.running){setTimeout(()=>{if(!this.running)return;this.stepAnimation()},this.speed);}}else{this.running=false;this.inAnimation=false;this.btnRun.html(HtmlUtils.getIconImage("fa-play"));}
this.display.applyVectorMap(true);},formatAnimationDate:function(d){if(this.dateFormat=="yyyy"){return Utils.formatDateYYYY(d);}else if(this.dateFormat=="yyyyMMdd"){return Utils.formatDateYYYYMMDD(d);}else{return Utils.formatDate(d);}},});}
