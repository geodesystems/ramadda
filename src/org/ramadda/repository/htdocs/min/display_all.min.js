...
if(!chunk){chunk=new NotebookChunk(cell,props);chunks.push(chunk);if(prevChunk)prevChunk.div.jq().after(chunk.div.toString());else cell.output.html(chunk.div.toString());}else{chunk.initChunk(props);}
...
if(error){this.handleError(chunk,error,"io");return;}else{this.notebook.log("Loaded: "+url+msgExtra,"output","io");}}},processMd:async function(chunk){var content=chunk.content;this.rawOutput+=content+"\n";if(content.match("%\n*$")){content=content.trim();content=content.substring(0,content.length-1);}
...
function NotebookChunk(cell,props){this.div=new Div(null,"display-notebook-chunk");this.cell=cell;$.extend(this,{getContent:function(){var content=this.content;for(name in this.cell.notebook.globals){var value=this.cell.notebook.getGlobalValue(name);if(typeof value=="object"){value=Utils.formatJson(value);}
content=content.replace("${"+name.trim()+"}",value);}
return content;},initChunk:function(props){if(props["skipoutput"]===true){this.div.set("");this.div=new Div();}
...
fields=allFields;this.groupField=this.getFieldById(fields,this.getProperty("groupBy"));this.groupByFields=this.getFieldsByIds(fields,this.getProperty("groupByFields","",true));this.imageField=this.getFieldOfType(fields,"image");this.urlField=this.getFieldOfType(fields,"url");this.tooltipFields=this.getFieldsByIds(fields,this.getProperty("tooltipFields","",true));this.labelField=this.getFieldById(fields,this.getProperty("labelField",null,true));this.captionFields=this.getFieldsByIds(fields,this.getProperty("captionFields","",true));this.captionTemplate=this.getProperty("captionTemplate",null,true);if(this.captionFields.length==0)this.captionFields=this.tooltipFields;this.sortField=this.getFieldById(fields,this.getProperty("sortField",null,true));this.colorByField=this.getFieldById(fields,this.getProperty("colorBy",null,true));this.colorList=this.getColorTable(true);this.foregroundList=this.getColorTable(true,"foreground");if(!this.getProperty("showImages",true))this.imageField=null;if(!this.imageField){if(this.captionFields.length==0){this.displayError("No image or caption fields specified");return;}}
if(this.sortField){var sortAscending=this.getProperty("sortAscending",true);this.records.sort((a,b)=>{var row1=this.getDataValues(a);var row2=this.getDataValues(b);var result=0;var v1=row1[this.sortField.getIndex()];var v2=row2[this.sortField.getIndex()];if(v1<v2)result=-1;else if(v1>v2)result=1;if(sortAscending)return result;return result==0?result:-result;});}
this.searchFields=[];var contents="";var searchBy=this.getProperty("searchFields","",true).split(",");var searchBar="";if(this.groupByFields.length>0){var enums=[];this.groupByFields.map(field=>{enums.push([field.getId(),field.getLabel()]);});searchBar+="Group by: "+HtmlUtils.select("",["id",this.getDomId(ID_GROUPBY_FIELDS)],enums)+"&nbsp;";}
for(var i=0;i<searchBy.length;i++){var searchField=this.getFieldById(fields,searchBy[i]);if(!searchField)continue;this.searchFields.push(searchField);var widget;var widgetId=this.getDomId("searchby_"+searchField.getId());if(searchField.getType()=="enumeration"){var enums=[["","All"]];this.records.map(record=>{var value=this.getDataValues(record)[searchField.getIndex()];if(!enums.includes(value))enums.push(value);});widget=HtmlUtils.select("",["id",widgetId],enums);}else{widget=HtmlUtils.input("","",["id",widgetId]);}
widget=searchField.getLabel()+": "+widget;if(i==0)searchBar+=" Display: ";searchBar+=widget+"&nbsp;&nbsp;";}
contents+=HtmlUtils.div(["id",this.getDomId(ID_SEARCHBAR)],"<center>"+searchBar+"</center>");contents+=HtmlUtils.div(["id",this.getDomId(ID_RESULTS)]);this.writeHtml(ID_DISPLAY_CONTENTS,contents);this.jq(ID_SEARCHBAR).find("select").selectBoxIt({});let _this=this;this.jq(ID_SEARCHBAR).find("input, input:radio,select").change(function(){if($(this).attr("id")==_this.getDomId(ID_GROUPBY_FIELDS)){_this.groupField=_this.getFieldById(fields,$(this).val());}
_this.doSearch();});this.displaySearchResults(this.records);},doSearch:function(){var records=[];var values=[];for(var i=0;i<this.searchFields.length;i++){var searchField=this.searchFields[i];values.push($("#"+this.getDomId("searchby_"+searchField.getId())).val());}
...
this.displaySearchResults(records);},displaySearchResults:function(records){var fontSize=this.getProperty("fontSize",null);var cardStyle=this.getProperty("cardStyle",null);var width=this.getProperty("imageWidth","50");var margin=this.getProperty("imageMargin","0");var groups={};var groupCnt={};var groupHeaders=[];if(!this.groupField){groups[""]="";groupCnt[""]=0;}
var colorMap={};var colorCnt=0;for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);var contents="";var tooltip="";this.tooltipFields.map(field=>{if(tooltip!="")tooltip+="&#10;";tooltip+=field.getValue(row);});var label="";var caption="";if(this.captionFields.length>0){if(this.captionTemplate)caption=this.captionTemplate;this.captionFields.map(field=>{if(this.captionTemplate)
...
if(this.labelField)label="<br>"+row[this.labelField.getIndex()];var html="";if(this.imageField){var img=row[this.imageField.getIndex()];var attrs=["class","display-cards-popup","data-fancybox",this.getDomId("gallery")];attrs.push("data-caption");attrs.push(caption);img=HtmlUtils.href(img,HtmlUtils.image(img,["width",width]),attrs)+label;html=HtmlUtils.div(["class","display-cards-item","title",tooltip,"style","margin:"+margin+"px;"],img);}else{var style="";if(fontSize){style+=" font-size:"+fontSize+"; ";}
...
if(cardStyle)
style+=cardStyle;if(rowIdx<3)console.log(style);var attrs=["title",tooltip,"class","ramadda-gridbox display-cards-card","style",style];html=HtmlUtils.div(attrs,caption);}
if(this.groupField){var groupOn=row[this.groupField.getIndex()];if(!groups[groupOn]){groupCnt[groupOn]=0;groups[groupOn]="";groupHeaders.push(groupOn);}
groupCnt[groupOn]++;groups[groupOn]+=html;}else{groups[""]+=html;}
}
var html="";if(!this.groupField)html=groups[""];else{groupHeaders.sort();var width=groupHeaders.length==0?"100%":100/groupHeaders.length;html+="<table width=100%><tr valign=top>";for(var i=0;i<groupHeaders.length;i++){var header=groupHeaders[i];var cnt=groupCnt[header]
html+="<td width="+width+"%><center><b>&nbsp;"+header+" ("+cnt+")&nbsp;</b></center>"+HtmlUtils.div(["class","display-cards-items"],groups[header])+"</td>";}
html+="</tr></table>";}
this.writeHtml(ID_RESULTS,html);this.jq(ID_RESULTS).find("a.display-cards-popup").fancybox({caption:function(instance,item){return $(this).data('caption')||'';}});}});}
