...
if(v==dfltValue)extra=" display-filterby-button-selected ";if(useImage){var image=null;if(imageMap)image=imageMap[v];if(!image||image=="")image=enums[j].image;if(image){buttons+=HtmlUtils.div(["class","display-filterby-button display-filterby-button-image"+extra,"value",v,"title",label],HtmlUtils.image(image,imageAttrs));}else{buttons+=HtmlUtils.div(["class","display-filterby-button display-filterby-button-image"+extra,"value",v,"title",label],label);}}else{buttons+=HtmlUtils.div(["class","display-filterby-button  display-filterby-button-image"+extra,"value",v],label);}}
...
_this.propagateEvent("handleEventPropertyChanged",args);_this.settingFilterValue=false;};this.jq(ID_FILTERBAR).find(".display-filterby-buttons").each(function(){let parent=$(this);$(this).find(".display-filterby-button").click(function(){parent.find(".display-filterby-button").removeClass("display-filterby-button-selected");$(this).addClass("display-filterby-button-selected");var value=$(this).attr("value");parent.attr("value",value);$("#"+parent.attr("id")+"_label").html(value==FILTER_ALL?"&nbsp;":value);inputFunc(parent,null,value);});});this.jq(ID_FILTERBAR).find(".display-filter-input").keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){return;}
