...
var ID_FIELDS="fields";var ID_HEADER="header";var ID_TITLE=ATTR_TITLE;var ID_TITLE_EDIT="title_edit";var ID_TOP_RIGHT="topright";var ID_TOP_LEFT="topleft";var ID_DETAILS="details";var ID_DISPLAY_CONTENTS="contents";var ID_DISPLAY_TOP="top";var ID_DISPLAY_BOTTOM="bottom";var ID_GROUP_CONTENTS="group_contents";var ID_DETAILS_MAIN="detailsmain";var ID_TOOLBAR="toolbar";var ID_TOOLBAR_INNER="toolbarinner";var ID_LIST="list";var ID_DIALOG="dialog";var ID_DIALOG_TABS="dialog_tabs";var ID_DIALOG_BUTTON="dialog_button";var ID_FOOTER="footer";var ID_FOOTER_LEFT="footer_left";var ID_FOOTER_RIGHT="footer_right";var ID_MENU_BUTTON="menu_button";var ID_MENU_OUTER="menu_outer";var ID_MENU_INNER="menu_inner";var ID_REPOSITORY="repository";var displayDebug=false;var PROP_DISPLAY_FILTER="displayFilter";var PROP_EXCLUDE_ZERO="excludeZero";var PROP_DIVID="divid";var PROP_FIELDS="fields";var PROP_LAYOUT_HERE="layoutHere";var PROP_HEIGHT="height";var PROP_WIDTH="width";function initRamaddaDisplays(){ramaddaCheckForResize();if(window.globalDisplaysList==null){return;}
...
if(this.displayParent!=null){return this.displayParent.getProperty(key,dflt);}
if(this.getDisplayManager){return this.getDisplayManager().getProperty(key,dflt);}
value=getGlobalDisplayProperty(key);if(value)return value;return dflt;}});}
...
return titleToShow;},handleEventFieldValueSelected:function(source,args){this.setProperty("filterPattern",args.value);this.setProperty("patternFilterField",args.field.getId());this.updateUI();},handleEventPropertyChanged:function(source,prop){if(prop.property=="filterPattern"){this.jq("filterValueMenu").val(prop.value);}
...
if(args.selected){this.jq(ID_TITLE).addClass("display-title-select");}else{this.jq(ID_TITLE).removeClass("display-title-select");}},highlightEntry:function(entry){this.jq(ID_TITLE).addClass("display-title-select");},getEntries:function(){return this.entries;},getDisplayEntry:async function(){var entries=this.getEntries();if(entries!=null&&entries.length>0){return entries[0];}
if(this.entryId){var entry;await this.getRamadda().getEntry(this.entryId,e=>{entry=e});}
return null;},hasEntries:function(){return this.entries!=null&&this.entries.length>0;},getWaitImage:function(){return HtmlUtils.image(ramaddaBaseUrl+"/icons/progress.gif");},getLoadingMessage:function(msg){if(!msg)msg="Loading data...";return HtmlUtils.div(["text-align","center"],this.getMessage("&nbsp;"+msg));},getMessage:function(msg){return HtmlUtils.div([ATTR_CLASS,"display-message"],msg);},getFieldValue:function(id,dflt){var jq=$("#"+id);if(jq.size()>0){return jq.val();}
return dflt;},getFooter:function(){return HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER),ATTR_CLASS,"display-footer"],HtmlUtils.leftRight(HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER_LEFT),ATTR_CLASS,"display-footer-left"],""),HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER_RIGHT),ATTR_CLASS,"display-footer-right"],"")));},checkFixedLayout:function(){if(this.getIsLayoutFixed()){var divid=this.getProperty(PROP_DIVID);if(divid!=null){var html=this.getHtml();$("#"+divid).html(html);}else{console.log("error: no div defined for display:"+this.getType());}}},shouldSkipField:function(field){if(this.skipFields&&!this.skipFieldsList){this.skipFieldsList=this.skipFields.split(",");}
...
return[];},getFieldById:function(fields,id){if(!id)return null;if(!fields){var pointData=this.getData();if(pointData==null)return null;fields=pointData.getRecordFields();}
...
var list=[];var numeric=(type=="numeric");for(a in fields){var field=fields[a];if(type==null)return field;if(numeric){if(field.isFieldNumeric()){list.push(field);}}else if(field.getType()==type){list.push(field);}}
...
return{values:values,min:min,max:max};},filterData:function(dataList,fields){if(!dataList){var pointData=this.getData();if(pointData==null)return null;dataList=pointData.getRecords();}
if(!fields){fields=pointData.getRecordFields();}
var patternFieldId=this.getProperty("patternFilterField");var numericFieldId=this.getProperty("numericFilterField");var pattern=this.getProperty("filterPattern");var prefix=this.getProperty("filterPatternPrefix");if(prefix)pattern=prefix+pattern;var suffix=this.getProperty("filterPatternSuffix");if(suffix)pattern=pattern+suffix;var notPattern=false;if(pattern){notPattern=pattern.startsWith("!");if(notPattern){pattern=pattern.substring(1);}}
var filterSValue=this.getProperty("numericFilterValue");var filterOperator=this.getProperty("numericFilterOperator","<");if((numericFieldId||patternFieldId)&&(pattern||(filterSValue&&filterOperator))){var patternFields=this.getFieldsByIds(fields,patternFieldId);var numericField=null;for(var i=0;i<fields.length;i++){if(!numericField&&(fields[i].getId()==numericFieldId||numericFieldId=="#"+(i+1))){numericField=fields[i];}
if(patternField&&numericField)break;}
if(patternFields.length||numericField){var list=[];var filterValue;if(filterSValue){filterValue=parseFloat(filterSValue);}
var isPointRecord=false;if(dataList.length>0)
isPointRecord=dataList[0].isPointRecord;for(var i=0;i<dataList.length;i++){var obj=dataList[i];var row=this.getDataValues(obj);var array=row;if(!isPointRecord&&i==0){list.push(obj);continue;}
var ok=false;if(numericField&&filterSValue&&filterOperator){var value=parseFloat(array[numericField.getIndex()]);var filterValue=parseFloat(filterSValue);if(filterOperator=="<"){ok=value<filterValue;}else if(filterOperator=="<="){ok=value<=filterValue;}else if(filterOperator=="=="){ok=value==filterValue;}else if(filterOperator==">"){ok=value>filterValue;}else if(filterOperator==">="){ok=value>=filterValue;}
if(!ok){continue;}}
if(patternFields.length&&pattern){for(var fieldIdx=0;fieldIdx<patternFields.length;fieldIdx++){var patternField=patternFields[fieldIdx];var value=""+array[patternField.getIndex()];ok=value.match(pattern);if(notPattern)ok=!ok;if(ok)break;}}
if(ok){list.push(obj);}}
dataList=list;}}
var stride=parseInt(this.getProperty("stride",-1));if(stride>0){var list=[];var cnt=0;for(var i=0;i<dataList.length;i+=(stride+1)){list.push(dataList[i]);}
dataList=list;}
...
HtmlUtils.sqt("table")+","+HtmlUtils.sqt(jsonUrl)+");"],HtmlUtils.image(ramaddaBaseUrl+"/icons/table.png",["border",0,ATTR_TITLE,"Create Tabular Display"])));var x;toolbarItems.push(x=HtmlUtils.tag(TAG_A,["onclick",get+".createDisplay("+HtmlUtils.sqt(entry.getFullId())+","+
HtmlUtils.sqt("linechart")+","+HtmlUtils.sqt(jsonUrl)+");"],HtmlUtils.image(ramaddaBaseUrl+"/icons/chart_line_add.png",["border",0,ATTR_TITLE,"Create Chart"])));}
toolbarItems.push(HtmlUtils.tag(TAG_A,["onclick",get+".createDisplay("+HtmlUtils.sqt(entry.getFullId())+","+
HtmlUtils.sqt("entrydisplay")+","+HtmlUtils.sqt(jsonUrl)+");"],HtmlUtils.image(ramaddaBaseUrl+"/icons/layout_add.png",["border",0,ATTR_TITLE,"Show Entry"])));if(entry.getFilesize()>0){toolbarItems.push(HtmlUtils.tag(TAG_A,[ATTR_HREF,entry.getResourceUrl()],HtmlUtils.image(ramaddaBaseUrl+"/icons/download.png",["border",0,ATTR_TITLE,"Download ("+entry.getFormattedFilesize()+")"])));}
...
var props={showMenu:true,sourceEntry:entry,entryId:entry.getId(),showTitle:true,showDetails:true,title:entry.getName(),};if(displayProps)$.extend(props,displayProps);if(displayType!=DISPLAY_ENTRYLIST){if(jsonUrl==null){jsonUrl=this.getPointUrl(entry);}
...
form+="</form>";tabTitles.push("Display");tabContents.push(form);},popup:function(srcId,popupId){var popup=GuiUtils.getDomObject(popupId);var srcObj=GuiUtils.getDomObject(srcId);if(!popup||!srcObj)return;var myalign='left top';var atalign='left bottom';showObject(popup);jQuery("#"+popupId).position({of:jQuery("#"+srcId),my:myalign,at:atalign,collision:"none none"});jQuery("#"+popupId).position({of:jQuery("#"+srcId),my:myalign,at:atalign,collision:"none none"});$("#"+popupId).draggable();},checkLayout:function(){},displayData:function(){},createUI:function(){this.checkFixedLayout();},setDisplayReady:function(){this.displayReady=true;},getDisplayReady:function(){return this.displayReady;},pageHasLoaded:function(){this.setDisplayReady(true);},initDisplay:function(){this.createUI();this.checkFilterValueMenu();},checkFilterValueMenu:function(){if(!this.getProperty("showFilterWidget",true))return;var filterValues=this.getProperty("filterValues");var filterValuesLabel=this.getProperty("filterValuesLabel","");if(!filterValues){var filterFieldId=this.getProperty("patternFilterField");if(filterFieldId){var filterField=this.getFieldById(null,filterFieldId);if(filterField){var pointData=this.getData();if(pointData==null)return;if(filterValuesLabel=="")filterValuesLabel=filterField.getLabel()+": ";filterValues=this.getColumnValues(pointData.getRecords(),filterField).values;filterValues=Utils.getUniqueValues(filterValues);filterValues.unshift("");}}}
var filterValue=this.getProperty("filterPattern");if(filterValues){var toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}
var menu="<select class='ramadda-pulldown' id='"+this.getDomId("filterValueMenu")+"'>";for(var i=0;i<toks.length;i++){var extra="";if(filterValue==toks[i])extra="selected ";if(toks[i]==""){menu+="<option value='' "+extra+">"+"All"+"</option>\n";}else{menu+="<option "+extra+">"+toks[i]+"</option>\n";}}
menu+="</select>";let _this=this;this.writeHtml(ID_TOP_RIGHT,filterValuesLabel+menu);this.jq("filterValueMenu").change(function(){var value=$(this).val();if(_this.getProperty("filterPattern")==value)return;if(value=="")value=null;_this.setProperty("filterPattern",value);_this.updateUI();_this.propagateEvent("handleEventPropertyChanged",{property:"filterPattern",value:value});});}},updateUI:function(){},getDoBs:function(){if(!(typeof this.dobs==='undefined')){return dobs;}
...
left=HtmlUtils.div(["id",this.getDomId(ID_TOP_LEFT)],left);var right=HtmlUtils.div(["id",this.getDomId(ID_TOP_RIGHT)],"");html+=HtmlUtils.leftRightTable(left,right,{valign:"bottom"});var contents=this.getContentsDiv();html+=contents;html+=HtmlUtils.closeTag(TAG_DIV);if(dobs){html+=HtmlUtils.closeTag(TAG_DIV);}
return html;},makeToolbar:function(props){var toolbar="";var get=this.getGet();var addLabel=props.addLabel;var images=[];var calls=[];var labels=[];if(!this.getIsLayoutFixed()){calls.push("removeRamaddaDisplay('"+this.getId()+"')");images.push(ramaddaBaseUrl+"/icons/page_delete.png");labels.push("Delete display");}
calls.push(get+".copyDisplay();");images.push(ramaddaBaseUrl+"/icons/page_copy.png");labels.push("Copy Display");if(this.jsonUrl!=null){calls.push(get+".fetchUrl('json');");images.push(ramaddaBaseUrl+"/icons/json.png");labels.push("Download JSON");calls.push(get+".fetchUrl('csv');");images.push(ramaddaBaseUrl+"/icons/csv.png");labels.push("Download CSV");}
for(var i=0;i<calls.length;i++){var inner=HtmlUtils.image(images[i],[ATTR_TITLE,labels[i],ATTR_CLASS,"display-dialog-header-icon"]);if(addLabel)inner+=" "+labels[i]+"<br>";toolbar+=HtmlUtils.onClick(calls[i],inner);}
return toolbar;},makeDialog:function(){var html="";html+=HtmlUtils.div([ATTR_ID,this.getDomId(ID_HEADER),ATTR_CLASS,"display-header"]);var close=HtmlUtils.onClick("$('#"+this.getDomId(ID_DIALOG)+"').hide();",HtmlUtils.image(icon_close,[ATTR_CLASS,"display-dialog-close",ATTR_TITLE,"Close Dialog"]));var right=close;var left="";var header=HtmlUtils.div([ATTR_CLASS,"display-dialog-header"],HtmlUtils.leftRight(left,right));var tabTitles=[];var tabContents=[];this.getDialogContents(tabTitles,tabContents);tabTitles.push("Edit");tabContents.push(this.makeToolbar({addLabel:true}));var tabLinks="<ul>";var tabs="";for(var i=0;i<tabTitles.length;i++){var id=this.getDomId("tabs")+i;tabLinks+=HtmlUtils.tag("li",[],HtmlUtils.tag("a",["href","#"+id],tabTitles[i]));tabLinks+="\n";var contents=HtmlUtils.div([ATTR_CLASS,"display-dialog-tab"],tabContents[i]);tabs+=HtmlUtils.div(["id",id],contents);tabs+="\n";}
...
height=height+"px";return height;},getContentsStyle:function(){var style="";var height=this.getHeightForStyle();if(height){style+=" height:"+height+";";}
...
this.setContents(this.getMessage(msg));},clearCache:function(){},pointDataLoaded:function(pointData,url,reload){this.inError=false;this.clearCache();if(!reload){this.addData(pointData);this.checkFilterValueMenu();}
...
display.notifyEvent(func,source,data);}},getDisplaysToLayout:function(){var result=[];for(var i=0;i<this.displays.length;i++){if(this.displays[i].getIsLayoutFixed())continue;result.push(this.displays[i]);}
...
if(this.layout==LAYOUT_TABLE){if(displaysToLayout.length==1){html+=HtmlUtils.div(["class"," display-wrapper"],displaysToLayout[0].getHtml());}else{var weight=12/this.columns;var i=0;var map={};for(;i<displaysToLayout.length;i++){var d=displaysToLayout[i];if(Utils.isDefined(d.column)&&Utils.isDefined(d.row)&&d.columns>=0&&d.row>=0){var key=d.column+"_"+d.row;if(map[key]==null)map[key]=[];map[key].push(d);}}
...
html+=HtmlUtils.tag(TAG_LI,[],HtmlUtils.tag(TAG_A,["href","#"+tabId+"-"+cnt],label));hidden+=HtmlUtils.div(["id",tabId+"-"+cnt,"class","ui-tabs-hide"],display.getHtml());cnt++;}
...
this.writeHtml(ID_DISPLAYS,html);if(this.layout==LAYOUT_TABS){$("#"+tabId).tabs({});}
...
return false;},startLoading:function(){this.loadingCnt++;},stopLoading:function(){this.loadingCnt--;},loadData:function(display,reload){if(this.url==null){console.log("No URL");return;}
...
console.log("load data:"+url);var jqxhr=$.getJSON(url,function(data){if(GuiUtils.isJsonError(data)){display.pointDataLoadFailed(data);return;}
...
pointData.stopLoading();}).fail(function(jqxhr,textStatus,error){var err=textStatus+": "+error;console.log("JSON error:"+err);display.pointDataLoadFailed(err);pointData.stopLoading();});}});}
...
return true;},initData:function(){var pointData1=this.pointDataList[0];if(this.pointDataList.length==1){this.records=pointData1.getRecords();this.recordFields=pointData1.getRecordFields();}else if(this.pointDataList.length>1){var results=this.combineData(pointData1,this.pointDataList[1]);this.records=results.records;this.recordFields=results.recordFields;}
this.display.pointDataLoaded(this);},combineData:function(pointData1,pointData2){var records1=pointData1.getRecords();var records2=pointData2.getRecords();var newRecords=[];var newRecordFields;if(records1.length!=records2.length){console.log("bad records:"+records1.length+" "+records2.length);}
...
function RecordField(props){$.extend(this,{isDate:props.type=="date",isLatitude:false,isLongitude:false,isElevation:false,});$.extend(this,props);$.extend(this,{isNumeric:props.type=="double"||props.type=="integer",properties:props});RamaddaUtil.defineMembers(this,{getIndex:function(){return this.index;},isFieldGeo:function(){return this.isFieldLatitude()||this.isFieldLongitude()||this.isFieldElevation();},isFieldLatitude:function(){return this.isLatitude||this.id.toLowerCase()=="latitude";},isFieldLongitude:function(){return this.isLongitude||this.id.toLowerCase()=="longitude";},isFieldElevation:function(){return this.isElevation||this.id.toLowerCase()=="elevation"||this.id.toLowerCase()=="altitude";},isFieldNumeric:function(){return this.isNumeric;},isFieldDate:function(){return this.isDate;},isChartable:function(){return this.chartable;},getSortOrder:function(){return this.sortorder;},getId:function(){return this.id;},getUnitLabel:function(){var label=this.getLabel();if(this.unit&&this.unit!="")
label=label+" ["+this.unit+"]";return label;},getUnitSuffix:function(){if(this.unit&&this.unit!="")
return" ["+this.unit+"]";return"";},getLabel:function(){if(this.label==null||this.label.length==0)return this.id;return this.label;},setLabel:function(l){this.label=l;},getType:function(){return this.type;},getMissing:function(){return this.missing;},setUnit:function(u){this.unit=u;},getUnit:function(){return this.unit;}});}
function PointRecord(lat,lon,elevation,time,data){this.isPointRecord=true;RamaddaUtil.defineMembers(this,{latitude:lat,longitude:lon,elevation:elevation,recordTime:time,data:data,getData:function(){return this.data;},allZeros:function(){var tuple=this.getData();var allZeros=false;var nums=0;var nonZero=0;for(var j=0;j<tuple.length;j++){if(typeof tuple[j]=="number"){nums++;if(!isNaN(tuple[j])&&tuple[j]!=0){nonZero++;break;}}}
...
var DISPLAY_NOTEBOOK="notebook";addGlobalDisplayType({type:DISPLAY_NOTEBOOK,label:"Notebook",requiresData:false,category:"Misc"});function RamaddaNotebookDisplay(displayManager,id,properties){var ID_NOTEBOOK="notebook";var ID_IMPORTS="imports";var ID_CELLS="cells";var ID_INPUTS="inputs";var ID_OUTPUTS="inputs";var ID_CELL="cell";var ID_MENU="menu";let SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_NOTEBOOK,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{cells:[],cellCount:0,runOnLoad:true,displayMode:false,fetchedNotebook:false,currentEntries:{},globals:{},baseEntries:{},layout:"sidebyside",columns:1,initDisplay:async function(){this.createUI();var imports=HtmlUtils.div(["id",this.getDomId(ID_IMPORTS)]);var contents=imports+HtmlUtils.div([ATTR_CLASS,"display-notebook-cells",ATTR_ID,this.getDomId(ID_CELLS)],"&nbsp;&nbsp;Loading...");var popup=HtmlUtils.div(["class","ramadda-popup",ATTR_ID,this.getDomId(ID_MENU)]);contents=HtmlUtils.div([ATTR_ID,this.getDomId(ID_NOTEBOOK)],popup+contents);this.setContents(contents);this.jq(ID_NOTEBOOK).hover(()=>{},()=>{this.jq(ID_MENU).hide()});if(!this.fetchedNotebook){if(!this.fetchingNotebook){this.fetchingNotebook=true;await Utils.importJS(ramaddaBaseHtdocs+"/lib/ace/src-min/ace.js");setTimeout(()=>this.fetchNotebook(1),10);}}else{this.layoutCells();}},fetchNotebook:async function(cnt){if(!window["ace"]){if(cnt>50){alert("Could not load ace.js");return;}
setTimeout(()=>this.fetchNotebook(cnt+1),cnt*10);return;}
ace.config.set('basePath',ramaddaBaseHtdocs+"/lib/ace/src-min");let _this=this;this.fetchedNotebook=true;await this.getEntry(this.getProperty("entryId",""),entry=>{this.baseEntry=entry;});await this.baseEntry.getRoot(entry=>{this.rootEntry=entry;});var id=this.getProperty("entryId","");var url=ramaddaBaseUrl+"/getnotebook?entryid="+id;url+="&notebookId="+this.getProperty("notebookId","default_notebook");var jqxhr=$.getJSON(url,function(data){_this.loadJson(data);}).fail(function(){var props={showInput:true,}
this.addCell("init cell",props,false).run();this.cells[0].focus();});},addGlobal:function(name,value){this.globals[name]=value;},getBaseEntry:function(){return this.baseEntry;},getRootEntry:function(){return this.rootEntry;},getPopup:function(){return this.jq(ID_MENU);},loadJson:async function(data){if(data.error){this.setContents(_this.getMessage("Failed to load notebook: "+data.error));return;}
if(Utils.isDefined(data.runOnLoad)){this.runOnLoad=data.runOnLoad;}
if(Utils.isDefined(data.displayMode)){this.displayMode=data.displayMode;}
if(Utils.isDefined(data.columns)){this.columns=data.columns;}
if(Utils.isDefined(data.layout)){this.layout=data.layout;}
...
this.addCell("",props,false);this.layoutCells();this.cells[0].focus();}
...
this.currentEntries[a]=this.baseEntries[a];},saveNotebook:function(output){var json=this.getJson(output);json=JSON.stringify(json,null,2);var args={entryid:this.getProperty("entryId",""),notebookId:this.getProperty("notebookId","default_notebook"),notebook:json};var url=ramaddaBaseUrl+"/savenotebook";$.post(url,args,function(result){if(result.error){alert("Error saving notebook: "+result.error);return;}
if(result.result!="ok"){alert("Error saving notebook: "+result.result);return;}
alert("Notebook saved");});},showInput:function(){if(this.displayMode&&!this.user)
return false;return true;},getJson:function(output){var obj={cells:[],currentEntries:{},runOnLoad:this.runOnLoad,displayMode:this.displayMode,layout:this.layout,columns:this.columns,};for(var name in this.currentEntries){var e=this.currentEntries[name];obj.currentEntries[name]={entryId:e.entryId};}
this.cells.map(cell=>obj.cells.push(cell.getJson(output)));return obj;},layoutCells:function(){this.jq(ID_CELLS).html("");var html;if(this.showInput()&&this.layout=="sidebyside"){var left=HtmlUtils.openTag("div",["id",this.getDomId(ID_INPUTS)]);var right=HtmlUtils.openTag("div",["id",this.getDomId(ID_OUTPUTS)]);for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];cell.index=i+1;left+=HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id+"_cellinput"],"");right+=HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id+"_celloutput"],"");}
left+=HtmlUtils.closeTag("div");right+=HtmlUtils.closeTag("div");var center=HtmlUtils.div([],"");html="<table width=100%><tr valign=top><td width=50%>"+left+"</td><td style='border-left:1px #ccc solid;' width=1>"+center+"</td><td xwidth=50%>"+right+"</td></tr></table>";}else{html="<div class=row style='padding:0px;margin:0px;'>";var clazz=HtmlUtils.getBootstrapClass(this.columns);var colCnt=0;for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];cell.index=i+1;html+=HtmlUtils.openTag("div",["class",clazz]);html+=HtmlUtils.openTag("div",["style","max-width:100%;overflow-x:auto;padding:0px;margin:px;"]);html+=HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id+"_cellinput"],"");html+=HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id+"_celloutput"],"");html+=HtmlUtils.closeTag("div");html+=HtmlUtils.closeTag("div");colCnt++;if(colCnt>=this.columns){colCnt=0;html+=HtmlUtils.closeTag("div");html+="<div class=row style='padding:0px;margin:0px;'>";}};html+=HtmlUtils.closeTag("div");}
this.jq(ID_CELLS).append(html);for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];cell.createCell();};},addCell:function(content,props,layoutLater){cell=this.createCell(content,props);this.cells.push(cell);if(!layoutLater){if(this.showInput()&&this.layout=="sidebyside"){this.jq(ID_INPUTS).append(HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id+"_cellinput"],""));this.jq(ID_OUTPUTS).append(HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id+"_celloutput"],""));}else{this.jq(ID_CELLS).append(HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id+"_cellinput"],""));this.jq(ID_CELLS).append(HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id+"_celloutput"],""));}
cell.createCell();}
...
return idx;},moveCellUp:function(cell){var cells=[];var newCell=null;var idx=this.getIndex(cell);if(idx==0)return;this.cells.splice(idx,1);this.cells.splice(idx-1,0,cell);this.layoutCells();cell.focus();},moveCellDown:function(cell){var cells=[];var newCell=null;var idx=this.getIndex(cell);if(idx==this.cells.length-1)return;this.cells.splice(idx,1);this.cells.splice(idx+1,0,cell);this.layoutCells();cell.focus();},newCellAbove:function(cell){var cells=[];var newCell=null;for(var i=0;i<this.cells.length;i++){if(cell.id==this.cells[i].id){newCell=this.createCell(null,{showInput:true,showHeader:false});cells.push(newCell);}
cells.push(this.cells[i]);}
this.cells=cells;this.layoutCells();newCell.focus();},newCellBelow:function(cell){var cells=[];var newCell=null;for(var i=0;i<this.cells.length;i++){cells.push(this.cells[i]);if(cell.id==this.cells[i].id){newCell=this.createCell(null,{showInput:true,showHeader:false});cells.push(newCell);}}
...
return input;},runAll:async function(){var ok=true;this.cellValues={};for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];cell.hasRun=false;}
...
if(!ok)return;for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];if(cell.runFirst)continue;await this.runCell(cell).then(result=>ok=result);}},runCell:async function(cell){if(cell.hasRun)return true;await cell.run(result=>ok=result);if(!ok)return false;var raw=cell.getRawOutput();if(raw){raw=raw.trim();if(Utils.stringDefined(cell.cellName)){this.cellValues[cell.cellName]=raw;}}
return true;},toggleAll:function(on){this.cells.map(cell=>{cell.showInput=on;cell.applyStyle();});},});}
function skulptRead(x){if(Sk.builtinFiles===undefined||Sk.builtinFiles["files"][x]===undefined)
throw"File not found: '"+x+"'";return Sk.builtinFiles["files"][x];}
function NotebookState(cell,div){this.id=HtmlUtils.getUniqueId();this.cell=cell;this.notebook=cell.notebook;$.extend(this,{entries:{},div:div,stopFlag:false,prefix:null,result:null,getStop:function(){return this.stopFlag;},getCell:function(){return this.cell;},setValue:function(name,value){this.notebook.setCellValue(name,value);},makeData:async function(entry){if(!entry)
...
this.write(h);},stop:function(){this.stopFlag=true;},setGlobal:function(name,value){this.cell.notebook.addGlobal(name,value);},setEntry:function(name,entryId){this.cell.notebook.addEntry(name,entryId);},getEntry:async function(entryId,callback){await this.cell.notebook.getEntry(e=>entry=e);return Utils.call(callback,entry);},wiki:async function(s,entry,callback){var write=false;if(!callback)
callback=h=>this.write(h);if(entry==null)
await this.cell.getCurrentEntry(e=>entry=e);if((typeof entry)!="string")entry=entry.getId();await GuiUtils.loadHtml(ramaddaBaseUrl+"/wikify?doImports=false&entryid="+entry+"&text="+encodeURIComponent(s),callback);},write:function(s){if(this.prefix==null){this.prefix=s;}else{this.prefix+=s;}},linechart:async function(entry,props){if(!entry)
await this.cell.getCurrentEntry(e=>entry=e);this.cell.createDisplay(this,entry,DISPLAY_LINECHART,props);},help:function(){return"Enter an expression, e.g.:<pre>1+2</pre> or wrap you code in brackets:<pre>{\nvar x=1+2;\nreturn x;\n}</pre>"+
...
var notebookStates={};function RamaddaNotebookCell(notebook,id,content,props){this.notebook=notebook;var ID_CELL="cell";var ID_HEADER="header";var ID_CELLNAME="cellname";var ID_INPUT="input";var ID_INPUT_TOOLBAR="inputtoolbar";var ID_OUTPUT="output";var ID_MESSAGE="message";var ID_BUTTON_MENU="menubutton";var ID_BUTTON_RUN="runbutton";var ID_BUTTON_TOGGLE="togglebutton";var ID_MENU="menu";var ID_CELLNAME_INPUT="cellnameinput";var ID_SHOWHEADER_INPUT="showheader";var ID_SHOWEDIT="showedit";var ID_RUN_ON_LOAD="runonload";var ID_DISPLAY_MODE="displaymode";var ID_LAYOUT_TYPE="layouttype";var ID_LAYOUT_COLUMNS="layoutcolumns";var ID_RUNFIRST="runfirst";var ID_SHOW_OUTPUT="showoutput";let SUPER=new DisplayThing(id,{});RamaddaUtil.inherit(this,SUPER);RamaddaUtil.defineMembers(this,{id:id,inputRows:1,index:0,content:content,outputHtml:"",showInput:false,showHeader:false,cellName:"",runFirst:false,showOutput:true,});if(props){$.extend(this,props);}
...
this.makeButton(ID_BUTTON_RUN,Utils.getIcon("runall.png"),"Run all","runall");var header=HtmlUtils.div([ATTR_CLASS,"display-notebook-header",ATTR_ID,this.getDomId(ID_HEADER),"tabindex","0","title","Click to toggle input\nShift-click to clear output"],"&nbsp;"+buttons+"&nbsp;"+HtmlUtils.span(["id",this.getDomId(ID_CELLNAME)],this.cellName));var content=this.content.replace(/</g,"&lt;").replace(/>/g,"&gt;");var input=HtmlUtils.div([ATTR_CLASS,"display-notebook-input ace_editor",ATTR_ID,this.getDomId(ID_INPUT)],content);var inputToolbar=HtmlUtils.div(["id",this.getDomId(ID_INPUT_TOOLBAR)],"");input=HtmlUtils.div(["class","display-notebook-input-container"],inputToolbar+input);var output=HtmlUtils.div([ATTR_CLASS,"display-notebook-output",ATTR_ID,this.getDomId(ID_OUTPUT)],this.outputHtml);output=HtmlUtils.div(["class","display-notebook-output-container"],output);var menu=HtmlUtils.div(["id",this.getDomId(ID_MENU),"class","ramadda-popup"],"");var html=header+input;html=HtmlUtils.div(["id",this.getDomId(ID_CELL)],html);$("#"+this.id+"_cellinput").html(html);$("#"+this.id+"_celloutput").html(output);var url=ramaddaBaseUrl+"/wikitoolbar?entryid="+this.entryId+"&handler="+this.editId;url+="&extrahelp="+ramaddaBaseUrl+"/userguide/notebook.html|Notebook Help";GuiUtils.loadHtml(url,h=>{this.inputToolbar=h;this.jq(ID_INPUT_TOOLBAR).html(h);$("#"+this.editId+"_prefix").html(HtmlUtils.span(["id",this.getDomId("toolbar_notebook"),"style","border-right:1px #ccc solid;","class","ramadda-menubar-button"],"Notebook"));this.jq("toolbar_notebook").click(()=>this.showNotebookMenu());});this.header=this.jq(ID_HEADER);this.header.click((e)=>{if(e.shiftKey)
this.processCommand("clear");else{this.hidePopup();this.processCommand("toggle");}});this.editor=HtmlUtils.initAceEditor("",this.getDomId(ID_INPUT));this.menuButton=this.jq(ID_BUTTON_MENU);this.toggleButton=this.jq(ID_BUTTON_TOGGLE);this.cell=this.jq(ID_CELL);this.input=this.jq(ID_INPUT);this.output=this.jq(ID_OUTPUT);this.inputContainer=this.cell.find(".display-notebook-input-container");this.inputMenu=this.cell.find(".display-notebook-input-container");this.applyStyle();this.header.find(".display-notebook-menu-button").click(function(e){_this.processCommand($(this).attr("what"));e.stopPropagation();});this.calculateInputHeight();this.input.focus(()=>this.hidePopup());this.input.click(()=>this.hidePopup());this.output.click(()=>this.hidePopup());var moveFunc=(e)=>{var key=e.key;if(key=='v'&&e.ctrlKey){this.notebook.moveCellDown(_this);return;}
if(key==6&&e.ctrlKey){this.notebook.moveCellUp(_this);return;}};this.input.keydown(moveFunc);this.header.keydown(moveFunc);this.input.keypress(function(e){var key=e.key;if(key=='Enter'){if(e.shiftKey){if(e.preventDefault){e.preventDefault();}
_this.run();}else if(e.ctrlKey){if(e.preventDefault){e.preventDefault();}
_this.notebook.runAll();}}});},selectClick(type,id,entryId,value){if(type=="entryid"){this.insertText(entryId);}else{this.insertText("[["+entryId+"|"+value+"]]");}
...
menu+=HtmlUtils.div(["class","ramadda-link","what","clearall"],"Clear all outputs");menu+="<br>";var cols=this.notebook.columns;var colId=_this.getDomId(ID_LAYOUT_COLUMNS);menu+="<b>Layout:</b> ";menu+=HtmlUtils.checkbox(_this.getDomId(ID_LAYOUT_TYPE),[],_this.notebook.layout=="sidebyside")+" Side by side"+"<br>";menu+="Columns: ";menu+=HtmlUtils.input(colId,this.notebook.columns,["size","3","id",_this.getDomId(ID_LAYOUT_COLUMNS)]);menu+=line;menu+=HtmlUtils.checkbox(_this.getDomId(ID_SHOW_OUTPUT),[],_this.showOutput)+" Output enabled"+"<br>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_RUNFIRST),[],_this.runFirst)+" Run first"+"<br>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_RUN_ON_LOAD),[],_this.notebook.runOnLoad)+" Run on load"+"<br>";menu+=HtmlUtils.div(["title","Don't show the left side and input for anonymous users"],HtmlUtils.checkbox(_this.getDomId(ID_DISPLAY_MODE),[],_this.notebook.displayMode)+" Display mode"+"<br>");menu+=line;menu+=HtmlUtils.div(["class","ramadda-link","what","savewithout"],"Save notebook")+"<br>";menu+=line;menu+=HtmlUtils.div(["class","ramadda-link","what","delete"],"Delete")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","help"],"Help")+"<br>";menu=HtmlUtils.div(["class","display-notebook-menu"],menu);var popup=this.getPopup();this.dialogShown=true;popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();popup.position({of:src,my:"left top",at:at,collision:"fit fit"});_this.jq(ID_SHOWHEADER_INPUT).focus();_this.jq(ID_SHOWHEADER_INPUT).change(function(e){_this.showHeader=_this.jq(ID_SHOWHEADER_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_RUNFIRST).change(function(e){_this.runFirst=_this.jq(ID_RUNFIRST).is(':checked');});_this.jq(ID_SHOW_OUTPUT).change(function(e){_this.showOutput=_this.jq(ID_SHOW_OUTPUT).is(':checked');_this.applyStyle();});_this.jq(ID_RUN_ON_LOAD).change(function(e){_this.notebook.runOnLoad=_this.jq(ID_RUN_ON_LOAD).is(':checked');});_this.jq(ID_DISPLAY_MODE).change(function(e){_this.notebook.displayMode=_this.jq(ID_DISPLAY_MODE).is(':checked');});_this.jq(ID_SHOWEDIT).change(function(e){_this.showInput=_this.jq(ID_SHOWEDIT).is(':checked');_this.applyStyle();});_this.jq(ID_LAYOUT_TYPE).change(function(e){if(_this.jq(ID_LAYOUT_TYPE).is(':checked')){_this.notebook.layout="sidebyside";}else{_this.notebook.layout="flow";}
...
if(!src.is(":visible")){src=this.header;}
popup.position({of:src,my:"left top",at:"left top",collision:"fit fit"});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");_this.hidePopup();if(what=="yes"){_this.notebook.deleteCell(_this);}});},run:async function(callback){if(this.running)return Utils.call(callback,true);this.running=true;try{var ok=true;await this.runInner().then(r=>ok=r);if(!ok){this.running=false;return Utils.call(callback,false);}
this.outputUpdated();}catch(e){this.running=false;this.writeOutput("An error occurred:"+e.toString());console.log("error:"+e.toString());console.log(e.stack);return Utils.call(callback,false);}
this.running=false;return Utils.call(callback,true);},getInputText:function(){return this.editor.getValue();},runInner:async function(){var value=this.getInputText();value=value.trim();var help="More information <a target=_help href='"+ramaddaBaseUrl+"/userguide/notebook.html'>here</a>";value=value.replace(/{cellname}/g,this.cellName);value=value.replace(/{help}/g,help);value=this.notebook.convertInput(value);var type="wiki";var blobs=[];var blob="";var commands=value.split("\n");var ok=true;for(var i=0;i<commands.length;i++){if(!ok)break;var command=commands[i];var _command=command.trim();if(_command.startsWith("//"))continue;if(_command.startsWith("%%")){var rest=_command.substring(2).trim();var newType;var index=rest.indexOf(" ");if(index<0){newType=rest;rest="";}else{newType=rest.substring(0,index).trim();rest=rest.substring(index);}
if(blob!=""){blobs.push({blob:blob,type:type,div:new Div(),ok:true});}
blob=rest;if(blob!="")blob+="\n";type=newType;continue;}
blob=blob+command+"\n";}
if(blob!=""){blobs.push({blob:blob,type:type,div:new Div(),ok:true});}
var result="";for(var i=0;i<blobs.length;i++){result+=blobs[i].div.toString()+"\n";}
this.output.html(result);this.rawOutput="";for(var i=0;i<blobs.length;i++){var blob=blobs[i];await this.processBlob(blob);if(!blob.ok){return false;}}
Utils.initContent("#"+this.getDomId(ID_OUTPUT));return ok;},writeOutput:function(h){if(!this.output){err=new Error();console.log("no output:"+err.stack);return;}
this.output.html(h);this.outputUpdated();},outputUpdated:function(){this.outputHtml=this.jq(ID_OUTPUT).html();},getRawOutput:function(){return this.rawOutput;},focus:function(){this.input.focus();},clearOutput:function(){this.output.html("");this.outputHtml="";},processHtml:async function(blob){this.rawOutput+=blob.blob+"\n";blob.div.set(blob.blob);},processCss:async function(blob){var css=HtmlUtils.tag("style",["type","text/css"],blob.blob);this.rawOutput+=css+"\n";blob.div.set(css);},handleError:function(blob,error){blob.ok=false;blob.div.append(error);},processFetch:async function(blob){var lines=blob.blob.split("\n");for(var i=0;i<lines.length;i++){var line=lines[i].trim();if(line=="")continue;var origLine=line;var error=null;if(line.startsWith("js:")){var url=line.substring(3).trim();await Utils.importJS(url,()=>{},(jqxhr,settings,exception)=>error="Error fetching "+origLine+" "+exception);}else if(line.startsWith("css:")){var url=line.substring(4).trim();await Utils.importCSS(url,null,(jqxhr,settings,exception)=>error="Error fetching "+origLine+" "+exception);}else if(line.startsWith("html:")){var url=line.substring(5).trim();await Utils.importText(url,h=>blob.div.append(h),(jqxhr,settings,exception)=>error="Error fetching "+origLine+" "+exception);}else if(line.startsWith("text:")){line=line.substring(5).trim();var v=null;var indexEquals=line.indexOf("=");var indexHttp=line.indexOf("http");var indexSlash=line.indexOf("/");if(indexEquals>0){var index=indexHttp>=0?indexHttp:indexSlash;if(indexSlash>=0&&indexSlash<indexHttp)index=indexSlash;if(indexEquals<index){v=line.substring(0,indexEquals).trim();line=line.substring(indexEquals+1).trim();}}
var url=line;var results=null;await Utils.importText(url,h=>results=h,(jqxhr,settings,err)=>error="Error fetching "+origLine+" error:"+err.toString());if(results){if(v){this.notebook.addGlobal(v,results);}else{blob.div.append(HtmlUtils.pre([],results));}}}else{blob.div.append("Unknown fetch:"+line);}
if(error){this.handleError(blob,error);return;}}},processMd:async function(blob){await Utils.importJS(ramaddaBaseUrl+"/lib/showdown.min.js");this.rawOutput+=blob.blob+"\n";var converter=new showdown.Converter();var html=converter.makeHtml(blob.blob);blob.div.set(html);},processPy:async function(blob){await Utils.importJS(ramaddaBaseUrl+"/lib/skulpt.min.js");await Utils.importJS(ramaddaBaseUrl+"/lib/skulpt-stdlib.js");var output="";var outf=function(t){if(t.trim()!=""){output+=t.trim()+"<br>"}}
Sk.configure({output:outf,read:skulptRead});var myPromise=Sk.misceval.asyncToPromise(function(){return Sk.importMainWithBody("<stdin>",false,blob.blob,true);});myPromise.then(function(mod){},function(err){output+="Error:"+err.toString();console.log(err.toString());});blob.div.set(output);},processWiki:async function(blob){this.rawOutput+=blob.blob+"\n";var id=this.notebook.getProperty("entryId","");await this.getCurrentEntry(e=>entry=e);if(entry)id=entry.getId();let _this=this;let divId=HtmlUtils.getUniqueId();var wikiCallback=function(html){var h=HtmlUtils.div(["id",divId,"style"],html);blob.div.set(h);}
var wiki="{{group showMenu=false}}\n"+blob.blob;await GuiUtils.loadHtml(ramaddaBaseUrl+"/wikify?doImports=false&entryid="+id+"&text="+encodeURIComponent(blob.blob),wikiCallback);},processSh:async function(blob){var r="";var lines=blob.blob.split("\n");var commands=[];for(var i=0;i<lines.length;i++){var fullLine=lines[i].trim();if(fullLine=="")continue;var cmds=fullLine.split(";");for(var cmdIdx=0;cmdIdx<cmds.length;cmdIdx++){var line=cmds[cmdIdx].trim();if(line==""||line.startsWith("#")||line.startsWith("//"))continue;for(name in this.notebook.globals){line=line.replace("${"+name+"}",this.notebook.globals[name]);}
var toks=line.split(" ");var command=toks[0].trim();var proc=null;var extra=null;if(this["processCommand_"+command]){proc=this["processCommand_"+command];}else{proc=this.processCommand_help;extra="Unknown command: <i>"+command+"</i>";}
var div=new Div("");commands.push({proc:proc,line:line,toks:toks,extra:extra,div:div});r+=div.set("");}}
let _this=this;blob.div.set(r);var i=0;for(i=0;i<commands.length;i++){var cmd=commands[i];if(cmd.extra){cmd.div.append(extra);}
await cmd.proc.call(_this,cmd.line,cmd.toks,cmd.div,cmd.extra);}},processJs:async function(blob){try{await this.getCurrentEntry(e=>{current=e});var state=new NotebookState(this,blob.div);notebookStates[state.id]=state;var notebookEntries=this.notebook.getCurrentEntries();for(name in notebookEntries){state.entries[name]=notebookEntries[name].entry;}
var jsSet="";state.entries["current"]=current;state.entries["parent"]=this.parentEntry;state.entries["base"]=this.notebook.getBaseEntry();state.entries["root"]=this.notebook.getRootEntry();var stateJS="notebookStates['"+state.id+"']";jsSet+="state= "+stateJS+";\n";for(name in state.entries){var e=state.entries[name];jsSet+=name+"= state.entries['"+name+"'];\n"}
for(name in this.notebook.globals){jsSet+=name+"= state.getNotebook().globals['"+name+"'];\n";}
nbCell=this;nbNotebook=this.notebook;var js=blob.blob.trim();var lines=js.split("\n");if(lines.length==1&&!js.startsWith("return ")&&!js.startsWith("{")){js="return "+js;}
if(lines.length>1||js.startsWith("return ")||js.startsWith("{")){js="async function nbFunc() {\n"+jsSet+"\n"+js+"\n}";}
eval(js);await nbFunc().then(r=>{state.result=r;});if(state.getStop()){blob.ok=false;}
var html="";if(Utils.stringDefined(state.prefix))html+=state.prefix;if(state.result){html+=state.result;this.rawOutput+=html+"\n";}
blob.div.append(html);}catch(e){blob.ok=false;var lines=blob.blob.split("\n");var line=lines[e.lineNumber-2];blob.div.append("Error: "+e.message+"<br>"+HtmlUtils.span(["class","display-notebook-error"]," &gt; "+line));console.log(e.stack);}
notebookStates[state.id]=null;},processBlob:async function(blob){if(blob.type=="html"){await this.processHtml(blob);}else if(blob.type=="wiki"){await this.processWiki(blob);}else if(blob.type=="css"){await this.processCss(blob);}else if(blob.type=="fetch"){await this.processFetch(blob);}else if(blob.type=="raw"){this.rawOutput+=blob.blob+"\n";}else if(blob.type=="js"){await this.processJs(blob);}else if(blob.type=="sh"){await this.processSh(blob);}else if(blob.type=="md"){await this.processMd(blob);}else if(blob.type=="py"){await this.processPy(blob);}else{blob.div.set("Unknown type:"+blob.type);}},calculateInputHeight:function(){this.content=this.getInputText();if(!this.content)return;var lines=this.content.split("\n");if(lines.length!=this.inputRows){this.inputRows=lines.length;this.input.attr("rows",Math.max(1,this.inputRows));}},writeStatusMessage:function(v){var msg=this.jq(ID_MESSAGE);if(!v){msg.hide();msg.html("");}else{msg.show();msg.position({of:this.getOutput(),my:"left top",at:"left+4 top+4",collision:"none none"});msg.html(v);}},handleControlKey:function(event){var k=event.which;},getOutput:function(){return this.jq(ID_OUTPUT);},getInput:function(){return this.jq(ID_INPUT);},writeResult:function(html){this.writeStatusMessage(null);html=HtmlUtils.div([ATTR_CLASS,"display-notebook-result"],html);var output=this.jq(ID_OUTPUT);output.append(html);output.animate({scrollTop:output.prop("scrollHeight")},1000);this.currentOutput=output.html();this.currentInput=this.getInputText();},writeError:function(msg){this.writeStatusMessage(msg);},header:function(msg){return HtmlUtils.div([ATTR_CLASS,"display-notebook-header"],msg);},processCommand_help:function(line,toks,div,callback,prefix){if(div==null)div=new Div();var help="";if(prefix!=null)help+=prefix;help+="<pre>pwd, ls, cd</pre>";return div.append(help);},entries:{},selectEntry:function(entryId){var cnt=1;var entries=this.notebook.getCurrentEntries();while(entries["entry"+cnt]){cnt++;}
...
var name=toks[1];if(toks.length==2){var v=this.notebook.globals[name];if(v){div.append(v);}else{div.append("Unknown: "+name);}}else{var v=Utils.join(toks," ",2);v=v.replace(/\"/g,"");this.notebook.addGlobal(name,v);}},processCommand_clearEntries:function(line,toks,div){this.notebook.clearEntries();div.set("Entries cleared");},processCommand_printEntries:async function(line,toks,div){var h="";await this.getCurrentEntry(e=>current=e);h+="current"+"="+current.getName()+"<br>";var entries=this.notebook.getCurrentEntries();for(var name in entries){var e=entries[name];h+=name+"="+e.entry.getName()+"<br>";}
...
var arg=Utils.join(toks," ",1).trim();if(arg.startsWith("/")){await this.getCurrentEntry(e=>entry=e);var root;await entry.getRoot(e=>{root=e});this.setCurrentEntry(root);}
var dirs=arg.split("/");await this.getCurrentEntry(e=>entry=e);for(var i=0;i<dirs.length;i++){var dir=dirs[i];if(dir=="")continue;if(dir==".."){if(!this.parentEntry){div.msg("No parent entry");break;}
await this.setCurrentEntry(this.parentEntry);}else{await this.currentEntry.getChildrenEntries(c=>children=c);var child=null;var startsWith=false;var endsWith=false;if(dir.endsWith("*")){dir=dir.substring(0,dir.length-1);startsWith=true;}
...
if(!child){div.msg("No entry:"+dir);break;}
await this.setCurrentEntry(child);}}},processCommand_ls:async function(line,toks,div){if(div==null)div=new Div();div.set("Listing entries...");await this.getCurrentEntry(e=>entry=e);await entry.getChildrenEntries(children=>{this.displayEntries(children,div)},"");},entryListChanged:function(entryList){var entries=entryList.getEntries();if(entries.length==0){this.writeStatusMessage("Sorry, nothing found");}else{this.displayEntries(entries);}},processCommand_search:function(line,toks,div){var text="";for(var i=1;i<toks.length;i++)text+=toks[i]+" ";text=text.trim();var searchSettings=new EntrySearchSettings({text:text,});var repository=this.getRamadda();this.writeStatusMessage("Searching...");var jsonUrl=repository.getSearchUrl(searchSettings,OUTPUT_JSON);this.entryList=new EntryList(repository,jsonUrl,this,true);},processCommand_clear:function(line,toks,div){this.clearOutput();},processCommand_save:function(line,toks,div){this.notebook.saveNotebook();},});}
...
google.charts.setOnLoadCallback(googleChartsHaveLoaded);function haveGoogleChartsLoaded(){if(!googleChartsLoaded){if(Utils.isDefined(google.visualization)){if(Utils.isDefined(google.visualization.Gauge)){googleChartsLoaded=true;}}}
...
var justData=[];var begin=props.includeIndex?1:0;var tooltipFields=[];var toks=this.getProperty("tooltipFields","").split(",");for(var i=0;i<toks.length;i++){var tooltipField=this.getFieldById(null,toks[i]);if(tooltipField)
...
this.chart.draw(dataTable,this.chartOptions);var theDisplay=this;google.visualization.events.addListener(this.chart,'onmouseover',function(event){mapVar=theDisplay.getProperty("mapVar",null);if(!Utils.stringDefined(mapVar)){return;}
...
return new google.visualization.Sankey(document.getElementById(this.getChartId()));},defaultSelectedToAll:function(){return true;},makeDataTable:function(dataList,props,selectedFields){dataList=this.filterData(dataList,selectedFields);if(!this.getProperty("doCategories",false)){var values=this.makeDataArray(dataList);return google.visualization.arrayToDataTable(values);}
var strings=[];for(var i=0;i<selectedFields.length;i++){var field=selectedFields[i];if(field.getType()=="string"){strings.push(field);}}
...
if(this.showType){header.push("Type");dummy.push("&nbsp;");}
...
if(this.showType){values.push(stats[col].type);}
...
label=toks[toks.length-1];if(justOne){label+=":";}
label=label.replace(/ /g,"&nbsp;")
var row=HtmlUtils.tr([],HtmlUtils.td(["align",align],"<b>"+HtmlUtils.tag("div",["title",tooltip],label)+"</b>")+right);if(justOne){html+=row;}else{html+=row;}}
...
function RamaddaRankingDisplay(displayManager,id,properties){var ID_TABLE="table";$.extend(this,{height:"500px;"});let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_RANKING,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},fieldSelectionChanged:function(){this.updateUI();},updateUI:function(pointData){SUPER.updateUI.call(this,pointData);if(!this.hasData()){this.setContents(this.getLoadingMessage());return;}
var dataList=this.getStandardData(null,{includeIndex:false});var allFields=this.dataCollection.getList()[0].getRecordFields();var fields=this.getSelectedFields([]);if(fields.length==0)fields=allFields;var numericFields=this.getFieldsOfType(fields,"numeric");var sortField=this.getFieldById(numericFields,this.getProperty("sortField"));if(numericFields.length==0){this.setContents("No fields specified");return;}
if(!sortField){sortField=numericFields[0];}
if(!sortField){this.setContents("No fields specified");return;}
var stringField=this.getFieldOfType(fields,"string");var menu="<select class='ramadda-pulldown' id='"+this.getDomId("sortfields")+"'>";for(var i=0;i<numericFields.length;i++){var field=numericFields[i];var extra="";if(field.getId()==sortField.getId())extra=" selected ";menu+="<option value='"+field.getId()+"'  "+extra+" >"+field.getLabel()+"</option>\n";}
menu+="</select>";if(this.getProperty("showRankingMenu",true)){this.jq(ID_TOP_LEFT).html(menu);}
var html="";html+=HtmlUtils.openTag("div",["style","max-height:100%;overflow-y:auto;"]);html+=HtmlUtils.openTag("table",["id",this.getDomId(ID_TABLE)]);var tmp=[];for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var obj=dataList[rowIdx];obj.originalRow=rowIdx;tmp.push(obj);}
var cnt=0;tmp.sort((a,b)=>{var t1=this.getDataValues(a);var t2=this.getDataValues(b);var v1=t1[sortField.getIndex()];var v2=t2[sortField.getIndex()];if(v1<v2)return 1;if(v1>v2)return-1;return 0;});for(var rowIdx=0;rowIdx<tmp.length;rowIdx++){var obj=tmp[rowIdx];var tuple=this.getDataValues(obj);var label="";if(stringField)
...
var data=[];for(var rowIdx=1;rowIdx<this.dataList.length;rowIdx++){var tuple=this.getDataValues(this.dataList[rowIdx]);var nums=[];for(var i=0;i<this.fields.length;i++){if(this.fields[i].isNumeric)
...
function RamaddaSkewtDisplay(displayManager,id,properties){var SUPER;var ID_SKEWT="skewt";RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_SKEWT,properties));addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{initDisplay:function(){this.createUI();var html="<p><script src='/repository/lib/skewt/d3skewt.js'></script>\n";html+="<link rel='stylesheet' type='text/css' href='/lib/skewt/sounding.css'>\n";var skewtId=this.getDomId(ID_SKEWT);html+=HtmlUtils.div(["id",skewtId],"");this.writeHtml(ID_DISPLAY_CONTENTS,html);var _this=this;var func=function(){if(!window["D3Skewt"]){console.log("no skewt yet");setTimeout(func,1000);return;}
var options={};if(_this.propertyDefined("showHodograph"))
options.showHodograph=_this.getProperty("showHodograph",true);if(_this.propertyDefined("showText"))
options.showText=_this.getProperty("showText",true);if(_this.propertyDefined("chartWidth"))
options.width=parseInt(_this.getProperty("chartWidth"));if(_this.propertyDefined("chartHeight"))
options.height=parseInt(_this.getProperty("chartHeight"));_this.skewt1=new D3Skewt(skewtId,options);}
...
RamaddaUtil.defineMembers(this,{searchSettings:new EntrySearchSettings({parent:properties.entryParent,provider:properties.provider,text:properties.entryText,entryType:properties.entryType,orderBy:properties.orderBy}),entryList:properties.entryList,entryMap:{},getSearchSettings:function(){if(this.providers!=null){var provider=this.searchSettings.provider;var fromSelect=this.jq(ID_PROVIDERS).val();if(fromSelect!=null){provider=fromSelect;}else{var toks=this.providers.split(",");if(toks.length>0){var tuple=toks[0].split(":");provider=tuple[0];}}
...
this.currentPopupEntry=entry;if(src==null)src=this.getDomId("entry_"+entry.getIdForDom());var close=HtmlUtils.onClick(this.getGet()+".hideEntryDetails('"+entryId+"');",HtmlUtils.image(icon_close));var contents=this.getEntryHtml(entry,{headerRight:close});$(popupId).html(contents);$(popupId).show();},getResultsHeader:function(entries){var left="Showing "+(this.searchSettings.skip+1)+"-"+(this.searchSettings.skip+Math.min(this.searchSettings.max,entries.length));var nextPrev=[];var lessMore=[];if(this.searchSettings.skip>0){nextPrev.push(HtmlUtils.onClick(this.getGet()+".loadPrevUrl();",HtmlUtils.image(ramaddaBaseUrl+"/icons/arrow_left.png",[ATTR_TITLE,"Previous","border","0"]),[ATTR_CLASS,"display-link"]));}
var addMore=false;if(entries.length==this.searchSettings.getMax()){nextPrev.push(HtmlUtils.onClick(this.getGet()+".loadNextUrl();",HtmlUtils.image(ramaddaBaseUrl+"/icons/arrow_right.png",[ATTR_TITLE,"Next","border","0"]),[ATTR_CLASS,"display-link"]));addMore=true;}
lessMore.push(HtmlUtils.onClick(this.getGet()+".loadLess();",HtmlUtils.image(ramaddaBaseUrl+"/icons/greenminus.png",[ATTR_ALT,"View less",ATTR_TITLE,"View less","border","0"]),[ATTR_CLASS,"display-link"]));if(addMore){lessMore.push(HtmlUtils.onClick(this.getGet()+".loadMore();",HtmlUtils.image(ramaddaBaseUrl+"/icons/greenplus.png",[ATTR_ALT,"View more",ATTR_TITLE,"View more","border","0"]),[ATTR_CLASS,"display-link"]));}
...
var buttonLabel=HtmlUtils.image(ramaddaBaseUrl+"/icons/magnifier.png",[ATTR_BORDER,"0",ATTR_TITLE,"Search"]);var topItems=[];var extra="";extra+=HtmlUtils.formTable();if(this.showArea){this.areaWidget=new AreaWidget(this);extra+=HtmlUtils.formEntry("Area:",this.areaWidget.getHtml());}
...
this.providerMap={};if(this.providers!=null){var options="";var selected=Utils.getUrlArgs(document.location.search).provider;var toks=this.providers.split(",");var currentCategory=null;var catToBuff={};var cats=[];for(var i=0;i<toks.length;i++){var tuple=toks[i].split(":");var id=tuple[0];id=id.replace(/_COLON_/g,":");var label=tuple.length>1?tuple[1]:id;if(label.length>40){label=label.substring(0,39)+"...";}
...
function RamaddaMapDisplay(displayManager,id,properties){var ID_LATFIELD="latfield";var ID_LONFIELD="lonfield";var ID_MAP="map";var ID_BOTTOM="bottom";var ID_RUN="maprun";var ID_STEP="mapstep";var ID_SHOWALl="showall";var ID_ANIMATION_LABEL="animationlabel";var SUPER;RamaddaUtil.defineMembers(this,{showLocationReadout:false,showBoxes:true,showPercent:false,percentFields:null,kmlLayer:null,kmlLayerName:"",geojsonLayer:null,geojsonLayerName:"",theMap:null});RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_MAP,properties));addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{mapBoundsSet:false,features:[],myMarkers:{},mapEntryInfos:{},sourceToLine:{},sourceToPoints:{},snarf:true,initDisplay:function(){SUPER.initDisplay.call(this);var _this=this;var html="";var extraStyle="min-height:200px;";var width=this.getWidth();if(Utils.isDefined(width)){if(width>0){extraStyle+="width:"+width+"px; ";}else if(width<0){extraStyle+="width:"+(-width)+"%;";}else if(width!=""){extraStyle+="width:"+width+";";}}
var height=this.getProperty("height",300);if(height>0){extraStyle+=" height:"+height+"px; ";}else if(height<0){extraStyle+=" height:"+(-height)+"%; ";}
if(this.getProperty("doAnimation",false)){var buttons=HtmlUtils.div(["id",this.getDomId(ID_RUN),"class","ramadda-button","what","run"],"Start Animation")+"&nbsp;"+
...
HtmlUtils.span(["id",this.getDomId(ID_ANIMATION_LABEL),"class","display-map-animation-label"]);buttons=HtmlUtils.div(["class","display-map-toolbar"],buttons);this.jq(ID_TOP_LEFT).append(buttons);this.run=this.jq(ID_RUN);this.step=this.jq(ID_STEP);this.showAll=this.jq(ID_SHOWALl);this.animation.label=this.jq(ID_ANIMATION_LABEL);this.run.button().click(()=>{this.toggleAnimation();});this.step.button().click(()=>{if(!this.animation.running)
this.startAnimation(true);});this.showAll.button().click(()=>{this.animation.running=false;this.animation.inAnimation=false;this.animation.label.html("");this.run.html("Start Animation");this.showAllPoints();});}
...
this.map=this.getProperty("theMap",null);if(this.map){this.map.setMapDiv(this.getDomId(ID_MAP));}else{this.map=new RepositoryMap(this.getDomId(ID_MAP),params);this.lastWidth=this.jq(ID_MAP).width();}
...
layer.removeAllFeatures();this.map.map.addLayer(layer);layer.addFeatures(clonedFeatures);this.vectorLayer=layer;this.applyVectorMap();this.map.addSelectCallback(layer,this.doDisplayMap(),function(layer){theDisplay.mapFeatureSelected(layer);});},baseMapLoaded:function(layer,url){this.vectorLayer=layer;this.applyVectorMap();mapLoadInfo=displayMapUrlToVectorListeners[url];if(mapLoadInfo){mapLoadInfo.layer=layer;for(var i=0;i<mapLoadInfo.otherMaps.length;i++){mapLoadInfo.otherMaps[i].cloneLayer(layer);}
...
return source.getProperty(prop,dflt);},applyVectorMap:function(){if(this.vectorMapApplied){return;}
if(!this.doDisplayMap()){return;}
if(!this.vectorLayer){return;}
if(!this.points){return;}
this.vectorMapApplied=true;var features=this.vectorLayer.features.slice();var circles=this.points;for(var i=0;i<circles.length;i++){var circle=circles[i];var center=circle.center;var matchedFeature=null;var index=-1;for(var j=0;j<features.length;j++){var feature=features[j];var geometry=feature.geometry;if(!geometry){break;}
...
if(comp.containsPoint&&comp.containsPoint(center)){matchedFeature=feature;index=j;break;}}
if(matchedFeature)
break;continue;}
if(!geometry.containsPoint){console.log("unknown geometry:"+geometry.CLASS_NAME);continue;}
if(geometry.containsPoint(center)){matchedFeature=feature;index=j;break;}}
if(matchedFeature){features.splice(index,1);style=matchedFeature.style;if(!style)style={"stylename":"from display"};$.extend(style,circle.style);matchedFeature.style=style;matchedFeature.popupText=circle.text;matchedFeature.dataIndex=i;}}
if((""+this.getProperty("pruneFeatures",""))=="true"){this.vectorLayer.removeFeatures(features);var dataBounds=this.vectorLayer.getDataExtent();bounds=this.map.transformProjBounds(dataBounds);if(this.getProperty("bounds")==null)
this.map.centerOnMarkers(bounds,true);}
this.vectorLayer.redraw();},needsData:function(){return true;},animation:{running:false,inAnimation:false,begin:null,end:null,dateMin:null,dateMax:null,dateRange:0,dateFormat:this.getProperty("animationDateFormat","yyyyMMdd"),mode:this.getProperty("animationMode","cumulative"),steps:this.getProperty("animationSteps",60),windowUnit:this.getProperty("animationWindow",""),window:0,speed:parseInt(this.getProperty("animationSpeed",250)),},toggleAnimation:function(){this.animation.running=!this.animation.running;this.run.html(this.animation.running?"Stop Animation":"Start Animation");if(this.animation.running)
...
this.stepAnimation();},stepAnimation:function(){if(!this.points)return;if(!this.animation.dateMax)return;var oldEnd=this.animation.end;var unit=this.animation.windowUnit;var date=new Date(this.animation.end.getTime()+this.animation.window);if(unit=="year"){this.animation.end=new Date(date.getUTCFullYear(),0);}else if(unit=="month"){this.animation.end=new Date(date.getUTCFullYear(),date.getMonth());}else if(unit=="day"){this.animation.end=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay());}else if(unit=="hour"){this.animation.end=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours());}else if(unit=="minute"){this.animation.end=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getMinutes());}else{this.animation.end=new Date(this.animation.end.getTime()+this.animation.window);}
if(this.animation.mode=="sliding"){this.animation.begin=oldEnd;}
var windowStart=this.animation.begin.getTime();var windowEnd=this.animation.end.getTime();for(var i=0;i<this.points.length;i++){var point=this.points[i];if(point.date>=windowStart&&point.date<=windowEnd)
point.style.display='inline';else
point.style.display='none';}
if(this.map.circles)
this.map.circles.redraw();if(windowEnd<this.animation.dateMax.getTime()){this.animation.label.html(this.formatAnimationDate(this.animation.begin)+" - "+this.formatAnimationDate(this.animation.end));if(this.animation.running){setTimeout(()=>this.stepAnimation(),this.animation.speed);}}else{this.animation.running=false;this.animation.label.html("");this.animation.inAnimation=false;this.animation.label.html("");this.run.html("Start Animation");}},formatAnimationDate:function(d){if(this.animation.dateFormat=="yyyy"){return Utils.formatDateYYYY(d);}else if(this.animation.dateFormat=="yyyyMMdd"){return Utils.formatDateYYYYMMDD(d);}else{return Utils.formatDate(d);}},showAllPoints:function(){if(!this.points)return;for(var i=0;i<this.points.length;i++){var point=this.points[i];point.style.display='inline';}
if(this.map.circles)
this.map.circles.redraw();},updateUI:function(){this.haveCalledUpdateUI=true;SUPER.updateUI.call(this);if(!this.getDisplayReady()){return;}
...
var fields=pointData.getRecordFields();var bounds={};var points=RecordUtil.getPoints(records,bounds);if(isNaN(bounds.north)){console.log("no bounds:"+bounds);return;}
this.initBounds=bounds;this.setInitMapBounds(bounds.north,bounds.west,bounds.south,bounds.east);if(this.map==null){return;}
if(points.length==0){console.log("points.legnth==0");return;}
source=this;var radius=parseFloat(this.getDisplayProp(source,"radius",8));var strokeWidth=parseFloat(this.getDisplayProp(source,"strokeWidth","1"));var strokeColor=this.getDisplayProp(source,"strokeColor","#000");var colorByAttr=this.getDisplayProp(source,"colorBy",null);var colors=this.getColorTable(true);var sizeByAttr=this.getDisplayProp(source,"sizeBy",null);var isTrajectory=this.getDisplayProp(source,"isTrajectory",false);if(isTrajectory){var attrs={strokeWidth:2,strokeColor:"blue"}
...
var latField1=this.getFieldById(fields,this.getProperty("latField1"));var latField2=this.getFieldById(fields,this.getProperty("latField2"));var lonField1=this.getFieldById(fields,this.getProperty("lonField1"));var lonField2=this.getFieldById(fields,this.getProperty("lonField2"));var showSegments=this.getProperty("showSegments",false);var sizeSegments=this.getProperty("sizeSegments",false);var sizeEndPoints=this.getProperty("sizeEndPoints",true);var showEndPoints=this.getProperty("showEndPoints",false);var endPointSize=parseInt(this.getProperty("endPointSize","4"));var dfltEndPointSize=endPointSize;var segmentWidth=parseInt(this.getProperty("segmentWidth","1"));var dfltSegmentWidth=segmentWidth;var showPoints=this.getProperty("showPoints",true);var lineColor=this.getProperty("lineColor","green");var colorBy={id:colorByAttr,minValue:0,maxValue:0,field:null,index:-1,isString:false,};var sizeBy={id:this.getDisplayProp(source,"sizeBy",null),minValue:0,maxValue:0,field:null,index:-1,isString:false,stringMap:{}};var sizeByMap=this.getProperty("sizeByMap");if(sizeByMap){var toks=sizeByMap.split(",");for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){sizeBy.stringMap[toks2[0]]=toks2[1];}}}
for(var i=0;i<fields.length;i++){var field=fields[i];if(field.getId()==colorBy.id||("#"+(i+1))==colorBy.id){colorBy.field=field;if(field.getType()=="string")colorBy.isString=true;}
if(field.getId()==sizeBy.id||("#"+(i+1))==sizeBy.id){sizeBy.field=field;if(field.getType()=="string")sizeBy.isString=true;}}
...
menu+="</select>";this.writeHtml(ID_TOP_RIGHT,"Color by: "+menu);this.jq("colorByMenu").change(()=>{var value=this.jq("colorByMenu").val();this.vectorMapApplied=false;this.setProperty("colorBy",value);this.updateUI();});}
sizeBy.index=sizeBy.field!=null?sizeBy.field.getIndex():-1;colorBy.index=colorBy.field!=null?colorBy.field.getIndex():-1;var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);this.animation.dateMin=null;this.animation.dateMax=null;var colorByMap={};var colorByValues=[];var colorByMinPerc=this.getDisplayProp(source,"colorByMinPercentile",-1);var colorByMaxPerc=this.getDisplayProp(source,"colorByMaxPercentile",-1);for(var i=0;i<points.length;i++){var pointRecord=records[i];if(this.animation.dateMin==null){this.animation.dateMin=pointRecord.getDate();this.animation.dateMax=pointRecord.getDate();}else{var date=pointRecord.getDate();if(date){if(date.getTime()<this.animation.dateMin.getTime())
...
if(isNaN(v)||v===null)
continue;if(excludeZero&&v==0){continue;}
if(!colorBy.isString){if(i==0||v>colorBy.maxValue)colorBy.maxValue=v;if(i==0||v<colorBy.minValue)colorBy.minValue=v;}
if(!sizeBy.isString){v=tuple[sizeBy.index];if(i==0||v>sizeBy.maxValue)sizeBy.maxValue=v;if(i==0||v<sizeBy.minValue)sizeBy.minValue=v;}}
if(this.animation.dateMax){this.animation.dateRange=this.animation.dateMax.getTime()-this.animation.dateMin.getTime();}
if(this.showPercent){colorBy.minValue=0;colorBy.maxValue=100;}
colorBy.minValue=this.getDisplayProp(source,"colorByMin",colorBy.minValue);colorBy.maxValue=this.getDisplayProp(source,"colorByMax",colorBy.maxValue);if(this.points){for(var i=0;i<this.points.length;i++)
this.map.removePoint(this.points[i]);for(var i=0;i<this.lines.length;i++)
this.map.removePolygon(this.lines[i]);this.points=[];this.lines=[];}
if(!this.points){this.points=[];this.lines=[];}
var dontAddPoint=this.doDisplayMap();var didColorBy=false;var seen={};for(var i=0;i<points.length;i++){var pointRecord=records[i];var point=points[i];var values=pointRecord.getData();var props={pointRadius:radius,strokeWidth:strokeWidth,strokeColor:strokeColor,};if(sizeBy.index>=0){var value=values[sizeBy.index];if(sizeBy.isString){if(Utils.isDefined(sizeBy.stringMap[value])){var v=parseInt(sizeBy.stringMap[value]);segmentWidth=dfltSegmentWidth+v;props.pointRadius=v;}else if(Utils.isDefined(sizeBy.stringMap["*"])){var v=parseInt(sizeBy.stringMap["*"]);segmentWidth=dfltSegmentWidth+v;props.pointRadius=v;}else{segmentWidth=dfltSegmentWidth;}}else{var denom=(sizeBy.maxValue-sizeBy.minValue);var percent=(denom==0?NaN:(value-sizeBy.minValue)/denom);props.pointRadius=6+parseInt(15*percent);if(sizeEndPoints){endPointSize=dfltEndPointSize+parseInt(10*percent);}
if(sizeSegments){segmentWidth=dfltSegmentWidth+parseInt(10*percent);}}
}
if(colorBy.index>=0){var value=pointRecord.getData()[colorBy.index];var percent=0;var msg="";var pctFields=null;if(this.percentFields!=null){pctFields=this.percentFields.split(",");}
...
if(total!=0){percent0=percent=value/total*100;percent=(percent-colorBy.minValue)/(colorBy.maxValue-colorBy.minValue);}}else{if(colorBy.isString){value=colorByMap[value];}
percent=(value-colorBy.minValue)/(colorBy.maxValue-colorBy.minValue);}
var index=parseInt(percent*colors.length);if(index>=colors.length)index=colors.length-1;else if(index<0)index=0;props.fillOpacity=0.8;props.fillColor=colors[index];didColorBy=true;}
var html=this.getRecordHtml(pointRecord,fields);if(showSegments&&latField1&&latField2&&lonField1&&lonField2){var lat1=values[latField1.getIndex()];var lat2=values[latField2.getIndex()];var lon1=values[lonField1.getIndex()];var lon2=values[lonField2.getIndex()];var attrs={};if(props.fillColor)
attrs.strokeColor=props.fillColor;else
attrs.strokeColor=lineColor;attrs.strokeWidth=segmentWidth;this.lines.push(this.map.addLine("line-"+i,"",lat1,lon1,lat2,lon2,attrs,html));if(showEndPoints){var pointProps={};$.extend(pointProps,props);pointProps.fillColor=attrs.strokeColor;pointProps.strokeColor=attrs.strokeColor;pointProps.pointRadius=dfltEndPointSize;pointProps.pointRadius=endPointSize;var p1=new OpenLayers.LonLat(lon1,lat1);var p2=new OpenLayers.LonLat(lon2,lat2);if(!Utils.isDefined(seen[p1])){seen[p1]=true;var point=this.map.addPoint("endpt-"+i,p1,pointProps,html);this.points.push(point);}
if(!Utils.isDefined(seen[p2])){seen[p2]=true;this.points.push(this.map.addPoint("endpt2-"+i,p2,pointProps,html));}}}
if(showPoints){if(Utils.isDefined(seen[point]))continue;seen[point]=true;point=this.map.addPoint("pt-"+i,point,props,html,dontAddPoint);var date=pointRecord.getDate();if(date){point.date=date.getTime();}
this.points.push(point);}}
if(didColorBy){this.displayColorTable(colors,ID_BOTTOM,colorBy.minValue,colorBy.maxValue,{stringValues:colorByValues});}
this.applyVectorMap();},handleEventRemoveDisplay:function(source,display){if(!this.map){return;}
