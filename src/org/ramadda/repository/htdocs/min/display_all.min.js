...
if(f.getType()=="image"){if(value&&value.trim().length>1){var imgAttrs=[];if(this.getProperty("imageWidth","")!=""){imgAttrs.push("width");imgAttrs.push(this.getProperty("imageWidth",""));}
var img=HtmlUtils.image(value,imgAttrs);attrs[f.getId()+"_image"]=img;attrs[f.getId()+"_url"]=value;}else{attrs[f.getId()+"_url"]=ramaddaBaseUrl+"/icons/blank.gif";attrs[f.getId()+"_image"]="";}}else if(f.getType()=="url"){if(value&&value.trim().length>1){attrs[f.getId()+"_href"]=HtmlUtils.href(value,value);attrs[f.getId()]=value;}else{attrs[f.getId()+"_href"]="";attrs[f.getId()]="";}
