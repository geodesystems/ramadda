...
let filterBy=this.getProperty("filterFields","",true).split(",");let hideFilterWidget=this.getProperty("hideFilterWidget",false,true);let dateIds=[];let fieldMap={};if(filterBy.length>0){let searchBar="";let bottom="";let widgetStyle="";if(hideFilterWidget)
widgetStyle="display:none;";for(let i=0;i<filterBy.length;i++){let filterField=this.getFieldById(fields,filterBy[i]);if(!filterField)continue;fieldMap[filterField.getId()]={field:filterField,values:[],};if(!filterField)continue;this.filterFields.push(filterField);let widget;let widgetId=this.getFilterId(filterField.getId());if(filterField.getType()=="enumeration"){let dfltValue=this.getProperty(filterField.getId()+".filterValue",FILTER_ALL);let filterValues=this.getProperty(filterField.getId()+".filterValues");let enums=null;if(filterValues){let toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}
...
this.map.clearSeenMarkers();var t4=new Date();this.addPoints(records,fields,points);var t5=new Date();this.addLabels(records,fields,points);this.applyVectorMap();},addPoints:function(records,fields,points){let cidx=0
let polygonField=this.getFieldById(fields,this.getProperty("polygonField"));let polygonColorTable=this.getColorTable(true,"polygonColorTable",null);let latlon=this.getProperty("latlon",true);let source=this;let radiusThreshold=+this.getProperty("radiusThreshold",-1);let radius=parseFloat(this.getDisplayProp(source,"radius",8));let strokeWidth=parseFloat(this.getDisplayProp(source,"strokeWidth","1"));let strokeColor=this.getDisplayProp(source,"strokeColor","#000");let sizeByAttr=this.getDisplayProp(source,"sizeBy",null);let isTrajectory=this.getDisplayProp(source,"isTrajectory",false);if(isTrajectory){let attrs={strokeWidth:2,strokeColor:"blue"}
...
props.graphicName=this.getProperty("shape","circle");if(radius>0){if(records.length<radiusThreshold){props.pointRadius=8;}
mapPoint=this.map.addPoint("pt-"+i,point,props,null,dontAddPoint);addedPoints.push(mapPoint);}}
...
if(this.map.circles)
this.map.circles.redraw();if(showSegments){this.map.centerOnMarkers(null,true,null);}
this.jq(ID_BOTTOM).append(HtmlUtils.div(["id",this.getDomId(ID_SHAPES)]));if(didColorBy){colorBy.displayColorTable();}
