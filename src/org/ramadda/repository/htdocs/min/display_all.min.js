...
widget=HtmlUtils.select("",["id",widgetId,"fieldId",filterField.getId()],enums,value);}else if(filterField.isNumeric){var opid=widgetId+"_operator";var operator=this.getProperty(filterField.getId()+".filterOperator",">");widget=HtmlUtils.select("",["id",opid,"fieldId",filterField.getId()],["<",">","="],operator,);widget+=HtmlUtils.input("",value,["id",widgetId,"size",4,"fieldId",filterField.getId()]);}else if(filterField.getType()=="date"){widget=HtmlUtils.datePicker("","",["id",widgetId+"_date1"])+" - "+
HtmlUtils.datePicker("","",["id",widgetId+"_date2"]);dateIds.push(widgetId+"_date1");dateIds.push(widgetId+"_date2");}else{widget=HtmlUtils.input("",value,["id",widgetId,"fieldId",filterField.getId()]);}
var label=this.getProperty(filterField.getId()+".filterLabel",filterField.getLabel());widget=label+": "+widget;searchBar+=widget+"&nbsp;&nbsp;";}
this.jq(ID_HEADER2).html(searchBar);for(var i=0;i<dateIds.length;i++){HtmlUtils.datePickerInit(dateIds[i]);}
