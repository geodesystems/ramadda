...
if(!d.isRow&&!d.isColumn){d.isRow=true;}}}
...
this[func].apply(this,[source,data]);},displayColorTable:function(ct,domId,min,max,args){Utils.displayColorTable(ct,this.getDomId(domId),min,max,args);},getColorList:function(){if(this.colorList&&this.colorList.length>0){return this.colorList;}
...
if(this.getProperty("rotateTable")){let dataList=pointData.getRecords();let rotated=[];let header=this.getDataValues(dataList[0]);for(var colIdx=0;colIdx<header.length;colIdx++){rotated.push([]);}
let includeFields=this.getProperty("includeFields");if(includeFields){pointData.getRecordFields().map((f,colIdx)=>{rotated[colIdx].push(colIdx==0?"":f.getLabel());});}
for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);for(var colIdx=0;colIdx<row.length;colIdx++){var value=row[colIdx];if(value.f)value=value.f;if(value.getTime){value=this.formatDate(value);}
if(!includeFields&&rowIdx==0&&colIdx==0)value="";rotated[colIdx].push(value);}}
pointData=convertToPointData(rotated);pointData.entryId=originalPointData.entryId;}
if(this.getProperty("furlTable",false)){var dataList=pointData.getRecords();var newRecords=[];var newFields=[];var lastRecord=dataList[dataList.length-1];var fields=pointData.getRecordFields();var newData=[];newData.push(["label","value"]);fields.map((f,idx)=>{let row=[f.getLabel(),lastRecord.getValue(f.getIndex())];newData.push();});pointData=convertToPointData(newData);pointData.entryId=originalPointData.entryId;}
if(this.getProperty("showPercentIncrease",false)){var dataList=pointData.getRecords();var fields=pointData.getRecordFields();var newRecords=[];var newFields=[];var firstRow=dataList[0];var firstRecord=dataList[0];fields.map(f=>{var newField=f.clone();newFields.push(newField);newField.label=newField.label+" % increase";});for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var record=dataList[rowIdx];var newRecord=record.clone();newRecords.push(newRecord);fields.map(f=>{if(!f.isNumeric())return;if(rowIdx==0){newRecord.data[f.getIndex()]=0;}else{var v=newRecord.data[f.getIndex()];var basev=firstRecord.data[f.getIndex()];newRecord.data[f.getIndex()]=basev==0?0:(v-basev)/basev;}});};pointData=new PointData("pointdata",newFields,newRecords,null,null);pointData.entryId=originalPointData.entryId;}
if(this.getProperty("derived")){this.getProperty("derived").split(",").map(tok=>{[name,func]=tok.split(";");let records=pointData.getRecords();let fields=pointData.getRecordFields();let setVars="";fields.map((field,idx)=>{if(field.isFieldNumeric()){setVars+="\tvar "+field.getId()+"=displayGetFunctionValue(args."+field.getId()+");\n";}});if(func.indexOf("return")<0){func="return "+func;}
let code="function displayDerivedEval(args) {\n"+setVars+func+"\n}";console.log("code:"+code);eval(code);records.map(r=>{let args={};fields.map((field,idx)=>{if(field.isFieldNumeric()){args[field.getId()]=r.getValue(field.getIndex());}});let value=displayDerivedEval(args);console.log("\t"+value);});});}
...
let ok=true;for(var i=0;i<dataFilters.length;i++){if(!dataFilters[i].isRecordOk(record))return false;}
return true;},getDataFilters:function(prop){prop=prop||this.getProperty("dataFilter");if(!prop){return null;}
let filters=[];prop.split(";").map(tok=>{[type,fieldId,value,enabled,label]=tok.split(",");if(!Utils.isDefined(enabled))
enabled=true;else
enabled=enabled=="true";if(label){var cbx=this.jq("datafilterenabled_"+fieldId);if(cbx.length&&cbx.is(':checked')){enabled=true;}}
if(type=="match"||type=="notmatch")
value=new RegExp(value);else
value=+value;let field=this.getFieldById(null,fieldId);if(!field){return;}
filters.push({type:type.trim(),field:field,value:value,label:label,isRecordOk:function(r){let value=r.getValue(this.field.getIndex());if(this.type=="match"){return String(value).match(this.value);}else if(this.type=="notmatch"){return!String(value).match(this.value);}else if(this.type=="lessthan"){return value<this.value;}else if(this.type=="greaterthan"){return value>this.value;}else if(this.type=="equals"){return value==this.value;}else if(this.type=="notequals"){return value!=this.value;}
return true;}});});return filters;},filterData:function(dataList,fields,doGroup,skipFirst){var startDate=this.getProperty("startDate");var endDate=this.getProperty("endDate");if(startDate){this.startDateObject=new Date(startDate);}
if(endDate){this.endDateObject=new Date(endDate);}
let filterDate=this.getProperty("filterDate");if(filterDate){let date=$("#"+this.getFilterId("filterDate")).val();if(date){if(date=="all"){this.minDateObj=null;this.maxDateObj=null;}else{date=new Date(date);if(filterDate=="year"){this.minDateObj=new Date(date.getFullYear()+"-01-01");this.maxDateObj=new Date(date.getFullYear()+"-12-31");}else{}}}}
...
if(stride>0){var list=[];var cnt=0;for(var i=0;i<dataList.length;i+=(stride+1)){list.push(dataList[i]);}
dataList=list;}
...
dataList=binned;}
if(this.requiresGeoLocation()){dataList=dataList.filter(r=>{return r.hasLocation();});}
let dataFilters=this.getDataFilters();if(dataFilters){dataList=dataList.filter(r=>{if(!this.checkDataFilters(dataFilters,r))return false;return true;});}
...
let dataFilterIds=[];if(this.getProperty("dataFilter")){this.getProperty("dataFilter").split(";").map(tok=>{[type,fieldId,value,enabled,label]=tok.split(",");if(!label)return;enabled=enabled=="true";let cbxid=this.getDomId("datafilterenabled_"+fieldId);dataFilterIds.push(cbxid);header2+=HtmlUtils.checkbox("",["id",cbxid],enabled)+" "+label+"&nbsp;&nbsp;"});}
if(this.getProperty("filterDate")){let type=this.getProperty("filterDate");var enums=[];if(this.getProperty("filterDateIncudeAll")){enums.push(["all","All"]);}
...
style+="display:none;";var selectId=this.getFilterId("filterDate");header2+=HtmlUtils.span(["class","display-filterby","style",style],HtmlUtils.span(["class","display-filterby-label"],"Select "+label+":")+HtmlUtils.select("",["fieldId","filterDate","style","","id",selectId],enums,selected))+"&nbsp;";}
...
this.jq(ID_HEADER2).html(header2);this.initHeader2();var theDisplay=this;dataFilterIds.map(id=>{$("#"+id).click(function(e){inputFunc($(this));});});if(filterBy.length>0){if(!hideFilterWidget){for(var i=0;i<dateIds.length;i++){HtmlUtils.datePickerInit(dateIds[i]);}}
...
tooltip=`<div style='padding:8px;'>${tooltip}</div>`;let newRow=[];if(debug&&rowIdx<debugRows)
...
console.log("\t fixed:"+fixedValueN);}else{if(formatNumbers){if(typeof value=="number"){value={v:value,f:String(this.formatNumber(value))};if(debug&&rowIdx<debugRows)
console.log("\t format:"+value+" "+(typeof value));}}
if(debug&&rowIdx<debugRows)
console.log("\t value:"+value+" "+(typeof value));newRow.push(value);}
...
this.map.clearSeenMarkers();var t1=new Date();this.addPoints(records,fields,points);var t2=new Date();this.addLabels(records,fields,points);this.applyVectorMap();},addPoints:function(records,fields,points){let cidx=0
let polygonField=this.getFieldById(fields,this.getProperty("polygonField"));let polygonColorTable=this.getColorTable(true,"polygonColorTable",null);let latlon=this.getProperty("latlon",true);let source=this;let radius=parseFloat(this.getDisplayProp(source,"radius",8));let strokeWidth=parseFloat(this.getDisplayProp(source,"strokeWidth","1"));let strokeColor=this.getDisplayProp(source,"strokeColor","#000");let sizeByAttr=this.getDisplayProp(source,"sizeBy",null);let isTrajectory=this.getDisplayProp(source,"isTrajectory",false);if(isTrajectory){let attrs={strokeWidth:2,strokeColor:"blue"}
...
let colorBy=this.getColorByInfo(records);let sizeBy={id:this.getDisplayProp(source,"sizeBy",null),minValue:0,maxValue:0,field:null,index:-1,isString:false,stringMap:{},};let sizeByMap=this.getProperty("sizeByMap");if(sizeByMap){let toks=sizeByMap.split(",");for(let i=0;i<toks.length;i++){let toks2=toks[i].split(":");if(toks2.length>1){sizeBy.stringMap[toks2[0]]=toks2[1];}}}
...
return null;};for(let i=0;i<records.length;i++){let record=records[i];let tuple=record.getData();if(!record.point)
...
p.push(new OpenLayers.Geometry.Point(lon1,lat1));p.push(new OpenLayers.Geometry.Point(lon2,lat2));let poly=this.map.addPolygon("polygon"+pIdx,"",p,polygonProps);poly.textGetter=textGetter;poly.record=record;if(date){poly.date=date.getTime();}
...
table+="</tr>";table+="<tr><td></td>";table+=`<td colspan=${cellCount}class=display-datatable-footer align=center id='` + this.getDomId("ct")+"'></td>";
	    table+="</tr>";
	    table +="</table>";
...
function RamaddaXlsDisplay(displayManager, id, properties) {
    ...
    RamaddaUtil.defineMembers(this, {
        ...
        showTableData: function(data) {
            ...
            var tableHtml = "<table width=100%style=\"max-width:1000px;\" > ";if(this.sheets.length>1){weight="10";}
