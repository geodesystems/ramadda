...
var list=[];var numeric=(type=="numeric");var isString=(type=="string");for(a in fields){var field=fields[a];if(type==null)return field;if(numeric){if(field.isFieldNumeric()){list.push(field);}}else if(isString){if(field.isFieldString()){list.push(field);}}else if(field.getType()==type){list.push(field);}}
return list;},getColumnValues:function(records,field){var values=[];var min=Number.MAX_VALUE;var max=Number.MIN_VALUE;for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];var row=record.getData();var value=row[field.getIndex()];values.push(value);if(Utils.isNumber(value)){min=Math.min(min,value);max=Math.max(max,value);}}
...
value=element.val();return value;},filterData:function(dataList,fields,doGroup,skipFirst){var pointData=this.getData();if(!dataList){if(pointData==null)return null;dataList=pointData.getRecords();}
...
if(this.getProperty(filterField.getId()+".filterShowButtons")){if(!includeAll&&dfltValue==FILTER_ALL)dfltValue=enums[0];var buttons="";for(var j=0;j<enums.length;j++){var extra="";if(enums[j]==dfltValue)extra=" display-filterby-button-selected ";buttons+=HtmlUtils.div(["class","display-filterby-button"+extra,"value",enums[j]],enums[j]);}
...
width=width+"px";return width;},getHeightForStyle:function(dflt){var height=this.getProperty("height",-1);if(height==-1)return dflt;if(!height.endsWith("px")&&!height.endsWith("%"))
...
tabTitles.push("Fields");tabContents.push(html);SUPER.RamaddaDisplay.getDialogContents.call(this,tabTitles,tabContents);},okToHandleEventRecordSelection:function(){return true;},handleEventRecordSelection:function(source,args){if(source==this){return;}
...
if(index>=0){this.slideIndex=index;this.displaySlide();}},updateUI:function(){var pointData=this.getData();if(pointData==null)return;this.records=this.filterData();if(!this.records)return;this.fields=this.getData().getRecordFields();this.records=this.sortRecords(this.records);var template=this.getProperty("template","");var slideWidth=this.getProperty("slideWidth","100%");var height=this.getProperty("height","400");var left=HtmlUtils.div(["id",this.getDomId(ID_PREV),"style","font-size:200%;","class","display-slides-arrow-left fas fa-angle-left"]);var right=HtmlUtils.div(["id",this.getDomId(ID_NEXT),"style","font-size:200%;","class","display-slides-arrow-right fas fa-angle-right"]);var slide=HtmlUtils.div(["style","height:"+height+"px;","id",this.getDomId(ID_SLIDE),"class","display-slides-slide"]);var contents="<table width=100%><tr><td valign=center width=20>"+left+"</td><td>"+
slide+"</td><td>"+
"<td valign=center width=20>"+right+"</td></tr></table>";this.writeHtml(ID_DISPLAY_CONTENTS,contents);this.jq(ID_PREV).click(()=>{this.slideIndex--;this.displaySlide(true);});this.jq(ID_NEXT).click(()=>{this.slideIndex++;this.displaySlide(true);});setTimeout(()=>{this.displaySlide();},200);},displaySlide:function(propagateEvent){if(this.slideIndex<0)this.slideIndex=0;if(this.slideIndex>=this.records.length)this.slideIndex=this.records.length-1;if(this.slideIndex==0)
...
for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);for(var col=0;col<fields.length;col++){var f=fields[col];var s=summary[f.getId()];var value=row[f.getIndex()];if(!Utils.isDefined(s.min)){s.min=value;s.max=value;}
...
s.counts[label]=tuple;s.values.push(tuple);for(var i=0;i<numBins;i++){var label=(Utils.formatNumber(s.min+binWidth*i))+" - "+Utils.formatNumber(s.min+binWidth*(i+1));s.bins.push(label);var tuple={value:label,count:0}
s.counts[label]=tuple;s.values.push(tuple);}
for(var i=0;i<s.numbers.length;i++){var value=s.numbers[i];var bin=0;if(!isNaN(value)){if(binWidth!=0){var perc=(value-s.min)/range;bin=Math.round(perc/(1/numBins))+1;if(bin>numBins)bin=numBins;}}
...
html+=HtmlUtils.openTag("div",["class","display-frequency-table","style",hor?"":"display:block;"]);html+=HtmlUtils.openTag("table",["id",this.getDomId("summary"+col),"table-height",this.getProperty("tableHeight","300",true),"class","stripe row-border nowrap ramadda-table"]);html+=HtmlUtils.openTag("thead",[]);var label=HtmlUtils.span(["title","Click to reset","class","display-frequency-label","data-field",s.field.getId()],f.getLabel());html+=HtmlUtils.tr([],HtmlUtils.th(["width","60%"],HtmlUtils.div(["style","max-width:500px;overflow-x:auto;"],label))+HtmlUtils.th(["align","right","width","20%"],HtmlUtils.div(["style","text-align:right"],"Count"))+HtmlUtils.th(["align","right","width","20%"],HtmlUtils.div(["style","text-align:right"],"Percent")));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);if(!f.isNumeric){s.values.sort((a,b)=>{if(a.count<b.count)return 1;if(a.count>b.count)return-1;return 0;});}
for(var i=0;i<s.values.length;i++){var value=s.values[i].value;var label=value;if(label=="")label="&lt;blank&gt;";var count=s.values[i].count;if(count==0)continue;value=value.replace(/\'/g,"&apos;");value=HtmlUtils.span(["title","Click to select","class","display-frequency-value","data-field",s.field.getId(),"data-value",value],label);html+=HtmlUtils.tr([],HtmlUtils.td([],value)+
...
var fields=this.getSelectedFields(this.getData().getRecordFields());var stringField=this.getFieldOfType(fields,"string");if(!stringField){this.displayError("No string field specified");return;}
var numericFields=this.getFieldsOfType(fields,"numeric");if(numericFields.length==0){this.displayError("No numeric fields specified");return;}
var theta=this.getColumnValues(records,stringField).values;var values=[];var min=Number.MAX_VALUE;var max=Number.MIN_VALUE;var plotData=[];for(var i=0;i<numericFields.length;i++){var field=numericFields[i];var column=this.getColumnValues(records,field);min=Math.min(min,column.min);max=Math.max(max,column.max);plotData.push({type:this.getPlotType(),r:column.values,theta:theta,fill:'toself',name:field.getLabel()});}
layout={polar:{radialaxis:{visible:true,range:[min,max]}},}
