...
return;}
if(prop.property=="filterValue"){if(!this.getProperty("acceptFilterEvent",true))return;this.haveCalledUpdateUI=false;var widgetId=this.getFilterId(prop.fieldId);this.settingFilterValue=true;if(Utils.isDefined(prop.value2)){$("#"+widgetId+"_min").val(prop.value);$("#"+widgetId+"_min").attr("data-value",prop.value);$("#"+widgetId+"_max").val(prop.value2);$("#"+widgetId+"_max").attr("data-value",prop.value2);}else{var widget=$("#"+widgetId);widget.val(prop.value);widget.attr("data-value",prop.value);if(widget.attr("isButton")){widget.find(".display-filterby-button").removeClass("display-filterby-button-selected");widget.find("[value='"+prop.value+"']").addClass("display-filterby-button-selected");}}
...
return{values:values,min:min,max:max};},requiresGrouping:function(){return false;},getFilterFieldValues:function(field){var element=$("#"+this.getDomId("filterby_"+field.getId()));var value=null;if(element.attr("isButton")){value=element.attr("data-value");}else{value=element.val();}
...
value=[date1,date2];}else{values=this.getFilterFieldValues(filterField);if(!values&&!Array.isArray(values))values=[values];if(values.length==0)continue;var tmp=[];values.map(v=>{tmp.push(v.replace(/_comma_/g,","));});values=tmp;var useIfAll=this.getProperty(filterField.getId()+".filterUseIfAllIsSet",true);if(!useIfAll){var allIsUsed=false;if(values.includes(FILTER_ALL)){allIsUsed=true;break;}
