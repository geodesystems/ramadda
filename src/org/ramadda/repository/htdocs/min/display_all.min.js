...
var record=selected[rowIdx];var row=this.getDataValues(record);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);s=this.getRecordTemplate(row,fields,s,props);contents+=HtmlUtils.div(["style","","id",this.getId()+"-"+record.getId(),"title","","class","display-template-record","recordIndex",rowIdx],s);if(cols>0){contents+='</div>\n';}}
if(cols>0){contents+='</div>';}}
if(selected.length>0)
contents+=footerTemplate;this.writeHtml(ID_DISPLAY_CONTENTS,contents);this.makeTooltips(this.jq(ID_DISPLAY_CONTENTS).find(".display-template-record"),selected);},highlightCount:0,handleEventRecordHighlight:function(source,args){let myCount=++this.highlightCount;var id="#"+this.getId()+"-"+args.record.getId();var container=this.jq(ID_DISPLAY_CONTENTS);if(args.highlight){setTimeout(()=>{if(myCount==this.highlightCount){var element=$(id);var css=this.getProperty("highlightOnCss","").split(",");if(css.length>1){for(var i=0;i<css.length;i+=2)
element.css(css[i],css[i+1]);}else{element.addClass("display-template-record-highlight");}
try{var eo=element.offset();if(eo==null)return;var etop=eo.top;var ctop=container.offset().top;var scrollTop=container.scrollTop();container.scrollTop(etop-ctop+scrollTop)}catch(err){console.log("Error:"+err);}}},500);}else{var css=this.getProperty("highlightOffCss","").split(",");var element=$(id);if(css.length>1){for(var i=0;i<css.length;i+=2)
