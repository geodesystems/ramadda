...
return source.getProperty(prop,dflt);return null;},getProperty:function(key,dflt,skipThis){return this.getPropertyInner(key,dflt,skipThis);},getPropertyInner:function(key,dflt,skipThis){if(!skipThis&&Utils.isDefined(this[key])){return this[key];}
var value=this.properties[key];if(value!=null){return value;}
if(this.displayParent!=null){return this.displayParent.getProperty(key,dflt,skipThis);}
if(this.getDisplayManager){return this.getDisplayManager().getProperty(key,dflt);}
...
let roots=[];var parentField=this.getFieldById(null,this.getProperty("parentField"));var labelField=this.getFieldById(null,this.getProperty("labelField"));var idField=this.getFieldById(null,this.getProperty("idField"));if(!parentField){throw new Error("No parent field specified");return;}
if(!idField){throw new Error("No id field specified");}
let idToNode={};let nodes=[];let idToRoot={};records.map(r=>{var parent=r.getValue(parentField.getIndex());var id=r.getValue(idField.getIndex());var label=labelField==null?id:r.getValue(labelField.getIndex());var node={id:id,label:label,children:[],record:r,parentId:parent,parent:null};idToNode[id]=node;nodes.push(node);if(parent==""){idToRoot[id]=node;roots.push(node);}});nodes.map(node=>{let parentNode=idToNode[node.parentId];if(!parentNode){if(!idToRoot[node.id]){throw new Error("No parent :"+node.parentId+" for node:"+node.label);}
return;}
...
chartOptions.orientation="horizontal";return new google.visualization.BarChart(document.getElementById(this.getChartId()));}});}
...
if(select=="max"||select=="min"||select=="="||select=="<"||select==">"||select=="<="||select=="?>="||select=="match"){var selectField=this.getProperty("selectField","");if(selectField)selectField=this.getFieldById(fields,selectField);if(!selectField){this.writeHtml(ID_DISPLAY_CONTENTS,"No selectField specified");return;}
var selectValue=this.getProperty("selectValue","XXX");var selectValueNum=parseFloat(selectValue);var max=0;var min=0;var cnt=0;var maxRecord;var minRecord;var equalsRecord;records.map(record=>{var v=record.getValue(selectField.getIndex());if(select=="match"){if(v.match(selectValue)){selected.push(record);}
return;}
...
var html="";let baseId=this.getDomId("node");let cnt=0;let depth=0;let maxDepth=+this.getProperty("maxDepth",10);let template=this.getProperty("recordTemplate","${default}");let showDetails=this.getProperty("showDetails",true);let _this=this;let func=function(node){cnt++;if(node.record){_this.countToRecord[cnt]=node.record;}
...
values.push(node.value);ids.push(node.id);labels.push(node.label);parents.push(node.parent==null?"":node.parent.id);node.children.map(makeList);}
roots.map(makeList);var data=[{type:"sunburst",ids:ids,labels:labels,parents:parents,outsidetextfont:{size:20,color:"#377eb8"},leaf:{opacity:0.4},marker:{line:{width:2}},branchvalues:'total'}];if(valueField){data[0].values=values;}
var colors=this.getColorTable(true);if(!colors){var colorMap=Utils.parseMap(this.getProperty("colorMap"));if(colorMap){colors=[];let dfltIdx=0;let dflt=["#1f77b4","#ff7f0e","#2ca02c","#d62728","#9467bd","#8c564b","#e377c2","#7f7f7f","#bcbd22","#17becf"];ids.map((id,idx)=>{let color=colors[id];if(!color){color=colorMap[labels[idx]];}
if(!color){if(dfltIdx>=dflt.length)dfltIdx=0;color=dflt[dfltIdx];dfltIdx++;}
color="#000";colors.push(color);});}}
var layout={margin:{l:0,r:0,b:0,t:0},width:+this.getProperty("width"),height:+this.getProperty("height"),};if(colors){layout.sunburstcolorway=colors;layout.extendsunburstcolors=false;}
