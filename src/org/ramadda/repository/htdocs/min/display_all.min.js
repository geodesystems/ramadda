...
this.layoutCells();}},addEntry:async function(name,entryId){var entry;await this.getEntry(entryId,e=>entry=e);this.currentEntries[name]={entryId:entryId,entry:entry};},getCurrentEntries:function(){return this.currentEntries;},clearEntries:function(){this.currentEntries={};for(a in this.baseEntries)
...
alert("Notebook saved");});},getJson:function(output){var obj={cells:[],currentEntries:{}};for(var name in this.currentEntries){var e=this.currentEntries[name];obj.currentEntries[name]={entryId:e.entryId};}
...
_this.notebook.runAll();}}});},makeMenu:function(){let _this=this;var space="&nbsp;&nbsp;";var line="<tr><td colspan=2 style='border-top:1px #ccc solid;'</td></tr>";var menu="";menu+=HtmlUtils.input(ID_CELLNAME_INPUT,_this.cellName,["placeholder","Cell name","style","width:100%;","xsize","20","id",_this.getDomId(ID_CELLNAME_INPUT)]);menu+="<br>";menu+="<table>";menu+="<tr><td align=right><b>Run:</b>&nbsp;</td><td>";menu+=HtmlUtils.div(["title","shift-return","class","ramadda-link","what","run"],"This cell")+space;menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","runall"],"All");menu+="</td></tr>"
...
menu+="<tr><td align=right><b>Edit:</b>&nbsp;</td><td>";menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","showall"],"Show All");menu+=space;menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","hideall"],"Hide All");menu+="</td></tr>";menu+=line;menu+="</table>";menu+=HtmlUtils.div(["class","ramadda-link","what","savewithout"],"Save Notebook (w/o output)")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","savewith"],"Save Notebook (with output)")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","delete"],"Delete")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","help"],"Help")+"<br>";var popup=_this.jq(ID_MENU);popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();popup.position({of:_this.menuButton,my:"left top",at:"right top",collision:"fit fit"});_this.jq(ID_SHOWHEADER_INPUT).focus();_this.jq(ID_SHOWHEADER_INPUT).change(function(e){_this.showHeader=_this.jq(ID_SHOWHEADER_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_SHOWBORDER_INPUT).change(function(e){_this.showBorder=_this.jq(ID_SHOWBORDER_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_SHOWEDIT_INPUT).change(function(e){_this.showEdit=_this.jq(ID_SHOWEDIT_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_PINNED_INPUT).change(function(e){_this.pinned=_this.jq(ID_PINNED_INPUT).is(':checked');_this.checkMenuButton(false);});_this.jq(ID_CELLNAME_INPUT).keypress(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){_this.cellName=$(this).val();_this.applyStyle();popup.hide();return;}});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");popup.hide();if(what=="run"){_this.run();}else if(what=="showthis"){_this.showEdit=true;_this.applyStyle();}else if(what=="hidethis"){_this.showEdit=false;_this.applyStyle();}else if(what=="showall"){_this.notebook.toggleAll(true);}else if(what=="hideall"){_this.notebook.toggleAll(false);}else if(what=="runall"){_this.notebook.runAll();}else if(what=="clear"){_this.clearOutput();}else if(what=="clearall"){_this.notebook.clearOutput();}else if(what=="moveup"){_this.notebook.moveCellUp(_this);}else if(what=="movedown"){_this.notebook.moveCellDown(_this);}else if(what=="newabove"){_this.notebook.newCellAbove(_this);}else if(what=="newbelow"){_this.notebook.newCellBelow(_this);}else if(what=="savewith"){_this.notebook.saveNotebook(true);}else if(what=="savewithout"){_this.notebook.saveNotebook(false);}else if(what=="help"){var win=window.open(ramaddaBaseUrl+"/userguide/wikidisplay.html#notebook",'_blank');win.focus();}else if(what=="delete"){_this.askDelete();}});},checkMenuButton:function(vis){if(vis||this.pinned){this.menuButton.css("display","block");this.cell.find(".display-notebook-gutter-container").css("background","rgb(250,250,250)");this.cell.find(".display-notebook-gutter-container").css("border-right","1px #ccc solid");}else{this.menuButton.css("display","none");this.cell.find(".display-notebook-gutter-container").css("background","#fff");this.cell.find(".display-notebook-gutter-container").css("border-right","1px #fff solid");}},applyStyle:function(){this.checkMenuButton(false);if(this.showHeader){this.header.css("display","block");}else{this.header.css("display","none");}
...
if(h=="")h="No entries";div.set(h);},processCommand_cd:async function(line,toks,div){if(div==null)div=new Div();if(toks.length<=1){this.setCurrentEntry(this.notebook.currentEntry);return this.getEntryHeading(this.currentEntry,div);}
