...
if(i>0){html+=HtmlUtils.closeTag(TAG_DIV);}}}else if(this.layout==LAYOUT_TABLE){if(displaysToLayout.length==1){html+=displaysToLayout[0].layoutDiv;}else{var percent=Math.round((100/this.columns))+"%";var i=0;html+="<table width=100%><tr valign=top>\n";var colCnt=0;for(var i=0;i<displaysToLayout.length;i++){colCnt++;if(colCnt>this.columns){if(i>0){html+=HtmlUtils.closeTag(TAG_TR);}
...
var tooltip=this.getProperty("tooltip");var html=this.getRecordHtml(pointRecord,fields,tooltip);if(polygonField){var s=values[polygonField.getIndex()];var delimiter;[";",","].map(d=>{if(s.indexOf(d)>=0)delimiter=d;});var toks=s.split(delimiter);var polygonProps={};$.extend(polygonProps,props);if(polygonProps.strokeWidth==0)
...
lastPoint=point;var date=pointRecord.getDate();if(mapPoint){mapPoint.record=pointRecord;mapPoint.hasColorByValue=hasColorByValue;mapPoint.colorByValue=colorByValue;mapPoint.colorByColor=colorByColor;if(date){mapPoint.date=date.getTime();}
