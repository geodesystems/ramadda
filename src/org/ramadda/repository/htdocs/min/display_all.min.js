...
value+"</td></tr>";}
html+=HtmlUtils.closeTag("table");html+=HtmlUtils.closeTag("div");this.setContents(html);let _this=this;this.jq(ID_TABLE).find(".display-ranking-row").click(function(e){_this.getDisplayManager().propagateEventRecordSelection(_this,_this.getPointData(),{index:parseInt($(this).attr("what"))-1});});this.jq("sortfields").change(function(){_this.setProperty("sortField",$(this).val());_this.updateUI();});},});}
...
var html=this.getRecordHtml(pointRecord,fields);if(polygonField){var s=values[polygonField.getIndex()];var delimiter;[";",","].map(d=>{if(s.indexOf(d)>=0)delimiter=d;});var toks=s.split(delimiter);var polygonProps={};$.extend(polygonProps,props);if(polygonProps.strokeWidth==0)
