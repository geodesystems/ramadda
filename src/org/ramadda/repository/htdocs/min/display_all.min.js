...
return;}
if(prop.property=="filterValue"){if(!this.getProperty("acceptFilterEvent",true))return;this.haveCalledUpdateUI=false;var widgetId=this.getFilterId(prop.fieldId);this.settingFilterValue=true;if(Utils.isDefined(prop.value2)){this.jq(widgetId+"_min").val(prop.value);this.jq(widgetId+"_min").attr("value",prop.value);this.jq(widgetId+"_max").val(prop.value2);this.jq(widgetId+"_max").attr("value",prop.value2);}else{$("#"+widgetId).val(prop.value);$("#"+widgetId).attr("value",prop.value);}
...
return{values:values,min:min,max:max};},requiresGrouping:function(){return false;},getFilterFieldValue:function(field){var value;var element=$("#"+this.getDomId("filterby_"+field.getId()));value=element.attr("value");if(!value)
value=element.val();return value;},filterData:function(dataList,fields,doGroup,skipFirst){var t1=new Date();var pointData=this.getData();if(!dataList){if(pointData==null)return null;dataList=pointData.getRecords();}
...
dataList=binned;}
dataList=this.sortRecords(dataList);return dataList;},canDoGroupBy:function(){return false;},canDoMultiFields:function(){return true;},useChartableFields:function(){return true;},getFieldsToSelect:function(pointData){if(this.useChartableFields())
...
var inputFunc=function(input,input2){var id=input.attr("id");if(!input2){if(id.endsWith("_min")){input2=$("#"+id.replace(/_min$/,"_max"));}else if(id.endsWith("_max")){var tmp=input;input=$("#"+id.replace(/_max$/,"_min"));input2=tmp;}}
var value=input.val();if(!value||value=="")
...
_this.propagateEvent("handleEventPropertyChanged",args);_this.settingFilterValue=false;};this.jq(ID_FILTERBAR).find(".display-filterby-buttons").each(function(){let parent=$(this);$(this).find(".display-filterby-button").click(function(){parent.find(".display-filterby-button").removeClass("display-filterby-button-selected");$(this).addClass("display-filterby-button-selected");var value=$(this).attr("value");parent.attr("value",value);$("#"+parent.attr("id")+"_label").html(value==FILTER_ALL?"&nbsp;":value);inputFunc(parent);});});this.jq(ID_FILTERBAR).find(".display-filter-input").keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){return;}
...
return" ["+this.unit+"]";return"";},getLabel:function(){if(this.label==null||this.label.length==0)return this.id;return this.label;},setLabel:function(l){this.label=l;},isString:function(){return this.type=="string"||this.type=="enumeration";},getType:function(){return this.type;},getMissing:function(){return this.missing;},setUnit:function(u){this.unit=u;},getUnit:function(){return this.unit;}});}
...
if(!args.record)return;var index=this.recordToIndex[args.record.getId()];if(Utils.isDefined(index)){this.setChartSelection(index);}},getFieldsToSelect:function(pointData){return pointData.getChartableFields();},canDoGroupBy:function(){return false;},clearCache:function(){this.computedData=null;},googleChartCallbackPending:false,includeIndexInData:function(){return false;},getGroupBy:function(){return this.groupBy;},getIncludeIndexIfDate:function(){return false;},displayData:function(){var _this=this;if(!this.getDisplayReady()){return;}
...
var matchedFeatures=[];var seen={};var maxCnt=-1;var minCnt=-1;for(var i=0;i<circles.length;i++){var circle=circles[i];if(circle.style&&circle.style.display=="none")continue;var center=circle.center;var tmp={index:-1,maxExtent:maxExtent};var matchedFeature=this.findContainingFeature(features,center,tmp);if(circle.hasColorByValue&&isNaN(circle.colorByValue))continue;if(!matchedFeature)continue;maxExtent=tmp.maxExtent;if(!seen[matchedFeature.featureIndex]){seen[matchedFeature.featureIndex]=true;matchedFeatures.push(matchedFeature);}
...
if(isNaN(props.pointRadius)||props.pointRadius==0)props.pointRadius=radius;var hasColorByValue=false;var colorByValue;if(colorBy.index>=0){var value=pointRecord.getData()[colorBy.index];hasColorByValue=true;colorByValue=value;didColorBy=true;props.fillColor=colorBy.getColor(value,pointRecord);}
...
lastPoint=point;var date=pointRecord.getDate();if(mapPoint){mapPoint.record=pointRecord;mapPoint.hasColorByValue=hasColorByValue;mapPoint.colorByValue=colorByValue;if(date){mapPoint.date=date.getTime();}
...
function RamaddaDotplotDisplay(displayManager,id,properties){$.extend(this,{width:"600px",height:"400px",});let SUPER=new RamaddaPlotlyDisplay(displayManager,id,DISPLAY_PLOTLY_DOTPLOT,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{getDisplayStyle:function(){return"";return"border: 1px #ccc solid;";},updateUI:function(){var records=this.filterData();if(!records)return;var fields=this.getSelectedFields(this.getData().getRecordFields());var stringField=this.getFieldOfType(fields,"string");if(!stringField){stringField=fields[0];}
var fields=this.getFieldsOfType(fields,"numeric");if(fields.length==0){this.displayError("No numeric fields specified");return;}
...
var plotData=[];for(i in fields){var color=i>=colors.length?colors[0]:colors[i];var field=fields[i];var values=this.getColumnValues(records,field).values;if(!labels){labels=[];for(var j=0;j<values.length;j++){labels.push("Point "+(j+1));}}
