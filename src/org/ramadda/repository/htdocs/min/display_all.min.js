...
function DisplayAnimation(display){var ID_RUN="animrun";var ID_NEXT="animnext";var ID_PREV="animprev";var ID_BEGIN="animbegin";var ID_END="animend";var ID_SLIDER="slider";var ID_SHOWALL="showall";var ID_ANIMATION_LABEL="animationlabel";this.display=display;$.extend(this,{running:false,inAnimation:false,begin:null,end:null,dateMin:null,dateMax:null,dateRange:0,dateFormat:display.getProperty("animationDateFormat","yyyyMMdd"),mode:display.getProperty("animationMode","cumulative"),speed:parseInt(display.getProperty("animationSpeed",250)),toggleAnimation:function(){this.running=!this.running;if(this.btnRun)
...
buttons+=HtmlUtils.span(["id",this.getDomId(ID_ANIMATION_LABEL),"class","display-animation-label"]);buttons=HtmlUtils.div(["class","display-animation-buttons"],buttons);if(display.getProperty("animationShowSlider",true)){buttons+=HtmlUtils.div(["class","display-animation-slider","id",this.getDomId(ID_SLIDER)]);}
...
contents+=footerTemplate;this.writeHtml(ID_DISPLAY_CONTENTS,contents);this.makeTooltips(this.jq(ID_DISPLAY_CONTENTS).find(".display-template-entry"),selected);},handleEventRecordHighlight:function(source,args){var element=$("#"+this.getId()+"-"+args.record.getId());if(args.highlight)
element.addClass("display-template-record-highlight");else
...
var date=pointRecord.getDate();if(mapPoint){mapPoint.hasColorByValue=hasColorByValue;mapPoint.colorByValue=colorByValue;if(date){mapPoint.date=date.getTime();}
