...
var currentNotebookState;var iodide={test:function(){return"test it";},addOutputRenderer:function(renderer){},output:{text:function(t){currentNotebookState.iodideText(t);},element:function(t){console.log("element:"+t);return currentNotebookState.iodideElement(t);}}}
...
function NotebookState(cell,div){this.id=HtmlUtils.getUniqueId();this.cell=cell;this.notebook=cell.notebook;$.extend(this,{entries:{},div:div,stopFlag:false,result:null,getStop:function(){return this.stopFlag;},getCell:function(){return this.cell;},iodideText:function(t){this.write(t);},iodideElement:function(e){var id=HtmlUtils.getUniqueId();this.write(HtmlUtils.tag(e,["id",id]));return document.getElementById(id);},setValue:function(name,value){this.notebook.setCellValue(name,value);},makeData:async function(entry){if(!entry)
...
chunk.div.set(HtmlUtils.div(["class","display-notebook-md"],html));},processPy:async function(chunk){if(!this.notebook.loadedPyodide){chunk.div.set("Loading Python...");await Utils.importCSS(ramaddaBaseHtdocs+"/lib/fontawesome/font-awesome.css");await Utils.importJS(ramaddaBaseHtdocs+"/lib/pyodide/pyodide.js");await languagePluginLoader.then(()=>{pyodide.runPython('import sys\nsys.version;');},(e)=>console.log("error:"+e));await pyodide.loadPackage(['numpy','cycler','pytz','matplotlib'])
chunk.div.set("");this.notebook.loadedPyodide=true;}
var state=new NotebookState(this,chunk.div);currentNotebookState=state;var output="";pyodide.runPython(chunk.content);chunk.div.append(output);},processWiki:async function(chunk){this.rawOutput+=chunk.content+"\n";var id=this.notebook.getProperty("entryId","");await this.getCurrentEntry(e=>entry=e);if(entry)id=entry.getId();let _this=this;let divId=HtmlUtils.getUniqueId();var wikiCallback=function(html){var h=HtmlUtils.div(["id",divId,"style"],html);chunk.div.set(h);}
...
await cmd.proc.call(_this,cmd.line,cmd.toks,cmd.div,cmd.extra);}},processJs:async function(chunk){var lines;var topLines=0;try{await this.getCurrentEntry(e=>{current=e});var state=new NotebookState(this,chunk.div);currentNotebookState=state;notebookStates[state.id]=state;var notebookEntries=this.notebook.getCurrentEntries();for(name in notebookEntries){state.entries[name]=notebookEntries[name].entry;}
