...
this.displaySearchResults(records);},displaySearchResults:function(records){var width=this.getProperty("imageWidth","50");var margin=this.getProperty("imageMargin","0");var groups={};var groupCnt={};var groupHeaders=[];if(!this.groupField){groups[""]="";groupCnt[""]=0;}
for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);var contents="";var tooltip="";this.tooltipFields.map(field=>{if(tooltip!="")tooltip+="&#10;";tooltip+=field.getValue(row);});var label="";var caption="";if(this.captionFields.length>0){if(this.captionTemplate)caption=this.captionTemplate;this.captionFields.map(field=>{if(this.captionTemplate)
...
html+="</tr></table>";}
this.writeHtml(ID_RESULTS,html);this.jq(ID_RESULTS).find("a.display-images-popup").fancybox({caption:function(instance,item){return $(this).data('caption')||'';}});}});}
