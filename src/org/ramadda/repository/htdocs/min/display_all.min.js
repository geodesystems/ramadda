...
return{values:values,min:min,max:max};},requiresGrouping:function(){return false;},filterData:function(dataList,fields,doGroup,skipFirst){var pointData=this.getData();if(!dataList){if(pointData==null)return null;dataList=pointData.getRecords();}
...
var newData=[];var values=[];for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];var prefix=this.getProperty(filterField.getId()+".filterPrefix");var suffix=this.getProperty(filterField.getId()+".filterSuffix");if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;var value;var _values=[];if(filterField.isNumeric){var minField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_min"));var maxField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_max"));var minValue=parseFloat(minField.val().trim());var maxValue=parseFloat(maxField.val().trim());var dfltMinValue=parseFloat(minField.attr("data-min"));var dfltMaxValue=parseFloat(maxField.attr("data-max"));if(minValue!=dfltMinValue||maxValue!=dfltMaxValue){value=[minValue,maxValue];}else{value=[NaN,NaN];}}else if(filterField.getType()=="date"){var date1=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date1")).val();var date2=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date2")).val();if(date1!=null&&date1.trim()!="")
date1=Utils.parseDate(date1);else
date1=null;if(date2!=null&&date2.trim()!="")
date2=Utils.parseDate(date2);else
date2=null;value=[date1,date2];}else{var element=$("#"+this.getDomId("filterby_"+filterField.getId()));value=element.attr("value");if(!value)
value=element.val();if(!Array.isArray(value))value=[value];value.map(v=>_values.push((""+v).toLowerCase()));}
var filterStartsWith=this.getProperty(filterField.getId()+".filterStartsWith",false);var anyValues=false;_values.map(v=>{if(v.length>0)anyValues=true});values.push({value:value,_values:_values,anyValues:anyValues,startsWith:filterStartsWith});}
...
ok=false;}}else{var startsWith=values[i].startsWith;var _values=values[i]._values;if(values[i].anyValues){ok=false;for(var j=0;j<_values.length;j++){var fv=_values[j];value=(""+value).toLowerCase();if(startsWith){if(value.startsWith(fv)){ok=true;break;}}else if(value.indexOf(fv)>=0){ok=true;break;}}}}}
...
var pattern=this.getProperty("pattern");if(pattern&&pattern.length==0)pattern=null;this.writeHtml(ID_TOP_RIGHT,HtmlUtils.span(["id",this.getDomId(ID_LABEL)]," ")+" "+HtmlUtils.input("pattern",(pattern?pattern:""),["placeholder","Search text","id",this.getDomId(ID_SEARCH)]));let _this=this;this.jq(ID_SEARCH).keypress(function(event){if(event.which==13){_this.setProperty("pattern",$(this).val());_this.propagateEvent("handleEventPropertyChanged",{property:"pattern",value:$(this).val()});_this.updateUI();}});var height=this.getProperty("height","600");var html=HtmlUtils.div(["id",this.getDomId(ID_TEXT),"style","padding:4px;border:1px #ccc solid;border-top:1px #ccc solid; max-height:"+height+"px;overflow-y:auto;"]);this.writeHtml(ID_DISPLAY_CONTENTS,html);this.showText();},handleEventPropertyChanged:function(source,prop){if(prop.property=="pattern"){this.setProperty("pattern",prop.value);this.updateUI();return;}
...
var lineCnt=0;var displayedLineCnt=0;var re;if(pattern){re=new RegExp("("+pattern+")");}
for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];this.indexToRecord[rowIdx]=record;this.recordToIndex[record.getId()]=rowIdx;var row=this.getDataValues(record);var line="";for(var col=0;col<fields.length;col++){var f=fields[col];line+=" ";line+=row[f.getIndex()];}
line=line.trim();if(!includeEmptyLines&&line.length==0)continue;line=line.replace(/</g,"&lt;").replace(/>/g,"&gt;");lineCnt++;if(re){if(!line.toLowerCase().match(re))continue;line=line.replace(re,"<span style=background:yellow;>$1</span>");}
