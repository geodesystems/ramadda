...
var v=this.getProperty("showTitle",dflt);return v;},getTimeZone:function(){return this.getProperty("timeZone");},formatDate:function(date,args){try{return this.formatDateInner(date,args);}catch(e){console.log("Error formatting date:"+e);if(!date.getTime&&date.v)date=date.v;return""+date;}},formatDateInner:function(date,args){if(this.dateFormat=="yyyy"){return Utils.formatDateYYYY(date);}else if(this.dateFormat=="yyyyMMdd"){return Utils.formatDateYYYYMMDD(date);}else if(this.dateFormat=="yyyyMM"){return Utils.formatDateYYYYMM(date);}else if(this.dateFormat=="yearmonth"){return Utils.formatDateYearMonth(date);}else if(this.dateFormat=="monthdayyear"){return Utils.formatDateMonthDayYear(date);}else if(this.dateFormat=="monthday"){return Utils.formatDateMonthDay(date);}else if(this.dateFormat=="mdy"){return Utils.formatDateMDY(date);}else if(this.dateFormat=="hhmm"){return Utils.formatDateHHMM(date);}
...
let html="";if(template){html=headerTemplate;}else{html="<br>";html+=HtmlUtils.openTag("table",["class","nowrap ramadda-table","id",this.getDomId("percentchange")]);html+=HtmlUtils.openTag("thead",[]);html+="\n";html+=HtmlUtils.tr([],HtmlUtils.th(["style","text-align:center"],"Field")+HtmlUtils.th(["style","text-align:center"],label1)+HtmlUtils.th(["style","text-align:center"],label2)
+HtmlUtils.th(["style","text-align:center"],"Percent Change"));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);}
var tuples=[];fields.map(f=>{var val1=record1.getValue(f.getIndex());var val2=record2.getValue(f.getIndex());var percent=parseInt(1000*(val2-val1)/val1)/10;tuples.push({field:f,val1:val1,val2:val2,percent:percent});});tuples.sort((a,b)=>{return-(a.percent-b.percent);})
