...
this.currentEntries[a]=this.baseEntries[a];var id=this.getProperty("entryId","");var url=ramaddaBaseUrl+"/getnotebook?entryid="+id;url+="&notebookId="+this.getProperty("notebookId","default_notebook");var jqxhr=$.getJSON(url,function(data){_this.loadJson(data);}).fail(function(){var props={showInput:true,}
...
return false;return true;},getJson:function(output){var obj={cells:[],currentEntries:{},runOnLoad:this.runOnLoad,displayMode:this.displayMode,};for(var name in this.currentEntries){var e=this.currentEntries[name];obj.currentEntries[name]={entryId:e.entryId};}
...
this.cells=cells;if(this.cells.length==0){this.addCell("",null);}},runAll:function(){for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];if(!cell.run()){break;}}},toggleAll:function(on){for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];cell.showInput=on;cell.applyStyle();}},});}
...
var notebookStates={};function RamaddaNotebookCell(notebook,id,content,props){this.notebook=notebook;var ID_CELL="cell";var ID_GUTTER="gutter";var ID_GUTTER_CONTAINER="guttercontainer";var ID_HEADER="header";var ID_INPUT="input";var ID_INPUT_TOOLBAR="inputtoolbar";var ID_OUTPUT="output";var ID_MESSAGE="message";var ID_BUTTON_MENU="menubutton";var ID_BUTTON_RUN="runbutton";var ID_BUTTON_TOGGLE="togglebutton";var ID_MENU="menu";var ID_CELLNAME_INPUT="cellnameinput";var ID_SHOWHEADER_INPUT="showheader";var ID_SHOWBORDER="showborder";var ID_SHOWEDIT="showedit";var ID_RUN_ON_LOAD="runonload";var ID_DISPLAY_MODE="displaymode";let SUPER=new DisplayThing(id,{});RamaddaUtil.inherit(this,SUPER);RamaddaUtil.defineMembers(this,{id:id,inputRows:1,content:content,outputHtml:"",showInput:false,showHeader:false,showBorder:false,cellName:"",});if(props){$.extend(this,props);}
RamaddaUtil.defineMembers(this,{getJson:function(output){var obj={id:this.id,inputRows:this.inputRows,content:this.content,showInput:this.showInput,showHeader:this.showHeader,showBorder:this.showBorder,cellName:this.cellName,};if(this.currentEntry)
...
menu+=line;menu+="<tr valign=top><td align=right><b>Show:</b>&nbsp;</td><td>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_SHOWEDIT),[],_this.showInput)+" Input";menu+=space;menu+=HtmlUtils.checkbox(_this.getDomId(ID_SHOWBORDER),[],_this.showBorder)+" Border";menu+="</td></tr>"
menu+="<tr><td align=right><b>Inputs:</b>&nbsp;</td><td>";menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","showall"],"Show All");menu+=space;menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","hideall"],"Hide All");menu+="</td></tr>";menu+=line;menu+="</table>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_RUN_ON_LOAD),[],_this.notebook.runOnLoad)+" Run on load"+"<br>";menu+=HtmlUtils.div(["title","Don't show the left side and input for anonymous users"],HtmlUtils.checkbox(_this.getDomId(ID_DISPLAY_MODE),[],_this.notebook.displayMode)+" Display mode"+"<br>");menu+=line2;menu+=HtmlUtils.div(["class","ramadda-link","what","savewithout"],"Save Notebook (w/o output)")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","savewith"],"Save Notebook (with output)")+"<br>";menu+=line2;menu+=HtmlUtils.div(["class","ramadda-link","what","delete"],"Delete")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","help"],"Help")+"<br>";var popup=_this.jq(ID_MENU);popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();popup.position({of:src,my:"left top",at:at,collision:"fit fit"});_this.jq(ID_SHOWHEADER_INPUT).focus();_this.jq(ID_SHOWHEADER_INPUT).change(function(e){_this.showHeader=_this.jq(ID_SHOWHEADER_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_RUN_ON_LOAD).change(function(e){_this.notebook.runOnLoad=_this.jq(ID_RUN_ON_LOAD).is(':checked');});_this.jq(ID_DISPLAY_MODE).change(function(e){_this.notebook.displayMode=_this.jq(ID_DISPLAY_MODE).is(':checked');});_this.jq(ID_SHOWBORDER).change(function(e){_this.showBorder=_this.jq(ID_SHOWBORDER).is(':checked');_this.applyStyle();});_this.jq(ID_SHOWEDIT).change(function(e){_this.showInput=_this.jq(ID_SHOWEDIT).is(':checked');_this.applyStyle();});_this.jq(ID_CELLNAME_INPUT).keypress(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){_this.cellName=$(this).val();_this.applyStyle();popup.hide();return;}});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");popup.hide();_this.processCommand(what);});},processCommand:function(command){if(command=="run"){this.run();}else if(command=="showmenu"){this.makeMenu();}else if(command=="toggle"){this.showInput=!this.showInput;this.applyStyle();}else if(command=="showthis"){this.showInput=true;this.applyStyle();}else if(command=="hidethis"){this.showInput=false;this.applyStyle();}else if(command=="showall"){this.notebook.toggleAll(true);}else if(command=="hideall"){this.notebook.toggleAll(false);}else if(command=="runall"){this.notebook.runAll();}else if(command=="clear"){this.clearOutput();}else if(command=="clearall"){this.notebook.clearOutput();}else if(command=="moveup"){this.notebook.moveCellUp(this);}else if(command=="movedown"){this.notebook.moveCellDown(this);}else if(command=="newabove"){this.notebook.newCellAbove(this);}else if(command=="newbelow"){this.notebook.newCellBelow(this);}else if(command=="savewith"){this.notebook.saveNotebook(true);}else if(command=="savewithout"){this.notebook.saveNotebook(false);}else if(command=="help"){var win=window.open(ramaddaBaseUrl+"/userguide/notebook.html",'_blank');win.focus();}else if(command=="delete"){this.askDelete();}else{console.log("unknown command:"+command);}},checkMenuButton:function(vis){if(vis){this.gutter.css("display","block");this.cell.find(".display-notebook-gutter-container").css("background","rgb(250,250,250)");}else{this.gutter.css("display","none");this.cell.find(".display-notebook-gutter-container").css("background","#fff");}},applyStyle:function(){this.checkMenuButton(false);if(this.showHeader){this.header.css("display","block");}else{this.header.css("display","none");}
this.header.html(this.cellName);if(this.showInput&&this.notebook.showInput()){this.toggleButton.html(HtmlUtils.image(Utils.getIcon("togglearrowdown.gif")));this.jq(ID_INPUT_TOOLBAR).css("display","block");this.inputContainer.css("display","block");}else{this.toggleButton.html(HtmlUtils.image(Utils.getIcon("togglearrowright.gif")));this.jq(ID_INPUT_TOOLBAR).css("display","none");this.inputContainer.css("display","none");}
