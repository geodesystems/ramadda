...
this.displaySearchResults(records);},displaySearchResults:function(records){var width=this.getProperty("imageWidth","50");var margin=this.getProperty("imageMargin","0");var splits={};var splitCnt={};var splitHeaders=[];if(!this.splitField){splits[""]="";splitCnt[""]=0;}
...
var img=HtmlUtils.href(img,HtmlUtils.image(img,["width",width]),["class","display-images-popup"]);var html=HtmlUtils.div(["class","display-images-item","title",tooltip,"style","margin:"+margin+"px;"],img+label);if(this.splitField){var splitOn=row[this.splitField.getIndex()];if(!splits[splitOn]){splitCnt[splitOn]=0;splits[splitOn]="";splitHeaders.push(splitOn);}
splitCnt[splitOn]++;splits[splitOn]+=html;}else{splits[""]+=html;}
}
var html="";if(!this.splitField)html=splits[""];else{var width=splitHeaders.length==0?"100%":100/splitHeaders.length;html+="<table width=100%><tr valign=top>";for(var i=0;i<splitHeaders.length;i++){var header=splitHeaders[i];var cnt=splitCnt[header]
