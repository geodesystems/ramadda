...
this[func].apply(this,[source,data]);},displayColorTable:function(ct,domId,min,max){Utils.displayColorTable(ct,this.getDomId(domId),min,max);},getColorTableName:function(name){var ct;if(name){ct=this.getProperty(name);}else{ct=this.getProperty("colorBar",this.getProperty("colorTable"));}
...
this.vectorLayer.redraw();},needsData:function(){return true;},updateUI:function(){this.haveCalledUpdateUI=true;SUPER.updateUI.call(this);if(!this.getDisplayReady()){return;}
...
var latField1=this.getFieldById(fields,this.getProperty("latField1"));var latField2=this.getFieldById(fields,this.getProperty("latField2"));var lonField1=this.getFieldById(fields,this.getProperty("lonField1"));var lonField2=this.getFieldById(fields,this.getProperty("lonField2"));var showSegments=this.getProperty("showSegments",false);var sizeSegments=this.getProperty("sizeSegments",false);var sizeEndPoints=this.getProperty("sizeEndPoints",true);var showEndPoints=this.getProperty("showEndPoints",false);var endPointSize=parseInt(this.getProperty("endPointSize","4"));var dfltEndPointSize=endPointSize;var segmentWidth=parseInt(this.getProperty("segmentWidth","1"));var dfltSegmentWidth=segmentWidth;var showPoints=this.getProperty("showPoints",true);var lineColor=this.getProperty("lineColor","green");var colorBy={id:colorByAttr,minValue:0,maxValue:0,field:null,index:-1,};var sizeBy={id:this.getDisplayProp(source,"sizeBy",null),minValue:0,maxValue:0,field:null,index:-1,};for(var i=0;i<fields.length;i++){var field=fields[i];if(field.getId()==colorBy.id||("#"+(i+1))==colorBy.id){colorBy.field=field;}
if(field.getId()==sizeBy.id||("#"+(i+1))==sizeBy.id){sizeBy.field=field;}}
...
sizeBy.index=sizeBy.field!=null?sizeBy.field.getIndex():-1;colorBy.index=colorBy.field!=null?colorBy.field.getIndex():-1;var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);for(var i=0;i<points.length;i++){var pointRecord=records[i];var tuple=pointRecord.getData();var v=tuple[colorBy.index];if(isNaN(v)||v===null)
continue;if(excludeZero&&v==0){continue;}
if(i==0||v>colorBy.maxValue)colorBy.maxValue=v;if(i==0||v<colorBy.minValue)colorBy.minValue=v;v=tuple[sizeBy.index];if(i==0||v>sizeBy.maxValue)sizeBy.maxValue=v;if(i==0||v<sizeBy.minValue)sizeBy.minValue=v;}
...
var dontAddPoint=this.doDisplayMap();var didColorBy=false;var seen={};for(var i=0;i<points.length;i++){var pointRecord=records[i];var point=points[i];var values=pointRecord.getData();var props={pointRadius:radius,strokeWidth:strokeWidth,strokeColor:strokeColor,};if(sizeBy.index>=0){var value=values[sizeBy.index];var denom=(sizeBy.maxValue-sizeBy.minValue);var percent=(denom==0?NaN:(value-sizeBy.minValue)/denom);props.pointRadius=6+parseInt(15*percent);if(sizeEndPoints){endPointSize=dfltEndPointSize+parseInt(10*percent);}
...
if(total!=0){percent0=percent=value/total*100;percent=(percent-colorBy.minValue)/(colorBy.maxValue-colorBy.minValue);}}else{percent=(value-colorBy.minValue)/(colorBy.maxValue-colorBy.minValue);}
...
if(showPoints){if(Utils.isDefined(seen[point]))continue;seen[point]=true;point=this.map.addPoint("pt-"+i,point,props,html,dontAddPoint);this.points.push(point);}}
if(didColorBy)
this.displayColorTable(colors,ID_BOTTOM,colorBy.minValue,colorBy.maxValue);this.applyVectorMap();},handleEventRemoveDisplay:function(source,display){if(!this.map){return;}
