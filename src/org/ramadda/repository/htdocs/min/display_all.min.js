...
var html="";html+=HtmlUtils.openTag("div",["style","max-height:100%;overflow-y:auto;"]);html+="<table>";var tmp=[];for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){tmp.push(dataList[rowIdx]);}
var cnt=0;tmp.sort((a,b)=>{var t1=this.getDataValues(a);var t2=this.getDataValues(b);var v1=t1[sortField.getIndex()];var v2=t2[sortField.getIndex()];if(v1<v2)return 1;if(v1>v2)return-1;return 0;});for(var rowIdx=0;rowIdx<tmp.length;rowIdx++){var tuple=this.getDataValues(tmp[rowIdx]);var label="";if(stringField)
label=tuple[stringField.getIndex()];value=tuple[sortField.getIndex()];if(isNaN(value)||value===null)value="NA";html+="<tr><td> #"+(rowIdx+1)+"</td><td>&nbsp;"+label+"</td><td align=right>"+
value+"</td></tr>";}
html+=HtmlUtils.closeTag("table");html+=HtmlUtils.closeTag("div");this.setContents(html);let _this=this;this.jq("sortfields").change(function(){_this.setProperty("sortField",$(this).val());_this.updateUI();});},});}
