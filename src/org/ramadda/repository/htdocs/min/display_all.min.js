...
this.jq(ID_FILTERBAR).find(".display-filter-range").mousedown(function(){var id=$(this).attr("id");id=id.replace(/_min$/,"").replace(/_max$/,"");var min=$("#"+id+"_min");var max=$("#"+id+"_max");range={min:parseFloat(min.attr("data-min")),max:parseFloat(max.attr("data-max"))};var minValue=parseFloat(min.val());var maxValue=parseFloat(max.val());var html=HtmlUtils.div(["id","filterby-range","style","width:200px;"],"");var popup=getTooltip();popup.html(html);popup.show();popup.position({of:min,my:"left top",at:"left bottom+2",collision:"fit fit"});let _this=this;if(isNaN(minValue))minValue=range.min;if(isNaN(maxValue))maxValue=range.max;$("#filterby-range").slider({range:true,min:parseFloat(range.min*10000),max:parseFloat(range.max*10000),values:[parseFloat(minValue*10000),parseFloat(maxValue*10000)],slide:function(event,ui){min.val(ui.values[0]/10000);max.val(ui.values[1]/10000);},stop:function(){var popup=getTooltip();popup.hide();min.trigger("change");}});});let _this=this;this.jq("colorbyselect").change(function(){_this.colorByFieldChanged($(this).val());});this.jq(ID_FILTERBAR).find("input, input:radio,select").change(function(){var id=$(this).attr("id");var value=$(this).val();var fieldId=$(this).attr("fieldId");_this.haveCalledUpdateUI=false;if(_this.settingFilterValue){return;}
