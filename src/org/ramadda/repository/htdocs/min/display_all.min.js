...
this.currentEntries[a]=this.baseEntries[a];var id=this.getProperty("entryId","");var url=ramaddaBaseUrl+"/getnotebook?entryid="+id;url+="&id="+this.getProperty("notebookId","default_notebook");var jqxhr=$.getJSON(url,function(data){_this.loadJson(data);});}
if(this.cells.length>0){this.layoutCells();}else{var props={showEdit:true,}
this.addCell("",props,false).run();}
this.cells[0].focus();},loadJson:async function(data){if(data.error){this.setContents(_this.getMessage("Failed to load notebook: "+data.error));return;}
