...
var ID_FIELDS="fields";var ID_HEADER="header";var ID_HEADER1="header1";var ID_HEADER2="header2";var ID_HEADER3="header3";var ID_FILTERBAR="filterbar";var ID_TITLE=ATTR_TITLE;var ID_TITLE_EDIT="title_edit";var ID_TOP_RIGHT="topright";var ID_TOP_LEFT="topleft";var ID_DETAILS="details";var ID_DISPLAY_CONTENTS="contents";var ID_DISPLAY_TOP="top";var ID_DISPLAY_BOTTOM="bottom";var ID_GROUP_CONTENTS="group_contents";var ID_DETAILS_MAIN="detailsmain";var ID_GROUPBY_FIELDS="groupdbyfields";var ID_TOOLBAR="toolbar";var ID_TOOLBAR_INNER="toolbarinner";var ID_LIST="list";var ID_DIALOG="dialog";var ID_DIALOG_TABS="dialog_tabs";var ID_DIALOG_BUTTON="dialog_button";var ID_FOOTER="footer";var ID_FOOTER_LEFT="footer_left";var ID_FOOTER_RIGHT="footer_right";var ID_MENU_BUTTON="menu_button";var ID_MENU_OUTER="menu_outer";var ID_MENU_INNER="menu_inner";var ID_REPOSITORY="repository";var displayDebug=false;var PROP_DISPLAY_FILTER="displayFilter";var PROP_EXCLUDE_ZERO="excludeZero";var PROP_DIVID="divid";var PROP_FIELDS="fields";var PROP_LAYOUT_HERE="layoutHere";var PROP_HEIGHT="height";var PROP_WIDTH="width";var FILTER_ALL="-all-";function initRamaddaDisplays(){ramaddaCheckForResize();if(window.globalDisplaysList==null){return;}
...
return dflt;},});}
function RamaddaDisplay(argDisplayManager,argId,argType,argProperties){RamaddaUtil.initMembers(this,{orientation:"horizontal",});var SUPER;RamaddaUtil.inherit(this,SUPER=new DisplayThing(argId,argProperties));this.getSuper=function(){return SUPER;}
...
return ct;}
if(this.getProperty("colors")){var colors=this.getProperty("colors");if((typeof colors)=="object")return colors;return this.addAlpha(colors.split(","));}
...
return titleToShow;},handleEventFieldValueSelected:function(source,args){this.setProperty("filterPattern",args.value);this.setProperty("patternFilterField",args.field.getId());this.updateUI();},handleEventPropertyChanged:function(source,prop){if(prop.property=="filterValue"){if(!this.getProperty("acceptFilterEvent",true))return;this.haveCalledUpdateUI=false;var widgetId="filterby_"+prop.fieldId;if(prop.id.endsWith("_min"))widgetId+="_min";if(prop.id.endsWith("_max"))widgetId+="_max";this.settingFilterValue=true;this.jq(widgetId).val(prop.value);this.settingFilterValue=false;this.dataFilterChanged();return;}
...
var newData=[];var values=[];for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];var prefix=this.getProperty(filterField.getId()+".filterPrefix");var suffix=this.getProperty(filterField.getId()+".filterSuffix");if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;if(filterField.isNumeric){values.push([parseFloat($("#"+this.getDomId("filterby_"+filterField.getId()+"_min")).val().trim()),parseFloat($("#"+this.getDomId("filterby_"+filterField.getId()+"_max")).val().trim())]);continue;}
var value=$("#"+this.getDomId("filterby_"+filterField.getId())).val();if(filterField.getType()=="date"){var date1=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date1")).val();var date2=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date2")).val();if(date1!=null&&date1.trim()!="")
date1=Utils.parseDate(date1);else
date1=null;if(date2!=null&&date2.trim()!="")
date2=Utils.parseDate(date2);else
date2=null;value=[date1,date2];}else{if(!Array.isArray(value))value=[value];}
values.push(value);}
for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);var ok=true;for(var i=0;i<this.filterFields.length;i++){if(!ok)break;var filterField=this.filterFields[i];var filterValue=values[i];if(filterValue==null||filterValue.length==0||(filterValue.length==1&&filterValue[0]==FILTER_ALL))continue;var value=row[filterField.getIndex()];if(filterField.getType()=="enumeration"){ok=false;for(var j=0;j<filterValue.length;j++){var v=""+filterValue[j];if((""+value)==v){ok=true;break;}}}else if(filterField.isNumeric){if(!isNaN(filterValue[0])&&value<filterValue[0])ok=false;else if(!isNaN(filterValue[1])&&value>filterValue[1])ok=false;}else if(filterField.getType()=="date"){if(value&&typeof value=="object"){var date1=filterValue[0];var date2=filterValue[1];if(date1&&value.getTime()<date1.getTime())
ok=false;else if(date2&&value.getTime()>date2.getTime())
ok=false;}}else{ok=false;for(var j=0;j<filterValue.length;j++){var v=""+filterValue[j];value=(""+value).toLowerCase();if(value.indexOf(v.toLowerCase())>=0){ok=true;break;}}}}
...
var props={showMenu:true,sourceEntry:entry,entryId:entry.getId(),showTitle:true,showDetails:true,title:entry.getName(),layoutHere:false,};if(displayProps)$.extend(props,displayProps);if(displayType!=DISPLAY_ENTRYLIST){if(jsonUrl==null){jsonUrl=this.getPointUrl(entry);}
...
"&nbsp;&nbsp;&nbsp;"+
HtmlUtils.onClick(get+".askSetTitle();","Set Title");menu+=HtmlUtils.formTable()+HtmlUtils.formEntry("Show:",tmp)+"</table>";return menu;},isLayoutHorizontal:function(){return this.orientation=="horizontal";},loadInitialData:function(){if(!this.needsData()||this.properties.data==null){return;}
...
groupByList.push("");this.minDateObj=Utils.parseDate(this.minDate,false);this.maxDateObj=Utils.parseDate(this.maxDate,true,this.minDateObj);if(this.minDateObj==null&&this.maxDateObj!=null){this.minDateObj=Utils.parseDate(this.minDate,false,this.maxDateObj);}
...
HtmlUtils.span(["id",this.getDomId(ID_ANIMATION_LABEL),"class","display-animation-label"]);buttons=HtmlUtils.div(["class","display-animation-buttons"],buttons);if(display.getProperty("animationShowSlider",true)){buttons+=HtmlUtils.div(["class","display-animation-slider","id",this.getDomId(ID_SLIDER)]);}
...
this.stopAnimation();this.applyAnimation();});this.btnPrev.button().click(()=>{this.begin=new Date(this.begin.getTime()-this.window);if(this.begin.getTime()<this.dateMin.getTime())
this.begin=this.dateMin;if(this.mode=="sliding"){this.end=new Date(this.begin.getTime()+this.window);}
...
this.applyAnimation();},startAnimation:function(){if(!this.display.points){return;}
if(!this.dateMax)return;if(!this.inAnimation){this.inAnimation=true;this.label.html("");var date=this.dateMin;this.begin=date;var unit=this.windowUnit;if(unit!=""){var tmp=0;if(unit=="decade"){this.begin=new Date(date.getUTCFullYear(),0);}else if(unit=="year"){this.begin=new Date(date.getUTCFullYear(),0);}else if(unit=="month"){this.begin=new Date(date.getUTCFullYear(),date.getMonth());}else if(unit=="day"){this.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay());}else if(unit=="hour"){this.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours());}else if(unit=="minute"){this.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getMinutes());}else{this.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getSeconds());}}
this.end=this.begin;this.display.animationStart();}
this.doNext();},stopAnimation:function(){this.btnRun.html(HtmlUtils.getIconImage("fa-play"));this.running=false;},applyAnimation:function(skipSlider){if(!skipSlider){this.jq(ID_SLIDER).slider('values',0,this.begin.getTime());this.jq(ID_SLIDER).slider('values',1,this.end.getTime());}
this.display.animationApply(this);this.updateLabels();var windowEnd=this.end.getTime();if(windowEnd<=this.dateMax.getTime()){if(this.running){setTimeout(()=>{if(!this.running)return;this.doNext()},this.speed);}}else{this.running=false;this.inAnimation=false;this.btnRun.html(HtmlUtils.getIconImage("fa-play"));}},updateLabels:function(){if(this.label)
...
return;var data=this.dataCollection.getList()[0];if(data!=args.data){return;}
...
function AreachartDisplay(displayManager,id,properties){RamaddaUtil.inherit(this,new RamaddaSeriesChart(displayManager,id,DISPLAY_AREACHART,properties));addRamaddaDisplay(this);$.extend(this,{doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){if(this.isStacked)
chartOptions.isStacked=true;return new google.visualization.AreaChart(document.getElementById(this.getChartId()));}});}
function RamaddaBaseBarchart(displayManager,id,type,properties){RamaddaUtil.inherit(this,new RamaddaSeriesChart(displayManager,id,type,properties));$.extend(this,{doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){var chartType=this.getChartType();if(chartType==DISPLAY_BARSTACK){chartOptions.isStacked=true;}
...
function HistogramDisplay(displayManager,id,properties){RamaddaUtil.inherit(this,new RamaddaGoogleChart(displayManager,id,DISPLAY_HISTOGRAM,properties));addRamaddaDisplay(this);RamaddaUtil.inherit(this,{okToHandleEventRecordSelection:function(){return false;},makeDataTable:function(dataList,props,selectedFields){return google.visualization.arrayToDataTable(this.makeDataArray(dataList));},doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){if(this.legendPosition){if(!chartOptions.legend)
...
function PiechartDisplay(displayManager,id,properties){RamaddaUtil.inherit(this,new RamaddaTextChart(displayManager,id,DISPLAY_PIECHART,properties));addRamaddaDisplay(this);$.extend(this,{canDoGroupBy:function(){return true;},getGroupBy:function(){if(!this.groupBy&&this.groupBy!=""){var stringField=this.getFieldOfType(this.allFields,"string");if(stringField){this.groupBy=stringField.getId();}}
...
function TableDisplay(displayManager,id,properties){RamaddaUtil.inherit(this,new RamaddaTextChart(displayManager,id,DISPLAY_TABLE,properties));addRamaddaDisplay(this);$.extend(this,{canDoGroupBy:function(){return true;},defaultSelectedToAll:function(){return true;},doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){chartOptions.height=null;if(this.chartHeight){chartOptions.height=this.chartHeight;}
...
function BartableDisplay(displayManager,id,properties){RamaddaUtil.inherit(this,new RamaddaSeriesChart(displayManager,id,DISPLAY_BARTABLE,properties));addRamaddaDisplay(this);$.extend(this,{xgetIncludeIndexIfDate:function(){return true;},doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){var height="";if(Utils.isDefined(this.chartHeight)){height=this.chartHeight;}else{if(dataList.length>1){var numBars=dataList.length;if(this.isStacked){height=numBars*22;}else{height=numBars*22+numBars*14*(this.getDataValues(dataList[0]).length-2);}}}
...
function RamaddaCrosstabDisplay(displayManager,id,properties){let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_CROSSTAB,properties);var ID_TABLE="crosstab";RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},fieldSelectionChanged:function(){this.updateUI();},updateUI:function(pointData){SUPER.updateUI.call(this,pointData);if(!this.hasData()){this.setContents(this.getLoadingMessage());return;}
...
function RamaddaRankingDisplay(displayManager,id,properties){var ID_TABLE="table";$.extend(this,{height:"500px;",sortAscending:false,});if(properties.sortAscending)this.sortAscending="true"==properties.sortAscending;let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_RANKING,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},fieldSelectionChanged:function(){this.updateUI();},updateUI:function(pointData){SUPER.updateUI.call(this,pointData);if(!this.hasData()){this.setContents(this.getLoadingMessage());return;}
...
var stringField=this.getFieldById(allFields,this.getProperty("nameField","",true));if(!stringField)
stringField=this.getFieldOfType(allFields,"string");var menu="<select class='ramadda-pulldown' id='"+this.getDomId("sortfields")+"'>";for(var i=0;i<numericFields.length;i++){var field=numericFields[i];var extra="";if(field.getId()==sortField.getId())extra=" selected ";menu+="<option value='"+field.getId()+"'  "+extra+" >"+field.getLabel()+"</option>\n";}
...
var cnt=0;tmp.sort((a,b)=>{var t1=this.getDataValues(a);var t2=this.getDataValues(b);var v1=t1[sortField.getIndex()];var v2=t2[sortField.getIndex()];if(v1<v2)return this.sortAscending?-1:1;if(v1>v2)return this.sortAscending?1:-1;return 0;});for(var rowIdx=0;rowIdx<tmp.length;rowIdx++){var obj=tmp[rowIdx];var tuple=this.getDataValues(obj);var label="";if(stringField)
label=tuple[stringField.getIndex()];value=tuple[sortField.getIndex()];if(isNaN(value)||value===null){if(!includeNaN)continue;value="NA";}
...
return null;},});}
var topGroup=new groupNode("");var colorMap={};var colorCnt=0;var imgCnt=0;for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);var contents="";var tooltip="";this.tooltipFields.map(field=>{if(tooltip!="")tooltip+="&#10;";tooltip+=field.getValue(row);});var label="";var caption="";if(this.captionFields.length>0){if(this.captionTemplate)caption=this.captionTemplate;this.captionFields.map(field=>{if(this.captionTemplate)
caption=caption.replace("\${"+field.getId()+"}",field.getValue(row));else
caption+=field.getValue(row)+"<br>";});if(this.urlField){var url=this.urlField.getValue(row);if(url&&url!=""){caption="<a style='color:inherit;'  href='"+url+"' target=_other>"+caption+"</a>";}}}
if(this.labelField)label=row[this.labelField.getIndex()];var html="";var img=null;if(this.imageField){img=row[this.imageField.getIndex()];if(this.onlyShowImages&&!Utils.stringDefined(img))continue;}
...
function RamaddaTemplateDisplay(displayManager,id,properties){if(!Utils.isDefined(properties.showTitle))properties.showTitle=false;if(!Utils.isDefined(properties.showMenu))properties.showMenu=false;if(!Utils.isDefined(properties.displayStyle))properties.displayStyle="background:rgba(0,0,0,0);";let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_TEMPLATE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{updateUI:function(){var pointData=this.getData();if(pointData==null)return;var records=this.filterData();if(!records)return;records=this.sortRecords(records);var fields=pointData.getRecordFields();var template=this.getProperty("template","");var select=this.getProperty("select","all");var selected=[];var summary={};records.map(r=>{r=this.getDataValues(r);for(var i=0;i<fields.length;i++){var f=fields[i];var v=f.getValue(r);if(!summary[f.getId()]){summary[f.getId()]={total:0,min:v,max:v,count:0,uniques:{},uniqueCount:0}}
...
function RamaddaFrequencyDisplay(displayManager,id,properties){let SUPER=new RamaddaBaseTextDisplay(displayManager,id,DISPLAY_FREQUENCY,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{updateUI:function(){let records=this.filterData();if(!records)return;var allFields=this.getData().getRecordFields();var fields=this.getSelectedFields(allFields);if(fields.length==0)
...
var displayId=this.getUniqueId("display");if(props.data==null&&this.dataList.length>0){props.data=this.dataList[0];}
...
if(this.getProperty("doAnimation",false)){this.animation.makeControls();}
...
this.map.initMap(false);this.map.addRegionSelectorControl(function(bounds){theDisplay.getDisplayManager().handleEventMapBoundsChanged(this,bounds,true);});this.map.addClickHandler(this.getDomId(ID_LONFIELD),this.getDomId(ID_LATFIELD),null,this);this.map.map.events.register("zoomend","",function(){theDisplay.mapBoundsChanged();});this.map.map.events.register("moveend","",function(){theDisplay.mapBoundsChanged();});var overrideBounds=false;if(this.getProperty("bounds")){overrideBounds=true;var toks=this.getProperty("bounds","").split(",");if(toks.length==4){if(this.getProperty("showBounds",true)){var attrs={};if(this.getProperty("boundsColor")){attrs.strokeColor=this.getProperty("boundsColor","");}
...
this.vectorLayer.redraw();if(maxExtent){this.map.map.zoomToExtent(maxExtent,true);}},findContainingFeature:function(features,center,info){var matchedFeature=null;for(var j=0;j<features.length;j++){var feature=features[j];var geometry=feature.geometry;if(!geometry){continue;}
...
return matchedFeature;},needsData:function(){return true;},animation:new DisplayAnimation(this),animationStart:function(animation){if(this.points){for(var i=0;i<this.points.length;i++){var point=this.points[i];point.style.display='none';}}
if(this.map.circles)
this.map.circles.redraw();},animationApply:function(animation){var windowStart=animation.begin.getTime();var windowEnd=animation.end.getTime();var atLoc={};for(var i=0;i<this.lines.length;i++){var line=this.lines[i];if(line.date<windowStart||line.date>windowEnd){line.style.display='none';continue;}
...
var latField1=this.getFieldById(fields,this.getProperty("latField1"));var latField2=this.getFieldById(fields,this.getProperty("latField2"));var lonField1=this.getFieldById(fields,this.getProperty("lonField1"));var lonField2=this.getFieldById(fields,this.getProperty("lonField2"));var sizeSegments=this.getProperty("sizeSegments",false);var sizeEndPoints=this.getProperty("sizeEndPoints",true);var showEndPoints=this.getProperty("showEndPoints",false);var endPointSize=parseInt(this.getProperty("endPointSize","4"));var dfltEndPointSize=endPointSize;var segmentWidth=parseInt(this.getProperty("segmentWidth","1"));var dfltSegmentWidth=segmentWidth;var showPoints=this.getProperty("showPoints",true);var lineColor=this.getProperty("lineColor","green");var pointIcon=this.getProperty("pointIcon");if(pointIcon)this.pointsAreMarkers=true;var dfltShape=this.getProperty("defaultShape",null);var dfltShapes=["circle","triangle","star","square","cross","x","lightning","rectangle","church"];var dfltShapeIdx=0;var shapeBy={id:this.getDisplayProp(source,"shapeBy",null),field:null,map:{}}
...
sizeBy.range=sizeBy.maxValue-sizeBy.minValue;if(dateMax){this.animation.init(dateMin,dateMax,dates);}
...
var dontAddPoint=this.doDisplayMap();var didColorBy=false;var seen={};for(var i=0;i<points.length;i++){var pointRecord=records[i];var point=points[i];if(justOneMarker){if(this.justOneMarker)
...
if(showPoints){if(!Utils.isDefined(seen[point]))seen[point]=0;if(seen[point]>500)continue;seen[point]++;var mapPoint=null;if(pointIcon){mapPoint=this.map.addMarker("pt-"+i,point,pointIcon,"pt-"+i,html);}else{if(!props.graphicName)
