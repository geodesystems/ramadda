...
fields=allFields;var strings=this.getFieldsOfType(fields,"string");if(strings.length==0){this.displayError("No string fields specified");return null;}
var corpus="";if(addLineNumbers){corpus="<table width=100%>";}
var lineCnt=0;var displayedLineCnt=0;var regexp;if(pattern){regexp=new RegExp("("+pattern+")","i");}
...
line+=value;}
line=line.trim();if(!includeEmptyLines&&line.length==0)continue;lineCnt++;if(regexp){if(!line.toLowerCase().match(regexp))continue;line=line.replace(re,"<span style=background:yellow;>$1</span>");}
...
line=HtmlUtils.div(lineAttrs,line);if(addLineNumbers){corpus+=HtmlUtils.tr(rowAttrs,HtmlUtils.td(["width","10px"],"<a name=line_"+lineCnt+"></a>"+
