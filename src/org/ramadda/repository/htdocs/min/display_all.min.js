...
this.displaySearchResults(records);},displaySearchResults:function(records){var width=this.getProperty("imageWidth","50");var margin=this.getProperty("imageMargin","0");var splits={};var splitHeaders=[];if(!this.splitField)splits[""]="";for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);var img=row[this.imageField.getIndex()];var tooltip="";var label="";if(this.labelField)label="<br>"+row[this.labelField.getIndex()];for(var i=0;i<this.tooltipFields.length;i++){if(tooltip!="")tooltip+="&#10;";tooltip+=row[this.tooltipFields[i].getIndex()];}
var img=HtmlUtils.href(img,HtmlUtils.image(img,["width",width]),["class","display-images-popup"]);var html=HtmlUtils.div(["class","display-images-item","title",tooltip,"style","margin:"+margin+"px;"],img+label);if(this.splitField){var splitOn=row[this.splitField.getIndex()];if(!splits[splitOn]){splits[splitOn]="";splitHeaders.push(splitOn);}
splits[splitOn]+=html;}else{splits[""]+=html;}
}
var html="";if(!this.splitField)html=splits[""];else{var width=splitHeaders.length==0?"100%":100/splitHeaders.length;html+="<table width=100%><tr valign=top>";for(var i=0;i<splitHeaders.length;i++){var header=splitHeaders[i];html+="<td width="+width+"%><center><b>"+header+"</b></center>"+HtmlUtils.div(["class","display-images-items"],splits[header])+"</td>";}
html+="</tr></table>";}
