...
var displayDebug=false;var ID_BOTTOM="bottom";var ID_COLORTABLE="colortable";var ID_LEGEND="legend";var ID_FIELDS="fields";var ID_HEADER="header";var ID_HEADER1="header1";var ID_HEADER2="header2";var ID_HEADER3="header3";var ID_FILTERBAR="filterbar";var ID_TITLE=ATTR_TITLE;var ID_TITLE_EDIT="title_edit";var ID_TOP_RIGHT="topright";var ID_TOP_LEFT="topleft";var ID_DETAILS="details";var ID_DISPLAY_CONTENTS="contents";var ID_DISPLAY_TOP="top";var ID_DISPLAY_BOTTOM="bottom";var ID_GROUP_CONTENTS="group_contents";var ID_DETAILS_MAIN="detailsmain";var ID_GROUPBY_FIELDS="groupdbyfields";var ID_TOOLBAR="toolbar";var ID_TOOLBAR_INNER="toolbarinner";var ID_LIST="list";var ID_DIALOG="dialog";var ID_DIALOG_TABS="dialog_tabs";var ID_FOOTER="footer";var ID_FOOTER_LEFT="footer_left";var ID_FOOTER_RIGHT="footer_right";var ID_MENU_BUTTON="menu_button";var ID_MENU_OUTER="menu_outer";var ID_MENU_INNER="menu_inner";var ID_DISPLAY_PROGRESS="display_progress";var ID_REPOSITORY="repository";var CATEGORY_MISC="Misc";var PROP_DISPLAY_FILTER="displayFilter";var PROP_EXCLUDE_ZERO="excludeZero";var PROP_DIVID="divid";var PROP_FIELDS="fields";var PROP_LAYOUT_HERE="layoutHere";var PROP_HEIGHT="height";var PROP_WIDTH="width";var FILTER_ALL="-all-";var HIGHLIGHT_COLOR="yellow";function initRamaddaDisplays(){ramaddaCheckForResize();if(window.globalDisplaysList==null){return;}
...
function ramaddaDisplayCheckLayout(){for(var i=0;i<window.globalDisplaysList.length;i++){if(window.globalDisplaysList[i].checkLayout){window.globalDisplaysList[i].checkLayout();}}}
function ramaddaCheckForResize(){var redisplayPending=false;var redisplayPendingCnt=0;$(window).resize(function(){if(window.globalDisplaysList==null){return;}
if(redisplayPending){redisplayPendingCnt++;return;}
var timeoutFunc=function(myCnt){if(myCnt==redisplayPendingCnt){redisplayPending=false;redisplayPendingCnt=0;for(var i=0;i<window.globalDisplaysList.length;i++){var display=window.globalDisplaysList[i];if(display.displayData)
display.displayData();}}else{setTimeout(timeoutFunc.bind(null,redisplayPendingCnt),1000);}}
...
const ID_ENTRIES_MENU="entries_menu";const ID_ENTRIES_PREV="entries_prev";const ID_ENTRIES_NEXT="entries_next";function DisplayThing(argId,argProperties){if(argProperties==null){argProperties={};}
...
var v=this.getProperty("showTitle",dflt);return v;},getTimeZone:function(){return this.getProperty("timeZone");},formatDate:function(date,args){if(!date)return"";try{return this.formatDateInner(date,args);}catch(e){console.log("Error formatting date:"+e);if(!date.getTime&&date.v)date=date.v;return""+date;}},formatDateInner:function(date,args){var fmt=this.getProperty("dateFormat");if(fmt=="yyyy"){return Utils.formatDateYYYY(date);}else if(fmt=="yyyyMMdd"){return Utils.formatDateYYYYMMDD(date);}else if(fmt=="yyyyMM"){return Utils.formatDateYYYYMM(date);}else if(fmt=="yearmonth"){return Utils.formatDateYearMonth(date);}else if(fmt=="monthdayyear"){return Utils.formatDateMonthDayYear(date);}else if(fmt=="monthday"){return Utils.formatDateMonthDay(date);}else if(fmt=="mdy"){return Utils.formatDateMDY(date);}else if(fmt=="hhmm"){return Utils.formatDateHHMM(date);}
if(!date.getTime&&date.v)date=date.v;if(!date.toLocaleDateString){return""+date;}
if(!args)args={};var suffix;if(!Utils.isDefined(args.suffix))
suffix=args.suffix;else
suffix=this.getProperty("dateSuffix");var timeZone=this.getTimeZone();if(!suffix&&timeZone)suffix=timeZone;return Utils.formatDate(date,args.options,{timeZone:timeZone,suffix:suffix});},getUniqueId:function(base){return HtmlUtils.getUniqueId(base);},handleError:function(code,message){GuiUtils.handleError("An error has occurred:"+message,true,true);},toString:function(){return"DisplayThing:"+this.getId();},getDomId:function(suffix){return this.getId()+"_"+suffix;},jq:function(componentId){return $("#"+this.getDomId(componentId));},writeHtml:function(idSuffix,html){$("#"+this.getDomId(idSuffix)).html(html);},getTemplateProps:function(fields){return{iconField:this.getFieldById(fields,this.getProperty("iconField")),iconSize:parseFloat(this.getProperty("iconSize",16)),iconMap:this.getIconMap(),colorBy:this.getProperty("colorBy"),colorByMap:this.getColorByMap()}},xcnt:0,applyRecordTemplate:function(row,fields,s,props){if(!props){props=this.getTemplateProps(fields);}
if(props.iconMap&&props.iconField){var value=row[props.iconField.getIndex()];var icon=props.iconMap[value];if(icon){s=s.replace("${"+props.iconField.getId()+"_icon}",HtmlUtils.image(icon,["width",props.iconSize]));}}
for(var col=0;col<fields.length;col++){var f=fields[col];var value=row[f.getIndex()];if(props.iconMap){var icon=props.iconMap[f.getId()+"."+value];if(icon){s=s.replace("${"+f.getId()+"_icon}",HtmlUtils.image(icon,["size",props.iconSize]));}}
if(f.getType()=="image"){if(value&&value.trim().length>1){var attrs=[];if(this.getProperty("imageWidth","")!=""){attrs.push("width");attrs.push(this.getProperty("imageWidth",""));}
var img=HtmlUtils.image(value,attrs);s=s.replace("${"+f.getId()+"_image}",img);s=s.replace("${"+f.getId()+"_url}",value);}else{s=s.replace("${"+f.getId()+"_url}",ramaddaBaseUrl+"/icons/blank.gif");s=s.replace("${"+f.getId()+"_image}","");}}else if(f.getType()=="url"){if(value&&value.trim().length>1){s=s.replace("${"+f.getId()+"_href}",HtmlUtils.href(value,value));s=s.replace("${"+f.getId()+"}",value);}else{s=s.replace("${"+f.getId()+"_href}","");s=s.replace("${"+f.getId()+"}","");}
continue;}else if(f.isDate){if(value){s=s.replace("${"+f.getId()+"}",value);s=s.replace("${"+f.getId()+"_yyyy}",Utils.formatDateYYYY(value));s=s.replace("${"+f.getId()+"_yyyymmdd}",Utils.formatDateYYYYMMDD(value));s=s.replace("${"+f.getId()+"_monthdayyear}",Utils.formatDateMonthDayYear(value));s=s.replace("${"+f.getId()+"_monthday}",Utils.formatDateMonthDay(value));s=s.replace("${"+f.getId()+"_mdy}",Utils.formatDateMDY(value));}
continue;}
if(typeof value=="number"){value=Utils.formatNumber(value);}
...
if(color){s=s.replace("${"+f.getId()+"_color}",color);}
s=s.replace(new RegExp("\\${"+f.getId()+"}","g"),value);if(f.isNumeric()){s=s.replace(new RegExp("\\${"+f.getId()+"_format}","g"),Utils.formatNumberComma(value));}}
return s;},getRecordHtml:function(record,fields,template){if(!fields){var pointData=this.getData();if(pointData==null){return null;}
fields=pointData.getRecordFields();}
var showDate=this.getProperty("showDate",true);var showImage=this.getProperty("showImage",true);var showGeo=false;if(Utils.isDefined(this.showGeo)){showGeo=(""+this.showGeo)=="true";}
if(!template)
template=this.getProperty("recordTemplate");if(template){if(template!="${default}"){var row=this.getDataValues(record);return this.applyRecordTemplate(row,fields,template);}}
var values="<table>";for(var doDerived=0;doDerived<2;doDerived++){for(var i=0;i<record.getData().length;i++){var field=fields[i];if(doDerived==0&&!field.derived)continue;else if(doDerived==1&&field.derived)continue;var label=field.getLabel();label=this.formatRecordLabel(label);if(!showDate){if(field.isFieldDate()){continue;}}
if(!showGeo){if(field.isFieldGeo()){continue;}}
var value=record.getValue(i);if(typeof value=="number"){value=this.formatNumber(value);}
if(field.isFieldDate()){value=this.formatDate(value);}
if(field.getType()=="image"&&value!=""){if(!showImage)continue;value=HtmlUtils.image(value,["width","200"]);}
if(field.getType()=="url"){value=HtmlUtils.href(value,value);}
value=value+field.getUnitSuffix();if(value.length>200){value=HtmlUtils.div(["style","max-height:200px; overflow-y:auto;"],value);}
...
values+="</table>";if(this.getProperty("recordHtmlStyle")){values=HtmlUtils.div(["style",this.getProperty("recordHtmlStyle")],values);}
...
return this.displayParent;},removeProperty:function(key){this.properties[key]=null;},setProperty:function(key,value){this[key]=value;this.properties[key]=value;},getSelfProperty:function(key,dflt){if(this[key]!=null){return this[key];}
return this.getProperty(key,dflt);},initTooltip:function(){},formatNumber:function(number){let f=this.formatNumberInner(number);let fmt=this.getProperty("numberTemplate");if(fmt)f=fmt.replace("${number}",f);return f;},formatNumberInner:function(number){if(!this.getProperty("format",true))return number;if(this.getProperty("formatNumberComma",false))
...
prop="map-"+prop;if(Utils.isDefined(source[prop])){return source[prop];}
if(source.getProperty)
return source.getProperty(prop,dflt);return null;},getProperty:function(key,dflt,skipThis){var value=this.getPropertyInner(key,null,skipThis);if(!Utils.isDefined(value))return dflt;return value;},getPropertyInner:function(key,dflt,skipThis){let debug=false;if(debug)console.log("getProperty");if(!skipThis&&Utils.isDefined(this[key])){if(debug)console.log("\tgetProperty-1");return this[key];}
var value=this.properties[key];if(value!=null){if(debug)console.log("\tgetProperty-2");return value;}
var fromParent=null;if(this.displayParent!=null){fromParent=this.displayParent.getPropertyInner("inherit."+key,skipThis);}
...
if(this.displayParent!=null){if(debug)console.log("\tgetProperty-4");return this.displayParent.getPropertyInner(key,skipThis);}
if(this.getDisplayManager){if(debug)console.log("\tgetProperty-5");return this.getDisplayManager().getPropertyInner(key);}
value=getGlobalDisplayProperty(key);if(value){if(debug)console.log("\tgetProperty-6:"+value);return value;}
if(debug)console.log("\tgetProperty-6 dflt:"+dflt);return dflt;},});}
function RamaddaDisplay(argDisplayManager,argId,argType,argProperties){RamaddaUtil.initMembers(this,{});let SUPER=new DisplayThing(argId,argProperties);RamaddaUtil.inherit(this,SUPER);this.getSuper=function(){return SUPER;}
if(this.derived){if(this.derived.indexOf("[")==0){this.derived=JSON.parse(this.derived);}else{this.derived=[JSON.parse(this.derived)];}
for(var i=0;i<this.derived.length;i++){var d=this.derived[i];if(!Utils.isDefined(d.isRow)&&!Utils.isDefined(d.isColumn)){d.isRow=true;d.isColumn=false;}
if(!d.isRow&&!d.isColumn){d.isRow=true;}}}else{}
RamaddaUtil.defineMembers(this,{displayReady:Utils.getPageLoaded(),type:argType,displayManager:argDisplayManager,filters:[],dataCollection:new DataCollection(),selectedCbx:[],entries:[],wikiAttrs:["title","showTitle","showDetails","minDate","maxDate"],getDisplayManager:function(){return this.displayManager;},getLayoutManager:function(){return this.getDisplayManager().getLayoutManager();},getAnimation:function(){if(!this.animation){this.animation=new DisplayAnimation(this);}
return this.animation;},propagateEvent:function(func,data){var dm=this.getDisplayManager();dm[func](this,data);},displayError:function(msg){this.displayHtml(HtmlUtils.getErrorDialog(msg));},clearHtml:function(){this.displayHtml("");},displayHtml:function(html){this.jq(ID_DISPLAY_CONTENTS).html(html);},notifyEvent:function(func,source,data){if(this[func]==null){return;}
this[func].apply(this,[source,data]);},displayColorTable:function(ct,domId,min,max,args){Utils.displayColorTable(ct,this.getDomId(domId),min,max,args);},getColorTableName:function(name){var ct;if(name){ct=this.getProperty(name);}else{ct=this.getProperty("colorBar",this.getProperty("colorTable"));}
if(ct=="none")return null;return ct;},getColorList:function(){if(this.colorList&&this.colorList.length>0){return this.colorList;}
...
return this.colorList;},addAlpha:function(colors){var alpha=this.getProperty("colorTableAlpha");if(!alpha)return colors;colors=Utils.cloneList(colors);var ac=[];colors.map((c)=>{ac.push(Utils.addAlphaToColor(c,alpha));});return ac;},getColorTable:function(justColors,name,dflt){var colorTable=this.getColorTableName(name);if(!colorTable){colorTable=dflt;}
var list;if(colorTable){var ct=Utils.ColorTables[colorTable];if(ct&&justColors)return this.addAlpha(ct.colors);if(!ct&&name){return this.addAlpha(colorTable.split(","));}
return ct;}
if(this.getProperty("colors")&&this.getProperty("colors")!="default"){var colors=this.getProperty("colors");if((typeof colors)=="object")return this.addAlpha(colors);return this.addAlpha(colors.split(","));}
return null;},getColorByColors:function(records,dfltColorTable){var colorBy=this.getProperty("colorBy");if(!colorBy){return null;}
...
return{colors:colorValues,min:min,max:max};},getIconMap:function(){var iconMap;var iconMapProp=this.getProperty("iconMap");if(iconMapProp){var toks=iconMapProp.split(",");iconMap={};for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){iconMap[toks2[0]]=toks2[1];}}}
return iconMap;},getColorByInfo:function(records,prop,colorByMapProp,defaultColorTable){var pointData=this.getData();if(pointData==null)return null;var fields=pointData.getRecordFields();var colorByAttr=this.getProperty(prop||"colorBy",null);var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);var _this=this;var colorBy={id:colorByAttr,fields:fields,minValue:0,maxValue:0,field:null,index:-1,isString:false,stringMap:null,colorByMap:{},colorByValues:[],colorByMinPerc:this.getDisplayProp(this,"colorByMinPercentile",-1),colorByMaxPerc:this.getDisplayProp(this,"colorByMaxPercentile",-1),colorByOffset:0,pctFields:null,compareFields:this.getFieldsByIds(null,this.getProperty("colorByCompareFields","",true)),};$.extend(colorBy,{displayColorTable:function(width){if(this.compareFields.length>0){var legend="";this.compareFields.map((f,idx)=>{legend+=HtmlUtils.div(["style","display:inline-block;width: 15px;height: 15px; background:"+this.colors[idx]+";"])+" "+
f.getLabel()+" ";});_this.jq(ID_COLORTABLE).html(HtmlUtils.div(["style","text-align:center; margin-top:5px;"],legend));}
if(this.index<0||!_this.getProperty("showColorTable",true))return;if(this.stringMap){var colors=[];this.colorByValues=[];for(var i in this.stringMap){this.colorByValues.push(i);colors.push(this.stringMap[i]);}
_this.displayColorTable(colors,ID_COLORTABLE,this.origMinValue,this.origMaxValue,{width:width,stringValues:this.colorByValues});}else{var colors=this.colors;if(_this.getProperty("clipColorTable",true)&&this.colorByValues.length){var tmp=[];for(var i=0;i<this.colorByValues.length&&i<colors.length;i++)
tmp.push(this.colors[i]);colors=tmp;}
_this.displayColorTable(colors,ID_COLORTABLE,this.origMinValue,this.origMaxValue,{width:width,stringValues:this.colorByValues});}},getColor:function(value,pointRecord){var percent=0;if(this.showPercent){var total=0;var data=pointRecord.getData();for(var j=0;j<data.length;j++){var ok=this.fields[j].isNumeric()&&!this.fields[j].isFieldGeo();if(ok&&this.pctFields!=null){ok=this.pctFields.indexOf(this.fields[j].getId())>=0||this.pctFields.indexOf("#"+(j+1))>=0;}
if(ok){total+=data[j];}}
if(total!=0){percent=percent=value/total*100;percent=(percent-this.minValue)/(this.maxValue-this.minValue);}}else{var v=value;if(this.stringMap){var color=this.stringMap[value];if(!Utils.isDefined(color)){return this.stringMap["default"];}
return color;}
if(this.isString){color=this.colorByMap[v];if(color)return color;}
v+=this.colorByOffset;if(this.colorByLog){v=this.colorByFunc(v);}
...
return null;},convertColor:function(color,colorByValue){color=this.convertColorIntensity(color,colorByValue);color=this.convertColorAlpha(color,colorByValue);return color;},convertColorIntensity:function(color,colorByValue){if(!this.convertIntensity)return color;percent=(colorByValue-this.intensitySourceMin)/(this.intensitySourceMax-this.intensitySourceMin);intensity=this.intensityTargetMin+percent*(this.intensityTargetMax-this.intensityTargetMin);var result=Utils.pSBC(intensity,color);return result||color;},convertColorAlpha:function(color,colorByValue){if(!this.convertAlpha)return color;percent=(colorByValue-this.alphaSourceMin)/(this.alphaSourceMax-this.alphaSourceMin);alpha=this.alphaTargetMin+percent*(this.alphaTargetMax-this.alphaTargetMin);var result=Utils.addAlphaToColor(color,alpha);return result||color;}});colorBy.convertAlpha=this.getProperty("convertColorAlpha",false);if(colorBy.convertAlpha){if(!Utils.isDefined(this.getProperty("alphaSourceMin"))){var min=0,max=0;records.map((record,idx)=>{var tuple=record.getData();if(colorBy.compareFields.length>0){colorBy.compareFields.map((f,idx2)=>{var v=tuple[f.getIndex()];if(isNaN(v))return;min=idx==0&&idx2==0?v:Math.min(min,v);max=idx==0&&idx2==0?v:Math.max(max,v);});}else if(colorBy.index=0){var v=tuple[colorBy.index];if(isNaN(v))return;min=idx==0?v:Math.min(min,v);max=idx==0?v:Math.max(max,v);}});colorBy.alphaSourceMin=min;colorBy.alphaSourceMax=max;}else{colorBy.alphaSourceMin=+this.getProperty("alphaSourceMin",40);colorBy.alphaSourceMax=+this.getProperty("alphaSourceMax",80);}
colorBy.alphaTargetMin=+this.getProperty("alphaTargetMin",0);colorBy.alphaTargetMax=+this.getProperty("alphaTargetMax",1);}
colorBy.convertIntensity=this.getProperty("convertColorIntensity",false);if(colorBy.convertIntensity){if(!Utils.isDefined(this.getProperty("intensitySourceMin"))){var min=0,max=0;records.map((record,idx)=>{var tuple=record.getData();if(colorBy.compareFields.length>0){colorBy.compareFields.map((f,idx2)=>{var v=tuple[f.getIndex()];if(isNaN(v))return;min=idx==0&&idx2==0?v:Math.min(min,v);max=idx==0&&idx2==0?v:Math.max(max,v);});}else if(colorBy.index=0){var v=tuple[colorBy.index];if(isNaN(v))return;min=idx==0?v:Math.min(min,v);max=idx==0?v:Math.max(max,v);}});colorBy.intensitySourceMin=min;colorBy.intensitySourceMax=max;}else{colorBy.intensitySourceMin=+this.getProperty("intensitySourceMin",80);colorBy.intensitySourceMax=+this.getProperty("intensitySourceMax",40);}
colorBy.intensityTargetMin=+this.getProperty("intensityTargetMin",1);colorBy.intensityTargetMax=+this.getProperty("intensityTargetMax",0);}
if(this.percentFields!=null){colorBy.pctFields=this.percentFields.split(",");}
var colors=defaultColorTable||this.getColorTable(true,colorByAttr+".colorTable");if(!colors){var c=this.getProperty(colorByAttr+".colors");if(c)
colors=c.split(",");}
if(!colors)
colors=this.getColorTable(true);colorBy.colors=colors;if(!colorBy.colors&&this.colors&&this.colors.length>0){colorBy.colors=source.colors;if(colorBy.colors.length==1&&Utils.ColorTables[colorBy.colors[0]]){colorBy.colors=Utils.ColorTables[colorBy.colors[0]].colors;}}
if(colorBy.colors==null){colorBy.colors=Utils.ColorTables.grayscale.colors;}
for(var i=0;i<fields.length;i++){var field=fields[i];if(field.getId()==colorBy.id||("#"+(i+1))==colorBy.id){colorBy.field=field;if(field.isString())colorBy.isString=true;}}
colorBy.index=colorBy.field!=null?colorBy.field.getIndex():-1;colorBy.stringMap=this.getColorByMap(colorByMapProp);if(colorBy.index>=0){var cnt=0;records.map(record=>{var tuple=record.getData();var v=tuple[colorBy.index];if(colorBy.isString){if(!Utils.isDefined(colorBy.colorByMap[v])){var index=colorBy.colorByValues.length;colorBy.colorByValues.push(v);var color=index>=colorBy.colors.length?colorBy.colors[colorBy.colors.length-1]:colorBy.colors[index];colorBy.colorByMap[v]=color;colorBy.minValue=1;colorBy.maxValue=colorBy.colorByValues.length;}
return;}
if(excludeZero&&v===0){return;}
if(!isNaN(v)&&!(v===null)){if(cnt==0||v>colorBy.maxValue)colorBy.maxValue=v;if(cnt==0||v<colorBy.minValue)colorBy.minValue=v;cnt++;}});}
if(this.showPercent){colorBy.minValue=0;colorBy.maxValue=100;}
var steps=this.getProperty("colorBySteps");if(steps){colorBy.steps=steps.split(",");}
colorBy.minValue=this.getDisplayProp(this,"colorByMin",colorBy.minValue);colorBy.minValue=this.getDisplayProp(this,"colorByMin",colorBy.minValue);colorBy.maxValue=this.getDisplayProp(this,"colorByMax",colorBy.maxValue);colorBy.origMinValue=colorBy.minValue;colorBy.origMaxValue=colorBy.maxValue;var colorByLog=this.getProperty("colorByLog",false);colorBy.colorByFunc=Math.log;if(colorByLog){if(colorBy.minValue<1){colorBy.colorByOffset=1-colorBy.minValue;}
colorBy.minValue=colorBy.colorByFunc(colorBy.minValue+colorBy.colorByOffset);colorBy.maxValue=colorBy.colorByFunc(colorBy.maxValue+colorBy.colorByOffset);}
colorBy.range=colorBy.maxValue-colorBy.minValue;return colorBy;},getColorByMap:function(prop){return Utils.parseMap(this.getProperty(prop||"colorByMap"));},toString:function(){return"RamaddaDisplay:"+this.type+" - "+this.getId();},getType:function(){return this.type;},getClass:function(suffix){if(suffix==null){return this.getBaseClass();}
return this.getBaseClass()+"-"+suffix;},getBaseClass:function(){return"display-"+this.getType();},setDisplayManager:function(cm){this.displayManager=cm;this.setDisplayParent(cm.getLayoutManager());},setContents:function(contents){var style="";contents=HtmlUtils.div([ATTR_CLASS,"display-contents-inner display-"+this.getType()+"-inner","style",style],contents);this.writeHtml(ID_DISPLAY_CONTENTS,contents);},addEntry:function(entry){this.entries.push(entry);},clearCachedData:function(){},setEntry:function(entry){this.entries=[];this.addEntry(entry);this.entry=entry;this.entryId=entry.getId();this.clearCachedData();if(this.properties.data){this.dataCollection=new DataCollection();var attrs={entryId:this.entryId,lat:this.getProperty("latitude"),lon:this.getProperty("longitude"),};this.properties.data=this.data=new PointData(entry.getName(),null,null,this.getRamadda().getRoot()+"/entry/show?entryid="+entry.getId()+"&output=points.product&product=points.json&max=5000",attrs);this.startProgress();this.data.loadData(this);}else{this.updateUI();}
var title="";if(this.getShowTitle()){this.jq(ID_TITLE).html(entry.getName());}},getTextColor:function(property){if(property)return this.getProperty(property,this.getProperty("textColor"));return this.getProperty("textColor","#000");},getTitleHtml:function(title){var titleToShow="";if(this.getShowTitle()){var titleStyle=" color:"+this.getTextColor("titleColor")+";";var bg=this.getProperty("titleBackground");if(bg)titleStyle+="background:"+bg+"; padding:2px;padding-right:6px;padding-left:6px;";titleToShow=this.getShowTitle()?this.getDisplayTitle(title):"";if(this.entryId)
...
return titleToShow;},handleEventMapBoundsChanged:function(source,args){if(this.getProperty("acceptBoundsChange")){this.filterBounds=args.bounds;this.updateUI();}},handleEventFieldValueSelected:function(source,args){this.setProperty("filterPattern",args.value);this.setProperty("patternFilterField",args.field.getId());this.updateUI();},handleDateRangeChanged:function(source,prop){this.minDateObj=prop.minDate;this.maxDateObj=prop.maxDate;this.getAnimation().setDateRange(prop.minDate,prop.maxDate);this.haveCalledUpdateUI=false;this.dataFilterChanged();},handleEventPropertyChanged:function(source,prop){if(prop.property=="dateRange"){if(this.getProperty("acceptDateRangeChange")){this.handleDateRangeChanged(source,prop);}
return;}
...
this.settingFilterValue=true;if(Utils.isDefined(prop.value2)){$("#"+widgetId+"_min").val(prop.value);$("#"+widgetId+"_min").attr("data-value",prop.value);$("#"+widgetId+"_max").val(prop.value2);$("#"+widgetId+"_max").attr("data-value",prop.value2);}else{var widget=$("#"+widgetId);if(widget.attr("isCheckbox")){var on=widget.attr("onValue");var off=widget.attr("offValue");widget.prop('checked',prop.value.includes(on));}else{widget.val(prop.value);}
widget.attr("data-value",prop.value);if(widget.attr("isButton")){widget.find(".display-filterby-button").removeClass("display-filterby-button-selected");widget.find("[value='"+prop.value+"']").addClass("display-filterby-button-selected");}}
this.settingFilterValue=false;this.dataFilterChanged();return;}
this.setProperty(prop.property,prop.value);this.updateUI();},handleEventRecordHighlight:function(source,args){if(this.getProperty("doAnimation",false)&&!args.skipAnimation){this.getAnimation().handleEventRecordHighlight(source,args);}},handleEventRecordSelection:function(source,args){if(!source.getEntries){return;}
...
Utils.call(callback,entry);});}else{Utils.call(callback,null);}},hasEntries:function(){return this.entries!=null&&this.entries.length>0;},getWaitImage:function(){return HtmlUtils.image(ramaddaBaseUrl+"/icons/progress.gif");},getLoadingMessage:function(msg){if(!msg)msg=this.getProperty("loadingMessage","Loading data...");if(msg=="")return"";return HtmlUtils.div(["text-align","center"],this.getMessage("&nbsp;"+msg));},getMessage:function(msg){return HtmlUtils.div([ATTR_CLASS,"display-message"],msg);},getFieldValue:function(id,dflt){var jq=$("#"+id);if(jq.length>0){return jq.val();}
return dflt;},getFooter:function(){return HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER),ATTR_CLASS,"display-footer"],HtmlUtils.leftRight(HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER_LEFT),ATTR_CLASS,"display-footer-left"],""),HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER_RIGHT),ATTR_CLASS,"display-footer-right"],"")));},shouldSkipField:function(field){if(this.skipFields&&!this.skipFieldsList){this.skipFieldsList=this.skipFields.split(",");}
...
var seenLabels={};var allFields=this.dataCollection.getList()[0].getRecordFields();var badFields={};var flags=null;for(var collectionIdx=0;collectionIdx<dataList.length;collectionIdx++){var pointData=dataList[collectionIdx];var fields=this.getFieldsToSelect(pointData);if(this.canDoGroupBy()){var allFields=pointData.getRecordFields();var cnt=0;for(i=0;i<allFields.length;i++){var field=allFields[i];if(field.getType()!="string")continue;if(cnt==0){html+=HtmlUtils.div([ATTR_CLASS,"display-dialog-subheader"],"Group By");html+=HtmlUtils.openTag(TAG_DIV,[ATTR_CLASS,"display-fields"]);var on=this.groupBy==null||this.groupBy=="";html+=HtmlUtils.tag(TAG_DIV,[ATTR_TITLE,"none"],HtmlUtils.radio("none",this.getDomId("groupby"),groupByClass,"none",!on)+" None");}
cnt++;var on=this.groupBy==field.getId();var idBase="groupby_"+collectionIdx+"_"+i;field.radioId=this.getDomId(idBase);html+=HtmlUtils.tag(TAG_DIV,[ATTR_TITLE,field.getId()],HtmlUtils.radio(field.radioId,this.getDomId("groupby"),groupByClass,field.getId(),on)+" "+field.getUnitLabel()+" ("+field.getId()+")");}
if(cnt>0){html+=HtmlUtils.closeTag(TAG_DIV);}}
...
var disabledFields="";html+=HtmlUtils.div([ATTR_CLASS,"display-dialog-subheader"],"Displayed Fields");html+=HtmlUtils.openTag(TAG_DIV,[ATTR_CLASS,"display-fields"]);for(var tupleIdx=0;tupleIdx<fields.length;tupleIdx++){var field=fields[tupleIdx];var idBase="cbx_"+collectionIdx+"_"+tupleIdx;field.checkboxId=this.getDomId(idBase);var on=false;var hasValues=(flags?flags[field.getIndex()]:true);if(argFields!=null){for(var fIdx=0;fIdx<argFields.length;fIdx++){if(argFields[fIdx].getId()==field.getId()){on=true;this.overrideFields.push(field.getId());break;}}}else if(selectedIds.length>0){on=selectedIds.indexOf(field.getId())>=0;}else if(fixedFields!=null){on=(fixedFields.indexOf(field.getId())>=0);if(!on){on=(fixedFields.indexOf("#"+(tupleIdx+1))>=0);}
...
if(!hasValues){disabledFields+=HtmlUtils.div([],label);}else{if(field.derived){label+=" (derived)";}
var widget;if(this.canDoMultiFields()){widget=HtmlUtils.checkbox(field.checkboxId,["class",checkboxClass],on);}else{widget=HtmlUtils.radio(field.checkboxId,"field_radio",checkboxClass,"",on);}
html+=HtmlUtils.tag(TAG_DIV,[ATTR_TITLE,field.getId()],widget+" "+label);}
}}
if(disabledFields!=""){html+=HtmlUtils.div(["style","border-top:1px #888  solid;"],"<b>No Data Available</b>"+disabledFields);}
html+=HtmlUtils.closeTag(TAG_DIV);}
this.writeHtml(ID_FIELDS,html);this.userHasSelectedAField=false;var theDisplay=this;$("."+checkboxClass).click(function(event){theDisplay.fieldSelected(event);});$("."+groupByClass).change(function(event){theDisplay.groupBy=$(this).val();if(theDisplay.displayData){theDisplay.displayData();}});},fieldSelectionChanged:function(){var name="the display";this.setDisplayTitle();if(this.displayData){this.clearCachedData();this.displayData();}},defaultSelectedToAll:function(){return false;},setSelectedFields:function(fields){this.clearCachedData();this.selectedFields=fields;this.addFieldsCheckboxes(fields);},getSelectedFields:function(dfltList){if(this.getProperty("binDate")){var binType=this.getProperty("binType","total");var binCount=binType=="count";if(binCount){var fields=[];fields.push(new RecordField({id:binType,label:this.getProperty("binDateLabel",this.getProperty("binCountLabel","Count")),type:"double",chartable:true}));return fields;}}
this.debugSelected=false;this.lastSelectedFields=this.getSelectedFieldsInner(dfltList);var fixedFields=this.getProperty(PROP_FIELDS);if(fixedFields)fixedFields.length=0;this.setDisplayTitle();if(this.getProperty("binDate")){var binType=this.getProperty("binType","total");let fields=[];this.lastSelectedFields.map(field=>{if(!field.isNumeric()){fields.push(field);}else{const prefix=binType;if(field.getId().startsWith(prefix)){fields.push(field);}else{fields.push(new RecordField({id:prefix+"_"+field.getId(),index:field.getIndex(),label:this.getProperty("binDateLabel",Utils.camelCase(binType)+" of "+field.getLabel()),type:"double",chartable:field.isChartable()}));}}});this.lastSelectedFields=fields;}
return this.lastSelectedFields;},getSelectedFieldsInner:function(dfltList){if(this.debugSelected)
console.log("getSelectedFields dflt:"+(dfltList?dfltList.length:"null"));if(this.selectedFields){if(this.debugSelected)
console.log("\tgot this.selectedFields");return this.selectedFields;}
...
let aliases={};var tmp=this.getProperty("fieldAliases");if(tmp){tmp.split(",").map(tok=>{[name,alias]=tok.split(":");aliases[alias]=name;});}
...
console.log("\thave fixed fields:"+fixedFields.length);for(var i=0;i<fixedFields.length;i++){var sfield=fixedFields[i];var alias=aliases[sfield];if(this.debugSelected)
console.log("\t\tfixed field:"+sfield);for(var j=0;j<fields.length;j++){var field=fields[j];var id=field.getId();if(this.debugSelected)
console.log("\t\t\tlooking at:"+id);if(id==sfield||("#"+(j+1))==sfield||id==alias){if(this.debugSelected)
console.log("\t\t\t\tgot:"+field.getLabel());df.push(field);break;}}}}}
...
console.log("\tlastSelectedFields");return this.lastSelectedFields;}}
...
return df;},getDefaultSelectedFields:function(fields,dfltList){if(this.defaultSelectedToAll()&&this.allFields!=null){var tmp=[];for(i=0;i<this.allFields.length;i++){var field=this.allFields[i];if(!field.isFieldGeo()){tmp.push(field);}}
return tmp;}
if(dfltList!=null){return dfltList;}
...
if(sortFields.length==0)return records;records=Utils.cloneList(records);var sortAscending=this.getProperty("sortAscending",true);records.sort((a,b)=>{var row1=this.getDataValues(a);var row2=this.getDataValues(b);var result=0;for(var i=0;i<sortFields.length;i++){var sortField=sortFields[i];var v1=row1[sortField.getIndex()];var v2=row2[sortField.getIndex()];if(v1<v2)result=sortAscending?-1:1;else if(v1>v2)result=sortAscending?1:-1;else result=0;if(result!=0)break;}
return result;});return records;},getFieldById:function(fields,id){if(!id)return null;if(!fields){var pointData=this.getData();if(pointData==null)return null;fields=pointData.getRecordFields();}
for(var i=0;i<fields.length;i++){var field=fields[i];if(field.getId()==id||id==("#"+i)){return field;}}
return null;},getFieldsByIds:function(fields,ids){var result=[];if(!ids)return result;if((typeof ids)=="string")
ids=ids.split(",");for(var i=0;i<ids.length;i++){var f=this.getFieldById(fields,ids[i]);if(f)result.push(f);}
...
return list;},getDateValues:function(records){var dates=[];records.map(r=>{dates.push(r.getDate());});return dates;},getColumnValues:function(records,field){var values=[];var min=Number.MAX_VALUE;var max=Number.MIN_VALUE;for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];var row=record.getData();var value=row[field.getIndex()];values.push(value);if(Utils.isNumber(value)){min=Math.min(min,value);max=Math.max(max,value);}}
return{values:values,min:min,max:max};},requiresGrouping:function(){return false;},getFilterFieldValues:function(field){var element=$("#"+this.getDomId("filterby_"+field.getId()));var value=null;if(element.attr("isCheckbox")){if(element.is(':checked')){value=element.attr("onValue");}else{value=element.attr("offValue");}}else if(element.attr("isButton")){value=element.attr("data-value");}else{value=element.val();}
if(!value)value=FILTER_ALL;if(!Array.isArray(value))value=value.split(",");var tmp=[];value.map(v=>tmp.push(v.trim()));value=tmp;return value;},makeTree:function(records){if(records==null){var pointData=this.getData();if(pointData==null)return null;records=pointData.getRecords();}
...
if(this.getProperty("showPercentIncrease",false)){var dataList=pointData.getRecords();var newRecords=[];var newFields=[];var firstRow=dataList[0];var fields=pointData.getRecordFields();var firstRecord=dataList[0];fields.map(f=>{var newField=f.clone();newFields.push(newField);newField.label=newField.label+" % increase";});for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var record=dataList[rowIdx];var newRecord=record.clone();newRecords.push(newRecord);fields.map(f=>{if(!f.isNumeric())return;if(rowIdx==0){newRecord.data[f.getIndex()]=0;}else{var v=newRecord.data[f.getIndex()];var basev=firstRecord.data[f.getIndex()];newRecord.data[f.getIndex()]=basev==0?0:(v-basev)/basev;}});};pointData=new PointData("pointdata",newFields,newRecords,null,null);pointData.entryId=originalPointData.entryId;}
if(this.getProperty("showAverages",false)){var dataList=pointData.getRecords();var newRecords=[];var newFields=[];var firstRow=dataList[0];var fields=pointData.getRecordFields();var firstRecord=dataList[0];fields.map(f=>{var newField=f.clone();newFields.push(newField);newField.label=newField.label+" (avg)";});var sums=[];fields.map(f=>{sums.push(0)});var newRecord;for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var record=dataList[rowIdx];if(newRecord==null){newRecord=record.clone();newRecords.push(newRecord);}
fields.map((f,idx)=>{if(!f.isNumeric())return;var v=record.data[f.getIndex()];sums[idx]+=v;});fields.map((f,idx)=>{if(!f.isNumeric())return;newRecord.data[idx]=sums[idx]/dataList.length;});};pointData=new PointData("pointdata",newFields,newRecords,null,null);pointData.entryId=originalPointData.entryId;}
return pointData;},filterData:function(dataList,fields,doGroup,skipFirst){var startDate=this.getProperty("startDate");var endDate=this.getProperty("endDate");if(startDate){this.startDateObject=new Date(startDate);}
if(endDate){this.endDateObject=new Date(endDate);}
var pointData=this.getData();if(!dataList){if(pointData==null)return null;dataList=pointData.getRecords();}
if(!fields){fields=pointData.getRecordFields();}
if(doGroup||this.requiresGrouping()){dataList=pointData.extractGroup(this.dataGroup,dataList);}
var newData=[];var filters={};var allIsUsed=false;for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];if(filterField.isString()){var values=this.getFilterFieldValues(filterField);if(!values&&!Array.isArray(values))values=[values];if(values.includes(FILTER_ALL)){allIsUsed=true;break;}}}
for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];var prefix=this.getProperty(filterField.getId()+".filterPrefix");var suffix=this.getProperty(filterField.getId()+".filterSuffix");if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;var value=null;var values=null;var _values=[];var matchers=[];var values=null;if(filterField.isNumeric()){var minField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_min"));var maxField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_max"));var minValue=parseFloat(minField.val().trim());var maxValue=parseFloat(maxField.val().trim());var dfltMinValue=parseFloat(minField.attr("data-min"));var dfltMaxValue=parseFloat(maxField.attr("data-max"));if(minValue!=dfltMinValue||maxValue!=dfltMaxValue){value=[minValue,maxValue];}}else if(filterField.getType()=="date"){var date1=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date1")).val();var date2=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date2")).val();if(date1!=null&&date1.trim()!="")
...
value=[date1,date2];}else{values=this.getFilterFieldValues(filterField);if(!values&&!Array.isArray(values))values=[values];if(values.length==0)continue;var tmp=[];values.map(v=>{tmp.push(v.replace(/_comma_/g,","));});values=tmp;values.map(v=>{_values.push((""+v).toLowerCase());try{matchers.push(new TextMatcher(v));}catch(skipIt){}});}
var filterStartsWith=this.getProperty(filterField.getId()+".filterStartsWith",false);var anyValues=value!=null;if(!anyValues&&values){values.map(v=>{if(v.length>0&&v!=FILTER_ALL)anyValues=true});}
if(anyValues){filters[filterField.getId()]={value:value,values:values,matchers:matchers,_values:_values,anyValues:anyValues,startsWith:filterStartsWith,};}}
for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var record=dataList[rowIdx];var date=record.getDate();if(!this.dateInRange(date)){continue;}
if(this.filterBounds&&record.hasLocation()){var b=this.filterBounds;var lat=record.getLatitude();var lon=record.getLongitude();if(lat>b.top||lat<b.bottom||lon<b.left||lon>b.right)
continue;}
var row=this.getDataValues(record);var ok=true;for(var i=0;i<this.filterFields.length&&ok;i++){var filterField=this.filterFields[i];var filter=filters[filterField.getId()];if(filter==null)continue;var rowValue=row[filterField.getIndex()];var debug=false;if(debug)
console.log("filter field:"+filterField.getId()+" value:"+rowValue);if(filterField.getType()=="enumeration"){ok=filter.values.includes(""+rowValue);if(debug)
console.log("enum:"+rowValue+" ok:"+ok);}else if(filterField.isNumeric()){if(isNaN(filter.value[0])&&isNaN(filter.value[0]))continue;if(!isNaN(filter.value[0])&&rowValue<filter.value[0])ok=false;else if(!isNaN(filter.value[1])&&rowValue>filter.value[1])ok=false;}else if(filterField.getType()=="date"){if(filter.value&&Array.isArray(filter.value)){if(rowValue==null){ok=false;}else{var date1=filter.value[0];var date2=filter.value[1];var dttm=rowValue.getTime();if(isNaN(dttm))ok=false;else if(date1&&dttm<date1.getTime())
ok=false;else if(date2&&dttm>date2.getTime())
ok=false;}}}else{var startsWith=filter.startsWith;ok=false;roWValue=(""+rowValue).toLowerCase();for(var j=0;j<filter._values.length;j++){var fv=_values[j];if(startsWith){if(rowValue.toString().startsWith(fv)){ok=true;break;}}else if(rowValue.toString().indexOf(fv)>=0){ok=true;break;}}
if(!ok){for(ri=0;ri<filter.matchers.length;ri++){var matcher=filter.matchers[ri];if(matcher.matches(rowValue.toString())){ok=true;break;}}}}}
if(skipFirst&&rowIdx==0){ok=true;}
if(ok){newData.push(dataList[rowIdx]);}}
dataList=newData;var stride=parseInt(this.getProperty("stride",-1));if(stride<0){var maxSize=parseInt(this.getProperty("maxDisplayedPoints",-1));if(maxSize>0&&dataList.length>0){stride=1;while(dataList.length/stride>maxSize){stride++;}}}
if(stride>0){console.log("doing stride:"+stride);var list=[];var cnt=0;for(var i=0;i<dataList.length;i+=(stride+1)){list.push(dataList[i]);}
dataList=list;}
if(this.getProperty("binDate")){var what=this.getProperty("binDate");var binType=this.getProperty("binType","total");var binCount=binType=="count";var binned=[];var record=dataList[0];var map={};var counts={};for(var i=0;i<dataList.length;i++){var record=dataList[i];var tuple=this.getDataValues(record);var key;var baseDate=null
...
var newRecord=new PointRecord(record.getLatitude(),record.getLongitude(),record.getElevation(),date,data);map[key]=data;binned.push(newRecord);}else{counts[key]++;var tuple1=map[key];if(binCount){for(k=0;k<tuple1.length;k++)tuple1[k]++;continue;}
...
dataList=binned;}
dataList=this.sortRecords(dataList);if(this.getProperty("uniqueField")){let ufield=this.getFieldById(null,this.getProperty("uniqueField"));let umap={};let ulist=[];for(var i=dataList.length-1;i>=0;i--){var record=dataList[i];var v=record.getValue(ufield.getIndex());if(!Utils.isDefined(umap[v])){umap[v]=true;ulist.push(record);}}
dataList=ulist;}
this.recordToIndex={};this.indexToRecord={};for(var i=0;i<dataList.length;i++){var record=dataList[i];this.recordToIndex[record.getId()]=i;this.indexToRecord[i]=record;}
return dataList;},canDoGroupBy:function(){return false;},canDoMultiFields:function(){return true;},useChartableFields:function(){return true;},getFieldsToSelect:function(pointData){if(this.useChartableFields())
...
args.push("description_encoded");args.push(window.btoa(wiki));var url=HtmlUtils.getUrl(ramaddaBaseUrl+"/entry/publish",args);window.open(url,'_blank');},getChildEntries:function(includeFixed){var seen={};var allEntries=[];for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(!includeFixed&&display.getIsLayoutFixed()){continue;}
...
var url=ramaddaBaseUrl+"/entry/copy?action.force=copy&from="+from;window.open(url,'_blank');},entryHtmlHasBeenDisplayed:async function(entry){if(entry.getIsGroup()){var theDisplay=this;var callback=function(entries){var html=HtmlUtils.openTag(TAG_OL,[ATTR_CLASS,"display-entrylist-list",ATTR_ID,theDisplay.getDomId(ID_LIST)]);html+=theDisplay.getEntriesTree(entries);html+=HtmlUtils.closeTag(TAG_OL);theDisplay.jq(ID_GROUP_CONTENTS+entry.getIdForDom()).html(html);theDisplay.addEntrySelect();};await entry.getChildrenEntries(callback);}},getEntryHtml:function(entry,props){var dfltProps={showHeader:true,headerRight:false,showDetails:this.getShowDetails()};$.extend(dfltProps,props);props=dfltProps;var menu=this.getEntryMenuButton(entry);var html="";if(props.showHeader){var left=menu+" "+entry.getLink(entry.getIconImage()+" "+entry.getName());if(props.headerRight)html+=HtmlUtils.leftRight(left,props.headerRight);else html+=left;}
var divid=HtmlUtils.getUniqueId("entry_");html+=HtmlUtils.div(["id",divid],"");if(false){var url=this.getRamadda().getRoot()+"/entry/show?entryid="+entry.getId()+"&decorate=false&output=metadataxml&details=true";$("#"+divid).load(url,function(){alert("Load was performed.");});}
var desc=entry.getDescription();if(desc)
desc=desc.replace(/\n/g,"<br>");else
desc="";html+=desc;var metadata=entry.getMetadata();if(entry.isImage()){var img=HtmlUtils.tag(TAG_IMG,["src",entry.getResourceUrl(),ATTR_CLASS,"display-entry-image"]);html+=HtmlUtils.href(entry.getResourceUrl(),img)+"<br>";}else{for(var i=0;i<metadata.length;i++){if(metadata[i].type=="content.thumbnail"){var image=metadata[i].value.attr1;var url;if(image.indexOf("http")==0){url=image;}else{url=ramaddaBaseUrl+"/metadata/view/"+image+"?element=1&entryid="+entry.getId()+"&metadata_id="+metadata[i].id+"&thumbnail=false";}
html+=HtmlUtils.image(url,[ATTR_CLASS,"display-entry-thumbnail"]);}}}
if(entry.getIsGroup()){html+=HtmlUtils.div([ATTR_ID,this.getDomId(ID_GROUP_CONTENTS+entry.getIdForDom())],"");}
html+=HtmlUtils.formTable();if(props.showDetails){if(entry.url){html+=HtmlUtils.formEntry("URL:",HtmlUtils.href(entry.url,entry.url));}
if(entry.remoteUrl){html+=HtmlUtils.formEntry("URL:",HtmlUtils.href(entry.remoteUrl,entry.remoteUrl));}
if(entry.remoteRepository){html+=HtmlUtils.formEntry("From:",HtmlUtils.href(entry.remoteRepository.url,entry.remoteRepository.name));}}
var columns=entry.getAttributes();if(entry.getFilesize()>0){html+=HtmlUtils.formEntry("File:",entry.getFilename()+" "+
...
if(column.isUrl&&column.isUrl()){var tmp="";var toks=columnValue.split("\n");for(var i=0;i<toks.length;i++){var url=toks[i].trim();if(url.length==0)continue;tmp+=HtmlUtils.href(url,url);tmp+="<br>";}
columnValue=tmp;}
html+=HtmlUtils.formEntry(column.label+":",columnValue);}
html+=HtmlUtils.closeTag(TAG_TABLE);return html;},getEntriesTree:function(entries,props){if(!props)props={};var columns=this.getProperty("entryColumns",null);if(columns!=null){var columnNames=this.getProperty("columnNames",null);if(columnNames!=null){columnNames=columnNames.split(",");}
...
var icon=entry.getIconImage([ATTR_TITLE,"View entry"]);var link=HtmlUtils.tag(TAG_A,[ATTR_HREF,entry.getEntryUrl()],icon+" "+entryName);entryName="";var entryIdForDom=entry.getIdForDom()+domIdSuffix;var entryId=entry.getId();var arrow=HtmlUtils.image(icon_tree_closed,[ATTR_BORDER,"0","tree-open","false",ATTR_ID,this.getDomId(ID_TREE_LINK+entryIdForDom)]);var toggleCall=this.getGet()+".toggleEntryDetails(event, '"+entryId+"',"+suffix+",'"+props.handlerId+"');";var toggleCall2=this.getGet()+".entryHeaderClick(event, '"+entryId+"',"+suffix+"); ";var open=HtmlUtils.onClick(toggleCall,arrow);var extra="";if(showIndex){extra="#"+(i+1)+" ";}
if(handler&&handler.getEntryPrefix){extra+=handler.getEntryPrefix(props.handlerId,entry);}
var left=HtmlUtils.div([ATTR_CLASS,"display-entrylist-name"],entryMenuButton+" "+open+" "+extra+link+" "+entryName);var details=HtmlUtils.div([ATTR_ID,this.getDomId(ID_DETAILS+entryIdForDom),ATTR_CLASS,"display-entrylist-details"],HtmlUtils.div([ATTR_CLASS,"display-entrylist-details-inner",ATTR_ID,this.getDomId(ID_DETAILS_INNER+entryIdForDom)],""));var line;if(this.getProperty("showToolbar",true)){line=HtmlUtils.leftCenterRight(left,"",toolbar,"80%","1%","19%");}else{line=left;}
var mainLine=HtmlUtils.div(["onclick",toggleCall2,ATTR_ID,this.getDomId(ID_DETAILS_MAIN+entryIdForDom),ATTR_CLASS,"display-entrylist-entry-main"+" "+"entry-main-display-entrylist-"+(even?"even":"odd"),ATTR_ENTRYID,entryId],line);var line=HtmlUtils.div(["class",(even?"ramadda-row-even":"ramadda-row-odd"),ATTR_ID,this.getDomId("entryinner_"+entryIdForDom)],mainLine+details);html+=HtmlUtils.tag(TAG_DIV,[ATTR_ID,this.getDomId("entry_"+entryIdForDom),ATTR_ENTRYID,entryId,ATTR_CLASS,"display-entrylist-entry"+rowClass],line);html+="\n";}
...
var html=HtmlUtils.openTag(TAG_TABLE,[ATTR_WIDTH,"100%","cellpadding","0","cellspacing","0"]);html+=HtmlUtils.openTag(TAG_TR,["valign","top"]);for(var i=0;i<columnNames.length;i++){html+=HtmlUtils.td([ATTR_ALIGN,"center",ATTR_CLASS,"display-entrytable-header"],columnNames[i]);}
html+=HtmlUtils.closeTag(TAG_TR);for(var i=0;i<entries.length;i++){html+=HtmlUtils.openTag(TAG_TR,["valign","top"]);var entry=entries[i];this.entriesMap[entry.getId()]=entry;for(var j=0;j<columns.length;j++){var columnWidth=null;if(columnWidths!=null){columnWidth=columnWidths[j];}
var column=columns[j];var value="";if(column=="name"){value=HtmlUtils.tag(TAG_A,[ATTR_HREF,entry.getEntryUrl()],entry.getName());}else if(column.match(".*property:.*")){var type=column.substring("property:".length);var metadata=entry.getMetadata();value="";for(var j=0;j<metadata.length;j++){var m=metadata[j];if(m.type==type){if(value!=""){value+="<br>";}
...
html+=HtmlUtils.closeTag(TAG_TABLE);return html;},makeEntryToolbar:function(entry,handler,handlerId){var get=this.getGet();var toolbarItems=[];var props="{showMenu:true,showTitle:true}";if(entry.getType().getId()=="type_wms_layer"){toolbarItems.push(HtmlUtils.tag(TAG_A,["onclick",get+".addMapLayer("+HtmlUtils.sqt(entry.getId())+");"],HtmlUtils.image(ramaddaBaseUrl+"/icons/map.png",["border",0,ATTR_TITLE,"Add Map Layer"])));}
if(entry.getType().getId()=="geo_shapefile"||entry.getType().getId()=="geo_geojson"){toolbarItems.push(HtmlUtils.tag(TAG_A,["onclick",get+".addMapLayer("+HtmlUtils.sqt(entry.getId())+");"],HtmlUtils.image(ramaddaBaseUrl+"/icons/map.png",["border",0,ATTR_TITLE,"Add Map Layer"])));}
var jsonUrl=this.getPointUrl(entry);if(jsonUrl!=null){jsonUrl=jsonUrl.replace(/\'/g,"_");toolbarItems.push(HtmlUtils.tag(TAG_A,["onclick",get+".createDisplay("+HtmlUtils.sqt(entry.getFullId())+","+
HtmlUtils.sqt("table")+","+HtmlUtils.sqt(jsonUrl)+","+props+");"],HtmlUtils.getIconImage("fa-table",[ATTR_TITLE,"Create Tabular Display"])));var x;toolbarItems.push(x=HtmlUtils.tag(TAG_A,["onclick",get+".createDisplay("+HtmlUtils.sqt(entry.getFullId())+","+
HtmlUtils.sqt("linechart")+","+HtmlUtils.sqt(jsonUrl)+","+props+");"],HtmlUtils.getIconImage("fa-chart-line",[ATTR_TITLE,"Create Chart"])));}
toolbarItems.push(HtmlUtils.tag(TAG_A,["onclick",get+".createDisplay("+HtmlUtils.sqt(entry.getFullId())+","+
HtmlUtils.sqt("entrydisplay")+","+HtmlUtils.sqt(jsonUrl)+","+props+");"],HtmlUtils.getIconImage("fa-file",["border",0,ATTR_TITLE,"Show Entry"])));if(entry.getFilesize()>0){toolbarItems.push(HtmlUtils.tag(TAG_A,[ATTR_HREF,entry.getResourceUrl()],HtmlUtils.image(ramaddaBaseUrl+"/icons/download.png",["border",0,ATTR_TITLE,"Download ("+entry.getFormattedFilesize()+")"])));}
...
for(var i=0;i<toolbarItems.length;i++){tmp.push(HtmlUtils.div([ATTR_CLASS,"display-entry-toolbar-item"],toolbarItems[i]));}
toolbarItems=tmp;return HtmlUtils.div([ATTR_CLASS,"display-entry-toolbar",ATTR_ID,this.getEntryToolbarId(entry.getIdForDom())],HtmlUtils.join(toolbarItems,""));},getEntryToolbarId:function(entryId){var id=entryId.replace(/:/g,"_");id=id.replace(/=/g,"_");return this.getDomId(ID_TOOLBAR+"_"+id);},hideEntryDetails:function(entryId){},entryHeaderClick:function(event,entryId,suffix){var target=event.target;if(target.outerHTML){if(target.outerHTML.indexOf("<div")!=0){return;}}
...
link.attr("tree-open",open?"false":"true");var hereBefore=details.attr("has-content")!=null;details.attr("has-content","true");if(hereBefore){}else{if(entry.getIsGroup()){detailsInner.html(HtmlUtils.image(icon_progress));var theDisplay=this;var callback=function(entries){theDisplay.displayChildren(entry,entries,suffix,handlerId);};var entries=entry.getChildrenEntries(callback);}else{detailsInner.html(this.getEntryHtml(entry,{showHeader:false}));}}
...
entriesHtml+=this.getEntriesTree(entries,{handlerId:handlerId});detailsInner.html(entriesHtml);this.addEntrySelect();}},getEntryMenuButton:function(entry){var menuButton=HtmlUtils.onClick(this.getGet()+".showEntryMenu(event, '"+entry.getId()+"');",HtmlUtils.image(ramaddaBaseUrl+"/icons/menu.png",[ATTR_CLASS,"display-entry-toolbar-item",ATTR_ID,this.getDomId(ID_MENU_BUTTON+entry.getIdForDom())]));return menuButton;},setRamadda:function(e){this.ramadda=e;},getRamadda:function(){if(this.ramadda!=null){return this.ramadda;}
...
var get=this.getGet();var menus=[];var fileMenuItems=[];var viewMenuItems=[];var newMenuItems=[];viewMenuItems.push(HtmlUtils.tag(TAG_LI,[],HtmlUtils.tag(TAG_A,["href",entry.getEntryUrl(),"target","_"],"View Entry")));if(entry.getFilesize()>0){fileMenuItems.push(HtmlUtils.tag(TAG_LI,[],HtmlUtils.tag(TAG_A,["href",entry.getResourceUrl()],"Download "+entry.getFilename()+" ("+entry.getFormattedFilesize()+")")));}
if(this.jsonUrl!=null){fileMenuItems.push(HtmlUtils.tag(TAG_LI,[],"Data: "+HtmlUtils.onClick(get+".fetchUrl('json');","JSON")+
HtmlUtils.onClick(get+".fetchUrl('csv');","CSV")));}
var props="{showMenu:true,showTitle:true}";var newMenu="<a>New</a><ul>";newMenu+=HtmlUtils.tag(TAG_LI,[],HtmlUtils.onClick(get+".createDisplay('"+entry.getFullId()+"','entrydisplay',null,null,"+props+");","New Entry Display"));newMenuItems.push(HtmlUtils.tag(TAG_LI,[],HtmlUtils.onClick(get+".createDisplay('"+entry.getFullId()+"','entrydisplay',null,null,"+props+");","New Entry Display")));var pointUrl=this.getPointUrl(entry);if(pointUrl!=null){var types=window.globalDisplayTypes;var catMap={};if(types){for(var i=0;i<types.length;i++){var type=types[i];if(!type.requiresData||!type.forUser)continue;if(!Utils.isDefined(catMap[type.category])){catMap[type.category]="<li> <a>"+type.category+"</a><ul>\n";}
pointUrl=pointUrl.replace(/\'/g,"_");var call=get+".createDisplay("+HtmlUtils.sqt(entry.getFullId())+","+HtmlUtils.sqt(type.type)+","+HtmlUtils.sqt(pointUrl)+",null,"+props+");";var li=HtmlUtils.tag(TAG_LI,[],HtmlUtils.tag(TAG_A,["onclick",call],type.label));catMap[type.category]+=li+"\n";newMenuItems.push(li);}}
for(a in catMap){newMenu+=catMap[a]+"</li></ul>";}}
if(fileMenuItems.length>0)
menus.push("<a>File</a>"+HtmlUtils.tag(TAG_UL,[],HtmlUtils.join(fileMenuItems)));if(viewMenuItems.length>0)
menus.push("<a>View</a>"+HtmlUtils.tag(TAG_UL,[],HtmlUtils.join(viewMenuItems)));if(newMenuItems.length>0)
menus.push(newMenu);var topMenus="";for(var i=0;i<menus.length;i++){topMenus+=HtmlUtils.tag(TAG_LI,[],menus[i]);}
var menu=HtmlUtils.tag(TAG_UL,[ATTR_ID,this.getDomId(ID_MENU_INNER+entry.getIdForDom()),ATTR_CLASS,"sf-menu"],topMenus);callback(menu);},showEntryMenu:async function(event,entryId){var menu;await this.getEntryMenu(entryId,m=>{menu=m});this.writeHtml(ID_MENU_OUTER,menu);var srcId=this.getDomId(ID_MENU_BUTTON+Utils.cleanId(entryId));showPopup(event,srcId,this.getDomId(ID_MENU_OUTER),false,"left top","left bottom");$("#"+this.getDomId(ID_MENU_INNER+Utils.cleanId(entryId))).superfish({speed:'fast',delay:300});},fetchUrl:function(as,url){if(url==null){url=this.jsonUrl;}
...
window.open(url,'_blank');},getMenuItems:function(menuItems){},getDisplayMenuSettings:function(){var get="getRamaddaDisplay('"+this.getId()+"')";var moveRight=HtmlUtils.onClick(get+".moveDisplayRight();","Right");var moveLeft=HtmlUtils.onClick(get+".moveDisplayLeft();","Left");var moveTop=HtmlUtils.onClick(get+".moveDisplayTop();","Top");var moveUp=HtmlUtils.onClick(get+".moveDisplayUp();","Up");var moveDown=HtmlUtils.onClick(get+".moveDisplayDown();","Down");var menu="<table class=formtable>"+
...
"</table>";var tmp=HtmlUtils.checkbox(this.getDomId("showtitle"),[],this.showTitle)+" Title  "+
HtmlUtils.checkbox(this.getDomId("showdetails"),[],this.showDetails)+" Details "+
"&nbsp;&nbsp;&nbsp;"+
HtmlUtils.onClick(get+".askSetTitle();","Set Title");menu+=HtmlUtils.formTable()+HtmlUtils.formEntry("Show:",tmp)+"</table>";return menu;},isLayoutHorizontal:function(){return this.getProperty("orientation","horizontal")=="horizontal";},loadInitialData:function(){if(!this.needsData()||this.properties.data==null){return;}
...
this.properties.data.derived=this.derived;this.properties.data.loadData(this);},getData:function(){if(!this.hasData())return null;var dataList=this.dataCollection.getList();return dataList[0];},hasData:function(){if(this.dataCollection==null)return false;return this.dataCollection.hasData();},getCreatedInteractively:function(){return this.createdInteractively==true;},needsData:function(){return false;},askSetTitle:function(){var t=this.getTitle(false);var v=prompt("Title",t);if(v!=null){this.title=v;this.setProperty(ATTR_TITLE,v);this.setDisplayTitle(this.title);}},getShowDetails:function(){return this.getSelfProperty("showDetails",true);},setShowDetails:function(v){this.showDetails=v;if(this.showDetails){this.jq(ID_DETAILS).show();}else{this.jq(ID_DETAILS).hide();}},setShowTitle:function(v){this.showTitle=v;if(this.showTitle){this.jq(ID_TITLE).show();}else{this.jq(ID_TITLE).hide();}},setDisplayProperty:function(key,value){this.setProperty(key,value);$("#"+this.getDomId(key)).val(value);},deltaColumn:function(delta){var column=parseInt(this.getProperty("column",0));column+=delta;if(column<0)column=0;this.setDisplayProperty("column",column);this.getLayoutManager().layoutChanged(this);this.jq("col").val(column);},deltaRow:function(delta){var row=parseInt(this.getProperty("row",0));if(isNaN(row))row=0;row+=delta;if(row<0)row=0;this.setDisplayProperty("row",row);this.getLayoutManager().layoutChanged(this);this.jq("row").val(row);},moveDisplayRight:function(){if(this.getLayoutManager().isLayoutColumns()){this.deltaColumn(1);}else{this.getLayoutManager().moveDisplayDown(this);}},moveDisplayLeft:function(){if(this.getLayoutManager().isLayoutColumns()){this.deltaColumn(-1);}else{this.getLayoutManager().moveDisplayUp(this);}},moveDisplayUp:function(){if(this.getLayoutManager().isLayoutRows()){this.deltaRow(-1);}else{this.getLayoutManager().moveDisplayUp(this);}},moveDisplayDown:function(){if(this.getLayoutManager().isLayoutRows()){this.deltaRow(1);}else{this.getLayoutManager().moveDisplayDown(this);}},moveDisplayTop:function(){this.getLayoutManager().moveDisplayTop(this);},getDialogContents:function(tabTitles,tabContents){this.getDisplayDialogContents(tabTitles,tabContents);},getDisplayDialogContents:function(tabTitles,tabContents){var get=this.getGet();var menuItems=[];this.getMenuItems(menuItems);var form="<form>";form+=this.getDisplayMenuSettings();for(var i=0;i<menuItems.length;i++){form+=HtmlUtils.div([ATTR_CLASS,"display-menu-item"],menuItems[i]);}
form+="</form>";tabTitles.push("Display");tabContents.push(form);},checkLayout:function(){},displayData:function(){},createUI:function(){var divid=this.getProperty(PROP_DIVID);if(divid!=null){var html=this.getHtml();$("#"+divid).html(html);}else{console.log("error: no div defined for display:"+this.getType());}},setDisplayReady:function(){var callUpdate=!this.displayReady;this.displayReady=true;if(callUpdate){this.updateUI();}},getDisplayReady:function(){return this.displayReady;},pageHasLoaded:function(){if(!this.displayReady){this.setDisplayReady(true);}},initDisplay:function(){this.createUI();if(this.getProperty("doAnimation",false)){this.getAnimation().makeControls();}
this.checkSearchBar();this.updateUI();},getFilterId:function(id){return this.getDomId("filterby_"+id);},checkSearchBar:function(){let _this=this;var pointData=this.getData();if(pointData==null)return;this.filterFields=[];this.colorByFields=this.getFieldsByIds(null,this.getProperty("colorByFields","",true));this.sizeByFields=this.getFieldsByIds(null,this.getProperty("sizeByFields","",true));var filterBy=this.getProperty("filterFields","",true).split(",");var fields=pointData.getRecordFields();var records=pointData.getRecords();var dateMin=null;var dateMax=null;var dates=[];records.map(record=>{if(dateMin==null){dateMin=record.getDate();dateMax=record.getDate();}else{var date=record.getDate();if(date){dates.push(date);if(date.getTime()<dateMin.getTime())
dateMin=date;if(date.getTime()>dateMax.getTime())
dateMax=date;}}});if(dateMax){var animation=this.getAnimation();animation.init(dateMin,dateMax,records);if(!this.minDateObj){this.minDateObj=animation.begin;this.maxDateObj=animation.end;}}
var header2="";var searchBar="";if(this.getProperty("legendFields")||this.getProperty("showFieldLegend",false)){var colors=this.getColorList();var fields=this.getFieldsByIds(null,this.getProperty("legendFields",this.getProperty("fields",this.getProperty("sumFields"))));var html="";var colorCnt=0;fields.map((f)=>{if(colorCnt>=colors.length)colorCnt=0;var color=colors[colorCnt];html+=HtmlUtils.div(["style","display: inline-block;width:8px;height:8px;background:"+color+";"])+" "+f.getLabel()+"&nbsp;&nbsp; ";colorCnt++;});header2+=html;}
if(this.getProperty("showChartFieldsMenu")){var chartFields=pointData.getChartableFields();if(chartFields.length){var enums=[];chartFields.map(field=>{enums.push([field.getId(),field.getLabel()]);});header2+=HtmlUtils.span(["class","display-filterby"],HtmlUtils.span(["class","display-filterby-label"],"Display: ")+HtmlUtils.select("",["style","","id",this.getDomId("chartfields")],enums,this.getProperty("fields","")))+"&nbsp;";}}
if(this.colorByFields.length>0){var enums=[];this.colorByFields.map(field=>{if(field.isFieldGeo())return;enums.push([field.getId(),field.getLabel()]);});header2+=HtmlUtils.span(["class","display-filterby"],HtmlUtils.span(["class","display-filterby-label"],"Color by: ")+HtmlUtils.select("",["style","","id",this.getDomId("colorbyselect")],enums,this.getProperty("colorBy","")))+"&nbsp;";}
if(this.sizeByFields.length>0){var enums=[];this.sizeByFields.map(field=>{enums.push([field.getId(),field.getLabel()]);});header2+=HtmlUtils.span(["class","display-filterby"],HtmlUtils.span(["class","display-filterby-label"],"Size by: ")+HtmlUtils.select("",["style","","id",this.getDomId("sizebyselect")],enums,this.getProperty("sizeBy","")))+"&nbsp;";}
if(filterBy.length>0){var searchBar="";var bottom="";var dateIds=[];var hideFilterWidget=this.getProperty("hideFilterWidget",false,true);var widgetStyle="";if(hideFilterWidget)
widgetStyle="display:none;";var fieldMap={};for(var i=0;i<filterBy.length;i++){var filterField=this.getFieldById(fields,filterBy[i]);if(!filterField)continue;fieldMap[filterField.getId()]={field:filterField,values:[],};if(!filterField)continue;this.filterFields.push(filterField);var widget;var widgetId=this.getFilterId(filterField.getId());if(filterField.getType()=="enumeration"){var dfltValue=this.getProperty(filterField.getId()+".filterValue",FILTER_ALL);var filterValues=this.getProperty(filterField.getId()+".filterValues");var enums=null;if(filterValues){var toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}
enums=[];toks.map(tok=>{var tmp=tok.split(":");if(tmp.length>1){tok=[tmp[0],tmp[1]];}else if(tok==FILTER_ALL){tok=[tmp[0],"All"];}
enums.push({value:tok});})}
var includeAll=this.getProperty(filterField.getId()+".includeAll",true);if(enums==null){var allName=this.getProperty(filterField.getId()+".allName","All");enums=[];if(includeAll){enums.push({value:[FILTER_ALL,allName]});}
var seen={};var dflt=filterField.getEnumeratedValues();if(dflt){for(var v in dflt){seen[v]=true;enums.push({value:[v,dflt[v]]});}}
var enumValues=[];var imageField=this.getFieldOfType(null,"image");var valuesAreNumbers=true;records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(!seen[value]){seen[value]=true;var obj={};if(imageField)
obj.image=this.getDataValues(record)[imageField.getIndex()];if((+value+"")!=value)valuesAreNumbers=false;var label=value;if(label.length>20){label=label.substring(0,19)+"...";}
if(typeof value=="string")
value=value.replace(/\'/g,"&apos;");var tuple=[value,label];obj.value=tuple;enumValues.push(obj);}});enumValues.sort((a,b)=>{a=a.value;b=b.value;if(valuesAreNumbers){return+a-+b;}
return(""+a[1]).localeCompare(""+b[1]);});for(var j=0;j<enumValues.length;j++){var v=enumValues[j];enums.push(v);}}
var attrs=["style",widgetStyle,"id",widgetId,"fieldId",filterField.getId()];if(this.getProperty(filterField.getId()+".filterMultiple",false)){attrs.push("multiple");attrs.push("");attrs.push("size");attrs.push(this.getProperty(filterField.getId()+".filterMultipleSize","3"));dfltValue=dfltValue.split(",");}
var displayType=this.getProperty(filterField.getId()+".filterDisplay","menu");if(displayType!="menu"){if(!includeAll&&dfltValue==FILTER_ALL)dfltValue=enums[0].value;var buttons="";var colorMap=Utils.parseMap(this.getProperty(filterField.getId()+".filterColorByMap"));var useImage=displayType=="image";var imageAttrs=[];var imageMap=Utils.getNameValue(this.getProperty(filterField.getId()+".filterImages"));if(useImage){var w=this.getProperty(filterField.getId()+".filterImageWidth");var h=this.getProperty(filterField.getId()+".filterImageHeight");if(h){imageAttrs.push("height");imageAttrs.push(h);}
...
for(var j=0;j<enums.length;j++){var extra="";var v=enums[j].value;var color=colorMap?colorMap[v]:null;var label;if(Array.isArray(v)){label=v[1];v=v[0];}else{label=v;}
var style=this.getProperty(filterField.getId()+".filterItemStyle","");if(color){style+=" background-color:"+color+"; ";}
var clazz=" display-filterby-item display-filterby-item-"+displayType+" ";if(v==dfltValue){clazz+=" display-filterby-item-"+displayType+"-selected ";}else{}
if(v==FILTER_ALL){extra=" display-filterby-item-all ";}
...
buttons+="\n";}
if(useImage&&this.getProperty(filterField.getId()+".filterShowButtonsLabel")){buttons+=HtmlUtils.div(["class","display-filterby-item-label","id",this.getDomId("filterby_"+filterField.getId()+"_label")],"&nbsp;");}
bottom+=HtmlUtils.div(["data-value",dfltValue,"class","display-filterby-items","id",widgetId,"isButton","true","fieldId",filterField.getId()],buttons);continue;}else if(this.getProperty(filterField.getId()+".filterCheckbox")){attrs.push("isCheckbox");attrs.push(true);var tmp=[];enums.map(e=>tmp.push(e.value));var checked=tmp.includes(dfltValue);if(tmp.length>0){attrs.push("onValue");attrs.push(tmp[0]);}
if(tmp.length>1){attrs.push("offValue");attrs.push(tmp[1]);}
widget=HtmlUtils.checkbox("",attrs,checked);}else{var tmp=[];enums.map(e=>tmp.push(e.value));widget=HtmlUtils.select("",attrs,tmp,dfltValue);}}else if(filterField.isNumeric()){var min=0;var max=0;var cnt=0;records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(isNaN(value))return;if(cnt==0){min=value;max=value;}
else{min=Math.min(min,value);max=Math.max(max,value);}
cnt++;});var dfltValueMin=this.getProperty(filterField.getId()+".filterValueMin",min);var dfltValueMax=this.getProperty(filterField.getId()+".filterValueMax",max);widget=HtmlUtils.input("",dfltValueMin,["data-min",min,"class","display-filter-range display-filter-input","style",widgetStyle,"id",widgetId+"_min","size",5,"fieldId",filterField.getId()]);widget+=" - ";widget+=HtmlUtils.input("",dfltValueMax,["data-max",max,"class","display-filter-range display-filter-input","style",widgetStyle,"id",widgetId+"_max","size",5,"fieldId",filterField.getId()]);}else if(filterField.getType()=="date"){widget=HtmlUtils.datePicker("","",["style",widgetStyle,"id",widgetId+"_date1","fieldId",filterField.getId()])+" - "+
HtmlUtils.datePicker("","",["style",widgetStyle,"id",widgetId+"_date2","fieldId",filterField.getId()]);dateIds.push(widgetId+"_date1");dateIds.push(widgetId+"_date2");}else{var dfltValue=this.getProperty(filterField.getId()+".filterValue","");var attrs=["style",widgetStyle,"id",widgetId,"fieldId",filterField.getId(),"class","display-filter-input"];var placeholder=this.getProperty(filterField.getId()+".filterPlaceholder");if(placeholder){attrs.push("placeholder");attrs.push(placeholder);}
widget=HtmlUtils.input("",dfltValue,attrs);var values=fieldMap[filterField.getId()].values;var seen={};records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(!seen[value]){seen[value]=true;values.push(value);}});}
var label=this.getProperty(filterField.getId()+".filterLabel",filterField.getLabel());if(!hideFilterWidget){var tt=label;if(label.length>50)label=label.substring(0,49)+"...";if(!this.getProperty(filterField.getId()+".showFilterLabel",true)){label="";}
else
label=label+": ";widget=HtmlUtils.div(["style","display:inline-block;"],HtmlUtils.span(["class","display-filterby-label","title",tt],label)+widget);}
searchBar+=widget+(hideFilterWidget?"":"&nbsp;&nbsp;");}
var style=(hideFilterWidget?"display:none;":"")+this.getProperty("filterByStyle","");header2+=HtmlUtils.span(["class","display-filterby","style",style,"id",this.getDomId(ID_FILTERBAR)],searchBar+bottom);}
this.jq(ID_HEADER2).html(header2);var theDisplay=this;if(filterBy.length>0){if(!hideFilterWidget){for(var i=0;i<dateIds.length;i++){HtmlUtils.datePickerInit(dateIds[i]);}}
var inputFunc=function(input,input2,value){var id=input.attr("id");if(!input2){if(id.endsWith("_min")){input2=$("#"+id.replace(/_min$/,"_max"));}else if(id.endsWith("_max")){var tmp=input;input=$("#"+id.replace(/_max$/,"_min"));input2=tmp;}}
...
_this.propagateEvent("handleEventPropertyChanged",args);_this.settingFilterValue=false;};this.jq(ID_FILTERBAR).find(".display-filterby-items").each(function(){let parent=$(this);$(this).find(".display-filterby-item").click(function(event){var isAll=$(this).hasClass("display-filterby-item-all");var selectClazz="display-filterby-item-"+displayType+"-selected"
var wasSelected=$(this).hasClass(selectClazz);var fieldId=$(this).attr("fieldId");var multiples=_this.getProperty(fieldId+".filterMultiple",false);if(!event.metaKey||isAll||!multiples){parent.find(".display-filterby-item").removeClass(selectClazz);}else{parent.find(".display-filterby-item-all").removeClass(selectClazz);}
if(wasSelected&&event.metaKey){$(this).removeClass(selectClazz);}else{$(this).addClass(selectClazz);}
var values=[];parent.find("."+selectClazz).each(function(){values.push($(this).attr("data-value").replace(/,/g,"_comma_"));});if(values.length==0){parent.find(".display-filterby-item-all").addClass(selectClazz);values.push(FILTER_ALL);}
...
if(items.length>0){var html="";var itemCnt=0;items.map(item=>{var match=item[0];item=item[1];if(item.length>50)return;var label=item.replace(regexp,"<span style='background:"+HIGHLIGHT_COLOR+";'>"+match+"</span>");item=item.replace(/\'/g,"\'");html+=HtmlUtils.div(["class","display-filterby-popup-item","item",item],label)+"\n";itemCnt++;});if(itemCnt>0){popupObject=getTooltip();popupObject.html(HtmlUtils.div(["class","ramadda-popup-inner ramadda-snippet-popup"],html));popupObject.show();popupObject.position({of:$(this),my:"left top",at:"left bottom",});$(".display-filterby-popup-item").click(function(){hidePopupObject();input.val($(this).attr("item"));inputFunc(input);});}}});this.jq(ID_FILTERBAR).find(".display-filter-range").mousedown(function(){var id=$(this).attr("id");id=id.replace(/_min$/,"").replace(/_max$/,"");var min=$("#"+id+"_min");var max=$("#"+id+"_max");range={min:parseFloat(min.attr("data-min")),max:parseFloat(max.attr("data-max"))};var minValue=parseFloat(min.val());var maxValue=parseFloat(max.val());var html=HtmlUtils.div(["id","filterby-range","style","width:200px;"],"");var popup=getTooltip();popup.html(html);popup.show();popup.position({of:min,my:"left top",at:"left bottom+2",collision:"fit fit"});if(isNaN(minValue))minValue=range.min;if(isNaN(maxValue))maxValue=range.max;var step=1;if(parseInt(range.max)!=range.max||parseInt(range.min)!=range.min)
step=(range.max-range.min)/100000;$("#filterby-range").slider({range:true,min:range.min,max:range.max,step:step,values:[minValue,maxValue],slide:function(event,ui){min.val(ui.values[0]);max.val(ui.values[1]);min.attr("data-value",min.val());max.attr("data-value",max.val());},stop:function(){var popup=getTooltip();popup.hide();theDisplay.checkFilterField(max);inputFunc(min,max);}});});this.jq("chartfields").change(function(){_this.setProperty("fields",$(this).val());_this.updateUI();});this.jq("colorbyselect").change(function(){_this.colorByFieldChanged($(this).val());});this.jq("sizebyselect").change(function(){_this.sizeByFieldChanged($(this).val());});dateIds.map(id=>{$("#"+id).change(function(){inputFunc($(this));});});this.jq(ID_FILTERBAR).find("input").keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){inputFunc($(this));}});this.jq(ID_FILTERBAR).find("input:radio,select").change(function(){inputFunc($(this));});this.jq(ID_FILTERBAR).find("input:checkbox").change(function(){inputFunc($(this));});}},checkFilterField:function(f){var min=f.attr("data-min");var max=f.attr("data-max");var value=f.val();if(Utils.isDefined(min)){if(value!=min){f.css("background",HIGHLIGHT_COLOR);}else{f.css("background","white");}}else if(Utils.isDefined(max)){if(value!=max){f.css("background",HIGHLIGHT_COLOR);}else{f.css("background","white");}}},colorByFieldChanged:function(field){this.updateUI();},sizeByFieldChanged:function(field){},dataFilterChanged:function(){this.updateUI();},updateUI:function(){},makeTooltips:function(selector,records,callback){var tooltip=this.getProperty("tooltip");if(!tooltip)return;let _this=this;selector.tooltip({content:function(){var record=records[parseFloat($(this).attr('recordIndex'))];if(callback)callback(true,record);_this.getDisplayManager().notifyEvent("handleEventRecordHighlight",_this,{highlight:true,record:record});return _this.getRecordHtml(record,null,tooltip);},close:function(event,ui){var record=records[parseFloat($(this).attr('recordIndex'))];if(callback)callback(true,record);_this.getDisplayManager().notifyEvent("handleEventRecordHighlight",_this,{highlight:false,record:record});},position:{my:_this.getProperty("tooltipPositionMy","left top"),at:_this.getProperty("tooltipPositionAt","left bottom+2"),collision:_this.getProperty("tooltipCollision","none none")},show:{delay:parseFloat(_this.getProperty("tooltipDelay",1000)),duration:parseFloat(_this.getProperty("tooltipDuration",500)),},classes:{"ui-tooltip":_this.getProperty("tooltipClass","display-tooltip")}});},makePopups:function(selector,records,callback,popupTemplate){if(!popupTemplate)
...
popupTemplate=this.getProperty("popupTemplate");if(!popupTemplate)return;let _this=this;hidePopupObject();var html=_this.getRecordHtml(record,null,popupTemplate);html=HtmlUtils.div(["class","display-popup "+_this.getProperty("popupClass",""),"style",_this.getProperty("popupStyle","")],html);popupObject=getTooltip();popupObject.html(html);popupObject.show();popupObject.position({of:element,my:_this.getProperty("popupPositionMy","left top"),at:_this.getProperty("popupPositionAt","left bottom+2"),collision:_this.getProperty("popupCollision","none none")});},animationStart:function(animation){},animationApply:function(animation,skipUpdateUI){this.minDateObj=animation.begin;this.maxDateObj=animation.end;if(!skipUpdateUI){this.haveCalledUpdateUI=false;this.dataFilterChanged();}
this.propagateEvent("handleEventPropertyChanged",{property:"dateRange",minDate:animation.begin,maxDate:animation.end});},getDoBs:function(){if(!(typeof this.dobs==='undefined')){return dobs;}
if(this.displayParent){return this.displayParent.getDoBs();}
return false;},cnt:0,getHtml:function(){var dobs=this.getDoBs();var html="";html+=HtmlUtils.div([ATTR_CLASS,"ramadda-popup",ATTR_ID,this.getDomId(ID_MENU_OUTER)],"");var width=this.getWidth();if(dobs){html+=HtmlUtils.openDiv(["class","minitron"]);}
var style=this.getProperty("displayStyle","");if(width>0){style+="width:"+width+"px;"}
html+=HtmlUtils.openDiv(["class","display-contents","style",style]);var get=this.getGet();var button="";if(this.getShowMenu()){button=HtmlUtils.onClick(get+".showDialog();",HtmlUtils.image(ramaddaBaseUrl+"/icons/downdart.png",[ATTR_CLASS,"display-dialog-button",ATTR_ID,this.getDomId(ID_MENU_BUTTON)]));button+=" ";}
if(this.getProperty("showProgress",false)){button+=HtmlUtils.image(icon_progress,["style","xdisplay:none;","id",this.getDomId(ID_DISPLAY_PROGRESS)]);}
var title="";if(this.getShowTitle()){title=this.getTitle(false).trim();}
var left="";if(button!=""||title!=""){this.cnt++;var titleDiv=this.getTitleHtml(title);if(button==""){left=titleDiv;}else{left="<div class=display-header>"+button+"&nbsp;"+titleDiv+"</div>";}}
left=HtmlUtils.div(["id",this.getDomId(ID_TOP_LEFT)],left);var right=HtmlUtils.div(["id",this.getDomId(ID_TOP_RIGHT)],"");html+=HtmlUtils.div(["id",this.getDomId(ID_HEADER1),"class","display-header1"],"");html+=HtmlUtils.div(["id",this.getDomId(ID_HEADER2),"class","display-header2"],"");html+=HtmlUtils.leftRightTable(left,right,{valign:"bottom"});html+=HtmlUtils.div(["id",this.getDomId(ID_HEADER3),"class","display-header3"],"");var contents=this.getContentsDiv();html+=contents;html+=HtmlUtils.closeTag(TAG_DIV);if(dobs){html+=HtmlUtils.closeTag(TAG_DIV);}
html+=HtmlUtils.div([ATTR_CLASS,"",ATTR_ID,this.getDomId(ID_BOTTOM)],HtmlUtils.div(["id",this.getDomId(ID_COLORTABLE)])+
HtmlUtils.div(["id",this.getDomId(ID_LEGEND)]));return html;},makeToolbar:function(props){var toolbar="";var get=this.getGet();var addLabel=props.addLabel;var images=[];var calls=[];var labels=[];if(!this.getIsLayoutFixed()){calls.push("removeRamaddaDisplay('"+this.getId()+"')");images.push("fa-cut");labels.push("Delete display");}
...
for(var i=0;i<calls.length;i++){var inner=HtmlUtils.getIconImage(images[i],[ATTR_TITLE,labels[i],ATTR_CLASS,"display-dialog-header-icon"]);if(addLabel)inner+=" "+labels[i]+"<br>";toolbar+=HtmlUtils.onClick(calls[i],inner);}
return toolbar;},getWikiEditorTags:function(){return["label:Display Attributes","showMenu=\"true\"","showTitle=\"true\"","layoutHere=\"true\"","width=\"100%\"","height=\"400\"","title=\"\"","titleBackground=\"color\"","textColor=\"color\"","backgroundImage=\"\"","background=\"color\"","label:Filter Attributes","filterFields=\"\"","hideFilterWidget=true","acceptFilterEvent=false",'filterFieldsToPropagate=""',"&lt;field&gt;.filterValue=\"\"","&lt;field&gt;.filterValues=\"\"","&lt;field&gt;.filterMultiple=\"true\"","&lt;field&gt;.filterMultipleSize=\"5\"","&lt;field&gt;.filterByStyle=\"background:white;\"","&lt;field&gt;.includeAll=false","&lt;field&gt;.filterStartsWith=\"true\"","&lt;field&gt;.filterDisplay=\"menu|tab|button|image\"","binDate=\"day|month|year\"","label:Color Attributes","colorTable=\"\"","colors=\"color1,...,colorN\"","colorBy=\"\"","colorByFields=\"\"","colorTableAlpha=\"0.5\"","colorByMin=\"value\"","colorByMax=\"value\"","inlinelabel:Animation Attributes","doAnimation=true","acceptDateRangeChange=true","animationDateFormat=\"yyyy\"","animationWindow=\"decade|halfdecade|year|month|week|day|hour|minute\"","animationMode=\"sliding|frame|cumulative\"","animationShowSlider=\"false\"","animationWidgetShort=\"true\""];},makeDialog:function(text){var html="";html+=HtmlUtils.div([ATTR_ID,this.getDomId(ID_HEADER),ATTR_CLASS,"display-header"]);var closeImage=HtmlUtils.getIconImage(icon_close,[]);var close=HtmlUtils.onClick("$('#"+this.getDomId(ID_DIALOG)+"').hide();",closeImage);var right=close;var left="";var header=HtmlUtils.div([ATTR_CLASS,"display-dialog-header"],HtmlUtils.leftRight(left,right));if(!text){var tabTitles=[];var tabContents=[];this.getDialogContents(tabTitles,tabContents);tabTitles.push("Edit");tabContents.push(this.makeToolbar({addLabel:true}));var tabLinks="<ul>";var tabs="";for(var i=0;i<tabTitles.length;i++){var id=this.getDomId("tabs")+i;tabLinks+=HtmlUtils.tag("li",[],HtmlUtils.tag("a",["href","#"+id],tabTitles[i]));tabLinks+="\n";var contents=HtmlUtils.div([ATTR_CLASS,"display-dialog-tab"],tabContents[i]);tabs+=HtmlUtils.div(["id",id],contents);tabs+="\n";}
...
var changed=false;["column","row","width","height"].map(f=>{if(_this[f]!=_this.jq(f).val()&&(_this[f]||_this.jq(f).val().trim()!="")){changed=true;_this[f]=_this.jq(f).val();}});if(changed){_this.getLayoutManager().doLayout();}};["column","row","width","height"].map(f=>{this.jq(f).blur(updateFunc);this.jq(f).keypress(updateFunc);});this.jq("showtitle").change(function(){_this.setShowTitle(_this.jq("showtitle").is(':checked'));});this.jq("showdetails").change(function(){_this.setShowDetails(_this.jq("showdetails").is(':checked'));});this.jq(ID_DIALOG_TABS).tabs();},showDialog:function(text){if(!this.dialogElement){$(document.body).append(HtmlUtils.div([ATTR_CLASS,"display-dialog","id",this.getDomId(ID_DIALOG)]));this.dialogElement=this.jq(ID_DIALOG);}
this.dialogElement.html(this.makeDialog(text));this.popup(this.getDomId(ID_MENU_BUTTON),null,null,this.dialogElement);this.initDialog();},getWidthForStyle:function(dflt){var width=this.getProperty("width",-1);if(width==-1)return dflt;if(!width.endsWith("px")&&!width.endsWith("%"))
width=width+"px";return width;},getHeightForStyle:function(dflt){var height=this.getProperty("height",-1);if(height==-1)return dflt;if(height.match("^[0-9]+$"))
...
return style;},getContentsDiv:function(){var style=this.getContentsStyle();style+=this.getProperty("contentsStyle","");var image=this.getProperty("backgroundImage");if(image){image=HtmlUtils.getEntryImage(this.entryId,image);style+="background-attachment:auto;background-size:100% auto; background-image: url('"+image+"'); ";}
var background=this.getProperty("background");if(background)
style+="background: "+background+";";var topBottomStyle="";var width=this.getWidthForStyle();if(width){topBottomStyle+=" width:"+width+";";}
var top=HtmlUtils.div([ATTR_STYLE,topBottomStyle,ATTR_ID,this.getDomId(ID_DISPLAY_TOP)],"");var bottom=HtmlUtils.div([ATTR_STYLE,topBottomStyle,ATTR_ID,this.getDomId(ID_DISPLAY_BOTTOM)],"");var contents=top+"\n"+HtmlUtils.div([ATTR_CLASS,"display-contents-inner display-"+this.type,"style",style,ATTR_ID,this.getDomId(ID_DISPLAY_CONTENTS)],"")+"\n"+bottom;return contents;},copyDisplay:function(){var newOne={};$.extend(true,newOne,this);newOne.setId(newOne.getId()+this.getUniqueId("display"));addRamaddaDisplay(newOne);this.getDisplayManager().addDisplay(newOne);},removeDisplay:function(){this.getDisplayManager().removeDisplay(this);if(this.dialogElement)this.dialogElement.remove();},prepareToLayout:function(){this.getColumn();this.getWidth();this.getHeight();this.getName();this.getEventSource();},getColumn:function(){return this.getFormValue("column",0);},getRow:function(){return this.getFormValue("row",0);},getWidth:function(){return this.getFormValue("width",0);},getHeight:function(){return this.getFormValue("height",0);},getDisplayTitle:function(title){if(!title)title=this.title!=null?this.title:"";var text=title;var fields=this.lastSelectedFields;if(fields&&fields.length>0)
...
if(entry){pointData.entry=entry;this.addEntry(entry);}},clearProgress:function(){this.jq(ID_DISPLAY_PROGRESS).css("display","none");},startProgress:function(){this.jq(ID_DISPLAY_PROGRESS).css("display","inline-block");},pointDataLoadFailed:function(data){this.clearProgress();this.inError=true;errorMessage=this.getProperty("errorMessage",null);if(errorMessage!=null){this.setContents(errorMessage);return;}
var msg="";if(data&&data.errorcode&&data.errorcode=="warning"){msg=data.error;}else{msg="<b>Sorry, but an error has occurred:</b>";if(!data)data="No data returned from server";var error=data.error?data.error:data;error=error.replace(/<[^>]*>/g,"");var tmp="";var lines=error.split("\n");var seen={};for(var i=0;i<lines.length;i++){var line=lines[i].trim();if(line=="")continue;if(seen[line])continue;seen[line]=true;tmp+=line+"\n";}
error=tmp;error=HtmlUtils.tag("pre",["style","max-height:300px;overflow-y:auto;max-width:100%;overflow-x:auto;"],error);msg+=error;}
this.setContents(this.getMessage(msg));},clearCache:function(){},pointDataLoaded:function(pointData,url,reload){this.clearProgress();this.inError=false;this.clearCache();if(!reload){this.addData(pointData);this.checkSearchBar();}
if(url!=null){this.jsonUrl=url;}else{this.jsonUrl=null;}
if(!this.getDisplayReady()){return;}
...
this.updateUI(reload);if(!reload){this.lastPointData=pointData;this.propagateEvent("handleEventPointDataLoaded",pointData);}},getDateFormatter:function(){var date_formatter=null;if(this.googleLoaded()){var df=this.getProperty("dateFormat",null);if(df){var tz=0;this.timezone=this.getProperty("timezone");if(Utils.isDefined(this.timezone)){tz=parseFloat(this.timezone);}
...
lastDate=date;}
return this.hasDate;},dateInRange:function(date){if(date!=null){if(this.minDateObj!=null&&date.getTime()<this.minDateObj.getTime()){return false;}
if(this.maxDateObj!=null&&date.getTime()>this.maxDateObj.getTime()){return false;}
if(this.startDateObject!=null&&date.getTime()<this.startDateObject.getTime()){return false;}
if(this.endDateObject!=null&&date.getTime()>this.endDateObject.getTime()){return false;}}
...
for(a in fields)
console.log("   "+fields[a].getId());},makeIndexValue:function(indexField,value,offset){return value+offset;},getStandardData:function(fields,args){this.recordToIndex={};this.indexToRecord={};var pointData=this.getPointData();var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);if(fields==null){fields=pointData.getRecordFields();}
props={makeObject:true,includeIndex:true,includeIndexIfDate:false,groupByIndex:-1,raw:false,};if(args!=null){$.extend(props,args);}
let groupByIndex=props.groupByIndex;let groupByList=[];let groupByValues={};let groupByRecords=[];let groupByDate=this.getProperty("groupByDate");let groupByFill=this.getProperty("groupByFill");let groupByDateMap={};let groupByDates=[];var dataList=[];var fieldNames=[];if(this.getProperty("binDate")){var binType=this.getProperty("binType","total");var binCount=binType=="count";if(binCount){var f=[];fields.map((field)=>{f.push(new RecordField({index:0,id:field.getId(),label:this.getProperty("binDateLabel",this.getProperty("binCountLabel","Count")),type:"double",chartable:true}));});fields=f;}}
let seenDate=false;fields=fields.filter(f=>{if(f.isFieldDate()){if(seenDate&&f.isRecordDate())return null;seenDate=true;}
return f;});fields.map(f=>{});for(i=0;i<fields.length;i++){var field=fields[i];if(field.isFieldNumeric()&&field.isFieldDate()){}
var name=field.getLabel();if(field.getUnit()!=null){name+=" ("+field.getUnit()+")";}
name=name.replace(/!!/g," -- ")
fieldNames.push(name);}
if(props.makeObject){dataList.push({tuple:fieldNames,record:null});}else{dataList.push(fieldNames);}
groupByList.push("");groupByRecords.push(null);if(!this.minDateObj)
this.minDateObj=Utils.parseDate(this.minDate,false);if(!this.minDateObj)
this.maxDateObj=Utils.parseDate(this.maxDate,true,this.minDateObj);if(this.minDateObj==null&&this.maxDateObj!=null){this.minDateObj=Utils.parseDate(this.minDate,false,this.maxDateObj);}
var offset=0;if(Utils.isDefined(this.offset)){offset=parseFloat(this.offset);}
var nonNullRecords=0;var records=this.filterData();var allFields=pointData.getRecordFields();this.hasDate=this.getHasDate(records);var date_formatter=this.getDateFormatter();var rowCnt=-1;var indexField=this.getFieldById(null,this.getProperty("indexField"));for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];var date=record.getDate();if(!this.dateInRange(date)){continue;}
...
if(this.filters!=null){if(!this.applyFilters(record,values)){console.log(" skipping due to filters");continue;}}
...
if(this.getProperty("groupBySort")){agg.sort(function(a,b){return a[0]>b[0]});}
let tmp=[];tmp.push(title);agg.map(t=>tmp.push(t));return tmp;}
return dataList;},googleLoaded:function(){if((typeof google==='undefined')||(typeof google.visualization==='undefined')||(typeof google.visualization.DateFormat==='undefined')){return false;}
return true;},initDateFormats:function(){if(!this.googleLoaded()){return false;}
...
var s=formatter.formatValue(date);date={v:date,f:s};return date;},applyFilters:function(record,values){for(var i=0;i<this.filters.length;i++){if(!this.filters[i].recordOk(this,record,values)){return false;}}
...
function DisplayGroup(argDisplayManager,argId,argProperties,type){var LAYOUT_TABLE="table";var LAYOUT_HTABLE="htable";var LAYOUT_TABS="tabs";var LAYOUT_COLUMNS="columns";var LAYOUT_ROWS="rows";if(!type)type="group";let SUPER=new RamaddaDisplay(argDisplayManager,argId,type,argProperties);RamaddaUtil.inherit(this,SUPER);RamaddaUtil.defineMembers(this,{layout:this.getProperty(PROP_LAYOUT_TYPE,LAYOUT_TABLE)});RamaddaUtil.defineMembers(this,{displays:[],layout:this.getProperty(PROP_LAYOUT_TYPE,LAYOUT_TABLE),columns:this.getProperty(PROP_LAYOUT_COLUMNS,1),isLayoutColumns:function(){return this.layout==LAYOUT_COLUMNS;},getDoBs:function(){return this.dobs;},getWikiText:function(){var attrs=["layoutType",this.layout,"layoutColumns",this.columns,"showMenu","false","divid","$entryid_maindiv"];var wiki="";wiki+="<div id=\"{{entryid}}_maindiv\"></div>\n\n";wiki+="{{group "+HtmlUtils.attrs(attrs)+"}}\n\n"
return wiki;},walkTree:function(func,data){for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(display.walkTree!=null){display.walkTree(func,data);}else{func.call(data,display);}}},collectEntries:function(entries){if(entries==null)entries=[];for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(display.collectEntries!=null){display.collectEntries(entries);}else{var displayEntries=display.getEntries();if(displayEntries!=null&&displayEntries.length>0){entries.push({source:display,entries:displayEntries});}}}
return entries;},isLayoutRows:function(){return this.layout==LAYOUT_ROWS;},getPosition:function(){for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(display.getPosition){return display.getPosition();}}},getDisplays:function(){return this.displays;},notifyEvent:function(func,source,data){var displays=this.getDisplays();for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(display==source){continue;}
if(!display.getProperty("accept."+func,true))continue;var eventSource=display.getEventSource();if(eventSource!=null&&eventSource.length>0){if(eventSource!=source.getId()&&eventSource!=source.getName()){continue;}}
display.notifyEvent(func,source,data);}},getDisplaysToLayout:function(){var result=[];for(var i=0;i<this.displays.length;i++){if(this.displays[i].getIsLayoutFixed()){continue;}
result.push(this.displays[i]);}
return result;},pageHasLoaded:function(display){this.doLayout();},addDisplay:function(display){this.displays.push(display);if(display.getIsLayoutFixed()){display.initDisplay();}else{if(Utils.getPageLoaded()){this.doLayout();}}},layoutChanged:function(display){this.doLayout();},removeDisplay:function(display){var index=this.displays.indexOf(display);if(index>=0){this.displays.splice(index,1);}
this.doLayout();},doLayout:function(){var html="";var colCnt=100;var displaysToLayout=this.getDisplaysToLayout();var displaysToPrepare=this.displays;displaysToPrepare.map(display=>{if(display.prepareToLayout!=null){display.prepareToLayout();}});var weightIdx=0;var weights=null;if(typeof this.weights!="undefined"){weights=this.weights.split(",");}
for(var i=0;i<displaysToLayout.length;i++){var divId=HtmlUtils.getUniqueId("divid_");var div=HtmlUtils.div(["class"," display-wrapper","id",divId],"");displaysToLayout[i].setProperty(PROP_DIVID,divId);displaysToLayout[i].layoutDiv=div;}
if(this.layout==LAYOUT_TABLE){if(displaysToLayout.length==1){html+=displaysToLayout[0].layoutDiv;}else{var weight=12/this.columns;var i=0;var map={};for(;i<displaysToLayout.length;i++){var d=displaysToLayout[i];if(Utils.isDefined(d.column)&&Utils.isDefined(d.row)&&d.columns>=0&&d.row>=0){var key=d.column+"_"+d.row;if(map[key]==null)map[key]=[];map[key].push(d);}}
i=0;for(;i<displaysToLayout.length;i++){colCnt++;if(colCnt>=this.columns){if(i>0){html+=HtmlUtils.closeTag(TAG_DIV);}
html+=HtmlUtils.openTag("div",["class","row"]);colCnt=0;}
var weightToUse=weight;if(weights!=null){if(weightIdx>=weights.length){weightIdx=0;}
weightToUse=weights[weightIdx];weightIdx++;}
html+=HtmlUtils.div(["class","col-md-"+weightToUse+" display-wrapper display-cell"],displaysToLayout[i].layoutDiv);html+="\n";}
if(i>0){html+=HtmlUtils.closeTag(TAG_DIV);}}}else if(this.layout==LAYOUT_HTABLE){if(displaysToLayout.length==1){html+=displaysToLayout[0].layoutDiv;}else{var percent=Math.round((100/this.columns))+"%";var i=0;html+="<table width=100%>\n";var colCnt=100;for(var i=0;i<displaysToLayout.length;i++){colCnt++;if(colCnt>=this.columns){if(i>0){html+=HtmlUtils.closeTag(TAG_TR);}
html+=HtmlUtils.openTag("tr",["valign","top"]);html+="\n";colCnt=0;}
html+=HtmlUtils.td(["width",percent],displaysToLayout[i].layoutDiv);html+="\n";}
if(i>0){html+=HtmlUtils.closeTag("tr");}}}else if(this.layout==LAYOUT_TABS){var tabId=HtmlUtils.getUniqueId("tabs_");html+=HtmlUtils.openTag(TAG_DIV,["id",tabId,"class","ui-tabs"]);html+=HtmlUtils.openTag(TAG_UL,[]);var hidden="";var cnt=0;for(var i=0;i<displaysToLayout.length;i++){var display=displaysToLayout[i];var label=display.getTitle(false);if(label.length>20){label=label.substring(0,19)+"...";}
html+=HtmlUtils.tag(TAG_LI,[],HtmlUtils.tag(TAG_A,["href","#"+tabId+"-"+cnt],label));hidden+=HtmlUtils.div(["id",tabId+"-"+cnt,"class","ui-tabs-hide"],display.layoutDiv);cnt++;}
html+=HtmlUtils.closeTag(TAG_UL);html+=hidden;html+=HtmlUtils.closeTag(TAG_DIV);}else if(this.layout==LAYOUT_ROWS){var rows=[];for(var i=0;i<displaysToLayout.length;i++){var display=displaysToLayout[i];var row=display.getRow();if((""+row).length==0)row=0;while(rows.length<=row){rows.push([]);}
rows[row].push(display.layoutDiv);}
for(var i=0;i<rows.length;i++){var cols=rows[i];var width=Math.round(100/cols.length)+"%";html+=HtmlUtils.openTag(TAG_TABLE,["border","0","width","100%","cellpadding","0","cellspacing","0"]);html+=HtmlUtils.openTag(TAG_TR,["valign","top"]);for(var col=0;col<cols.length;col++){var cell=cols[col];cell=HtmlUtils.div(["class","display-cell"],cell);html+=HtmlUtils.tag(TAG_TD,["width",width],cell);}
html+=HtmlUtils.closeTag(TAG_TR);html+=HtmlUtils.closeTag(TAG_TABLE);}}else if(this.layout==LAYOUT_COLUMNS){var cols=[];for(var i=0;i<displaysToLayout.length;i++){var display=displaysToLayout[i];var column=display.getColumn();if((""+column).length==0)column=0;while(cols.length<=column){cols.push([]);}
cols[column].push(display.layoutDiv);}
html+=HtmlUtils.openTag(TAG_DIV,["class","row"]);var width=Math.round(100/cols.length)+"%";var weight=12/cols.length;for(var i=0;i<cols.length;i++){var rows=cols[i];var contents="";for(var j=0;j<rows.length;j++){contents+=rows[j];}
var weightToUse=weight;if(weights!=null){if(weightIdx>=weights.length){weightIdx=0;}
weightToUse=weights[weightIdx];weightIdx++;}
html+=HtmlUtils.div(["class","col-md-"+weightToUse],contents);}
html+=HtmlUtils.closeTag(TAG_DIV);}else{html+="Unknown layout:"+this.layout;}
...
this.doLayout();},askMinZAxis:function(){var v=prompt("Minimum axis value","0");if(v!=null){v=parseFloat(v);for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(display.setMinZAxis){display.setMinZAxis(v);}}}},askMaxZAxis:function(){var v=prompt("Maximum axis value","0");if(v!=null){v=parseFloat(v);for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(display.setMaxZAxis){display.setMaxZAxis(v);}}}},askMinDate:function(){var d=this.minDate;if(!d)d="1950-0-0";this.minDate=prompt("Minimum date",d);if(this.minDate!=null){for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(display.setMinDate){display.setMinDate(this.minDate);}}}},askMaxDate:function(){var d=this.maxDate;if(!d)d="2020-0-0";this.maxDate=prompt("Maximum date",d);if(this.maxDate!=null){for(var i=0;i<this.displays.length;i++){var display=this.displays[i];if(display.setMaxDate){display.setMaxDate(this.maxDate);}}}},titlesOff:function(){for(var i=0;i<this.displays.length;i++){this.displays[i].setShowTitle(false);}},titlesOn:function(){for(var i=0;i<this.displays.length;i++){this.displays[i].setShowTitle(true);}},detailsOff:function(){for(var i=0;i<this.displays.length;i++){this.displays[i].setShowDetails(false);}
this.doLayout();},detailsOn:function(){for(var i=0;i<this.displays.length;i++){this.displays[i].setShowDetails(true);}
this.doLayout();},deleteAllDisplays:function(){this.displays=[];this.doLayout();},moveDisplayUp:function(display){var index=this.displays.indexOf(display);if(index<=0){return;}
this.displays.splice(index,1);this.displays.splice(index-1,0,display);this.doLayout();},moveDisplayDown:function(display){var index=this.displays.indexOf(display);if(index>=this.displays.length){return;}
this.displays.splice(index,1);this.displays.splice(index+1,0,display);this.doLayout();},moveDisplayTop:function(display){var index=this.displays.indexOf(display);if(index>=this.displays.length){return;}
this.displays.splice(index,1);this.displays.splice(0,0,display);this.doLayout();},});}
function DisplayAnimation(display){var ID_RUN="animrun";var ID_NEXT="animnext";var ID_PREV="animprev";var ID_BEGIN="animbegin";var ID_END="animend";var ID_SLIDER="slider";var ID_TICKS="ticks";var ID_SHOWALL="showall";var ID_ANIMATION_LABEL="animationlabel";this.display=display;$.extend(this,{running:false,inAnimation:false,begin:null,end:null,dateMin:null,dateMax:null,dateRange:0,dateFormat:display.getProperty("animationDateFormat","yyyyMMdd"),mode:display.getProperty("animationMode","cumulative"),startAtEnd:display.getProperty("animationStartAtEnd",false),speed:parseInt(display.getProperty("animationSpeed",250)),toggleAnimation:function(){this.running=!this.running;if(this.btnRun)
this.btnRun.html(HtmlUtils.getIconImage(this.running?"fa-stop":"fa-play"));if(this.running)
this.startAnimation();},getDomId:function(id){return this.display.getDomId(id);},jq:function(id){return this.display.jq(id);},init:function(dateMin,dateMax,records){let _this=this;this.dateMin=dateMin;this.dateMax=dateMax;this.begin=this.dateMin;this.end=this.dateMax;if(!this.dateMin)return;this.dates=[];var seen={};records.map(r=>{if(!seen[r.getDate()]){seen[r.getDate()]=true;this.dates.push(r.getDate());}});this.dates.sort(function(a,b){return a.getTime()-b.getTime();});this.frameIndex=0;if(this.startAtEnd){this.begin=this.dateMax;this.end=this.dateMax;if(this.mode=="frame"){this.frameIndex=this.dates.length-1;}}
if(this.mode=="frame"){this.end=this.begin;}
...
this.updateLabels();this.setSliderValues=function(v){if(this.mode!="frame"){this.begin=new Date(v[0]);this.end=new Date(v[1]);}else{var sliderDate=new Date(v[0]);var closest=this.dates[0];var dist=0;this.dates.map(d=>{if(Math.abs(d.getTime()-sliderDate.getTime())<Math.abs(closest.getTime()-sliderDate.getTime())){closest=d;}});this.begin=this.end=closest;}}
var sliderValues=_this.mode!="frame"?[_this.begin.getTime(),_this.end.getTime()]:[_this.begin.getTime()];this.jq(ID_SLIDER).slider({range:_this.mode!="frame",min:_this.dateMin.getTime(),max:_this.dateMax.getTime(),values:sliderValues,slide:function(event,ui){_this.stopAnimation();_this.setSliderValues(ui.values);_this.updateLabels();},stop:function(event,ui){_this.stopAnimation();_this.setSliderValues(ui.values);_this.applyAnimation(true);}});if(records&&this.display.getProperty("animationShowTicks",true)){var ticks="";var min=this.dateMin.getTime();var max=this.dateMax.getTime();var p=0;for(var i=0;i<records.length;i++){var record=records[i];var date=record.getDate().getTime();var perc=Math.round((date-min)/(max-min)*100);var tt=this.formatAnimationDate(record.getDate());ticks+=HtmlUtils.div(["id",this.display.getId()+"-"+record.getId(),"class","display-animation-tick","style","left:"+perc+"%;","title",tt,"recordIndex",i],"");}
this.jq(ID_TICKS).html(ticks);this.display.makeTooltips(this.jq(ID_TICKS).find(".display-animation-tick"),records,(open,record)=>{this.display.handleEventRecordHighlight(this,{highlight:open,record:record,skipAnimation:true});});}},handleEventRecordHighlight:function(source,args){var element=$("#"+this.display.getId()+"-"+args.record.getId());if(args.highlight){element.addClass("display-animation-tick-highlight");}else{element.removeClass("display-animation-tick-highlight");}},makeControls:function(){var buttons="";if(this.display.getProperty("animationShowButtons",true)){var short=display.getProperty("animationWidgetShort",false);if(!short)
...
buttons+=HtmlUtils.span(["id",this.getDomId(ID_ANIMATION_LABEL),"class","display-animation-label"]);buttons=HtmlUtils.div(["class","display-animation-buttons"],buttons);if(display.getProperty("animationShowSlider",true)){buttons+=HtmlUtils.div(["class","display-animation-slider","id",this.getDomId(ID_SLIDER)],HtmlUtils.div(["class","display-animation-ticks","id",this.getDomId(ID_TICKS)]));}
this.jq(ID_TOP_LEFT).append(HtmlUtils.div(["style",this.display.getProperty("animationStyle")],buttons));this.btnRun=this.jq(ID_RUN);this.btnPrev=this.jq(ID_PREV);this.btnNext=this.jq(ID_NEXT);this.btnBegin=this.jq(ID_BEGIN);this.btnEnd=this.jq(ID_END);this.btnShowAll=this.jq(ID_SHOWALL);this.label=this.jq(ID_ANIMATION_LABEL);this.btnRun.button().click(()=>{this.toggleAnimation();});this.btnBegin.button().click(()=>{this.begin=this.dateMin;if(this.mode=="sliding"){this.end=new Date(this.dateMin.getTime()+this.window);}else if(this.mode=="frame"){this.frameIndex=0;this.begin=this.end=this.deltaFrame(0);}else{this.end=new Date(this.dateMin.getTime()+this.window);}
this.stopAnimation();this.applyAnimation();});this.btnEnd.button().click(()=>{this.end=this.dateMax;if(this.mode=="sliding"){this.begin=new Date(this.dateMax.getTime()-this.window);}else if(this.mode=="frame"){this.frameIndex=this.dates.length+1;this.begin=this.end=this.deltaFrame(0);}else{this.end=this.dateMax;}
this.stopAnimation();this.applyAnimation();});this.btnPrev.button().click(()=>{if(this.mode=="sliding"){this.begin=new Date(this.begin.getTime()-this.window);if(this.begin.getTime()<this.dateMin.getTime())
this.begin=this.dateMin;this.end=new Date(this.begin.getTime()+this.window);}else if(this.mode=="frame"){this.begin=this.end=this.deltaFrame(-1);}else{this.end=new Date(this.end.getTime()-this.window);if(this.end.getTime()<=this.begin.getTime()){this.end=new Date(this.begin.getTime()+this.window);}}
...
this.btnRun.html(HtmlUtils.getIconImage("fa-play"));this.applyAnimation();});},doNext:function(){if(this.mode=="sliding"){this.begin=this.end;this.end=new Date(this.end.getTime()+this.window);if(this.end.getTime()>this.dateMax.getTime()){this.end=this.dateMax;this.begin=new Date(this.end.getTime()-this.window);this.inAnimation=false;this.stopAnimation();}}else if(this.mode=="frame"){this.begin=this.end=this.deltaFrame(1);}else{this.end=new Date(this.end.getTime()+this.window);if(this.end.getTime()>=this.dateMax.getTime()){this.end=this.dateMax;this.inAnimation=false;this.stopAnimation();}}
this.applyAnimation();},deltaFrame:function(delta){this.frameIndex+=delta;if(this.frameIndex>=this.dates.length)
this.frameIndex=this.dates.length-1;else if(this.frameIndex<0)
this.frameIndex=0;return this.dates[this.frameIndex];},startAnimation:function(){if(!this.dateMax)return;if(!this.inAnimation){this.inAnimation=true;this.label.html("");if(this.mode=="frame"){this.frameIndex=0;this.begin=this.end=this.deltaFrame(0);this.display.animationStart();return;}
...
this.btnRun.html(HtmlUtils.getIconImage("fa-play"));}},updateLabels:function(){if(this.label){if(this.mode=="frame"&&this.begin==this.end){this.label.html(this.formatAnimationDate(this.begin));}else{this.label.html(this.formatAnimationDate(this.begin)+" - "+this.formatAnimationDate(this.end));}}},formatAnimationDate:function(d){if(this.dateFormat=="yyyy"){return Utils.formatDateYYYY(d);}else if(this.dateFormat=="yyyyMMdd"){return Utils.formatDateYYYYMMDD(d);}else if(this.dateFormat=="monthdayyear"){return Utils.formatDateMonthDayYear(d);}else if(this.dateFormat=="mdy"){return Utils.formatDateMDY(d);}else if(this.dateFormat=="hhmm"){return Utils.formatDateHHMM(d);}else{return Utils.formatDate(d);}},});}
function RamaddaFieldsDisplay(displayManager,id,type,properties){let _this=this;this.TYPE="RamaddaFieldsDisplay";let SUPER=new RamaddaDisplay(displayManager,id,type,properties);RamaddaUtil.inherit(this,this.RamaddaDisplay=SUPER);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},getWikiEditorTags:function(){var t=SUPER.getWikiEditorTags();var myTags=["fields=\"\"",];myTags.map(tag=>t.push(tag));return t;},handleEventMapClick:function(source,args){if(!this.dataCollection)return;var pointData=this.dataCollection.getList();for(var i=0;i<pointData.length;i++){pointData[i].handleEventMapClick(this,source,args.lon,args.lat);}},initDisplay:function(){SUPER.initDisplay.call(this);if(this.needsData()){this.setContents(this.getLoadingMessage());}
this.updateUI();},updateUI:function(){this.addFieldsCheckboxes();},checkLayout:function(){var _this=this;var d=_this.jq(ID_DISPLAY_CONTENTS);if(this.lastWidth!=d.width()){_this.displayData();}
if(true)return;if(d.width()==0){var cb=function(){_this.checkWidth(cnt+1);};setTimeout(cb,5000);}else{_this.displayData();}},getWikiAttributes:function(attrs){SUPER.getWikiAttributes.call(this,attrs);if(this.lastSelectedFields){attrs.push("fields");var v="";for(var i=0;i<this.lastSelectedFields.length;i++){v+=this.lastSelectedFields[i].getId();v+=",";}
attrs.push(v);}},initDialog:function(){SUPER.initDialog.call(this);this.addFieldsCheckboxes();},getDialogContents:function(tabTitles,tabContents){var height="600";var html=HtmlUtils.div([ATTR_ID,this.getDomId(ID_FIELDS),"style","overflow-y: auto;    max-height:"+height+"px;"]," FIELDS ");tabTitles.push("Fields");tabContents.push(html);SUPER.getDialogContents.call(this,tabTitles,tabContents);},handleEventFieldsSelected:function(source,fields){if(fields.length>0&&(typeof fields[0]=="string")){var tmp=[];fields.map(f=>{f=this.getFieldById(null,f);if(f)tmp.push(f);});fields=tmp;}
...
function convertToPointData(array){var fields=[];var records=[];var header=array[0];var samples=array[1];for(var i=0;i<header.length;i++){let label=header[i];let id=label.toLowerCase().replace(/[ ., ]+/g,"_");let sample=samples[i];let tof=typeof sample;let type;if(tof=="string")
...
for(var i=1;i<array.length;i++){records.push(new PointRecord(NaN,NaN,NaN,null,array[i]));}
return new PointData("pointdata",fields,records,null,null);}
function PointData(name,recordFields,records,url,properties){RamaddaUtil.inherit(this,new BasePointData(name,properties));RamaddaUtil.defineMembers(this,{recordFields:recordFields,records:records,url:url,loadingCnt:0,equals:function(that){return this.url==that.url;},getIsLoading:function(){return this.loadingCnt>0;},handleEventMapClick:function(myDisplay,source,lon,lat){this.lon=lon;this.lat=lat;if(myDisplay.getDisplayManager().hasGeoMacro(this.url)){console.log("url:"+this.url);this.loadData(myDisplay,true);return true;}
...
var props={lat:this.lat,lon:this.lon,};var jsonUrl=display.displayManager.getJsonUrl(this.url,display,props);this.loadPointJson(jsonUrl,display,reload);},loadPointJson:function(url,display,reload){var pointData=this;this.startLoading();var _this=this;var obj=pointDataCache[url];if(obj==null){obj={pointData:null,pending:[]};pointDataCache[url]=obj;}
if(obj.pointData!=null){display.pointDataLoaded(obj.pointData,url,reload);return;}
obj.pending.push(display);if(obj.pending.length>1){return;}
var fail=function(jqxhr,textStatus,error){var err=textStatus+": "+error;console.log("JSON error:"+err);display.pointDataLoadFailed(err);pointData.stopLoading();}
var success=function(data){if(GuiUtils.isJsonError(data)){display.pointDataLoadFailed(data);return;}
var newData=makePointData(data,_this.derived,display);obj.pointData=pointData.initWith(newData);var tmp=obj.pending;obj.pending=[];for(var i=0;i<tmp.length;i++){tmp[i].pointDataLoaded(pointData,url,reload);}
pointData.stopLoading();}
console.log("load data:"+url);Utils.doFetch(url,success,fail,"text");}});this.setGroupField();}
...
return" ["+this.unit+"]";return"";},getLabel:function(){if(this.label==null||this.label.length==0)return this.id;return this.label;},setLabel:function(l){this.label=l;},isNumeric:function(){return this.type=="double"||this.type=="integer";},isString:function(){return this.type=="string"||this.type=="enumeration";},getType:function(){return this.type;},getMissing:function(){return this.missing;},setUnit:function(u){this.unit=u;},getUnit:function(){return this.unit;}});}
function PointRecord(lat,lon,elevation,time,data){this.isPointRecord=true;RamaddaUtil.defineMembers(this,{latitude:lat,longitude:lon,elevation:elevation,recordTime:time,data:data,id:HtmlUtils.getUniqueId(),clone:function(){var newRecord={};$.extend(newRecord,this);newRecord.data=[];this.data.map((v,idx)=>{newRecord.data[idx]=v;});return newRecord;},toString:function(){return"data:"+data;},getId:function(){return this.id;},getData:function(){return this.data;},allZeros:function(){var tuple=this.getData();var allZeros=false;var nums=0;var nonZero=0;for(var j=0;j<tuple.length;j++){if(typeof tuple[j]=="number"){nums++;if(!isNaN(tuple[j])&&tuple[j]!=0){nonZero++;break;}}}
if(nums>0&&nonZero==0){return true;}
return false;},getValue:function(index){return this.data[index];},setValue:function(index,value){this.data[index]=value;},push:function(v){this.data.push(v);},hasDate:function(){return this.getDate()!=null;},hasLocation:function(){return!isNaN(this.latitude);},hasElevation:function(){return!isNaN(this.elevation);},getLatitude:function(){return this.latitude;},getLongitude:function(){return this.longitude;},getTime:function(){return this.recordTime;},getElevation:function(){return this.elevation;},getDate:function(){return this.recordTime;}});}
function makePointData(json,derived,source){var fields=[];var latitudeIdx=-1;var longitudeIdx=-1;var elevationIdx=-1;var dateIdx=-1;var dateIndexes=[];var offsetFields=[];var lastField=null;for(var i=0;i<json.fields.length;i++){var field=json.fields[i];var recordField=new RecordField(field);if(recordField.isFieldNumeric()){if(source.getProperty){var offset1=source.getProperty(recordField.getId()+".offset1",source.getProperty("offset1"));var offset2=source.getProperty(recordField.getId()+".offset2",source.getProperty("offset2"));var scale=source.getProperty(recordField.getId()+".scale",source.getProperty("scale"));if(offset1||offset2||scale){var unit=source.getProperty(recordField.getId()+".unit",source.getProperty("unit"));if(unit){recordField.unit=unit;}
...
if(!derived){derived=[]}
if(derived){var index=lastField.getIndex()+1;for(var dIdx=0;dIdx<derived.length;dIdx++){var d=derived[dIdx];var label=d.label;if(!label)label=d.name;var recordField=new RecordField({type:"double",index:(index+dIdx),chartable:true,id:d.name,label:label,});recordField.derived=d;fields.push(recordField);}}
var pointRecords=[];var rows=[];for(var i=0;i<json.data.length;i++){var tuple=json.data[i];var values=tuple.values;var date=null;if((typeof tuple.date==='undefined')){if(dateIdx>=0){date=new Date(values[dateIdx]);}}else{if(tuple.date!=null&&tuple.date!=0){date=new Date(tuple.date);}}
if((typeof tuple.latitude==='undefined')){if(latitudeIdx>=0)
tuple.latitude=values[latitudeIdx];else
tuple.latitude=NaN;}
if((typeof tuple.longitude==='undefined')){if(longitudeIdx>=0)
tuple.longitude=values[longitudeIdx];else
tuple.longitude=NaN;}
if((typeof tuple.elevation==='undefined')){if(elevationIdx>=0)
tuple.elevation=values[elevationIdx];else
tuple.elevation=NaN;}
for(var j=0;j<dateIndexes.length;j++){values[dateIndexes[j]]=new Date(values[dateIndexes[j]]);}
var h=values[2];for(var col=0;col<values.length;col++){if(values[col]==null){values[col]=NaN;}}
...
rows.push(values);var record=new PointRecord(tuple.latitude,tuple.longitude,tuple.elevation,date,values);pointRecords.push(record);}
for(var dIdx=0;dIdx<derived.length;dIdx++){var d=derived[dIdx];if(!d.isColumn)continue;var f=d["function"];var funcParams=[];var params=d.columns.split(",");for(var i=0;i<params.length;i++){var index=-1;for(var fIdx=0;fIdx<fields.length;fIdx++){var f=fields[fIdx];if(f.getId()==params[i]||f.getLabel()==params[i]){index=f.getIndex();break;}}
if(index<0){console.log("Could not find column index for field: "+params[i]);continue;}
funcParams.push("c"+(i+1));}
var columnData=RecordUtil.slice(rows,index);var daFunk=new Function(funcParams,d["function"]);console.log("daFunk - "+daFunk);var newData=daFunk(columnData);console.log("got new:"+newData+" "+(typeof newData));if((typeof newData)=="number"){for(var rowIdx=0;rowIdx<pointRecords.length;rowIdx++){var record=pointRecords[rowIdx];record.push(newData);}}else if(Utils.isDefined(newData.length)){console.log("newData.length:"+newData.length+" records.length:"+pointRecords.length);for(var rowIdx=0;rowIdx<newData.length;rowIdx++){var record=pointRecords[rowIdx];if(!record){console.log("bad record: "+rowIdx);record.push(NaN);}else{var v=newData[rowIdx];if(d.decimals>=0){v=parseFloat(v.toFixed(d.decimals));}
record.push(v);}}}}
if(source!=null){for(var i=0;i<fields.length;i++){var field=fields[i];var prefix="field."+field.getId()+".";if(Utils.isDefined(source[prefix+"unit"])){field.setUnit(source[prefix+"unit"]);}
...
pointRecords.sort(function(a,b){if(a.getDate()&&b.getDate()){if(a.getDate().getTime()<b.getDate().getTime())return-1;if(a.getDate().getTime()>b.getDate().getTime())return 1;return 0;}});return new PointData(name,fields,pointRecords);}
function makeTestPointData(){var json={fields:[{index:0,id:"field1",label:"Field 1",type:"double",missing:"-999.0",unit:"m"},{index:1,id:"field2",label:"Field 2",type:"double",missing:"-999.0",unit:"c"},],data:[[-64.77,-64.06,45,null,[8.0,1000]],[-65.77,-64.06,45,null,[9.0,500]],[-65.77,-64.06,45,null,[10.0,250]],]};return makePointData(json);}
function RecordFilter(properties){if(properties==null)properties={};RamaddaUtil.defineMembers(this,{properties:properties,recordOk:function(display,record,values){return true;}});}
function MonthFilter(param){RamaddaUtil.inherit(this,new RecordFilter());RamaddaUtil.defineMembers(this,{months:param.split(","),recordOk:function(display,record,values){for(i in this.months){var month=this.months[i];var date=record.getDate();if(date==null)return false;if(date.getMonth==null){return false;}
...
return min;},}
var RecordUtil={getRanges:function(fields,records){var maxValues=[];var minValues=[];for(var i=0;i<fields.length;i++){maxValues.push(NaN);minValues.push(NaN);}
...
$("#"+this.getDomId(ID_TIME)).html(label);if(propagate){this.displayManager.propagateEventRecordSelection(this,data,{index:this.index});}},handleEventRecordSelection:function(source,args){var data=this.displayManager.getDefaultData();if(data==null)return;if(data!=args.data){return;}
if(!data)return;this.index=args.index;this.applyStep(false);},faster:function(){this.sleepTime=this.sleepTime/2;if(this.sleepTime==0)this.sleepTime=100;},slower:function(){this.sleepTime=this.sleepTime*1.5;},start:function(){if(this.running)return;this.running=true;this.timestamp++;$("#"+this.getDomId(ID_START)).attr("src",this.iconStop);this.tick();},stop:function(){if(!this.running)return;this.running=false;this.timestamp++;$("#"+this.getDomId(ID_START)).attr("src",this.iconStart);},initDisplay:function(){this.createUI();this.stop();var get=this.getGet();var html="";html+=HtmlUtils.onClick(get+".setIndex(0);",HtmlUtils.image(this.iconBegin,[ATTR_TITLE,"beginning",ATTR_CLASS,"display-animation-button","xwidth","32"]));html+=HtmlUtils.onClick(get+".deltaIndex(-1);",HtmlUtils.image(this.iconBack,[ATTR_TITLE,"back 1",ATTR_CLASS,"display-animation-button","xwidth","32"]));html+=HtmlUtils.onClick(get+".toggle();",HtmlUtils.image(this.iconStart,[ATTR_TITLE,"play/stop",ATTR_CLASS,"display-animation-button","xwidth","32",ATTR_ID,this.getDomId(ID_START)]));html+=HtmlUtils.onClick(get+".deltaIndex(1);",HtmlUtils.image(this.iconForward,[ATTR_TITLE,"forward 1",ATTR_CLASS,"display-animation-button","xwidth","32"]));html+=HtmlUtils.onClick(get+".setIndex();",HtmlUtils.image(this.iconEnd,[ATTR_TITLE,"end",ATTR_CLASS,"display-animation-button","xwidth","32"]));html+=HtmlUtils.onClick(get+".faster();",HtmlUtils.image(this.iconFaster,[ATTR_CLASS,"display-animation-button",ATTR_TITLE,"faster","xwidth","32"]));html+=HtmlUtils.onClick(get+".slower();",HtmlUtils.image(this.iconSlower,[ATTR_CLASS,"display-animation-button",ATTR_TITLE,"slower","xwidth","32"]));html+=HtmlUtils.div(["style","display:inline-block; min-height:24px; margin-left:10px;",ATTR_ID,this.getDomId(ID_TIME)],"&nbsp;");this.setDisplayTitle("Animation");this.setContents(html);},});}
...
var v=prompt("Maximum date",ex);if(v==null)return;this.setMaxDate(v);},setMaxDate:function(v){this.maxDate=v;this.displayData();},trendLineEnabled:function(){return false;},getDialogContents:function(tabTitles,tabContents){var height="600";var html=HtmlUtils.div([ATTR_ID,this.getDomId(ID_FIELDS),"style","overflow-y: auto;    max-height:"+height+"px;"]," FIELDS ");if(this.trendLineEnabled()){html+=HtmlUtils.div([ATTR_CLASS,"display-dialog-subheader"],"Other");html+=HtmlUtils.checkbox(this.getDomId(ID_TRENDS_CBX),[],this.getProperty("showTrendLines",false))+"  "+"Show trend line";html+=" ";html+=HtmlUtils.checkbox(this.getDomId(ID_PERCENT_CBX),[],this.showPercent)+"  "+"Show percent of displayed total"+"<br>";html+="<br>";}
...
this.setChartSelection(index);},getFieldsToSelect:function(pointData){return pointData.getRecordFields();},canDoGroupBy:function(){return false;},clearCache:function(){this.computedData=null;},googleChartCallbackPending:false,includeIndexInData:function(){return false;},getGroupBy:function(){return this.groupBy;},getIncludeIndexIfDate:function(){return false;},makeIndexValue:function(indexField,value,offset){if(indexField.isString()){value={v:offset,f:value};}
return value;},displayData:function(reload){var _this=this;if(!this.getDisplayReady()){return;}
if(this.inError){return;}
if(!haveGoogleChartsLoaded()){if(!this.googleChartCallbackPending){this.googleChartCallbackPending=true;var func=function(){_this.googleChartCallbackPending=false;_this.displayData();}
this.setContents(this.getLoadingMessage());setTimeout(func,500);}
return;}
if(this.inError){return;}
if(!this.hasData()){this.clearChart();this.setContents(this.getLoadingMessage());return;}
this.setContents(HtmlUtils.div([ATTR_CLASS,"display-message"],"Building display..."));this.allFields=this.dataCollection.getList()[0].getRecordFields();var pointData=this.dataCollection.getList()[0];var selectedFields=this.getSelectedFields();if(selectedFields.length==0&&this.lastSelectedFields!=null){selectedFields=this.lastSelectedFields;}
if(selectedFields==null||selectedFields.length==0){if(this.getChartType()==DISPLAY_TABLE||this.getChartType()==DISPLAY_TREEMAP){selectedFields=this.dataCollection.getList()[0].getNonGeoFields();}else{selectedFields=this.getSelectedFields();}}
...
props.includeIndexIfDate=this.getIncludeIndexIfDate();var dataHasIndex=props.includeIndex;var dataList=null;if(this["function"]&&dataList==null){var pointData=this.dataCollection.getList()[0];var allFields=pointData.getRecordFields();var records=pointData.getRecords();var indexField=this.indexField;var chartableFields=this.getFieldsToSelect(pointData);this.hasDate=this.getHasDate(records);var date_formatter=this.getDateFormatter();var setVars="";for(var i=0;i<chartableFields.length;i++){var field=chartableFields[i];setVars+="\tvar "+field.getId()+"=displayGetFunctionValue(args."+field.getId()+");\n";}
var code="function displayChartEval(args) {\n"+setVars+"\treturn  "+this["function"]+"\n}";eval(code);var newList=[];var fieldNames=null;var rowCnt=-1;var indexField=this.getFieldById(null,this.getProperty("indexField"));for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];var row=record.getData();var date=record.getDate();if(!this.dateInRange(date))continue;rowCnt++;var values=[];var indexName=null;if(indexField){var value=record.getValue(indexField.getIndex());if(indexField.isString()){value={v:offset,f:value};}
values.push(value);indexName=indexField.getLabel();}else{if(this.hasDate){values.push(this.getDateValue(date,date_formatter));indexName="Date";}else{values.push(rowIdx);indexName=this.getProperty("indexName","Index");}}
if(fieldNames==null){fieldNames=[indexName,this.functionName?this.functionName:"value"];newList.push(fieldNames);}
var args={};for(var j=0;j<chartableFields.length;j++){var field=chartableFields[j];var value=row[field.getIndex()];args[field.getId()]=value;}
var value=displayChartEval(args);values.push(value);newList.push(values);}
dataList=newList;}
if(dataList==null){dataList=this.getStandardData(fieldsToSelect,props);}
this.computedData=dataList;if(this.getProperty("rotateTable")&&dataList.length>0){let rotated=[];var header=this.getDataValues(dataList[0]);for(var colIdx=0;colIdx<header.length;colIdx++){rotated.push([]);}
for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);for(var colIdx=0;colIdx<row.length;colIdx++){var value=row[colIdx];if(typeof value=="object"&&value.f)value=value.f;if(rowIdx==0&&colIdx==0)value="";rotated[colIdx].push(value);}}
dataList=rotated;}
...
if(dataList.length==0){this.setContents(HtmlUtils.div([ATTR_CLASS,"display-message"],"No data available"));return;}
...
var container=_this.jq(ID_CHART);if(_this.jq(ID_CHART).is(':visible')){this.lastWidth=container.width();}else{this.lastWidth=-1;}
...
var justData=[];var tooltipFields=this.getFieldsByIds(null,this.getProperty("tooltipFields",""));var dataTable=new google.visualization.DataTable();var header=this.getDataValues(dataList[0]);var sample=this.getDataValues(dataList[1]);var fixedValueS=this.getProperty("fixedValue");var fixedValueN;if(fixedValueS)fixedValueN=parseFloat(fixedValueS);let fIdx=0;let forceStrings=this.getProperty("forceStrings",false);for(var j=0;j<header.length;j++){var field=null;if(j>0||!props.includeIndex){field=selectedFields[fIdx++];}
var value=sample[j];if(j==0&&props.includeIndex){if((typeof value)=="object"){if(typeof value.v=="number"){if(forceStrings)
dataTable.addColumn('string',header[j]);else
dataTable.addColumn('number',header[j]);}else{dataTable.addColumn('date',header[j]);}}else{dataTable.addColumn((typeof value),header[j]);}}else{if(j>0&&fixedValueS){dataTable.addColumn('number',this.getProperty("fixedValueLabel","Count"));}else{if(field.isString()){dataTable.addColumn('string',header[j]);}else if(field.isFieldDate()){dataTable.addColumn('date',header[j]);}else{dataTable.addColumn('number',header[j]);}}
dataTable.addColumn({type:'string',role:'style'});dataTable.addColumn({type:'string',role:'tooltip','p':{'html':true}});if(j>0&&fixedValueS){break;}}}
...
var annotation={label:label,description:desc};annotationsList.push(annotation);annotation.index=isDate?index.getTime():index;var legendLabel=desc;if(url!=null){legendLabel=HtmlUtils.href(url,legendLabel,["target","_annotation"]);}
...
var colors=this.getColorTable(true);var colorBy=this.getColorByInfo(records);var didColorBy=false;var tuples=[];for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var record=dataList[rowIdx];var row=this.getDataValues(record);var index=row[0];if(index.v)index=index.v;var theRecord=record.record;var color="";if(colorBy.index>=0){var value=theRecord.getData()[colorBy.index];hasColorByValue=true;colorByValue=value;didColorBy=true;color=colorBy.getColor(value,theRecord);}
...
var tooltip="";tooltip+=label;for(var j=0;j<row.length;j++){if(j>0)
...
value=""+value;value=value.replace(/ /g,"&nbsp;");tooltip+="<b>"+label+"</b>:&nbsp;"+value;}
var tt=this.getProperty("tooltip");if(tt){tt=this.getRecordHtml(theRecord,null,tt);tt=tt.replace("${default}",tooltip);tooltip=tt;}
tooltip="<div style='padding:8px;'>"+tooltip+"</div>";var newRow=[];for(var j=0;j<row.length;j++){var value=row[j];if(forceStrings){if(value.f)value=(value.f).toString().replace(/\n/g," ");}
if(j>0&&fixedValueS){newRow.push(fixedValueN);}else{newRow.push(value);}
if(j==0&&props.includeIndex){}else{newRow.push(color);newRow.push(tooltip);}
...
if(indexToAnnotation){var annotation=indexToAnnotation[rowIdx];if(annotation){newRow.push(annotation.label);newRow.push(annotation.description);}else{newRow.push(null);newRow.push(null);}}
justData.push(newRow);}
dataTable.addRows(justData);if(didColorBy){colorBy.displayColorTable();}
return dataTable;},makeChartOptions:function(dataList,props,selectedFields){var chartOptions={tooltip:{isHtml:true},};$.extend(chartOptions,{lineWidth:this.getProperty("lineWidth",1),colors:this.getColorList(),curveType:this.curveType,pointShape:this.getProperty("pointShape"),pointSize:this.getProperty("pointSize"),vAxis:{}});chartOptions.backgroundColor={};chartOptions.chartArea={};chartOptions.chartArea.backgroundColor={};chartOptions.legend={textStyle:{}};chartOptions.hAxis={gridlines:{},minorGridlines:{},textStyle:{},};chartOptions.vAxis={gridlines:{},minorGridlines:{},textStyle:{}};chartOptions.hAxis.titleTextStyle={};chartOptions.vAxis.titleTextStyle={};if(this.getProperty("dateFormat")){chartOptions.hAxis.format=this.getProperty("dateFormat");}
var lineColor=this.getProperty("lineColor");this.setPropertyOn(chartOptions.backgroundColor,"chart.fill","fill",null);this.setPropertyOn(chartOptions.backgroundColor,"chart.stroke","stroke",this.getProperty("chartArea.fill",""));this.setPropertyOn(chartOptions.backgroundColor,"chart.strokeWidth","strokeWidth",null);this.setPropertyOn(chartOptions.chartArea.backgroundColor,"chartArea.fill","fill",null);this.setPropertyOn(chartOptions.chartArea.backgroundColor,"chartArea.stroke","stroke",null);this.setPropertyOn(chartOptions.chartArea.backgroundColor,"chartArea.strokeWidth","strokeWidth",null);this.setPropertyOn(chartOptions.hAxis.gridlines,"hAxis.gridlines.color","color",this.getProperty("gridlines.color",lineColor));this.setPropertyOn(chartOptions.hAxis.minorGridlines,"hAxis.minorGridlines.color","color",this.getProperty("gridlines.color",lineColor));this.setPropertyOn(chartOptions.hAxis,"hAxis.baselineColor","baselineColor",this.getProperty("baselineColor",lineColor));this.setPropertyOn(chartOptions.vAxis.gridlines,"vAxis.gridlines.color","color",this.getProperty("gridlines.color",lineColor));this.setPropertyOn(chartOptions.vAxis.minorGridlines,"vAxis.minorGridlines.color","color",this.getProperty("gridlines.color",lineColor));this.setPropertyOn(chartOptions.vAxis,"vAxis.baselineColor","baselineColor",this.getProperty("baselineColor",lineColor));var textColor=this.getProperty("textColor","#000");this.setPropertyOn(chartOptions.hAxis.textStyle,"hAxis.text.color","color",this.getProperty("axis.text.color",textColor));this.setPropertyOn(chartOptions.vAxis.textStyle,"vAxis.text.color","color",this.getProperty("axis.text.color",textColor));chartOptions.vAxis.text=this.getProperty("vAxis.text",this.getProperty("vAxisText"));chartOptions.hAxis.slantedText=this.getProperty("hAxis.slantedText",this.getProperty("slantedText",false));this.setPropertyOn(chartOptions.hAxis.titleTextStyle,"hAxis.text.color","color",textColor);this.setPropertyOn(chartOptions.vAxis.titleTextStyle,"vAxis.text.color","color",textColor);this.setPropertyOn(chartOptions.legend.textStyle,"legend.text.color","color",textColor);if(this.getProperty("hAxis.ticks")||this.getProperty("hAxis.ticks")==""){chartOptions.hAxis.ticks=this.getProperty("hAxis.ticks").split(",").filter(v=>v!="");}
...
if(this.lineWidth){chartOptions.lineWidth=this.lineWidth;}
...
this.setContents(HtmlUtils.div(["id",this.getDomId(ID_CHARTS)]));return chartOptions;},getChartHeight:function(){return this.getProperty("height");},getChartWidth:function(){return this.getProperty("width");},getChartDiv:function(chartId){var divAttrs=[ATTR_ID,chartId];divAttrs.push("style");var style="";var width=this.getChartWidth();if(width){if(width>0)
style+="width:"+width+"px;";else if(width<0)
style+="width:"+(-width)+"%;";else
style+="width:"+width+";";}else{style+="width:"+"100%;";}
var height=this.getChartHeight();if(height){if(height>0)
style+="height:"+height+"px;";else if(height<0)
style+="height:"+(-height)+"%;";else
style+="height:"+height+";";}else{style+="height:"+"100%;";}
divAttrs.push(style);return HtmlUtils.div(divAttrs,"");},doMakeGoogleChart:function(dataList,props,chartDiv,selectedFields,chartOptions){throw new Error("doMakeGoogleChart undefined");},makeGoogleChart:function(dataList,props,selectedFields){if(typeof google=='undefined'){this.setContents("No google");return;}
...
this.charts=[];this.chartCount=-1;if(this.getProperty("multipleCharts",false)){var headerPosition=this.getProperty("multipleChartsHeaderPosition","bottom");let map={};let times=[];let tmp=[];dataList.map((v,idx)=>{if(idx>0)tmp.push(v)});tmp.sort(function(a,b){var record=a.record;var date1=record?record.getDate():a.date;record=b.record;var date2=record?record.getDate():b.date;return date1.getTime()-date2.getTime();});dataList=Utils.mergeLists([dataList[0]],tmp);dataList.map((v,idx)=>{if(idx==0)return;var record=v.record;var date=record?record.getDate():v.date;let list=null;list=map[date];if(!list){list=[];map[date]=list;times.push(date);}
list.push(v);})
this.jq(ID_CHARTS).html(HtmlUtils.div(["id",this.getDomId(ID_CHARTS_INNER),"style","text-align:center;"]));times.map((date,idx)=>{this.chartCount=idx;let tmpDataList=[];let list=map[date];tmpDataList.push(dataList[0]);tmpDataList=Utils.mergeLists(tmpDataList,list);var innerId=this.getDomId(ID_CHART)+"_"+this.chartCount;var header=HtmlUtils.div(["class","display-multi-header"],this.formatDate(date));var top=headerPosition=="top"?header:"";var bottom=headerPosition=="bottom"?header+"<br>":"";var div=HtmlUtils.div(["class","display-multi-div","style","display:inline-block;"],top+this.getChartDiv(innerId)+bottom);this.jq(ID_CHARTS_INNER).append(div);let chart=this.makeGoogleChartInner(tmpDataList,innerId,props,selectedFields);if(chart)this.charts.push(chart);});}else{this.jq(ID_CHARTS).append(this.getChartDiv(this.getDomId(ID_CHART)));let chart=this.makeGoogleChartInner(dataList,this.getDomId(ID_CHART),props,selectedFields);if(chart)this.charts.push(chart);}},makeGoogleChartInner:function(dataList,chartId,props,selectedFields){let chartDiv=document.getElementById(chartId);let chart=this.doMakeGoogleChart(dataList,props,chartDiv,selectedFields,this.chartOptions);if(chart==null)return null;var dataTable=this.makeDataTable(dataList,props,selectedFields);if(!dataTable){this.setContents(this.getMessage("No data available"));return null;}
if(this.getProperty("animation",false,true)){this.chartOptions.animation={startup:true,duration:parseFloat(this.getProperty("animationDuration",1000,true)),easing:this.getProperty("animationEasing","linear",true)};HtmlUtils.callWhenScrolled(this.getDomId(ID_CHART),()=>{if(!this.animationCalled){this.animationCalled=true;this.mapCharts(chart=>{chart.draw(dataTable,this.chartOptions);});}});}else{chart.draw(dataTable,this.chartOptions);}
...
function TableDisplay(displayManager,id,properties){let SUPER=new RamaddaTextChart(displayManager,id,DISPLAY_TABLE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{canDoGroupBy:function(){return true;},defaultSelectedToAll:function(){return true;},doMakeGoogleChart:function(dataList,props,chartDiv,selectedFields,chartOptions){chartOptions.height=null;if(this.chartHeight){chartOptions.height=this.chartHeight;}
...
chartOptions.allowHtml=true;if(dataList.length&&this.getDataValues(dataList[0]).length>4){chartOptions.cssClassNames={headerCell:'display-table-header-max'};}else{chartOptions.cssClassNames={headerCell:'display-table-header'};}
return new google.visualization.Table(chartDiv);},makeDataTable:function(dataList,props,selectedFields){var rows=this.makeDataArray(dataList);var data=[];for(var rowIdx=0;rowIdx<rows.length;rowIdx++){var row=rows[rowIdx];for(var colIdx=0;colIdx<row.length;colIdx++){if((typeof row[colIdx])=="string"){row[colIdx]=row[colIdx].replace(/\n/g,"<br>");if(row[colIdx].startsWith("http:")||row[colIdx].startsWith("https:")){row[colIdx]="<a href='"+row[colIdx]+"'>"+row[colIdx]+"</a>";}}}
data.push(row);}
return google.visualization.arrayToDataTable(data);}});}
function BubbleDisplay(displayManager,id,properties){let SUPER=new RamaddaTextChart(displayManager,id,DISPLAY_BUBBLE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),['label:Bubble Chart Attibutes','legendPosition="none|top|right|left|bottom"','hAxisFormat="none|decimal|scientific|percent|short|long"','vAxisFormat="none|decimal|scientific|percent|short|long"','hAxisTitle=""','vAxisTitle=""'])},makeDataTable:function(dataList,props,selectedFields){var tmp=[];var a=this.makeDataArray(dataList);tmp.push(a[0]);for(var i=1;i<a.length;i++){var tuple=a[i];var ok=true;for(j=1;j<tuple.length&&ok;j++){if(isNaN(tuple[j]))ok=false;}
if(ok)
tmp.push(tuple);}
...
$.extend(chartOptions.chartArea,{left:this.getProperty("chartLeft",this.chartDimensions.left),right:this.getProperty("chartRight",this.chartDimensions.right),top:this.getProperty("chartTop","10"),bottom:this.getProperty("chartBottom"),width:'98%',height:'90%'});chartOptions.sizeAxis={}
chartOptions.colorAxis={legend:{position:this.getProperty("legendPosition","in")}}
var colorTable=this.getColorTable(true);if(colorTable)
chartOptions.colorAxis.colors=colorTable;chartOptions.bubble={textStyle:{auraColor:"none"},stroke:"#666"};chartOptions.hAxis={};chartOptions.vAxis={};header=this.getDataValues(dataList[0]);chartOptions.hAxis.format=this.getProperty("hAxisFormat",null);chartOptions.vAxis.format=this.getProperty("vAxisFormat",null);chartOptions.hAxis.title=this.getProperty("hAxisTitle",header.length>1?header[1]:null);chartOptions.vAxis.title=this.getProperty("vAxisTitle",header.length>2?header[2]:null);return new google.visualization.BubbleChart(chartDiv);}});}
...
cnt++;var dttm=this.formatDate(this.getDataValues(dataList[i])[0],{options:options});dttm=dttm.replace(/ /g,"&nbsp;");var tooltip="<center><b>"+dttm+"</b></center>"+
"<b>"+header[1].replace(/ /g,"&nbsp;")+"</b>:&nbsp;"+this.formatNumber(value);tooltip=HtmlUtils.tag("div",["style","padding:5px;"],tooltip);list.push([this.getDataValues(dataList[i])[0],value,tooltip]);}
...
chartOptions.vAxis.logScale=true;chartOptions.vAxis.viewWindowMode=this.getProperty("viewWindowMode","maximized");chartOptions.vAxis.viewWindowMode=this.getProperty("viewWindowMode","maximized");if(dataList.length>0&&this.getDataValues(dataList[0]).length>1){if(!chartOptions.vAxis)chartOptions.vAxis={};if(!chartOptions.hAxis)chartOptions.hAxis={};if(this.getProperty("hAxisTitle")){chartOptions.hAxis.title=this.getProperty("hAxisTitle");}
if(this.getProperty("vAxisTitle")){chartOptions.vAxis.title=this.getProperty("vAxisTitle");}
...
break;}}
return f;}});addRamaddaDisplay(this);}
...
let bubbleTestData=[{cat:'library',name:'D3',value:30,icon:'https://naustud.io/tech-stack/img/d3.svg',desc:`D3.js(or just D3 for Data-Driven Documents)is a JavaScript library for
producing dynamic,interactive data visualizations in web browsers.It makes use of the widely implemented SVG,HTML5,and CSS standards.<br>This infographic you are viewing is made with D3.`},{cat:'library',name:'Raphaël',value:10,icon:'https://naustud.io/tech-stack/img/raphael.svg',desc:`Raphaël is a cross-browser JavaScript library that draws Vector graphics for web sites.It will use SVG for most browsers,but will use VML for older versions of Internet Explorer.`},{cat:'library',name:'Relay',value:70,icon:'https://naustud.io/tech-stack/img/relay.svg',desc:`A JavaScript framework for building data-driven React applications.It uses GraphQL as the query language to exchange data between app and server efficiently.Queries live next to the views that rely on them,so you can easily reason
about your app.Relay aggregates queries into efficient network requests to fetch only what you need.`},{cat:'library',name:'Three.js',value:40,icon:'https://naustud.io/tech-stack/img/threejs.png',desc:`Three.js allows the creation of GPU-accelerated 3D animations using
the JavaScript language as part of a website without relying on
proprietary browser plugins.This is possible thanks to the advent of WebGL.`},{cat:'library sub',name:'Lodash',value:30,icon:'https://naustud.io/tech-stack/img/lodash.svg',desc:`Lodash is a JavaScript library which provides<strong>utility functions</strong>for
common programming tasks using the functional programming paradigm.`},{cat:'library sub',name:'Moment JS',value:30,icon:'https://naustud.io/tech-stack/img/momentjs.png',desc:`Handy and resourceful JavaScript library to parse,validate,manipulate,and display dates and times.`},{cat:'library sub',name:'Numeral.js',value:20,icon:'Numeral.js',desc:`A javascript library for formatting and manipulating numbers.`},{cat:'library sub',name:'Redux',value:80,icon:'https://naustud.io/tech-stack/img/redux.svg',desc:`Redux is an open-source JavaScript library designed for managing
application state.It is primarily used together with React for building user interfaces.Redux is inspired by Facebook’s Flux and influenced by functional programming language Elm.`},{cat:'framework',name:'Angular 2.0',value:30,icon:'https://naustud.io/tech-stack/img/angular2.svg',desc:`Angular(commonly referred to as'Angular 2+'or'Angular 2')is a TypeScript-based
...
developer's workflow while building complex web applications.
	`
     ...
     icon: '',
			  ...
			  icon: 'https:desc:`Bootstrap is a free and open-source front-end web framework for designing websites
and web applications.It contains HTML-and CSS-based design templates for typography,forms,buttons,navigation and other interface components,as well as optional JavaScript extensions.`},{cat:'framework',name:'Ember JS',value:10,icon:'https://naustud.io/tech-stack/img/ember.png',desc:`Ember.js is an open-source JavaScript web framework,based on the Model–view–viewmodel
(MVVM)pattern.It allows developers to create scalable single-page web applications by
incorporating common idioms and best practices into the framework.`},{cat:'framework',name:'ExpressJS',value:30,icon:'https://naustud.io/tech-stack/img/expressjs.png',desc:`Express.js,or simply Express,is a JavaScript framework designed for building web applications and APIs.It is the de facto server framework for Node.js.`},{cat:'framework',name:'Hexo',value:50,icon:'https://naustud.io/tech-stack/img/hexo.png',desc:`A fast,simple&powerful blog-aware<strong>static website</strong>generator,powered by Node.js.`},{cat:'framework',name:'ReactJS',value:100,icon:'https://naustud.io/tech-stack/img/react.png',desc:`React(sometimes written React.js or ReactJS)is an open-source JavaScript framework maintained by Facebook for building user interfaces.React processes only user interface in applications and can be used in combination with other JavaScript libraries
or frameworks such as Redux,Flux,Backbone...`},{cat:'tooling',name:'Atom',value:10,icon:'https://naustud.io/tech-stack/img/atom.png',desc:`Atom is a free and open-source text and source code editor for macOS,Linux,and Windows with support
for plug-ins written in Node.js,and embedded Git Control,developed by GitHub.Atom is a desktop application built using web technologies.`},{cat:'tooling',name:'Google Chrome & Devtool',value:70,icon:'https://naustud.io/tech-stack/img/chrome-devtools.svg',desc:`<strong>Web development tools(devtool)</strong>allow web developers to test and debug their code.At Nau,we use the one come with Google Chrome to debug our apps.It is one the the most powerful
and sophisticated devtool available.`},{cat:'tooling',name:'Jenkins CI',value:30,icon:'https://naustud.io/tech-stack/img/jenkins.png',desc:`Jenkins is an open source automation server.Jenkins helps to automate the non-human part of
the whole software development process,with now common things like continuous integration,but by further empowering teams to implement the technical part of a Continuous Delivery.`},{cat:'tooling',name:'Sublime Text 3',value:100,icon:'https://naustud.io/tech-stack/img/sublimetext.png',desc:`Sublime Text 3 is a powerful and cross-platform source code editor.It is well-known for
introducing the concept of multi-cursor and lots of text editing command.Besides,its
plugin ecosystem is very rich which allows enhancing productivity to the fullest.`},{cat:'tooling',name:'Visual Studio Code',value:50,icon:'https://naustud.io/tech-stack/img/vscode.png',desc:`Visual Studio Code is a cross-platform source code editor developed by Microsoft.It includes support for debugging,embedded Git control,syntax highlighting,intelligent code completion,snippets,and code refactoring.Its extensions eco system is
growing quickly and it is becoming the best Front End editors out there.`},{cat:'tooling',name:'Performance Tooling',value:30,icon:'Performance;Tooling',desc:`At Nau,web performance is our top priority when development web sites and applications.We're practicing code optimization and Front End delivery optimization from day 1.
	...
	`
				    ...
				    icon: 'https:desc:`Yeoman is an open source,command-line interface set of tools mainly used to generate
structure and scaffolding for new projects,especially in JavaScript and Node.js.At Nau,we have developed a Yeoman generator that help quickly set up new projects aligned with
Nau's conventions and standards.
	...
	`
				    ...
				    icon: 'https:desc:`PostCSS is a software development tool that uses JavaScript-based plugins to automate routine CSS operations.<br>We use PostCSS mainly for auto-vendor-prefixing,but very soon we'll use it for NextCSS compilation.
	`
				}, {
	...
	`
				    ...
				    icon: 'https:desc:`The de-facto CMS system for website built with Node.js.It can be compared with
Wordpress of PHP language.`},{cat:'backend',name:'KoaJS',value:10,icon:'https://naustud.io/tech-stack/img/koajs.png',desc:`The advanced and improved version of ExpressJS,with leaner middlewares architecture
thanks to the avent of ES6 generators.`},{cat:'backend',name:'Loopback',value:30,icon:'https://naustud.io/tech-stack/img/loopback.svg',desc:`Powerful API-focused web framework built for Node.js.It feature easy to use configurations
and auto API documentation page.`},{cat:'backend',name:'Restify',value:20,icon:'https://naustud.io/tech-stack/img/restify.png',desc:`High performance API development framework,built for Node.js.It has some convenient wrapper
to automatically generate admin backoffice site and API documentation page.`},{cat:'backend',name:'MongoDB',value:70,icon:'https://naustud.io/tech-stack/img/mongodb.png',desc:`The de-facto Database solution for JavaScript and Node.js applications.It is a light weight,high performance NoSQL database and can be used for small and large websites.`},{cat:'backend',name:'NodeJS',value:100,icon:'https://naustud.io/tech-stack/img/nodejs.svg',desc:`Node.js is a cross-platform JavaScript runtime environment.Node.js allows creation of high performance and high concurrency websites with smaller footprint compared to
other server-side solution.Node.js ecosystem is growing very fast and is trusted by a lot of big companies who
are adopting it to enhance current products as well as for new ones.`},{cat:'platform',name:'Docker Platform',value:10,icon:'https://naustud.io/tech-stack/img/docker.svg',desc:`Docker is an open-source project that automates the deployment of applications inside software containers.At Nau,we're still learning this technology to later facilitate easy web app deployments.
	`
				}, {
				    cat: 'platform', name: 'MeteorJS', value: 80,
				    icon: 'https:desc:`MeteorJS is a free and open-source JavaScript web framework written using Node.js.Meteor allows for rapid prototyping and produces cross-platform(Android,iOS,Web)code.It integrates with MongoDB and uses the Distributed Data Protocol and a publish–subscribe
pattern to automatically propagate data changes to clients without requiring the developer
to write any synchronization code.`},{cat:'platform',name:'Phonegap',value:50,icon:'https://naustud.io/tech-stack/img/phonegap.png',desc:`A platform,library and tool for building hybrid mobile app.`},{cat:'platform',name:'Reaction Commerce',value:20,icon:'https://naustud.io/tech-stack/img/reactioncommerce.png',desc:`Reaction Commerce is the first open source,real-time platform to
combine the flexibility developers and designers want with the stability
and support businesses need.ReactionCommerce is based on MeteorJS platform.`},{cat:'platform',name:'ReactNative',value:10,icon:'https://naustud.io/tech-stack/img/reactnative.png',desc:`React Native lets you build mobile apps using only JavaScript.It uses the same design as React,letting us compose a rich
mobile UI from declarative components.`},{cat:'platform',name:'SquareSpace',value:30,icon:'https://naustud.io/tech-stack/img/squarespace.svg',desc:`Squarespace is a SaaS-based content management system-integrated ecommerce-aware website builder and blogging platform.At Nau,we have built a website for Squarespace using their low-level API which allowed fully customization
of the interface and other Front End functionalities.`},{cat:'language',name:'HTML5 & CSS3',value:100,icon:'https://naustud.io/tech-stack/img/html5-css3.png',desc:`The languages of the Web Front End.At Nau,they are in our blood and with them we can build
world-class websites with any kind of visual effects or designs requested.`},{cat:'language',name:'JavaScript',value:100,icon:'https://naustud.io/tech-stack/img/javascript.png',desc:`JavaScript is the heart of modern Web front end development and essential element of any Single Page
Applications.In Nau,we invest a good deal in training developers to have good control of this universal language
and now caplable of developing full stack websites with only JavaScript.`},{cat:'language',name:'CSS Next',value:10,icon:'https://naustud.io/tech-stack/img/cssnext.png',desc:`The CSS language specs of the future but with the help of PostCSS(like Babel for ES6),we can use CSS Next today.`},{cat:'language',name:'GraphQL',value:50,icon:'https://naustud.io/tech-stack/img/graphql.svg',desc:`GraphQL is a data query language developed by Facebook publicly released in 2015.It provides an alternative to REST and ad-hoc webservice architectures.In combination
with RelayJS,this combo help us reduce the time to develop web apps for weeks.`},{cat:'language',name:'LESS CSS',value:20,icon:'https://naustud.io/tech-stack/img/less.svg',desc:`A preprocessor language to be compiled to CSS.This language is not as popular nowadays and we
only use them when requested.`},{cat:'language',name:'SASS (SCSS flavor)',value:70,icon:'https://naustud.io/tech-stack/img/sass.png',desc:`This is our main CSS preprocessor language helping us lay structured foundation to CSS as well
as assisting on writing more convenient BEM anotations.`},{cat:'language',name:'TypeScript 2',value:30,icon:'https://naustud.io/tech-stack/img/typescript.png',desc:`The strict-typing flavor of ECMAScript,always requires a compiler to compile to vanilla JavaScript
but the type checking and other syntactical sugar are exceptional.Right now,we only use it for
Angular 2 projects when needed.`},{cat:'workflow',name:'code.naustud.io',value:100,icon:'https://naustud.io/tech-stack/img/naustudio.svg',desc:`A set of guidelines,presets,configs and stadard documentation for Nau developers.Please visit the document site at:<a href='http://code.naustud.io'target='_blank'>code.naustud.io</a>`},{cat:'workflow',name:'Mobile First',value:100,icon:'Mobile First',desc:`This is one of our most important principle for web and mobile development.More details will be discussed in blog later.`},{cat:'workflow',name:'BabelJS',value:50,icon:'https://naustud.io/tech-stack/img/babel.png',desc:`The de-facto tool to work with ECMAScript 6 and ReactJS nowadays.`},{cat:'workflow',name:'CSS BEM Notation',value:70,icon:'CSS BEM Notation',desc:`Our naming standard for CSS,which enhance collaboration,documentation and reusability of
CSS rules.`},{cat:'workflow',name:'Front End Code Guide',value:30,icon:'Front End;Code Guide',desc:`Based on an existing best practice document for HTML and CSS.We're adopting it as our standards
		and guideline.
	`
				     }, {
					 cat: 'workflow', name: 'ESLint', value: 20,
					 icon: 'https:desc:`The tool to check and validate JavaScript code when we develop and prevent potential issues with code.`},{cat:'workflow',name:'Gitflow Workflow',value:70,icon:'https://naustud.io/tech-stack/img/gitflow.png',desc:`Our code version control tool is Git,and Gitflow is one of its workflow standard which
ensure good collaboration and avoid conflict-resolving efforts.For more info,visit:code.naustud.io`},{cat:'workflow',name:'GulpJS',value:50,icon:'https://naustud.io/tech-stack/img/gulp.png',desc:`GulpJS is a task automation tools written for Node.js.It is among the most popular
Front End and Node project automation tools nowadays`},{cat:'workflow',name:'Nau Code Styles',value:50,icon:'Nau Code Styles',desc:`Based on AirBnB's well-defined JavaScript code styles. Our derivation has some different standards such as
		TAB indentation. This code style has an accompanied ESLint config.
	`
				     }, {
					 cat: 'workflow', name: 'Stylelint', value: 50,
					 icon: 'https:desc:`Our on-stop tool to validate both CSS and SCSS with a set of conventions and guidelines from our best practice.`},{cat:'workflow',name:'SystemJS',value:20,icon:'SystemJS',desc:`A module loader library that come along Angular 2.Its use is scarce,however.`},{cat:'workflow',name:'Webpack',value:30,icon:'https://naustud.io/tech-stack/img/webpack.svg',desc:`A module bundler library that is becoming de-facto tool to use in ReactJS or SPA apps nowadays.`},{cat:'legacy',name:'AngularJS 1',value:10,icon:'https://naustud.io/tech-stack/img/angular1.png',desc:`Angular 1.Deprecated`},{cat:'legacy',name:'Backbone',value:30,icon:'https://naustud.io/tech-stack/img/backbone.png',desc:`A Model-View library.Deprecated`},{cat:'legacy',name:'Grunt & Automation Stack',value:30,icon:'https://naustud.io/tech-stack/img/grunt.svg',desc:`Grunt task automation tool.Deprecated`},{cat:'legacy',name:'jQuery',value:50,icon:'https://naustud.io/tech-stack/img/jquery.png',desc:`Deprecated,because<a href='http://youmightnotneedjquery.com/'target='_blank'>youmightnotneedjquery.com</a>`},{cat:'legacy',name:'RequireJS & AMD',value:30,icon:'https://naustud.io/tech-stack/img/requirejs.svg',desc:`AMD module loader.Deprecated and replaced by ES module and Webpack.`},{cat:'legacy tooling',name:'Browser Sync',value:40,icon:'Browser Sync',desc:`Web development server popular among gulp/grunt web apps.No deprecated and replaced by live-server
or webpackDevServer.`},{cat:'legacy tooling',name:'Git Pre-commit',value:30,icon:'Git;Pre-commit',desc:`Pre-commit hook for git,now deprecated due to slow commit time.Code validation should be done
in the code editor.`},{cat:'legacy tooling',name:'http-server',value:20,icon:'http-server',desc:`A quick test web server based on Node.js,deprecated and replaced by live-server.`},{cat:'legacy tooling',name:'LiveReload',value:20,icon:'Live;Reload',desc:`A propritery auto-reload solution for web developers,now deprecated in favor of live-server and
hot module reload in Webpack.`}];function RamaddaD3bubbleDisplay(displayManager,id,properties){const ID_BUBBLES="bubbles";var SUPER;RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_D3BUBBLE,properties));addRamaddaDisplay(this);if(!window["BubbleChart"]){Utils.importJS(ramaddaBaseUrl+"/lib/d3/d3-legend.min.js");Utils.importJS(ramaddaBaseUrl+"/lib/d3/bubblechart.js");}
RamaddaUtil.defineMembers(this,{needsData:function(){return true;},callbackWaiting:false,getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Bubble Chart",'labelField=""','categoryField=""','valueField=""','sizeLabel1=""','sizeLabel2=""','showSizeLegend=false'])},updateUI:function(){if(!window["BubbleChart"]){if(!this.callbackWaiting){this.callbackWaiting=true;setTimeout(()=>{this.callbackWaiting=false;this.updateUI()},100);}
return;}
var records=this.filterData();if(!records){return;}
...
this.jq(ID_DISPLAY_CONTENTS).html(html);let categoryField=this.getFieldById(null,this.getProperty("categoryField"));let valueField=this.getFieldById(null,this.getProperty("valueField"));let values;let min=0;let max=0;if(valueField){values=this.getColumnValues(records,recordField).values;values.map((v,idx)=>{min=idx==0?v:Math.min(v,min);max=idx==0?v:Math.max(v,max);});}
let labelField=this.getFieldById(null,this.getProperty("labelField","name"));let imageField=this.getFieldById(null,this.getProperty("imageField"));let template=this.getProperty("template","${default}");if(!labelField){this.setContents(this.getMessage("No label field found"));return}
records.map(r=>{let desc=this.getRecordHtml(r,null,template);let label=r.getValue(labelField.getIndex());let obj={name:label,icon:label,desc:desc,cat:"",value:10,}
if(categoryField)
obj.cat=r.getValue(categoryField.getIndex());if(valueField)
obj.value=r.getValue(valueField.getIndex());if(imageField)
obj.icon=r.getValue(imageField.getIndex());data.push(obj);});let colors=this.getColorTable(true);new BubbleChart("#"+this.getDomId(ID_BUBBLES),data,{label1:this.getProperty("sizeLabel1"),label2:this.getProperty("sizeLabel2"),colors:colors,showSizeLegend:this.getProperty("showSizeLegend",valueField!=null)});}})}
var DISPLAY_WORDCLOUD="wordcloud";var DISPLAY_TEXTSTATS="textstats";var DISPLAY_FREQUENCY="frequency";var DISPLAY_TEXTANALYSIS="textanalysis";var DISPLAY_TEXTRAW="textraw";var DISPLAY_TEXT="text";var DISPLAY_CARDS="cards";var DISPLAY_BLOCKS="blocks";var DISPLAY_TEMPLATE="template";var DISPLAY_SLIDES="slides";var DISPLAY_IMAGES="images";var DISPLAY_TOPFIELDS="topfields";var DISPLAY_PERCENTCHANGE="percentchange";addGlobalDisplayType({type:DISPLAY_TEXT,label:"Text Readout",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_CARDS,label:"Cards",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_IMAGES,label:"Images",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_TEMPLATE,label:"Template",requiresData:true,forUser:true,category:"Text"});addGlobalDisplayType({type:DISPLAY_SLIDES,label:"Slides",requiresData:true,forUser:true,category:"Text"});addGlobalDisplayType({type:DISPLAY_TOPFIELDS,label:"Top Fields",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_PERCENTCHANGE,label:"Percent Change",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_WORDCLOUD,forUser:true,label:"Word Cloud",requiresData:true,category:"Text"});addGlobalDisplayType({type:DISPLAY_TEXTSTATS,forUser:true,label:"Text Stats",requiresData:true,category:"Text"});addGlobalDisplayType({type:DISPLAY_FREQUENCY,forUser:true,label:"Frequency",requiresData:true,category:"Text"});addGlobalDisplayType({type:DISPLAY_TEXTRAW,forUser:true,label:"Text Raw",requiresData:true,category:"Text"});addGlobalDisplayType({type:DISPLAY_TEXTANALYSIS,forUser:true,label:"Text Analysis",requiresData:true,category:"Text"});addGlobalDisplayType({type:DISPLAY_BLOCKS,forUser:true,label:"Blocks",requiresData:false,category:"Other Charts"});function RamaddaBaseTextDisplay(displayManager,id,type,properties){var ID_TEXTBLOCK="textblock";let SUPER=new RamaddaFieldsDisplay(displayManager,id,type,properties);RamaddaUtil.inherit(this,SUPER);$.extend(this,{processText:function(cnt){let records=this.filterData();if(!records){return null;}
var allFields=this.getData().getRecordFields();var fields=this.getSelectedFields(allFields);if(fields.length==0)
...
var fieldInfo={};var minLength=parseInt(this.getProperty("minLength",0));var maxLength=parseInt(this.getProperty("maxLength",100));var stopWords=this.getProperty("stopWords");if(stopWords){if(stopWords=="default"){stopWords=Utils.stopWords;}else{stopWords=stopWords.split(",");}}
...
tmp.push(counts[i]);}
counts=tmp;}
for(var wordIdx=0;wordIdx<counts.length;wordIdx++){var word=counts[wordIdx];var obj1={weight:word.count,handlers:handlers,text:word.word,};var obj2={weight:word.count,handlers:handlers,text:field.getLabel()+":"+word.word,};fi.words.push(obj1);words.push(obj2);}
var label="";if(this.getProperty("showFieldLabel",true))
label="<b>"+fi.field.getLabel()+"</b>";divs+="<div style='display:inline-block;width:"+width+"'>"+label+HtmlUtils.div(["style","border: 1px #ccc solid;height:300px;","id",fi.divId],"")+"</div>";}
this.writeHtml(ID_DISPLAY_CONTENTS,"");var options={autoResize:true,};var colors=this.getColorTable(true);if(colors){options.colors=colors,options.classPattern=null;options.fontSize={from:0.1,to:0.02};}
if(this.getProperty("shape"))
options.shape=this.getProperty("shape");if(this.getProperty("combined",false)){this.writeHtml(ID_DISPLAY_CONTENTS,HtmlUtils.div(["id",this.getDomId("words"),"style","height:300px;"],""));$("#"+this.getDomId("words")).jQCloud(words,options);}else{this.writeHtml(ID_DISPLAY_CONTENTS,divs);for(a in fieldInfo){var fi=fieldInfo[a];$("#"+fi.divId).jQCloud(fi.words,options);}}},showRows:function(records,field,word){var tokenize=this.getProperty("tokenize",false);if(word.startsWith(field.getLabel()+":")){word=word.replace(field.getLabel()+":","");}
...
var tuple=[];data.push(tuple);for(var col=0;col<fields.length;col++){var f=fields[col];if(tableFields&&!tableFields[f.getId()])continue;var v=row[f.getIndex()];if(tokenize){v=v.replace(re,"<span style=background:yellow;>$1</span>");}
if(showRecords){html+=HtmlUtil.b(f.getLabel())+": "+v+"</br>";}else{tuple.push(v);}}
if(showRecords){html+="<p>";}}
if(showRecords){this.writeHtml(ID_DISPLAY_BOTTOM,html);}else{var prefix="";if(!tokenize){prefix=field.getLabel()+"="+word}
this.writeHtml(ID_DISPLAY_BOTTOM,prefix+HtmlUtils.div(["id",this.getDomId("table"),"style","height:300px"],""));var dataTable=google.visualization.arrayToDataTable(data);this.chart=new google.visualization.Table(document.getElementById(this.getDomId("table")));this.chart.draw(dataTable,{allowHtml:true});}}});}
...
img.crossOrigin='Anonymous';var c=colorThief.getColor(img);var p=colorThief.getPalette(img);var width=img.width/p.length;var html="";for(var i=0;i<p.length;i++){var c=p[i];html+=HtmlUtils.div(["style","display:inline-block;width:"+width+"px;height:"+img.height+"px;background:rgb("+c[0]+","+c[1]+","+c[2]+");"],"");}
...
img=ramaddaBaseUrl+"/proxy?url="+img;img=HtmlUtils.href(img,HtmlUtils.div(["id",this.getDomId("gallery")+"div"+imgCnt],HtmlUtils.image(img,["width",width,"id",this.getDomId("gallery")+"img"+imgCnt])),imgAttrs)+label;imgCnt++;html=HtmlUtils.div(["class","display-cards-item","title",tooltip,"style","margin:"+margin+"px;"],img);}else{var style="";if(fontSize){style+=" font-size:"+fontSize+"; ";}
...
if(cardStyle)
style+=cardStyle;var attrs=["title",tooltip,"class","ramadda-gridbox display-cards-card","style",style];if(this.altLabelField){html=HtmlUtils.div(attrs,this.altLabelField.getValue(row));}else{html=HtmlUtils.div(attrs,caption);}
...
var label="";var tt="";if(labelFields.length>0){labelFields.map(l=>{label+=" "+row[l.getIndex()]});label=HtmlUtils.div(["class","display-images-label"],label.trim());}
if(tooltipFields.length>0){tooltipFields.map(l=>{tt+="\n"+l.getLabel()+": "+row[l.getIndex()]});}
tt=tt.trim();contents+=HtmlUtils.div(["class","display-images-block","title",tt],HtmlUtils.image(image,["width",width])+
label);}
var header="";if(this.startIndex>0){header+=HtmlUtils.span(["id",this.getDomId(ID_PREV)],"Previous")+" ";}
if(this.startIndex+number<records.length){header+=HtmlUtils.span(["id",this.getDomId(ID_NEXT)],"Next")+" ";}
...
continue;}
if(f.isDate&&v.getTime){if(v.getTime()<s.min.getTime())s.min=v;if(v.getTime()>s.max.getTime())s.max=v;}else if(!isNaN(v)){s.total+=v;s.min=Math.min(s.min,v);s.max=Math.max(s.max,v);s.count++;}}});records=goodRecords;for(var i=0;i<fields.length;i++){var f=fields[i];if(!f.isNumeric())continue;var s=summary[f.getId()];if(s&&s.count){s.average=Utils.formatNumber(s.total/s.count);}}
...
var colorBy=this.getColorByInfo(selected);var headerTemplate=this.getProperty("headerTemplate","");var footerTemplate=this.getProperty("footerTemplate","");headerTemplate=headerTemplate.replace("${selectedCount}",selected.length);headerTemplate=headerTemplate.replace("${totalCount}",records.length);footerTemplate=footerTemplate.replace("${selectedCount}",selected.length);footerTemplate=footerTemplate.replace("${totalCount}",records.length);for(var i=0;i<fields.length;i++){var f=fields[i];var s=summary[f.getId()];if(!s)continue;if(f.isDate){headerTemplate=headerTemplate.replace("${"+f.getId()+"_min_yyyymmdd}",Utils.formatDateYYYYMMDD(s.min)).replace("${"+f.getId()+"_max_yyyymmdd}",Utils.formatDateYYYYMMDD(s.max)).replace("${"+f.getId()+"_min_yyyy}",Utils.formatDateYYYY(s.min)).replace("${"+f.getId()+"_max_yyyy}",Utils.formatDateYYYY(s.max));footerTemplate=footerTemplate.replace("${"+f.getId()+"_min_yyyymmdd}",Utils.formatDateYYYYMMDD(s.min)).replace("${"+f.getId()+"_max_yyyymmdd}",Utils.formatDateYYYYMMDD(s.max)).replace("${"+f.getId()+"_min_yyyy}",Utils.formatDateYYYY(s.min)).replace("${"+f.getId()+"_max_yyyy}",Utils.formatDateYYYY(s.max));continue;}
if(s&&f.isString()){headerTemplate=headerTemplate.replace("${"+f.getId()+"_uniques}",s.uniqueCount);footerTemplate=footerTemplate.replace("${"+f.getId()+"_uniques}",s.uniqueCount);continue;}
if(!f.isNumeric())continue;if(s){headerTemplate=headerTemplate.replace("${"+f.getId()+"_total}",s.total).replace("${"+f.getId()+"_min}",s.min).replace("${"+f.getId()+"_max}",s.max).replace("${"+f.getId()+"_average}",s.average);headerTemplate=headerTemplate.replace("${"+f.getId()+"_total_round}",Math.round(s.total)).replace("${"+f.getId()+"_min_round}",Math.round(s.min)).replace("${"+f.getId()+"_max_round}",Math.round(s.max)).replace("${"+f.getId()+"_average_round}",Math.round(s.average));headerTemplate=headerTemplate.replace("${"+f.getId()+"_total_format}",Utils.formatNumberComma(s.total)).replace("${"+f.getId()+"_min_format}",Utils.formatNumberComma(s.min)).replace("${"+f.getId()+"_max_format}",Utils.formatNumberComma(s.max)).replace("${"+f.getId()+"_average_format}",Utils.formatNumberComma(s.average));footerTemplate=footerTemplate.replace("${"+f.getId()+"_total}",s.total).replace("${"+f.getId()+"_min}",s.min).replace("${"+f.getId()+"_max}",s.max).replace("${"+f.getId()+"_average}",s.average);footerTemplate=footerTemplate.replace("${"+f.getId()+"_total_round}",Math.round(s.total)).replace("${"+f.getId()+"_min_round}",Math.round(s.min)).replace("${"+f.getId()+"_max_round}",Math.round(s.max)).replace("${"+f.getId()+"_average_round}",Math.round(s.average));footerTemplate=footerTemplate.replace("${"+f.getId()+"_total_format}",Utils.formatNumberComma(s.total)).replace("${"+f.getId()+"_min_format}",Utils.formatNumberComma(s.min)).replace("${"+f.getId()+"_max_format}",Utils.formatNumberComma(s.max)).replace("${"+f.getId()+"_average_format}",Utils.formatNumberComma(s.average));}}
if(selected.length==1){var row=this.getDataValues(selected[0]);headerTemplate=this.applyRecordTemplate(row,fields,headerTemplate);footerTemplate=this.applyRecordTemplate(row,fields,footerTemplate);}
if(this.filterFields){for(var filterIdx=0;filterIdx<this.filterFields.length;filterIdx++){var f=this.filterFields[filterIdx];if(f.isNumeric()){var min=$("#"+this.getDomId("filterby_"+f.getId()+"_min")).val().trim();var max=$("#"+this.getDomId("filterby_"+f.getId()+"_max")).val().trim();headerTemplate=headerTemplate.replace("${filter_"+f.getId()+"_min}",min);headerTemplate=headerTemplate.replace("${filter_"+f.getId()+"_max}",max);footerTemplate=footerTemplate.replace("${filter_"+f.getId()+"_min}",min);footerTemplate=footerTemplate.replace("${filter_"+f.getId()+"_max}",max);}else{var widget=$("#"+this.getDomId("filterby_"+f.getId()));if(!widget.val||widget.val()==null)continue;var value=widget.val();if(!value)continue;if(Array.isArray(value)){var tmp="";value.map(v=>{if(tmp!="")tmp+=", ";tmp+=v;});value=tmp;}
...
if(selected.length>0){contents+=headerTemplate;}
...
var row=this.getDataValues(record);var s=template.trim();s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);s=this.applyRecordTemplate(row,fields,s,props);s=s.replace(/\${recordIndex}/g,(rowIdx+1));var recordStyle=style;if(color){if(this.getProperty("colorBackground",false))
recordStyle="background: "+color+";"+recordStyle;s=s.replace("${color}",color);}
var tag=HtmlUtils.openTag("div",["style",recordStyle,"id",this.getId()+"-"+record.getId(),"title","","class","display-template-record","recordIndex",rowIdx]);if(s.startsWith("<td")){s=s.replace(/<td([^>]*)>/,"<td $1>"+tag);s=s.replace(/<\/td>$/,"</div></td>");contents+=s;}else if(s.startsWith("<tr")){s=s.replace(/<td([^>]*)>/g,"<td $1>"+tag);s=s.replace(/<\/td>/g,"</div></td>");contents+=s;}else{contents+=tag+s+"</div>"}
if(cols>0){contents+='</div>\n';}}
if(cols>0){contents+='</div>\n';}}
if(selected.length>0)
contents+=footerTemplate;this.writeHtml(ID_DISPLAY_CONTENTS,contents);var recordElements=this.jq(ID_DISPLAY_CONTENTS).find(".display-template-record");this.makeTooltips(recordElements,selected);this.makePopups(recordElements,selected);let _this=this;this.jq(ID_DISPLAY_CONTENTS).find(".display-template-record").click(function(){var record=selected[$(this).attr("recordIndex")];_this.handleEventRecordHighlight(this,{record:record,highlight:true,immediate:true,skipScroll:true});_this.getDisplayManager().notifyEvent("handleEventRecordSelection",_this,{highlight:true,record:record});});},highlightCount:0,handleEventRecordHighlight:function(source,args){let myCount=++this.highlightCount;var id="#"+this.getId()+"-"+args.record.getId();if(this.highlightedElement){this.unhighlightElement(this.highlightedElement);this.highlightedElement=null;}
if(args.highlight){if(args.immediate){this.highlightElement(args);}else{setTimeout(()=>{if(myCount==this.highlightCount){this.highlightElement(args);}},500);}}else{var id="#"+this.getId()+"-"+args.record.getId();var element=$(id);this.unhighlightElement(element);}},unhighlightElement:function(element){element.removeClass("display-template-record-highlight");var css=this.getProperty("highlightOffCss","").split(";");if(css.length>0){css.map(tok=>{var c=tok.split(":");var a=c[0];var v=c.length>1?c[1]:null;if(!v||v==""){v=element.attr("prev-"+a);}
...
function RamaddaPercentchangeDisplay(displayManager,id,properties){let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_PERCENTCHANGE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Percent change Attributes",'template="${date1} ${date2} ${value1} ${value2} ${percent} ${per_hour} ${per_day} ${per_week} ${per_month} ${per_year}"','fieldLabel=""','sortFields=false']);},updateUI:function(){var records=this.filterData();if(!records)return;var allFields=this.getData().getRecordFields();var fields=this.getSelectedFields(allFields);fields=this.getFieldsOfType(fields,"numeric");let record1=records[0];let record2=records[records.length-1];var template=this.getProperty("template",null);var headerTemplate=this.getProperty("headerTemplate","");var footerTemplate=this.getProperty("footerTemplate","");let date1=record1.getDate();let date2=record2.getDate();let label1="Start Value";let label2="End Value";let hours=1;let days=1;let years=1;let months=1;if(date1)
label1=this.formatDate(date1);if(date2)
label2=this.formatDate(date2);if(date1&&date2){var diff=date2.getTime()-date1.getTime();days=diff/1000/60/60/24;hours=days*24;years=days/365;months=years*12;}
let html="";if(template){html=headerTemplate;}else{html="<br>";html+=HtmlUtils.openTag("table",["class","stripe nowrap ramadda-table","id",this.getDomId("percentchange")]);html+=HtmlUtils.openTag("thead",[]);html+="\n";html+=HtmlUtils.tr([],HtmlUtils.th(["style","text-align:center"],this.getProperty("fieldLabel","Field"))+HtmlUtils.th(["style","text-align:center"],label1)+HtmlUtils.th(["style","text-align:center"],label2)
+HtmlUtils.th(["style","text-align:center"],"Percent Change"));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);}
var tuples=[];fields.map(f=>{var val1=record1.getValue(f.getIndex());var val2=record2.getValue(f.getIndex());var percent=parseInt(1000*(val2-val1)/val1)/10;tuples.push({field:f,val1:val1,val2:val2,percent:percent});});if(this.getProperty("sortFields",true)){tuples.sort((a,b)=>{return-(a.percent-b.percent);})}
tuples.map(t=>{if(template){var h=template.replace("${field}",t.field.getLabel()).replace("${value1}",this.formatNumber(t.val1)).replace("${value2}",this.formatNumber(t.val2)).replace("${percent}",this.formatNumber(t.percent)).replace("${date1}",label1).replace("${date2}",label2).replace("${difference}",this.formatNumber(t.val2-t.val1));h=h.replace(/\${per_hour}/g,this.formatNumber(t.percent/hours));h=h.replace(/\${per_day}/g,this.formatNumber(t.percent/days));h=h.replace(/\${per_week}/g,this.formatNumber(t.percent/(days/7)));h=h.replace(/\${per_month}/g,this.formatNumber(t.percent/months));h=h.replace(/\${per_year}/g,this.formatNumber(t.percent/years));html+=h;}else{html+=HtmlUtils.tr([],HtmlUtils.td([],t.field.getLabel())+
HtmlUtils.td(["align","right"],t.val1)+
HtmlUtils.td(["align","right"],t.val2)
+HtmlUtils.td(["align","right"],t.percent+"%"));}});if(template){html+=footerTemplate;}else{html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");}
this.writeHtml(ID_DISPLAY_CONTENTS,html);HtmlUtils.formatTable("#"+this.getDomId("percentchange"),{});},})}
...
var width=this.getWidthForStyle();if(width){style+=" width:"+width+";";}
return style;},updateUI:function(){var pointData=this.getData();if(pointData==null)return;this.records=this.filterData();if(!this.records)return;this.fields=this.getData().getRecordFields();this.records=this.sortRecords(this.records);var template=this.getProperty("template","");var slideWidth=this.getProperty("slideWidth","100%");var height=this.getHeightForStyle("400");var left=HtmlUtils.div(["id",this.getDomId(ID_PREV),"style","font-size:200%;","class","display-slides-arrow-left fas fa-angle-left"]);var right=HtmlUtils.div(["id",this.getDomId(ID_NEXT),"style","font-size:200%;","class","display-slides-arrow-right fas fa-angle-right"]);var slide=HtmlUtils.div(["style","overflow-y:auto;max-height:"+height+";","id",this.getDomId(ID_SLIDE),"class","display-slides-slide"]);var navStyle="padding-top:20px;";var contents=HtmlUtils.div(["style","position:relative;"],"<table width=100%><tr valign=top><td width=20>"+HtmlUtils.div(["style",navStyle],left)+"</td><td>"+
slide+"</td>"+
"<td width=20>"+HtmlUtils.div(["style",navStyle],right)+"</td></tr></table>");this.writeHtml(ID_DISPLAY_CONTENTS,contents);this.jq(ID_PREV).click(()=>{this.slideIndex--;this.displaySlide(true);});this.jq(ID_NEXT).click(()=>{this.slideIndex++;this.displaySlide(true);});setTimeout(()=>{this.displaySlide();},200);},displaySlide:function(propagateEvent){if(this.slideIndex<0)this.slideIndex=0;if(this.slideIndex>=this.records.length)this.slideIndex=this.records.length-1;if(this.slideIndex==0)
...
this.headers.push("");this.showBlocks(true);this.writeHtml(ID_DISPLAY_CONTENTS,HtmlUtils.div(["class","display-blocks-header","style",this.getProperty("headerStyle","",true),"id",this.getDomId(ID_BLOCKS_HEADER)])+
HtmlUtils.div(["class","display-blocks-blocks","id",this.getDomId(ID_BLOCKS)])+
HtmlUtils.div(["class","display-blocks-footer","style",this.getProperty("footerStyle","",true),"id",this.getDomId(ID_BLOCKS_FOOTER)]));this.showBlocks(true);HtmlUtils.callWhenScrolled(this.getDomId(ID_DISPLAY_CONTENTS),()=>{if(!this.displayedBlocks){this.displayedBlocks=true;setTimeout(()=>{this.showBlocks(false)},animStep);}},500);},showBlocks:function(initial,step){if(!Utils.isDefined(step)){if(initial)step=this.counts.length;else step=0;}
var contents="";contents+=HtmlUtils.openDiv(["class","display-blocks"]);var tmp=this.getProperty("colors","red,blue,gray,green",true);var ct=(typeof tmp)=="string"?tmp.split(","):tmp;var footer="";while(ct.length<this.counts.length){ct.push(ct[ct.length-1]);}
var multiplier=parseFloat(this.getProperty("multiplier","1",true));var dim=this.getProperty("blockDimensions","8",true);var labelStyle=this.getProperty("labelStyle","",true);var blockCnt=0;for(var i=0;i<this.counts2.length;i++){var label=this.footers[i].replace("${count}",multiplier*this.counts[i]);var style="width:"+dim+"px;height:"+dim+"px;";if(!initial){if(i<step){style+="background:"+ct[i]+";";footer+=HtmlUtils.div(["class","display-block","style",style],"")+" "+HtmlUtils.span(["style",labelStyle],label)+"&nbsp;&nbsp;";}else{footer+="&nbsp;&nbsp;";}}
var cnt=this.counts2[i];for(var j=0;j<this.counts2[i];j++){contents+=HtmlUtils.div(["class","display-block","style",style,"title",label],"");}
blockCnt++;}
contents+=HtmlUtils.closeDiv();this.jq(ID_BLOCKS_HEADER).html(this.getProperty("header",""));this.jq(ID_BLOCKS).html(contents);this.jq(ID_BLOCKS_FOOTER).html(footer);if(step<this.counts.length){setTimeout(()=>{this.showBlocks(false,step+1)},animStep);}}})}
...
html+="<b>"+fi.field.getLabel()+"</b><br>";var td1Width="20%";var td2Width="10%";if(this.getProperty("showSummary",true)){html+=HtmlUtils.openTag("table",["class","nowrap ramadda-table","id",this.getDomId("table_summary")]);html+=HtmlUtils.openTag("thead",[]);html+=HtmlUtils.tr([],HtmlUtils.th(["width",td1Width],"Summary")+HtmlUtils.th([],"&nbsp;"));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);html+=HtmlUtils.tr([],HtmlUtils.td(["align","right"],"Total lines:")+HtmlUtils.td([],records.length));html+=HtmlUtils.tr([],HtmlUtils.td(["align","right"],"Total words:")+HtmlUtils.td([],cnt.count));html+=HtmlUtils.tr([],HtmlUtils.td(["align","right"],"Average word length:")+HtmlUtils.td([],Math.round(cnt.total/cnt.count)));html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");html+="<br>"}
if(this.getProperty("showCounts",true)){html+=HtmlUtils.openTag("table",["class","row-border nowrap ramadda-table","id",this.getDomId("table_counts")]);html+=HtmlUtils.openTag("thead",[]);html+=HtmlUtils.tr([],HtmlUtils.th(["width",td1Width],"Word Length")+HtmlUtils.th(["width",td2Width],"Count")+(showBars?HtmlUtils.th([],""):""));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);for(var i=0;i<tmp.length;i++){var row=HtmlUtils.td([],tmp[i].length)+HtmlUtils.td([],tmp[i].count);if(showBars){var wpercent=(tmp[i].count-min)/max;var width=2+wpercent*barWidth;var color=barColor;var div=HtmlUtils.div(["style","height:10px;width:"+width+"px;background:"+color],"");row+=HtmlUtils.td([],div);}
html+=HtmlUtils.tr([],row);}
html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");html+="<br>"}
if(this.getProperty("showFrequency",true)){html+=HtmlUtils.openTag("table",["class","row-border ramadda-table","id",this.getDomId("table_frequency")]);html+=HtmlUtils.openTag("thead",[]);html+=HtmlUtils.tr([],HtmlUtils.th(["width",td1Width],"Word")+HtmlUtils.th(["width",td2Width],"Frequency")+(showBars?HtmlUtils.th([],""):""));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);var min=0;var max=0;if(counts.length>0){min=counts[0].count;max=counts[counts.length-1].count;}
var totalWords=0;for(var i=0;i<counts.length;i++){totalWords+=counts[i].count;}
for(var i=counts.length-1;i>=0;i--){var percent=Math.round(10000*(counts[i].count/totalWords))/100;var row=HtmlUtils.td([],counts[i].word+"&nbsp;:&nbsp;")+
HtmlUtils.td([],counts[i].count+"&nbsp;&nbsp;("+percent+"%)&nbsp;:&nbsp;");if(showBars){var wpercent=(counts[i].count-min)/max;var width=2+wpercent*barWidth;var color=barColor;var div=HtmlUtils.div(["style","height:10px;width:"+width+"px;background:"+color],"");row+=HtmlUtils.td([],div);}
html+=HtmlUtils.tr([],row);}
html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");}}
...
html+=HtmlUtils.openTag("div",["class","display-frequency-table","style",hor?"":"display:block;"]);html+=HtmlUtils.openTag("table",["cellpadding","3","id",this.getDomId("summary"+col),"table-height",this.getProperty("tableHeight","300",true),"class","stripe row-border nowrap ramadda-table"]);if(this.getProperty("showHeader",true)){html+=HtmlUtils.openTag("thead",[]);var label=HtmlUtils.span(["title","Click to reset","class","display-frequency-label","data-field",s.field.getId()],f.getLabel());label=HtmlUtils.div(["style","max-width:500px;overflow-x:auto;"],label);var count=showCount?HtmlUtils.th(["align","right","width","20%"],HtmlUtils.div(["style","text-align:right"],"Count")):"";var percent=showPercent?HtmlUtils.th(["align","right","width","20%"],HtmlUtils.div(["style","text-align:right"],"Percent")):"";var bars=showBars?HtmlUtils.th(["align","right","width",barWidth],HtmlUtils.div(["style","text-align:right"],"&nbsp;")):"";html+=HtmlUtils.tr([],HtmlUtils.th(["xxwidth","60%"],label+count+percent+bars));html+=HtmlUtils.closeTag("thead");}
html+=HtmlUtils.openTag("tbody",[]);var colors=this.getColorTable(true);var dfltColor=this.getProperty("barColor","blue");if(colors){for(var i=0;i<s.values.length;i++){var value=s.values[i].value;if(i<colors.length)
...
for(var i=0;i<s.values.length;i++){var value=s.values[i].value;var label=value;if(label=="")label="&lt;blank&gt;";var count=s.values[i].count;if(count==0)continue;var perc=count/s.total;value=value.replace(/\'/g,"&apos;");var countLabel=HtmlUtils.span(["title","Click to select","class","display-frequency-value","data-field",s.field.getId(),"data-value",value],count);value=HtmlUtils.span(["title","Click to select","class","display-frequency-value","data-field",s.field.getId(),"data-value",value],label);var color=s.values[i].color;if(!color)color=dfltColor;if(showPercent)countLabel+=" ("+Math.round(perc*100)+"%)";bannerHtml+=HtmlUtils.div(["class","display-frequency-banner-element"],value+"<br>"+countLabel);var tdv=HtmlUtils.td([],value);var tdc=(showCount?HtmlUtils.td(["align","right"],count):"");var tdp=showPercent?HtmlUtils.td(["align","right"],s.total==0?"0":Math.round(perc*100)+"%"):"";var bw=perc/maxPercent;var tdb=showBars?HtmlUtils.td(["valign","center","width",barWidth],HtmlUtils.div(["title",Math.round(perc*100)+"%","style","background:"+color+";height:10px;width:"+(Math.round(bw*barWidth))+"px"],"")):"";html+=HtmlUtils.tr([],tdv+tdc+tdp+tdb);}
html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");html+=HtmlUtils.closeTag("div");bannerHtml+="</div>";}
if(doBanner)html=HtmlUtils.div(["class","display-frequency-banner"],bannerHtml);this.writeHtml(ID_DISPLAY_CONTENTS,html);let _this=this;this.jq(ID_DISPLAY_CONTENTS).find(".display-frequency-value").click(function(){var click=_this.getProperty("clickFunction")
...
var height=this.getProperty("height","400");var html=HtmlUtils.openTag("div",["id",this.getDomId("tables")]);for(var i=0;i<cols.length;i+=3){var c1=cols[i];var c2=i+1<cols.length?cols[i+1]:null;var c3=i+2<cols.length?cols[i+2]:null;var width=c2?(c3?"33%":"50%"):"100%";var style="padding:5px";var row="";row+=HtmlUtils.td(["width",width],HtmlUtils.div(["style",style],c1));if(c2)
row+=HtmlUtils.td(["width",width],HtmlUtils.div(["style",style],c2));if(c3)
row+=HtmlUtils.td(["width",width],HtmlUtils.div(["style",style],c3));html+=HtmlUtils.tag("table",["width","100%"],HtmlUtils.tr(row));}
html+=HtmlUtils.closeTag("div");this.writeHtml(ID_DISPLAY_CONTENTS,html);HtmlUtils.formatTable("#"+this.getDomId("tables")+" .ramadda-table",{scrollY:this.getProperty("tableHeight","200")});},printList:function(title,l){var maxWords=parseInt(this.getProperty("maxWords",10));var minCount=parseInt(this.getProperty("minCount",0));var table=HtmlUtils.openTag("table",["width","100%","class","stripe hover ramadda-table"])+HtmlUtils.openTag("thead",[]);table+=HtmlUtils.tr([],HtmlUtils.th([],title)+HtmlUtils.th([],"&nbsp;"));table+=HtmlUtils.closeTag("thead");table+=HtmlUtils.openTag("tbody");var cnt=0;for(var i=0;i<l.length;i++){if(l[i].count<minCount)continue;var row=HtmlUtils.td([],l[i].normal)+
HtmlUtils.td([],l[i].count+" ("+l[i].percent+"%)");table+=HtmlUtils.tr([],row);if(cnt++>maxWords)break;}
table+=HtmlUtils.closeTag("tbody")+HtmlUtils.closeTag("table");return table;}});}
function RamaddaTextrawDisplay(displayManager,id,properties){var ID_TEXT="text";var ID_OVERLAY="overlay";var ID_OVERLAY_TABLE="overlaytable";var ID_LABEL="label";var ID_SEARCH="search";var ID_HIGHLIGHT="highlight";var ID_SHRINK="shrink";let SUPER=new RamaddaBaseTextDisplay(displayManager,id,DISPLAY_TEXTRAW,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{doShrink:this.getProperty("initialShrink",false),getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Raw Text Attributes",'doBubble=true','addLineNumbers=false','labelTemplate="${lineNumber}"','maxLines=1000','pattern="initial search pattern"','fromField=""','linesDescriptor=""']);},checkLayout:function(){this.updateUI();},updateUI:function(){let records=this.filterData();if(!records){return null;}
var pointData=this.getData();this.allRecords=pointData.getRecords();var pattern=this.getProperty("pattern");if(pattern&&pattern.length==0)pattern=null;if(pattern)pattern=pattern.replace(/"/g,"&quot;");var input="";if(!this.filterFields||this.filterFields.length==0)
input=" "+HtmlUtils.input("pattern",(pattern?pattern:""),["placeholder","Search text","id",this.getDomId(ID_SEARCH)]);this.showShrink=this.getProperty("showShrink",false);if(this.showShrink){input+=" "+HtmlUtils.checkbox("shrink",["id",this.getDomId(ID_SHRINK)],this.getProperty("initialShrink",true))+" Shrink ";}
this.writeHtml(ID_TOP_RIGHT,HtmlUtils.span(["id",this.getDomId(ID_LABEL)]," ")+input);let _this=this;this.jq(ID_SHRINK).click(function(){_this.doShrink=_this.jq(ID_SHRINK).is(':checked');_this.setProperty("initialShrink",_this.doShrink);_this.updateUI();});this.jq(ID_SEARCH).keypress(function(event){if(event.which==13){_this.setProperty("pattern",$(this).val());_this.propagateEvent("handleEventPropertyChanged",{property:"pattern",value:$(this).val()});_this.updateUI();}});var height=this.getProperty("height","600");var style=this.getProperty("displayInnerStyle","");var html=HtmlUtils.div(["id",this.getDomId(ID_TEXT),"style","padding:4px;border:1px #ccc solid; max-height:"+height+"px;overflow-y:auto;"+style]);this.writeHtml(ID_DISPLAY_CONTENTS,html);this.showText();},handleEventPropertyChanged:function(source,prop){if(prop.property=="pattern"){this.setProperty("pattern",prop.value);this.updateUI();return;}
...
var corpus=HtmlUtils.openTag("div",["style","position:relative;"]);corpus+=HtmlUtils.div(["id",this.getDomId(ID_OVERLAY),"style","position:absolute;top:0;left:0;"],HtmlUtils.tag("table",["id",this.getDomId(ID_OVERLAY_TABLE)]));var fromField=this.getFieldById(null,this.getProperty("fromField"));var bubble=this.getProperty("doBubble",false);if(labelTemplate){corpus+="<table width=100%>";}
var lineCnt=0;var displayedLineCnt=0;var patternMatch=new TextMatcher(pattern);var regexpMaps={};var filterFieldMap={};if(this.filterFields){this.filterFields.map(f=>{if(f.isString)filterFieldMap[f.getId()]=true;});}
var templates={};fields.map(f=>{templates[f.getId()]=this.getProperty(f.getId()+".template");});var colorBy=this.getColorByInfo(records);var delimiter=this.getProperty("delimiter","");var rowScale=this.showShrink?this.getProperty("rowScale",0.3):null;if(this.showShrink){corpus+="<tr><td>"+HtmlUtils.getIconImage("fa-caret-down")+"</td></tr>";}
for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];if(!Utils.isDefined(record.lineNumber)){record.lineNumber=(rowIdx+1);}
this.indexToRecord[rowIdx]=record;this.recordToIndex[record.getId()]=rowIdx;var row=this.getDataValues(record);var line="";for(var col=0;col<fields.length;col++){var f=fields[col];if(rowIdx==0){if(filterFieldMap[f.getId()]){var value=this.getFilterFieldValues(f);if(value){if(!Array.isArray(value)){value=[value];}
...
line=patternMatch.highlight(line);displayedLineCnt++;if(displayedLineCnt>maxLines)break;var lineAttrs=["title"," ","class"," display-raw-line ","recordIndex",rowIdx]
if(bubble)line=HtmlUtils.div(["class","ramadda-bubble"],line);if(fromField)line+=HtmlUtils.div(["class","ramadda-bubble-from"],""+row[fromField.getIndex()]);if(highlights){for(var hi=0;hi<highlights.length;hi++){var h=highlights[hi];var s=hi<highlightStyles.length?highlightStyles[hi]:highlightStyles[highlightStyles.length-1];s=s.replace(/_comma_/g,",");line=line.replace(h,"<span style='"+s+"'>$1</span>");}}
line=HtmlUtils.div(lineAttrs,line);if(labelTemplate){var label=this.getRecordHtml(record,null,labelTemplate);var num=record.lineNumber;if(!Utils.isDefined(num)){num-lineCnt;}
label=label.replace("${lineNumber}","#"+(num));label=label.replace(/ /g,"&nbsp;");var r="";if(this.showShrink){r+=HtmlUtils.td(["width","5px","style","background:#ccc;"],HtmlUtils.getIconImage("fa-caret-right",null,["style","line-height:0px;"]));}
r+=HtmlUtils.td(["width",labelWidth],"<a name=line_"+lineCnt+"></a>"+
"<a href=#line_"+lineCnt+">"+label+"</a>&nbsp;  ")+
HtmlUtils.td([],line);corpus+=HtmlUtils.tr(rowAttrs,r);}else{corpus+=line;if(asHtml){if(breakLines)
corpus+="<p>";else
corpus+="<br>";}else{corpus+="\n";if(breakLines)
corpus+="\n";}}}
if(addLineNumbers){corpus+="</table>";}
corpus+=HtmlUtils.closeTag("div");if(!asHtml)
corpus=HtmlUtils.tag("pre",[],corpus);this.writeHtml(ID_TEXT,corpus);colorBy.displayColorTable();var linesWord=" "+this.getProperty("linesDescriptor","lines");var label=displayedLineCnt+linesWord;if(this.allRecords.length!=displayedLineCnt){label=displayedLineCnt+"/"+this.allRecords.length+linesWord+" ("+Math.round(displayedLineCnt/this.allRecords.length*100)+"%)";}
...
var lines=this.jq(ID_TEXT).find(".display-raw-line");lines.click(function(){var idx=$(this).attr("recordIndex");var record=_this.indexToRecord[idx];if(record){_this.showRecordPopup($(this),record);_this.getDisplayManager().notifyEvent("handleEventRecordSelection",_this,{record:record});_this.highlightLine(idx);}});this.makeTooltips(lines,records);},highlightLine:function(index){var container=this.jq(ID_TEXT);container.find(".display-raw-line").removeClass("display-raw-line-selected");var sel="[recordIndex='"+index+"']";var element=container.find(sel);element.addClass("display-raw-line-selected");},handleEventRecordSelection:function(source,args){var index=this.findMatchingIndex(args.record);if(index<0||!Utils.isDefined(index)){return;}
...
function RamaddaTextDisplay(displayManager,id,properties){let SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_TEXT,properties);$.extend(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{lastHtml:"<p>&nbsp;<p>&nbsp;<p>",needsData:function(){return true;},updateUI:function(){SUPER.updateUI.call(this);if(!this.getProperty("recordTemplate")){this.setProperty("recordTemplate","${default}");}
let records=this.filterData();if(records&&records.length>0){this.lastHtml=this.getRecordHtml(records[0]);this.setContents(this.lastHtml);}},handleEventRecordSelection:function(source,args){this.lastHtml=this.getRecordHtml(args.record);this.setContents(this.lastHtml);}});}
var DISPLAY_ENTRYLIST="entrylist";var DISPLAY_TESTLIST="testlist";var DISPLAY_ENTRYDISPLAY="entrydisplay";var DISPLAY_ENTRY_GALLERY="entrygallery";var DISPLAY_ENTRY_GRID="entrygrid";var DISPLAY_OPERANDS="operands";var DISPLAY_METADATA="metadata";var DISPLAY_ENTRYTIMELINE="entrytimeline";var DISPLAY_REPOSITORIES="repositories";var DISPLAY_ENTRYTITLE="entrytitle";var ID_RESULTS="results";var ID_ENTRIES="entries";var ID_DETAILS="details";var ID_DETAILS_INNER="detailsinner";var ID_PROVIDERS="providers";var ID_SEARCH_SETTINGS="searchsettings";var ATTR_ENTRYID="entryid";var ID_TREE_LINK="treelink";addGlobalDisplayType({type:DISPLAY_ENTRYLIST,label:"Entry List",requiresData:false,category:"Entry Displays"});addGlobalDisplayType({type:DISPLAY_TESTLIST,label:"Test  List",requiresData:false,category:"Entry Displays"});addGlobalDisplayType({type:DISPLAY_ENTRYDISPLAY,label:"Entry Display",requiresData:false,category:"Entry Displays"});addGlobalDisplayType({type:DISPLAY_ENTRYTITLE,label:"Entry Title",requiresData:false,category:"Entry Displays"});addGlobalDisplayType({type:DISPLAY_ENTRY_GALLERY,label:"Entry Gallery",requiresData:false,category:"Entry Displays"});addGlobalDisplayType({type:DISPLAY_ENTRY_GRID,label:"Entry Date Grid",requiresData:false,category:"Entry Displays"});addGlobalDisplayType({type:DISPLAY_METADATA,label:"Metadata Table",requiresData:false,category:"Entry Displays"});function RamaddaEntryDisplay(displayManager,id,type,properties){var SUPER;RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,type,properties));this.ramaddas=new Array();var repos=this.getProperty("repositories",this.getProperty("repos",null));if(repos!=null){var toks=repos.split(",");for(var i=0;i<toks.length;i++){var tok=toks[i];tok=tok.trim();this.ramaddas.push(getRamadda(tok));}
...
displayDebug=false;var entriesDivAttrs=[ATTR_ID,this.getDomId(ID_ENTRIES),ATTR_CLASS,this.getClass("content")];var innerHeight=this.getProperty("innerHeight",null);if(innerHeight==null){innerHeight=this.getProperty("entriesHeight",null);}
...
var record=records[index];if(record==null)return;var values=this.getRecordHtml(record,fields);if(source.recordSelectionCallback){var func=source.recordSelectionCallback;if((typeof func)=="string"){func=window[func];}
...
if(props.data!=null){var haveItAlready=false;for(var i=0;i<this.dataList.length;i++){var existingData=this.dataList[i];if(existingData.equals(props.data)){props.data=existingData;haveItAlready=true;break;}}
if(!haveItAlready){this.dataList.push(props.data);}
}
...
var displayId=props.displayId;if(!displayId)
displayId=this.getUniqueId("display");if(props.data==null&&this.dataList.length>0){props.data=this.dataList[0];}
props.createdInteractively=true;if(!props.entryId){props.entryId=this.group.entryId;}
var display=eval(" new "+funcName+"(this,'"+displayId+"', props);");if(display==null){console.log("Error: could not create display using:"+funcName);alert("Error: could not create display using:"+funcName);return;}
if(props.dummy)return display;this.addDisplay(display);return display;},pageHasLoaded:function(display){this.getLayoutManager().pageHasLoaded();},addDisplay:function(display){display.setDisplayManager(this);display.loadInitialData();this.getLayoutManager().addDisplay(display);},notifyEvent:function(func,source,data){this.getLayoutManager().notifyEvent(func,source,data);},removeDisplay:function(display){this.getLayoutManager().removeDisplay(display);this.notifyEvent("handleEventRemoveDisplay",this,display);},setEntry:function(entry){var displays=this.getLayoutManager().getDisplays();for(var i=0;i<displays.length;i++){var display=displays[i];display.setEntry(entry);}},});addDisplayManager(this);var displaysHtml=HtmlUtils.div([ATTR_ID,this.getDomId(ID_DISPLAYS),ATTR_CLASS,"display-container"]);var html=HtmlUtils.openTag(TAG_DIV);html+=HtmlUtils.div(["id",this.getDomId(ID_MENU_CONTAINER)]);html+=this.getEntriesMenu(argProperties);if(this.getShowMenu()){html+=HtmlUtils.tag(TAG_A,[ATTR_CLASS,"display-menu-button",ATTR_ID,this.getDomId(ID_MENU_BUTTON)],"&nbsp;");}
var targetDiv=this.getProperty("target");var _this=this;if(targetDiv!=null){$(document).ready(function(){$("#"+targetDiv).html(displaysHtml);_this.getLayoutManager().doLayout();});}else{html+=displaysHtml;}
html+=HtmlUtils.closeTag(TAG_DIV);$("#"+this.getId()).html(html)
this.initializeEntriesMenu();if(this.showmap){this.createDisplay('map');}
var theDisplayManager=this;$("#"+this.getDomId(ID_MENU_BUTTON)).button({icons:{primary:"ui-icon-gear",secondary:"ui-icon-triangle-1-s"}}).click(function(event){var html=theDisplayManager.makeMainMenu();theDisplayManager.jq(ID_MENU_CONTAINER).html(html);var id=theDisplayManager.getDomId(ID_MENU_OUTER);showPopup(event,theDisplayManager.getDomId(ID_MENU_BUTTON),id,false,null,"left bottom");theDisplayManager.jq(ID_MENU_INNER).superfish({speed:'fast',delay:300});});}
...
var DISPLAY_MAP="map";var DISPLAY_MAPGRID="mapgrid";var displayMapMarkers=["marker.png","marker-blue.png","marker-gold.png","marker-green.png"];var displayMapCurrentMarker=-1;var displayMapUrlToVectorListeners={};var displayMapMarkerIcons={};addGlobalDisplayType({type:DISPLAY_MAP,label:"Map"});addGlobalDisplayType({type:DISPLAY_MAPGRID,label:"Map Grid"});function MapFeature(source,points){RamaddaUtil.defineMembers(this,{source:source,points:points});}
function RamaddaMapDisplay(displayManager,id,properties){var ID_LATFIELD="latfield";var ID_LONFIELD="lonfield";var ID_MAP="map";var ID_SHAPES="shapes";var SUPER;RamaddaUtil.defineMembers(this,{showLocationReadout:false,showBoxes:true,showPercent:false,percentFields:null,kmlLayer:null,kmlLayerName:"",geojsonLayer:null,geojsonLayerName:"",theMap:null});RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_MAP,properties));addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{mapBoundsSet:false,features:[],myMarkers:{},mapEntryInfos:{},sourceToLine:{},sourceToPoints:{},snarf:true,initDisplay:function(){SUPER.initDisplay.call(this);var _this=this;var html="";var extraStyle="min-height:200px;";var width=this.getWidth();if(Utils.isDefined(width)){if(width>0){extraStyle+="width:"+width+"px; ";}else if(width<0){extraStyle+="width:"+(-width)+"%;";}else if(width!=""){extraStyle+="width:"+width+";";}}
var height=this.getProperty("height",300);if(height>0){extraStyle+=" height:"+height+"px; ";}else if(height<0){extraStyle+=" height:"+(-height)+"%; ";}else if(height!=""){extraStyle+=" height:"+(height)+";";}
if(this.getProperty("doAnimation",false)){}
html+=HtmlUtils.div([ATTR_CLASS,"display-map-map","style",extraStyle,ATTR_ID,this.getDomId(ID_MAP)]);if(this.showLocationReadout){html+=HtmlUtils.openTag(TAG_DIV,[ATTR_CLASS,"display-map-latlon"]);html+=HtmlUtils.openTag("form");html+="Latitude: "+
HtmlUtils.input(this.getDomId(ID_LATFIELD),"",["size","7",ATTR_ID,this.getDomId(ID_LATFIELD)]);html+="  ";html+="Longitude: "+
HtmlUtils.input(this.getDomId(ID_LONFIELD),"",["size","7",ATTR_ID,this.getDomId(ID_LONFIELD)]);html+=HtmlUtils.closeTag("form");html+=HtmlUtils.closeTag(TAG_DIV);}
this.setContents(html);if(!this.map){this.createMap();}else{this.map.setMapDiv(this.getDomId(ID_MAP));}
if(!this.haveCalledUpdateUI){var callback=function(){_this.updateUI();}
setTimeout(callback,1);}},checkLayout:function(){if(!this.map){return;}
var d=this.jq(ID_MAP);if(d.width()>0&&this.lastWidth!=d.width()&&this.map){this.lastWidth=d.width();this.map.getMap().updateSize();}},createMap:function(){var theDisplay=this;var params={"defaultMapLayer":this.getProperty("defaultMapLayer",map_default_layer),};var displayDiv=this.getProperty("displayDiv",null);if(displayDiv){params.displayDiv=displayDiv;params.displayDivSticky=this.getProperty("displayDivSticky",false);}
...
params.linked=this.getProperty("linked",false);params.linkGroup=this.getProperty("linkGroup",null);this.map=this.getProperty("theMap",null);if(this.map){this.map.setMapDiv(this.getDomId(ID_MAP));}else{this.map=new RepositoryMap(this.getDomId(ID_MAP),params);this.lastWidth=this.jq(ID_MAP).width();}
if(this.doDisplayMap()){this.map.setDefaultCanSelect(false);}
this.map.initMap(false);this.map.addRegionSelectorControl(function(bounds){theDisplay.getDisplayManager().handleEventMapBoundsChanged(this,bounds,true);});this.map.addFeatureHighlightHandler((feature,highlight)=>{if(feature.record){if(this.lastHighlightedRecord){var args={highlight:false,record:this.lastHighlightedRecord};this.getDisplayManager().notifyEvent("handleEventRecordHighlight",this,args);if(this.getProperty("doAnimation",false)){this.getAnimation().handleEventRecordHighlight(this,args);}
this.lastHighlightedRecord=null;}
if(highlight){this.lastHighlightedRecord=feature.record;}
var args={highlight:highlight,record:feature.record};this.getDisplayManager().notifyEvent("handleEventRecordHighlight",this,args);if(this.getProperty("doAnimation",false)){this.getAnimation().handleEventRecordHighlight(this,args);}}});this.map.doPopup=this.getProperty("doPopup",true);if(!this.map.doPopup)
this.map.doSelect=false;this.map.addClickHandler(this.getDomId(ID_LONFIELD),this.getDomId(ID_LATFIELD),null,this);this.map.getMap().events.register("zoomend","",function(){theDisplay.mapBoundsChanged();});this.map.getMap().events.register("moveend","",function(){theDisplay.mapBoundsChanged();});var overrideBounds=false;if(this.getProperty("bounds")){overrideBounds=true;var toks=this.getProperty("bounds","").split(",");if(toks.length==4){if(this.getProperty("showBounds",false)){var attrs={};if(this.getProperty("boundsColor")){attrs.strokeColor=this.getProperty("boundsColor","");}
...
var boundsAnimation=this.getProperty("boundsAnimation");let _this=this;if(boundsAnimation){this.didAnimationBounds=false;let animationBounds=boundsAnimation.split(",");if(animationBounds.length==4){var pause=parseFloat(this.getProperty("animationPause","1000"));HtmlUtils.callWhenScrolled(this.getDomId(ID_MAP),()=>{if(_this.didAnimationBounds){return;}
...
var toks=this.layerEntries.split(",");for(var i=0;i<toks.length;i++){var tok=toks[i];var url=ramaddaBaseUrl+"/entry/show?output=shapefile.kml&entryid="+tok;this.map.addKMLLayer("layer",url,true,selectCallback,unselectCallback);}}
if(this.showDataLayers()){if(theDisplay.kmlLayer!=null){var url=ramaddaBaseUrl+"/entry/show?output=shapefile.kml&entryid="+theDisplay.kmlLayer;theDisplay.addBaseMapLayer(url,true);}
if(theDisplay.geojsonLayer!=null){url=theDisplay.getRamadda().getEntryDownloadUrl(theDisplay.geojsonLayer);theDisplay.addBaseMapLayer(url,false);}}
if(this.getProperty("latitude")){this.map.setCenter(createLonLat(parseFloat(this.getProperty("longitude",-105)),parseFloat(this.getProperty("latitude",40))));}},addBaseMapLayer:function(url,isKml){var theDisplay=this;mapLoadInfo=displayMapUrlToVectorListeners[url];if(mapLoadInfo==null){mapLoadInfo={otherMaps:[],layer:null};selectFunc=function(layer){theDisplay.mapFeatureSelected(layer);}
var hasBounds=this.getProperty("bounds")!=null;if(isKml)
this.map.addKMLLayer(this.kmlLayerName,url,this.doDisplayMap(),selectFunc,null,null,function(map,layer){theDisplay.baseMapLoaded(layer,url);},!hasBounds);else
this.map.addGeoJsonLayer(this.geojsonLayerName,url,this.doDisplayMap(),selectFunc,null,null,function(map,layer){theDisplay.baseMapLoaded(layer,url);},!hasBounds);}else if(mapLoadInfo.layer){this.cloneLayer(mapLoadInfo.layer);}else{this.map.showLoadingImage();mapLoadInfo.otherMaps.push(this);}},mapFeatureSelected:function(layer){if(!this.getPointData()){return;}
this.map.onFeatureSelect(layer);if(!Utils.isDefined(layer.feature.dataIndex)){return;}
this.getDisplayManager().propagateEventRecordSelection(this,this.getPointData(),{index:layer.feature.dataIndex});},showDataLayers:function(){return this.getProperty("showLayers",true);},doDisplayMap:function(){if(!this.showDataLayers())return false;if(!this.getProperty("displayAsMap",true))return false;return this.kmlLayer!=null||this.geojsonLayer!=null;},cloneLayer:function(layer){var theDisplay=this;this.map.hideLoadingImage();layer=layer.clone();var features=layer.features;var clonedFeatures=[];for(var j=0;j<features.length;j++){feature=features[j];feature=feature.clone();if(feature.style){oldStyle=feature.style;feature.style={};for(var a in oldStyle){feature.style[a]=oldStyle[a];}}
feature.layer=layer;clonedFeatures.push(feature);}
layer.removeAllFeatures();this.map.getMap().addLayer(layer);layer.addFeatures(clonedFeatures);this.vectorLayer=layer;this.applyVectorMap();this.map.addSelectCallback(layer,this.doDisplayMap(),function(layer){theDisplay.mapFeatureSelected(layer);});},handleEventPointDataLoaded:function(source,pointData){},handleEventFieldsSelected:function(source,fields){if(this.getProperty("selectedFieldIsColorBy")&&fields.length>0){this.colorByFieldChanged(fields[0]);}},baseMapLoaded:function(layer,url){this.vectorLayer=layer;this.applyVectorMap();mapLoadInfo=displayMapUrlToVectorListeners[url];if(mapLoadInfo){mapLoadInfo.layer=layer;for(var i=0;i<mapLoadInfo.otherMaps.length;i++){mapLoadInfo.otherMaps[i].cloneLayer(layer);}
...
url=HtmlUtils.appendArg(url,urlArg,attrValue);url=url.replace("${"+urlArg+"}",attrValue);}}}}
url=HtmlUtils.appendArg(url,"output","json");var entryList=new EntryList(this.getRamadda(),url,null,false);entryList.doSearch(this);this.getEntryList().showMessage("Searching",HtmlUtils.div([ATTR_STYLE,"margin:20px;"],this.getWaitImage()));},getEntryList:function(){if(!this.entryListDisplay){var props={showMenu:true,showTitle:true,showDetails:true,layoutHere:false,showForm:false,doSearch:false,};var id=this.getUniqueId("display");this.entryListDisplay=new RamaddaEntrylistDisplay(this.getDisplayManager(),id,props);this.getDisplayManager().addDisplay(this.entryListDisplay);}
...
this.map.addWMSLayer(entry.getName(),baseUrl,layer,false);},mapBoundsChanged:function(){var bounds=this.map.getMap().calculateBounds();bounds=bounds.transform(this.map.sourceProjection,this.map.displayProjection);this.getDisplayManager().handleEventMapBoundsChanged(this,bounds);},addFeature:function(feature){this.features.push(feature);feature.line=this.map.addPolygon("lines_"+
feature.source.getId(),RecordUtil.clonePoints(feature.points),null);},xloadInitialData:function(){if(this.getDisplayManager().getData().length>0){this.handleEventPointDataLoaded(this,this.getDisplayManager().getData()[0]);}},getContentsDiv:function(){return HtmlUtils.div([ATTR_CLASS,"display-contents",ATTR_ID,this.getDomId(ID_DISPLAY_CONTENTS)],"");},highlightMarker:null,handleEventRecordHighlight:function(source,args){SUPER.handleEventRecordHighlight.call(this,source,args);if(!this.map)return;if(this.highlightMarker){this.map.removePoint(this.highlightMarker);this.map.removeMarker(this.highlightMarker);this.highlightMarker=null;}
if(args.highlight){var point=new OpenLayers.LonLat(args.record.getLongitude(),args.record.getLatitude());var attrs={pointRadius:parseFloat(this.getProperty("recordHighlightRadius",+this.getProperty("radius",6)+8)),stroke:true,strokeColor:this.getProperty("recordHighlightStrokeColor","#000"),strokeWidth:parseFloat(this.getProperty("recordHighlightStrokeWidth",2)),fillColor:this.getProperty("recordHighlightFillColor","#ccc"),fillOpacity:parseFloat(this.getProperty("recordHighlightFillOpacity",0.5)),};if(this.getProperty("recordHighlightUseMarker",false)){var size=parseFloat(this.getProperty("recordHighlightRadius",+this.getProperty("radius",24)));this.highlightMarker=this.map.addMarker("pt-"+i,point,null,"pt-"+i,null,null,size);}else{this.highlightMarker=this.map.addPoint(args.record.getId(),point,attrs);}
if(this.getProperty("centerOnHighlight",false)){this.map.setCenter(point);}}},handleEventEntryMouseover:function(source,args){if(!this.map){return;}
...
var justOneMarker=this.getProperty("justOneMarker",false);if(justOneMarker){var pointData=this.getPointData();if(pointData){pointData.handleEventMapClick(this,source,lon,lat);}}
this.getDisplayManager().handleEventMapClick(this,lon,lat);},getPosition:function(){var lat=$("#"+this.getDomId(ID_LATFIELD)).val();var lon=$("#"+this.getDomId(ID_LONFIELD)).val();if(lat==null)
...
var selected=args.selected;if(!entry.hasLocation()){return;}
},addOrRemoveEntryMarker:function(id,entry,add,args){if(!args){args={};}
...
var theDisplay=this;if(mapEntryInfo.marker){mapEntryInfo.marker.entry=entry;mapEntryInfo.marker.ramaddaClickHandler=function(marker){theDisplay.handleMapClick(marker);};if(this.handledMarkers==null){this.map.centerToMarkers();this.handledMarkers=true;}}}
...
this.getDisplayManager().handleEventEntrySelection(this,{entry:marker.entry,selected:true});this.selectedMarker=marker;},applyVectorMap:function(force){if(!force&&this.vectorMapApplied){return;}
if(!this.doDisplayMap()){return;}
if(!this.vectorLayer){return;}
var linkField=this.getFieldById(null,this.getProperty("linkField"));var linkFeature=this.getProperty("linkFeature");var features=this.vectorLayer.features.slice();var recordToFeature={};if(linkFeature&&linkField){var recordMap={};this.points.map(p=>{var record=p.record;if(record){var tuple=record.getData();var value=tuple[linkField.getIndex()];value=value.toString().trim();record.linkValue=value;if(value.indexOf("xxxx0803")>=0)
console.log("record:"+value.replace(/ /g,"X")+":");recordMap[value]=record;}});features.map(feature=>{var attrs=feature.attributes;var ok=false;for(var attr in attrs){if(linkFeature==attr){ok=true;var value=this.map.getAttrValue(attrs,attr);var debug=value.indexOf("xxxx0803")>=0;if(debug)
console.log("A:"+value);if(value){if(debug)
...
var j=0;features.map(feature=>{feature.featureIndex=j++;feature.featureMatched=false;feature.pointCount=0;feature.circles=[];});if(!this.points){return;}
this.vectorMapApplied=true;var maxExtent=null;var circles=this.points;var doCount=this.getProperty("colorByCount",false);for(var j=0;j<features.length;j++){var feature=features[j];}
var matchedFeatures=[];var seen={};var maxCnt=-1;var minCnt=-1;for(var i=0;i<circles.length;i++){var circle=circles[i];if(circle.style&&circle.style.display=="none")continue;var record=circle.record;var center=circle.center;var tmp={index:-1,maxExtent:maxExtent};var matchedFeature=recordToFeature[record.getId()];if(matchedFeature){matchedFeature.featureMatched=true;}
if(!matchedFeature)
matchedFeature=this.findContainingFeature(features,center,tmp);if(!matchedFeature)continue;if(!circle.colorByColor&&circle.hasColorByValue&&isNaN(circle.colorByValue)){continue;}
...
matchedFeature.circles.push(circle);if(doCount){matchedFeature.pointCount++;maxCnt=maxCnt==-1?matchedFeature.pointCount:Math.max(maxCnt,matchedFeature.pointCount);minCnt=minCnt==-1?matchedFeature.pointCount:Math.min(minCnt,matchedFeature.pointCount);}else{if(tmp.index>=0)
features.splice(tmp.index,1);}}
if(!doCount){for(var i=0;i<matchedFeatures.length;i++){var matchedFeature=matchedFeatures[i];style=matchedFeature.style;if(!style)style={"stylename":"from display",};style.display=null;var circle=matchedFeature.circles[0];$.extend(style,circle.style);matchedFeature.style=style;matchedFeature.popupText=circle.text;matchedFeature.dataIndex=i;}}
var prune=this.getProperty("pruneFeatures",true);if(doCount){var colors=this.getColorTable(true);if(colors==null){colors=Utils.ColorTables.grayscale.colors;}
var range=maxCnt-minCnt;var labelSuffix=this.getProperty("doCountLabel","points");for(var j=0;j<features.length;j++){var feature=features[j];var percent=range==0?0:(feature.pointCount-minCnt)/range;var index=parseInt(percent*colors.length);if(index>=colors.length)index=colors.length-1;else if(index<0)index=0;var color=colors[index];var style=feature.style;if(!style)style={"stylename":"from display",};$.extend(style,{fillColor:color,"fillOpacity":0.75,"strokeWidth":1,});if(feature.pointCount==0){if(prune===true){style.display="none";}}
feature.style=style;feature.dataIndex=j;feature.popupText=HtmlUtils.div([],feature.pointCount+"&nbsp;"+labelSuffix);}
this.displayColorTable(colors,ID_BOTTOM,minCnt,maxCnt,{});}else{if(prune){for(var i=0;i<features.length;i++){var feature=features[i];if(feature.featureMatched){continue;}
var style=feature.style;if(!style)style={"stylename":"from display"};$.extend(style,{"display":"none",});feature.style=style;}}
}
this.vectorLayer.redraw();if(maxExtent&&!this.getProperty("bounds")){this.map.getMap().zoomToExtent(maxExtent,true);}},findContainingFeature:function(features,center,info){var matchedFeature=null;for(var j=0;j<features.length;j++){var feature=features[j];var geometry=feature.geometry;if(!geometry){continue;}
bounds=geometry.getBounds();if(!bounds.contains(center.x,center.y)){continue;}
if(geometry.components){for(var sub=0;sub<geometry.components.length;sub++){comp=geometry.components[sub];bounds=comp.getBounds();if(!bounds.contains(center.x,center.y)){continue;}
if(comp.containsPoint&&comp.containsPoint(center)){matchedFeature=feature;geometry=feature.geometry;if(geometry){if(info.maxExtent===null){info.maxExtent=new OpenLayers.Bounds();}
info.maxExtent.extend(geometry.getBounds());}
info.index=j;break;}}
if(matchedFeature)
break;continue;}
if(!geometry.containsPoint){console.log("unknown geometry:"+geometry.CLASS_NAME);continue;}
if(geometry.containsPoint(center)){matchedFeature=feature;info.index=j;break;}}
...
this.map.markers.redraw();this.applyVectorMap(true);},showAllPoints:function(){if(this.lines){for(var i=0;i<this.lines.length;i++){var line=this.lines[i];line.style.display='inline';}
if(this.map.lines)
this.map.lines.redraw();}
if(!this.points)return;for(var i=0;i<this.points.length;i++){var point=this.points[i];point.style.display='inline';}
if(this.map.circles)
this.map.circles.redraw();this.applyVectorMap(true);},colorByFieldChanged:function(field){this.haveCalledUpdateUI=false;this.setProperty("colorBy",field);this.vectorMapApplied=false;this.updateUI();},sizeByFieldChanged:function(field){this.haveCalledUpdateUI=false;this.setProperty("sizeBy",field);this.vectorMapApplied=false;this.updateUI();},dataFilterChanged:function(){this.vectorMapApplied=false;this.updateUI();if(this.getProperty("centerOnFilterChange",false)){if(this.vectorLayer&&this.points){}else{this.map.centerOnMarkers(null,false,false);}}},updateUI:function(){SUPER.updateUI.call(this);if(!this.getDisplayReady()){return;}
if(!this.hasData()){return;}
if(!this.getProperty("showData",true)){return;}
var pointData=this.getPointData();var records=this.filterData();if(records==null){err=new Error();console.log("null records:"+err.stack);return;}
if(this.haveCalledUpdateUI){return;}
this.haveCalledUpdateUI=true;var fields=pointData.getRecordFields();var bounds={};var points=RecordUtil.getPoints(records,bounds);var showSegments=this.getProperty("showSegments",false);if(records.length!=0){if(!isNaN(bounds.north)){this.initBounds=bounds;if(!showSegments){this.setInitMapBounds(bounds.north,bounds.west,bounds.south,bounds.east);}}}
...
this.map.clearSeenMarkers();if(points.length==0){}
var t1=new Date();this.addPoints(records,fields,points);var t2=new Date();this.addLabels(records,fields,points);this.applyVectorMap();},addPoints:function(records,fields,points){var cidx=0
var polygonField=this.getFieldById(fields,this.getProperty("polygonField"));var polygonColorTable=this.getColorTable(true,"polygonColorTable",null);var latlon=this.getProperty("latlon",true);var source=this;var radius=parseFloat(this.getDisplayProp(source,"radius",8));var strokeWidth=parseFloat(this.getDisplayProp(source,"strokeWidth","1"));var strokeColor=this.getDisplayProp(source,"strokeColor","#000");var sizeByAttr=this.getDisplayProp(source,"sizeBy",null);var isTrajectory=this.getDisplayProp(source,"isTrajectory",false);if(isTrajectory){var attrs={strokeWidth:2,strokeColor:"blue"}
this.map.addPolygon("id","",points,attrs,null);return;}
var latField1=this.getFieldById(fields,this.getProperty("latField1"));var latField2=this.getFieldById(fields,this.getProperty("latField2"));var lonField1=this.getFieldById(fields,this.getProperty("lonField1"));var lonField2=this.getFieldById(fields,this.getProperty("lonField2"));var sizeSegments=this.getProperty("sizeSegments",false);var sizeEndPoints=this.getProperty("sizeEndPoints",true);var showEndPoints=this.getProperty("showEndPoints",false);var endPointSize=parseInt(this.getProperty("endPointSize","4"));var dfltEndPointSize=endPointSize;var segmentWidth=parseInt(this.getProperty("segmentWidth","1"));var dfltSegmentWidth=segmentWidth;var showPoints=this.getProperty("showPoints",true);var lineColor=this.getProperty("lineColor","green");var pointIcon=this.getProperty("pointIcon");if(pointIcon)this.pointsAreMarkers=true;var iconField=this.getFieldById(fields,this.getProperty("iconField"));var iconSize=parseFloat(this.getProperty("iconSize",32));var iconMap=this.getIconMap();if(iconField||iconMap)
this.pointsAreMarkers=true;var dfltShape=this.getProperty("defaultShape",null);var dfltShapes=["circle","triangle","star","square","cross","x","lightning","rectangle","church"];var dfltShapeIdx=0;var shapeBy={id:this.getDisplayProp(source,"shapeBy",null),field:null,map:{}}
if(this.getDisplayProp(source,"shapeByMap",null)){this.getDisplayProp(source,"shapeByMap",null).split(",").map((pair)=>{var tuple=pair.split(":");shapeBy.map[tuple[0]]=tuple[1];})}
var colorBy=this.getColorByInfo(records);var sizeBy={id:this.getDisplayProp(source,"sizeBy",null),minValue:0,maxValue:0,field:null,index:-1,isString:false,stringMap:{},};var sizeByMap=this.getProperty("sizeByMap");if(sizeByMap){var toks=sizeByMap.split(",");for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){sizeBy.stringMap[toks2[0]]=toks2[1];}}}
for(var i=0;i<fields.length;i++){var field=fields[i];if(field.getId()==shapeBy.id||("#"+(i+1))==shapeBy.id){shapeBy.field=field;if(field.isString())shapeBy.isString=true;}
if(field.getId()==sizeBy.id||("#"+(i+1))==sizeBy.id){sizeBy.field=field;if(field.isString())sizeBy.isString=true;}}
if(this.getProperty("showColorByMenu",false)&&colorBy.field&&!this.madeColorByMenu){this.madeColorByMenu=true;var menu="<select class='ramadda-pulldown' id='"+this.getDomId("colorByMenu")+"'>";for(var i=0;i<fields.length;i++){var field=fields[i];if(!field.isNumeric()||field.isFieldGeo())continue;var extra="";if(colorBy.field.getId()==field.getId())extra="selected ";menu+="<option value='"+field.getId()+"' "+extra+">"+field.getLabel()+"</option>\n";}
menu+="</select>";this.writeHtml(ID_TOP_RIGHT,"Color by: "+menu);this.jq("colorByMenu").change(()=>{var value=this.jq("colorByMenu").val();this.vectorMapApplied=false;this.haveCalledUpdateUI=false;this.setProperty("colorBy",value);this.updateUI();});}
sizeBy.index=sizeBy.field!=null?sizeBy.field.getIndex():-1;shapeBy.index=shapeBy.field!=null?shapeBy.field.getIndex():-1;var dateMin=null;var dateMax=null;var dates=[];var justOneMarker=this.getProperty("justOneMarker",false);for(var i=0;i<records.length;i++){var pointRecord=records[i];dates.push(pointRecord.getDate());if(dateMin==null){dateMin=pointRecord.getDate();dateMax=pointRecord.getDate();}else{var date=pointRecord.getDate();if(date){if(date.getTime()<dateMin.getTime())
dateMin=date;if(date.getTime()>dateMax.getTime())
dateMax=date;}}
var tuple=pointRecord.getData();if(!sizeBy.isString){var v=tuple[sizeBy.index];if(!isNaN(v)&&!(v===null)){if(i==0||v>sizeBy.maxValue)sizeBy.maxValue=v;if(i==0||v<sizeBy.minValue)sizeBy.minValue=v;}}}
sizeBy.radiusMin=parseFloat(this.getProperty("sizeByRadiusMin",-1));sizeBy.radiusMax=parseFloat(this.getProperty("sizeByRadiusMax",-1));var sizeByOffset=0;var sizeByLog=this.getProperty("sizeByLog",false);var sizeByFunc=Math.log;if(sizeByLog){if(sizeBy.minValue<1){sizeByOffset=1-sizeBy.minValue;}
sizeBy.minValue=sizeByFunc(sizeBy.minValue+sizeByOffset);sizeBy.maxValue=sizeByFunc(sizeBy.maxValue+sizeByOffset);}
sizeBy.range=sizeBy.maxValue-sizeBy.minValue;if(dateMax){this.getAnimation().init(dateMin,dateMax,records);}
if(this.points){for(var i=0;i<this.points.length;i++){if(this.pointsAreMarkers)
this.map.removeMarker(this.points[i]);else
this.map.removePoint(this.points[i]);}
this.points=[];}
if(this.lines){for(var i=0;i<this.lines.length;i++)
this.map.removePolygon(this.lines[i]);this.lines=[];}
if(!this.points){this.points=[];this.lines=[];}
var dontAddPoint=this.doDisplayMap();var didColorBy=false;var seen={};var lastPoint;var pathAttrs={strokeColor:this.getProperty("pathColor",lineColor),strokeWidth:this.getProperty("pathWidth",1)};var fillColor=this.getProperty("fillColor");var fillOpacity=this.getProperty("fillOpacity",0.8);var isPath=this.getProperty("isPath",false);var showSegments=this.getProperty("showSegments",false);var tooltip=this.getProperty("tooltip");var highlight=this.getProperty("highlight");var addedPoints=[];for(var i=0;i<records.length;i++){var record=records[i];var tuple=record.getData();if(!record.point)
record.point=new OpenLayers.Geometry.Point(record.getLongitude(),record.getLatitude());var point=record.point;if(justOneMarker){if(this.justOneMarker)
...
var values=record.getData();var props={pointRadius:radius,strokeWidth:strokeWidth,strokeColor:strokeColor,fillColor:fillColor,fillOpacity:fillOpacity};if(shapeBy.field){var gv=values[shapeBy.index];if(gv){if(!Utils.isDefined(shapeBy.map[gv])){if(dfltShape){shapeBy.map[gv]=dfltShape;}else{if(dfltShapeIdx>=dfltShapes.length)
dfltShapeIdx=0;shapeBy.map[gv]=dfltShapes[dfltShapeIdx++];}}
if(Utils.isDefined(shapeBy.map[gv])){props.graphicName=shapeBy.map[gv];}}}
if(sizeBy.index>=0){var value=values[sizeBy.index];if(sizeBy.isString){if(Utils.isDefined(sizeBy.stringMap[value])){var v=parseInt(sizeBy.stringMap[value]);segmentWidth=dfltSegmentWidth+v;props.pointRadius=v;}else if(Utils.isDefined(sizeBy.stringMap["*"])){var v=parseInt(sizeBy.stringMap["*"]);segmentWidth=dfltSegmentWidth+v;props.pointRadius=v;}else{segmentWidth=dfltSegmentWidth;}}else{var denom=sizeBy.range;var v=value+sizeByOffset;if(sizeByLog)v=sizeByFunc(v);var percent=(denom==0?NaN:(v-sizeBy.minValue)/denom);if(sizeBy.radiusMax>=0&&sizeBy.radiusMin>=0){props.pointRadius=Math.round(sizeBy.radiusMin+percent*(sizeBy.radiusMax-sizeBy.radiusMin));}else{props.pointRadius=6+parseInt(15*percent);}
if(sizeEndPoints){endPointSize=dfltEndPointSize+parseInt(10*percent);}
if(sizeSegments){segmentWidth=dfltSegmentWidth+parseInt(10*percent);if(segmentWidth==0||isNaN(segmentWidth))segmentWidth=1;}}}
if(isNaN(props.pointRadius)||props.pointRadius==0)props.pointRadius=radius;var hasColorByValue=false;var colorByValue;var colorByColor;var theColor=null;if(colorBy.compareFields.length>0){var maxColor=null;var maxValue=0;colorBy.compareFields.map((f,idx)=>{var value=record.getData()[f.getIndex()];if(idx==0||value>maxValue){maxColor=colorBy.colors[idx];maxValue=value;}});colorByValue=maxValue;theColor=maxColor;}else if(colorBy.index>=0){var value=record.getData()[colorBy.index];colorByValue=value;theColor=colorBy.getColor(value,record);}
if(theColor){didColorBy=true;hasColorByValue=true;colorByColor=props.fillColor=colorBy.convertColor(theColor,colorByValue);}
var html=this.getRecordHtml(record,fields,tooltip);if(polygonField){var s=values[polygonField.getIndex()];var delimiter;[";",","].map(d=>{if(s.indexOf(d)>=0)delimiter=d;});var toks=s.split(delimiter);var polygonProps={};$.extend(polygonProps,props);if(polygonProps.strokeWidth==0)
...
for(var pIdx=2;pIdx<toks.length;pIdx+=2){var p=[];var lat1=parseFloat(toks[pIdx-2]);var lon1=parseFloat(toks[pIdx-1]);var lat2=parseFloat(toks[pIdx]);var lon2=parseFloat(toks[pIdx+1]);if(!latlon){var tmp=lat1;lat1=lon1;lon1=tmp;var tmp=lat2;lat2=lon2;lon2=tmp;}
p.push(new OpenLayers.Geometry.Point(lon1,lat1));p.push(new OpenLayers.Geometry.Point(lon2,lat2));var poly=this.map.addPolygon("polygon"+pIdx,"",p,polygonProps,html);if(date){poly.date=date.getTime();}
this.lines.push(poly);}}
if(showSegments&&latField1&&latField2&&lonField1&&lonField2){var lat1=values[latField1.getIndex()];var lat2=values[latField2.getIndex()];var lon1=values[lonField1.getIndex()];var lon2=values[lonField2.getIndex()];var attrs={};if(props.fillColor)
attrs.strokeColor=props.fillColor;else
attrs.strokeColor=lineColor;attrs.strokeWidth=segmentWidth;this.lines.push(this.map.addLine("line-"+i,"",lat1,lon1,lat2,lon2,attrs,html));if(showEndPoints){var pointProps={};$.extend(pointProps,props);pointProps.fillColor=attrs.strokeColor;pointProps.strokeColor=attrs.strokeColor;pointProps.pointRadius=dfltEndPointSize;pointProps.pointRadius=endPointSize;var p1=new OpenLayers.LonLat(lon1,lat1);var p2=new OpenLayers.LonLat(lon2,lat2);if(!Utils.isDefined(seen[p1])){seen[p1]=true;this.points.push(this.map.addPoint("endpt-"+i,p1,pointProps,html));}
if(!Utils.isDefined(seen[p2])){seen[p2]=true;this.points.push(this.map.addPoint("endpt2-"+i,p2,pointProps,html));}}}
if(showPoints){if(!seen[point])seen[point]=0;if(seen[point]>500){continue;}
seen[point]++;var mapPoint=null;if(iconField){var icon=tuple[iconField.getIndex()];if(iconMap){icon=iconMap[icon];if(!icon)icon=this.getMarkerIcon();}
var size=iconSize;if(sizeBy.index>=0){size=props.pointRadius;}
mapPoint=this.map.addMarker("pt-"+i,point,icon,"pt-"+i,html,null,size);}else if(pointIcon){mapPoint=this.map.addMarker("pt-"+i,point,pointIcon,"pt-"+i,html,null,props.pointRadius);}else{if(!props.graphicName)
props.graphicName=this.getProperty("shape","circle");if(radius>0){mapPoint=this.map.addPoint("pt-"+i,point,props,html,dontAddPoint);}}
if(isPath&&lastPoint){this.lines.push(this.map.addLine("line-"+i,"",lastPoint.y,lastPoint.x,point.y,point.x,pathAttrs));}
lastPoint=point;var date=record.getDate();if(mapPoint){if(highlight)
mapPoint.highlightText=this.getRecordHtml(record,fields,highlight);mapPoint.record=record;mapPoint.hasColorByValue=hasColorByValue;mapPoint.colorByValue=colorByValue;mapPoint.colorByColor=colorByColor;if(date){mapPoint.date=date.getTime();}
this.points.push(mapPoint);}}}
if(showSegments){this.map.centerOnMarkers(null,true,null);}
if(this.map.circles)
this.map.circles.redraw();this.jq(ID_BOTTOM).append(HtmlUtils.div(["id",this.getDomId(ID_SHAPES)]));if(didColorBy){colorBy.displayColorTable();}
if(iconField&&iconMap){var html="";for(a in iconMap){html+=HtmlUtils.image(iconMap[a],["width","32"])+" "+a+" ";}
this.jq(ID_SHAPES).html("<center>"+html+"</center>");}
if(shapeBy.field){var shapes=shapeBy.field.getLabel()+": ";for(v in shapeBy.map){var shape=shapeBy.map[v];if(shape=="circle")shape=HtmlUtils.getIconImage("fa-circle");else if(shape=="square")shape=HtmlUtils.getIconImage("fa-square");else if(shape=="rectangle")shape=HtmlUtils.getIconImage("fa-square");else if(shape=="star")shape=HtmlUtils.getIconImage("fa-star");else if(shape=="triangle")shape=HtmlUtils.getIconImage("/icons/triangle.png",["width","16px"]);else if(shape=="lightning")shape=HtmlUtils.getIconImage("/icons/lightning.png",["width","16px"]);else if(shape=="cross")shape=HtmlUtils.getIconImage("/icons/cross.png",["width","16px"]);else if(shape=="church")shape=HtmlUtils.getIconImage("fa-cross");shapes+=shape+" "+v+"&nbsp;&nbsp;"}
this.jq(ID_SHAPES).html("<center>"+shapes+"</center>");}
if(this.getProperty("animationTakeStep",false)){this.getAnimation().doNext();}},getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Map Attributes","defaultMapLayer =\"\"","showLocationSearch=\"true\"","strokeWidth=1","strokeColor=\"#000\"","fillColor=\"\"","radius=\"5\"","shape=\"star|cross|x|square|triangle|circle|lightning|church\"","colorBy=\"\"","colorByLog=\"true\"","colorByMap=\"value1:color1,...,valueN:colorN\"","sizeBy=\"\"","sizeByLog=\"true\"","sizeByMap=\"value1:size,...,valueN:size\"",'sizeByRadiusMin="2"','sizeByRadiusMax="20"',"boundsAnimation=\"true\"","centerOnFilterChange=\"true\"","markerIcon=\"/icons/...\"","showSegments=\"true\"",'showRecordSelection=false','convertColorIntensity=true','intensitySourceMin=0','intensitySourceMax=100','intensityTargetMin=1','intensityTargetMax=0','convertColorAlpha=true','alphaSourceMin=0','alphaSourceMax=100','alphaTargetMin=0','alphaTargetMax=1',]);},addLabels:function(records,fields,points){var labelTemplate=this.getProperty("labelTemplate");if(!labelTemplate)return;if(labelTemplate){labelTemplate=labelTemplate.replace(/_nl_/g,"\n");if(!this.map.labelLayer){this.map.labelLayer=new OpenLayers.Layer.Vector("Labels",{styleMap:new OpenLayers.StyleMap({'default':{label:labelTemplate,fontColor:this.getProperty("labelFontColor","#000"),fontSize:this.getProperty("labelFontSize","12px"),fontFamily:this.getProperty("labelFontFamily","'Open Sans', Helvetica Neue, Arial, Helvetica, sans-serif"),fontWeight:this.getProperty("labelFontWeight","plain"),labelAlign:this.getProperty("labelAlign","lb"),labelXOffset:this.getProperty("labelXOffset","0"),labelYOffset:this.getProperty("labelYOffset","0"),labelOutlineColor:this.getProperty("labelOutlineColor","#fff"),labelOutlineWidth:this.getProperty("labelOutlineWidth","0"),labelSelect:true,}}),});this.map.addVectorLayer(this.map.labelLayer,true);this.map.labelLayer.setZIndex(100);}}
var features=[];var seen={};var colorBy=this.getProperty("colorBy");var sizeBy=this.getProperty("sizeBy");for(var i=0;i<records.length;i++){var record=records[i];var point=record.point;if(seen[point])continue;seen[point]=true;var center=new OpenLayers.Geometry.Point(point.x,point.y);center.transform(this.map.displayProjection,this.map.sourceProjection);var tuple=record.getData();var pointFeature=new OpenLayers.Feature.Vector(center);pointFeature.noSelect=true;pointFeature.attributes={};pointFeature.attributes["recordIndex"]=(i+1);for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];pointFeature.attributes[field.getId()]=field.getValue(tuple);if(colorBy&&field.getId()==colorBy){pointFeature.attributes["colorBy"]=field.getValue(tuple);}
if(sizeBy&&field.getId()==sizeBy){pointFeature.attributes["sizeBy"]=field.getValue(tuple);}}
features.push(pointFeature);}
if(this.labelFeatures)
this.map.labelLayer.removeFeatures(this.labelFeatures);this.map.labelLayer.addFeatures(features);this.labelFeatures=features;$("#"+this.map.labelLayer.id).css("z-index",1000);},handleEventRemoveDisplay:function(source,display){if(!this.map){return;}
...
return icon;}
displayMapCurrentMarker++;if(displayMapCurrentMarker>=displayMapMarkers.length)displayMapCurrentMarker=0;return ramaddaBaseUrl+"/lib/openlayers/v2/img/"+displayMapMarkers[displayMapCurrentMarker];},handleEventRecordSelection:function(source,args){if(!this.map){return;}
args.highlight=true;this.handleEventRecordHighlight(source,args);return;if(!this.getProperty("showRecordSelection",true))return;var record=args.record;if(record.hasLocation()){var latitude=record.getLatitude();var longitude=record.getLongitude();if(latitude<-90||latitude>90||longitude<-180||longitude>180)return;var point=new OpenLayers.LonLat(longitude,latitude);var marker=this.myMarkers[source];if(marker!=null){this.map.removeMarker(marker);}
...
function RamaddaMapgridDisplay(displayManager,id,properties){let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_MAPGRID,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{needsData:function(){return true;},getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Grid Map Attributes",'stateField=""','cellSize="20"','showCellLabel=false',]);},handleEventFieldsSelected:function(source,fields){if(this.getProperty("selectedFieldIsColorBy")&&fields.length>0){this.colorByFieldChanged(fields[0]);}},colorByFieldChanged:function(field){this.haveCalledUpdateUI=false;this.setProperty("colorBy",field);this.vectorMapApplied=false;this.updateUI();},updateUI:function(){var pointData=this.getData();if(pointData==null)return;var records=this.filterData();if(!records)return;var fields=this.getData().getNonGeoFields();var stateField=this.getFieldById(fields,this.getProperty("stateField"));if(stateField==null){stateField=this.getFieldById(fields,"state");}
var minx=Number.MAX_VALUE;var miny=Number.MAX_VALUE;var maxx=Number.MIN_VALUE;var maxy=Number.MIN_VALUE;var map={};this.grid.map(o=>{minx=Math.min(minx,o.x);maxx=Math.max(maxx,o.x);miny=Math.min(miny,o.y);maxy=Math.max(maxy,o.y);map[o.x+"_"+o.y]=o;});var table="<table border=0 cellspacing=0 cellpadding=0>";var w=this.getProperty("cellSize","40");var showLabel=this.getProperty("showCellLabel",true);var cellStyle=this.getProperty("cellStyle","");var cellMap={};for(var y=1;y<=maxy;y++){table+="<tr>";for(var x=1;x<=maxx;x++){var id=x+"_"+y;var o=map[id];var extra=" id='"+id+"' ";var style="margin:1px;vertical-align:center;text-align:center;width:"+w+"px;"+"height:"+w+"px;";var c="";if(o){style+="background:#ccc;"+cellStyle;extra+=" title='"+o.state+"' ";extra+=" class='display-mapgrid-cell' ";c="<div>"+(showLabel?o.code:"")+"</div>";cellMap[o.code]=id;cellMap[o.state]=id;}
var td="<td>"+"<div "+extra+" style='"+style+"'>"+c+"</div></td>";table+=td;}
table+="</tr>";}
table+="<tr><td colspan="+maxx+"><br>"+HtmlUtils.div(["id",this.getDomId(ID_COLORTABLE)])+"</td></tr>";table+="</table>";var colorBy=this.getColorByInfo(records);var strokeColorBy=this.getColorByInfo(records,"strokeColorBy","strokeColorByMap");this.writeHtml(ID_DISPLAY_CONTENTS,table);var contents=this.jq(ID_DISPLAY_CONTENTS);for(var i=0;i<records.length;i++){var record=records[i];var tuple=record.getData();var state=tuple[stateField.getIndex()];var cellId=cellMap[state];if(!cellId){cellId=cellMap[state.toUpperCase()];}
if(!cellId){console.log("Could not find cell:"+state);continue;}
if(colorBy.index>=0){var value=record.getData()[colorBy.index];var color=colorBy.getColor(value,record);var cell=contents.find("#"+cellId);cell.css("background",color);cell.attr("recordIndex",i);}
if(strokeColorBy.index>=0){var value=record.getData()[strokeColorBy.index];var color=strokeColorBy.getColor(value,record);var cell=contents.find("#"+cellId);cell.css("border-color",color);cell.css("border-width","2px");}}
this.makePopups(contents.find(".display-mapgrid-cell"),records);let _this=this;contents.find(".display-mapgrid-cell").click(function(){var record=records[$(this).attr("recordIndex")];if(record){_this.getDisplayManager().notifyEvent("handleEventRecordSelection",_this,{record:record});}});if(colorBy.index>=0){colorBy.displayColorTable();}},handleEventRecordSelection:function(source,args){var contents=this.jq(ID_DISPLAY_CONTENTS);if(this.selectedCell){this.selectedCell.css("border",this.selectedBorder);}
var index=this.recordToIndex[args.record.getId()];if(!Utils.isDefined(index))return;this.selectedCell=contents.find("[recordIndex='"+index+"']");this.selectedBorder=this.selectedCell.css("border");this.selectedCell.css("border","1px solid red");},grid:[{state:"Alaska",code:"AK",x:2,y:1},{state:"Hawaii",code:"HI",x:2,y:8},{state:"Washington",code:"WA",x:3,y:3},{state:"Oregon",code:"OR",x:3,y:4},{state:"California",code:"CA",x:3,y:5},{state:"Idaho",code:"ID",x:4,y:3},{state:"Nevada",code:"NV",x:4,y:4},{state:"Utah",code:"UT",x:4,y:5},{state:"Arizona",code:"AZ",x:4,y:6},{state:"Montana",code:"MT",x:5,y:3},{state:"Wyoming",code:"WY",x:5,y:4},{state:"Colorado",code:"CO",x:5,y:5},{state:"New Mexico",code:"NM",x:5,y:6},{state:"North Dakota",code:"ND",x:6,y:3},{state:"South Dakota",code:"SD",x:6,y:4},{state:"Nebraska",code:"NE",x:6,y:5},{state:"Kansas",code:"KS",x:6,y:6},{state:"Oklahoma",code:"OK",x:6,y:7},{state:"Texas",code:"TX",x:6,y:8},{state:"Minnesota",code:"MN",x:7,y:3},{state:"Iowa",code:"IA",x:7,y:4},{state:"Missouri",code:"MO",x:7,y:5},{state:"Arkansas",code:"AR",x:7,y:6},{state:"Louisiana",code:"LA",x:7,y:7},{state:"Illinois",code:"IL",x:8,y:3},{state:"Indiana",code:"IN",x:8,y:4},{state:"Kentucky",code:"KY",x:8,y:5},{state:"Tennessee",code:"TN",x:8,y:6},{state:"Mississippi",code:"MS",x:8,y:7},{state:"Wisconsin",code:"WI",x:9,y:2},{state:"Ohio",code:"OH",x:9,y:4},{state:"West Virginia",code:"WV",x:9,y:5},{state:"North Carolina",code:"NC",x:9,y:6},{state:"Alabama",code:"AL",x:9,y:7},{state:"Michigan",code:"MI",x:9,y:3},{state:"Pennsylvania",code:"PA",x:10,y:4},{state:"Virginia",code:"VA",x:10,y:5},{state:"South Carolina",code:"SC",x:10,y:6},{state:"Georgia",code:"GA",x:10,y:7},{state:"New York",code:"NY",x:11,y:3},{state:"New Jersey",code:"NJ",x:11,y:4},{state:"Maryland",code:"MD",x:11,y:5},{state:"DC",code:"DC",x:11,y:6},{state:"Florida",code:"FL",x:11,y:8},{state:"Vermont",code:"VT",x:12,y:2},{state:"Rhode Island",code:"RI",x:12,y:3},{state:"Connecticut",code:"CT",x:12,y:4},{state:"Delaware",code:"DE",x:12,y:5},{state:"Maine",code:"ME",x:13,y:1},{state:"New Hampshire",code:"NH",x:13,y:2},{state:"Massachusetts",code:"MA",x:13,y:3},]})}
var DISPLAY_GRAPH="graph";var DISPLAY_TREE="tree";var DISPLAY_ORGCHART="orgchart";var DISPLAY_TIMELINE="timeline";var DISPLAY_BLANK="blank";var DISPLAY_RECORDS="records";var DISPLAY_TSNE="tsne";var DISPLAY_HEATMAP="heatmap";var DISPLAY_CROSSTAB="crosstab";var DISPLAY_CORRELATION="correlation";var DISPLAY_RANKING="ranking";var DISPLAY_STATS="stats";var DISPLAY_COOCCURENCE="cooccurence";var DISPLAY_BOXTABLE="boxtable";addGlobalDisplayType({type:DISPLAY_RANKING,label:"Ranking",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_CORRELATION,label:"Correlation",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_CROSSTAB,label:"Crosstab",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_STATS,label:"Stats Table",requiresData:false,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_RECORDS,label:"Records",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_TSNE,label:"TSNE",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_HEATMAP,label:"Heatmap",requiresData:true,forUser:true,category:CATEGORY_MISC});addGlobalDisplayType({type:DISPLAY_GRAPH,label:"Graph",requiresData:true,forUser:true,category:"Misc"});addGlobalDisplayType({type:DISPLAY_TREE,forUser:true,label:"Tree",requiresData:false,category:"Misc"});addGlobalDisplayType({type:DISPLAY_ORGCHART,label:"Org Chart",requiresData:true,forUser:true,category:"Misc"});addGlobalDisplayType({type:DISPLAY_TIMELINE,label:"Timeline",requiresData:true,forUser:true,category:"Misc"});addGlobalDisplayType({type:DISPLAY_BLANK,label:"Blank",requiresData:true,forUser:true,category:"Misc"});addGlobalDisplayType({type:DISPLAY_COOCCURENCE,label:"Cooccurence",requiresData:true,forUser:true,category:"Misc"});addGlobalDisplayType({type:DISPLAY_BOXTABLE,label:"Box Table",requiresData:true,forUser:true,category:"Misc"});function RamaddaGraphDisplay(displayManager,id,properties){const ID_GRAPH="graph";var SUPER;RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_GRAPH,properties));addRamaddaDisplay(this);if(!window["ForceGraph"]){Utils.importJS("https://unpkg.com/force-graph");}
RamaddaUtil.defineMembers(this,{needsData:function(){return true;},callbackWaiting:false,getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Graph Attributes",'sourceField=""','targetField=""','nodeBackground="#ccc"','drawCircle="true"','nodeWidth="10"','linkColor="red"','linkWidth="3"','linkDash="5"','linkWidth="3"','arrowLength="6"','arrowColor="green"','directionalParticles="2"'])},updateUI:function(){if(!window["ForceGraph"]){if(!this.callbackWaiting){this.callbackWaiting=true;setTimeout(()=>{this.callbackWaiting=false;this.updateUI()},100);}
return;}
let graphData=null;let html=HtmlUtils.div(["id",this.getDomId(ID_GRAPH)]);this.jq(ID_DISPLAY_CONTENTS).html(html);if(doGlobalGraphData){if(!globalGraphData){setTimeout(()=>{this.updateUI();},100);}
graphData=globalGraphData;}else{var records=this.filterData();if(!records){return;}
let seenNodes={};let nodes=[];let links=[];let valueFields=this.getFieldsByIds(null,this.getProperty("valueFields","",true));let labelField=this.getFieldById(null,this.getProperty("labelField"));if(!labelField){var strings=this.getFieldsOfType(null,"string");if(strings.length>0)labelField=strings[0];}
let sourceField=this.getFieldById(null,this.getProperty("sourceField","source"));let targetField=this.getFieldById(null,this.getProperty("targetField","target"));var textTemplate=this.getProperty("tooltip","${default}");if(valueFields.length>0){let seenValue={};records.map((r,index)=>{var label=labelField?r.getValue(labelField.getIndex()):index;var tooltip=this.getRecordHtml(r,null,textTemplate);nodes.push({id:index,label:label,tooltip:tooltip});valueFields.map(f=>{let value=r.getValue(f.getIndex());if(!seenValue[value+"_"+f.getId()]){seenValue[value+"_"+f.getId()]=true;nodes.push({id:value,isValue:true});}
links.push({source:value,target:index});});});}else if(sourceField!=null&&targetField!=null){records.map(r=>{var source=r.getValue(sourceField.getIndex());var target=r.getValue(targetField.getIndex());if(!seenNodes[source]){seenNodes[source]=true;nodes.push({id:source,tooltip:source});}
...
function RamaddaTreeDisplay(displayManager,id,properties){let SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_TREE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{countToRecord:{},needsData:function(){return true;},getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Tree Attributes","maxDepth=3","showDetails=false",])},updateUI:function(){var records=this.filterData();if(!records)return;let roots=null;try{roots=this.makeTree(records);}catch(error){this.setContents(this.getMessage(error.toString()));return;}
var html="";let baseId=this.getDomId("node");let cnt=0;let depth=0;let maxDepth=+this.getProperty("maxDepth",10);let template=this.getProperty("recordTemplate","${default}");var showDetails=this.getProperty("showDetails",true);let _this=this;let func=function(node){cnt++;if(node.record){_this.countToRecord[cnt]=node.record;}
...
let image="";if(node.children.length>0||details){image=HtmlUtils.image(on?icon_downdart:icon_rightdart,["id",baseId+"_toggle_image"+cnt])+" ";}
html+=HtmlUtils.div(["class","display-tree-toggle","id",baseId+"_toggle"+cnt,"toggle-state",on,"block-count",cnt],image+node.label);html+=HtmlUtils.openTag("div",["id",baseId+"_block"+cnt,"class","display-tree-block","style","display:"+(on?"block":"none")]);if(details&&details!=""){if(node.children.length>0){html+=HtmlUtils.div(["class","display-tree-toggle-details","id",baseId+"_toggle_details"+cnt,"toggle-state",false,"block-count",cnt],HtmlUtils.image(icon_rightdart,["id",baseId+"_toggle_details_image"+cnt])+" Details");html+=HtmlUtils.div(["id",baseId+"_block_details"+cnt,"class","display-tree-block","style","display:none"],details);}else{html+=details;}}
if(node.children.length>0){node.children.map(func);}
depth--;html+=HtmlUtils.closeTag("div");}
roots.map(func);this.myRecords=[];this.displayHtml(html);this.jq(ID_DISPLAY_CONTENTS).find(".display-tree-toggle").click(function(){var state=(/true/i).test($(this).attr("toggle-state"));state=!state;var cnt=$(this).attr("block-count");var block=$("#"+baseId+"_block"+cnt);var img=$("#"+baseId+"_toggle_image"+cnt);$(this).attr("toggle-state",state);if(state){block.css("display","block");img.attr("src",icon_downdart);}else{block.css("display","none");img.attr("src",icon_rightdart);}
let record=_this.countToRecord[cnt];if(record){_this.getDisplayManager().notifyEvent("handleEventRecordSelection",this,{record:record});}});this.jq(ID_DISPLAY_CONTENTS).find(".display-tree-toggle-details").click(function(){var state=(/true/i).test($(this).attr("toggle-state"));state=!state;var cnt=$(this).attr("block-count");var block=$("#"+baseId+"_block_details"+cnt);var img=$("#"+baseId+"_toggle_details_image"+cnt);$(this).attr("toggle-state",state);if(state){block.css("display","block");img.attr("src",icon_downdart);}else{block.css("display","none");img.attr("src",icon_rightdart);}});},});}
function OrgchartDisplay(displayManager,id,properties){let ID_ORGCHART="orgchart";let SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_ORGCHART,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{handleEventRecordSelection:function(source,args){},getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),['label:Orgchart Attributes','labelField=""','parentField=""','idField=""','treeRoot="some label"','treeTemplate=""','treeNodeSize="small|medium|large"'])},needsData:function(){return true;},updateUI:function(){if(!waitOnGoogleCharts(this,()=>{this.updateUI();})){return;}
this.displayHtml(HtmlUtils.div(["id",this.getDomId(ID_ORGCHART)],"HELLO"));if(this.jq(ID_ORGCHART).length==0){setTimeout(()=>this.updateUI(),1000);return;}
let roots=null;try{roots=this.makeTree();}catch(error){this.setContents(this.getMessage(error.toString()));return;}
var data=new google.visualization.DataTable();data.addColumn('string','Name');data.addColumn('string','Parent');data.addColumn('string','ToolTip');let rows=[];let func=function(node){cnt++;var value=node.label;if(node.display){value={'v':node.label,f:node.display};}
var row=[value,node.parent?node.parent.label:"",node.tooltip||""];rows.push(row);if(node.children.length>0){node.children.map(func);}
if(node.record){}}
roots.map(func);data.addRows(rows);var chart=new google.visualization.OrgChart(document.getElementById(this.getDomId(ID_ORGCHART)));chart.draw(data,{'allowHtml':true,'allowCollapse':true,'size':this.getProperty("treeNodeSize","medium")});}});}
function RamaddaTimelineDisplay(displayManager,id,properties){var ID_TIMELINE="timeline";let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_TIMELINE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);Utils.importJS(ramaddaBaseUrl+"/lib/timeline3/timeline.js");var css="https://cdn.knightlab.com/libs/timeline3/latest/css/timeline.css";$('<link rel="stylesheet" href="'+css+'" type="text/css" />').appendTo("head");$.extend(this,{needsData:function(){return true;},getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Timeline",'justTimeline="true"','titleField=""','startDateField=""','endDateField=""','textTemplate=""','timeTo="year|day|hour|second"','hideBanner="true"','startAtEnd=true','navHeight=250']);},loadCnt:0,timelineLoaded:false,updateUI:function(){if(!this.timelineLoaded){try{var tmp=TL.Timeline;this.timelineLoaded=true;}catch(err){if(this.loadCnt++<100){setTimeout(()=>this.updateUI(),100);return;}}}
if(!this.timelineLoaded){this.writeHtml(ID_DISPLAY_CONTENTS,"Could not load timeline");return;}
var records=this.filterData();if(records==null)return;var timelineId=this.getDomId(ID_TIMELINE);this.writeHtml(ID_DISPLAY_CONTENTS,HtmlUtils.div(["id",timelineId]));this.timelineReady=false;var opts={start_at_end:this.getProperty("startAtEnd",false),timenav_height:this.getProperty("navHeight",150),gotoCallback:(slide)=>{if(this.timelineReady){var record=records[slide];if(record){this.getDisplayManager().notifyEvent("handleEventRecordSelection",this,{record:record});}}}};var json={};var events=[];json.events=events;var titleField=this.getFieldById(null,this.getProperty("titleField"));if(titleField==null){titleField=this.getFieldById(null,"title");}
if(titleField==null){titleField=this.getFieldById(null,"name");}
var startDateField=this.getFieldById(null,this.getProperty("startDateField"));var endDateField=this.getFieldById(null,this.getProperty("endDateField"));var textTemplate=this.getProperty("textTemplate","${default}");var timeTo=this.getProperty("timeTo","day");this.recordToIndex={};for(var i=0;i<records.length;i++){var record=records[i];this.recordToIndex[record.getId()]=i;var tuple=record.getData();var event={};var text=this.getRecordHtml(record,null,textTemplate);event.text={headline:titleField?tuple[titleField.getIndex()]:" record:"+(i+1),text:text};if(startDateField)
...
events.push(event);}
this.timeline=new TL.Timeline(timelineId,json,opts);if(this.getProperty("hideBanner",false)){this.jq(ID_TIMELINE).find(".tl-storyslider").css("display","none");}
this.jq(ID_TIMELINE).find(".tl-text").css("padding","0px");this.jq(ID_TIMELINE).find(".tl-slide-content").css("padding","0px 0px");this.jq(ID_TIMELINE).find(".tl-slidenav-description").css("display","none");this.timelineReady=true;},handleEventRecordSelection:function(source,args){if(!args.record)return;var index=this.recordToIndex[args.record.getId()];if(!Utils.isDefined(index))return;this.timeline.goTo(index);},getDate:function(time){var timeTo=this.getProperty("timeTo","day");var dt={year:time.getUTCFullYear()};if(timeTo!="year"){dt.month=time.getUTCMonth()+1;if(timeTo!="month"){dt.day=time.getUTCDate();if(timeTo!="day"){dt.hour=time.getHours();dt.minute=time.getMinutes();if(timeTo!="hour"){dt.second=time.getSeconds();}}}}
return dt;}});}
function RamaddaBlankDisplay(displayManager,id,properties){properties.showMenu=false;properties.showTitle=false;let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_BLANK,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{needsData:function(){return true;},updateUI:function(){var records=this.filterData();this.writeHtml(ID_DISPLAY_CONTENTS,"");if(records){var colorBy=this.getColorByInfo(records);if(colorBy.index>=0){records.map(record=>{color=colorBy.getColor(record.getData()[colorBy.index],record);});colorBy.displayColorTable();}}}});}
function RamaddaTsneDisplay(displayManager,id,properties){var ID_BOTTOM="bottom";var ID_CANVAS="tsnecanvas";var ID_DETAILS="tsnedetails";var ID_RUN="tsnerun";var ID_RESET="tsnereset";var ID_STEP="tsnestep";var ID_SEARCH="tsnesearch";$.extend(this,{colorTable:"red_white_blue",colorByMin:"-1",colorByMax:"1",height:"500px;"});let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_TSNE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{nameToIndex:{},needsData:function(){return true;},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},updateUI:async function(pointData){SUPER.updateUI.call(this,pointData);if(!this.hasData()){this.setContents(this.getLoadingMessage());return;}
await Utils.importJS(ramaddaBaseUrl+"/lib/tsne.js");var height=this.getProperty("height");if(height.endsWith("px"))height=height.replace("px","");height=parseInt(height);var details=HtmlUtils.div(["style","height:"+height+"px;max-height:"+height+"px","class","display-tnse-details","id",this.getDomId(ID_DETAILS)],"");var canvas=HtmlUtils.div(["class","display-tnse-canvas-outer","style","height:"+height+"px"],HtmlUtils.div(["class","display-tnse-canvas","id",this.getDomId(ID_CANVAS)],""));var buttons=HtmlUtils.div(["id",this.getDomId(ID_RUN),"class","ramadda-button","what","run"],"Stop")+"&nbsp;"+
...
HtmlUtils.input("","",["id",this.getDomId(ID_SEARCH),"placeholder","search"]);buttons=HtmlUtils.div(["class","display-tnse-toolbar"],buttons);this.jq(ID_TOP_LEFT).append(buttons);this.setContents("<table  width=100%><tr valign=top><td width=80%>"+canvas+"</td><td width=20%>"+details+"</td></tr></table>");this.search=this.jq(ID_SEARCH);this.search.keyup(e=>{var v=this.search.val().trim();this.canvas.find(".display-tnse-mark").removeClass("display-tnse-highlight");if(v=="")return;v=v.toLowerCase();for(name in this.nameToIndex){if(name.toLowerCase().startsWith(v)){this.jq("element-"+this.nameToIndex[name]).addClass("display-tnse-highlight");}}});this.details=this.jq(ID_DETAILS);this.reset=this.jq(ID_RESET);this.step=this.jq(ID_STEP);this.step.button().click(()=>{this.running=false;this.run.html(this.running?"Stop":"Run");this.takeStep();});this.reset.button().click(()=>{this.start();});this.run=this.jq(ID_RUN);this.run.button().click(()=>{this.running=!this.running;if(this.running)this.takeStep();this.run.html(this.running?"Stop":"Run");});this.canvas=this.jq(ID_CANVAS);this.running=true;this.start();},start:function(){this.canvas.html("");this.haveStepped=false;this.dataList=this.getStandardData(null,{includeIndex:false});var allFields=this.dataCollection.getList()[0].getRecordFields();if(!this.fields){this.fields=this.getSelectedFields([]);if(this.fields.length==0)this.fields=allFields;var strings=this.getFieldsOfType(this.fields,"string");if(strings.length>0)
this.textField=strings[0];}
var data=[];for(var rowIdx=1;rowIdx<this.dataList.length;rowIdx++){var tuple=this.getDataValues(this.dataList[rowIdx]);var nums=[];for(var i=0;i<this.fields.length;i++){if(this.fields[i].isNumeric()){var v=tuple[this.fields[i].getIndex()];if(isNaN(v))v=0;nums.push(v);}}
data.push(nums);}
var opt={}
opt.epsilon=10;opt.perplexity=30;opt.dim=2;this.tsne=new tsnejs.tSNE(opt);this.tsne.initDataRaw(data);this.takeStep();},takeStep:function(){var numSteps=10;for(var step=0;step<numSteps;step++){this.tsne.step();}
var pts=this.tsne.getSolution();var minx,miny,maxx,maxy;for(var i=0;i<pts.length;i++){if(i==0){maxx=minx=pts[i][0];maxy=miny=pts[i][1];}else{maxx=Math.max(maxx,pts[i][0]);minx=Math.min(minx,pts[i][0]);maxy=Math.max(maxy,pts[i][1]);miny=Math.min(miny,pts[i][1]);}}
var sleep=250;for(var i=0;i<pts.length;i++){var x=pts[i][0];var y=pts[i][1];var px=100*(x-minx)/(maxx-minx);var py=100*(y-miny)/(maxy-miny);if(!this.haveStepped){var title="";if(this.textField){var tuple=this.getDataValues(this.dataList[i]);title=tuple[this.textField.getIndex()];}
if(title.length>10){title.length=10;}
this.nameToIndex[title]=i;this.canvas.append(HtmlUtils.div(["title",title,"index",i,"id",this.getDomId("element-"+i),"class","display-tnse-mark","style","left:"+px+"%;"+"top:"+py+"%;"],title));}else{this.jq("element-"+i).animate({left:px+"%",top:py+"%"},sleep,"linear");}}
let _this=this;if(!this.haveStepped){this.canvas.find(".display-tnse-mark").click(function(e){var index=parseInt($(this).attr("index"));if(index<0||index>=_this.dataList.length)return;var tuple=_this.getDataValues(_this.dataList[index]);var details="<table class=formtable width=100% >";for(var i=0;i<_this.fields.length;i++){var field=_this.fields[i];details+="<tr><td align=right class=formlabel>"+field.getLabel()+":</td><td>"+tuple[field.getIndex()]+"</td></tr>";}
details+="</table>";_this.details.html(details);});}
if(!this.haveStepped){}
this.haveStepped=true;if(this.running)
setTimeout(()=>this.takeStep(),sleep);},});}
function RamaddaHeatmapDisplay(displayManager,id,properties){$.extend(this,{colorTable:"red_white_blue",});let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_HEATMAP,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{"map-display":false,needsData:function(){return true;},getMenuItems:function(menuItems){SUPER.getMenuItems.call(this,menuItems);var get=this.getGet();var tmp=HtmlUtils.formTable();var colorTable=this.getColorTableName();var ct="<select id="+this.getDomId("colortable")+">";for(table in Utils.ColorTable){if(table==colorTable)
ct+="<option selected>"+table+"</option>";else
ct+="<option>"+table+"</option>";}
ct+="</select>";tmp+=HtmlUtils.formEntry("Color Table:",ct);tmp+=HtmlUtils.formEntry("Color By Range:",HtmlUtils.input("",this.colorByMin,["size","7",ATTR_ID,this.getDomId("colorbymin")])+" - "+
HtmlUtils.input("",this.colorByMax,["size","7",ATTR_ID,this.getDomId("colorbymax")]));tmp+="</table>";menuItems.push(tmp);},initDialog:function(){SUPER.initDialog.call(this);var _this=this;var updateFunc=function(){_this.colorByMin=_this.jq("colorbymin").val();_this.colorByMax=_this.jq("colorbymax").val();_this.updateUI();};var func2=function(){_this.colorTable=_this.jq("colortable").val();_this.updateUI();};this.jq("colorbymin").blur(updateFunc);this.jq("colorbymax").blur(updateFunc);this.jq("colortable").change(func2);},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},fieldSelectionChanged:function(){this.updateUI();},getContentsStyle:function(){var height=this.getProperty("height",-1);if(height>0){return" height:"+height+"px; "+" max-height:"+height+"px; overflow-y: auto;";}
return"";},updateUI:function(pointData){var _this=this;if(!haveGoogleChartsLoaded()){var func=function(){_this.updateUI();}
...
var dataList=this.getStandardData(null,{includeIndex:true});var header=this.getDataValues(dataList[0]);var showIndex=this.getProperty("showIndex",true);var showValue=this.getProperty("showValue",true);var textColor=this.getProperty("textColor","black");var cellHeight=this.getProperty("cellHeight",null);var extraTdStyle="";if(this.getProperty("showBorder","false")=="true"){extraTdStyle="border-bottom:1px #666 solid;";}
var extraCellStyle="";if(cellHeight)
extraCellStyle+="height:"+cellHeight+"px; max-height:"+cellHeight+"px; min-height:"+cellHeight+"px;";var allFields=this.dataCollection.getList()[0].getRecordFields();var fields=this.getSelectedFields([]);if(fields.length==0)fields=allFields;var html="";var colors=null;var colorByMin=null;var colorByMax=null;if(Utils.stringDefined(this.getProperty("colorByMins"))){colorByMin=[];var c=this.getProperty("colorByMins").split(",");for(var i=0;i<c.length;i++){colorByMin.push(parseFloat(c[i]));}}
if(Utils.stringDefined(this.getProperty("colorByMaxes"))){colorByMax=[];var c=this.getProperty("colorByMaxes").split(",");for(var i=0;i<c.length;i++){colorByMax.push(parseFloat(c[i]));}}
if(Utils.stringDefined(this.getProperty("colorTables"))){var c=this.getProperty("colorTables").split(",");colors=[];for(var i=0;i<c.length;i++){var name=c[i];if(name=="none"){colors.push(null);continue;}
var ct=Utils.getColorTable(name,true);colors.push(ct);}}else{colors=[this.getColorTable(true)];}
var mins=null;var maxs=null;for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);if(mins==null){mins=[];maxs=[];for(var colIdx=1;colIdx<row.length;colIdx++){mins.push(Number.MAX_VALUE);maxs.push(Number.MIN_VALUE);}}
for(var colIdx=0;colIdx<fields.length;colIdx++){var field=fields[colIdx];var index=field.getIndex()+1;if(!field||!field.isFieldNumeric()||field.isFieldGeo())continue;var value=row[index];if(value==Number.POSITIVE_INFINITY||isNaN(value)||!Utils.isNumber(value)||!Utils.isDefined(value)||value==null){continue;}
mins[colIdx]=Math.min(mins[colIdx],value);maxs[colIdx]=Math.max(maxs[colIdx],value);}}
html+=HtmlUtils.openTag("table",["border","0","class","display-heatmap"]);html+="<tr valign=bottom>";if(showIndex){html+="<td align=center>"+HtmlUtils.tag("div",["class","display-heatmap-heading-top"],header[0])+"</td>";}
for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];if((!field.isFieldNumeric()||field.isFieldGeo()))continue;html+="<td align=center>"+HtmlUtils.tag("div",["class","display-heatmap-heading-top"],field.getLabel())+"</td>";}
html+="</tr>\n";for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);var index=row[0];if(index.f){index=index.f;}
var rowLabel=index;html+="<tr valign='center'>\n";if(showIndex){html+=HtmlUtils.td(["class","display-heatmap-heading-side","style",extraCellStyle],rowLabel);}
var colCnt=0;for(var colIdx=0;colIdx<fields.length;colIdx++){var field=fields[colIdx];var index=field.getIndex()+1;if(!field||!field.isFieldNumeric()||field.isFieldGeo())continue;var style="";var value=row[index];var min=mins[colIdx];var max=maxs[colIdx];if(colorByMin&&colCnt<colorByMin.length)
min=colorByMin[colCnt];if(colorByMax&&colCnt<colorByMax.length)
max=colorByMax[colCnt];var ok=min!=max&&!(value==Number.POSITIVE_INFINITY||isNaN(value)||!Utils.isNumber(value)||!Utils.isDefined(value)||value==null);var title=header[0]+": "+rowLabel+" - "+field.getLabel()+": "+value;if(ok&&colors!=null){var ct=colors[Math.min(colCnt,colors.length-1)];if(ct){var percent=(value-min)/(max-min);var ctIndex=parseInt(percent*ct.length);if(ctIndex>=ct.length)ctIndex=ct.length-1;else if(ctIndex<0)ctIndex=0;style="background-color:"+ct[ctIndex]+";";}}
var number;if(!ok){number="-";}else{number=Utils.formatNumber(value)}
if(!showValue)number="";html+=HtmlUtils.td(["valign","center","align","right","style",style+extraCellStyle+extraTdStyle,"class","display-heatmap-cell"],HtmlUtils.div(["title",title,"style",extraCellStyle+"color:"+textColor],number));colCnt++;}
html+="</tr>";}
html+="</table>";this.setContents(html);this.initTooltip();},});}
function RamaddaRankingDisplay(displayManager,id,properties){var ID_TABLE="table";$.extend(this,{height:"500px;",sortAscending:false,});if(properties.sortAscending)this.sortAscending="true"==properties.sortAscending;let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_RANKING,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Chart Attributes","sortField=\"\"",'nameFields=""',]);},needsData:function(){return true;},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},fieldSelectionChanged:function(){this.updateUI();},updateUI:function(pointData){SUPER.updateUI.call(this,pointData);if(!this.hasData()){this.setContents(this.getLoadingMessage());return;}
var dataList=this.getStandardData(null,{includeIndex:false});var allFields=this.dataCollection.getList()[0].getRecordFields();var fields=this.getSelectedFields([]);if(fields.length==0)fields=allFields;var numericFields=this.getFieldsOfType(fields,"numeric");var sortField=this.getFieldById(numericFields,this.getProperty("sortField","",true));if(numericFields.length==0){this.setContents("No fields specified");return;}
if(!sortField){sortField=numericFields[0];}
if(!sortField){this.setContents("No fields specified");return;}
var stringFields=this.getFieldsByIds(allFields,this.getProperty("nameFields","",true));if(stringFields.length==0){var tmp=this.getFieldById(allFields,this.getProperty("nameField","",true));if(tmp)stringFields.push(tmp);}
if(stringFields.length==0){var stringField=this.getFieldOfType(allFields,"string");if(stringField)stringFields.push(stringField);}
var menu="<select class='ramadda-pulldown' id='"+this.getDomId("sortfields")+"'>";for(var i=0;i<numericFields.length;i++){var field=numericFields[i];var extra="";if(field.getId()==sortField.getId())extra=" selected ";menu+="<option value='"+field.getId()+"'  "+extra+" >"+field.getLabel()+"</option>\n";}
menu+="</select>";var top="";top+=HtmlUtils.span(["id",this.getDomId("sort")],HtmlUtils.getIconImage(this.sortAscending?"fa-sort-up":"fa-sort-down",["style","cursor:pointer;","title","Change sort order"]));if(this.getProperty("showRankingMenu",true)){top+=" "+HtmlUtils.div(["style","display:inline-block;","class","display-filterby"],menu);}
this.jq(ID_TOP_LEFT).html(top);this.jq("sort").click(()=>{this.sortAscending=!this.sortAscending;if(this.sortAscending)
this.jq("sort").html(HtmlUtils.getIconImage("fa-sort-up",["style","cursor:pointer;"]));else
this.jq("sort").html(HtmlUtils.getIconImage("fa-sort-down",["style","cursor:pointer;"]));this.updateUI();});var html="";html+=HtmlUtils.openTag("div",["style","max-height:100%;overflow-y:auto;"]);html+=HtmlUtils.openTag("table",["id",this.getDomId(ID_TABLE)]);var tmp=[];for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var obj=dataList[rowIdx];obj.originalRow=rowIdx;tmp.push(obj);}
var includeNaN=this.getProperty("includeNaN",false);if(!includeNaN){var tmp2=[];tmp.map(r=>{var t=this.getDataValues(r);var v=t[sortField.getIndex()];if(!isNaN(v))tmp2.push(r);});tmp=tmp2;}
var cnt=0;tmp.sort((a,b)=>{var t1=this.getDataValues(a);var t2=this.getDataValues(b);var v1=t1[sortField.getIndex()];var v2=t2[sortField.getIndex()];if(v1<v2)return this.sortAscending?-1:1;if(v1>v2)return this.sortAscending?1:-1;return 0;});for(var rowIdx=0;rowIdx<tmp.length;rowIdx++){var obj=tmp[rowIdx];var tuple=this.getDataValues(obj);var label="";stringFields.map(f=>{label+=tuple[f.getIndex()]+" ";});label=label.trim();value=tuple[sortField.getIndex()];if(isNaN(value)||value===null){if(!includeNaN)continue;value="NA";}
html+="<tr valign=top class='display-ranking-row' what='"+obj.originalRow+"'><td> #"+(rowIdx+1)+"</td><td>&nbsp;"+label+"</td><td align=right>&nbsp;"+
value+"</td></tr>";}
html+=HtmlUtils.closeTag("table");html+=HtmlUtils.closeTag("div");this.setContents(html);let _this=this;this.jq(ID_TABLE).find(".display-ranking-row").click(function(e){_this.getDisplayManager().propagateEventRecordSelection(_this,_this.getPointData(),{index:parseInt($(this).attr("what"))-1});});HtmlUtils.initSelect(this.jq("sortfields"));this.jq("sortfields").change(function(){_this.setProperty("sortField",$(this).val());_this.updateUI();});},});}
function RamaddaCrosstabDisplay(displayManager,id,properties){var ID_TABLE="crosstab";let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_CROSSTAB,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},fieldSelectionChanged:function(){this.updateUI();},updateUI:function(pointData){SUPER.updateUI.call(this,pointData);if(!this.hasData()){this.setContents(this.getLoadingMessage());return;}
var allFields=this.dataCollection.getList()[0].getRecordFields();var enums=[];allFields.map(field=>{var label=field.getLabel();if(label.length>30)label=label.substring(0,29);enums.push([field.getId(),label]);});var select=HtmlUtils.span(["class","display-filterby"],"Display: "+HtmlUtils.select("",["style","","id",this.getDomId("crosstabselect")],enums,this.getProperty("column","",true)));this.setContents(select+HtmlUtils.div(["id",this.getDomId(ID_TABLE)]));let _this=this;this.jq("crosstabselect").change(function(){_this.setProperty("column",$(this).val());_this.makeTable();});this.makeTable();},makeTable:function(){var dataList=this.getStandardData(null,{includeIndex:false});var allFields=this.dataCollection.getList()[0].getRecordFields();var col=this.getFieldById(null,this.getProperty("column","",true));var rows=this.getFieldsByIds(null,this.getProperty("rows",null,true));if(!col)col=allFields[0];if(rows.length==0)rows=allFields;var html=HtmlUtils.openTag("table",["border","1px","bordercolor","#ccc","class","display-crosstab","cellspacing","1","cellpadding","2"]);var total=dataList.length-1;var cnt=0;rows.map((row)=>{if(row.getId()==col.getId())return;cnt++;var colValues=[];var rowValues=[];var count={};var rowcount={};var colcount={};for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var tuple=this.getDataValues(dataList[rowIdx]);var colValue=(""+tuple[col.getIndex()]).trim();var rowValue=(""+tuple[row.getIndex()]).trim();var key=colValue+"--"+rowValue;if(colValues.indexOf(colValue)<0)colValues.push(colValue);if(rowValues.indexOf(rowValue)<0)rowValues.push(rowValue);if(!(rowValue in rowcount)){rowcount[rowValue]=0;}
rowcount[rowValue]++;if(!(key in count)){count[key]=0;}
count[key]++;}
colValues.sort();rowValues.sort();if(cnt==1)
html+="<tr><td></td><td align=center class=display-crosstab-header colspan="+colValues.length+">"+col.getLabel()+"</td><td>&nbsp;</td></tr>";html+="<tr valign=bottom class=display-crosstab-header-row><td class=display-crosstab-header>"+row.getLabel()+"</td>";for(var j=0;j<colValues.length;j++){var colValue=colValues[j];html+="<td>"+(colValue==""?"&lt;blank&gt;":colValue)+"</td>";}
html+="<td><b>Total</b></td>";html+="</tr>";for(var i=0;i<rowValues.length;i++){var rowValue=rowValues[i];html+="<tr>";html+="<td>"+(rowValue==""?"&lt;blank&gt;":rowValue)+"</td>";for(var j=0;j<colValues.length;j++){var colValue=colValues[j];var key=colValue+"--"+rowValue;if(Utils.isDefined(count[key])){var perc=Math.round(count[key]/total*100)+"%";html+="<td align=right>"+count[key]+"&nbsp;("+perc+")</td>";}else{html+="<td>&nbsp;</td>";}}
var perc=Math.round(rowcount[rowValue]/total*100)+"%";html+="<td align=right>"+rowcount[rowValue]+"&nbsp;("+perc+")</td>";html+="</tr>";}});html+="</table>";this.jq(ID_TABLE).html(html);},});}
function RamaddaCorrelationDisplay(displayManager,id,properties){var ID_BOTTOM="bottom";$.extend(this,{colorTable:"red_white_blue",colorByMin:"-1",colorByMax:"1",});let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_CORRELATION,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{"map-display":false,needsData:function(){return true;},getMenuItems:function(menuItems){SUPER.getMenuItems.call(this,menuItems);var get=this.getGet();var tmp=HtmlUtils.formTable();var colorTable=this.getColorTableName();var ct="<select id="+this.getDomId("colortable")+">";for(table in Utils.ColorTables){if(table==colorTable)
ct+="<option selected>"+table+"</option>";else
ct+="<option>"+table+"</option>";}
ct+="</select>";tmp+=HtmlUtils.formEntry("Color Bar:",ct);tmp+=HtmlUtils.formEntry("Color By Range:",HtmlUtils.input("",this.colorByMin,["size","7",ATTR_ID,this.getDomId("colorbymin")])+" - "+
HtmlUtils.input("",this.colorByMax,["size","7",ATTR_ID,this.getDomId("colorbymax")]));tmp+="</table>";menuItems.push(tmp);},initDialog:function(){SUPER.initDialog.call(this);var _this=this;var updateFunc=function(){_this.colorByMin=_this.jq("colorbymin").val();_this.colorByMax=_this.jq("colorbymax").val();_this.updateUI();};var func2=function(){_this.colorTable=_this.jq("colortable").val();_this.updateUI();};this.jq("colorbymin").blur(updateFunc);this.jq("colorbymax").blur(updateFunc);this.jq("colortable").change(func2);},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},fieldSelectionChanged:function(){this.updateUI();},updateUI:function(pointData){SUPER.updateUI.call(this,pointData);if(!this.hasData()){this.setContents(this.getLoadingMessage());return;}
var dataList=this.getStandardData(null,{includeIndex:false});var allFields=this.dataCollection.getList()[0].getRecordFields();var fields=this.getSelectedFields([]);if(fields.length==0)fields=allFields;var fieldCnt=0;for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field1=fields[fieldIdx];if(!field1.isFieldNumeric()||field1.isFieldGeo())continue;fieldCnt++;}
var html=HtmlUtils.openTag("table",["border","0","class","display-correlation","width","100%"]);var col1Width=10+"%";var width=90/fieldCnt+"%";html+="\n<tr valign=bottom><td class=display-heading width="+col1Width+">&nbsp;</td>";var short=this.getProperty("short",fieldCnt>8);var showValue=this.getProperty("showValue",!short);var useId=this.getProperty("useId",true);var useIdTop=this.getProperty("useIdTop",useId);var useIdSide=this.getProperty("useIdSide",useId);for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field1=fields[fieldIdx];if(!field1.isFieldNumeric()||field1.isFieldGeo())continue;var label=useIdTop?field1.getId():field1.getLabel();if(short)label="";html+="<td align=center width="+width+">"+HtmlUtils.tag("div",["class","display-correlation-heading-top"],label)+"</td>";}
html+="</tr>\n";var colors=null;colorByMin=parseFloat(this.colorByMin);colorByMax=parseFloat(this.colorByMax);colors=this.getColorTable(true);for(var fieldIdx1=0;fieldIdx1<fields.length;fieldIdx1++){var field1=fields[fieldIdx1];if(!field1.isFieldNumeric()||field1.isFieldGeo())continue;var label=useIdSide?field1.getId():field1.getLabel();html+="<tr valign=center><td>"+HtmlUtils.tag("div",["class","display-correlation-heading-side"],label.replace(/ /g,"&nbsp;"))+"</td>";var rowName=field1.getLabel();for(var fieldIdx2=0;fieldIdx2<fields.length;fieldIdx2++){var field2=fields[fieldIdx2];if(!field2.isFieldNumeric()||field2.isFieldGeo())continue;var colName=field2.getLabel();var t1=0;var t2=0;var cnt=0;for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var tuple=this.getDataValues(dataList[rowIdx]);var v1=tuple[field1.getIndex()];var v2=tuple[field2.getIndex()];t1+=v1;t2+=v2;cnt++;}
var avg1=t1/cnt;var avg2=t2/cnt;var sum1=0;var sum2=0;var sum3=0;for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var tuple=this.getDataValues(dataList[rowIdx]);var v1=tuple[field1.getIndex()];var v2=tuple[field2.getIndex()];sum1+=(v1-avg1)*(v2-avg2);sum2+=(v1-avg1)*(v1-avg1);sum3+=(v2-avg2)*(v2-avg2);}
r=sum1/Math.sqrt(sum2*sum3);var style="";if(colors!=null){var percent=(r-colorByMin)/(colorByMax-colorByMin);var index=parseInt(percent*colors.length);if(index>=colors.length)index=colors.length-1;else if(index<0)index=0;style="background-color:"+colors[index];}
var value=r.toFixed(3);var label=value;if(!showValue||short)label="&nbsp;";html+="<td class=display-correlation-cell align=right style=\""+style+"\">"+HtmlUtils.tag("div",["class","display-correlation-element","title","&rho;("+rowName+","+colName+") = "+value],label)+"</td>";}
html+="</tr>";}
html+="<tr><td></td><td colspan = "+(fieldCnt+1)+">"+HtmlUtils.div(["id",this.getDomId(ID_BOTTOM)],"")+"</td></tr>";html+="</table>";this.setContents(html);this.displayColorTable(colors,ID_BOTTOM,colorByMin,colorByMax);this.initTooltip();this.displayManager.propagateEventRecordSelection(this,this.dataCollection.getList()[0],{index:0});},});}
function RamaddaRecordsDisplay(displayManager,id,properties,type){let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_RECORDS,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["maxHeight=\"\"",]);},needsData:function(){return true;},handleEventPointDataLoaded:function(source,pointData){if(!this.needsData()){if(this.dataCollection==null){this.dataCollection=source.dataCollection;this.updateUI();}}},getFieldsToSelect:function(pointData){return pointData.getRecordFields();},defaultSelectedToAll:function(){return true;},fieldSelectionChanged:function(){SUPER.fieldSelectionChanged.call(this);this.updateUI();},updateUI:function(reload){SUPER.updateUI.call(this,reload);var records=this.filterData();if(!records){this.setContents(this.getLoadingMessage());return;}
this.records=records;let _this=this;var fields=this.getSelectedFields(this.getData().getRecordFields());var html="";for(var rowIdx=0;rowIdx<records.length;rowIdx++){var div="";var tuple=this.getDataValues(records[rowIdx]);for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];var v=tuple[field.getIndex()];div+=HtmlUtil.b(field.getLabel())+": "+v+"</br>";}
html+=HtmlUtils.div(["class","display-records-record","recordIndex",rowIdx],div);}
var height=this.getProperty("maxHeight","400px");if(!height.endsWith("px")){height=height+"px";}
this.setContents(HtmlUtil.div(["style","max-height:"+height+";overflow-y:auto;"],html));this.jq(ID_DISPLAY_CONTENTS).find(".display-records-record").click(function(){var record=_this.records[$(this).attr("recordIndex")];if(record){_this.getDisplayManager().notifyEvent("handleEventRecordSelection",_this,{highlight:true,record:record});}});},handleEventRecordSelection:function(source,args){}});}
function RamaddaStatsDisplay(displayManager,id,properties,type){var dflt=Utils.isDefined(properties["showDefault"])?properties["showDefault"]:true;$.extend(this,{showMin:dflt,showMax:dflt,showAverage:dflt,showStd:dflt,showCount:dflt,showTotal:dflt,showPercentile:dflt,showMissing:dflt,showUnique:dflt,showType:dflt,showText:dflt,});let SUPER=new RamaddaFieldsDisplay(displayManager,id,type||DISPLAY_STATS,properties);RamaddaUtil.inherit(this,SUPER);if(!type)
...
var tuples=this.getStandardData(null,{includeIndex:false});var justOne=(tuples.length==2);var l=[];for(i=0;i<fields.length;i++){var field=fields[i];if(!justOne&&(!this.showText&&!field.isNumeric()))continue;var lbl=field.getLabel().toLowerCase();if(lbl.indexOf("latitude")>=0||lbl.indexOf("longitude")>=0){continue;}
l.push(field);}
return l;},getFieldsToSelect:function(pointData){return pointData.getRecordFields();},defaultSelectedToAll:function(){return true;},fieldSelectionChanged:function(){SUPER.fieldSelectionChanged.call(this);this.updateUI();},updateUI:function(reload){SUPER.updateUI.call(this,reload);if(!this.hasData()){this.setContents(this.getLoadingMessage());return;}
var dataList=this.getStandardData(null,{includeIndex:false});var allFields=this.dataCollection.getList()[0].getRecordFields();this.allFields=allFields;var fields=this.getSelectedFields([]);var fieldMap={};var stats=[];var justOne=(dataList.length==2);for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var tuple=this.getDataValues(dataList[rowIdx]);if(rowIdx==1){for(var col=0;col<tuple.length;col++){stats.push({isNumber:false,count:0,min:Number.MAX_SAFE_INTEGER,uniqueMap:{},unique:0,std:0,max:Number.MIN_SAFE_INTEGER,total:0,numMissing:0,numNotMissing:0,type:null,values:[]});}}
for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];var col=field.getIndex()
stats[col].type=field.getType();var v=tuple[col];if(v){if(!Utils.isDefined(stats[col].uniqueMap[v])){stats[col].uniqueMap[v]=1;stats[col].unique++;}else{stats[col].uniqueMap[v]++;}}
stats[col].isNumber=field.isNumeric();stats[col].count++;if(v==null){stats[col].numMissing++;}else{stats[col].numNotMissing++;}
if(v&&(typeof v=='number')){var label=field.getLabel().toLowerCase();if(label.indexOf("latitude")>=0||label.indexOf("longitude")>=0){continue;}
stats[col].total+=v;stats[col].max=Math.max(stats[col].max,v);stats[col].min=Math.min(stats[col].min,v);stats[col].values.push(v);}}}
if(this.showUnique){for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];var col=field.getIndex();stats[col].uniqueMax=0;stats[col].uniqueValue="";for(var v in stats[col].uniqueMap){var count=stats[col].uniqueMap[v];if(count>stats[col].uniqueMax){stats[col].uniqueMax=count;stats[col].uniqueValue=v;}}}}
if(this.showStd){for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];var col=field.getIndex();var values=stats[col].values;if(values.length>0){var average=stats[col].total/values.length;var stdTotal=0;for(var i=0;i<values.length;i++){var diff=values[i]-average;stdTotal+=diff*diff;}
var mean=stdTotal/values.length;stats[col].std=Math.sqrt(mean);}}}
var border=(justOne?"0":"1");var html=HtmlUtils.openTag("table",["border",border,"bordercolor","#ccc","class","display-stats","cellspacing","1","cellpadding","5"]);var dummy=["&nbsp;"];if(!justOne){header=[""];if(this.showCount){header.push("Count");dummy.push("&nbsp;");}
if(this.showMin){header.push("Min");dummy.push("&nbsp;");}
if(this.showPercentile){header.push("25%");dummy.push("&nbsp;");header.push("50%");dummy.push("&nbsp;");header.push("75%");dummy.push("&nbsp;");}
if(this.showMax){header.push("Max");dummy.push("&nbsp;");}
if(this.showTotal){header.push("Total");dummy.push("&nbsp;");}
if(this.showAverage){header.push("Average");dummy.push("&nbsp;");}
if(this.showStd){header.push("Std");dummy.push("&nbsp;");}
if(this.showUnique){header.push("# Unique");dummy.push("&nbsp;");header.push("Top");dummy.push("&nbsp;");header.push("Freq.");dummy.push("&nbsp;");}
if(this.showMissing){header.push("Not&nbsp;Missing");dummy.push("&nbsp;");header.push("Missing");dummy.push("&nbsp;");}
html+=HtmlUtils.tr(["valign","bottom"],HtmlUtils.tds(["class","display-stats-header","align","center"],header));}
var cats=[];var catMap={};let doValueSelection=this.getProperty("doValueSelection",false);for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];var col=field.getIndex();var field=allFields[col];var right="";var total="&nbsp;";var label=field.getLabel().toLowerCase();var avg=stats[col].numNotMissing==0?"NA":this.formatNumber(stats[col].total/stats[col].numNotMissing);if(label.indexOf("%")<0&&label.indexOf("percent")<0&&label.indexOf("median")<0){total=this.formatNumber(stats[col].total);}
if(justOne){right=HtmlUtils.tds(["xalign","right"],[this.formatNumber(stats[col].min)]);continue;}
var values=[];if(!stats[col].isNumber&&this.showText){if(this.showCount)
values.push(stats[col].count);if(this.showMin)
values.push("-");if(this.showPercentile){values.push("-");values.push("-");values.push("-");}
if(this.showMax)
values.push("-");values.push("-");if(this.showAverage){values.push("-");}
if(this.showStd){values.push("-");}
if(this.showUnique){values.push(stats[col].unique);values.push(stats[col].uniqueValue);values.push(stats[col].uniqueMax);}
if(this.showMissing){values.push(stats[col].numNotMissing);values.push(stats[col].numMissing);}}else{if(this.showCount){values.push(stats[col].count);}
if(this.showMin){var s=this.formatNumber(stats[col].min);values.push(s);}
if(this.showPercentile){var range=stats[col].max-stats[col].min;var tmp=p=>{var s=this.formatNumber(stats[col].min+range*p);if(doValueSelection){s=HtmlUtils.span(["class","display-stats-value","data-type","percentile","data-value",p],s);}
values.push(s);}
var percs=[.25,.5,.75];percs.map(v=>tmp(v));}
if(this.showMax){var s=this.formatNumber(stats[col].max);values.push(s);}
if(this.showTotal){values.push(total);}
if(this.showAverage){values.push(avg);}
if(this.showStd){values.push(this.formatNumber(stats[col].std));}
if(this.showUnique){values.push(stats[col].unique);if(Utils.isNumber(stats[col].uniqueValue)){values.push(this.formatNumber(stats[col].uniqueValue));}else{values.push(stats[col].uniqueValue);}
values.push(stats[col].uniqueMax);}
if(this.showMissing){values.push(stats[col].numNotMissing);values.push(stats[col].numMissing);}}
right=HtmlUtils.tds(["align","right"],values);var align=(justOne?"right":"left");var label=field.getLabel();var toks=label.split("!!");var tooltip="";tooltip+=field.getId();if(field.description&&field.description!=""){tooltip+="\n"+field.description+"\n";}
label=toks[toks.length-1];if(field.unit&&field.unit!="")
label=label+" ["+field.unit+"]";if(justOne){label+=":";}
label=label.replace(/ /g,"&nbsp;")
var row=HtmlUtils.tr([],HtmlUtils.td(["align",align],field.getTypeLabel()+"&nbsp;<b>"+HtmlUtils.span(["title",tooltip],label)+"</b>")+right);if(justOne){html+=row;}else{html+=row;}}
html+="</table>";this.setContents(html);this.initTooltip();if(doValueSelection){var values=this.jq(ID_DISPLAY_CONTENTS).find(".display-stats-value");values.each(function(){var type=$(this).attr("data-type");var value=$(this).attr("data-value");var links="&nbsp;"+HtmlUtils.getIconImage("fa-less-than",["title","Filter other displays","class","display-stats-value-link","data-type",type,"data-value",value],["style","font-size:8pt;"]);$(this).append(links);});values=this.jq(ID_DISPLAY_CONTENTS).find(".display-stats-value-link");values.each(function(){});}
...
function RamaddaCooccurenceDisplay(displayManager,id,properties){let ID_TABLE="table";let ID_HEADER="coocheader";let ID_SORTBY="sortby";var SUPER;RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_COOCCURENCE,properties));addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Cooccurence Attributes",'sourceField=""','targetField=""','colorBy=""','directed=false','missingBackground=#eee','showSortBy=false','sortBy=weight','minWeight=""','topSpace=50px'])},updateUI:function(){var records=this.filterData();if(!records){return;}
let html=HtmlUtils.div(["id",this.getDomId(ID_HEADER)])+
HtmlUtils.div(["id",this.getDomId(ID_TABLE)]);this.jq(ID_DISPLAY_CONTENTS).html(html);let sourceField=this.getFieldById(null,this.getProperty("sourceField","source"));let targetField=this.getFieldById(null,this.getProperty("targetField","target"));let weightField=this.getFieldById(null,this.getProperty("colorBy","weight"));if(weightField&&this.getProperty("showSortBy",true)){var enums=[["name","Name"],["weight","Weight"]];var header=HtmlUtils.div(["style","margin-left:100px;"],"Sort by: "+HtmlUtils.select("",["id",this.getDomId(ID_SORTBY),],enums,this.getProperty("sortBy","")));this.jq(ID_HEADER).html(header);let _this=this;this.jq(ID_SORTBY).change(function(){_this.setProperty("sortBy",$(this).val());_this.updateUI();});}
if(sourceField==null||targetField==null){this.jq(ID_DISPLAY_CONTENTS).html("No source/target fields specified");return;}
var colors=this.getColorTable();if(!colors)colors=Utils.getColorTable("blues",true);var colorBy=this.getColorByInfo(records,null,null,colors);let names={};let nameList=[];let sources=[];let targets=[];let links={};let maxWeight=0;let sortBy=this.getProperty("sortBy","name");let directed=this.getProperty("directed",true);let missing=-999999;records.map(r=>{var source=r.getValue(sourceField.getIndex());var target=r.getValue(targetField.getIndex());var weight=missing;if(weightField){weight=r.getValue(weightField.getIndex());maxWeight=Math.max(maxWeight,weight);}
...
sources.sort(sortFunc);targets.sort(sortFunc);let minWeight=this.getProperty("minWeight",missing);var seen={}
var tmp=[];let pruneFunc=t=>{if(minWeight!=missing){if(t.weight==missing||t.weight<minWeight)return;}
if(!seen[t.name]){seen[t.name]=true;tmp.push(t.name);}}
sources.map(pruneFunc);sources=tmp;seen={}
tmp=[];targets.map(pruneFunc);targets=tmp;let table="<div style='margin-top:"+this.getProperty("topSpace","100px")+";'></div><table style='height:100%;' class='display-cooc-table' order=0 cellpadding=0 cellspacing=0  >"
table+="<tr valign=bottom><td style='border:none;'></td>";targets.map(target=>{target=target.replace(/ /g,"&nbsp;").replace(/-/g,"&nbsp;");table+=HtmlUtils.td(["style","border:none;","width","6"],HtmlUtils.div(["class","display-cooc-colheader"],target));});var missingBackground=this.getProperty("missingBackground","#eee");sources.map(source=>{var label=source.replace(/ /g,"&nbsp;");table+="<tr valign=bottom ><td style='   border:none;' align=right>"+HtmlUtils.div(["class","display-cooc-rowheader"],label)+"</td>";targets.map(target=>{var weight=links[source+"--"+target];if(!directed&&!Utils.isDefined(weight))
weight=links[target+"--"+source];var style="";if(weight){if(weight==missing||maxWeight==0)
style="background:#ccc;";else{if(colorBy.index>=0){color=colorBy.getColor(weight);style="background:"+color+";";}
}}else{style="background:"+missingBackground+";";}
table+=HtmlUtils.td(["title",source+" -> "+target+(weight>0?" "+weight:""),"width","3"],HtmlUtils.div(["class","display-cooc-cell","style",style+"height:100%;"],"&nbsp;"));});table+="</tr>";});table+="</tr>";table+="</table><div style='margin:5px;'></div>";this.jq(ID_TABLE).html(table);colorBy.displayColorTable();}})}
function RamaddaBoxtableDisplay(displayManager,id,properties){let ID_TABLE="table";let ID_HEADER="coocheader";let ID_SORTBY="sortby";var SUPER;RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_BOXTABLE,properties));addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Color Boxes",'categoryField=""','colorBy=""','tableWidth=300',])},updateUI:function(){var records=this.filterData();if(!records){return;}
...
var colors=this.getColorTable();if(!colors)colors=Utils.getColorTable("blues",true);var colorBy=this.getColorByInfo(records,null,null,colors);let catMap={};let cats=[];records.map(r=>{let category=r.getValue(categoryField.getIndex());let value=colorBy.index>=0?r.getValue(colorBy.index):0;let list=catMap[category]&&catMap[category].list;if(!list){list=[];catMap[category]={list:list,max:0};cats.push(category);}
catMap[category].max=Math.max(catMap[category].max,value);list.push(r);});let html="<table class='display-colorboxes-table' border=0 cellpadding=5>";let tableWidth=this.getProperty("tableWidth",300);cats.sort((a,b)=>{return catMap[b].max-catMap[a].max;});cats.map(cat=>{let length=catMap[cat].list.length;let row=`<tr valign=top><td align=right class=display-colorboxes-header>${cat}(${length})</td><td width=${tableWidth}>`;if(colorBy.index){catMap[cat].list.sort((a,b)=>{return b.getData()[colorBy.index]-a.getData()[colorBy.index];});}
...
row+=HtmlUtils.div(["title","","recordIndex",idx,"class","display-colorboxes-box","style","background:"+color+";"],"");});row+="</td></tr>"
html+=row;});html+="</table>";this.displayHtml(html);colorBy.displayColorTable(500);if(!this.getProperty("tooltip"))
this.setProperty("tooltip","${default}");this.makeTooltips(this.jq(ID_DISPLAY_CONTENTS).find(".display-colorboxes-box"),records);}})}
...
this.jq(fieldId).html(lbl);},loadSheet:function(sheetIdx){var all=$("[id^="+this.getDomId("sheet_")+"]");var sel=$("#"+this.getDomId("sheet_")+sheetIdx);all.css('font-weight','normal');sel.css('font-weight','bold');all.css('border','1px #ccc solid');sel.css('border','1px #666 solid');this.currentSheet=sheetIdx;var sheet=this.sheets[sheetIdx];if(sheet){var rows=sheet.rows.slice(0);if(rows.length>0){this.header=rows[0];}}
...
if(record){this.getDisplayManager().notifyEvent("handleEventRecordSelection",this,{record:record});}}},addEvents:function(plot,myPlot){let _this=this;myPlot.on('plotly_click',function(data){_this.handleClickEvent(data);});}});}
...
var theta;var thetaType;if(this.getProperty("useDates")){var tmp=this.getDateValues(records);theta=[];var dateFormat=this.getProperty("dateFormat","yyyyMMdd");thetaType="category";tmp.map(d=>{theta.push(Utils.formatDateFromProperty(dateFormat,d));});}else{var thetaField=this.getFieldById(null,this.getProperty("thetaField"));if(thetaField){theta=this.getColumnValues(records,thetaField).values;}}
...
var pointNumber=data.points[0].pointNumber;var record=this.myRecords[pointNumber];if(record){this.getDisplayManager().notifyEvent("handleEventRecordSelection",this,{record:record});}},});}
...
var pointNumber=data.points[0].pointNumber;var pattern=this.patternList[pointNumber];var args={property:"filterValue",fieldId:this.textField.getId(),value:pattern};this.propagateEvent("handleEventPropertyChanged",args);},});}
