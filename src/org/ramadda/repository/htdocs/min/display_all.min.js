...
this.output.html(h);this.outputUpdated();},outputUpdated:function(){this.outputHtml=this.jq(ID_OUTPUT).html();},getRawOutput:function(){return this.rawOutput;},focus:function(){this.input.focus();},clearOutput:function(){this.output.html("");this.outputHtml="";},processHtml:async function(chunk){this.rawOutput+=chunk.content+"\n";chunk.div.set(chunk.content);},processCss:async function(chunk){var css=HtmlUtils.tag("style",["type","text/css"],chunk.content);this.rawOutput+=css+"\n";chunk.div.set(css);},handleError:function(chunk,error){chunk.ok=false;chunk.div.append(error);},processFetch:async function(chunk){var lines=chunk.content.split("\n");for(var i=0;i<lines.length;i++){var line=lines[i].trim();if(line=="")continue;var origLine=line;var error=null;if(line.startsWith("js:")){var url=line.substring(3).trim();await Utils.importJS(url,()=>{},(jqxhr,settings,exception)=>error="Error fetching "+origLine+" "+exception);}else if(line.startsWith("css:")){var url=line.substring(4).trim();await Utils.importCSS(url,null,(jqxhr,settings,exception)=>error="Error fetching "+origLine+" "+exception);}else if(line.startsWith("html:")){var url=line.substring(5).trim();await Utils.importText(url,h=>chunk.div.append(h),(jqxhr,settings,exception)=>error="Error fetching "+origLine+" "+exception);}else if(line.startsWith("text:")){line=line.substring(5).trim();var v=null;var indexEquals=line.indexOf("=");var indexHttp=line.indexOf("http");var indexSlash=line.indexOf("/");if(indexEquals>0){var index=indexHttp>=0?indexHttp:indexSlash;if(indexSlash>=0&&indexSlash<indexHttp)index=indexSlash;if(indexEquals<index){v=line.substring(0,indexEquals).trim();line=line.substring(indexEquals+1).trim();}}
var url=line;var results=null;await Utils.importText(url,h=>results=h,(jqxhr,settings,err)=>error="Error fetching "+origLine+" error:"+err.toString());if(results){if(v){this.notebook.addGlobal(v,results);}else{chunk.div.append(HtmlUtils.pre([],results));}}}else{chunk.div.append("Unknown fetch:"+line);}
if(error){this.handleError(chunk,error);return;}}},processMd:async function(chunk){await Utils.importJS(ramaddaBaseUrl+"/lib/showdown.min.js");this.rawOutput+=chunk.content+"\n";var converter=new showdown.Converter();var html=converter.makeHtml(chunk.content);chunk.div.set(html);},processPy:async function(chunk){await Utils.importJS(ramaddaBaseUrl+"/lib/skulpt.min.js");await Utils.importJS(ramaddaBaseUrl+"/lib/skulpt-stdlib.js");var output="";var outf=function(t){if(t.trim()!=""){output+=t.trim()+"<br>"}}
Sk.configure({output:outf,read:skulptRead});var myPromise=Sk.misceval.asyncToPromise(function(){return Sk.importMainWithBody("<stdin>",false,chunk.content,true);});myPromise.then(function(mod){},function(err){output+="Error:"+err.toString();console.log(err.toString());});chunk.div.set(output);},processWiki:async function(chunk){this.rawOutput+=chunk.content+"\n";var id=this.notebook.getProperty("entryId","");await this.getCurrentEntry(e=>entry=e);if(entry)id=entry.getId();let _this=this;let divId=HtmlUtils.getUniqueId();var wikiCallback=function(html){var h=HtmlUtils.div(["id",divId,"style"],html);chunk.div.set(h);}
var wiki="{{group showMenu=false}}\n"+chunk.content;await GuiUtils.loadHtml(ramaddaBaseUrl+"/wikify?doImports=false&entryid="+id+"&text="+encodeURIComponent(chunk.content),wikiCallback);},processSh:async function(chunk){var r="";var lines=chunk.content.split("\n");var commands=[];for(var i=0;i<lines.length;i++){var fullLine=lines[i].trim();if(fullLine=="")continue;var cmds=fullLine.split(";");for(var cmdIdx=0;cmdIdx<cmds.length;cmdIdx++){var line=cmds[cmdIdx].trim();if(line==""||line.startsWith("#")||line.startsWith("//"))continue;var toks=line.split(" ");var command=toks[0].trim();var proc=null;var extra=null;if(this["processCommand_"+command]){proc=this["processCommand_"+command];}else{proc=this.processCommand_help;extra="Unknown command: <i>"+command+"</i>";}
...
await cmd.proc.call(_this,cmd.line,cmd.toks,cmd.div,cmd.extra);}},processJs:async function(chunk){try{await this.getCurrentEntry(e=>{current=e});var state=new NotebookState(this,chunk.div);notebookStates[state.id]=state;var notebookEntries=this.notebook.getCurrentEntries();for(name in notebookEntries){state.entries[name]=notebookEntries[name].entry;}
