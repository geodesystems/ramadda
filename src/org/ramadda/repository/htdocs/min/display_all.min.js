...
this.checkSearchBar();this.updateUI();},getFilterId:function(id){return this.getDomId("filterby_"+id);},getHeader2:function(){return"";},initHeader2:function(){},getRequestMacros:function(){let macros=[];this.getProperty("macros","").split(",").every(macro=>{if(macro=="")return true;macros.push({display:this,name:macro,type:this.getProperty("macro."+macro+".type","string"),dflt:this.getProperty("macro."+macro+".default",""),label:this.getProperty("macro."+macro+".label",Utils.makeLabel(macro)),enums:this.getProperty("macro."+macro+".values","").split(","),getWidget:function(){let visible=this.display.getProperty("macro."+this.name+".visible",this.display.getProperty("macros.visible",true));let style=visible?"":"display:none;";return(visible?this.label+": ":"")+HtmlUtils.input("",this.dflt,["style",style,"id",this.display.getDomId(this.getId()),"size","10","class","display-filter-input"]);},getId:function(){return"macro_"+this.name;},getValue:function(){let widget=this.display.jq(this.getId());let value=this.dflt;if(widget.length!=0)value=widget.val();this.display.setProperty(this.name+".default",value);return value;},apply:function(url){url=url+"&"+HtmlUtils.urlArg("macro_"+this.name,this.getValue());return url;}});return true;});return macros;},checkSearchBar:function(){let _this=this;this.filterFields=[];this.colorByFields=this.getFieldsByIds(null,this.getProperty("colorByFields","",true));this.sizeByFields=this.getFieldsByIds(null,this.getProperty("sizeByFields","",true));let pointData=this.getData();if(pointData==null)return;let fields=pointData.getRecordFields();let records=pointData.getRecords();let header2=HtmlUtils.span(["xstyle","display:inline-block;","id",this.getDomId(ID_HEADER2_PREFIX)]);header2+=this.getHeader2();let macros=this.getRequestMacros();macros.every(macro=>{header2+=macro.getWidget();return true;});if(this.getProperty("legendFields")||this.getProperty("showFieldLegend",false)){let colors=this.getColorList();let fields=this.getFieldsByIds(null,this.getProperty("legendFields",this.getProperty("fields",this.getProperty("sumFields"))));let html="";let colorCnt=0;fields.map((f)=>{if(colorCnt>=colors.length)colorCnt=0;let color=colors[colorCnt];html+=HtmlUtils.div(["style","display: inline-block;width:8px;height:8px;background:"+color+";"])+" "+f.getLabel()+"&nbsp;&nbsp; ";colorCnt++;});header2+=html;}
...
header2+=HtmlUtils.span(["xstyle","display:inline-block;","id",this.getDomId(ID_HEADER2_SUFFIX)]);this.jq(ID_HEADER2).html(header2);this.initHeader2();var theDisplay=this;macros.every(macro=>{this.jq(macro.getId()).keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){_this.reloadData();}});return true;});let inputFunc=function(input,input2,value){var id=input.attr("id");if(!input2){if(id.endsWith("_min")){input2=$("#"+id.replace(/_min$/,"_max"));}else if(id.endsWith("_max")){var tmp=input;input=$("#"+id.replace(/_max$/,"_min"));input2=tmp;}}
...
if(!this.getDisplayReady()){console.log("pointDataLoaded: display not ready");return;}
...
this.haveCalledUpdateUI=false;this.updateUI(reload);if(!reload){this.lastPointData=pointData;this.propagateEvent("handleEventPointDataLoaded",pointData);}},getDateFormatter:function(){var date_formatter=null;if(this.googleLoaded()){var df=this.getProperty("dateFormat",null);if(df){var tz=0;this.timezone=this.getProperty("timezone");if(Utils.isDefined(this.timezone)){tz=parseFloat(this.timezone);}
...
obj.pending.push(display);if(obj.pending.length>1){console.log("Waiting on callback:"+obj.pending.length+" "+url+" d:"+display);return;}
