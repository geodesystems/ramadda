...
this.updateUI();if(!reload){this.lastPointData=pointData;this.propagateEvent("handleEventPointDataLoaded",pointData);}},doSearch:function(){var records=this.filterData();if(!records){return;}
var newRecords=[];var values=[];for(var i=0;i<this.searchFields.length;i++){var searchField=this.searchFields[i];values.push($("#"+this.getDomId("searchby_"+searchField.getId())).val());}
for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);var ok=true;for(var i=0;i<this.searchFields.length;i++){var searchField=this.searchFields[i];var searchValue=values[i];if(searchValue=="")continue;var value=row[searchField.getIndex()];if(searchField.getType()=="enumeration"){if(value!=searchValue){ok=false;break;}}else{value=(""+value).toLowerCase();if(value.indexOf(searchValue.toLowerCase())<0){ok=false;break;}}}
if(ok)newRecords.push(records[rowIdx]);}
this.displaySearchResults(newRecords);},displaySearchResults:function(records){},getDateFormatter:function(){var date_formatter=null;if(this.googleLoaded()){var df=this.getProperty("dateFormat",null);if(df){var tz=0;this.timezone=this.getProperty("timezone");if(Utils.isDefined(this.timezone)){tz=parseFloat(this.timezone);}
