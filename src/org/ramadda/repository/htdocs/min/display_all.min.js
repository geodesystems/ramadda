...
HtmlUtils.div(["class","display-notebook-console-output","id",this.getDomId(ID_CONSOLE_OUTPUT)],contents||""));},makeCellLayout:function(){var html="";var consoleContainer=HtmlUtils.div(["id",this.getDomId(ID_CONSOLE_CONTAINER)]);if(this.showInput()&&this.layout=="horizontal"){var left=HtmlUtils.div(["id",this.getDomId(ID_INPUTS)]);var right=HtmlUtils.div(["id",this.getDomId(ID_OUTPUTS)]);var center=HtmlUtils.div([],"");left+=consoleContainer;html="<table width=100%><tr valign=top><td width=50%>"+left+"</td><td style='border-left:1px #ccc solid;' width=1>"+center+"</td><td width=49%>"+right+"</td></tr></table>";}else{this.jq(ID_CELLS_BOTTOM).html(consoleContainer);}
this.jq(ID_CELLS).html(html);},log:function(msg,type,from,div){var icon="";var clazz="display-notebook-console-item";if(type=="error"){clazz+=" display-notebook-console-item-error";icon=HtmlUtils.image(Utils.getIcon("cross-octagon.png"));if(div){div.append(msg);}}else if(type=="output"){clazz+=" display-notebook-console-item-output";icon=HtmlUtils.image(Utils.getIcon("arrow-000-small.png"));}else if(type=="info"){clazz+=" display-notebook-console-item-info";icon=HtmlUtils.image(Utils.getIcon("information.png"));}
...
this.console.scrollTop(height-200);},layoutCells:function(){for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];cell.prepareToLayout();}
...
await cmd.proc.call(_this,cmd.line,cmd.toks,cmd.div,cmd.extra);}},processJs:async function(chunk,global){try{await this.getCurrentEntry(e=>{current=e});var state=new NotebookState(this,chunk.div);notebookStates[state.id]=state;var notebookEntries=this.notebook.getCurrentEntries();for(name in notebookEntries){state.entries[name]=notebookEntries[name].entry;}
...
nbCell=this;nbNotebook=this.notebook;var js=chunk.content.trim();var lines=js.split("\n");if(!global){if(lines.length==1&&!js.startsWith("return ")&&!js.startsWith("{")){js="return "+js;}
if(lines.length>1||js.startsWith("return ")||js.startsWith("{")){js="async function nbFunc() {\n"+jsSet+"\n"+js+"\n}";}
eval(js);await nbFunc().then(r=>{state.result=r;});}else{state.result=eval(js);console.log("r: "+state.result);}
if(state.getStop()){chunk.ok=false;}
