...
if(polygonField){var toks=values[polygonField.getIndex()].split(";");var p=[];for(var pIdx=0;pIdx<toks.length;pIdx+=2){var lat=parseFloat(toks[pIdx]);var lon=parseFloat(toks[pIdx+1]);p.push(new OpenLayers.Geometry.Point(lon,lat));}
var poly=this.map.addPolygon("polygon"+pIdx,"",p,props);if(date){poly.date=date.getTime();}
this.lines.push(poly);}
var showSegments=this.getProperty("showSegments",false);var html=this.getRecordHtml(pointRecord,fields);if(showSegments&&latField1&&latField2&&lonField1&&lonField2){var lat1=values[latField1.getIndex()];var lat2=values[latField2.getIndex()];var lon1=values[lonField1.getIndex()];var lon2=values[lonField2.getIndex()];var attrs={};if(props.fillColor)
