...
if(!template)
template=this.getProperty("recordTemplate");if(template){if(template!="${default}"){var row=this.getDataValues(record);return this.getRecordTemplate(row,fields,template);}}
...
var newData=[];var values=[];for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];var prefix=this.getProperty(filterField.getId()+".filterPrefix");var suffix=this.getProperty(filterField.getId()+".filterSuffix");if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;var value;var _values=[];var regexps=[];if(filterField.isNumeric){var minField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_min"));var maxField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_max"));var minValue=parseFloat(minField.val().trim());var maxValue=parseFloat(maxField.val().trim());var dfltMinValue=parseFloat(minField.attr("data-min"));var dfltMaxValue=parseFloat(maxField.attr("data-max"));if(minValue!=dfltMinValue||maxValue!=dfltMaxValue){value=[minValue,maxValue];}else{value=[NaN,NaN];}}else if(filterField.getType()=="date"){var date1=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date1")).val();var date2=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date2")).val();if(date1!=null&&date1.trim()!="")
...
enums=toks;}
var includeAll=this.getProperty(filterField.getId()+".includeAll",true);if(enums==null){var allName=this.getProperty(filterField.getId()+".allName","All");enums=[];var enumValues=[];var seen={};var valuesAreNumbers=true;records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(!seen[value]){seen[value]=true;if((+value+"")!=value)valuesAreNumbers=false;var label=value;if(label.length>20){label=label.substring(0,19)+"...";}
if(typeof value=="string")
value=value.replace(/\'/g,"&apos;");var tuple=[value,label];enumValues.push(tuple);}});enumValues.sort((a,b)=>{if(valuesAreNumbers){return+a-+b;}
return(""+a[1]).localeCompare(""+b[1]);});if(includeAll){enums=Utils.mergeLists([[FILTER_ALL,allName]],enums);}
...
if(this.getProperty(filterField.getId()+".filterShowButtons")){if(!includeAll&&dfltValue==FILTER_ALL)dfltValue=enums[0];var buttons="";for(var j=0;j<enums.length;j++){var extra="";var v=enums[j];var label;if(Array.isArray(v)){label=v[1];v=v[0];}else{label=v;}
if(v==dfltValue)extra=" display-filterby-button-selected ";buttons+=HtmlUtils.div(["class","display-filterby-button"+extra,"value",v],label);}
bottom+=HtmlUtils.div(["value",dfltValue,"class","display-filterby-buttons","id",widgetId,"fieldId",filterField.getId()],buttons);continue;}else{widget=HtmlUtils.select("",attrs,enums,dfltValue);}}else if(filterField.isNumeric){var min=0;var max=0;var cnt=0;records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(isNaN(value))return;if(cnt==0){min=value;max=value;}
...
_this.propagateEvent("handleEventPropertyChanged",args);_this.settingFilterValue=false;};this.jq(ID_FILTERBAR).find(".display-filterby-buttons").each(function(){let parent=$(this);$(this).find(".display-filterby-button").click(function(){parent.find(".display-filterby-button").removeClass("display-filterby-button-selected");$(this).addClass("display-filterby-button-selected");var value=$(this).attr("value");parent.attr("value",value);inputFunc(parent);});});this.jq(ID_FILTERBAR).find(".display-filter-input").keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){return;}
...
myTags.map(tag=>t.push(tag));return t;},makeChartOptions:function(dataList,props,selectedFields){chartOptions=SUPER.makeChartOptions.call(this,dataList,props,selectedFields);var useMultipleAxes=this.getProperty("useMultipleAxes",true);chartOptions.height=this.getProperty("chartHeight",this.getProperty("height","150"));if(!chartOptions.legend)
chartOptions.legend={};$.extend(chartOptions.legend,{position:this.getProperty("legendPosition",'bottom')});if(!chartOptions.chartArea){chartOptions.chartArea={};}
$.extend(chartOptions.chartArea,{left:this.getProperty("chartLeft",this.chartDimensions.left),right:this.getProperty("chartRight",this.chartDimensions.right),top:this.getProperty("chartTop","10"),bottom:this.getProperty("chartBottom"),height:this.getProperty("chartHeight","70%"),width:this.getProperty("chartWidth",this.chartDimensions.width),});if(useMultipleAxes){$.extend(chartOptions,{series:[{targetAxisIndex:0},{targetAxisIndex:1}]});}
...
function RamaddaTemplateDisplay(displayManager,id,properties){if(!Utils.isDefined(properties.showTitle))properties.showTitle=false;if(!Utils.isDefined(properties.showMenu))properties.showMenu=false;if(!Utils.isDefined(properties.displayStyle))properties.displayStyle="background:rgba(0,0,0,0);";let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_TEMPLATE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Template Attributes","template=\"\"","headerTemplate=\"... ${totalCount} ... ${selectedCount}\"","footerTemplate=\"... ${totalCount} ... ${selectedCount}\"","emptyMessage=\"\"","select=\"max|min|<|>|=|<=|>=|contains\"","selectField=\"\"","selectValue=\"\"",'${&lt;field&gt;_total}','${&lt;field&gt;_max}','${&lt;field&gt;_min}','${&lt;field&gt;_average}',]);},updateUI:function(){var pointData=this.getData();if(pointData==null)return;var records=this.filterData();if(!records)return;records=this.sortRecords(records);var fields=pointData.getRecordFields();var template=this.getProperty("template","");var select=this.getProperty("select","all");var selected=[];var summary={};records.map(r=>{r=this.getDataValues(r);for(var i=0;i<fields.length;i++){var f=fields[i];var v=f.getValue(r);if(!summary[f.getId()]){summary[f.getId()]={total:0,min:v,max:v,count:0,uniques:{},uniqueCount:0}}
var s=summary[f.getId()];if(f.isString()){if(!s.uniques[v]){s.uniqueCount++;s.uniques[v]=true;}
continue;}
if(f.isDate&&v.getTime){if(v.getTime()<s.min.getTime())s.min=v;if(v.getTime()>s.max.getTime())s.max=v;}else if(!isNaN(v)){s.total+=v;if(f.getLabel()=="Dead"&&v>1000)console.log(f.getLabel()+" v:"+v+" t:"+s.total);s.min=Math.min(s.min,v);s.max=Math.max(s.max,v);s.count++;}}});for(var i=0;i<fields.length;i++){var f=fields[i];if(!f.isNumeric)continue;var s=summary[f.getId()];if(s&&s.count){s.average=Utils.formatNumber(s.total/s.count);}}
...
if(!f.isNumeric)continue;if(s){headerTemplate=headerTemplate.replace("${"+f.getId()+"_total}",s.total).replace("${"+f.getId()+"_min}",s.min).replace("${"+f.getId()+"_max}",s.max).replace("${"+f.getId()+"_average}",s.average);footerTemplate=footerTemplate.replace("${"+f.getId()+"_total}",s.total).replace("${"+f.getId()+"_min}",s.min).replace("${"+f.getId()+"_max}",s.max).replace("${"+f.getId()+"_average}",s.average);}}
...
var row=this.getDataValues(record);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);s=this.getRecordTemplate(row,fields,s,props);s=s.replace(/\${recordIndex}/g,(rowIdx+1));var tag=HtmlUtils.openTag("div",["style",color?"background: "+color:"","id",this.getId()+"-"+record.getId(),"title","","class","display-template-record","recordIndex",rowIdx]);if(s.startsWith("<td")){s=s.replace(/<td([^>]*)>/,"<td $1>"+tag);s=s.replace(/<\/td>$/,"</div></td>");contents+=s;}else if(s.startsWith("<tr")){s=s.replace(/<td([^>]*)>/g,"<td $1>"+tag);s=s.replace(/<\/td>/g,"</div></td>");contents+=s;}else{contents+=tag+s+"</div>"}
...
this.jq(ID_NEXT).show();var record=this.records[this.slideIndex];var row=this.getDataValues(record);var html=this.getRecordTemplate(row,this.fields,this.getProperty("template",""));html=html.replace(/\${recordIndex}/g,(this.slideIndex+1));this.jq(ID_SLIDE).html(html);var args={highlight:true,record:record};if(propagateEvent)
