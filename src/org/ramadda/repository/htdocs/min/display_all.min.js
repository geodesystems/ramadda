...
tuples.map(t=>{if(template){var h=template.replace("${field}",t.field.getLabel()).replace("${value1}",t.val1).replace("${value2}",t.val2).replace("${percent}",this.formatNumber(t.percent)).replace("${date1}",label1).replace("${date2}",label2);h=h.replace(/\${per_hour}/g,this.formatNumber(t.percent/hours));h=h.replace(/\${per_day}/g,this.formatNumber(t.percent/days));h=h.replace(/\${per_week}/g,this.formatNumber(t.percent/(days/7)));h=h.replace(/\${per_month}/g,this.formatNumber(months));h=h.replace(/\${per_year}/g,this.formatNumber(years));html+=h;}else{html+=HtmlUtils.tr([],HtmlUtils.td([],t.field.getLabel())+
