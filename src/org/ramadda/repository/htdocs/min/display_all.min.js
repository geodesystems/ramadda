...
form+="</form>";tabTitles.push("Display");tabContents.push(form);},popup:function(srcId,popupId){var popup=GuiUtils.getDomObject(popupId);var srcObj=GuiUtils.getDomObject(srcId);if(!popup||!srcObj)return;var myalign='left top';var atalign='left bottom';showObject(popup);jQuery("#"+popupId).position({of:jQuery("#"+srcId),my:myalign,at:atalign,collision:"none none"});jQuery("#"+popupId).position({of:jQuery("#"+srcId),my:myalign,at:atalign,collision:"none none"});$("#"+popupId).draggable();},checkLayout:function(){},displayData:function(){},createUI:function(){var divid=this.getProperty(PROP_DIVID);if(divid!=null){var html=this.getHtml();$("#"+divid).html(html);}else{console.log("error: no div defined for display:"+this.getType());}},setDisplayReady:function(){this.displayReady=true;},getDisplayReady:function(){return this.displayReady;},pageHasLoaded:function(){this.setDisplayReady(true);},initDisplay:function(){this.createUI();this.checkSearchBar();},checkSearchBar:function(){let _this=this;var pointData=this.getData();if(pointData==null)return;this.searchFields=[];var searchBy=this.getProperty("searchFields","",true).split(",");var fields=pointData.getRecordFields();var records=pointData.getRecords();if(searchBy.length>0){var searchBar="";for(var i=0;i<searchBy.length;i++){var searchField=this.getFieldById(fields,searchBy[i]);if(!searchField)continue;this.searchFields.push(searchField);var widget;var widgetId=this.getDomId("searchby_"+searchField.getId());if(searchField.getType()=="enumeration"){var enums=[["","All"]];var seen={};records.map(record=>{var value=this.getDataValues(record)[searchField.getIndex()];if(!seen[value]){seen[value]=true;var label=value;if(label.length>20){label=label.substring(0,19)+"...";}
var tuple=[value,label];enums.push(tuple);}});widget=HtmlUtils.select("",["id",widgetId],enums);}else{widget=HtmlUtils.input("","",["id",widgetId]);}
widget=searchField.getLabel()+": "+widget;if(i==0)searchBar+="<br>Display: ";searchBar+=widget+"&nbsp;&nbsp;";}
this.jq(ID_HEADER2).html(searchBar);this.jq(ID_HEADER2).find("input, input:radio,select").change(function(){var id=$(this).attr("id");_this.doSearch();});}
if(!this.getProperty("showFilterWidget",true))return;var filterValues=this.getProperty("filterValues");var filterValuesLabel=this.getProperty("filterValuesLabel","");if(!filterValues){var filterFieldId=this.getProperty("patternFilterField");if(filterFieldId){var filterField=this.getFieldById(null,filterFieldId);if(filterField){if(filterValuesLabel=="")filterValuesLabel=filterField.getLabel()+": ";filterValues=this.getColumnValues(records,filterField).values;filterValues=Utils.getUniqueValues(filterValues);filterValues.unshift("");}}}
...
menu+="</select>";this.writeHtml(ID_TOP_RIGHT,filterValuesLabel+menu);this.jq("filterValueMenu").change(function(){var value=$(this).val();if(_this.getProperty("filterPattern")==value)return;if(value=="")value=null;_this.setProperty("filterPattern",value);_this.updateUI();_this.propagateEvent("handleEventPropertyChanged",{property:"filterPattern",value:value});});}},updateUI:function(){},getDoBs:function(){if(!(typeof this.dobs==='undefined')){return dobs;}
...
var records=pointData.getRecords();var allFields=this.getData().getRecordFields();var fields=this.getSelectedFields(allFields);if(fields.length==0)
fields=allFields;if(!this.getDisplayReady()){return;}
this.updateUI();if(!reload){this.lastPointData=pointData;this.propagateEvent("handleEventPointDataLoaded",pointData);}},doSearch:function(){var records=[];var values=[];for(var i=0;i<this.searchFields.length;i++){var searchField=this.searchFields[i];values.push($("#"+this.getDomId("searchby_"+searchField.getId())).val());}
for(var rowIdx=0;rowIdx<this.records.length;rowIdx++){var row=this.getDataValues(this.records[rowIdx]);var ok=true;for(var i=0;i<this.searchFields.length;i++){var searchField=this.searchFields[i];var searchValue=values[i];if(searchValue=="")continue;var value=row[searchField.getIndex()];if(searchField.getType()=="enumeration"){if(value!=searchValue){ok=false;break;}}else{value=(""+value).toLowerCase();if(value.indexOf(searchValue.toLowerCase())<0){ok=false;break;}}}
if(ok)records.push(this.records[rowIdx]);}
this.displaySearchResults(records);},displaySearchResults:function(records){},getDateFormatter:function(){var date_formatter=null;if(this.googleLoaded()){var df=this.getProperty("dateFormat",null);if(df){var tz=0;this.timezone=this.getProperty("timezone");if(Utils.isDefined(this.timezone)){tz=parseFloat(this.timezone);}
...
props[name.toLowerCase()]=props[name];this.div=new Div(null,"display-notebook-chunk");this.cell=cell;$.extend(this,{getContent:function(){var content=this.content;console.log("chunk.getContent:"+content);for(name in this.cell.notebook.globals){var value=this.cell.notebook.getGlobalValue(name);if(typeof value=="object"){value=Utils.formatJson(value);}
console.log("   name:"+name+"="+value);content=content.replace("${"+name.trim()+"}",value);}
...
if(this.haveCalledUpdateUI)return;this.haveCalledUpdateUI=true;var fields=pointData.getRecordFields();var bounds={};var points=RecordUtil.getPoints(records,bounds);if(isNaN(bounds.north)){console.log("no bounds:"+bounds);return;}
