...
function RamaddaTimelineDisplay(displayManager,id,properties){var ID_TIMELINE="timeline";let SUPER=new RamaddaFieldsDisplay(displayManager,id,DISPLAY_TIMELINE,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);Utils.importJS(ramaddaBaseUrl+"/lib/timeline3/timeline.js");$('<link rel="stylesheet" href="'+"https://cdn.knightlab.com/libs/timeline3/latest/css/timeline.css"+'" type="text/css" />').appendTo("head");$.extend(this,{needsData:function(){return true;},getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Timeline",'justTimeline="true"','titleField=""','startDateField=""','endDateField=""','textTemplate=""','timeTo="year|day|hour|second"','justTimeline="true"','startAtEnd=true','navHeight=250']);},loadCnt:0,timelineLoaded:false,updateUI:function(){if(!this.timelineLoaded){try{var tmp=TL.Timeline;this.timelineLoaded=true;}catch(err){if(this.loadCnt++<100){setTimeout(()=>this.updateUI(),100);return;}}}
if(!this.timelineLoaded){this.writeHtml(ID_DISPLAY_CONTENTS,"Could not load timeline");return;}
var records=this.filterData();if(records==null)return;var timelineId=this.getDomId(ID_TIMELINE);this.writeHtml(ID_DISPLAY_CONTENTS,HtmlUtils.div(["id",timelineId]));this.timelineReady=false;var opts={start_at_end:this.getProperty("startAtEnd",false),timenav_height:this.getProperty("navHeight",250),gotoCallback:(slide)=>{if(this.timelineReady){var record=records[slide];if(record){this.getDisplayManager().notifyEvent("handleEventRecordSelection",this,{record:record});}}}};var json={};var events=[];json.events=events;var titleField=this.getFieldById(null,this.getProperty("titleField"));var startDateField=this.getFieldById(null,this.getProperty("startDateField"));var endDateField=this.getFieldById(null,this.getProperty("endDateField"));var textTemplate=this.getProperty("textTemplate","${default}");var timeTo=this.getProperty("timeTo","day");this.recordToIndex={};for(var i=0;i<records.length;i++){var record=records[i];this.recordToIndex[record.getId()]=i;var tuple=record.getData();var event={};var text=this.getRecordHtml(record,null,textTemplate);event.text={headline:titleField?tuple[titleField.getIndex()]:" record:"+(i+1),text:text};if(startDateField)
...
events.push(event);}
this.timeline=new TL.Timeline(timelineId,json,opts);if(this.getProperty("",false)){this.jq(ID_TIMELINE).find(".tl-storyslider").css("display","none");}
