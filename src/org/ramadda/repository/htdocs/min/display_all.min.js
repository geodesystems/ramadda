...
if(!valueField){this.setContents(this.getMessage("No value field specified"));return;}
let labels=[];let parents=[];let values=[];let calcValue=function(node){if(node.children.length==0){var value=node.record.getValue(valueField.getIndex());node.value=value;return value;}
...
roots.map(calcValue);this.myRecords=[];let recordList=this.myRecords;let makeList=function(node){recordList.push(node.record);values.push(node.value);labels.push(node.label);parents.push(node.parent==null?"":node.parent.label);node.children.map(makeList);}
roots.map(makeList);var data=[{type:"sunburst",labels:labels,parents:parents,values:values,outsidetextfont:{size:20,color:"#377eb8"},leaf:{opacity:0.4},marker:{line:{width:2}},branchvalues:'total'}];var layout={margin:{l:0,r:0,b:0,t:0},width:+this.getProperty("width"),height:+this.getProperty("height"),};var myPlot=this.makePlot(data,layout);myPlot.on('plotly_sunburstclick',d=>{this.handleSunburstClickEvent(d)});},handleSunburstClickEvent:function(data){if(!data.points||data.points.length<=0){return;}
