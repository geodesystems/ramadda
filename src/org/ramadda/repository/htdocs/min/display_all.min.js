...
if(enums==null){var includeAll=this.getProperty(filterField.getId()+".includeAll",true);enums=includeAll?[["","All"]]:[];var seen={};records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(!seen[value]){seen[value]=true;var label=value;if(label.length>20){label=label.substring(0,19)+"...";}
var tuple=[value,label];enums.push(tuple);}});}
...
this.headers.push("");this.showBlocks(true);HtmlUtils.callWhenScrolled(this.getDomId(ID_DISPLAY_CONTENTS),()=>{if(!this.displayedBlocks){this.displayedBlocks=true;this.showBlocks(false);}},500);},showBlocks:function(initial,step){if(!Utils.isDefined(step)){if(initial)step=this.counts.length;else step=0;}
var contents="";contents+=HtmlUtils.div(["class","display-block-header"],this.getProperty("header",""));contents+=HtmlUtils.openDiv(["class","display-blocks"]);var tmp=this.getProperty("colors","red,blue,gray,green",true);var ct=(typeof tmp)=="string"?tmp.split(","):tmp;var footer="";while(ct.length<this.counts.length){ct.push(ct[ct.length-1]);}
var multiplier=parseFloat(this.getProperty("multiplier","1",true));var dim=this.getProperty("blockDimensions","8",true);var labelStyle=this.getProperty("labelStyle","",true);for(var i=0;i<this.counts2.length;i++){var label=this.footers[i].replace("${count}",multiplier*this.counts[i]);var style="width:"+dim+"px;height:"+dim+"px;";if(!initial){if(i<step){style+="background:"+ct[i]+";";footer+=HtmlUtils.div(["class","display-block","style",style],"")+" "+HtmlUtils.span(["style",labelStyle],label)+"&nbsp;&nbsp;";}else{footer+="&nbsp;&nbsp;";}}
var cnt=this.counts2[i];for(var j=0;j<this.counts2[i];j++){contents+=HtmlUtils.div(["class","display-block","style",style,"title",label],"");}}
contents+=HtmlUtils.closeDiv();contents+=HtmlUtils.div(["class","display-block-footer"],footer);this.writeHtml(ID_DISPLAY_CONTENTS,contents);if(step<this.counts.length){setTimeout(()=>{this.showBlocks(false,step+1)},1000);}}})}
...
var boundsAnimation=this.getProperty("boundsAnimation");let _this=this;if(boundsAnimation){this.didAnimationBounds=false;let animationBounds=boundsAnimation.split(",");if(animationBounds.length==4){var pause=parseFloat(this.getProperty("animationPause","3000"));HtmlUtils.callWhenScrolled(this.getDomId(ID_MAP),()=>{if(this.didAnimationBounds)return;this.didAnimationBounds=true;var a=animationBounds;var b=createBounds(parseFloat(a[1]),parseFloat(a[2]),parseFloat(a[3]),parseFloat(a[0]));_this.map.setViewToBounds(b);},pause);}}
