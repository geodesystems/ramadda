...
if(record.getLongitude()<-180||record.getLatitude()>90){console.log("Bad index="+j+" "+record.getLatitude()+" "+record.getLongitude());}
...
var windowStart=this.animation.begin.getTime();var windowEnd=this.animation.end.getTime();var atLoc={};for(var i=0;i<this.points.length;i++){var point=this.points[i];if(point.date<windowStart||point.date>windowEnd){point.style.display='none';continue;}
...
if(points.length==0){}
this.addPoints(records,fields,points);this.addLabels(records,fields,points);this.applyVectorMap();},addPoints:function(records,fields,points){var source=this;var radius=parseFloat(this.getDisplayProp(source,"radius",8));var strokeWidth=parseFloat(this.getDisplayProp(source,"strokeWidth","1"));var strokeColor=this.getDisplayProp(source,"strokeColor","#000");var colorByAttr=this.getDisplayProp(source,"colorBy",null);var colors=this.getColorTable(true);var sizeByAttr=this.getDisplayProp(source,"sizeBy",null);var isTrajectory=this.getDisplayProp(source,"isTrajectory",false);if(isTrajectory){var attrs={strokeWidth:2,strokeColor:"blue"}
...
if(showPoints){if(!Utils.isDefined(seen[point]))seen[point]=0;if(seen[point]>500)continue;seen[point]++;var mapPoint;if(pointIcon){mapPoint=this.map.addMarker("pt-"+i,point,pointIcon,"pt-"+i,html);}else{if(!props.graphicName)
props.graphicName=this.getProperty("shape","circle");mapPoint=this.map.addPoint("pt-"+i,point,props,html,dontAddPoint);}
var date=pointRecord.getDate();if(date){mapPoint.date=date.getTime();}
