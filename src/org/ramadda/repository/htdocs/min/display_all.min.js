...
for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];if(!cell.runFirst)continue;if(cell.hasRun)continue;await cell.run(result=>ok=result);if(!ok)break;var content=cell.getOutputContent();if(Utils.stringDefined(cell.cellName)){this.cellValues[cell.cellName]=content;}}
if(!ok)return;for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];if(cell.runFirst)continue;if(cell.hasRun)continue;await cell.run(result=>ok=result);if(!ok)break;var content=cell.getOutputContent();if(Utils.stringDefined(cell.cellName)){this.cellValues[cell.cellName]=content;}}},toggleAll:function(on){this.cells.map(cell=>{cell.showInput=on;cell.applyStyle();});},});}
...
if(this.showBorder){this.cell.css("border-top","1px #ccc solid");}else{this.cell.css("border-top","1px #fff solid");}},getPopup:function(){return this.notebook.getPopup();},askDelete:function(){let _this=this;var menu="";menu+="You sure you want to delete this cell?<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","yes"],"Yes")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","cancel"],"No");var popup=this.getPopup();popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();popup.position({of:this.menuButton,my:"left top",at:"right top",collision:"fit fit"});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");popup.hide();if(what=="yes"){_this.notebook.deleteCell(_this);}});},run:async function(callback){if(this.running)return Utils.call(callback,true);this.running=true;try{var ok=true;await this.runInner().then(r=>ok=r);if(!ok){this.running=false;return Utils.call(callback,false);}
...
this.output.html(result);for(var i=0;i<blobs.length;i++){var blob=blobs[i];await this.processBlob(blob);if(!blob.ok){console.log("not ok");return false;}}
...
this.output.html(h);this.outputUpdated();},outputUpdated:function(){this.outputHtml=this.jq(ID_OUTPUT).html();},getOutputContent:function(){return this.jq(ID_OUTPUT).html();},focus:function(){this.input.focus();},clearOutput:function(){this.output.html("");this.outputHtml="";},processHtml:async function(blob){blob.div.set(blob.blob);},processWiki:async function(blob){var id=this.notebook.getProperty("entryId","");await this.getCurrentEntry(e=>entry=e);if(entry)id=entry.getId();let _this=this;let divId=HtmlUtils.getUniqueId();var wikiCallback=function(html){var h=HtmlUtils.div(["id",divId,"style"],html);blob.div.set(h);}
...
var html="";if(Utils.stringDefined(state.prefix))html+=state.prefix;if(state.result)html+=state.result;blob.div.append(html);}catch(e){blob.ok=false;var lines=blob.blob.split("\n");var line=lines[e.lineNumber-2];blob.div.append("Error: "+e.message+"<br>"+HtmlUtils.span(["class","display-notebook-error"]," &gt; "+line));console.log(e.stack);}
