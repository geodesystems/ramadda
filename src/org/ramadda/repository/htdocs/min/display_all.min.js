...
var get=this.getGet();var button="";if(this.getShowMenu()){button=HtmlUtils.onClick(get+".showDialog();",HtmlUtils.image(ramaddaBaseUrl+"/icons/downdart.png",[ATTR_CLASS,"display-dialog-button",ATTR_ID,this.getDomId(ID_DIALOG_BUTTON)]));}
...
var titleToShow=this.getShowTitle()?this.getDisplayTitle(title):"";titleDiv=HtmlUtils.tag("span",[ATTR_CLASS,"display-title",ATTR_ID,this.getDomId(ID_TITLE)],titleToShow);if(button==""){left=titleDiv;}else{left="<div class=display-header>"+button+"&nbsp;"+titleDiv+"</div>";}}
var right=HtmlUtils.div(["id",this.getDomId(ID_TOP_RIGHT)],"");html+=HtmlUtils.leftRightTable(left,right,{valign:"bottom"});var contents=this.getContentsDiv();html+=contents;html+=HtmlUtils.closeTag(TAG_DIV);if(dobs){html+=HtmlUtils.closeTag(TAG_DIV);}
...
var msg="";if(data&&data.errorcode&&data.errorcode=="warning"){msg=data.error;}else{msg="<b>Sorry, but an error has occurred:</b>";if(!data)data="No data returned from server";var error=data.error?data.error:data;error=error.replace(/<[^>]*>/g,"");error=error.replace(/\n\n+/g,"\n");error=HtmlUtils.tag("pre",["style","max-height:300px;overflow-y:auto;max-width:100%;overflow-x:auto;"],error);msg+=HtmlUtils.tag("div",["style","background:#fff;margin:10px;padding:10px;border:1px #ccc solid;   border-radius: 0px;"],error);}
...
return false;return true;},showGutter:function(){if(this.displayMode&&!this.user)
...
var notebookStates={};function RamaddaNotebookCell(notebook,id,content,props){this.notebook=notebook;var ID_CELL="cell";var ID_GUTTER="gutter";var ID_GUTTER_CONTAINER="guttercontainer";var ID_HEADER="header";var ID_INPUT="input";var ID_INPUT_TOOLBAR="inputtoolbar";var ID_OUTPUT="output";var ID_MESSAGE="message";var ID_BUTTON_MENU="menubutton";var ID_BUTTON_RUN="runbutton";var ID_BUTTON_TOGGLE="togglebutton";var ID_MENU="menu";var ID_CELLNAME_INPUT="cellnameinput";var ID_SHOWHEADER_INPUT="showheader";var ID_SHOWBORDER="showborder";var ID_SHOWEDIT="showedit";var ID_RUN_ON_LOAD="runonload";var ID_DISPLAY_MODE="displaymode";var ID_LAYOUT_COLUMNS="layoutcolumns";var ID_RUNFIRST="runfirst";var ID_SHOW_OUTPUT="showoutput";let SUPER=new DisplayThing(id,{});RamaddaUtil.inherit(this,SUPER);RamaddaUtil.defineMembers(this,{id:id,inputRows:1,index:0,content:content,outputHtml:"",showInput:false,showHeader:true,showBorder:false,cellName:"",runFirst:false,showOutput:true,});if(props){$.extend(this,props);}
RamaddaUtil.defineMembers(this,{getJson:function(output){var obj={id:this.id,inputRows:this.inputRows,content:this.content,showInput:this.showInput,showHeader:this.showHeader,showBorder:this.showBorder,runFirst:this.runFirst,showOutput:this.showOutput,cellName:this.cellName,};if(this.currentEntry)
...
this.makeButton(ID_BUTTON_RUN,Utils.getIcon("runall.png"),"Run all","runall");var hbuttons=this.makeButton(ID_BUTTON_MENU,icon_menu,"Show menu","showmenu",true)+
...
this.input.focus();},insertTags:function(tagOpen,tagClose,sampleText){var id=this.getDomId(ID_INPUT);var textComp=GuiUtils.getDomObject(id);insertTagsInner(id,textComp.obj,tagOpen,tagClose,sampleText);this.calculateInputHeight();},insertText:function(value){var id=this.getDomId(ID_INPUT);var textComp=GuiUtils.getDomObject(id);insertAtCursor(id,textComp.obj,value);this.calculateInputHeight();},showNotebookMenu:function(){var link=this.jq("toolbar_notebook");this.makeMenu(link,"left bottom");},makeButton:function(id,icon,title,command,horiz){if(!command)command="noop";var style="";if(horiz)style="display:inline-block;";return HtmlUtils.div(["what",command,"title",title,"class","display-notebook-menu-button","style",style,"id",this.getDomId(id)],HtmlUtils.image(icon,[]));},makeMenu:function(src,at){if(!src){src=this.input;}
...
menu+=line;menu+="<tr valign=top><td align=right><b>Show:</b>&nbsp;</td><td>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_SHOWEDIT),[],_this.showInput)+" Input";menu+=space;menu+=HtmlUtils.checkbox(_this.getDomId(ID_SHOW_OUTPUT),[],_this.showOutput)+" Output";menu+="</td></tr>"
menu+="<tr><td align=right><b>Inputs:</b>&nbsp;</td><td>";menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","showall"],"Show All");menu+=space;menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","hideall"],"Hide All");menu+="</td></tr>";menu+=line;menu+="</table>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_RUNFIRST),[],_this.runFirst)+" Run first"+"<br>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_RUN_ON_LOAD),[],_this.notebook.runOnLoad)+" Run on load"+"<br>";menu+=HtmlUtils.div(["title","Don't show the left side and input for anonymous users"],HtmlUtils.checkbox(_this.getDomId(ID_DISPLAY_MODE),[],_this.notebook.displayMode)+" Display mode"+"<br>");menu+=line2;var cols=this.notebook.columns;var colId=_this.getDomId(ID_LAYOUT_COLUMNS);menu+="Columns: ";menu+=HtmlUtils.input(colId,this.notebook.columns,["size","3","id",_this.getDomId(ID_LAYOUT_COLUMNS)]);menu+="<br>";menu+=line2;menu+=HtmlUtils.div(["class","ramadda-link","what","savewithout"],"Save Notebook (w/o output)")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","savewith"],"Save Notebook (with output)")+"<br>";menu+=line2;menu+=HtmlUtils.div(["class","ramadda-link","what","delete"],"Delete")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","help"],"Help")+"<br>";var popup=this.getPopup();popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();popup.position({of:src,my:"left top",at:at,collision:"fit fit"});_this.jq(ID_SHOWHEADER_INPUT).focus();_this.jq(ID_SHOWHEADER_INPUT).change(function(e){_this.showHeader=_this.jq(ID_SHOWHEADER_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_RUNFIRST).change(function(e){_this.runFirst=_this.jq(ID_RUNFIRST).is(':checked');});_this.jq(ID_SHOW_OUTPUT).change(function(e){_this.showOutput=_this.jq(ID_SHOW_OUTPUT).is(':checked');_this.applyStyle();});_this.jq(ID_RUN_ON_LOAD).change(function(e){_this.notebook.runOnLoad=_this.jq(ID_RUN_ON_LOAD).is(':checked');});_this.jq(ID_DISPLAY_MODE).change(function(e){_this.notebook.displayMode=_this.jq(ID_DISPLAY_MODE).is(':checked');});_this.jq(ID_SHOWBORDER).change(function(e){_this.showBorder=_this.jq(ID_SHOWBORDER).is(':checked');_this.applyStyle();});_this.jq(ID_SHOWEDIT).change(function(e){_this.showInput=_this.jq(ID_SHOWEDIT).is(':checked');_this.applyStyle();});_this.jq(ID_LAYOUT_COLUMNS).keypress(function(e){var keyCode=e.keyCode||e.which;if(keyCode!=13){return;}
...
_this.notebook.columns=cols;popup.hide();_this.notebook.layoutCells();});_this.jq(ID_CELLNAME_INPUT).keypress(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){_this.cellName=$(this).val();_this.header.html("&nbsp;"+_this.cellName);_this.applyStyle();popup.hide();return;}});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");_this.processCommand(what);});},processCommand:function(command){if(command=="run"){this.notebook.runCell(this);}else if(command=="showmenu"){this.makeMenu();}else if(command=="toggle"){this.showInput=!this.showInput;this.applyStyle();}else if(command=="showthis"){this.showInput=true;this.applyStyle();}else if(command=="hidethis"){this.showInput=false;this.applyStyle();}else if(command=="showall"){this.notebook.toggleAll(true);}else if(command=="hideall"){this.notebook.toggleAll(false);}else if(command=="runall"){this.notebook.runAll();}else if(command=="clear"){this.clearOutput();}else if(command=="clearall"){this.notebook.clearOutput();}else if(command=="moveup"){this.notebook.moveCellUp(this);}else if(command=="movedown"){this.notebook.moveCellDown(this);}else if(command=="newabove"){this.notebook.newCellAbove(this);}else if(command=="newbelow"){this.notebook.newCellBelow(this);}else if(command=="savewith"){this.getPopup().hide();this.notebook.saveNotebook(true);}else if(command=="savewithout"){this.getPopup().hide();this.notebook.saveNotebook(false);}else if(command=="help"){this.getPopup().hide();var win=window.open(ramaddaBaseUrl+"/userguide/notebook.html",'_blank');win.focus();}else if(command=="delete"){this.askDelete();}else{console.log("unknown command:"+command);}},checkHover:function(vis){if(Utils.isDefined(this.lastHover)){if(vis==this.lastHover)return;}
this.lastHover=vis;var showingHeader=this.header.is(":visible");if(vis){if(!showingHeader)
this.gutter.css("display","block");this.cell.find(".display-notebook-gutter-container").css("background","rgb(250,250,250)");}else{this.gutter.css("display","none");this.cell.find(".display-notebook-gutter-container").css("background","#fff");}
if(showingHeader)
this.gutter.css("display","none");},applyStyle:function(){this.checkHover(false);if(this.showHeader){this.header.css("display","block");}else{this.header.css("display","none");}
if(this.showInput&&this.notebook.showInput()){this.jq(ID_INPUT_TOOLBAR).css("display","block");this.inputContainer.show("show");}else{this.jq(ID_INPUT_TOOLBAR).css("display","none");this.inputContainer.hide();}
if(this.showOutput){this.output.css("display","block");}else{this.output.css("display","none");if(!this.showInput){if(this.notebook.showInput()){this.header.css("background","rgb(250,250,250)");this.header.css("display","block");}}}
if(this.showBorder){this.cell.css("border-top","1px #ccc solid");}else{this.cell.css("border-top","1px #fff solid");}},getPopup:function(){return this.notebook.getPopup();},askDelete:function(){let _this=this;var menu="";menu+="Are you sure you want to delete this cell?<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","yes"],"Yes")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","cancel"],"No");var popup=this.getPopup();popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();var src=this.input;if(!src.is(":visible")){src=this.output;}
if(!src.is(":visible")){src=this.header;}
popup.position({of:src,my:"left top",at:"left top",collision:"fit fit"});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");popup.hide();if(what=="yes"){_this.notebook.deleteCell(_this);}});},run:async function(callback){if(this.running)return Utils.call(callback,true);this.running=true;try{var ok=true;await this.runInner().then(r=>ok=r);if(!ok){this.running=false;return Utils.call(callback,false);}
...
if(dataList.length>0&&this.getDataValues(dataList[0]).length>1){this.chartOptions.hAxis={title:this.getDataValues(dataList[0])[0]};this.chartOptions.vAxis={title:this.getDataValues(dataList[0])[1]};if(!isNaN(this.getVAxisMinValue())){chartOptions.hAxis.minValue=this.getVAxisMinValue();chartOptions.vAxis.minValue=this.getVAxisMinValue();}
...
menu+="</select>";this.writeHtml(ID_TOP_RIGHT,"Color by: "+menu);this.jq("colorByMenu").change(()=>{var value=this.jq("colorByMenu").val();this.setProperty("colorBy",value);this.updateUI();});}
