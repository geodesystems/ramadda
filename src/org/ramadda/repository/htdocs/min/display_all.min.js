...
return true;},getDataFilters:function(v){return DataUtils.getDataFilters(this,v||this.getProperty("dataFilters"));},filterData:function(records,fields,doGroup,skipFirst){var startDate=this.getProperty("startDate");var endDate=this.getProperty("endDate");if(startDate){this.startDateObject=new Date(startDate);}
...
grid[p.y][p.x]=0;grid[p.y][p.x]+=p.v;});for(var rowIdx=0;rowIdx<rows;rowIdx++){for(var colIdx=0;colIdx<cols;colIdx++){let count=counts[rowIdx][colIdx];if(count==0)continue;let total=grid[rowIdx][colIdx];grid[rowIdx][colIdx]=total/count;}}},gridData:function(gridId,records,args){if(!args)args={};let opts={shape:"circle",color:"blue",w:800,h:400,cellSize:2,cellSizeX:2,cellSizeY:2}
...
x=Math.floor(x/opts.cellSizeX);y=Math.floor(y/opts.cellSizeY);points.push({x:x,y:y,v:v});});RecordUtil.gridPoints(grid,points,args);let tmpc=["red","green","blue"];let tmpcnt=0;for(var rowIdx=0;rowIdx<rows;rowIdx++){let row=grid[rowIdx];for(var colIdx=0;colIdx<cols;colIdx++){let v=row[colIdx];if(isNaN(v))continue;let c;if(opts.colorBy&&opts.colorBy.index>=0){c=opts.colorBy.getColor(v);}else{if(tmpcnt>=tmpc.length)
...
let args=$.extend({colorBy:colorBy,w:w,h:h},this.getDefaultGridByArgs());let img=RecordUtil.gridData(this.getId(),records,args);this.map.addImageLayer("test","test","",img,true,bounds.north,bounds.west,bounds.south,bounds.east,w,h,{isBaseLayer:false});return;}
let cidx=0
