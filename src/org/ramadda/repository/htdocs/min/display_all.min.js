...
this[func].apply(this,[source,data]);},displayColorTable:function(ct,domId,min,max,args){Utils.displayColorTable(ct,this.getDomId(domId),min,max,args);if(!args.colorByInfo)return;this.jq(domId).find(".display-colortable-slice").css("cursor","pointer");let _this=this;if(!this.originalColorRange){this.originalColorRange=[min,max];}
...
return titleToShow;},handleEventMapBoundsChanged:function(source,args){if(this.getProperty("acceptBoundsChange")){this.filterBounds=args.bounds;this.updateUI();}},handleEventFieldValueSelected:function(source,args){this.setProperty("filterPattern",args.value);this.setProperty("patternFilterField",args.field.getId());this.updateUI();},setDateRange:function(min,max){this.minDateObj=min;if(max){console.log("setDateRange:"+max);console.trace();}
this.maxDateObj=max;},handleDateRangeChanged:function(source,prop){this.setDateRange(prop.minDate,prop.maxDate);if(this.getAnimation().getEnabled()){this.getAnimation().setDateRange(prop.minDate,prop.maxDate);}
...
let xcnt=0;for(var i=0;i<dataFilters.length;i++){if(!dataFilters[i].isRecordOk(record))return false;}
...
continue;}
var row=this.getDataValues(record);var ok=true;for(var i=0;i<this.filterFields.length&&ok;i++){var filterField=this.filterFields[i];var filter=filters[filterField.getId()];if(filter==null)continue;var rowValue=row[filterField.getIndex()];var debugFilter=false;if(debugfilter)
console.log("filter field:"+filterField.getId()+" value:"+rowValue);if(filterField.getType()=="enumeration"){ok=filter.values.includes(""+rowValue);if(debugFilter)
...
this.checkSearchBar();this.updateUI();},getFilterId:function(id){return this.getDomId("filterby_"+id);},getHeader2:function(){return"";},initHeader2:function(){},getRequestMacros:function(){let macros=[];if(!this.getProperty("macros",""))return macros;this.getProperty("macros","").split(",").every(macro=>{if(macro=="")return true;let values=null;let enums=this.getProperty("macro."+macro+".values");if(enums){values=[];values.push(["","All"]);enums.split(",").every(tok=>{[id,label]=tok.split(":");values.push([id,label||id]);return true;});}
macros.push({display:this,name:macro,values:values,urlarg:this.getProperty("macro."+macro+".urlarg","macro_"+macro),type:this.getProperty("macro."+macro+".type","string"),dflt:this.getProperty("macro."+macro+".default",""),label:this.getProperty("macro."+macro+".label",Utils.makeLabel(macro)),enums:this.getProperty("macro."+macro+".values","").split(","),getWidget:function(dateIds){let visible=this.display.getProperty("macro."+this.name+".visible",this.display.getProperty("macros.visible",true));let style=visible?"":"display:none;";let widget;let label=this.label;if(this.type=="enumeration"){if(values)
...
return url;}});return true;});return macros;},checkSearchBar:function(){let _this=this;this.filterFields=[];this.colorByFields=this.getFieldsByIds(null,this.getProperty("colorByFields","",true));this.sizeByFields=this.getFieldsByIds(null,this.getProperty("sizeByFields","",true));let pointData=this.getData();if(pointData==null)return;let fields=pointData.getRecordFields();let records=pointData.getRecords();let header2=HtmlUtils.span(["xstyle","display:inline-block;","id",this.getDomId(ID_HEADER2_PREFIX)]);header2+=this.getHeader2();let macros=this.getRequestMacros();let macroDateIds=[];macros.every(macro=>{header2+=macro.getWidget(macroDateIds)+"&nbsp;&nbsp;";return true;});if(this.getProperty("legendFields")||this.getProperty("showFieldLegend",false)){let colors=this.getColorList();let fields=this.getFieldsByIds(null,this.getProperty("legendFields",this.getProperty("fields",this.getProperty("sumFields"))));let html="";let colorCnt=0;fields.map((f)=>{if(colorCnt>=colors.length)colorCnt=0;let color=colors[colorCnt];html+=HtmlUtils.div(["style","display: inline-block;width:8px;height:8px;background:"+color+";"])+" "+f.getLabel()+"&nbsp;&nbsp; ";colorCnt++;});header2+=html;}
if(this.getProperty("showChartFieldsMenu")){let chartFields=pointData.getChartableFields();if(chartFields.length){let enums=[];chartFields.map(field=>{enums.push([field.getId(),field.getLabel()]);});header2+=HtmlUtils.span(["class","display-filterby"],HtmlUtils.span(["class","display-filter-label"],"Display: ")+HtmlUtils.select("",["style","","id",this.getDomId("chartfields")],enums,this.getProperty("fields","")))+"&nbsp;";}}
if(this.colorByFields.length>0){let enums=[];this.colorByFields.map(field=>{if(field.isFieldGeo())return;enums.push([field.getId(),field.getLabel()]);});header2+=HtmlUtils.span(["class","display-filterby"],HtmlUtils.span(["class","display-filter-label"],"Color by: ")+HtmlUtils.select("",["style","","id",this.getDomId("colorbyselect")],enums,this.getProperty("colorBy","")))+"&nbsp;";}
if(this.sizeByFields.length>0){let enums=[];this.sizeByFields.map(field=>{enums.push([field.getId(),field.getLabel()]);});header2+=HtmlUtils.span(["class","display-filterby"],HtmlUtils.span(["class","display-filter-label"],"Size by: ")+HtmlUtils.select("",["style","","id",this.getDomId("sizebyselect")],enums,this.getProperty("sizeBy","")))+"&nbsp;";}
let dataFilterIds=[];this.getDataFilters().map(f=>{if(!f.label)return;let cbxid=this.getDomId("datafilterenabled_"+f.id);dataFilterIds.push(cbxid);header2+=HtmlUtils.checkbox("",["id",cbxid],f.enabled)+" "+f.label+"&nbsp;&nbsp;"});if(this.getProperty("filterDate")){let type=this.getProperty("filterDate");let enums=[];if(this.getProperty("filterDateIncludeAll")){enums.push(["all","All"]);}
...
style+="display:none;";let selectId=this.getFilterId("filterDate");header2+=HtmlUtils.span(["class","display-filterby","style",style],HtmlUtils.span(["class","display-filter-label"],"Select "+label+":")+HtmlUtils.select("",["fieldId","filterDate","style","","id",selectId],enums,selected))+"&nbsp;";}
...
label=label+": ";widget=HtmlUtils.div(["style","display:inline-block;"],HtmlUtils.span(["class","display-filter-label","title",tt],label)+widget);}
searchBar+=widget+(hideFilterWidget?"":"&nbsp;&nbsp;");}
var style=(hideFilterWidget?"display:none;":"")+this.getProperty("filterByStyle","");let filterBar=searchBar+bottom;if(filterBar!=""){header2+=HtmlUtils.span(["class","display-filterby","style",style,"id",this.getDomId(ID_FILTERBAR)],searchBar+bottom);}}
...
"#"+this.getDomId(macro.getId()+"_to")).keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){macroChange(macro,$(this).val());}});this.jq(macro.getId()).change(function(e){macroChange(macro,$(this).val());});this.jq(macro.getId()+"_min").change(function(e){macroChange(macro,$(this).val(),"min");});this.jq(macro.getId()+"_max").change(function(e){macroChange(macro,$(this).val(),"max");});this.jq(macro.getId()+"_from").change(function(e){macroChange(macro,$(this).val(),"from");});this.jq(macro.getId()+"_to").change(function(e){macroChange(macro,$(this).val(),"to");});return true;});let inputFunc=function(input,input2,value){var id=input.attr("id");if(!input2){if(id.endsWith("_min")){input2=$("#"+id.replace(/_min$/,"_max"));}else if(id.endsWith("_max")){var tmp=input;input=$("#"+id.replace(/_max$/,"_min"));input2=tmp;}}
...
this.setContents(this.getLoadingMessage());},pointDataLoadFailed:function(data){this.clearProgress();this.inError=true;errorMessage=this.getProperty("errorMessage",null);if(errorMessage!=null){this.setContents(errorMessage);return;}
...
var success=function(data){if(debug)
console.log("\tgot data");if(GuiUtils.isJsonError(data)){if(debug)
console.log("\tfail");display.pointDataLoadFailed(data);return;}
var newData=makePointData(data,_this.derived,display);obj.pointData=pointData.initWith(newData);if(debug)
...
$(this).css("border","1px solid #aaa");_this.haveCalledUpdateUI=false;_this.clipBounds=true;_this.updateUI();});},updateUI:function(callback){this.lastUpdateTime=null;SUPER.updateUI.call(this);if(!this.getDisplayReady()){return;}
