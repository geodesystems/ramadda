...
this.addCell("init cell",props,false).run();this.cells[0].focus();});},formatObject:function(value){if(!value)return null;if(Array.isArray(value))
return HtmlUtils.div(["style"," white-space: pre;"],JSON.stringify(value));return HtmlUtils.div(["style"," white-space: pre;"],JSON.stringify(value,null,2));},initOutputRenderers:function(){let notebook=this;this.outputRenderers=[];this.addOutputRenderer({shouldRender:(value)=>{return typeof value==="object"&&value.getTime;},render:(value)=>{return notebook.formatDate(value)},});this.addOutputRenderer({shouldRender:(value)=>{var t=typeof value;return t==="string"||t==="number"||t==="boolean";},render:(value)=>{if(typeof value==="string"){if(value.split("\n").length>1){return HtmlUtils.div(["style"," white-space: pre;"],value);}}
...
return null;},addGlobal:function(name,value){if(Utils.isDefined(window[name]))window[name]=value;else this.globals[name]=value;},getBaseEntry:function(){return this.baseEntry;},getRootEntry:function(){return this.rootEntry;},getPopup:function(){return this.jq(ID_MENU);},loadJson:async function(data){if(data.error){this.setContents(_this.getMessage("Failed to load notebook: "+data.error));return;}
...
function NotebookState(cell,div){this.id=HtmlUtils.getUniqueId();this.cell=cell;this.notebook=cell.notebook;$.extend(this,{entries:{},div:div,stopFlag:false,result:null,log:function(msg,type,from){this.getNotebook().log(msg,type,from,this.div);},getStop:function(){return this.stopFlag;},getCell:function(){return this.cell;},setValue:function(name,value){this.notebook.setCellValue(name,value);},makeData:async function(entry){if(!entry)
...
if(key==6&&e.ctrlKey){this.notebook.moveCellUp(_this);return;}};this.input.keydown(moveFunc);this.header.keydown(moveFunc);this.input.keydown(function(e){var key=e.key;if(key=='Enter'){if(e.shiftKey||e.ctrlKey){if(e.preventDefault){e.preventDefault();}
if(e.shiftKey&&e.ctrlKey){_this.run(null,false,false);}else{_this.run(null,true,e.ctrlKey);if(!e.ctrlKey){_this.stepToNextChunk();}}}}});},selectClick(type,id,entryId,value){if(type=="entryid"){this.insertText(entryId);}else{this.insertText("[["+entryId+"|"+value+"]]");}
...
this.jq(ID_RUN_ICON).attr("src",icon_progress);await this.runInner(value,doRows).then(r=>ok=r);this.jq(ID_RUN_ICON).attr("src",icon_blank);if(!ok){this.running=false;return Utils.call(callback,false);}
...
if(chunk!=""){var div=makeDiv();chunks.push({content:chunk,type:type,rest:rest,div:div,doChunk:doChunk,ok:true});}
...
this.rawOutput="";for(var i=0;i<chunks.length;i++){var chunk=chunks[i];if(!chunk.doChunk)continue;if(chunk.rest.indexOf(" dontRun ")>=0)continue;chunk.div.set("");await this.processChunk(chunk);if(!chunk.ok){return false;}}
Utils.initContent("#"+this.getDomId(ID_OUTPUT));return true;},writeOutput:function(h){if(!this.output){err=new Error();console.log("no output:"+err.stack);return;}
this.output.html(h);this.outputUpdated();},outputUpdated:function(){this.outputHtml=this.jq(ID_OUTPUT).html();},getRawOutput:function(){return this.rawOutput;},focus:function(){this.input.focus();},clearOutput:function(){this.output.html("");this.outputHtml="";this.divs=[];},processHtml:async function(chunk){var content=chunk.content;if(content.match("%\n*$")){content=content.trim();content=content.substring(0,content.length-1);}
this.rawOutput+=content+"\n";chunk.div.set(content);},processCss:async function(chunk){var css=HtmlUtils.tag("style",["type","text/css"],chunk.content);this.rawOutput+=css+"\n";chunk.div.set(css);},handleError:function(chunk,error,from){chunk.ok=false;console.log("An error occurred:"+error);this.notebook.log(error,"error",from,chunk.div);},processFetch:async function(chunk){var lines=chunk.content.split("\n");for(var i=0;i<lines.length;i++){var line=lines[i].trim();if(line=="")continue;var origLine=line;var error=null;var url;var msgExtra="";var idx=line.indexOf(":");if(idx<0){this.handleError(chunk,"Bad fetch line:"+line,"io");return;}
...
chunk.div.set(HtmlUtils.div(["class","display-notebook-md"],html));},processPy:async function(chunk){if(!this.notebook.loadedPyodide){chunk.div.set("Loading Python...");await Utils.importJS(ramaddaBaseHtdocs+"/lib/pyodide/pyodide.js");await languagePluginLoader.then(()=>{pyodide.runPython('import sys\nsys.version;');},(e)=>console.log("error:"+e));await pyodide.loadPackage(['numpy','cycler','pytz','matplotlib'])
chunk.div.set("");this.notebook.loadedPyodide=true;}
var state=new NotebookState(this,chunk.div);window.iodide=window.notebook=state;pyodide.runPython(chunk.content);if(state.getStop()){chunk.ok=false;}},processWiki:async function(chunk){this.rawOutput+=chunk.content+"\n";var id=this.notebook.getProperty("entryId","");await this.getCurrentEntry(e=>entry=e);if(entry)id=entry.getId();let _this=this;let divId=HtmlUtils.getUniqueId();var wikiCallback=function(html){var h=HtmlUtils.div(["id",divId,"style"],html);chunk.div.set(h);}
...
for(name in this.notebook.globals){if(!Utils.isDefined(window[name])){topLines++;jsSet+="var "+name+"= notebook.getNotebook().globals['"+name+"'];\n";}}
var js=chunk.content.trim();lines=js.split("\n");js=jsSet+"\n"+js;var result=eval.call(null,js);if(state.getStop()){chunk.ok=false;}
var html="";if(result!=null){var rendered=this.notebook.formatOutput(result);if(rendered!=null){html=rendered;}else{var type=typeof result;if(type!="object"&&type!="function"){html=result;this.rawOutput+=html+"\n";}}}
