...
if(this.filterFields){for(var filterIdx=0;filterIdx<this.filterFields.length;filterIdx++){var f=this.filterFields[filterIdx];if(f.isNumeric){var min=$("#"+this.getDomId("filterby_"+f.getId()+"_min")).val().trim();var max=$("#"+this.getDomId("filterby_"+f.getId()+"_max")).val().trim();headerTemplate=headerTemplate.replace("${filter_"+f.getId()+"_min}",min);headerTemplate=headerTemplate.replace("${filter_"+f.getId()+"_max}",max);footerTemplate=footerTemplate.replace("${filter_"+f.getId()+"_min}",min);footerTemplate=footerTemplate.replace("${filter_"+f.getId()+"_max}",max);}else{var value=$("#"+this.getDomId("filterby_"+f.getId())).val().trim();var tmp=parseFloat(value);headerTemplate=headerTemplate.replace("${filter_"+f.getId()+"}",value);footerTemplate=footerTemplate.replace("${filter_"+f.getId()+"}",value);}}}
if(selected.length>0)
contents+=headerTemplate;if(template!=""){var max=parseFloat(this.getProperty("maxNumber",-1));console.log("max:"+max);for(var rowIdx=0;rowIdx<selected.length;rowIdx++){if(max!=-1&&rowIdx>max)break;var row=this.getDataValues(selected[rowIdx]);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);for(var col=0;col<fields.length;col++){var f=fields[col];var value=row[f.getIndex()];if(f.getType()=="image"){if(value&&value.trim().length>1){var attrs=[];if(this.getProperty("imageWidth","")!=""){attrs.push("width");attrs.push(this.getProperty("imageWidth",""));}
