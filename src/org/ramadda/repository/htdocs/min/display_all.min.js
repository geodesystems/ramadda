...
for(a in fields)
console.log("   "+fields[a].getId());},getStandardData:function(fields,args){this.recordToIndex={};this.indexToRecord={};var pointData=this.getPointData();var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);if(fields==null){fields=pointData.getRecordFields();}
...
rowCnt++;this.recordToIndex[record.getId()]=rowCnt;this.indexToRecord[rowCnt]=record;var values=[];if(props&&(props.includeIndex||props.includeIndexIfDate)){var indexName=null;if(indexField){values.push(record.getValue(indexField.getIndex())+offset);indexName=indexField.getLabel();}else{if(this.hasDate){values.push(this.getDateValue(date,date_formatter));indexName="Date";}else{if(!props.includeIndexIfDate){values.push(j);indexName="Index";}}}
...
var code="function displayChartEval(args) {\n"+setVars+"\treturn  "+this["function"]+"\n}";eval(code);var newList=[];var fieldNames=null;var rowCnt=-1;var indexField=this.getFieldById(null,this.getProperty("indexField"));for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];var row=record.getData();var date=record.getDate();if(!this.dateInRange(date))continue;rowCnt++;var values=[];var indexName=null;if(indexField){values.push(record.getValue(indexField.getIndex())+offset);indexName=indexField.getLabel();}else{if(this.hasDate){values.push(this.getDateValue(date,date_formatter));indexName="Date";}else{values.push(rowIdx);indexName="Index";}}
...
chartOptions.hAxis.textPosition=this.getProperty("hAxisTextPosition");chartOptions.vAxis.textPosition=this.getProperty("vAxisTextPosition");if(this.hAxis){chartOptions.hAxis.title=this.hAxis;}
if(this.vAxis){chartOptions.vAxis.title=this.vAxis;}
...
this.jq(ID_TIMELINE).find(".tl-text").css("padding","0px");this.jq(ID_TIMELINE).find(".tl-slide-content").css("padding","0px");this.jq(ID_TIMELINE).find(".tl-slidenav-description").css("display","none");if(this.getProperty("bannerHeight")){}
this.timelineReady=true;},handleEventRecordSelection:function(source,args){if(!args.record)return;var index=this.recordToIndex[args.record.getId()];if(!Utils.isDefined(index))return;this.timeline.goTo(index);},getDate:function(time){var timeTo=this.getProperty("timeTo","day");var dt={year:time.getUTCFullYear()};if(timeTo!="year"){dt.month=time.getUTCMonth()+1;if(timeTo!="month"){dt.day=time.getUTCDate();if(timeTo!="day"){dt.hour=time.getHours();dt.minute=time.getMinutes();if(timeTo!="hour"){dt.second=time.getSeconds();}}}}
return dt;}});}
