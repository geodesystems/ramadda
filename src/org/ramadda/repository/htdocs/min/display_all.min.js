...
if(comp.containsPoint&&comp.containsPoint(center)){matchedFeature=feature;geometry=feature.geometry;if(geometry){if(maxExtent===null){maxExtent=new OpenLayers.Bounds();}
...
this.vectorLayer.redraw();if(maxExtent){this.map.map.zoomToExtent(maxExtent,true);}},needsData:function(){return true;},animation:{running:false,inAnimation:false,begin:null,end:null,dateMin:null,dateMax:null,dateRange:0,dateFormat:this.getProperty("animationDateFormat","yyyyMMdd"),mode:this.getProperty("animationMode","cumulative"),steps:this.getProperty("animationSteps",60),windowUnit:this.getProperty("animationWindow",""),window:0,speed:parseInt(this.getProperty("animationSpeed",250)),},toggleAnimation:function(){this.animation.running=!this.animation.running;this.run.html(HtmlUtils.getIconImage(this.animation.running?"fa-stop":"fa-play"));if(this.animation.running)
