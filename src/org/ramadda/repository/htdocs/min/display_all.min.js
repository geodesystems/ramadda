...
return dflt;},});}
function RamaddaDisplay(argDisplayManager,argId,argType,argProperties){RamaddaUtil.initMembers(this,{});var SUPER;RamaddaUtil.inherit(this,SUPER=new DisplayThing(argId,argProperties));this.getSuper=function(){return SUPER;}
...
dataList=newData;var stride=parseInt(this.getProperty("stride",-1));if(stride>0){var list=[];var cnt=0;for(var i=0;i<dataList.length;i+=(stride+1)){list.push(dataList[i]);}
dataList=list;}
...
form+="</form>";tabTitles.push("Display");tabContents.push(form);},checkLayout:function(){},displayData:function(){},createUI:function(){var divid=this.getProperty(PROP_DIVID);if(divid!=null){var html=this.getHtml();$("#"+divid).html(html);}else{console.log("error: no div defined for display:"+this.getType());}},setDisplayReady:function(){this.displayReady=true;},getDisplayReady:function(){return this.displayReady;},pageHasLoaded:function(){this.setDisplayReady(true);},initDisplay:function(){this.createUI();if(this.getProperty("doAnimation",false)){this.getAnimation().makeControls();}
...
if(this.colorByFields.length>0){var enums=[];this.colorByFields.map(field=>{enums.push([field.getId(),field.getLabel()]);});header2+=HtmlUtils.span(["class","display-filterby"],HtmlUtils.span(["class","display-filterby-label"],"Color by: ")+HtmlUtils.select("",["style","","id",this.getDomId("colorbyselect")],enums,this.getProperty("colorBy","")))+"&nbsp;";}
...
function RamaddaSkewtDisplay(displayManager,id,properties){let SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_SKEWT,properties);var ID_SKEWT="skewt";var ID_DATE_LABEL="skewtdate";RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},initDisplay:function(){SUPER.initDisplay.call(this);},handleEventMapClick:function(source,args){if(!this.dataCollection){return;}
...
SUPER.updateUI.call(this);var skewtId=this.getDomId(ID_SKEWT);var html=HtmlUtils.div(["id",skewtId],"");this.setContents(html);var pointData=this.getData();if(pointData==null)return;var records=pointData.getRecords();if(!records||records.length==0){console.log("no data yet");return;}
var date=records[0].getDate();if(this.jq(ID_DATE_LABEL).length==0){this.jq(ID_TOP_LEFT).append(HtmlUtils.div(["id",this.getDomId(ID_DATE_LABEL)]));}
...
var fields=this.getData().getRecordFields();var names=[{id:"pressure",aliases:["vertCoord"]},{id:"height",aliases:["Geopotential_height_isobaric"]},{id:"temperature",aliases:["Temperature_isobaric"]},{id:"dewpoint",aliases:[]},{id:"rh",aliases:["Relative_humidity_isobaric"]},{id:"wind_direction",aliases:[]},{id:"wind_speed",aliases:[]},{id:"uwind",aliases:["u-component_of_wind_isobaric"]},{id:"vwind",aliases:["v-component_of_wind_isobaric"]},];var data={};var dataFields={};for(var i=0;i<names.length;i++){var obj=names[i];var id=obj.id;var field=this.getFieldById(fields,id);if(field==null){for(var j=0;j<obj.aliases.length;j++){field=this.getFieldById(fields,obj.aliases[j]);if(field)break;}}
...
if(data.height.length>1){if(data.height[0]>data.height[1]){for(name in data)
...
tuples.map(t=>{if(template){var h=template.replace("${field}",t.field.getLabel()).replace("${value1}",this.formatNumber(t.val1)).replace("${value2}",this.formatNumber(t.val2)).replace("${percent}",this.formatNumber(t.percent)).replace("${date1}",label1).replace("${date2}",label2).replace("${difference}",this.formatNumber(t.val2-t.val1));h=h.replace(/\${per_hour}/g,this.formatNumber(t.percent/hours));h=h.replace(/\${per_day}/g,this.formatNumber(t.percent/days));h=h.replace(/\${per_week}/g,this.formatNumber(t.percent/(days/7)));h=h.replace(/\${per_month}/g,this.formatNumber(months));h=h.replace(/\${per_year}/g,this.formatNumber(years));html+=h;}else{html+=HtmlUtils.tr([],HtmlUtils.td([],t.field.getLabel())+
