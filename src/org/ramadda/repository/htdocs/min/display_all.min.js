...
_this.hidePopup();});_this.jq(ID_CELLNAME_INPUT).keypress(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){_this.hidePopup();return;}});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");_this.processCommand(what);});},hidePopup:function(){var popup=this.getPopup();if(popup&&this.dialogShown){console.log("setting cellname");var cols=parseInt(this.jq(ID_LAYOUT_COLUMNS).val());this.cellName=this.jq(ID_CELLNAME_INPUT).val();this.jq(ID_CELLNAME).html(this.cellName);popup.hide();this.applyStyle();if(!isNaN(cols)&&this.notebook.columns!=cols){this.notebook.columns=cols;this.notebook.layoutCells();}}
...
this.running=false;return Utils.call(callback,true);},runInner:async function(){var value=this.input.val();value=value.trim();var help="More information <a target=_help href='"+ramaddaBaseUrl+"/userguide/notebook.html'>here</a>";value=value.replace(/{cellname}/g,this.cellName);value=value.replace(/{help}/g,help);value=this.notebook.convertInput(value);var type="wiki";var blobs=[];var blob="";var commands=value.split("\n");var types=["wiki","html","sh","js","raw","md","py"];var ok=true;for(var i=0;i<commands.length;i++){if(!ok)break;var command=commands[i];var _command=command.trim();if(_command.startsWith("//"))continue;var isType=false;for(var typeIdx=0;typeIdx<types.length;typeIdx++){var t=types[typeIdx];if(_command.startsWith(t+":")){if(blob!=""){blobs.push({blob:blob,type:type,div:new Div(),ok:true});}
blob=_command.substring((t+":").length);if(blob!="")blob+="\n";type=t;isType=true;break;}}
if(isType)continue;blob=blob+command+"\n";}
...
this.output.html(h);this.outputUpdated();},outputUpdated:function(){this.outputHtml=this.jq(ID_OUTPUT).html();},getRawOutput:function(){return this.rawOutput;},focus:function(){this.input.focus();},clearOutput:function(){this.output.html("");this.outputHtml="";},processHtml:async function(blob){this.rawOutput+=blob.blob+"\n";blob.div.set(blob.blob);},processMd:async function(blob){await Utils.import(ramaddaBaseUrl+"/lib/showdown.min.js");this.rawOutput+=blob.blob+"\n";var converter=new showdown.Converter();var html=converter.makeHtml(blob.blob);blob.div.set(html);},processPy:async function(blob){await Utils.import(ramaddaBaseUrl+"/lib/skulpt.min.js");await Utils.import(ramaddaBaseUrl+"/lib/skulpt-stdlib.js");var output="";var outf=function(t){if(t.trim()!=""){output+=t.trim()+"<br>"}}
