...
return source.getProperty(prop,dflt);return null;},getProperty:function(key,dflt,skipThis){var value=this.getPropertyInner(key,null,skipThis);if(!value)return dflt;return value;},getPropertyInner:function(key,dflt,skipThis){if(!skipThis&&Utils.isDefined(this[key])){return this[key];}
...
if(fromParent)return fromParent;if(this.displayParent!=null){return this.displayParent.getPropertyInner(key,skipThis);}
...
let roots=[];let idToNode={};let nodes=[];let idToRoot={};var labelField=this.getFieldById(null,this.getProperty("labelField"));var nodeFields=this.getFieldsByIds(null,this.getProperty("nodeFields"));if(nodeFields.length>0){let cnt=0;let valueToNode={};let parentId="";records.map(r=>{var label=labelField==null?id:r.getValue(labelField.getIndex());let parentId=null;let parentNode=null;nodeFields.map(nodeField=>{let id=r.getValue(nodeField.getIndex());let nodeId=parentId?parentId+"-"+id:id;let tmpNode=idToNode[nodeId];if(!tmpNode){tmpNode={id:nodeId,label:id,children:[],parent:parentNode};idToNode[nodeId]=tmpNode;if(!parentNode){roots.push(tmpNode);}
...
colors.push(color);});}}
var data=[{type:"sunburst",ids:ids,labels:labels,parents:parents,outsidetextfont:{size:20,color:"#377eb8"},leaf:{opacity:0.4},marker:{line:{width:1}},branchvalues:'total'}];console.log("l:"+ids);console.log("p:"+parents);console.log("v:"+values);if(valueField){data[0].values=values;}
