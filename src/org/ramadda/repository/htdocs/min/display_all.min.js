...
_this.displayColorTable(colors,ID_COLORTABLE,this.origMinValue,this.origMaxValue,{width:width,stringValues:this.colorByValues});}},setRange:function(minValue,maxValue){this.minValue=minValue;this.maxValue=maxValue;this.origMinValue=minValue;this.origMaxValue=maxValue;this.range=maxValue-minValue;},getColor:function(value,pointRecord){var percent=0;if(this.showPercent){var total=0;var data=pointRecord.getData();for(var j=0;j<data.length;j++){var ok=this.fields[j].isNumeric()&&!this.fields[j].isFieldGeo();if(ok&&this.pctFields!=null){ok=this.pctFields.indexOf(this.fields[j].getId())>=0||this.pctFields.indexOf("#"+(j+1))>=0;}
if(ok){total+=data[j];}}
...
let dataFilterIds=[];this.getDataFilters().map(f=>{if(!f.label||!f.field)return;let cbxid=this.getDomId("datafilterenabled_"+f.field.getId());dataFilterIds.push(cbxid);header2+=HtmlUtils.checkbox("",["id",cbxid],f.enabled)+" "+f.label+"&nbsp;&nbsp;"});if(this.getProperty("filterDate")){let type=this.getProperty("filterDate");let enums=[];if(this.getProperty("filterDateIncudeAll")){enums.push(["all","All"]);}
...
return min;},}
var RecordUtil={gridPoints:function(grid,points,args){let rows=grid.length;let cols=grid[0].length;let counts=[];for(var rowIdx=0;rowIdx<rows;rowIdx++){for(var colIdx=0;colIdx<cols;colIdx++){grid[rowIdx][colIdx]=NaN;}}
for(var rowIdx=0;rowIdx<rows;rowIdx++){let row=[];counts.push(row);for(var colIdx=0;colIdx<cols;colIdx++){row.push(0);}}
points.map((p,idx)=>{if(p.y>=rows)p.y=rows-1;if(p.x>=cols)p.x=cols-1;counts[p.y][p.x]++;if(isNaN(grid[p.y][p.x]))
grid[p.y][p.x]=0;grid[p.y][p.x]+=p.v;});let minCount=-1;let maxCount=-1;for(var rowIdx=0;rowIdx<rows;rowIdx++){for(var colIdx=0;colIdx<cols;colIdx++){let count=counts[rowIdx][colIdx];if(count==0)continue;let total=grid[rowIdx][colIdx];minCount=minCount==-1?count:Math.min(minCount,count);maxCount=maxCount==-1?count:Math.max(maxCount,count);if(args.doCount)
grid[rowIdx][colIdx]=count;else
grid[rowIdx][colIdx]=total/count;}}
grid.minCount=minCount;grid.maxCount=maxCount;},gridData:function(gridId,records,args){if(!args)args={};let opts={shape:"circle",color:"blue",w:800,h:400,cellSize:2,cellSizeX:2,cellSizeY:2,doCount:false}
$.extend(opts,args);let id=HtmlUtils.getUniqueId();let canvas='<canvas style="display:none;" id="'+id+'" width="'+opts.w+'" height="'+opts.h+'"></canvas>';$(document.body).append(canvas);var c=document.getElementById(id);var ctx=c.getContext("2d");let cnt=0;let bounds={};RecordUtil.getPoints(records,bounds);let ew=bounds.east-bounds.west;let eh=bounds.north-bounds.south;let halfW=opts.cellSize/2;let halfH=opts.cellSize/2;if(opts.doHeatmap){let cols=Math.floor(opts.w/opts.cellSizeX);let rows=Math.floor(opts.h/opts.cellSizeY);let grid=[];for(var rowIdx=0;rowIdx<rows;rowIdx++){let row=[];grid.push(row);for(var colIdx=0;colIdx<cols;colIdx++){row.push(0);}}
let points=[];records.map((record,idx)=>{let lat=record.getLatitude();let lon=record.getLongitude();let x=Math.round(opts.w*(lon-bounds.west)/ew)-halfW;let y=opts.h-(Math.round(opts.h*(lat-bounds.south)/eh)-halfH);record[gridId+"_coordinates"]={x:x,y:y};let v=idx;if(opts.colorBy&&opts.colorBy.index>=0){v=record.getValue(opts.colorBy.index);}
x=Math.floor(x/opts.cellSizeX);y=Math.floor(y/opts.cellSizeY);points.push({x:x,y:y,v:v});});RecordUtil.gridPoints(grid,points,args);let tmpc=["red","green","blue"];let tmpcnt=0;if(args.doCount){if(opts.colorBy){opts.colorBy.setRange(grid.minCount,grid.maxCount);}}
for(var rowIdx=0;rowIdx<rows;rowIdx++){let row=grid[rowIdx];for(var colIdx=0;colIdx<cols;colIdx++){let v=row[colIdx];if(isNaN(v))continue;let c;if(args.doCount){c=opts.colorBy.getColor(v);}else if(opts.colorBy&&opts.colorBy.index>=0){c=opts.colorBy.getColor(v);}else{if(tmpcnt>=tmpc.length)
tmpcnt=0;c=tmpc[tmpcnt++];}
ctx.fillStyle=c;ctx.fillRect(colIdx*opts.cellSizeX,rowIdx*opts.cellSizeY,opts.cellSizeX,opts.cellSizeY);}}
return c.toDataURL("image/png");}
records.map((record,idx)=>{let lat=record.getLatitude();let lon=record.getLongitude();let x=Math.round(opts.w*(lon-bounds.west)/ew)-halfW;let y=opts.h-(Math.round(opts.h*(lat-bounds.south)/eh)-halfH);record[gridId+"_coordinates"]={x:x,y:y};let c=opts.color||"#000";if(opts.colorBy&&opts.colorBy.index>=0){c=opts.colorBy.getColor(record.getData()[opts.colorBy.index],record);}
ctx.fillStyle=c;ctx.strokeStyle=c;if(opts.shape=="circle"){ctx.beginPath();ctx.arc(x,y,opts.cellSize,0,2*Math.PI);if(args.stroke)
ctx.stroke();else
ctx.fill();}else{if(args.stroke)
ctx.strokeRect(x,y,opts.cellSize,opts.cellSize);else
ctx.fillRect(x,y,opts.cellSize,opts.cellSize);}});return c.toDataURL("image/png");},getRanges:function(fields,records){var maxValues=[];var minValues=[];for(var i=0;i<fields.length;i++){maxValues.push(NaN);minValues.push(NaN);}
...
return values;},sort:function(fields){fields=fields.slice(0);fields.sort(function(a,b){var s1=a.getSortOrder();var s2=b.getSortOrder();return s1<s2;});return fields;},getPoints:function(records,bounds){var points=[];var north=NaN,west=NaN,south=NaN,east=NaN;if(records!=null){for(j=0;j<records.length;j++){var record=records[j];if(!isNaN(record.getLatitude())&&!isNaN(record.getLongitude())){if(record.getLatitude()==0){console.log("ll:"+record);}
if(j==0){north=record.getLatitude();south=record.getLatitude();west=record.getLongitude();east=record.getLongitude();}else{north=Math.max(north,record.getLatitude());south=Math.min(south,record.getLatitude());west=Math.min(west,record.getLongitude());east=Math.max(east,record.getLongitude());}
...
if(bounds){bounds.north=north;bounds.west=west;bounds.south=south;bounds.east=east;}
return points;},findClosest:function(records,lon,lat,indexObj){if(records==null)return null;var closestRecord=null;var minDistance=1000000000;var index=-1;for(j=0;j<records.length;j++){var record=records[j];if(isNaN(record.getLatitude())){continue;}
...
if(command!=""){result.push({command:command,args:args});}});return result;},getDataFilters:function(display,prop){let filters=[];if(!prop)return filters;DataUtils.parseCommands(prop).map(cmd=>{let[type,fieldId,value,enabled,label]=[cmd.command,cmd.args.field,cmd.args.value,cmd.args.enabled,cmd.args.label];if(!Utils.isDefined(enabled))
enabled=true;else
...
if(this.map.circles)
this.map.circles.redraw();this.applyVectorMap(true);},colorByFieldChanged:function(field){this.haveCalledUpdateUI=false;this.setProperty("colorBy",field);this.vectorMapApplied=false;this.updateUI();},sizeByFieldChanged:function(field){this.haveCalledUpdateUI=false;this.setProperty("sizeBy",field);this.vectorMapApplied=false;this.updateUI();},dataFilterChanged:function(){this.vectorMapApplied=false;this.updateUI(()=>{if(this.getProperty("centerOnFilterChange",false)){if(this.vectorLayer&&this.points){}else{this.map.centerOnMarkers(null,false,false);}}});},requiresGeoLocation:function(){return true;},getHeader2:function(){let html=SUPER.getHeader2.call(this);if(this.getProperty("showClipToBounds")){this.clipToView=false;return HtmlUtils.div(["style","display:inline-block;cursor:pointer;padding:1px;border:1px solid rgba(0,0,0,0);","title","Clip to view","id",this.getDomId("clip")],HtmlUtils.getIconImage("fa-globe-americas"))+"&nbsp;&nbsp;"+html;}
return html;},xcnt:0,initHeader2:function(){let _this=this;this.jq("clip").click(function(e){console.log("clip:"+_this.clipToView);_this.clipToView=!_this.clipToView;if(!_this.clipToView){$(this).css("border","1px solid rgba(0,0,0,0)");_this.clipToView=false;return;}
...
this.map.setProgress(HtmlUtils.div([ATTR_CLASS,"display-map-message"],msg));setTimeout(()=>{try{this.updateUIInner(pointData,records);}catch(exc){this.map.setProgress(HtmlUtils.div([ATTR_CLASS,"display-map-message"],""+exc));return;}
...
this.map.clearSeenMarkers();var t2=new Date();this.addPoints(records,fields,points,pointBounds);var t3=new Date();this.addLabels(records,fields,points);this.applyVectorMap();this.lastUpdateTime=new Date();},addPoints:function(records,fields,points,bounds){let colorBy=this.getColorByInfo(records);if(this.getProperty("doGridPoints",false)||this.getProperty("doHeatmap",false)){let w=Math.round(this.getProperty("gridWidth",800));let h=Math.round(this.getProperty("gridHeight"));if(isNaN(h)||!Utils.isDefined(h)){let ratio=(bounds.east-bounds.west)/(bounds.north-bounds.south);h=Math.round(w/ratio);}
let doCount=this.getProperty("heatmapDoCount",false);let args=$.extend({colorBy:colorBy,w:w,h:h,doCount:doCount},this.getDefaultGridByArgs());let img=RecordUtil.gridData(this.getId(),records,args);this.map.addImageLayer("heatmap","Heatmap","",img,true,bounds.north,bounds.west,bounds.south,bounds.east,w,h,{isBaseLayer:false});colorBy.displayColorTable(null,doCount);return;}
let cidx=0
let polygonField=this.getFieldById(fields,this.getProperty("polygonField"));let polygonColorTable=this.getColorTable(true,"polygonColorTable",null);let latlon=this.getProperty("latlon",true);let source=this;let radius=parseFloat(this.getDisplayProp(source,"radius",8));if(this.getProperty("scaleRadius")){let radiusScale=[10000,1,8000,2,5000,3,2000,3,1000,5,500,8,250,10,100,12,50,14];radius=radiusScale[1];for(let i=0;i<radiusScale.length;i+=2){if(points.length<+radiusScale[i]){radius=+radiusScale[i+1];}}}
