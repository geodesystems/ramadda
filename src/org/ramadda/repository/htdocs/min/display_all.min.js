...
enabled=true;else
enabled=enabled=="true";if(label){var cbx=this.jq("datafilterenabled_"+fieldId);if(cbx.length){enabled=cbx.is(':checked');}}
...
let dataFilters=this.getDataFilters();if(dataFilters){dataList=dataList.filter((r,idx)=>{if(!this.checkDataFilters(dataFilters,r))return false;return true;});}
...
return result;}};function CsvUtil(){let eg="convertData=\"derived(field=field_id, function=population*10);\nrotateData(includeFields=true,includeDate=true,flipColumns=true);\naddPercentIncrease(replaceValues=false);\"\n";$.extend(this,{process:function(display,pointData,commands){this.display=display;if(!commands)return pointData;commands.split(";").map(command=>{command=command.trim();let toks=command.match(/([^\(]+)\(([^\)]*)\)/);let rest="";if(toks){command=toks[1];rest=toks[2];}
let args={};rest.split(",").map(arg=>{arg=arg.trim();let value="";let atoks=arg.match(/(.*)=(.*)/);if(atoks){arg=atoks[1];value=atoks[2];}
arg=arg.trim();value=value.trim();if(arg!=""){args[arg]=value;}});if(this[command]){let orig=pointData;pointData=this[command](pointData,args);if(!pointData)pointData=orig;else pointData.entryId=orig.entryId;}else{console.log("unknown command:"+command);}});return pointData;},help:function(pointData,args){console.log(eg);return null;},furlData:function(pointData,args){},derived:function(pointData,args){let records=pointData.getRecords();let fields=pointData.getRecordFields().slice();let newRecords=[];let id=args["field"]||("field_"+fields.length);fields.push(new RecordField({id:id,index:fields.length,label:Utils.makeLabel(id),type:"double",chartable:true}));let func=args["function"];if(!func){console.log("No func specified in derived");return null;}
func=func.replace(/_nl_/g,"\n");if(func.indexOf("return")<0){func="return "+func;}
let setVars="";fields.map((field,idx)=>{if(field.isFieldNumeric()&&field.getId()!=""){setVars+="\tvar "+field.getId()+"=displayGetFunctionValue(args."+field.getId()+");\n";}});let code="function displayDerivedEval(args) {\n"+setVars+func+"\n}";eval(code);records.map((record,rowIdx)=>{let newRecord=record.clone();newRecord.data=record.data.slice();newRecords.push(newRecord);let funcArgs={};fields.map((field,idx)=>{if(field.isFieldNumeric()&&field.getId()!=""){funcArgs[field.getId()]=record.getValue(field.getIndex());}});let value=displayDerivedEval(funcArgs);newRecord.data.push(value);});return new PointData("pointdata",fields,newRecords,null,null);},rotateData:function(pointData,args){let records=pointData.getRecords();let header=this.display.getDataValues(records[0]);let rotated=[];for(var colIdx=0;colIdx<header.length;colIdx++){rotated.push([]);}
...
if(rowIdx==0){data.push(0);}else{let basev=firstRecord.data[f.getIndex()];let perc=basev==0?0:(value-basev)/basev;data.push(perc);}});newRecord.data=data;});return new PointData("pointdata",newFields,newRecords,null,null);},});}
