...
HtmlUtils.div(["class","display-notebook-console-output","id",this.getDomId(ID_CONSOLE_OUTPUT)],contents||""));},makeCellLayout:function(){var html="";var con=this.makeConsole();if(this.showInput()&&this.layout=="horizontal"){var left=HtmlUtils.div(["id",this.getDomId(ID_INPUTS)]);var right=HtmlUtils.div(["id",this.getDomId(ID_OUTPUTS)]);var center=HtmlUtils.div([],"");left+=con;html="<table width=100%><tr valign=top><td width=50%>"+left+"</td><td style='border-left:1px #ccc solid;' width=1>"+center+"</td><td width=49%>"+right+"</td></tr></table>";}else{this.jq(ID_CELLS_BOTTOM).html(con);}
...
if(error){this.handleError(chunk,error,"io");return;}else{this.notebook.log("Loaded: "+url+msgExtra,"output","io");}}},processMd:async function(chunk){this.rawOutput+=chunk.content+"\n";var o="";var tex=null;var lines=chunk.content.split("\n");var texs=[];for(var i=0;i<lines.length;i++){var line=lines[i];var _line=line.trim();if(_line.startsWith("$$")){if(tex!=null){try{var html=katex.renderToString(tex,{throwOnError:true});o+="tex:"+texs.length+":\n";texs.push(html);}catch(e){o+="Error parsing tex:"+e+"<pre>"+tex+"</pre>";}
tex=null;}else{tex="";}}else if(tex!=null){tex+=line+"\n";}else{o+=line+"\n";}}
var converter=new showdown.Converter();var html=converter.makeHtml(o);for(var i=0;i<texs.length;i++){html=html.replace("tex:"+i+":","<center>"+texs[i]+"</center>");}
chunk.div.set(html);},processPy:async function(chunk){await Utils.importJS(ramaddaBaseUrl+"/lib/skulpt.min.js");await Utils.importJS(ramaddaBaseUrl+"/lib/skulpt-stdlib.js");var output="";var outf=function(t){if(t.trim()!=""){output+=t.trim()+"<br>"}}
...
var html="";if(Utils.stringDefined(state.prefix))html+=state.prefix;if(state.result){var type=typeof state.result;if(type!="object"&&type!="function"){html+=state.result;this.rawOutput+=html+"\n";}else{}}
