...
for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);var img=row[this.imageField.getIndex()];var tooltip="";var label="";if(this.labelField)label="<br>"+row[this.labelField.getIndex()];this.tooltipFields.map(field=>{if(tooltip!="")tooltip+="&#10;";tooltip+=field.getValue(row);});var attrs=["class","display-images-popup","data-fancybox",this.getDomId("gallery")];if(this.captionFields.length>0){var caption="";if(this.captionTemplate)caption=this.captionTemplate;if(this.urlField)
caption+="<a href='"+this.urlField.getValue(row)+"' target=_other>";this.captionFields.map(field=>{if(this.captionTemplate)
caption=caption.replace("\${"+field.getId()+"}",field.getValue(row));else
caption+=field.getValue(row)+"<br>";});if(this.urlField)
caption+="</a>";attrs.push("data-caption");attrs.push(caption);}
var img=HtmlUtils.href(img,HtmlUtils.image(img,["width",width]),attrs);var html=HtmlUtils.div(["class","display-images-item","title",tooltip,"style","margin:"+margin+"px;"],img+label);if(this.groupField){var groupOn=row[this.groupField.getIndex()];if(!groups[groupOn]){groupCnt[groupOn]=0;groups[groupOn]="";groupHeaders.push(groupOn);}
