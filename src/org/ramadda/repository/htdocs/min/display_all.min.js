...
this.label.html(this.formatAnimationDate(this.dateMin)+" - "+this.formatAnimationDate(this.dateMax));if(!this.dateMin)return;this.jq(ID_SLIDER).slider({range:true,min:this.dateMin.getTime(),max:this.dateMax.getTime(),values:[this.dateMin.getTime(),this.dateMax.getTime()],slide:function(event,ui){_this.stopAnimation();_this.begin=new Date(ui.values[0]);_this.end=new Date(ui.values[1]);_this.updateLabels();},stop:function(event,ui){_this.stopAnimation();_this.begin=new Date(ui.values[0]);_this.end=new Date(ui.values[1]);_this.applyAnimation(true);}});if(dates&&display.getProperty("animationShowTicks",true)){var ticks="";var min=this.dateMin.getTime();var max=this.dateMax.getTime();var p=0;for(var i=0;i<dates.length;i++){var date=dates[i].getTime();var perc=Math.round((date-min)/(max-min)*100);ticks+=HtmlUtils.div(["class","display-animation-tick","style","left:"+perc+"%;","title","x"],"");}
this.jq(ID_SLIDER).append(ticks);}},makeControls:function(){var buttons=HtmlUtils.span(["id",this.getDomId(ID_BEGIN),"title","Go to beginning"],HtmlUtils.getIconImage("fa-fast-backward"))+
...
var windowStart=this.begin.getTime();var windowEnd=this.end.getTime();var atLoc={};for(var i=0;i<this.display.lines.length;i++){var line=this.display.lines[i];if(line.date<windowStart||line.date>windowEnd){line.style.display='none';continue;}
...
this.display.map.lines.redraw();this.updateLabels();if(windowEnd<=this.dateMax.getTime()){if(this.running){setTimeout(()=>{if(!this.running)return;this.doNext()},this.speed);}}else{this.running=false;this.inAnimation=false;this.btnRun.html(HtmlUtils.getIconImage("fa-play"));}
this.display.applyVectorMap(true);},updateLabels:function(){this.label.html(this.formatAnimationDate(this.begin)+" - "+this.formatAnimationDate(this.end));},formatAnimationDate:function(d){if(this.dateFormat=="yyyy"){return Utils.formatDateYYYY(d);}else if(this.dateFormat=="yyyyMMdd"){return Utils.formatDateYYYYMMDD(d);}else{return Utils.formatDate(d);}},});}
...
return matchedFeature;},needsData:function(){return true;},animation:new DisplayAnimation(this),showAllPoints:function(){if(this.lines){for(var i=0;i<this.lines.length;i++){var line=this.lines[i];line.style.display='inline';}
if(this.map.lines)
