...
this.charts=[];this.chartCount=-1;if(this.getProperty("multipleCharts",false)){var headerPosition=this.getProperty("multipleChartsHeaderPosition","bottom");for(var chartCount=0;chartCount<dataList.length-1;chartCount++){this.chartCount=chartCount;let tmpDataList=[];tmpDataList.push(dataList[0]);tmpDataList.push(dataList[chartCount+1]);var record=dataList[chartCount+1].record;var innerId=this.getDomId(ID_CHART)+"_"+chartCount;var header=HtmlUtils.div(["class","display-multi-header"],this.formatDate(new Date(record.getDate())));var top=headerPosition=="top"?header:"";var bottom=headerPosition=="bottom"?header+"<br>":"";var div=HtmlUtils.div(["class","display-multi-div","style","display:inline-block;"],top+this.getChartDiv(innerId)+bottom);this.jq(ID_CHARTS).append(div);let chart=this.makeGoogleChartInner(tmpDataList,innerId,props,selectedFields);if(chart)this.charts.push(chart);}}else{this.jq(ID_CHARTS).append(this.getChartDiv(this.getDomId(ID_CHART)));let chart=this.makeGoogleChartInner(dataList,this.getDomId(ID_CHART),props,selectedFields);if(chart)this.charts.push(chart);}},makeGoogleChartInner:function(dataList,chartId,props,selectedFields){let chartDiv=document.getElementById(chartId);let chart=this.doMakeGoogleChart(dataList,props,chartDiv,selectedFields,this.chartOptions);if(chart==null)return null;var dataTable=this.makeDataTable(dataList,props,selectedFields);if(!dataTable){this.setContents(this.getMessage("No data available"));return null;}
...
style+="height:"+height+";";}else{style+="height:"+"100%;";}
style+="padding:5px;"
divAttrs.push(style);return HtmlUtils.div(divAttrs,"");},doMakeGoogleChart:function(dataList,props,chartDiv,selectedFields,chartOptions){chartOptions.tooltip={textStyle:{color:'#000000'},showColorCode:true};this.chartOptions.legend={'position':this.getProperty("legendPosition",'right'),'alignment':'center'};if(this.getProperty("bins",null)){chartOptions.title="Bins: "+this.getDataValues(dataList[0])[1];}else if(this.getProperty("sumFields")){chartOptions.title=this.getProperty("chartTitle","Categories/Values");}else{chartOptions.title=this.getDataValues(dataList[0])[0]+" - "+this.getDataValues(dataList[0])[1];}
...
if(!isNaN(this.getVAxisMaxValue())){chartOptions.vAxis.maxValue=this.getVAxisMaxValue();}}
return chartOptions;},doMakeGoogleChart:function(dataList,props,chartDiv,selectedFields,chartOptions){var height=this.getProperty("height",400);if(Utils.isDefined(this.getProperty("chartHeight"))){height=this.getProperty("chartHeight");}
var width="100%";if(Utils.isDefined(this.getProperty("chartWidth"))){width=this.getProperty("chartWidth");}
if((typeof height)=="number")height=height+"px";if((typeof width)=="number")width=width+"px";$("#"+chartId).css("width",width);$("#"+chartId).css("height",height);return new google.visualization.ScatterChart(chartDiv);},getDefaultSelectedFields:function(fields,dfltList){var f=[];for(i=0;i<fields.length;i++){var field=fields[i];if(field.isNumeric()){f.push(field);if(f.length>=2)
break;}}
