...
var displayDebug=false;var ID_BOTTOM="bottom";var ID_COLORTABLE="colortable";var ID_FIELDS="fields";var ID_HEADER="header";var ID_HEADER1="header1";var ID_HEADER2="header2";var ID_HEADER3="header3";var ID_FILTERBAR="filterbar";var ID_TITLE=ATTR_TITLE;var ID_TITLE_EDIT="title_edit";var ID_TOP_RIGHT="topright";var ID_TOP_LEFT="topleft";var ID_DETAILS="details";var ID_DISPLAY_CONTENTS="contents";var ID_DISPLAY_TOP="top";var ID_DISPLAY_BOTTOM="bottom";var ID_GROUP_CONTENTS="group_contents";var ID_DETAILS_MAIN="detailsmain";var ID_GROUPBY_FIELDS="groupdbyfields";var ID_TOOLBAR="toolbar";var ID_TOOLBAR_INNER="toolbarinner";var ID_LIST="list";var ID_DIALOG="dialog";var ID_DIALOG_TABS="dialog_tabs";var ID_FOOTER="footer";var ID_FOOTER_LEFT="footer_left";var ID_FOOTER_RIGHT="footer_right";var ID_MENU_BUTTON="menu_button";var ID_MENU_OUTER="menu_outer";var ID_MENU_INNER="menu_inner";var ID_REPOSITORY="repository";var PROP_DISPLAY_FILTER="displayFilter";var PROP_EXCLUDE_ZERO="excludeZero";var PROP_DIVID="divid";var PROP_FIELDS="fields";var PROP_LAYOUT_HERE="layoutHere";var PROP_HEIGHT="height";var PROP_WIDTH="width";var FILTER_ALL="-all-";var HIGHLIGHT_COLOR="yellow";function initRamaddaDisplays(){ramaddaCheckForResize();if(window.globalDisplaysList==null){return;}
...
return source.getProperty(prop,dflt);return null;},getProperty:function(key,dflt,skipThis){if(!skipThis&&Utils.isDefined(this[key])){return this[key];}
...
var index=parseInt(percent*this.colors.length);if(index>=this.colors.length)index=this.colors.length-1;else if(index<0)index=0;if(this.stringMap){var color=this.stringMap[value];if(!Utils.isDefined(color)){return this.stringMap["default"];}
...
colorBy.range=colorBy.maxValue-colorBy.minValue;return colorBy;},getColorByMap:function(){var colorByMapProp=this.getProperty("colorByMap");if(colorByMapProp){var toks=colorByMapProp.split(",");var stringMap={};for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){stringMap[toks2[0].trim()]=toks2[1].trim();}}
return stringMap;}
return null;},toString:function(){return"RamaddaDisplay:"+this.type+" - "+this.getId();},getType:function(){return this.type;},getClass:function(suffix){if(suffix==null){return this.getBaseClass();}
...
console.log("\t\tfield:"+sfield);for(var j=0;j<fields.length;j++){var field=fields[j];var id=field.getId();if(this.debugSelected)
console.log("\t\t\tid:"+id);if(id==sfield||("#"+(j+1))==sfield){if(this.debugSelected)
...
if(!value)value=FILTER_ALL;if(!Array.isArray(value))value=value.split(",");var tmp=[];value.map(v=>tmp.push(v.trim()));value=tmp;return value;},filterData:function(dataList,fields,doGroup,skipFirst){var pointData=this.getData();if(!dataList){if(pointData==null)return null;dataList=pointData.getRecords();}
...
for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];var prefix=this.getProperty(filterField.getId()+".filterPrefix");var suffix=this.getProperty(filterField.getId()+".filterSuffix");if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;var value=null;var values=null;var _values=[];var regexps=[];var values=null;if(filterField.isNumeric()){var minField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_min"));var maxField=$("#"+this.getDomId("filterby_"+filterField.getId()+"_max"));var minValue=parseFloat(minField.val().trim());var maxValue=parseFloat(maxField.val().trim());var dfltMinValue=parseFloat(minField.attr("data-min"));var dfltMaxValue=parseFloat(maxField.attr("data-max"));if(minValue!=dfltMinValue||maxValue!=dfltMaxValue){value=[minValue,maxValue];}}else if(filterField.getType()=="date"){var date1=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date1")).val();var date2=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date2")).val();if(date1!=null&&date1.trim()!="")
...
value=[date1,date2];}else{values=this.getFilterFieldValues(filterField);if(!values&&!Array.isArray(values))values=[values];if(values.length==0)continue;var tmp=[];values.map(v=>{tmp.push(v.replace(/_comma_/g,","));});values=tmp;values.map(v=>{_values.push((""+v).toLowerCase());try{v=v.replace(/\./g,"\\.");regexps.push(new RegExp(v,"i"));}catch(skipIt){}});}
...
if(anyValues){filters[filterField.getId()]={value:value,values:values,regexps:regexps,_values:_values,anyValues:anyValues,startsWith:filterStartsWith,};}}
...
if(!ok){for(ri=0;ri<filter.regexps.length;ri++){if(rowValue.toString().match(filter.regexps[ri])){ok=true;break;}}}}}
...
dataList=list;}
if(this.getProperty("binDate")){var what=this.getProperty("binDate");var binType=this.getProperty("binType","total");var binCount=binType=="count";var binned=[];var record=dataList[0];var map={};for(var i=0;i<dataList.length;i++){var record=dataList[i];var tuple=this.getDataValues(record);var key;if(what=="month"){key=record.getDate().getUTCFullYear()+"-"+(record.getDate().getUTCMonth()+1);}else if(what=="day"){key=record.getDate().getUTCFullYear()+"-"+(record.getDate().getUTCMonth()+1)+"-"+record.getDate().getUTCDate();}else{key=record.getDate().getUTCFullYear()+"";}
if(!Utils.isDefined(map[key])){var date=Utils.parseDate(key);var data=Utils.cloneList(record.getData());if(binCount){for(k=0;k<data.length;k++)data[k]=1;}
var newRecord=new PointRecord(record.getLatitude(),record.getLongitude(),record.getElevation(),date,data);map[key]=data;binned.push(newRecord);}else{var tuple1=map[key];if(binCount){for(k=0;k<tuple1.length;k++)tuple1[k]++;continue;}
...
var includeAll=this.getProperty(filterField.getId()+".includeAll",true);if(enums==null){var allName=this.getProperty(filterField.getId()+".allName","All");enums=[];var enumValues=[];var imageField=this.getFieldOfType(null,"image");var seen={};var valuesAreNumbers=true;records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(!seen[value]){seen[value]=true;var obj={};if(imageField)
...
return(""+a[1]).localeCompare(""+b[1]);});if(includeAll){enums.push({value:[FILTER_ALL,allName]});}
for(var j=0;j<enumValues.length;j++){var v=enumValues[j];enums.push(v);}}
...
for(var j=0;j<enums.length;j++){var extra="";var v=enums[j].value;var label;if(Array.isArray(v)){label=v[1];v=v[0];}else{label=v;}
if(v==dfltValue)extra=" display-filterby-button-selected ";if(v==FILTER_ALL)extra=" display-filterby-button-all ";if(useImage){var image=null;if(imageMap)image=imageMap[v];if(!image||image=="")image=enums[j].image;if(image){buttons+=HtmlUtils.div(["fieldId",filterField.getId(),"class","display-filterby-button display-filterby-button-image"+extra,"data-value",v,"title",label],HtmlUtils.image(image,imageAttrs));}else{buttons+=HtmlUtils.div(["fieldId",filterField.getId(),"class","display-filterby-button display-filterby-button-image"+extra,"data-value",v,"title",label],label);}}else{buttons+=HtmlUtils.div(["fieldId",filterField.getId(),"class","display-filterby-button  display-filterby-button-noimage"+extra,"data-value",v],label);}
buttons+="\n";}
if(useImage&&this.getProperty(filterField.getId()+".filterShowButtonsLabel")){buttons+=HtmlUtils.div(["class","display-filterby-button-label","id",this.getDomId("filterby_"+filterField.getId()+"_label")],"&nbsp;");}
bottom+=HtmlUtils.div(["data-value",dfltValue,"class","display-filterby-buttons","id",widgetId,"isButton","true","fieldId",filterField.getId()],buttons);continue;}else if(this.getProperty(filterField.getId()+".filterCheckbox")){attrs.push("isCheckbox");attrs.push(true);var tmp=[];enums.map(e=>tmp.push(e.value));var checked=tmp.includes(dfltValue);if(tmp.length>0){attrs.push("onValue");attrs.push(tmp[0]);}
...
_this.propagateEvent("handleEventPropertyChanged",args);_this.settingFilterValue=false;};this.jq(ID_FILTERBAR).find(".display-filterby-buttons").each(function(){let parent=$(this);$(this).find(".display-filterby-button").click(function(event){var isAll=$(this).hasClass("display-filterby-button-all");var wasSelected=$(this).hasClass("display-filterby-button-selected");var fieldId=$(this).attr("fieldId");var multiples=_this.getProperty(fieldId+".filterMultiple",false);if(!event.metaKey||isAll||!multiples){parent.find(".display-filterby-button").removeClass("display-filterby-button-selected");}else{parent.find(".display-filterby-button-all").removeClass("display-filterby-button-selected");}
if(wasSelected&&event.metaKey){$(this).removeClass("display-filterby-button-selected");}else{$(this).addClass("display-filterby-button-selected");}
var values=[];parent.find(".display-filterby-button-selected").each(function(){values.push($(this).attr("data-value").replace(/,/g,"_comma_"));});if(values.length==0){parent.find(".display-filterby-button-all").addClass("display-filterby-button-selected");values.push(FILTER_ALL);}
...
this.updateUI();if(!reload){this.lastPointData=pointData;this.propagateEvent("handleEventPointDataLoaded",pointData);}},getDateFormatter:function(){var date_formatter=null;if(this.googleLoaded()){var df=this.getProperty("dateFormat",null);if(df){var tz=0;this.timezone=this.getProperty("timezone");if(Utils.isDefined(this.timezone)){tz=parseFloat(this.timezone);}
...
var nonNullRecords=0;var records=this.filterData();var allFields=pointData.getRecordFields();this.hasDate=this.getHasDate(records);var date_formatter=this.getDateFormatter();var rowCnt=-1;var indexField=this.getFieldById(null,this.getProperty("indexField"));for(j=0;j<records.length;j++){var record=records[j];var date=record.getDate();if(!this.dateInRange(date)){continue;}
rowCnt++;this.recordToIndex[record.getId()]=rowCnt;this.indexToRecord[rowCnt]=record;var values=[];if(props&&(props.includeIndex||props.includeIndexIfDate)){var indexName=null;if(indexField){var value=this.makeIndexValue(indexField,record.getValue(indexField.getIndex()),j);values.push(value);indexName=indexField.getLabel();}else{if(this.hasDate){values.push(this.getDateValue(date,date_formatter));indexName="Date";}else{if(!props.includeIndexIfDate){values.push(j);indexName="Index";}}}
if(indexName!=null&&rowCnt==0){fieldNames.unshift(indexName);}}
var allNull=true;var allZero=true;var hasNumber=false;for(var i=0;i<fields.length;i++){var field=fields[i];if(field.isFieldNumeric()&&field.isFieldDate()){}
...
if(nonNullRecords==0){return[];}
if(groupByIndex>=0){var groupToTuple={};var groups=[];var agg=[];var title=[];title.push(props.groupByField.getLabel());for(var j=0;j<fields.length;j++){var field=fields[j];if(field.getIndex()!=groupByIndex){title.push(field.getLabel());}}
agg.push(title);for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var data=this.getDataValues(dataList[rowIdx]);if(rowIdx==0){continue;}
var groupBy=groupByList[rowIdx];var tuple=groupToTuple[groupBy];if(tuple==null){groups.push(groupBy);tuple=new Array();agg.push(tuple);tuple.push(groupBy);for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];if(field.getIndex()==groupByIndex){continue;}
tuple.push(0);}
groupToTuple[groupBy]=tuple;}
var index=0;for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];if(field.getIndex()==groupByIndex){continue;}
...
var eventSource=display.getEventSource();if(eventSource!=null&&eventSource.length>0){if(eventSource!=source.getId()&&eventSource!=source.getName()){continue;}}
...
function RamaddaGoogleChart(displayManager,id,chartType,properties){var ID_TRENDS_CBX="trends_cbx";var ID_PERCENT_CBX="percent_cbx";var ID_COLORS="colors";var _this=this;$.extend(this,{indexField:-1,colorList:['blue','red','green','orange','fuchsia','teal','navy','silver'],curveType:'none',fontSize:0,showPercent:false,percentFields:null,});if(properties.colors){this.colorList=(""+properties.colors).split(",");}
let SUPER=new RamaddaFieldsDisplay(displayManager,id,chartType,properties);RamaddaUtil.inherit(this,SUPER);RamaddaUtil.defineMembers(this,{clearCachedData:function(){SUPER.clearCachedData();this.computedData=null;},updateUI:function(){SUPER.updateUI.call(this);if(!this.getDisplayReady()){return;}
this.displayData();},getWikiAttributes:function(attrs){this.defineWikiAttributes(["vAxisMinValue","vAxisMaxValue"]);SUPER.getWikiAttributes.call(this,attrs);if(this.colorList.join(",")!="blue,red,green"){attrs.push("colors");attrs.push(this.colorList.join(", "));}},initDialog:function(){SUPER.initDialog.call(this);var _this=this;var updateFunc=function(e){if(e&&e.which!=13&&e.which!=0){return;}
...
if(!args.record)return;var index=this.recordToIndex[args.record.getId()];if(Utils.isDefined(index)){this.setChartSelection(index);}},getFieldsToSelect:function(pointData){return pointData.getRecordFields();},canDoGroupBy:function(){return false;},clearCache:function(){this.computedData=null;},googleChartCallbackPending:false,includeIndexInData:function(){return false;},getGroupBy:function(){return this.groupBy;},getIncludeIndexIfDate:function(){return false;},makeIndexValue:function(indexField,value,offset){if(indexField.isString()){value={v:offset,f:value};}
return value;},displayData:function(){var _this=this;if(!this.getDisplayReady()){return;}
...
var code="function displayChartEval(args) {\n"+setVars+"\treturn  "+this["function"]+"\n}";eval(code);var newList=[];var fieldNames=null;var rowCnt=-1;var indexField=this.getFieldById(null,this.getProperty("indexField"));for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];var row=record.getData();var date=record.getDate();if(!this.dateInRange(date))continue;rowCnt++;var values=[];var indexName=null;console.log("row:"+rowIdx);if(indexField){var value=record.getValue(indexField.getIndex());console.log("v:"+value);if(indexField.isString()){value={v:offset,f:value};}
...
this.computedData=dataList;if(this.rotateTable&&dataList.length){var header=dataList[0];var flipped=[];for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var row=dataList[rowIdx];}}
...
var d=_this.jq(ID_CHART);this.lastWidth=d.width();},printDataList:function(dataList){console.log("data list:"+dataList.length);for(var i=0;i<dataList.length;i++){var row=dataList[i];var s="";for(var j=0;j<row.length;j++){if(j>0)s+=", ";s+=row[j];}
...
var annotationsMap={};if(annotations){dataTable.addColumn({type:'string',role:'annotation','p':{'html':true}});dataTable.addColumn({type:'string',role:'annotationText','p':{'html':true}});var toks=annotations.split(",");for(var i=0;i<toks.length;i++){var toks2=toks[i].split(";");if(toks2.length!=3)continue;var index=toks2[0].trim();var label=toks2[1];var desc=toks2[2];if(index.match(/^[0-9]+$/)){index=parseFloat(index);}else{index=Utils.parseDate(index,false);}
annotationsMap[index]={label:label,description:desc}}}
var annotationCnt=0;var records=[];for(var i=1;i<dataList.length;i++){records.push(dataList[i].record);}
var colors=this.getColorTable(true);var colorBy=this.getColorByInfo(records);var didColorBy=false;for(var i=1;i<dataList.length;i++){var record=dataList[i];var theRecord=dataList[i].record;var row=this.getDataValues(record);var color="";if(colorBy.index>=0){var value=theRecord.getData()[colorBy.index];hasColorByValue=true;colorByValue=value;didColorBy=true;color=colorBy.getColor(value,theRecord);}
row=row.slice(0);var label="";if(dataList[i].record){for(var j=0;j<tooltipFields.length;j++){label+="<b>"+tooltipFields[j].getLabel()+"</b>: "+
...
tooltip="<div style='padding:8px;'>"+tooltip+"</div>";newRow=[];for(var j=0;j<row.length;j++){var value=row[j];if(j>0&&fixedValueS){newRow.push(fixedValueN);}else{newRow.push(value);}
if(j==0&&props.includeIndex){}else{newRow.push(color);newRow.push(tooltip);}
if(j>0&&fixedValueS){break;}}
if(annotationFields.length>0){if(dataList[i].record){var desc="";annotationFields.map(f=>{var d=""+dataList[i].record.getValue(f.getIndex());if(d!="")
desc+=(d+"<br>");});desc=desc.trim();desc=desc.replace(/ /g,"&nbsp;");annotationCnt++;var label=null;if(desc.trim().length>0){label=""+(annotationLabelField?dataList[i].record.getValue(annotationLabelField.getIndex()):(annotationCnt))
...
if(this.getProperty("barWidth")){chartOptions.bar={groupWidth:this.getProperty("barWidth")}}
...
if(ok)
tmp.push(tuple);}
return google.visualization.arrayToDataTable(tmp);},doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){var ct=this.getColorTable(true);if(ct){chartOptions.colors=ct;}else if(!this.colors){chartOptions.colors=this.colorList;}
...
chartOptions.vAxis.logScale=true;if(dataList.length>0&&this.getDataValues(dataList[0]).length>1){$.extend(chartOptions.hAxis,{title:this.getDataValues(dataList[0])[0]});if(!chartOptions.vAxis)chartOptions.vAxis={};$.extend(chartOptions.vAxis,{title:this.getDataValues(dataList[0])[1]});if(!isNaN(this.getVAxisMinValue())){chartOptions.hAxis.minValue=this.getVAxisMinValue();chartOptions.vAxis.minValue=this.getVAxisMinValue();}
if(!isNaN(this.getVAxisMaxValue())){chartOptions.hAxis.maxValue=this.getVAxisMaxValue();chartOptions.vAxis.maxValue=this.getVAxisMaxValue();}}
return chartOptions;},doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){var height=400;if(Utils.isDefined(this.chartHeight)){height=this.chartHeight;}
var width="100%";if(Utils.isDefined(this.chartWidth)){width=this.chartWidth;}
var chartId=this.getChartId();$("#"+chartId).css("width",width);$("#"+chartId).css("height",height);return new google.visualization.ScatterChart(document.getElementById(this.getChartId()));},getDefaultSelectedFields:function(fields,dfltList){var f=[];for(i=0;i<fields.length;i++){var field=fields[i];if(field.isNumeric()){f.push(field);if(f.length>=2)
break;}}
...
function RamaddaTextrawDisplay(displayManager,id,properties){var ID_TEXT="text";var ID_LABEL="label";var ID_SEARCH="search";let SUPER=new RamaddaBaseTextDisplay(displayManager,id,DISPLAY_TEXTRAW,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);$.extend(this,{getWikiEditorTags:function(){return Utils.mergeLists(SUPER.getWikiEditorTags(),["label:Raw Text Attributes",'doBubble=true','addLineNumbers=false','labelTemplate="${lineNumber}"','maxLines=1000','pattern="initial search pattern"','fromField=""','linesDescriptor=""']);},checkLayout:function(){this.updateUI();},updateUI:function(){let records=this.filterData();if(!records){return null;}
var pointData=this.getData();this.allRecords=pointData.getRecords();var pattern=this.getProperty("pattern");if(pattern&&pattern.length==0)pattern=null;var input="";if(!this.filterFields||this.filterFields.length==0)
input=" "+HtmlUtils.input("pattern",(pattern?pattern:""),["placeholder","Search text","id",this.getDomId(ID_SEARCH)]);this.writeHtml(ID_TOP_RIGHT,HtmlUtils.span(["id",this.getDomId(ID_LABEL)]," ")+input);let _this=this;this.jq(ID_SEARCH).keypress(function(event){if(event.which==13){_this.setProperty("pattern",$(this).val());_this.propagateEvent("handleEventPropertyChanged",{property:"pattern",value:$(this).val()});_this.updateUI();}});var height=this.getProperty("height","600");var html=HtmlUtils.div(["id",this.getDomId(ID_TEXT),"style","padding:4px;border:1px #ccc solid;border-top:1px #ccc solid; max-height:"+height+"px;overflow-y:auto;"]);this.writeHtml(ID_DISPLAY_CONTENTS,html);this.showText();},handleEventPropertyChanged:function(source,prop){if(prop.property=="pattern"){this.setProperty("pattern",prop.value);this.updateUI();return;}
...
fields=allFields;var strings=this.getFieldsOfType(fields,"string");if(strings.length==0){this.displayError("No string fields specified");return null;}
var corpus="";var fromField=this.getFieldById(null,this.getProperty("fromField"));var bubble=this.getProperty("doBubble",false);if(labelTemplate){corpus="<table width=100%>";}
var lineCnt=0;var displayedLineCnt=0;var regexp;if(pattern){pattern=pattern.replace(/\./g,"\\.");regexp=new RegExp("("+pattern+")","ig");}
var regexpMaps={};var filterFieldMap={};if(this.filterFields){this.filterFields.map(f=>{if(f.isString)filterFieldMap[f.getId()]=true;});}
var templates={};fields.map(f=>{templates[f.getId()]=this.getProperty(f.getId()+".template");});var colorBy=this.getColorByInfo(records);var delimiter=this.getProperty("delimiter","");for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];this.indexToRecord[rowIdx]=record;this.recordToIndex[record.getId()]=rowIdx;var row=this.getDataValues(record);var line="";for(var col=0;col<fields.length;col++){var f=fields[col];if(rowIdx==0){if(filterFieldMap[f.getId()]){var value=this.getFilterFieldValues(f);if(value){if(!Array.isArray(value)){value=[value];}
try{regexpMaps[f.getId()]=[];value.map(v=>{if(v==""||v==FILTER_ALL)return;v=v.replace(/\./g,"\\.");var re=new RegExp("("+v+")","ig");regexpMaps[f.getId()].push(re);})}catch(e){console.log("Error making regexp:"+e);}}}}
var value=""+row[f.getIndex()];value=value.replace(/</g,"&lt;").replace(/>/g,"&gt;");if(regexpMaps[f.getId()]){regexpMaps[f.getId()].map(re=>{value=value.replace(re,"<span style=background:yellow;>$1</span>");});}
if(line!="")
line+=delimiter+" ";if(templates[f.getId()]){value=templates[f.getId()].replace("${value}",value);}
line+=value;}
line=line.trim();if(!includeEmptyLines&&line.length==0)continue;lineCnt++;if(regexp){if(!line.toLowerCase().match(regexp))continue;line=line.replace(regexp,"<span style=background:yellow;>$1</span>");}
displayedLineCnt++;if(displayedLineCnt>maxLines)break;var lineAttrs=["title"," ","class"," display-raw-line ","recordIndex",rowIdx]
var rowAttrs=["valign","top"];if(colorBy.index>=0){var value=record.getData()[colorBy.index];var color=colorBy.getColor(value,record);if(color){rowAttrs.push("style");rowAttrs.push("background:"+Utils.addAlphaToColor(color,"0.25")+";");}}
if(bubble)line=HtmlUtils.div(["class","ramadda-bubble"],line);if(fromField)line+=HtmlUtils.div(["class","ramadda-bubble-from"],""+row[fromField.getIndex()]);line=HtmlUtils.div(lineAttrs,line);if(labelTemplate){var label=this.getRecordHtml(record,null,labelTemplate);label=label.replace("${lineNumber}","#"+(lineCnt));label=label.replace(/ /g,"&nbsp;");corpus+=HtmlUtils.tr(rowAttrs,HtmlUtils.td(["width","10px"],"<a name=line_"+lineCnt+"></a>"+
"<a href=#line_"+lineCnt+">"+label+"</a>&nbsp;  ")+
HtmlUtils.td([],line));}else{corpus+=line;if(asHtml){if(breakLines)
corpus+="<p>";else
...
this.jq(ID_LABEL).html(label);this.jq(ID_SEARCH).focus();var lines=this.jq(ID_TEXT).find(".display-raw-line");lines.click(function(){var idx=$(this).attr("recordIndex");var record=_this.indexToRecord[idx];if(record){_this.showRecordPopup($(this),record);_this.getDisplayManager().notifyEvent("handleEventRecordSelection",_this,{record:record});}});this.makeTooltips(lines,records);},handleEventRecordSelection:function(source,args){if(!args.record)return;var index=this.recordToIndex[args.record.getId()];if(!Utils.isDefined(index))return;var container=this.jq(ID_TEXT);container.find(".display-raw-line").removeClass("display-raw-line-selected");var sel="[recordIndex='"+index+"']";var element=container.find(sel);element.addClass("display-raw-line-selected");var c=container.offset().top;var s=container.scrollTop();var eo=element.offset();var diff=eo.top-c+s;container.scrollTop(diff)},});}
