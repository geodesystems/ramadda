...
function NotebookState(cell){this.id=HtmlUtils.getUniqueId();this.cell=cell;this.notebook=cell.notebook;$.extend(this,{entries:{},stopFlag:false,prefix:null,result:null,getStop:function(){return this.stopFlag;},getCell:function(){return this.cell;},getNotebook:function(){return this.notebook;},clear:function(){this.cell.clearOutput();},save:function(output){this.notebook.saveNotebook(output);return"notebook saved";},cearEntries:function(){this.clearEntries();},ls:function(entry){var div=new Div();if(!entry)
...
this.write(h);},stop:function(){this.stopFlag=true;},setEntry:function(name,entryId){this.notebook.addEntry(name,entryId);},wiki:async function(s,entry,callback){var write=false;if(!callback)
...
this.running=false;return Utils.call(callback,true);},runInner:async function(){var value=this.input.val();value=value.trim();var help="More information <a target=_help href='"+ramaddaBaseUrl+"/userguide/notebook.html'>here</a>";value=value.replace(/{cellname}/g,this.cellName);value=value.replace(/{help}/g,help);var type="wiki";var blobs=[];var blob="";var commands=value.split("\n");var types=["wiki","html","sh","js"];var ok=true;for(var i=0;i<commands.length;i++){if(!ok)break;value=commands[i].trim();if(value.startsWith("//"))continue;var isType=false;for(var typeIdx=0;typeIdx<types.length;typeIdx++){var t=types[typeIdx];if(value.startsWith(t+":")){if(blob!=""){blobs.push({blob:blob,type:type,div:new Div(),ok:true});}
...
this.output.html(result);for(var i=0;i<blobs.length;i++){var blob=blobs[i];await this.processBlob(blob);if(!blob.ok){console.log("not ok");break;}}
...
this.output.html(h);this.outputUpdated();},outputUpdated:function(){this.outputHtml=this.jq(ID_OUTPUT).html();},focus:function(){this.input.focus();},clearOutput:function(){this.output.html("");this.outputHtml="";},processHtml:async function(blob){blob.div.set(blob.blob);},processWiki:async function(blob){var id=this.notebook.getProperty("entryId","");var entry=this.getCurrentEntry(null);if(entry)id=entry.getId();let _this=this;let divId=HtmlUtils.getUniqueId();var wikiCallback=function(html){var h=HtmlUtils.div(["id",divId,"style"],html);blob.div.set(h);}
...
let _this=this;blob.div.set(r);var i=0;for(i=0;i<commands.length;i++){var cmd=commands[i];await cmd.proc.call(_this,cmd.line,cmd.toks,cmd.div,cmd.extra);}},processJs:async function(blob){try{var current=this.getCurrentEntry();var entries=this.notebook.getCurrentEntries();var state=new NotebookState(this);notebookStates[state.id]=state;var jsSet="";state.entries["current"]=current;name="current";var stateJS="notebookStates['"+state.id+"']";jsSet+="state= "+stateJS+";\n";jsSet+=name+"=state.entries['"+name+"'];\n"
...
if(toks[1].startsWith("..")){var entry=this.getCurrentEntry();await entry.getParentEntry(entry=>{if(entry){this.setCurrentEntry(entry);}else{div.msg("No parent entry");}});return;}
