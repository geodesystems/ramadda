...
var colorBy=this.getColorByInfo(records);for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];this.indexToRecord[rowIdx]=record;this.recordToIndex[record.getId()]=rowIdx;var row=this.getDataValues(record);var line="";for(var col=0;col<fields.length;col++){var f=fields[col];if(rowIdx==0){if(filterFieldMap[f.getId()]){var value=this.getFilterFieldValues(f);if(value&&value.length>0){try{regexpMaps[f.getId()]=[];if(Array.isArray(value)){value.map(v=>regexpMaps[f.getId()].push(new RegExp("("+v+")","i")));}else{regexpMaps[f.getId()].push(new RegExp("("+value+")","i"));}}catch(e){}}}}
