...
fields=allFields;var summary={};for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);for(var col=0;col<fields.length;col++){var f=fields[col];if(!Utils.isDefined(summary[f.getId()])){summary[f.getId()]={counts:{},values:[]}}
var s=summary[f.getId()];var value=row[f.getIndex()];if(!Utils.isDefined(s.counts[value])){var tuple={value:value,count:0}
s.counts[value]=tuple;s.values.push(tuple);}
s.counts[value].count++;}}
var html="";for(var col=0;col<fields.length;col++){var f=fields[col];html+="<b>Field:</b> "+f.getLabel();html+=HtmlUtils.openTag("table",["id",this.getDomId("summary"+col),"table-height","300","class","stripe row-border nowrap ramadda-table"]);html+=HtmlUtils.openTag("thead",[]);html+=HtmlUtils.tr([],HtmlUtils.th([],"Count")+HtmlUtils.th([],"Value"));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);var s=summary[f.getId()];s.values.sort((a,b)=>{if(a.count<b.count)return 1;if(a.count>b.count)return-1;return 0;});for(var i=0;i<s.values.length;i++){var value=s.values[i].value;var count=s.values[i].count;html+=HtmlUtils.tr([],HtmlUtils.td(["align","right"],count)+HtmlUtils.td([],value));}
html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");}
