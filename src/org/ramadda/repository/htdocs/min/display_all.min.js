...
var value=this.getPropertyInner(key,null,skipThis);if(!Utils.isDefined(value))return dflt;return value;},getPropertyInner:function(key,dflt,skipThis){let debug=false;if(debug)console.log("getProperty");if(!skipThis&&Utils.isDefined(this[key])){if(debug)console.log("\tgetProperty-1");return this[key];}
...
let _this=this;let doTooltip=this.getProperty("sparklineDoTooltip",false)||attrs.doTooltip;svg.on("click",function(){var coords=d3.mouse(this);let record=records[Math.round(x.invert(coords[0]))]
_this.getDisplayManager().notifyEvent("handleEventRecordSelection",_this,{select:true,record:record});if(!doTooltip)return;let html=_this.getRecordHtml(record);let ele=$(dom);let offset=ele.offset().top+ele.height();tt.transition().duration(200).style("opacity",.9);tt.html(html).style("left",(d3.event.pageX)+"px").style("top",offset+"px");}).on("mouseout",function(d){tt.transition().duration(500).style("opacity",0);});},canDoGroupBy:function(){return false;},canDoMultiFields:function(){return true;},useChartableFields:function(){return true;},getFieldsToSelect:function(pointData){if(this.useChartableFields())
...
var minx=Number.MAX_VALUE;var miny=Number.MAX_VALUE;var maxx=Number.MIN_VALUE;var maxy=Number.MIN_VALUE;var map={};this.grid.map(o=>{minx=Math.min(minx,o.x);maxx=Math.max(maxx,o.x);miny=Math.min(miny,o.y);maxy=Math.max(maxy,o.y);map[this.getDomId("cell_"+o.x+"_"+o.y)]=o;});var table="<table border=0 cellspacing=0 cellpadding=0>";var w=this.getProperty("cellSize","40");var showLabel=this.getProperty("showCellLabel",true);var cellStyle=this.getProperty("cellStyle","");var cellMap={};for(var y=1;y<=maxy;y++){table+="<tr>";for(var x=1;x<=maxx;x++){var id=this.getDomId("cell_"+x+"_"+y);var o=map[id];var extra=" id='"+id+"' ";var style="position:relative;margin:1px;vertical-align:center;xtext-align:center;width:"+w+"px;"+"height:"+w+"px;";var c="";if(o){style+="background:#ccc;"+cellStyle;extra+=" title='"+o.state+"' ";extra+=" class='display-mapgrid-cell' ";c=HtmlUtils.div(["style","padding-left:3px;"],(showLabel?o.code:""));cellMap[o.code]=id;cellMap[o.state]=id;}
...
let minData=0;let maxData=0;let seen={};var contents=this.jq(ID_DISPLAY_CONTENTS);for(var i=0;i<records.length;i++){var record=records[i];var tuple=record.getData();var state=tuple[stateField.getIndex()];var cellId=cellMap[state];if(!cellId){cellId=cellMap[state.toUpperCase()];}
if(!cellId){console.log("Could not find cell:"+state);continue;}
if(!stateData[state]){states.push(state);stateData[state]={cellId:cellId,data:[],records:[]}}
