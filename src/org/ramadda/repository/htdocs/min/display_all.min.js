...
this.addCell("init cell",props,false).run();this.cells[0].focus();});},formatObject:function(value){return Utils.formatJson(value);},initOutputRenderers:function(){let notebook=this;this.outputRenderers=[];this.addOutputRenderer({shouldRender:(value)=>{return typeof value==="object"&&value.getTime;},render:(value)=>{return notebook.formatDate(value)},});this.addOutputRenderer({shouldRender:(value)=>{var t=typeof value;return t==="string"||t==="number"||t==="boolean";},render:(value)=>{if(typeof value==="string"){if(value.split("\n").length>1){return HtmlUtils.div(["style"," white-space: pre;"],value);}}
return value},});this.addOutputRenderer({shouldRender:(value)=>{return typeof value==="object"&&"lat"in value&&"lon"in value;},render:(value)=>{return"<img src='http://staticmap.openstreetmap.de/staticmap.php?center="+value.lat+","+value.lon+"&zoom=17&size=300x200&maptype=mapnik'/>"},});},addOutputRenderer:function(renderer){if(this.outputRenderers.indexOf(renderer)<0){this.outputRenderers.push(renderer);}},formatOutput:function(value){for(var i=this.outputRenderers.length-1;i>=0;i--){var renderer=this.outputRenderers[i];if(renderer.shouldRender(value)){return renderer.render(value);}}
return null;},addGlobal:function(name,value){if(Utils.isDefined(window[name]))window[name]=value;this.globals[name]=value;},getBaseEntry:function(){return this.baseEntry;},getRootEntry:function(){return this.rootEntry;},getPopup:function(){return this.jq(ID_MENU);},loadJson:async function(data){if(data.error){this.setContents(_this.getMessage("Failed to load notebook: "+data.error));return;}
...
this.jq(ID_CELLS).html(html);},plugins:{},addPlugin:async function(plugin,chunk){var error;await Utils.importJS(plugin.url,()=>{},(jqxhr,settings,exception)=>{error="Error fetching plugin url:"+plugin.url;});if(!error){var module=plugin.module;var tries=500;while(window[module]==null&&tries-->0){await new Promise(resolve=>setTimeout(resolve,10));}
if(!window[module]){error="Could not load plugin module: "+module;}}
if(error){this.log(error,"error","nb",chunk.div);return;}
this.plugins[plugin.languageId]=plugin;},hasPlugin:function(id){return this.plugins[id]!=null;},processChunkWithPlugin:async function(id,chunk,callback){var module=this.plugins[id].module;var func=this.plugins[id].evaluator;var result=window[module][func](chunk.content);return Utils.call(callback,result);},log:function(msg,type,from,div){var icon="";var clazz="display-notebook-console-item";if(typeof msg=="object"){msg=Utils.formatJson(msg);}
...
if(!ok)return;for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];if(cell.runFirst)continue;await this.runCell(cell,true).then(result=>ok=result);}},runCell:async function(cell,doingAll){if(cell.hasRun)return true;await cell.run(result=>ok=result,{doingAll:doingAll});if(!ok)return false;var raw=cell.getRawOutput();if(raw){raw=raw.trim();if(Utils.stringDefined(cell.cellName)){this.cellValues[cell.cellName]=raw;}}
return true;},toggleAll:function(on){this.cells.map(cell=>{cell.showInput=on;cell.applyStyle();});},});}
function NotebookState(cell,div){this.id=HtmlUtils.getUniqueId();this.cell=cell;this.notebook=cell.notebook;$.extend(this,{entries:{},div:div,stopFlag:false,result:null,log:function(msg,type,from){this.getNotebook().log(msg,type,from,this.div);},clearConsole:function(){this.getNotebook().clearConsole();},getStop:function(){return this.stopFlag;},getCell:function(){return this.cell;},setValue:function(name,value){this.notebook.setCellValue(name,value);},makeData:async function(entry){if(!entry)
...
this.running=false;return Utils.call(callback,true);},prepareToLayout:function(){this.content=this.getInputText();},getInputText:function(){if(!this.editor)return this.content;return this.editor.getValue();},runInner:async function(value,doRows,doingAll){value=value.trim();value=value.replace(/{cellname}/g,this.cellName);value=this.notebook.convertInput(value);if(!this.divs)this.divs=[];var type="wiki";var rest="";var chunks=[];var chunk="";var commands=value.split("\n");var prevDiv=null;var divCnt=0;var makeChunk=(type,chunk,div,doChunk,rest)=>{var props=Utils.parseAttributes(rest);if(props["skipoutput"]===true){div.set("");div=new Div();}
var depends=[];if(props["depends"]&&typeof props["depends"]=="string")depends=props["depends"].split(",");var chunk={name:props["name"],depends:depends,output:null,runFirst:props["runFirst"],hasRun:false,content:chunk,type:type,props:props,div:div,doChunk:doChunk,ok:true}
return chunk;};var makeDiv=()=>{var div=(divCnt<this.divs.length?this.divs[divCnt]:null);divCnt++;if(div){if(div.jq().size()==0){div=null;}else{}}else{}
if(!div){div=new Div(null,"display-notebook-chunk");this.divs.push(div);if(prevDiv)prevDiv.jq().after(div.toString());else this.output.html(div.toString());}else{}
prevDiv=div;div.jq().show();return div;};var doChunk=true;for(var i=0;i<commands.length;i++){var command=commands[i];var _command=command.trim();if(_command.startsWith("//"))continue;if(_command.startsWith("%%")){var newRest=_command.substring(2).trim();var newType;var index=newRest.indexOf(" ");if(index<0){newType=newRest;newRest="";}else{newType=newRest.substring(0,index).trim();newRest=newRest.substring(index);}
if(chunk!=""){var div=makeDiv();chunks.push(makeChunk(type,chunk,div,doChunk,rest));}
doChunk=doRows?doRows[i]:true;chunk="";if(chunk!="")chunk+="\n";if(newType!="")
type=newType;rest=newRest;continue;}
chunk=chunk+command+"\n";}
if(chunk!=""){var div=makeDiv();chunks.push(makeChunk(type,chunk,div,doChunk,rest));}
var chunkMap={};for(var i=0;i<chunks.length;i++){var chunk=chunks[i];if(chunk.name){chunkMap[chunk.name]=chunk;}}
for(var i=divCnt;i<this.divs.length;i++){this.divs[i].jq().hide();}
this.rawOutput="";var ok=true;await this.runChunks(chunks,chunkMap,doingAll,true,r=>ok=r);if(!ok)return false;await this.runChunks(chunks,chunkMap,doingAll,false,r=>ok=r);if(!ok)return false;Utils.initContent("#"+this.getDomId(ID_OUTPUT));return true;},runChunks:async function(chunks,chunkMap,doingAll,justFirst,callback){for(var i=0;i<chunks.length;i++){var chunk=chunks[i];var ok=true;await this.runChunk(chunk,chunkMap,doingAll,justFirst,(r=>ok=r));if(!ok)return Utils.call(callback,false);}
return Utils.call(callback,true);},runChunk:async function(chunk,chunkMap,doingAll,justFirst,callback){var ok=true;if(!chunk.doChunk){ok=false;}else if(chunk.hasRun){ok=false;}else if(justFirst===true&&!chunk.props["runfirst"]){ok=false;}else if(justFirst===false&&chunk.props["runfirst"]===true){ok=false;}
if(!ok){return Utils.call(callback,true);}
chunk.div.set("");if(doingAll&&chunk.props["skiprunall"]===true){return Utils.call(callback,true);}
chunk.hasRun=true;for(var i=0;i<chunk.depends.length;i++){var name=chunk.depends[i];if(chunkMap[name]&&!chunkMap[name].hasRun){var ok=true;await this.runChunk(chunkMap[name],chunkMap,false,null,(r=>ok=r));if(!ok){console.log("run chunk after");return Utils.call(callback,false);}}}
await this.processChunk(chunk);if(!chunk.ok){Utils.call(callback,false);return;}
if(chunk.name&&(typeof chunk.name=="string")){var name=chunk.name.trim();if(chunk.output){if(name!=""){this.notebook.addGlobal(name,chunk.output);}}else{this.notebook.addGlobal(name,null);}}
...
this.output.html(h);this.outputUpdated();},outputUpdated:function(){this.outputHtml=this.jq(ID_OUTPUT).html();},getRawOutput:function(){return this.rawOutput;},focus:function(){this.input.focus();},clearOutput:function(){if(this.divs)
this.divs.map(div=>div.set(""));this.outputHtml="";},processHtml:async function(chunk){var content=chunk.content;if(content.match("%\n*$")){content=content.trim();content=content.substring(0,content.length-1);}
this.rawOutput+=content+"\n";chunk.output=content;chunk.div.set(content);},processCss:async function(chunk){var css=HtmlUtils.tag("style",["type","text/css"],chunk.content);this.rawOutput+=css+"\n";chunk.output=css;chunk.div.set(css);},handleError:function(chunk,error,from){chunk.ok=false;console.log("An error occurred:"+error);this.notebook.log(error,"error",from,chunk.div);},processFetch:async function(chunk){var lines=chunk.content.split("\n");for(var i=0;i<lines.length;i++){var line=lines[i].trim();if(line=="")continue;var origLine=line;var error=null;var url;var msgExtra="";var idx=line.indexOf(":");if(idx<0){this.handleError(chunk,"Bad fetch line:"+line,"io");return;}
var tag=line.substring(0,idx);line=line.substring(idx+1).trim();var idx=line.indexOf(" //");if(idx>=0){line=line.substring(0,idx).trim();}
if(tag=="js"){url=line;if(url.match("jquery-.*\\.js"))return;await Utils.importJS(url,()=>{},(jqxhr,settings,exception)=>{error="Error fetching "+origLine+" "+(exception?exception.toString():"");},false);}else if(tag=="css"){url=line;await Utils.importCSS(url,null,(jqxhr,settings,exception)=>error="Error fetching "+origLine+" "+exception,true);}else if(tag=="html"){url=line;await Utils.importText(url,h=>chunk.div.append(h),(jqxhr,settings,exception)=>error="Error fetching "+origLine+" "+exception);}else if(tag=="text"||tag=="json"){var isJson=tag=="json";var v=null;var indexEquals=line.indexOf("=");var indexHttp=line.indexOf("http");var indexSlash=line.indexOf("/");if(indexEquals>0){var index=indexHttp>=0?indexHttp:indexSlash;if(indexSlash>=0&&indexSlash<indexHttp)index=indexSlash;if(indexEquals<index){v=line.substring(0,indexEquals).trim();msgExtra=" (var "+v+")";line=line.substring(indexEquals+1).trim();}}
...
if(v){this.notebook.addGlobal(v,results);}else{if(isJson){results=JSON.stringify(results,null,2);}
chunk.div.append(HtmlUtils.pre(["style","max-width:100%;overflow-x:auto;"],results));}}}else{error="Unknown fetch:"+origLine;}
if(error){this.handleError(chunk,error,"io");return;}else{this.notebook.log("Loaded: "+url+msgExtra,"output","io");}}},processMd:async function(chunk){this.rawOutput+=chunk.content+"\n";var content=chunk.content;if(content.match("%\n*$")){content=content.trim();content=content.substring(0,content.length-1);}
...
var state=new NotebookState(this,chunk.div);window.iodide=window.notebook=state;pyodide.runPython(chunk.content);if(state.getStop()){chunk.ok=false;}},processPlugin:async function(chunk){var state=new NotebookState(this,chunk.div);window.iodide=window.notebook=state;var plugin=JSON.parse(chunk.content);await this.notebook.addPlugin(plugin,chunk);},processWiki:async function(chunk){this.rawOutput+=chunk.content+"\n";var id=this.notebook.getProperty("entryId","");await this.getCurrentEntry(e=>entry=e);if(entry)id=entry.getId();let _this=this;let divId=HtmlUtils.getUniqueId();var wikiCallback=function(html){var h=HtmlUtils.div(["id",divId,"style"],html);chunk.div.set(h);chunk.output=h;}
...
await cmd.proc.call(_this,cmd.line,cmd.toks,cmd.div,cmd.extra);}},processJs:async function(chunk){var lines;var topLines=0;try{await this.getCurrentEntry(e=>{current=e});var state=new NotebookState(this,chunk.div);window.iodide=window.notebook=state;notebookStates[state.id]=state;var notebookEntries=this.notebook.getCurrentEntries();for(name in notebookEntries){state.entries[name]=notebookEntries[name].entry;}
...
for(name in this.notebook.globals){name=name.trim();if(name=="")continue;if(!Utils.isDefined(window[name])){topLines++;jsSet+="var "+name+"= notebook.getNotebook().globals['"+name+"'];\n";}}
var js=chunk.content.trim();lines=js.split("\n");js=jsSet+"\n"+js;var result=eval.call(null,js);if(state.getStop()){chunk.ok=false;}
var html="";if(result!=null){chunk.output=result;var rendered=this.notebook.formatOutput(result);if(rendered!=null){html=rendered;}else{var type=typeof result;if(type!="object"&&type!="function"){html=result;this.rawOutput+=html+"\n";}}}
chunk.div.append(html);}catch(e){chunk.ok=false;var line=lines[e.lineNumber-topLines-1];this.notebook.log("Error: "+e.message+"<br>&gt;"+(line?line:""),"error","js",chunk.div);}
notebookStates[state.id]=null;},processChunk:async function(chunk){for(name in this.notebook.globals){var value=this.notebook.globals[name];if(typeof value=="object"){value=Utils.formatJson(value);}
chunk.content=chunk.content.replace("${"+name.trim()+"}",value);}
if(chunk.type=="html"){await this.processHtml(chunk);}else if(chunk.type=="plugin"){await this.processPlugin(chunk);}else if(chunk.type=="wiki"){await this.processWiki(chunk);}else if(chunk.type=="css"){await this.processCss(chunk);}else if(chunk.type=="fetch"){await this.processFetch(chunk);}else if(chunk.type=="raw"){var content=chunk.content;chunk.output=content;var re=new RegExp("^ *var *= *(.*)");this.rawOutput+=content;}else if(chunk.type=="js"){await this.processJs(chunk);}else if(chunk.type=="sh"){await this.processSh(chunk);}else if(chunk.type=="meta"){}else if(chunk.type=="md"){await this.processMd(chunk);}else if(chunk.type=="py"){await this.processPy(chunk);}else{if(this.notebook.hasPlugin(chunk.type)){var result;await this.notebook.processChunkWithPlugin(chunk.type,chunk,r=>result);return;}
this.notebook.log("Unknown type:"+chunk.type,"error",null,chunk.div);chunk.ok=false;}},calculateInputHeight:function(){this.content=this.getInputText();if(!this.content)return;var lines=this.content.split("\n");if(lines.length!=this.inputRows){this.inputRows=lines.length;this.input.attr("rows",Math.max(1,this.inputRows));}},writeStatusMessage:function(v){var msg=this.jq(ID_MESSAGE);if(!v){msg.hide();msg.html("");}else{msg.show();msg.position({of:this.getOutput(),my:"left top",at:"left+4 top+4",collision:"none none"});msg.html(v);}},handleControlKey:function(event){var k=event.which;},getOutput:function(){return this.jq(ID_OUTPUT);},getInput:function(){return this.jq(ID_INPUT);},writeResult:function(html){this.writeStatusMessage(null);html=HtmlUtils.div([ATTR_CLASS,"display-notebook-result"],html);var output=this.jq(ID_OUTPUT);output.append(html);output.animate({scrollTop:output.prop("scrollHeight")},1000);this.currentOutput=output.html();this.currentInput=this.getInputText();},writeError:function(msg){this.writeStatusMessage(msg);},header:function(msg){return HtmlUtils.div([ATTR_CLASS,"display-notebook-header"],msg);},processCommand_help:function(line,toks,div,callback,prefix){if(div==null)div=new Div();var help="";if(prefix!=null)help+=prefix;help+="<pre>pwd, ls, cd</pre>";return div.append(help);},entries:{},selectEntry:function(entryId){var cnt=1;var entries=this.notebook.getCurrentEntries();while(entries["entry"+cnt]){cnt++;}
...
function RamaddaSkewtDisplay(displayManager,id,properties){let SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_SKEWT,properties);var ID_SKEWT="skewt";RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},initDisplay:function(){SUPER.initDisplay.call(this);},updateUI:async function(){if(!this.loadedResources){await Utils.importCSS(ramaddaBaseUrl+"/lib/skewt/sounding.css");await Utils.importJS(ramaddaBaseUrl+"/lib/skewt/d3skewt.js");this.loadedResources=true;}
if(!window["D3Skewt"]){setTimeout(()=>this.updateUI(),100);return;}
SUPER.updateUI.call(this);var skewtId=this.getDomId(ID_SKEWT);var html=HtmlUtils.div(["id",skewtId],"");this.setContents(html);var records=this.filterData();if(!records){console.log("no data yet");return;}
...
var fields=this.getData().getRecordFields();var names=["pressure","height","temperature","dewpoint","wind_direction","wind_speed"];var pressure=this.getFieldById(fields,"pressure");var height=this.getFieldById(fields,"height");var height=this.getFieldById(fields,"height");var data={};for(var i=0;i<names.length;i++){var field=this.getFieldById(fields,names[i]);if(field==null){this.displayError("No "+names[i]+" defined in data");return;}
var values=this.getColumnValues(records,field).values;data[names[i]]=values;}
this.skewt=new D3Skewt(skewtId,options,data);await this.getDisplayEntry((e)=>{var q=e.getAttribute("variables");if(!q)return;q=q.value;q=q.replace(/\r\n/g,"\n");q=q.replace(/^ *\n/,"");q=q.replace(/^ *([^:]+):([^\n].*)$/gm,"<div title='$1' class=display-skewt-index-label>$1</div>: <div title='$2'  class=display-skewt-index>$2</div>");q=q.replace(/[[\r\n]/g,"\n");q=HtmlUtils.div(["class","display-skewt-text"],q);$("#"+this.skewt.textBoxId).html(q);});}});}
...
return source.getProperty(prop,dflt);},applyVectorMap:function(force){if(!force&&this.vectorMapApplied){return;}
if(!this.doDisplayMap()){return;}
if(!this.vectorLayer){return;}
if(!this.points){return;}
this.vectorMapApplied=true;var features=this.vectorLayer.features.slice();var circles=this.points;for(var i=0;i<circles.length;i++){var circle=circles[i];if(circle.style&&circle.style.display=="none")continue;var center=circle.center;var matchedFeature=null;var index=-1;for(var j=0;j<features.length;j++){var feature=features[j];var geometry=feature.geometry;if(!geometry){break;}
...
for(var i=0;i<features.length;i++){var feature=features[i];style=feature.style;if(!style)style={"stylename":"from display"};$.extend(style,{"display":"none"});}
...
if(this.map.circles)
this.map.circles.redraw();this.applyVectorMap(true);},updateUI:function(){this.haveCalledUpdateUI=true;SUPER.updateUI.call(this);if(!this.getDisplayReady()){return;}
...
colorBy.range=colorBy.maxValue-colorBy.minValue;if(this.points){for(var i=0;i<this.points.length;i++)
...
if(showPoints){if(!Utils.isDefined(seen[point]))seen[point]=0;if(seen[point]>500)continue;seen[point]++;point=this.map.addPoint("pt-"+i,point,props,html,dontAddPoint);var date=pointRecord.getDate();if(date){point.date=date.getTime();}
this.points.push(point);}}
if(didColorBy){this.displayColorTable(colors,ID_BOTTOM,colorBy.origMinValue,colorBy.origMaxValue,{stringValues:colorByValues});}
