...
this.startAnimation();},getDomId:function(id){return this.display.getDomId(id);},jq:function(id){return this.display.jq(id);},init:function(dateMin,dateMax){let _this=this;this.dateMin=dateMin;this.dateMax=dateMax;this.dateRange=this.dateMax.getTime()-this.dateMin.getTime();this.begin=this.dateMin;this.end=this.dateMin;if(this.label)
this.label.html(this.formatAnimationDate(this.dateMin)+" - "+this.formatAnimationDate(this.dateMax));if(this.dateMin){this.jq(ID_SLIDER).slider({range:true,min:this.dateMin.getTime(),max:this.dateMax.getTime(),values:[this.dateMin.getTime(),this.dateMax.getTime()],slide:function(event,ui){_this.stopAnimation();_this.begin=new Date(ui.values[0]);_this.end=new Date(ui.values[1]);_this.updateLabels();},stop:function(event,ui){_this.stopAnimation();_this.begin=new Date(ui.values[0]);_this.end=new Date(ui.values[1]);_this.applyAnimation(true);}});}},makeControls:function(){var buttons=HtmlUtils.span(["id",this.getDomId(ID_BEGIN),"title","Go to beginning"],HtmlUtils.getIconImage("fa-fast-backward"))+
...
HtmlUtils.span(["id",this.getDomId(ID_ANIMATION_LABEL),"class","display-map-animation-label"]);buttons=HtmlUtils.div(["class","display-map-animation-buttons"],buttons);if(display.getProperty("animationShowSlider",true)){buttons+=HtmlUtils.div(["class","display-map-slider","id",this.getDomId(ID_SLIDER)]);}
