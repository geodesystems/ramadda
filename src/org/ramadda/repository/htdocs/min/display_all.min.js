...
var colors=this.getColorTable();if(!colors)colors=Utils.getColorTable("blues",true);var colorBy=this.getColorByInfo(records,null,null,colors);let names={};let nameList=[];let links={};let maxWeight=0;let sortBy=this.getProperty("sortBy","name");let directed=this.getProperty("directed",true);let missing=-999999;records.map(r=>{var source=r.getValue(sourceField.getIndex());var target=r.getValue(targetField.getIndex());var weight=missing;if(weightField){weight=r.getValue(weightField.getIndex());maxWeight=Math.max(maxWeight,weight);}
if(true||!names[source]){names[source]=true;nameList.push({name:source,weight:weight});}
if(true||!names[target]){names[target]=true;nameList.push({name:target,weight:weight});}
links[source+"--"+target]=weight;});maxWeight=this.getProperty("maxWeight",maxWeight);nameList.sort((a,b)=>{if(sortBy=="name"||sortBy==""){return a.name.localeCompare(b.name);}else{return b.weight-a.weight;}});var seen={}
var tmp=[];let minWeight=this.getProperty("minWeight",missing);nameList=nameList.map(t=>{if(minWeight!=missing){if(t.weight==missing||t.weight<minWeight)return;}
if(!seen[t.name]){seen[t.name]=true;tmp.push(t.name);}});nameList=tmp;let table="<div style='margin-top:"+this.getProperty("topSpace","100px")+";'></div><table style='height:100%;' class='display-cooc-table' order=0 cellpadding=0 cellspacing=0  >"
table+="<tr valign=bottom><td style='border:none;'></td>";nameList.map(target=>{target=target.replace(/ /g,"&nbsp;").replace(/-/g,"&nbsp;");table+=HtmlUtils.td(["style","border:none;","width","6"],HtmlUtils.div(["class","display-cooc-colheader"],target));});var missingBackground=this.getProperty("missingBackground","#eee");nameList.map(source=>{var label=source.replace(/ /g,"&nbsp;");table+="<tr valign=bottom ><td style='   border:none;' align=right>"+HtmlUtils.div(["class","display-cooc-rowheader"],label)+"</td>";nameList.map(target=>{var weight=links[source+"--"+target];if(!directed&&!Utils.isDefined(weight))
