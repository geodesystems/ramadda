...
var newData=[];var values=[];var operators=[];for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];var prefix=this.getProperty(filterField.getId()+".filterPrefix");var suffix=this.getProperty(filterField.getId()+".filterSuffix");var value=$("#"+this.getDomId("filterby_"+filterField.getId())).val();if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;values.push(value);operators.push($("#"+this.getDomId("filterby_"+filterField.getId()+"_operator")).val());}
for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);var ok=true;for(var i=0;i<this.filterFields.length;i++){if(!ok)break;var filterField=this.filterFields[i];var filterValue=values[i];if(filterValue=="")continue;var value=row[filterField.getIndex()];if(filterField.getType()=="enumeration"){if(value!=filterValue){ok=false;}}else if(filterField.isNumeric){filterValue=parseFloat(filterValue);var op=operators[i];if(op==null)op="<";if(op=="<"){ok=value<filterValue;}else if(op=="<="){ok=value<=filterValue;}else if(op=="="){ok=value==filterValue;}else if(op==">"){ok=value>filterValue;}else if(op==">="){ok=value>=filterValue;}}else{value=(""+value).toLowerCase();if(value.indexOf(filterValue.toLowerCase())<0){ok=false;}}}
...
widget=HtmlUtils.select("",["id",widgetId,"fieldId",filterField.getId()],enums,value);}else if(filterField.isNumeric){var opid=widgetId+"_operator";var operator=this.getProperty(filterField.getId()+".filterOperator","<");widget=HtmlUtils.select("",["id",opid,"fieldId",filterField.getId()],["<",">","="],operator,);widget+=HtmlUtils.input("",value,["id",widgetId,"size",4,"fieldId",filterField.getId()]);}else{widget=HtmlUtils.input("",value,["id",widgetId,"fieldId",filterField.getId()]);}
var label=this.getProperty(filterField.getId()+".filterLabel",filterField.getLabel());widget=label+": "+widget;searchBar+=widget+"&nbsp;&nbsp;";}
this.jq(ID_HEADER2).html(searchBar);this.jq(ID_HEADER2).find("input, input:radio,select").change(function(){var id=$(this).attr("id");var value=$(this).val();var fieldId=$(this).attr("fieldId");_this.haveCalledUpdateUI=false;if(_this.settingFilterValue){console.log("setting value");return;}
