...
function NotebookState(cell){this.id=HtmlUtils.getUniqueId();this.cell=cell;this.notebook=cell.notebook;$.extend(this,{entries:{},stop:false,prefix:null,result:null,getCell:function(){return this.cell;},getNotebook:function(){return this.notebook;},clear:function(){this.cell.clearOutput();},save:function(output){this.notebook.saveNotebook(output);return"notebook saved";},cearEntries:function(){this.clearEntries();},ls:function(entry){var div=new Div();if(!entry)
entry=this.cell.getCurrentEntry(null);this.call.getEntryHeading(entry,div);this.write(div.toString());},lsEntries:function(){var h="";var entries=this.currentEntries;for(var name in entries){var e=entries[name];h+=name+"="+e.entry.getName()+"<br>";}
this.write(h);},stop:function(){this.stop=true;},setEntry:function(name,entryId){this.notebook.addEntry(name,entryId);},wiki:async function(s,entry,callback){var write=false;if(!callback)
callback=h=>this.write(h);if(entry==null)entry=this.cell.getCurrentEntry();if((typeof entry)!="string")entry=entry.getId();await GuiUtils.loadHtml(ramaddaBaseUrl+"/wikify?doImports=false&entryid="+entry+"&text="+encodeURIComponent(s),callback);},write:function(s){if(this.prefix==null){this.prefix=s;}else{this.prefix+=s;}},linechart:function(entry){this.cell.createDisplay(entry,DISPLAY_LINECHART);},help:function(){return"Enter an expression, e.g.:<pre>1+2</pre> or wrap you code in brackets:<pre>{\nvar x=1+2;\nreturn x;\n}</pre>"+
...
setTimeout(process,1);return r;},processJs:async function(blob,result,callback){try{var current=this.getCurrentEntry();var entries=this.notebook.getCurrentEntries();var state=new NotebookState(this);notebookStates[state.id]=state;var jsSet="";state.entries["current"]=current;name="current";var stateJS="notebookStates['"+state.id+"']";jsSet+="state= "+stateJS+";\n";jsSet+=name+"=state.entries['"+name+"'];\n"
for(var name in entries){var e=entries[name];state.entries[name]=e.entry;jsSet+=name+"= state.entries['"+name+"'];\n"}
nbCell=this;nbNotebook=this.notebook;blob=blob.trim();var lines=blob.split("\n");var js;if(lines.length==1&&!blob.startsWith("return ")&&!blob.startsWith("{")){blob="return "+blob;}
...
var jsReturn;eval(js);await nbFunc().then(function(r){jsReturn=r;});if(state.stop){result.ok=false;}
var html="";if(Utils.stringDefined(state.prefix))html+=state.prefix;if(jsReturn)html+=jsReturn;return Utils.call(callback,html);}catch(e){result.ok=false;var lines=blob.split("\n");var line=lines[e.lineNumber-2];return Utils.call(callback,"Error: "+e.message+"<br>"+HtmlUtils.span(["class","display-notebook-error"]," &gt; "+line));}},processBlob:async function(type,blob,result,callback){if(blob==""){return Utils.call(callback,"");}
