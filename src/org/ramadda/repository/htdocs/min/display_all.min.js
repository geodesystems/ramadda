...
value=HtmlUtils.image(value,attrs);}}else if(f.getType()=="url"){if(value&&value.trim().length>1){value=HtmlUtils.href(value,value);}}else if(f.isDate){if(value){s=s.replace("${"+f.getId()+"}",value);s=s.replace("${"+f.getId()+"_yyyy}",Utils.formatDateYYYY(value));s=s.replace("${"+f.getId()+"_yyyymmdd}",Utils.formatDateYYYYMMDD(value));}
continue;}
