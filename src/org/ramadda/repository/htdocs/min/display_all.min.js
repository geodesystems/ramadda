...
return ct;}
if(this.getProperty("colors")){var colors=this.getProperty("colors");if((typeof colors)=="object")return this.addAlpha(colors);return this.addAlpha(colors.split(","));}
...
console.log("\thave fixed fields:"+fixedFields.length);for(var i=0;i<fixedFields.length;i++){var sfield=fixedFields[i];if(this.debugSelected)
console.log("\t\tfixed field:"+sfield);for(var j=0;j<fields.length;j++){var field=fields[j];var id=field.getId();if(this.debugSelected)
console.log("\t\t\tlooking at:"+id);if(id==sfield||("#"+(j+1))==sfield){if(this.debugSelected)
...
if(this.getProperty("colors")){var v=this.getProperty("colors");if(!Array.isArray(v)){v=v.split(",");}
this.colorList=v;}
...
newList.push(newRow);}
dataList=newList;}
try{this.chart=this.makeChart(dataList,props,selectedFields);}catch(e){console.log("Error making chart:\n"+e+"\n"+e.stack);return;}
...
if(options.windStride>1){var new_wind_speed=[];var new_wind_direction=[];for(var i=0;i<data.wind_speed.length;i++){var pres=data.pressure[i];if(i%options.windStride==0){new_wind_speed.push(data.wind_speed[i]);new_wind_direction.push(data.wind_direction[i]);}else{new_wind_speed.push(0);new_wind_direction.push(0);}}
data.wind_speed=new_wind_speed;data.wind_direction=new_wind_direction;}
...
if(args.highlight){var point=new OpenLayers.LonLat(args.record.getLongitude(),args.record.getLatitude());var attrs={pointRadius:parseFloat(this.getProperty("recordHighlightRadius",+this.getProperty("radius",8)+8)),stroke:true,strokeColor:this.getProperty("recordHighlightStrokeColor","#000"),strokeWidth:parseFloat(this.getProperty("recordHighlightStrokeWidth",1)),fillColor:this.getProperty("recordHighlightFillColor","#ccc"),fillOpacity:parseFloat(this.getProperty("recordHighlightFillOpacity",0.75)),};if(this.getProperty("recordHighlightUseMarker",false)){var size=parseFloat(this.getProperty("recordHighlightRadius",+this.getProperty("radius",24)));this.highlightMarker=this.map.addMarker("pt-"+i,point,null,"pt-"+i,null,null,size);}else{this.highlightMarker=this.map.addPoint(args.record.getId(),point,attrs);}
