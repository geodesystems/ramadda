...
return Math.floor(perc*opts.h);};}else{scaleY=(lat,lon)=>{return Math.floor(opts.h*(lon-args.bounds.west)/earthHeight);}}
...
bounds=RecordUtil.expandBounds(bounds,0.1);if(dfltArgs.cellSize==0){let size=1;while(w/(bounds.east-bounds.west)/size>1000)size++;dfltArgs.cellSizeX=dfltArgs.cellSizeY=dfltArgs.cellSize=size;console.log("s:"+size);}else if(String(dfltArgs.cellSize).endsWith("%")){dfltArgs.cellSize=dfltArgs.cellSizeX=dfltArgs.cellSizeY=Math.floor(parseFloat(dfltArgs.cellSize.substring(0,dfltArgs.cellSize.length-1))/100*w);}
