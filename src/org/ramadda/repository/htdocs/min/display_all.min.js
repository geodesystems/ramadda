...
return min;},}
var RecordUtil={gridData:function(records,w,h,colorBy,color,cellSize,args){if(args)args={};if(!args.type)args.type="circle";let id=HtmlUtils.getUniqueId();let canvas='<canvas style="display:none;" id="'+id+'" width="'+w+'" height="'+h+'"></canvas>';$(document.body).append(canvas);var c=document.getElementById(id);var ctx=c.getContext("2d");let cnt=0;let bounds={};RecordUtil.getPoints(records,bounds);let ew=bounds.east-bounds.west;let eh=bounds.north-bounds.south;let halfW=cellSize/2;let halfH=cellSize/2;records.map(record=>{let lat=record.getLatitude();let lon=record.getLongitude();let x=Math.round(w*(lon-bounds.west)/ew)-halfW;let y=h-(Math.round(h*(lat-bounds.south)/eh)-halfH);record.coordinates={x:x,y:y};let c=color||"#000";if(colorBy&&colorBy.index>=0){c=colorBy.getColor(record.getData()[colorBy.index],record);}
ctx.fillStyle=c;ctx.strokeStyle=c;if(args.type=="circle"){ctx.beginPath();ctx.arc(x,y,cellSize,0,2*Math.PI);if(args.stroke)
ctx.stroke();else
ctx.fill();}else{if(args.stroke)
ctx.strokeRect(x,y,cellSize||4,cellSize||4);else
ctx.fillRect(x,y,cellSize||4,cellSize||4);}});return c.toDataURL("image/png");},getRanges:function(fields,records){var maxValues=[];var minValues=[];for(var i=0;i<fields.length;i++){maxValues.push(NaN);minValues.push(NaN);}
...
this.map.clearSeenMarkers();var t2=new Date();this.addPoints(records,fields,points,pointBounds);var t3=new Date();this.addLabels(records,fields,points);this.applyVectorMap();this.lastUpdateTime=new Date();},addPoints:function(records,fields,points,pointBounds){let cidx=0
