...
this.checkSearchBar();this.updateUI();},getFilterId:function(id){return this.getDomId("filterby_"+id);},getHeader2:function(){return"";},initHeader2:function(){},getRequestMacros:function(){let macros=[];if(!this.getProperty("macros",""))return macros;this.getProperty("macros","").split(",").every(macro=>{if(macro=="")return true;let values=null;let enums=this.getProperty("macro."+macro+".values");if(enums){values=[]
...
if(!widget)return"";return(visible?this.display.makeFilterLabel(label+": "):"")+widget;},getId:function(){return"macro_"+this.name;},getValue:function(){let widget=this.display.jq(this.getId());let value=this.dflt;if(widget.length!=0)value=widget.val();this.display.setProperty(this.name+".default",value);return value;},apply:function(url){if(this.type=="numeric"){let min=this.display.jq(this.getId()+"_min").val()||"";let max=this.display.jq(this.getId()+"_max").val()||"";if(min!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg+"_from",min);if(max!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg+"_to",max);}else if(this.type=="date"){let from=this.display.jq(this.getId()+"_from").val()||"";let to=this.display.jq(this.getId()+"_to").val()||"";if(from!="")
...
return url;}});return true;});return macros;},makeFilterLabel:function(label,tt){let attrs=["class","display-filter-label"];if(tt){attrs.push("title");attrs.push(tt);}
return HtmlUtils.span(attrs,label);},checkSearchBar:function(){let _this=this;this.filterFields=[];this.colorByFields=this.getFieldsByIds(null,this.getProperty("colorByFields","",true));this.sizeByFields=this.getFieldsByIds(null,this.getProperty("sizeByFields","",true));let pointData=this.getData();if(pointData==null)return;let fields=pointData.getRecordFields();let records=pointData.getRecords();let header2=HtmlUtils.span(["xstyle","display:inline-block;","id",this.getDomId(ID_HEADER2_PREFIX)]);header2+=this.getHeader2();if(this.getProperty("pageRequest",false)){header2+=HtmlUtils.span(["id",this.getDomId(ID_PAGE_COUNT)]);}
let macros=this.getRequestMacros();let macroDateIds=[];macros.every(macro=>{header2+=macro.getWidget(macroDateIds)+"&nbsp;&nbsp;";return true;});if(this.getProperty("includeBounds")&&this.getBounds){header2+=HtmlUtils.checkbox("",["id",this.getDomId(ID_INCLUDE_BOUNDS)],false)+" In bounds&nbsp;&nbsp";}
...
header2+=HtmlUtils.span(["xstyle","display:inline-block;","id",this.getDomId(ID_HEADER2_SUFFIX)]);this.jq(ID_HEADER2).html(header2);this.initHeader2();var theDisplay=this;let macroChange=(macro,value,what)=>{if(this.settingMacroValue)return;this.macroChanged();this.reloadData();this.settingMacroValue=true;var args={entryId:this.entryId,property:"macroValue",id:macro.name,what:what,value:value};this.propagateEvent("handleEventPropertyChanged",args);this.settingMacroValue=false;};macroDateIds.every(id=>{HtmlUtils.datePickerInit(id);return true;});this.jq(ID_INCLUDE_BOUNDS).change(function(){macroChange("bounds",$(this).is(':checked'));});macros.every(macro=>{this.jq(macro.getId()).change(function(e){macroChange(macro,$(this).val());});this.jq(macro.getId()+"_min").change(function(e){macroChange(macro,$(this).val(),"min");});this.jq(macro.getId()+"_max").change(function(e){macroChange(macro,$(this).val(),"max");});this.jq(macro.getId()+"_from").change(function(e){macroChange(macro,$(this).val(),"from");});this.jq(macro.getId()+"_to").change(function(e){macroChange(macro,$(this).val(),"to");});return true;});let inputFunc=function(input,input2,value){var id=input.attr("id");if(!input2){if(id.endsWith("_min")){input2=$("#"+id.replace(/_min$/,"_max"));}else if(id.endsWith("_max")){var tmp=input;input=$("#"+id.replace(/_max$/,"_min"));input2=tmp;}}
...
this.setContents(this.getLoadingMessage());},handleNoData:function(pointData,reload){this.jq(ID_PAGE_COUNT).html("");if(!reload){if(debug)console.log("\tno reload");this.addData(pointData);this.checkSearchBar();}else{if(!this.dataCollection)
...
if(data.errorcode=="nodata"||!data.fields){display.handleNoData(new PointData("",[],[]),reload);return;}
if(debug)
