...
widgetStyle="display:none;";for(var i=0;i<filterBy.length;i++){var filterField=this.getFieldById(fields,filterBy[i]);if(!filterField)continue;this.filterFields.push(filterField);var widget;var widgetId=this.getDomId("filterby_"+filterField.getId());var dfltValue=this.getProperty(filterField.getId()+".filterValue","-all-");if(filterField.getType()=="enumeration"){var filterValues=this.getProperty(filterField.getId()+".filterValues");var enums=null;if(filterValues){var toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}}
...
var tuple=[value,label];enumValues.push(tuple);}});enumValues.sort((a,b)=>{return(""+a[1]).localeCompare(""+b[1]);});for(var j=0;j<enumValues.length;j++)
...
HtmlUtils.datePicker("","",["style",widgetStyle,"id",widgetId+"_date2"]);dateIds.push(widgetId+"_date1");dateIds.push(widgetId+"_date2");}else{widget=HtmlUtils.input("",dfltValue,["style",widgetStyle,"id",widgetId,"fieldId",filterField.getId()]);}
...
HtmlUtils.formatTable("#"+this.getDomId("table_counts"),{scrollY:this.getProperty("tableCountsHeight",tableHeight)});if(this.getProperty("showFrequency",true))
HtmlUtils.formatTable("#"+this.getDomId("table_frequency"),{scrollY:this.getProperty("tableFrequenecyHeight",tableHeight),searching:this.getProperty("showSearch",true)});},});}
...
fields=allFields;var summary={};for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);for(var col=0;col<fields.length;col++){var f=fields[col];var value=row[f.getIndex()];if(!Utils.isDefined(summary[f.getId()])){summary[f.getId()]={counts:{},values:[],min:value,max:value,total:0,values:[],numbers:[],field:f}}
var s=summary[f.getId()];if(!s)continue;if(f.isNumeric){s.numbers.push(value);if(isNaN(s.max))s.max=value;else if(!isNaN(value))s.max=Math.max(value,s.max);if(isNaN(s.min))s.min=value;else if(!isNaN(value))s.min=Math.min(value,s.min);}else{if(!Utils.isDefined(s.counts[value])){var tuple={value:value,count:0}
...
html+=HtmlUtils.openTag("div",["class","display-frequency-table","style",this.getProperty("floatTable",false)?"":"display:block;"]);html+=HtmlUtils.openTag("table",["id",this.getDomId("summary"+col),"table-height",this.getProperty("tableHeight","300",true),"class","stripe row-border nowrap ramadda-table"]);html+=HtmlUtils.openTag("thead",[]);var label=HtmlUtils.span(["title","Click to reset","class","display-frequency-label","data-field",s.field.getId()],f.getLabel());html+=HtmlUtils.tr([],HtmlUtils.th([],HtmlUtils.div(["style","max-width:300px;overflow-x:auto;"],label))+HtmlUtils.th(["align","right","width","20%"],HtmlUtils.div(["style","text-align:right"],"Count"))+HtmlUtils.th(["align","right","width","20%"],HtmlUtils.div(["style","text-align:right"],"Percent")));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);if(!f.isNumeric){s.values.sort((a,b)=>{if(a.count<b.count)return 1;if(a.count>b.count)return-1;return 0;});}
for(var i=0;i<s.values.length;i++){var value=s.values[i].value;var label=value;if(label=="")label="&lt;blank&gt;";var count=s.values[i].count;if(count==0)continue;value=HtmlUtils.span(["title","Click to select","class","display-frequency-value","data-field",s.field.getId(),"data-value",value],label);html+=HtmlUtils.tr([],HtmlUtils.td([],value)+
...
var pattern=this.getProperty("pattern");if(pattern&&pattern.length==0)pattern=null;this.writeHtml(ID_TOP_RIGHT,HtmlUtils.input("pattern",(pattern?pattern:""),["placeholder","Search text","id",this.getDomId("search")]));let _this=this;this.jq("search").keypress(function(event){if(event.which==13){_this.setProperty("pattern",$(this).val());_this.updateUI();}});this.writeHtml(ID_DISPLAY_CONTENTS,HtmlUtils.div(["id",this.getDomId(ID_TEXT)],""));this.showText();},showText:function(){let records=this.filterData();if(!records){return null;}
...
this.stopAnimation();});this.btnNext.button().click(()=>{if(this.mode=="sliding"){if(this.end.getTime()<this.dateMax.getTime()-this.window){this.begin=this.end;this.end=new Date(this.end.getTime()+this.window);}}else{if(this.end.getTime()<this.dateMax.getTime()-this.window){this.end=new Date(this.end.getTime()+this.window);}else{return;}}
this.stopAnimation();});this.btnShowAll.button().click(()=>{this.begin=this.dateMin;this.end=this.dateMin;this.inAnimation=false;this.label.html(this.formatAnimationDate(this.dateMin)+" - "+this.formatAnimationDate(this.dateMax));this.running=false;this.btnRun.html(HtmlUtils.getIconImage("fa-play"));this.display.showAllPoints();});},startAnimation:function(justOneStep,back){if(!this.display.points){return;}
