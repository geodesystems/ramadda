...
this.addCell("init cell",props,false).run();this.cells[0].focus();});},initOutputRenderers:function(){let notebook=this;this.outputRenderesr=[];this.addOutputRenderer({shouldRender:(value)=>{return typeof value==="object";},render:(value)=>{if(Array.isArray(value))return HtmlUtils.div(["style"," white-space: pre;"],JSON.stringify(value));return HtmlUtils.div(["style"," white-space: pre;"],JSON.stringify(value,null,2))},});this.addOutputRenderer({shouldRender:(value)=>{return typeof value==="object"&&value.getTime;},render:(value)=>{return notebook.formatDate(value)},});this.addOutputRenderer({shouldRender:(value)=>{var t=typeof value;return t==="string"||t==="number"||t==="boolean";},render:(value)=>{if(typeof value==="string"){if(value.split("\n").length>1){return HtmlUtils.div(["style"," white-space: pre;"],value);}}
...
if(chunk!=""){var div=makeDiv();chunks.push({content:chunk,type:type,div:div,doChunk:doChunk,ok:true});}
doChunk=doRows?doRows[i]:true;chunk=rest;if(chunk!="")chunk+="\n";type=newType;continue;}
chunk=chunk+command+"\n";}
if(chunk!=""){var div=makeDiv();chunks.push({content:chunk,type:type,div:div,doChunk:doChunk,ok:true});}
for(var i=divCnt;i<this.divs.length;i++){this.divs[i].jq().hide();}
this.rawOutput="";for(var i=0;i<chunks.length;i++){var chunk=chunks[i];if(!chunk.doChunk)continue;chunk.div.set("");await this.processChunk(chunk);if(!chunk.ok){return false;}}
...
if(chunk.type=="html"){await this.processHtml(chunk);}else if(chunk.type=="wiki"){await this.processWiki(chunk);}else if(chunk.type=="css"){await this.processCss(chunk);}else if(chunk.type=="fetch"){await this.processFetch(chunk);}else if(chunk.type=="raw"){var content=chunk.content;var re=new RegExp("^ *var *= *(.*)");var toks=re.exec(content);if(toks){content=content.replace(re,"").trim();var id=toks[1];this.notebook.setCellValue(id,content);}
