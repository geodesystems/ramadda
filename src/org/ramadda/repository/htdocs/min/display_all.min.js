...
var index=0;if(this.steps){percent=0;var inc=1/this.steps.length;for(var step=0;step<this.steps.length-1;step++){if(v<=this.steps[step]){index=step;break;}}}else{index=parseInt(percent*this.colors.length);}
...
if(groupByIndex>=0){var groupToTuple={};var groups=[];var agg=[];var title=[];let groupByCount=this.getProperty("groupByCount");title.push(props.groupByField.getLabel());if(groupByCount){title.push(this.getProperty("groupByCountLabel","Count"));}else{for(var j=0;j<fields.length;j++){var field=fields[j];if(field.getIndex()!=groupByIndex){title.push(field.getLabel());}}}
...
var annotationsMap={};if(annotations){let legend="";dataTable.addColumn({type:'string',role:'annotation','p':{'html':true}});dataTable.addColumn({type:'string',role:'annotationText','p':{'html':true}});var toks=annotations.split(",");for(var i=0;i<toks.length;i++){var toks2=toks[i].split(";");if(toks2.length!=3)continue;var index=toks2[0].trim();var label=toks2[1];var desc=toks2[2];let isDate=false;let dateLabel="";if(index.match(/^[0-9]+$/)){index=parseFloat(index);}else{index=Utils.parseDate(index,false);isDate=true;dateLabel=Utils.formatDateYYYYMMDD(index)+": ";}
let annotation={label:label,description:desc};legend+=HtmlUtils.b(label)+":"+desc+" ";annotationsMap[index]=annotation;if(isDate){annotationsMap[Utils.formatDateYYYYMMDD(index)]=annotation;annotationsMap[Utils.formatDateYYYYWeek(index)]=annotation;}}
...
if(annotations){var index=newRow[0];if(index.v)index=index.v;var annotation=annotationsMap[index];if(annotation){annotationsMap[index]=null;}
if(!annotation){annotation=annotationsMap[i];}
if(!annotation&&index.getTime){annotation=annotationsMap[Utils.formatDateYYYYMMDD(index)];if(annotation){annotationsMap[Utils.formatDateYYYYMMDD(index)]=null;}
if(!annotation){annotation=annotationsMap[Utils.formatDateYYYYWeek(index)];if(annotation){annotationsMap[Utils.formatDateYYYYWeek(index)]=null;}}}
