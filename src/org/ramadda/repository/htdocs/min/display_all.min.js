...
return s;},getRecordHtml:function(record,fields){if(!fields){var pointData=this.getData();if(pointData==null)return null;fields=pointData.getRecordFields();}
var showGeo=false;if(Utils.isDefined(this.showGeo)){showGeo=(""+this.showGeo)=="true";}
var template=this.getProperty("recordTemplate");if(template){var row=this.getDataValues(record);return this.getRecordTemplate(row,fields,template);}
...
this.setProperty(prop.property,prop.value);this.updateUI();},handleEventRecordSelection:function(source,args){if(!source.getEntries){return;}
...
this.updateLabels();this.jq(ID_SLIDER).slider({range:true,min:this.dateMin.getTime(),max:this.dateMax.getTime(),values:[this.dateMin.getTime(),this.dateMax.getTime()],slide:function(event,ui){_this.stopAnimation();_this.begin=new Date(ui.values[0]);_this.end=new Date(ui.values[1]);_this.updateLabels();},stop:function(event,ui){_this.stopAnimation();_this.begin=new Date(ui.values[0]);_this.end=new Date(ui.values[1]);_this.applyAnimation(true);}});if(records&&display.getProperty("animationShowTicks",true)){var ticks="";var min=this.dateMin.getTime();var max=this.dateMax.getTime();var p=0;for(var i=0;i<records.length;i++){var date=records[i].getDate().getTime();var perc=Math.round((date-min)/(max-min)*100);var tt=this.formatAnimationDate(records[i].getDate());ticks+=HtmlUtils.div(["class","display-animation-tick","style","left:"+perc+"%;","title",tt,"index",i],"");}
this.jq(ID_SLIDER).append(ticks);this.jq(ID_SLIDER).find(".display-animation-tick").tooltip({content:function(){var record=records[parseFloat($(this).attr('index'))];return _this.display.getRecordHtml(record);return $(this).prop('title');},position:{my:"left top",at:"left bottom+2"},classes:{"ui-tooltip":"ramadda-popup"}});}},makeControls:function(){var buttons="";if(this.display.getProperty("animationShowButtons",true)){buttons+=HtmlUtils.span(["id",this.getDomId(ID_BEGIN),"title","Go to beginning"],HtmlUtils.getIconImage("fa-fast-backward"))+
...
function PointRecord(lat,lon,elevation,time,data){this.isPointRecord=true;RamaddaUtil.defineMembers(this,{latitude:lat,longitude:lon,elevation:elevation,recordTime:time,data:data,toString:function(){return"data:"+data;},getData:function(){return this.data;},allZeros:function(){var tuple=this.getData();var allZeros=false;var nums=0;var nonZero=0;for(var j=0;j<tuple.length;j++){if(typeof tuple[j]=="number"){nums++;if(!isNaN(tuple[j])&&tuple[j]!=0){nonZero++;break;}}}
...
if(template!=""){var props=this.getTemplateProps(fields);var max=parseFloat(this.getProperty("maxNumber",-1));for(var rowIdx=0;rowIdx<selected.length;rowIdx++){if(max!=-1&&rowIdx>=max)break;var row=this.getDataValues(selected[rowIdx]);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);s=this.getRecordTemplate(row,fields,s,props);contents+=s;}}
if(selected.length>0)
contents+=footerTemplate;this.writeHtml(ID_DISPLAY_CONTENTS,contents);},})}
