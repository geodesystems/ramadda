...
if(this.getProperty("doAnimation",false)){var buttons=HtmlUtils.div(["id",this.getDomId(ID_RUN),"xclass","xramadda-button","what","run","title","Run/Stop"],HtmlUtils.getIconImage("fa-play"))+"&nbsp;"+
...
"";buttons=HtmlUtils.div(["xclass","display-map-toolbar"],buttons);this.jq(ID_TOP_LEFT).append(buttons);HtmlUtils.rangeInputInit(this.getDomId(ID_SLIDER));this.run=this.jq(ID_RUN);this.step=this.jq(ID_STEP);this.showAll=this.jq(ID_SHOWALL);this.animation.label=this.jq(ID_ANIMATION_LABEL);this.run.button().click(()=>{this.toggleAnimation();});this.step.button().click(()=>{if(!this.animation.running)
...
this.startAnimation();},startAnimation:function(justOneStep){if(!this.points){return;}
...
this.stepAnimation();},stepAnimation:function(){if(!this.points)return;if(!this.animation.dateMax)return;var oldEnd=this.animation.end;var unit=this.animation.windowUnit;var date=new Date(this.animation.end.getTime()+this.animation.window);if(unit=="decade"){this.animation.end=new Date(date.getUTCFullYear(),0);}else if(unit=="year"){this.animation.end=new Date(date.getUTCFullYear(),0);}else if(unit=="month"){this.animation.end=new Date(date.getUTCFullYear(),date.getMonth());}else if(unit=="day"){this.animation.end=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay());}else if(unit=="hour"){this.animation.end=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours());}else if(unit=="minute"){this.animation.end=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getMinutes());}else{this.animation.end=new Date(this.animation.end.getTime()+this.animation.window);}
if(this.animation.mode=="sliding"){this.animation.begin=oldEnd;}
var windowStart=this.animation.begin.getTime();var windowEnd=this.animation.end.getTime();var atLoc={};for(var i=0;i<this.lines.length;i++){var line=this.lines[i];if(line.date<windowStart||line.date>windowEnd){line.style.display='none';continue;}
...
this.map.lines.redraw();if(windowEnd<this.animation.dateMax.getTime()){this.animation.label.html(this.formatAnimationDate(this.animation.begin)+" - "+this.formatAnimationDate(this.animation.end));if(this.animation.running){setTimeout(()=>this.stepAnimation(),this.animation.speed);}}else{this.animation.running=false;this.animation.label.html("");this.animation.inAnimation=false;this.animation.label.html("");this.run.html(HtmlUtils.getIconImage("fa-play"));}
