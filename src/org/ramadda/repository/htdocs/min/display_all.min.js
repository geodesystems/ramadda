...
return;}
node.parent=parentNode;parentNode.children.push(node);});return roots;},filterData:function(dataList,fields,doGroup,skipFirst){var startDate=this.getProperty("startDate");var endDate=this.getProperty("endDate");if(startDate){this.startDateObject=new Date(startDate);}
...
html+=HtmlUtils.openDiv(["class","display-contents","style",style]);var get=this.getGet();var button="";if(this.getShowMenu()){button=HtmlUtils.onClick(get+".showDialog();",HtmlUtils.image(ramaddaBaseUrl+"/icons/downdart.png",[ATTR_CLASS,"display-dialog-button",ATTR_ID,this.getDomId(ID_MENU_BUTTON)]));}
...
this.dataCollection.addData(pointData);var entry=pointData.entry;if(entry==null){await this.getRamadda().getEntry(pointData.entryId,e=>{entry=e});}
if(entry){pointData.entry=entry;this.addEntry(entry);}},pointDataLoadFailed:function(data){this.inError=true;errorMessage=this.getProperty("errorMessage",null);if(errorMessage!=null){this.setContents(errorMessage);return;}
...
this.setContents(this.getMessage(msg));},clearCache:function(){},pointDataLoaded:function(pointData,url,reload){this.inError=false;this.clearCache();if(!reload){this.addData(pointData);this.checkSearchBar();}
...
var minDate=(this.minDateObj!=null?this.minDateObj.getTime():-1);var maxDate=(this.maxDateObj!=null?this.maxDateObj.getTime():-1);if(this.minDateObj!=null||this.maxDateObj!=null){}
...
return result;},pageHasLoaded:function(display){for(var i=0;i<this.displays.length;i++){this.displays[i].setDisplayReady(true);}
this.doLayout();},addDisplay:function(display){this.displays.push(display);if(Utils.getPageLoaded()){this.doLayout();}},layoutChanged:function(display){this.doLayout();},removeDisplay:function(display){var index=this.displays.indexOf(display);if(index>=0){this.displays.splice(index,1);}
this.doLayout();},doLayout:function(){var html="";var colCnt=100;var displaysToLayout=this.getDisplaysToLayout();var displaysToPrepare=this.displays;for(var i=0;i<displaysToPrepare.length;i++){var display=displaysToPrepare[i];if(display.prepareToLayout!=null){display.prepareToLayout();}}
var weightIdx=0;var weights=null;if(typeof this.weights!="undefined"){weights=this.weights.split(",");}
...
this.initDisplays();},initDisplays:function(){for(var i=0;i<this.displays.length;i++){try{this.displays[i].initDisplay();}catch(e){this.displays[i].displayError("Error creating display:<br>"+e);console.log("error creating display: "+this.displays[i].getType());console.log(e.stack)}}},displayData:function(){},setLayout:function(layout,columns){this.layout=layout;if(columns){this.columns=columns;}
...
function convertToPointData(array){var fields=[];var records=[];var header=array[0];var samples=array[1];for(var i=0;i<header.length;i++){var label=header[i];var id=label.toLowerCase().replace(/ /g,"_");var sample=samples[i];var tof=typeof sample;var type;if(tof=="string")
type="string";else if(tof=="number")
type="double";else if(sample.getTime)
type="date";else
console.log("Unknown type:"+tof);fields.push(new RecordField({id:id,label:label,type:type,chartable:true}));}
for(var i=1;array.length;i++){records.push(new PointRecord(NaN,NaN,NaN,null,array[i]));}
...
function PointRecord(lat,lon,elevation,time,data){this.isPointRecord=true;RamaddaUtil.defineMembers(this,{latitude:lat,longitude:lon,elevation:elevation,recordTime:time,data:data,id:HtmlUtils.getUniqueId(),toString:function(){return"data:"+data;},getId:function(){return this.id;},getData:function(){return this.data;},allZeros:function(){var tuple=this.getData();var allZeros=false;var nums=0;var nonZero=0;for(var j=0;j<tuple.length;j++){if(typeof tuple[j]=="number"){nums++;if(!isNaN(tuple[j])&&tuple[j]!=0){nonZero++;break;}}}
...
values.push(value);indexName=indexField.getLabel();}else{if(this.hasDate){values.push(this.getDateValue(date,date_formatter));indexName="Date";}else{values.push(rowIdx);indexName="Index";}}
...
newList.push(newRow);}
dataList=newList;}
try{this.makeChart(dataList,props,selectedFields);}catch(e){console.log(e.stack);this.displayError(""+e);return;}
var d=_this.jq(ID_CHART);if(_this.jq(ID_CHART).is(':visible')){this.lastWidth=d.width();}else{this.lastWidth=-1;}
...
chartOptions.orientation="horizontal";return chartOptions;},doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){return new google.visualization.BarChart(document.getElementById(this.getChartId()));}});}
...
style+="height:"+(-height)+"%;";else
style+="height:"+height+";";}else{style+="height:"+"100%;";}
divAttrs.push(style);return HtmlUtils.div(divAttrs,"");},doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){chartOptions.tooltip={textStyle:{color:'#000000'},showColorCode:true};if(this.getProperty("bins",null)){chartOptions.title="Bins: "+this.getDataValues(dataList[0])[1];}else{chartOptions.title=this.getDataValues(dataList[0])[0]+" - "+this.getDataValues(dataList[0])[1];}
...
for(var binIdx=0;binIdx<bins;binIdx++){var bin=binList[binIdx];list.push(["Bin:"+this.formatNumber(bin.min)+"-"+this.formatNumber(bin.max),bin.values.length]);}}else{for(var i=1;i<dataList.length;i++){var tuple=this.getDataValues(dataList[i]);var s=""+(tuple.length==1?"#"+i:tuple[0]);var v=tuple.length==1?tuple[0]:tuple[1];list.push([s,v]);}}
...
for(var valueIdx=0;valueIdx<values.length;valueIdx++){var value=values[valueIdx].trim();if(value.length<=1)continue;if(value.startsWith("&"))continue;var _value=value.toLowerCase();if(cnt){cnt.count++;cnt.total+=value.length;if(!Utils.isDefined(cnt.lengths[value.length]))
...
var imgAttrs=["class","display-cards-popup","data-fancybox",this.getDomId("gallery"),"data-caption",caption];if(img)img=img.trim();if(img!=""){if(this.colorAnalysisEnabled)
...
var html="";for(var col=0;col<fields.length;col++){var f=fields[col];var s=summary[f.getId()];if(f.isNumeric()){var numBins=parseFloat(this.getProperty("numBins",10,true));s.bins=[];var range=s.max-s.min;var binWidth=(s.max-s.min)/numBins;var label="Not defined";s.bins.push(label);var tuple={value:label,count:0}
...
var hor=this.getProperty("orientation","")!="vertical";if(this.getProperty("floatTable")!=null){hor=this.getProperty("floatTable")==true;}
...
for(var i=0;i<s.values.length;i++){var value=s.values[i].value;var label=value;if(label=="")label="&lt;blank&gt;";var count=s.values[i].count;if(count==0)continue;value=value.replace(/\'/g,"&apos;");value=HtmlUtils.span(["title","Click to select","class","display-frequency-value","data-field",s.field.getId(),"data-value",value],label);var tdv=HtmlUtils.td([],value);var tdc=(showCount?HtmlUtils.td(["align","right"],count):"");var perc=count/s.total;var tdp=showPercent?HtmlUtils.td(["align","right"],s.total==0?"0":Math.round(perc*100)+"%"):"";var color=s.values[i].color;if(!color)color=dfltColor;var bw=perc/maxPercent;var tdb=showBars?HtmlUtils.td(["valign","center","width",barWidth],HtmlUtils.div(["title",Math.round(perc*100)+"%","style","background:"+color+";height:10px;width:"+(Math.round(bw*barWidth))+"px"],"")):"";html+=HtmlUtils.tr([],tdv+tdc+tdp+tdb);}
html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");html+=HtmlUtils.closeTag("div");}
this.writeHtml(ID_DISPLAY_CONTENTS,html);let _this=this;this.jq(ID_DISPLAY_CONTENTS).find(".display-frequency-value").click(function(){var click=_this.getProperty("clickFunction")
...
if(props.dummy)return display;this.addDisplay(display);return display;},pageHasLoaded:function(display){this.getLayoutManager().pageHasLoaded();},addDisplay:function(display){display.setDisplayManager(this);display.loadInitialData();this.getLayoutManager().addDisplay(display);},notifyEvent:function(func,source,data){this.getLayoutManager().notifyEvent(func,source,data);},removeDisplay:function(display){this.getLayoutManager().removeDisplay(display);this.notifyEvent("handleEventRemoveDisplay",this,display);},handleEntryMenu:async function(entryId){await getGlobalRamadda().getEntry(entryId,e=>{var displays=this.getLayoutManager().getDisplays();for(var i=0;i<displays.length;i++){var display=displays[i];display.setEntry(e);}});}});addDisplayManager(this);var displaysHtml=HtmlUtils.div([ATTR_ID,this.getDomId(ID_DISPLAYS),ATTR_CLASS,"display-container"]);var html=HtmlUtils.openTag(TAG_DIV);html+=HtmlUtils.div(["id",this.getDomId(ID_MENU_CONTAINER)]);if(argProperties.entryCollection){let entries=argProperties.entryCollection.split(",");let enums=[];entries.map(t=>{var toks=t.split(":");enums.push([toks[0],toks[1]]);});html+=(argProperties.changeEntriesLabel||"Change Entry: ")+HtmlUtils.select("",[ATTR_ID,this.getDomId(ID_ENTRIES_MENU)],enums);}
if(this.getShowMenu()){html+=HtmlUtils.tag(TAG_A,[ATTR_CLASS,"display-menu-button",ATTR_ID,this.getDomId(ID_MENU_BUTTON)],"&nbsp;");}
...
this.jq(ID_ENTRIES_MENU).change(e=>{var entry=this.jq(ID_ENTRIES_MENU).val();this.handleEntryMenu(entry);});if(this.showmap){this.createDisplay('map');}
...
this.addPoints(records,fields,points);this.addLabels(records,fields,points);this.applyVectorMap();},addPoints:function(records,fields,points){var cidx=0
...
var dontAddPoint=this.doDisplayMap();var didColorBy=false;var seen={};var lastPoint;var pathAttrs={strokeColor:this.getProperty("pathColor",lineColor),strokeWidth:this.getProperty("pathWidth",1)};for(var i=0;i<records.length;i++){var record=records[i];var tuple=record.getData();if(!record.point)
...
var tooltip=this.getProperty("tooltip");var highlight=this.getProperty("highlight");var html=this.getRecordHtml(record,fields,tooltip);if(polygonField){var s=values[polygonField.getIndex()];var delimiter;[";",","].map(d=>{if(s.indexOf(d)>=0)delimiter=d;});var toks=s.split(delimiter);var polygonProps={};$.extend(polygonProps,props);if(polygonProps.strokeWidth==0)
...
var isPath=this.getProperty("isPath",false);var showSegments=this.getProperty("showSegments",false);if(showSegments&&latField1&&latField2&&lonField1&&lonField2){var lat1=values[latField1.getIndex()];var lat2=values[latField2.getIndex()];var lon1=values[lonField1.getIndex()];var lon2=values[lonField2.getIndex()];var attrs={};if(props.fillColor)
...
if(showPoints){if(!Utils.isDefined(seen[point]))seen[point]=0;if(seen[point]>500)continue;seen[point]++;var mapPoint=null;if(iconField){var icon=tuple[iconField.getIndex()];if(iconMap){icon=iconMap[icon];if(!icon)icon=this.getMarkerIcon();}
...
props.graphicName=this.getProperty("shape","circle");if(radius>0)
