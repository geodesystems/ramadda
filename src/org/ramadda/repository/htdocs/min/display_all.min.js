...
return macros.apply(attrs);},getRecordHtml:function(record,fields,template){return record.id+" "+record.getLatitude()+" "+record.getLongitude();if(!fields){var pointData=this.getData();if(pointData==null){return null;}
...
return true;},getDataFilters:function(v){return DataUtils.getDataFilters(this,v||this.getProperty("dataFilters"));},filterData:function(records,fields,doGroup,skipFirst){var startDate=this.getProperty("startDate");var endDate=this.getProperty("endDate");if(startDate){this.startDateObject=Utils.createDate(startDate);}
...
records=newData;var stride=parseInt(this.getProperty("stride",-1));if(stride<0){var maxSize=parseInt(this.getProperty("maxDisplayedPoints",-1));if(maxSize>0&&records.length>0){stride=1;while(records.length/stride>maxSize){stride++;}}}
...
this.checkSearchBar();this.updateUI();},getFilterId:function(id){return this.getDomId("filterby_"+id);},getHeader2:function(){return"";},initHeader2:function(){},checkSearchBar:function(){let _this=this;this.filterFields=[];this.colorByFields=this.getFieldsByIds(null,this.getProperty("colorByFields","",true));this.sizeByFields=this.getFieldsByIds(null,this.getProperty("sizeByFields","",true));let pointData=this.getData();if(pointData==null)return;let fields=pointData.getRecordFields();let records=pointData.getRecords();let header2=HtmlUtils.div(["style","display:inline-block;","id",this.getDomId(ID_HEADER2_PREFIX)]);header2+=this.getHeader2();if(this.getProperty("legendFields")||this.getProperty("showFieldLegend",false)){let colors=this.getColorList();let fields=this.getFieldsByIds(null,this.getProperty("legendFields",this.getProperty("fields",this.getProperty("sumFields"))));let html="";let colorCnt=0;fields.map((f)=>{if(colorCnt>=colors.length)colorCnt=0;let color=colors[colorCnt];html+=HtmlUtils.div(["style","display: inline-block;width:8px;height:8px;background:"+color+";"])+" "+f.getLabel()+"&nbsp;&nbsp; ";colorCnt++;});header2+=html;}
...
if(this.sizeByFields.length>0){let enums=[];this.sizeByFields.map(field=>{enums.push([field.getId(),field.getLabel()]);});header2+=HtmlUtils.span(["class","display-filterby"],HtmlUtils.span(["class","display-filterby-label"],"Size by: ")+HtmlUtils.select("",["style","","id",this.getDomId("sizebyselect")],enums,this.getProperty("sizeBy","")))+"&nbsp;";}
let dataFilterIds=[];this.getDataFilters().map(f=>{if(!f.label)return;let cbxid=this.getDomId("datafilterenabled_"+f.id);dataFilterIds.push(cbxid);header2+=HtmlUtils.checkbox("",["id",cbxid],f.enabled)+" "+f.label+"&nbsp;&nbsp;"});if(this.getProperty("filterDate")){let type=this.getProperty("filterDate");let enums=[];if(this.getProperty("filterDateIncudeAll")){enums.push(["all","All"]);}
...
style+="display:none;";let selectId=this.getFilterId("filterDate");header2+=HtmlUtils.span(["class","display-filterby","style",style],HtmlUtils.span(["class","display-filterby-label"],"Select "+label+":")+HtmlUtils.select("",["fieldId","filterDate","style","","id",selectId],enums,selected))+"&nbsp;";}
...
header2+=HtmlUtils.div(["style","display:inline-block;","id",this.getDomId(ID_HEADER2_SUFFIX)]);this.jq(ID_HEADER2).html(header2);this.initHeader2();var theDisplay=this;let inputFunc=function(input,input2,value){var id=input.attr("id");if(!input2){if(id.endsWith("_min")){input2=$("#"+id.replace(/_min$/,"_max"));}else if(id.endsWith("_max")){var tmp=input;input=$("#"+id.replace(/_max$/,"_min"));input2=tmp;}}
...
this.jq(ID_FILTERBAR).find(".display-filterby-items").each(function(){let parent=$(this);$(this).find(".display-filterby-item").click(function(event){var isAll=$(this).hasClass("display-filterby-item-all");var selectClazz="display-filterby-item-"+displayType+"-selected"
...
for(a in fields)
console.log("   "+fields[a].getId());},makeIndexValue:function(indexField,value,offset){return value+offset;},getStandardData:function(fields,args){let showUnit=this.getProperty("showUnit",true);this.recordToIndex={};this.indexToRecord={};var pointData=this.getPointData();var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);if(fields==null){fields=pointData.getRecordFields();}
...
return min;},}
var RecordUtil={expandBounds:function(bounds,perc){return{east:bounds.east+(bounds.east-bounds.west)*perc,west:bounds.west-(bounds.east-bounds.west)*perc,north:bounds.north+(bounds.north-bounds.south)*perc,south:bounds.south-(bounds.north-bounds.south)*perc,}},convertBounds:function(bounds){if(Utils.isDefined(bounds.top))
...
ctx.stroke();else
ctx.fill();}else{if(opts.cell3D){let height=perc*20;height=opts.cellSizeH||opts.cellSize;ctx.strokeStyle="#000";ctx.strokeStyle="rgba(0,0,0,0)"
...
ctx.fillRect(crx,cry,opts.cellSizeX,opts.cellSizeY);ctx.strokeStyle="black";ctx.font="8px arial"
ctx.fillStyle="black";}}
...
$.extend(opts,args);let id=HtmlUtils.getUniqueId();$(document.body).append('<canvas style="display:none;" id="'+id+'" width="'+opts.w+'" height="'+opts.h+'"></canvas>');let canvas=document.getElementById(id);var ctx=canvas.getContext("2d");let cnt=0;let ew=args.bounds.east-args.bounds.west;let eh=args.bounds.north-args.bounds.south;let halfW=opts.cellSizeX/2;let halfH=opts.cellSizeY/2;ctx.font=opts.cellFont||"8pt Arial;"
var gradient=ctx.createLinearGradient(0,0,0,canvas.height);gradient.addColorStop(0,'white');gradient.addColorStop(1,'red');ctx.lineStyle="#000";if(opts.doHeatmap){let cols=Math.floor(opts.w/opts.cellSizeX);let rows=Math.floor(opts.h/opts.cellSizeY);let points=[];records.map((record,idx)=>{let lat=record.getLatitude();let lon=record.getLongitude();let x=Math.floor(opts.w*(lon-args.bounds.west)/ew);let y=opts.h-(Math.floor(opts.h*(lat-args.bounds.south)/eh));record[gridId+"_coordinates"]={x:x,y:y};let v=idx;if(opts.colorBy&&opts.colorBy.index>=0){v=record.getValue(opts.colorBy.index);}
x=Math.floor(x/opts.cellSizeX);y=Math.floor(y/opts.cellSizeY);if(x<0)x=0;if(y<0)y=0;if(x>=cols)x=cols-1;if(y>=rows)y=rows-1;points.push({x:x,y:y,v:v,r:record});});let grid=RecordUtil.gridPoints(rows,cols,points,args);opts.cellSizeX=+opts.cellSizeX;opts.cellSizeY=+opts.cellSizeY;if(opts.filter){let copy=this.cloneGrid(grid,v=>v.v);let filtered=opts.filter=="average"?this.averageGrid(copy):opts.filter=="gauss"?this.gaussGrid(copy):null;for(var rowIdx=0;rowIdx<rows;rowIdx++){let row=grid[rowIdx];for(var colIdx=0;colIdx<cols;colIdx++){let cell=row[colIdx];cell.v=filtered[rowIdx][colIdx];}}}
let mm=this.getMinMaxGrid(grid,v=>v.v);if(opts.colorBy){opts.colorBy.setRange(mm.min,mm.max);opts.colorBy.index=0;}
for(var rowIdx=0;rowIdx<rows;rowIdx++){let row=grid[rowIdx];for(var colIdx=0;colIdx<cols;colIdx++){let cell=row[colIdx];let v=cell.v;if(isNaN(v))continue;let x=colIdx*opts.cellSizeX;let y=rowIdx*opts.cellSizeY;RecordUtil.drawGridCell(opts,canvas,ctx,x,y,cell.v,colIdx,rowIdx);}}}else{records.map((record,idx)=>{let lat=record.getLatitude();let lon=record.getLongitude();let x=Math.round(opts.w*(lon-opts.bounds.west)/ew);let y=opts.h-(Math.round(opts.h*(lat-opts.bounds.south)/eh));record[gridId+"_coordinates"]={x:x,y:y};let v=opts.colorBy?record.getData()[opts.colorBy.index]:null;if(false&&opts.forMercator){var[tx,ty]=RecordUtil.convertGeoToPixel(lat,lon,opts.bounds,opts.w,opts.h);x=tx;y=ty;}
...
if(true||this.getProperty("showMarkerToggle")){html+=HtmlUtils.checkbox("",["id",this.getDomId("showMarkerToggle")],true)+" "+
...
let bounds={};RecordUtil.getPoints(records,bounds);let ratio=(bounds.east-bounds.west)/(bounds.north-bounds.south);let style=this.getProperty("padding")?"padding:"+this.getProperty("padding")+"px;":"";let html=HtmlUtils.div(["id",this.getDomId("inner"),"style","width:100%;height:100%;"+style]);this.writeHtml(ID_DISPLAY_CONTENTS,html);let pad=10;let w=Math.round(this.jq("inner").width());let h=Math.round(w/ratio);var divid=this.getProperty(PROP_DIVID);$("#"+divid).css("height",h+pad);html=HtmlUtils.div(["id",this.getDomId("inner"),"style","width:"+w+";height:"+h+"px;"+style]);this.jq(ID_DISPLAY_CONTENTS).css("height",h+pad);this.writeHtml(ID_DISPLAY_CONTENTS,html);var colorBy=this.getColorByInfo(records);bounds=RecordUtil.expandBounds(bounds,0.1);let args=$.extend({colorBy:colorBy,w:w,h:h,cell3D:this.getProperty("cell3D"),bounds:bounds},this.getDefaultGridByArgs());let img=RecordUtil.gridData(this.getId(),records,args);this.jq("inner").html(HtmlUtils.image(img,["title","","id",this.getDomId("image")]));this.jq("inner").append(HtmlUtils.div(["id",this.getDomId("tooltip"),"style","z-index:2000;display:none;position:absolute;background:#fff;border:1px solid #ccc;padding:5px;"]));let _this=this;if(this.getProperty("showTooltips",true)){this.jq("image").mouseout(function(event){_this.jq("tooltip").hide();});this.jq("image").mousemove(function(event){let closest=_this.findClosest(records,event);if(closest){let html=HtmlUtils.div(["style","max-height:400px;overflow-y:auto;"],_this.getRecordHtml(closest));_this.jq("tooltip").html(html);_this.jq("tooltip").show();}});}
