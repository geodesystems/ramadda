...
if(template=="${fields}"){fields=this.getFieldsByIds(null,this.getProperty("fields"));}
...
var value=record.getValue(field.getIndex());if(typeof value=="number"){value=this.formatNumber(value);}
...
ids=ids.split(",");if(!fields){var pointData=this.getData();if(pointData==null)return null;fields=pointData.getRecordFields();}
...
return true;});this.writeHeader(ID_REQUEST_PROPERTIES,requestProps);let macroChange=(macro,value,what)=>{if(this.settingMacroValue)return;if(macro.triggerReload){this.macroChanged();this.reloadData();}
...
return HtmlUtils.span(attrs,label);},writeHeader:function(header,html){if(html==""){this.jq(header).css("display","none");}else{this.jq(header).css("display","inline-block");}
this.jq(header).html(html);},checkSearchBar:function(){let _this=this;this.filterFields=[];this.colorByFields=this.getFieldsByIds(null,this.getProperty("colorByFields","",true));this.sizeByFields=this.getFieldsByIds(null,this.getProperty("sizeByFields","",true));let pointData=this.getData();if(pointData==null)return;let fields=pointData.getRecordFields();let records=pointData.getRecords();let header2="";if(this.getProperty("showProgress",false)){header2+=HtmlUtils.div(["id",this.getDomId(ID_DISPLAY_PROGRESS),"style","display:inline-block;margin-right:4px;min-width:20px;"]);}
header2+=HtmlUtils.div(["id",this.getDomId(ID_HEADER2_PREFIX),"class","display-header-span"],"");header2+=this.getHeader2();if(this.getProperty("pageRequest",false)){header2+=HtmlUtils.span(["id",this.getDomId(ID_PAGE_COUNT)]);}
header2+=HtmlUtils.div(["id",this.getDomId(ID_REQUEST_PROPERTIES),"class","display-header-span"],"");if(this.getProperty("legendFields")||this.getProperty("showFieldLegend",false)){let colors=this.getColorList();let fields=this.getFieldsByIds(null,this.getProperty("legendFields",this.getProperty("fields",this.getProperty("sumFields"))));let html="";let colorCnt=0;fields.map((f)=>{if(colorCnt>=colors.length)colorCnt=0;let color=colors[colorCnt];html+=HtmlUtils.div(["style","display: inline-block;width:8px;height:8px;background:"+color+";"])+" "+f.getLabel()+"&nbsp;&nbsp; ";colorCnt++;});header2+=html;}
...
this.charts=[];this.chartCount=-1;if(this.getProperty("doMultiCharts",this.getProperty("multipleCharts",false))){let multiField=this.getFieldById(null,this.getProperty("multiField"));let labelPosition=this.getProperty("multiChartsLabelPosition","bottom");let map={};let groups=[];let tmp=[];dataList.map((v,idx)=>{if(idx>0)tmp.push(v)});if(!multiField)
tmp.sort(function(a,b){var v1=a.record?a.record.getDate():a.date;var v2=b.record?b.record.getDate():b.date;return v1.getTime()-v2.getTime();});dataList=Utils.mergeLists([dataList[0]],tmp);dataList.map((v,idx)=>{if(idx==0)return;var record=v.record;var groupValue=record?multiField?record.getValue(multiField.getIndex()):record.getDate():v.date;let list=null;list=map[groupValue];if(!list){list=[];map[groupValue]=list;groups.push(groupValue);}
list.push(v);})
this.jq(ID_CHARTS).html(HtmlUtils.div(["id",this.getDomId(ID_CHARTS_INNER),"style","text-align:center;"]));let multiStyle=this.getProperty("multiStyle","");let multiLabelTemplate=this.getProperty("multiLabelTemplate","${value}");groups.map((groupValue,idx)=>{this.chartCount=idx;let tmpDataList=[];let list=map[groupValue];tmpDataList.push(dataList[0]);tmpDataList=Utils.mergeLists(tmpDataList,list);var innerId=this.getDomId(ID_CHART)+"_"+this.chartCount;var label=groupValue;if(groupValue.getTime)label=this.formatDate(groupValue);label=multiLabelTemplate.replace("${value}",label);var header=HtmlUtils.div(["class","display-multi-header"],label);var top=labelPosition=="top"?header:"";var bottom=labelPosition=="bottom"?header:"";var div=HtmlUtils.div(["class","display-multi-div","style","display:inline-block;"+multiStyle],top+this.getChartDiv(innerId)+bottom);this.jq(ID_CHARTS_INNER).append(div);let chart=this.makeGoogleChartInner(tmpDataList,innerId,props,selectedFields);if(chart)this.charts.push(chart);});}else{this.jq(ID_CHARTS).append(this.getChartDiv(this.getDomId(ID_CHART)));let chart=this.makeGoogleChartInner(dataList,this.getDomId(ID_CHART),props,selectedFields);if(chart)this.charts.push(chart);}},makeGoogleChartInner:function(dataList,chartId,props,selectedFields){let chartDiv=document.getElementById(chartId);let chart=this.doMakeGoogleChart(dataList,props,chartDiv,selectedFields,this.chartOptions);if(chart==null)return null;var dataTable=this.makeDataTable(dataList,props,selectedFields);if(!dataTable){this.setContents(this.getMessage("No data available"));return null;}
...
console.log("r:"+records.length);let msg=this.getProperty("loadingMessage","Loading map...");if(msg!="")
