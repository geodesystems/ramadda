...
if(this.getProperty("dataFilter")){let exclude=[];this.getProperty("dataFilter").split(";").map(tok=>{[type,fieldId,value]=tok.split(",");if(type=="match"||type=="notmatch")
...
console.log("row: "+i+"  "+s);}},mapCharts:function(func){if(this.charts!=null){this.charts.map(chart=>{func(chart);});}},clearChart:function(){this.mapCharts(chart=>{if(chart.clearChart){chart.clearChart();}});},setChartSelection:function(index){this.mapCharts(chart=>{if(chart.setSelection){chart.setSelection([{row:index,column:null}]);}});},tableHeaderMouseover:function(i,tooltip){},getAddToolTip:function(){return true;},getAddStyle:function(){return true;},getFormatNumbers:function(){return false;},makeDataTable:function(dataList,props,selectedFields){let addTooltip=this.getAddToolTip();let addStyle=this.getAddStyle();let formatNumbers=this.getFormatNumbers();if(dataList.length==1){return google.visualization.arrayToDataTable(this.makeDataArray(dataList));}
...
if(addStyle)
dataTable.addColumn({type:'string',role:'style'});if(addTooltip)
dataTable.addColumn({type:'string',role:'tooltip','p':{'html':true}});if(j>0&&fixedValueS){break;}}}
let debug=false;let debugRows=3;if(debug){for(var i=0;i<dataTable.getNumberOfColumns();i++)
...
if(j==0&&props.includeIndex){}else{if(addStyle){newRow.push(color);if(debug&&rowIdx<debugRows)
