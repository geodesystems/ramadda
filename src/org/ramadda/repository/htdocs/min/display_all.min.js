...
widgetStyle="display:none;";for(var i=0;i<filterBy.length;i++){var filterField=this.getFieldById(fields,filterBy[i]);if(!filterField)continue;this.filterFields.push(filterField);var widget;var widgetId=this.getDomId("filterby_"+filterField.getId());var dfltValue=this.getProperty(filterField.getId()+".filterValue","");if(filterField.getType()=="enumeration"){var filterValues=this.getProperty(filterField.getId()+".filterValues");var enums=null;if(filterValues){var toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}}
if(enums==null){var includeAll=this.getProperty(filterField.getId()+".includeAll",true);enums=includeAll?[["","All"]]:[];var enumValues=[];var seen={};records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(!seen[value]){seen[value]=true;var label=value;if(label.length>20){label=label.substring(0,19)+"...";}
...
var label=this.getProperty(filterField.getId()+".filterLabel",filterField.getLabel());if(!hideFilterWidget)
widget=HtmlUtils.div(["style","display:inline-block;"],HtmlUtils.span(["class","display-fitlerby-label"],label)+": "+widget);searchBar+=widget+(hideFilterWidget?"":"&nbsp;&nbsp;");}
...
fields=allFields;var summary={};for(var rowIdx=0;rowIdx<records.length;rowIdx++){var row=this.getDataValues(records[rowIdx]);for(var col=0;col<fields.length;col++){var f=fields[col];var value=row[f.getIndex()];if(!Utils.isDefined(summary[f.getId()])){summary[f.getId()]={counts:{},values:[],min:value,max:value,values:[],numbers:[],}}
var s=summary[f.getId()];if(f.isNumeric){s.numbers.push(value);if(isNaN(s.max))s.max=value;else if(!isNaN(value))s.max=Math.max(value,s.max);if(isNaN(s.min))s.min=value;else if(!isNaN(value))s.min=Math.min(value,s.min);}else{if(!Utils.isDefined(s.counts[value])){var tuple={value:value,count:0}
...
html+=HtmlUtils.openTag("div",["class","display-frequency-table"]);html+=HtmlUtils.openTag("table",["id",this.getDomId("summary"+col),"table-height",this.getProperty("tableHeight","300",true),"class","stripe row-border nowrap ramadda-table"]);html+=HtmlUtils.openTag("thead",[]);html+=HtmlUtils.tr([],HtmlUtils.th([],f.getLabel())+HtmlUtils.th(["align","right"],"Count"));html+=HtmlUtils.closeTag("thead");html+=HtmlUtils.openTag("tbody",[]);if(!f.isNumeric){s.values.sort((a,b)=>{if(a.count<b.count)return 1;if(a.count>b.count)return-1;return 0;});}
for(var i=0;i<s.values.length;i++){var value=s.values[i].value;var count=s.values[i].count;if(count==0)continue;html+=HtmlUtils.tr([],HtmlUtils.td([],value)+
HtmlUtils.td(["align","right"],count));}
html+=HtmlUtils.closeTag("tbody");html+=HtmlUtils.closeTag("table");html+=HtmlUtils.closeTag("div");}
this.writeHtml(ID_DISPLAY_CONTENTS,html);for(var col=0;col<fields.length;col++){HtmlUtils.formatTable("#"+this.getDomId("summary"+col),{});}}});}
