...
let names={};let nameList=[];let links={};let maxWeight=0;let sortBy=this.getProperty("sortBy","name");let directed=this.getProperty("directed",true);records.map(r=>{var source=r.getValue(sourceField.getIndex());var target=r.getValue(targetField.getIndex());var weight=-999;if(weightField){weight=r.getValue(weightField.getIndex());maxWeight=Math.max(maxWeight,weight);}
...
var tmp=[];nameList=nameList.map(t=>{if(!seen[t.name]){seen[t.name]=true;tmp.push(t.name);}});nameList=tmp;let table="<div style='margin-top:"+this.getProperty("topSpace","100px")+";'></div><table style='height:100%;' class='display-cooc-table' order=0 cellpadding=0 cellspacing=0  >"
table+="<tr valign=bottom><td style='border:none;'></td>";nameList.map(target=>{target=target.replace(/ /g,"&nbsp;").replace(/-/g,"&nbsp;");table+=HtmlUtils.td(["style","border:none;","width","6"],HtmlUtils.div(["class","display-cooc-colheader"],target));});var colors=this.getColorTable();if(!colors)colors=Utils.getColorTable("blues",true);var missingBackground=this.getProperty("missingBackground","#eee");nameList.map(source=>{var label=source.replace(/ /g,"&nbsp;");table+="<tr valign=bottom ><td style='   border:none;' align=right>"+HtmlUtils.div(["class","display-cooc-rowheader"],label)+"</td>";nameList.map(target=>{var weight=links[source+"--"+target];if(!directed&&!Utils.isDefined(weight))
weight=links[target+"--"+source];var style="";if(weight){if(weight==-999||maxWeight==0)
style="background:#ccc;";else{var percent=weight/maxWeight;var index=parseInt(percent*colors.length);if(index>=colors.length)index=colors.length-1;style="background:"+colors[index]+";";}}else{style="background:"+missingBackground+";";}
table+=HtmlUtils.td(["title",source+" -> "+target+(weight>0?" "+weight:""),"width","3"],HtmlUtils.div(["class","display-cooc-cell","style",style+"height:100%;"],"&nbsp;"));});table+="</tr>";});table+="</tr>";table+="</table>";this.jq(ID_TABLE).html(table);}})}
