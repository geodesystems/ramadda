...
return source.getProperty(prop,dflt);return null;},getProperty:function(key,dflt,skipThis){var value=this.getPropertyInner(key,null,skipThis);if(!Utils.isDefined(value))return dflt;return value;},getPropertyInner:function(key,dflt,skipThis){if(!skipThis&&Utils.isDefined(this[key])){return this[key];}
var value=this.properties[key];if(value!=null){return value;}
...
if(this.displayParent!=null){return this.displayParent.getPropertyInner(key,skipThis);}
if(this.getDisplayManager){return this.getDisplayManager().getPropertyInner(key);}
value=getGlobalDisplayProperty(key);if(value){return value;}
return dflt;},});}
...
let roots=[];let idToNode={};let nodes=[];let idToRoot={};var labelField=this.getFieldById(null,this.getProperty("labelField"));var nodeFields=this.getFieldsByIds(null,this.getProperty("nodeFields"));let treeRootLabel=this.getProperty("treeRoot");let treeRoot=null;if(treeRootLabel){treeRoot={id:treeRootLabel,label:treeRootLabel,children:[],parent:null};roots.push(treeRoot);}
...
records.map(r=>{var parent=r.getValue(parentField.getIndex());var id=r.getValue(idField.getIndex());var label=labelField==null?id:r.getValue(labelField.getIndex());var node={id:id,label:label,children:[],record:r,parentId:parent,parent:null};idToNode[id]=node;nodes.push(node);if(parent==""){idToRoot[id]=node;if(treeRoot){node.parent=treeRoot;node.parentId=treeRoot.id;treeRoot.children.push(node);}else{roots.push(node);}}});nodes.map(node=>{let parentNode=idToNode[node.parentId];if(!parentNode){if(!idToRoot[node.id]){throw new Error("No parent :"+node.parentId+" for node:"+node.label);}
return;}
...
dataList=newData;var stride=parseInt(this.getProperty("stride",-1));if(stride>0){var list=[];var cnt=0;for(var i=0;i<dataList.length;i+=(stride+1)){console.log(i);list.push(dataList[i]);}
dataList=list;}
...
var alldata=data;data={};for(a in alldata)data[a]=[];alldata[names[0].id].map((v,idx)=>{var ok=true;for(var i=0;i<names.length;i++){var id=names[i].id;if(isNaN(alldata[id][idx])){ok=false;break;}}
if(ok){for(var i=0;i<names.length;i++){var id=names[i].id;data[id].push(alldata[id][idx]);}}});if(data.height.length>1){if(data.height[0]>data.height[1]){for(name in data)
...
function OrgchartDisplay(displayManager,id,properties){let ID_ORGCHART="orgchart";let SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_ORGCHART,properties);RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);console.log("orgchart");$.extend(this,{handleEventRecordSelection:function(source,args){},needsData:function(){return true;},updateUI:function(){console.log("ui");if(!waitOnGoogleCharts(this,()=>{this.updateUI();})){return;}
this.displayHtml(HtmlUtils.div(["id",this.getDomId(ID_ORGCHART)],"HELLO"));let roots=null;try{roots=this.makeTree();}catch(error){this.setContents(this.getMessage(error.toString()));return;}
var data=new google.visualization.DataTable();data.addColumn('string','Name');data.addColumn('string','Parent');data.addColumn('string','ToolTip');let rows=[];let func=function(node){cnt++;rows.push([node.label,node.parent?node.parent.label:"",""]);if(node.record){}}
roots.map(func);console.log(rows);data.addRows(rows);var chart=new google.visualization.OrgChart(document.getElementById(this.getDomId(ID_ORGCHART)));chart.draw(data,{'allowHtml':true});}});}
...
if(this.showmap){this.createDisplay('map');}
