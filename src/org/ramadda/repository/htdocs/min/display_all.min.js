...
var img=HtmlUtils.image(value,attrs);s=s.replace("${"+f.getId()+"_image}",img);s=s.replace("${"+f.getId()+"_url}",value);}else{s=s.replace("${"+f.getId()+"_url}",ramaddaBaseUrl+"/icons/blank.gif");s=s.replace("${"+f.getId()+"_image}","");}}else if(f.getType()=="url"){if(value&&value.trim().length>1){value=HtmlUtils.href(value,value);}}else if(f.isDate){if(value){s=s.replace("${"+f.getId()+"}",value);s=s.replace("${"+f.getId()+"_yyyy}",Utils.formatDateYYYY(value));s=s.replace("${"+f.getId()+"_yyyymmdd}",Utils.formatDateYYYYMMDD(value));s=s.replace("${"+f.getId()+"_monthdayyear}",Utils.formatDateMonthDayYear(value));s=s.replace("${"+f.getId()+"_monthday}",Utils.formatDateMonthDay(value));s=s.replace("${"+f.getId()+"_mdy}",Utils.formatDateMDY(value));}
continue;}
...
type="date";else
console.log("Unknwon type:"+tof);fields.push(new RecordField({id:id,label:label,type:type,chartable:true}));}
...
var records=this.filterData();if(records==null)return;var timelineId=this.getDomId(ID_TIMELINE);this.writeHtml(ID_DISPLAY_CONTENTS,HtmlUtils.div(["id",timelineId]));this.timelineReady=false;var opts={start_at_end:this.getProperty("startAtEnd",false),timenav_height:this.getProperty("navHeight",250),gotoCallback:(slide)=>{if(this.timelineReady){var record=records[slide];if(record){this.getDisplayManager().notifyEvent("handleEventRecordSelection",this,{record:record});}}}};var json={};var events=[];json.events=events;var headlineField=this.getFieldById(null,this.getProperty("headlineField"));var startDateField=this.getFieldById(null,this.getProperty("startDateField"));var endDateField=this.getFieldById(null,this.getProperty("endDateField"));var textTemplate=this.getProperty("textTemplate","${default}");var timeTo=this.getProperty("timeTo","day");this.recordToIndex={};for(var i=0;i<records.length;i++){var record=records[i];this.recordToIndex[record.getId()]=i;var tuple=record.getData();var event={};var text=this.getRecordHtml(record,null,textTemplate);event.text={headline:headlineField?tuple[headlineField.getIndex()]:" record:"+(i+1),text:text};if(startDateField)
