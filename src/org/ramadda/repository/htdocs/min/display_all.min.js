...
contents+=HtmlUtils.div(["id",this.getDomId(ID_SEARCHBAR)],"<center>"+searchBar+"</center>");contents+=HtmlUtils.div(["id",this.getDomId(ID_RESULTS)]);this.writeHtml(ID_DISPLAY_CONTENTS,contents);let _this=this;this.jq(ID_SEARCHBAR).find("input, input:radio,select").change(function(){var id=$(this).attr("id");if(id&&id.includes(_this.getDomId(ID_GROUPBY_FIELDS))){_this.groupField=_this.getFieldById(fields,$(this).val());}
...
this.displaySearchResults(records);},displaySearchResults:function(records){var fontSize=this.getProperty("fontSize",null);var cardStyle=this.getProperty("cardStyle",null);var width=this.getProperty("imageWidth","50");var margin=this.getProperty("imageMargin","0");var groupFields=[];var seen=[];for(var i=0;i<this.groupByDepth;i++){var id=this.jq(ID_GROUPBY_FIELDS+i).val();if(!seen[id]){seen[id]=true;var field=this.getFieldById(this.fields,id);if(field)groupFields.push(field);}}
...
var group=topGroup;for(var groupIdx=0;groupIdx<groupFields.length;groupIdx++){var groupField=groupFields[groupIdx];var value=row[groupField.getIndex()];var child=group.findGroup(value);if(!child){group.members.push(child=new groupNode(value));}
group=child;}
