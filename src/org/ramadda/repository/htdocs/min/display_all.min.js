...
this.jq(ID_DISPLAY_CONTENTS).html(html);let categoryField=this.getFieldById(null,this.getProperty("categoryField"));let valueField=this.getFieldById(null,this.getProperty("valueField"));let values;let min=0;let max=0;if(valueField){values=this.getColumnValues(records,valueField).values;values.map((v,idx)=>{min=idx==0?v:Math.min(v,min);max=idx==0?v:Math.max(v,max);});}
