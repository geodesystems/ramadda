...
var ID_FIELDS="fields";var ID_HEADER="header";var ID_HEADER1="header1";var ID_HEADER2="header2";var ID_HEADER3="header3";var ID_TITLE=ATTR_TITLE;var ID_TITLE_EDIT="title_edit";var ID_TOP_RIGHT="topright";var ID_TOP_LEFT="topleft";var ID_DETAILS="details";var ID_DISPLAY_CONTENTS="contents";var ID_DISPLAY_TOP="top";var ID_DISPLAY_BOTTOM="bottom";var ID_GROUP_CONTENTS="group_contents";var ID_DETAILS_MAIN="detailsmain";var ID_GROUPBY_FIELDS="groupdbyfields";var ID_TOOLBAR="toolbar";var ID_TOOLBAR_INNER="toolbarinner";var ID_LIST="list";var ID_DIALOG="dialog";var ID_DIALOG_TABS="dialog_tabs";var ID_DIALOG_BUTTON="dialog_button";var ID_FOOTER="footer";var ID_FOOTER_LEFT="footer_left";var ID_FOOTER_RIGHT="footer_right";var ID_MENU_BUTTON="menu_button";var ID_MENU_OUTER="menu_outer";var ID_MENU_INNER="menu_inner";var ID_REPOSITORY="repository";var displayDebug=false;var PROP_DISPLAY_FILTER="displayFilter";var PROP_EXCLUDE_ZERO="excludeZero";var PROP_DIVID="divid";var PROP_FIELDS="fields";var PROP_LAYOUT_HERE="layoutHere";var PROP_HEIGHT="height";var PROP_WIDTH="width";function initRamaddaDisplays(){ramaddaCheckForResize();if(window.globalDisplaysList==null){return;}
...
values.push(value);}
for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);var ok=true;for(var i=0;i<this.filterFields.length;i++){if(!ok)break;var filterField=this.filterFields[i];var filterValue=values[i];if(filterValue==null||filterValue=="")continue;filterValue=""+filterValue;var value=row[filterField.getIndex()];if(filterField.getType()=="enumeration"){if((""+value)!==filterValue){ok=false;}}else if(filterField.isNumeric){if(!isNaN(filterValue[0])&&value<filterValue[0])ok=false;else if(!isNaN(filterValue[1])&&value>filterValue[1])ok=false;}else if(filterField.getType()=="date"){if(value&&typeof value=="object"){var date1=filterValue[0];var date2=filterValue[1];if(date1&&value.getTime()<date1.getTime())
ok=false;else if(date2&&value.getTime()>date2.getTime())
ok=false;}}else{value=(""+value).toLowerCase();if(value.indexOf(filterValue.toLowerCase())<0){ok=false;}}}
...
var style=(hideFilterWidget?"display:none;":"")+this.getProperty("filterByStyle","");this.jq(ID_HEADER2).html(HtmlUtils.div(["class","display-filterby","style",style],searchBar));if(!hideFilterWidget){for(var i=0;i<dateIds.length;i++){HtmlUtils.datePickerInit(dateIds[i]);}}
this.jq(ID_HEADER2).find(".display-filter-range").mousedown(function(){var id=$(this).attr("id");id=id.replace(/_min$/,"").replace(/_max$/,"");var min=$("#"+id+"_min");var max=$("#"+id+"_max");range={min:parseFloat(min.attr("data-min")),max:parseFloat(max.attr("data-max"))};var minValue=parseFloat(min.val());var maxValue=parseFloat(max.val());var html=HtmlUtils.div(["id","filterby-range","style","width:200px;"],"");var popup=getTooltip();popup.html(html);popup.show();popup.position({of:min,my:"left top",at:"left bottom+2",collision:"fit fit"});let _this=this;if(isNaN(minValue))minValue=range.min;if(isNaN(maxValue))maxValue=range.max;$("#filterby-range").slider({range:true,min:range.min,max:range.max,values:[minValue,maxValue],slide:function(event,ui){min.val(ui.values[0]);max.val(ui.values[1]);},stop:function(){var popup=getTooltip();popup.hide();min.trigger("change");}});});this.jq(ID_HEADER2).find("input, input:radio,select").change(function(){var id=$(this).attr("id");var value=$(this).val();var fieldId=$(this).attr("fieldId");_this.haveCalledUpdateUI=false;if(_this.settingFilterValue){return;}
_this.settingFilterValue=true;_this.dataFilterChanged();_this.propagateEvent("handleEventPropertyChanged",{property:"filterValue",id:id,fieldId:fieldId,value:value});_this.settingFilterValue=false;});}},dataFilterChanged:function(){this.updateUI();},updateUI:function(){},getDoBs:function(){if(!(typeof this.dobs==='undefined')){return dobs;}
...
var cnt=0;tmp.sort((a,b)=>{var t1=this.getDataValues(a);var t2=this.getDataValues(b);var v1=t1[sortField.getIndex()];var v2=t2[sortField.getIndex()];if(cnt++<100)console.log("Sort:"+v1+" "+v2);if(v1<v2)return this.sortAscending?-1:1;if(v1>v2)return this.sortAscending?1:-1;return 0;});for(var rowIdx=0;rowIdx<tmp.length;rowIdx++){var obj=tmp[rowIdx];var tuple=this.getDataValues(obj);var label="";if(stringField)
...
if(rowIdx<100)console.log(value);html+="<tr valign=top class='display-ranking-row' what='"+obj.originalRow+"'><td> #"+(rowIdx+1)+"</td><td>&nbsp;"+label+"</td><td align=right>&nbsp;"+
value+"</td></tr>";}
...
if(f.isString()){headerTemplate=headerTemplate.replace("${"+f.getId()+"_uniques}",s.uniqueCount);footerTemplate=footerTemplate.replace("${"+f.getId()+"_uniques}",s.uniqueCount);continue;}
...
if(this.map.circles)
this.map.circles.redraw();this.applyVectorMap(true);},dataFilterChanged:function(){this.vectorMapApplied=false;this.updateUI();},updateUI:function(){SUPER.updateUI.call(this);if(!this.getDisplayReady()){return;}
