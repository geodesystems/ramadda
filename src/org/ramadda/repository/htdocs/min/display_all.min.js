...
let filterBy=this.getProperty("filterFields","",true).split(",");let hideFilterWidget=this.getProperty("hideFilterWidget",false,true);let dateIds=[];if(filterBy.length>0){let searchBar="";let bottom="";let widgetStyle="";if(hideFilterWidget)
widgetStyle="display:none;";let fieldMap={};for(let i=0;i<filterBy.length;i++){let filterField=this.getFieldById(fields,filterBy[i]);if(!filterField)continue;fieldMap[filterField.getId()]={field:filterField,values:[],};if(!filterField)continue;this.filterFields.push(filterField);let widget;let widgetId=this.getFilterId(filterField.getId());if(filterField.getType()=="enumeration"){let dfltValue=this.getProperty(filterField.getId()+".filterValue",FILTER_ALL);let filterValues=this.getProperty(filterField.getId()+".filterValues");let enums=null;if(filterValues){let toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}
...
obj.image=this.getDataValues(record)[imageField.getIndex()];if((+value+"")!=value)valuesAreNumbers=false;let label=value;if(label.length>20){label=label.substring(0,19)+"...";}
