...
var templates={};fields.map(f=>{templates[f.getId()]=this.getProperty(f.getId()+".template");});var colorBy=this.getColorByInfo(records);var delimiter=this.getProperty("delimiter","");for(var rowIdx=0;rowIdx<records.length;rowIdx++){var record=records[rowIdx];if(!Utils.isDefined(record.lineNumber)){record.lineNumber=(rowIdx+1);}
...
line+=value;}
line=line.trim();if(!includeEmptyLines&&line.length==0)continue;lineCnt++;if(!patternMatch.matches(line)){continue;}
...
if(bubble)line=HtmlUtils.div(["class","ramadda-bubble"],line);if(fromField)line+=HtmlUtils.div(["class","ramadda-bubble-from"],""+row[fromField.getIndex()]);line=HtmlUtils.div(lineAttrs,line);if(labelTemplate){var label=this.getRecordHtml(record,null,labelTemplate);var num=record.lineNumber;if(!Utils.isDefined(num)){num-lineCnt;}
label=label.replace("${lineNumber}","#"+(num));label=label.replace(/ /g,"&nbsp;");corpus+=HtmlUtils.tr(rowAttrs,HtmlUtils.td(["width","10px"],"<a name=line_"+lineCnt+"></a>"+
