...
function RamaddaSkewtDisplay(displayManager,id,properties){let SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_SKEWT,properties);var ID_SKEWT="skewt";RamaddaUtil.inherit(this,SUPER);addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{needsData:function(){return true;},initDisplay:function(){SUPER.initDisplay.call(this);},handleEventMapClick:function(source,args){if(!this.dataCollection){return;}
var pointData=this.dataCollection.getList();for(var i=0;i<pointData.length;i++){pointData[i].handleEventMapClick(this,source,args.lon,args.lat);}},handleEventPointDataLoaded:function(source,pointData){this.updateUI();},updateUI:async function(){if(!this.loadedResources){await Utils.importCSS(ramaddaBaseUrl+"/lib/skewt/sounding.css");await Utils.importJS(ramaddaBaseUrl+"/lib/skewt/d3skewt.js");this.loadedResources=true;}
if(!window["D3Skewt"]){setTimeout(()=>this.updateUI(),100);return;}
SUPER.updateUI.call(this);var skewtId=this.getDomId(ID_SKEWT);var html=HtmlUtils.div(["id",skewtId],"");this.setContents(html);var pointData=this.getData();if(pointData==null)return;var records=pointData.getRecords();if(!records){console.log("no data yet");return;}
