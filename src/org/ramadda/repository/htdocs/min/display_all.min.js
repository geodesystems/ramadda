...
console.log("\returning value:"+value);return value;},debugGetProperty:false,getPropertyInner:function(key,dflt,skipThis){let debug=this.debugGetProperty;if(debug)console.log("getProperty:"+key+" dflt:"+dflt);if(this.dynamicProperties){if(Utils.isDefined(this.dynamicProperties[key])){return this.dynamicProperties[key];}}
if(!skipThis&&Utils.isDefined(this[key])){if(debug)console.log("\tgetProperty-1");return this[key];}
...
url=url+"&"+HtmlUtils.urlArg(this.urlarg+"_todate",to);}else if(this.type=="enumeration"){let value=this.getValue();if(!Array.isArray(value)){value=[value];}
if(value.length>0&&value[0]!=""){let regexp=new RegExp(this.urlarg+"=[^$&]*",'g');url=url.replace(regexp,"");value.map(v=>{if(v!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg,v);});}}else{let value=this.getValue();this.dflt=value;if(value!=""){let regexp=new RegExp(this.urlarg+"=[^$&]*",'g');url=url.replace(regexp,"");url=url+"&"+HtmlUtils.urlArg(this.urlarg,value);}}
return url;}});return true;});return macros;},applyRequestProperties:function(props){if(!props)return;this.requestMacros=null;this.dynamicProperties=props;this.createRequestProperties();},createRequestProperties:function(){let requestProps="";let macros=this.getRequestMacros();let macroDateIds=[];macros.every(macro=>{if(this.getProperty("request."+macro.name+".show",true)){requestProps+=macro.getWidget(macroDateIds)+"&nbsp;&nbsp;";}
return true;});this.jq(ID_REQUEST_PROPERTIES).html(requestProps);let macroChange=(macro,value,what)=>{if(this.settingMacroValue)return;if(macro.triggerReload){this.macroChanged();this.reloadData();}
...
return;}
if(debug)
console.log("\tgot data");var newData=makePointData(data,_this.derived,display);obj.pointData=pointData.initWith(newData);if(data.properties){display.applyRequestProperties(data.properties);}
if(debug)
...
return;}
let cidx=0
let polygonField=this.getFieldById(fields,this.getProperty("polygonField"));let polygonColorTable=this.getColorTable(true,"polygonColorTable",null);let latlon=this.getProperty("latlon",true);let source=this;let radius=parseFloat(this.getDisplayProp(source,"radius",8));if(this.getProperty("scaleRadius")){let seen={};let numLocs=0;points.every(p=>{if(!seen[p]){numLocs++;seen[p]=true;}
