...
let _this=this;blob.div.set(r);var i=0;for(i=0;i<commands.length;i++){var cmd=commands[i];await cmd.proc.call(_this,cmd.line,cmd.toks,cmd.div,cmd.extra);}},processJs:async function(blob){try{await this.getCurrentEntry(e=>{current=e});var state=new NotebookState(this);notebookStates[state.id]=state;var notebookEntries=this.notebook.getCurrentEntries();for(name in notebookEntries){state.entries[name]=notebookEntries[name].entry;}
var jsSet="";state.entries["current"]=current;state.entries["parent"]=this.parentEntry;var stateJS="notebookStates['"+state.id+"']";jsSet+="state= "+stateJS+";\n";for(name in state.entries){var e=state.entries[name];jsSet+=name+"= state.entries['"+name+"'];\n"}
...
await this.currentEntry.getParentEntry(entry=>{this.parentEntry=entry;});},getCurrentEntry:async function(callback){if(this.currentEntry==null){await this.setCurrentEntry(this.notebook.currentEntry);}
...
return Utils.call(callback,this.currentEntry);},createDisplay:async function(entry,displayType){if(!entry)await this.getCurrentEntry(e=>entry=e);if((typeof entry)=="string"){await this.notebook.getEntry(entry,e=>entry=e);}
var jsonUrl=this.notebook.getPointUrl(entry);if(jsonUrl==null){this.writeError("Not a point type:"+entry.getName());return;}
this.notebook.createDisplay(entry.getId(),displayType,jsonUrl);},createPointDisplay:async function(toks,displayType){await this.getCurrentEntry(e=>current=e);var entry=this.getEntryFromArgs(toks,currentEntry);var jsonUrl=this.notebook.getPointUrl(entry);if(jsonUrl==null){this.writeError("Not a point type:"+entry.getName());return;}
...
if(h=="")h="No entries";div.set(h);},processCommand_echo:async function(line,toks,div){line=line.replace(/^echo */,"");div.set(line);},processCommand_print:async function(line,toks,div){line=line.replace(/^print */,"");div.set(line);},processCommand_cd:async function(line,toks,div){if(div==null)div=new Div();if(toks.length<=1){await this.setCurrentEntry(this.notebook.currentEntry);return;}
...
await this.setCurrentEntry(this.parentEntry);}else{await this.currentEntry.getChildrenEntries(c=>children=c);var child=null;for(var childIdx=0;childIdx<children.length;childIdx++){if(children[childIdx].getName()==dir){child=children[childIdx];break;}}
