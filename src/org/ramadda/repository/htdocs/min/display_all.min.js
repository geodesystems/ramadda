...
return titleToShow;},handleEventFieldValueSelected:function(source,args){this.setProperty("filterPattern",args.value);this.setProperty("patternFilterField",args.field.getId());this.updateUI();},handleEventPropertyChanged:function(source,prop){if(prop.property=="filterValue"){this.haveCalledUpdateUI=false;var widgetId="filterby_"+prop.fieldId;if(prop.id.endsWith("_operator"))widgetId+="_operator";this.settingFilterValue=true;this.jq(widgetId).val(prop.value);this.settingFilterValue=false;}
...
var newData=[];var values=[];var operators=[];for(var i=0;i<this.filterFields.length;i++){var filterField=this.filterFields[i];var prefix=this.getProperty(filterField.getId()+".filterPrefix");var suffix=this.getProperty(filterField.getId()+".filterSuffix");var value=$("#"+this.getDomId("filterby_"+filterField.getId())).val();if(prefix)pattern=prefix+value;if(suffix)pattern=value+suffix;if(filterField.getType()=="date"){var date1=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date1")).val();var date2=$("#"+this.getDomId("filterby_"+filterField.getId()+"_date2")).val();if(date1!=null&&date1.trim()!="")
...
values.push(value);operators.push($("#"+this.getDomId("filterby_"+filterField.getId()+"_operator")).val());}
for(var rowIdx=0;rowIdx<dataList.length;rowIdx++){var row=this.getDataValues(dataList[rowIdx]);var ok=true;for(var i=0;i<this.filterFields.length;i++){if(!ok)break;var filterField=this.filterFields[i];var filterValue=values[i];if(filterValue==null||filterValue=="")continue;var value=row[filterField.getIndex()];if(filterField.getType()=="enumeration"){if(value!=filterValue){ok=false;}}else if(filterField.isNumeric){filterValue=parseFloat(filterValue);var op=operators[i];if(op==null)op="<";if(op=="<"){ok=value<filterValue;}else if(op=="<="){ok=value<=filterValue;}else if(op=="="){ok=value==filterValue;}else if(op==">"){ok=value>filterValue;}else if(op==">="){ok=value>=filterValue;}}else if(filterField.getType()=="date"){if(value&&typeof value=="object"){var date1=filterValue[0];var date2=filterValue[1];if(date1&&value.getTime()<date1.getTime())
...
widgetStyle="display:none;";for(var i=0;i<filterBy.length;i++){var filterField=this.getFieldById(fields,filterBy[i]);if(!filterField)continue;this.filterFields.push(filterField);var widget;var widgetId=this.getDomId("filterby_"+filterField.getId());var value=this.getProperty(filterField.getId()+".filterValue","");if(filterField.getType()=="enumeration"){var filterValues=this.getProperty(filterField.getId()+".filterValues");var enums=null;if(filterValues){var toks;if((typeof filterValues)=="string"){filterValues=Utils.getMacro(filterValues);toks=filterValues.split(",");}else{toks=filterValues;}}
...
widget=HtmlUtils.select("",["style",widgetStyle,"id",widgetId,"fieldId",filterField.getId()],enums,value);}else if(filterField.isNumeric){var opid=widgetId+"_operator";var operator=this.getProperty(filterField.getId()+".filterOperator",">");widget=HtmlUtils.select("",["style",widgetStyle,"id",opid,"fieldId",filterField.getId()],["<",">","="],operator,);widget+=HtmlUtils.input("",value,["style",widgetStyle,"id",widgetId,"size",4,"fieldId",filterField.getId()]);}else if(filterField.getType()=="date"){widget=HtmlUtils.datePicker("","",["style",widgetStyle,"id",widgetId+"_date1"])+" - "+
HtmlUtils.datePicker("","",["style",widgetStyle,"id",widgetId+"_date2"]);dateIds.push(widgetId+"_date1");dateIds.push(widgetId+"_date2");}else{widget=HtmlUtils.input("",value,["style",widgetStyle,"id",widgetId,"fieldId",filterField.getId()]);}
...
this.jq(ID_HEADER2).html(searchBar);if(!hideFilterWidget){for(var i=0;i<dateIds.length;i++){HtmlUtils.datePickerInit(dateIds[i]);}}
this.jq(ID_HEADER2).find("input, input:radio,select").change(function(){var id=$(this).attr("id");var value=$(this).val();var fieldId=$(this).attr("fieldId");_this.haveCalledUpdateUI=false;if(_this.settingFilterValue){console.log("setting value");return;}
