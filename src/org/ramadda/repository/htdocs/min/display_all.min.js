...
suffix=this.getProperty("dateSuffix");var timeZone=this.getTimeZone();if(!suffix&&timeZone)suffix=timeZone;return Utils.formatDate(date,args.options,{timeZone:timeZone,suffix:suffix});},getUniqueId:function(base){return HtmlUtils.getUniqueId(base);},handleError:function(code,message){GuiUtils.handleError("An error has occurred:"+message,true,true);},toString:function(){return"DisplayThing:"+this.getId();},getDomId:function(suffix){return this.getId()+"_"+suffix;},jq:function(componentId){return $("#"+this.getDomId(componentId));},writeHtml:function(idSuffix,html){$("#"+this.getDomId(idSuffix)).html(html);},getRecordHtml:function(record,fields){var showGeo=false;if(Utils.isDefined(this.showGeo)){showGeo=(""+this.showGeo)=="true";}
...
return titleToShow;},handleEventFieldValueSelected:function(source,args){this.setProperty("filterPattern",args.value);this.setProperty("patternFilterField",args.field.getId());this.updateUI();},handleDateRangeChanged:function(source,prop){this.minDateObj=prop.minDate;this.maxDateObj=prop.maxDate;this.getAnimation().setDateRange(prop.minDate,prop.maxDate);this.haveCalledUpdateUI=false;this.dataFilterChanged();},handleEventPropertyChanged:function(source,prop){if(prop.property=="dateRange"){if(this.getProperty("acceptDateRangeChange")){this.handleDateRangeChanged(source,prop);}
return;}
...
if(template!=""){var iconField=this.getFieldById(fields,this.getProperty("iconField"));var iconSize=parseFloat(this.getProperty("iconSize",16));var iconMap=this.getIconMap();var colorBy=this.getProperty("colorBy");var colorByMap=this.getColorByMap();var max=parseFloat(this.getProperty("maxNumber",-1));for(var rowIdx=0;rowIdx<selected.length;rowIdx++){if(max!=-1&&rowIdx>=max)break;var row=this.getDataValues(selected[rowIdx]);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);if(iconMap&&iconField){var value=row[iconField.getIndex()];var icon=iconMap[value];if(icon){s=s.replace("${"+iconField.getId()+"_icon}",HtmlUtils.image(icon,["width",iconSize]));}}
for(var col=0;col<fields.length;col++){var f=fields[col];var value=row[f.getIndex()];if(iconMap){var icon=iconMap[f.getId()+"."+value];if(icon){s=s.replace("${"+f.getId()+"_icon}",HtmlUtils.image(icon,["size",iconSize]));}}
...
var color;if(colorByMap){if(colorBy&&colorBy==f.getId()){color=colorByMap[value];}else{color=colorByMap[f.getId()+"."+value];}}
