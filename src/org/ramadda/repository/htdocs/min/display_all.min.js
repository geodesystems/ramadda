...
var ID_FIELDS="fields";var ID_HEADER="header";var ID_TITLE=ATTR_TITLE;var ID_TITLE_EDIT="title_edit";var ID_TOP_RIGHT="topright";var ID_DETAILS="details";var ID_DISPLAY_CONTENTS="contents";var ID_DISPLAY_TOP="top";var ID_DISPLAY_BOTTOM="bottom";var ID_GROUP_CONTENTS="group_contents";var ID_DETAILS_MAIN="detailsmain";var ID_TOOLBAR="toolbar";var ID_TOOLBAR_INNER="toolbarinner";var ID_LIST="list";var ID_DIALOG="dialog";var ID_DIALOG_TABS="dialog_tabs";var ID_DIALOG_BUTTON="dialog_button";var ID_FOOTER="footer";var ID_FOOTER_LEFT="footer_left";var ID_FOOTER_RIGHT="footer_right";var ID_MENU_BUTTON="menu_button";var ID_MENU_OUTER="menu_outer";var ID_MENU_INNER="menu_inner";var ID_REPOSITORY="repository";var displayDebug=false;var PROP_DISPLAY_FILTER="displayFilter";var PROP_EXCLUDE_ZERO="excludeZero";var PROP_DIVID="divid";var PROP_FIELDS="fields";var PROP_LAYOUT_HERE="layoutHere";var PROP_HEIGHT="height";var PROP_WIDTH="width";function addHandler(obj){if(window.globalHandlers==null){window.globalHandlers={};}
var id=HtmlUtils.getUniqueId();window.globalHandlers[id]=obj;return id;}
function getHandler(id){if(!id||window.globalHandlers==null){return null;}
return window.globalHandlers[id];}
function initRamaddaDisplays(){ramaddaCheckForResize();if(window.globalDisplaysList==null){return;}
...
return this.getProperty(key,dflt);},initTooltip:function(){},formatNumber:function(number){if(!this.getProperty("format",true))return number;return Utils.formatNumber(number);},propertyDefined:function(key){return Utils.isDefined(this.getProperty(key));},getProperty:function(key,dflt){if(this[key]){return this[key];}
...
return this.getBaseClass()+"-"+suffix;},getBaseClass:function(){return"display-"+this.getType();},setDisplayManager:function(cm){this.displayManager=cm;this.setDisplayParent(cm.getLayoutManager());},setContents:function(contents){contents=HtmlUtils.div([ATTR_CLASS,"display-contents-inner display-"+this.getType()+"-inner"],contents);this.writeHtml(ID_DISPLAY_CONTENTS,contents);},addEntry:function(entry){this.entries.push(entry);},clearCachedData:function(){},setEntry:function(entry){this.entries=[];this.addEntry(entry);this.entry=entry;this.entryId=entry.getId();this.clearCachedData();if(this.properties.data){this.dataCollection=new DataCollection();var attrs={entryId:this.entryId,lat:this.getProperty("latitude"),lon:this.getProperty("longitude"),};this.properties.data=this.data=new PointData(entry.getName(),null,null,this.getRamadda().getRoot()+"/entry/show?entryid="+entry.getId()+"&output=points.product&product=points.json&max=5000",attrs);this.data.loadData(this);}
this.updateUI();var title="";if(this.getShowTitle()){title=entry.getName();title=HtmlUtils.href(this.getRamadda().getEntryUrl(this.entryId),title);this.jq(ID_TITLE).html(title);}},handleEventFieldValueSelected:function(source,args){this.setProperty("filterPattern",args.value);this.setProperty("patternFilterField",args.field.getId());this.updateUI();},handleEventPropertyChanged:function(source,prop){if(prop.property=="filterPattern"){this.jq("filterValueMenu").val(prop.value);}
...
if(args.selected){$("#"+this.getDomId(ID_TITLE)).addClass("display-title-select");}else{$("#"+this.getDomId(ID_TITLE)).removeClass("display-title-select");}},highlightEntry:function(entry){$("#"+this.getDomId(ID_TITLE)).addClass("display-title-select");},getEntries:function(){return this.entries;},getDisplayEntry:function(){var entries=this.getEntries();if(entries!=null&&entries.length>0){return entries[0];}
if(this.entryId){var callback={call:function(entry){console.log("got entry "+entry);}};return this.getRamadda().getEntry(this.entryId,callback);}
...
return dflt;},getFooter:function(){return HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER),ATTR_CLASS,"display-footer"],HtmlUtils.leftRight(HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER_LEFT),ATTR_CLASS,"display-footer-left"],""),HtmlUtils.div([ATTR_ID,this.getDomId(ID_FOOTER_RIGHT),ATTR_CLASS,"display-footer-right"],"")));},checkFixedLayout:function(){if(this.getIsLayoutFixed()){var divid=this.getProperty(PROP_DIVID);if(divid!=null){var html=this.getHtml();$("#"+divid).html(html);}}},shouldSkipField:function(field){if(this.skipFields&&!this.skipFieldsList){this.skipFieldsList=this.skipFields.split(",");}
...
var df=[];var dataList=this.dataCollection.getList();var fixedFields=this.getProperty(PROP_FIELDS);for(var collectionIdx=0;collectionIdx<dataList.length;collectionIdx++){var pointData=dataList[collectionIdx];var fields=this.getFieldsToSelect(pointData);if(fixedFields!=null&&fixedFields.length>0){if(this.debugSelected)
...
var url=ramaddaBaseUrl+"/entry/copy?action.force=copy&from="+from;window.open(url,'_blank');},entryHtmlHasBeenDisplayed:function(entry){if(entry.getIsGroup()){var theDisplay=this;var callback=function(entries){var html=HtmlUtils.openTag(TAG_OL,[ATTR_CLASS,"display-entrylist-list",ATTR_ID,theDisplay.getDomId(ID_LIST)]);html+=theDisplay.getEntriesTree(entries);html+=HtmlUtils.closeTag(TAG_OL);theDisplay.jq(ID_GROUP_CONTENTS+entry.getIdForDom()).html(html);theDisplay.addEntrySelect();};var entries=entry.getChildrenEntries(callback);}},getEntryHtml:function(entry,props){var dfltProps={showHeader:true,headerRight:false,showDetails:this.getShowDetails()};$.extend(dfltProps,props);props=dfltProps;var menu=this.getEntryMenuButton(entry);var html="";if(props.showHeader){var left=menu+" "+entry.getLink(entry.getIconImage()+" "+entry.getName());if(props.headerRight)html+=HtmlUtils.leftRight(left,props.headerRight);else html+=left;}
var divid=HtmlUtils.getUniqueId("entry_");html+=HtmlUtils.div(["id",divid],"");if(false){var url=this.getRamadda().getRoot()+"/entry/show?entryid="+entry.getId()+"&decorate=false&output=metadataxml&details=true";console.log(url);$("#"+divid).load(url,function(){alert("Load was performed.");});}
...
this.entriesMap={};for(var i=0;i<entries.length;i++){even=!even;var entry=entries[i];this.entriesMap[entry.getId()]=entry;var toolbar=this.makeEntryToolbar(entry);var entryMenuButton=this.getEntryMenuButton(entry);var entryName=entry.getDisplayName();if(entryName.length>100){entryName=entryName.substring(0,99)+"...";}
...
return html;},addEntrySelect:function(){var theDisplay=this;var entryRows=$("#"+this.getDomId(ID_DISPLAY_CONTENTS)+"  ."+this.getClass("entry-main"));entryRows.unbind();entryRows.mouseover(function(event){var entryId=$(this).attr(ATTR_ENTRYID);entry=theDisplay.getEntry(entryId);if(!entry){console.log("no entry:"+entryId);return;}
theDisplay.propagateEvent("handleEventEntryMouseover",{entry:entry});if(true)return;var domEntryId=Utils.cleanId(entryId);var toolbarId=theDisplay.getEntryToolbarId(domEntryId);var toolbar=$("#"+toolbarId);toolbar.show();var myalign='right top+1';var atalign='right top';var srcId=theDisplay.getDomId(ID_DETAILS_MAIN+domEntryId);toolbar.position({of:$("#"+srcId),my:myalign,at:atalign,collision:"none none"});});entryRows.mouseout(function(event){var entryId=$(this).attr(ATTR_ENTRYID);entry=theDisplay.getEntry(entryId);if(!entry)return;theDisplay.propagateEvent("handleEventEntryMouseout",{entry:entry});var domEntryId=Utils.cleanId(entryId);var toolbarId=theDisplay.getEntryToolbarId(entryId);var toolbar=$("#"+toolbarId);});if(this.madeList){}
this.madeList=true;if(false){this.jq(ID_LIST).selectable({cancel:'a',selected:function(event,ui){var entryId=ui.selected.getAttribute(ATTR_ENTRYID);theDisplay.toggleEntryDetails(event,entryId);if(true)return;theDisplay.hideEntryDetails(entryId);var entry=theDisplay.getEntry(entryId);if(entry==null)return;var zoom=null;if(event.shiftKey){zoom={zoomIn:true};}
theDisplay.selectedEntries.push(entry);theDisplay.propagateEvent("handleEventEntrySelection",{entry:entry,selected:true,zoom:zoom});theDisplay.lastSelectedEntry=entry;},unselected:function(event,ui){if(true)return;var entryId=ui.unselected.getAttribute(ATTR_ENTRYID);var entry=theDisplay.getEntry(entryId);var index=theDisplay.selectedEntries.indexOf(entry);if(index>-1){theDisplay.selectedEntries.splice(index,1);theDisplay.propagateEvent("handleEventEntrySelection",{entry:entry,selected:false});}},});}},getEntriesTable:function(entries,columns,columnNames){if(this.entriesMap==null)
...
html+=HtmlUtils.closeTag(TAG_TABLE);return html;},makeEntryToolbar:function(entry){var get=this.getGet();var toolbarItems=[];if(entry.getType().getId()=="type_wms_layer"){toolbarItems.push(HtmlUtils.tag(TAG_A,["onclick",get+".addMapLayer("+HtmlUtils.sqt(entry.getId())+");"],HtmlUtils.image(ramaddaBaseUrl+"/icons/map.png",["border",0,ATTR_TITLE,"Add Map Layer"])));}
if(entry.getType().getId()=="geo_shapefile"){toolbarItems.push(HtmlUtils.tag(TAG_A,["onclick",get+".addMapLayer("+HtmlUtils.sqt(entry.getId())+");"],HtmlUtils.image(ramaddaBaseUrl+"/icons/map.png",["border",0,ATTR_TITLE,"Add Map Layer"])));}
...
var entryMenuButton=this.getEntryMenuButton(entry);var tmp=[];for(var i=0;i<toolbarItems.length;i++){tmp.push(HtmlUtils.div([ATTR_CLASS,"display-entry-toolbar-item"],toolbarItems[i]));}
...
this.toggleEntryDetails(event,entryId);},toggleEntryDetails:function(event,entryId,suffix,handlerId){var entry=this.getEntry(entryId);if(suffix==null)suffix="";var link=this.jq(ID_TREE_LINK+entry.getIdForDom()+suffix);var id=ID_DETAILS+entry.getIdForDom()+suffix;var details=this.jq(id);var detailsInner=this.jq(ID_DETAILS_INNER+entry.getIdForDom()+suffix);if(event&&event.shiftKey){var id=Utils.cleanId(entryId);var line=this.jq(ID_DETAILS_MAIN+id);if(!this.selectedEntriesFromTree){this.selectedEntriesFromTree={};}
...
return getGlobalRamadda();},getEntry:function(entryId,callback){if(this.entriesMap&&this.entriesMap[entryId])
return this.entriesMap[entryId];var ramadda=this.getRamadda();var toks=entryId.split(",");if(toks.length==2){entryId=toks[1];ramadda=getRamadda(toks[0]);}
var entry=null;if(this.entryList!=null){entry=this.entryList.getEntry(entryId);}
if(entry==null){entry=ramadda.getEntry(entryId);}
if(entry==null){entry=this.getRamadda().getEntry(entryId,callback);}
return entry;},addMapLayer:function(entryId){var entry=this.getEntry(entryId);if(entry==null){console.log("No entry:"+entryId);return;}
this.getDisplayManager().addMapLayer(this,entry);},doit:function(){console.log("doit");},createDisplay:function(entryId,displayType,jsonUrl){console.log("createDisplay: "+entryId+" "+displayType+" "+jsonUrl);var entry=this.getEntry(entryId);if(entry==null){console.log("No entry:"+entryId);return null;}
var props={showMenu:true,sourceEntry:entry,entryId:entry.getId(),showTitle:true,showDetails:true,title:entry.getName(),};if(displayType!=DISPLAY_ENTRYLIST){if(jsonUrl==null){jsonUrl=this.getPointUrl(entry);}
...
return null;},getEntryMenu:function(entryId){var entry=this.getEntry(entryId);if(entry==null){return"null entry";}
...
var menu=HtmlUtils.tag(TAG_UL,[ATTR_ID,this.getDomId(ID_MENU_INNER+entry.getIdForDom()),ATTR_CLASS,"sf-menu"],topMenus);return menu;},showEntryMenu:function(event,entryId){var menu=this.getEntryMenu(entryId);this.writeHtml(ID_MENU_OUTER,menu);var srcId=this.getDomId(ID_MENU_BUTTON+Utils.cleanId(entryId));showPopup(event,srcId,this.getDomId(ID_MENU_OUTER),false,"left top","left bottom");$("#"+this.getDomId(ID_MENU_INNER+Utils.cleanId(entryId))).superfish({speed:'fast',delay:300});},fetchUrl:function(as,url){if(url==null){url=this.jsonUrl;}
...
return false;},cnt:0,getHtml:function(){var dobs=this.getDoBs();var html="";var width=this.getWidth();if(dobs){html+=HtmlUtils.openDiv(["class","minitron"]);}
if(width>0){html+=HtmlUtils.openDiv(["class","display-contents","style","width:"+width+"px;"]);}else{html+=HtmlUtils.openDiv(["class","display-contents"]);}
html+=HtmlUtils.div([ATTR_CLASS,"ramadda-popup",ATTR_ID,this.getDomId(ID_MENU_OUTER)],"");var menu=HtmlUtils.div([ATTR_CLASS,"display-dialog",ATTR_ID,this.getDomId(ID_DIALOG)],"");var get=this.getGet();var button="";if(this.getShowMenu()){button=HtmlUtils.onClick(get+".showDialog();",HtmlUtils.image(ramaddaBaseUrl+"/icons/downdart.png",[ATTR_CLASS,"display-dialog-button",ATTR_ID,this.getDomId(ID_DIALOG_BUTTON)]));}
var title="";if(this.getShowTitle()){title=this.getTitle(false).trim();}
var left="";if(button!=""||title!=""){this.cnt++;var titleDiv="";var label=title;if(title!=""&&this.entryId){label=HtmlUtils.href(this.getRamadda().getEntryUrl(this.entryId),title);}
titleDiv=HtmlUtils.tag("span",[ATTR_CLASS,"display-title",ATTR_ID,this.getDomId(ID_TITLE)],this.getDisplayTitle(title));if(button==""){left=titleDiv;}else{left="<div class=display-header>"+button+"&nbsp;"+titleDiv+"</div>";}}
var right=HtmlUtils.div(["id",this.getDomId(ID_TOP_RIGHT)],"");html+=HtmlUtils.leftRightTable(left,right,{valign:"bottom"});var contents=this.getContentsDiv();html+=contents;html+=menu;html+=HtmlUtils.closeTag(TAG_DIV);if(dobs){html+=HtmlUtils.closeTag(TAG_DIV);}
...
var text=this.getDisplayTitle(title);if(this.entryId){text=HtmlUtils.href(this.getRamadda().getEntryUrl(this.entryId),text);}
if(this.getShowTitle()){this.jq(ID_TITLE).show();}else{this.jq(ID_TITLE).hide();}
...
callback(names);this.doingQuickEntrySearch=false;},addData:function(pointData){var records=pointData.getRecords();if(records&&records.length>0){this.hasElevation=records[0].hasElevation();}else{this.hasElevation=false;}
this.dataCollection.addData(pointData);var entry=pointData.entry;if(entry==null){entry=this.getRamadda().getEntry(pointData.entryId);}
...
var msg="";if(data&&data.errorcode&&data.errorcode=="warning"){msg=data.error;}else{msg="<b>Sorry, but an error has occurred:</b>";if(!data)data="No data returned from server";msg+=HtmlUtils.tag("div",["style","background:#fff;margin:10px;padding:10px;border:1px #ccc solid;   border-radius: 0px;"],(data.error?data.error:data));}
...
RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_LABEL,properties));addRamaddaDisplay(this);this.text="";this.editMode=properties.editMode;if(properties.text)this.text=properties.text;else if(properties.label)this.text=properties.label;else if(properties.html)this.text=properties.html;else if(properties.title)this.text=properties.title;if(properties["class"])this["class"]=properties["class"];else this["class"]="display-text";RamaddaUtil.defineMembers(this,{initDisplay:function(){var theDisplay=this;this.createUI();var textClass=this["class"];if(this.editMode){textClass+=" display-text-edit ";}
var html=HtmlUtils.div([ATTR_CLASS,textClass,ATTR_ID,this.getDomId(ID_TEXT)],this.text);if(this.editMode){html+=HtmlUtils.textarea(ID_EDIT,this.text,["rows",5,"cols",120,ATTR_SIZE,"120",ATTR_CLASS,"display-text-input",ATTR_ID,this.getDomId(ID_EDIT)]);}
this.setContents(html);if(this.editMode){var editObj=this.jq(ID_EDIT);editObj.blur(function(){theDisplay.text=editObj.val();editObj.hide();theDisplay.initDisplay();});this.jq(ID_TEXT).click(function(){var src=theDisplay.jq(ID_TEXT);var edit=theDisplay.jq(ID_EDIT);edit.show();edit.css('z-index','9999');edit.position({of:src,my:"left top",at:"left top",collision:"none none"});theDisplay.jq(ID_TEXT).html("hello there");});}},getWikiAttributes:function(attrs){SUPER.getWikiAttributes(attrs);attrs.push("text");attrs.push(this.text);},});}
var DISPLAY_NOTEBOOK="notebook";addGlobalDisplayType({type:DISPLAY_NOTEBOOK,label:"Notebook",requiresData:false,category:"Misc"});function RamaddaNotebookDisplay(displayManager,id,properties){var SUPER;var ID_CELLS="cells";var ID_CELL="cell";RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_NOTEBOOK,properties));addRamaddaDisplay(this);RamaddaUtil.defineMembers(this,{cells:[],cellId:0,fetchedNotebook:false,initDisplay:function(){let _this=this;this.createUI();var cells=HtmlUtils.div([ATTR_CLASS,"display-notebook-cells",ATTR_ID,this.getDomId(ID_CELLS)],"");var html=cells;this.setContents(html);if(!this.fetchedNotebook){this.currentEntry=this.getEntry(this.getProperty("entryId",""),entry=>{_this.currentEntry=entry});this.fetchedNotebook=true;var id=this.getProperty("entryId","");var url=ramaddaBaseUrl+"/getnotebook?entryid="+id;url+="&id="+this.getProperty("notebookId","default_notebook");var jqxhr=$.getJSON(url,function(data){if(data.error){_this.setContents(_this.getMessage("Failed to load notebook: "+data.error));return;}
if(!Utils.isDefined(data.cells)){return;}
_this.cells=[];for(var i=0;i<data.cells.length;i++){var props=data.cells[i];_this.cells.push(_this.addCell(props.outputHtml,props));}
_this.layoutCells();});}
if(this.cells.length>0){this.layoutCells();}else{var props={showEdit:true,}
this.cells.push(this.addCell("html:{help}",props));this.processCells();}
this.cells[0].focus();},saveNotebook:function(){var json=this.getJson();json=JSON.stringify(json,null,2);var args={entryid:this.getProperty("entryId",""),notebookId:this.getProperty("notebookId","default_notebook"),notebook:json};var url=ramaddaBaseUrl+"/savenotebook";$.post(url,args,function(result){if(result.error){alert("Error saving notebook: "+result.error);return;}
if(result.result!="ok"){alert("Error saving notebook: "+result.result);return;}
alert("Notebook saved");});},getJson:function(){var obj={cells:[],};for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];obj.cells.push(cell.getJson());}
return obj;},layoutCells:function(){this.jq(ID_CELLS).html("");for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];this.jq(ID_CELLS).append(HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cell.id],""));cell.createCell();}},addCell:function(content,props){if(!props)props={};var cellId=this.getId()+"_"+this.cellId;this.cellId++;this.jq(ID_CELLS).append(HtmlUtils.div([ATTR_CLASS,"display-notebook-cell",ATTR_ID,cellId],""));var cell=new RamaddaNotebookCell(this,cellId,content,props);cell.createCell();return cell;},clearOutput:function(){for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];cell.clearOutput();}},moveCellUp:function(cell){var cells=[];var newCell=null;var idx=0;for(var i=0;i<this.cells.length;i++){if(cell.id==this.cells[i].id){idx=i;break;}}
if(idx==0)return;this.cells.splice(idx,1);this.cells.splice(idx-1,0,cell);this.layoutCells();cell.focus();},moveCellDown:function(cell){var cells=[];var newCell=null;var idx=0;for(var i=0;i<this.cells.length;i++){if(cell.id==this.cells[i].id){idx=i;break;}}
if(idx==this.cells.length-1)return;this.cells.splice(idx,1);this.cells.splice(idx+1,0,cell);this.layoutCells();cell.focus();},newCellAbove:function(cell){var cells=[];var newCell=null;for(var i=0;i<this.cells.length;i++){if(cell.id==this.cells[i].id){newCell=this.addCell(null,{showEdit:true,showHeader:false});cells.push(newCell);}
cells.push(this.cells[i]);}
this.cells=cells;this.layoutCells();newCell.focus();},newCellBelow:function(cell){var cells=[];var newCell=null;for(var i=0;i<this.cells.length;i++){cells.push(this.cells[i]);if(cell.id==this.cells[i].id){newCell=this.addCell(null,{showEdit:true,showHeader:false});cells.push(newCell);}}
this.cells=cells;this.layoutCells();newCell.focus();},deleteCell:function(cell){cell.jq(ID_CELL).remove();var cells=[];for(var i=0;i<this.cells.length;i++){if(cell.id!=this.cells[i].id){cells.push(this.cells[i]);}}
this.cells=cells;if(this.cells.length==0){this.cells.push(this.addCell());}},processCells:function(){for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];if(!cell.processCell()){break;}}},toggleAll:function(on){for(var i=0;i<this.cells.length;i++){var cell=this.cells[i];cell.showEdit=on;cell.applyStyle();}},});}
function nbClear(){notebookState.cell.notebook.clearOutput();}
function nbStop(){notebookState.stop=true;}
var notebookState={cell:null,stop:false,displayNotebookResult:null,}
function RamaddaNotebookCell(notebook,id,content,props){this.notebook=notebook;var ID_CELL="cell";var ID_GUTTER="gutter";var ID_INPUT_GUTTER="inputgutter";var ID_OUTPUT_GUTTER="inputgutter";var ID_HEADER="header";var ID_INPUT="input";var ID_OUTPUT="output";var ID_MESSAGE="message";var ID_MENU_BUTTON="menubutton";var ID_MENU="menu";var ID_CELLNAME_INPUT="cellnameinput";var ID_SHOWHEADER_INPUT="showheader";var ID_SHOWBORDER_INPUT="showborder";var ID_SHOWEDIT_INPUT="showedit";var ID_PINNED_INPUT="pinned";let SUPER=new DisplayThing(id,{});RamaddaUtil.inherit(this,SUPER);RamaddaUtil.defineMembers(this,{id:id,inputRows:1,content:content,outputHtml:"",pinned:false,showEdit:false,showHeader:false,showBorder:false,cellName:"",});if(props)
$.extend(this,props);RamaddaUtil.defineMembers(this,{getJson:function(){return{id:this.id,inputRows:this.inputRows,content:this.content,outputHtml:this.outputHtml,pinned:this.pinned,showEdit:this.showEdit,showHeader:this.showHeader,showBorder:this.showBorder,cellName:this.cellName,}},createCell:function(){if(this.content==null)this.content="html:";var _this=this;var header=HtmlUtils.div([ATTR_CLASS,"display-notebook-header",ATTR_ID,this.getDomId(ID_HEADER)],this.cellName);var input=HtmlUtils.textarea(TAG_INPUT,this.content,["rows",this.inputRows,ATTR_CLASS,"display-notebook-input",ATTR_ID,this.getDomId(ID_INPUT)]);input=HtmlUtils.div(["class","display-notebook-input-container"],input);var output=HtmlUtils.div([ATTR_CLASS,"display-notebook-output",ATTR_ID,this.getDomId(ID_OUTPUT)],this.outputHtml);output=HtmlUtils.div(["class","display-notebook-output-container"],output);var menu=HtmlUtils.div(["id",this.getDomId(ID_MENU),"class","ramadda-popup"],"");var gutter=HtmlUtils.div(["class","display-notebook-gutter",ATTR_ID,this.getDomId(ID_INPUT_GUTTER)],menu+
HtmlUtils.div(["class","display-notebook-menu-button","id",this.getDomId(ID_MENU_BUTTON)],HtmlUtils.image(icon_menu,[])));var table=HtmlUtils.openTag("table",["cellspacing","0","cellpadding","0","width","100%","border","0"]);table+=HtmlUtils.tr(["valign","top"],HtmlUtils.td(["rowspan","3","width","5","class","display-notebook-gutter-container"],gutter)+
HtmlUtils.td([],header));table+=HtmlUtils.tr(["valign","top"],"\n"+HtmlUtils.td([],input));table+=HtmlUtils.tr(["valign","top"],"\n"+HtmlUtils.td([],output));table+="</table>";var html=table;html=HtmlUtils.div(["id",this.getDomId(ID_CELL)],html);$("#"+this.id).html(html);this.header=this.jq(ID_HEADER);this.menuButton=this.jq(ID_MENU_BUTTON);this.cell=this.jq(ID_CELL);this.input=this.jq(ID_INPUT);this.output=this.jq(ID_OUTPUT);this.inputContainer=this.cell.find(".display-notebook-input-container");this.applyStyle();this.menuButton.click(function(){var space="&nbsp;&nbsp;";var menu="";menu+=HtmlUtils.input(ID_CELLNAME_INPUT,_this.cellName,["placeholder","Cell name","style","width:100%;","xsize","20","id",_this.getDomId(ID_CELLNAME_INPUT)]);menu+="<br>";menu+="<table>";menu+="<tr><td align=right><b>Run:</b>&nbsp;</td><td>";menu+=HtmlUtils.div(["title","shift-return","class","ramadda-link","what","run"],"This cell")+space;menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","runall"],"All");menu+="</td></tr>"
menu+="<tr valign=top><td align=right><b>Show:</b>&nbsp;</td><td>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_SHOWEDIT_INPUT),[],_this.showEdit)+" Edit";menu+=space;menu+=HtmlUtils.checkbox(_this.getDomId(ID_SHOWHEADER_INPUT),[],_this.showHeader)+" Header";menu+="<br>";menu+=HtmlUtils.checkbox(_this.getDomId(ID_SHOWBORDER_INPUT),[],_this.showBorder)+" Border";menu+=space;menu+=HtmlUtils.checkbox(_this.getDomId(ID_PINNED_INPUT),[],_this.pinned)+" Left side";menu+="</td></tr>"
menu+="<tr><td align=right><b>Edit:</b>&nbsp;</td><td>";menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","showall"],"Show All");menu+=space;menu+=HtmlUtils.div(["title","ctrl-return","class","ramadda-link","what","hideall"],"Hide All");menu+="</td></tr>"
menu+="<tr><td align=right><b>Clear:</b>&nbsp;</td><td>";menu+=HtmlUtils.div(["class","ramadda-link","what","clear"],"This cell")+space;menu+=HtmlUtils.div(["class","ramadda-link","what","clearall"],"All");menu+="</td></tr>"
...
menu+="<tr><td align=right><b>New Cell:</b>&nbsp;</td><td>";menu+=HtmlUtils.div(["class","ramadda-link","what","newabove"],"Above")+space;menu+=HtmlUtils.div(["class","ramadda-link","what","newbelow"],"Below");menu+="</td></tr>"
menu+="</table>";menu+=HtmlUtils.div(["class","ramadda-link","what","delete"],"Delete")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","save"],"Save Notebook")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","help"],"Help")+"<br>";var popup=_this.jq(ID_MENU);popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();popup.position({of:_this.menuButton,my:"left top",at:"right top",collision:"fit fit"});_this.jq(ID_SHOWHEADER_INPUT).focus();_this.jq(ID_SHOWHEADER_INPUT).change(function(e){_this.showHeader=_this.jq(ID_SHOWHEADER_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_SHOWBORDER_INPUT).change(function(e){_this.showBorder=_this.jq(ID_SHOWBORDER_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_SHOWEDIT_INPUT).change(function(e){_this.showEdit=_this.jq(ID_SHOWEDIT_INPUT).is(':checked');_this.applyStyle();});_this.jq(ID_PINNED_INPUT).change(function(e){_this.pinned=_this.jq(ID_PINNED_INPUT).is(':checked');_this.checkMenuButton(false);});_this.jq(ID_CELLNAME_INPUT).keypress(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){_this.cellName=$(this).val();_this.applyStyle();popup.hide();return;}});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");popup.hide();if(what=="run"){_this.processCell();}else if(what=="showthis"){_this.showEdit=true;_this.applyStyle();}else if(what=="hidethis"){_this.showEdit=false;_this.applyStyle();}else if(what=="showall"){_this.notebook.toggleAll(true);}else if(what=="hideall"){_this.notebook.toggleAll(false);}else if(what=="runall"){_this.notebook.processCells();}else if(what=="clear"){_this.clearOutput();}else if(what=="clearall"){_this.notebook.clearOutput();}else if(what=="moveup"){_this.notebook.moveCellUp(_this);}else if(what=="movedown"){_this.notebook.moveCellDown(_this);}else if(what=="newabove"){_this.notebook.newCellAbove(_this);}else if(what=="newbelow"){_this.notebook.newCellBelow(_this);}else if(what=="save"){_this.notebook.saveNotebook();}else if(what=="help"){var win=window.open(ramaddaBaseUrl+"/userguide/wikidisplay.html#notebook",'_blank');win.focus();}else if(what=="delete"){_this.askDelete();}});});var hoverIn=function(){_this.checkMenuButton(true);}
var hoverOut=function(){_this.jq(ID_MENU).hide();_this.checkMenuButton(false);}
this.cell.hover(hoverIn,hoverOut);this.calculateInputHeight();this.input.on('input selectionchange propertychange',function(){_this.calculateInputHeight();});this.input.keydown(function(e){var key=e.key;if(key=='v'&&e.ctrlKey){_this.notebook.moveCellDown(_this);return;}
if(key==6&&e.ctrlKey){_this.notebook.moveCellUp(_this);return;}});this.input.keypress(function(e){var key=e.key;if(key=='Enter'){if(e.shiftKey){if(e.preventDefault){e.preventDefault();}
_this.processCell();}else if(e.ctrlKey){if(e.preventDefault){e.preventDefault();}
_this.notebook.processCells();}}});},checkMenuButton:function(vis){if(vis||this.pinned){this.menuButton.css("display","block");this.cell.find(".display-notebook-gutter-container").css("background","rgb(250,250,250)");this.cell.find(".display-notebook-gutter-container").css("border-right","1px #ccc solid");}else{this.menuButton.css("display","none");this.cell.find(".display-notebook-gutter-container").css("background","#fff");this.cell.find(".display-notebook-gutter-container").css("border-right","1px #fff solid");}},applyStyle:function(){this.checkMenuButton(false);if(this.showHeader){this.header.css("display","block");}else{this.header.css("display","none");}
this.header.html(this.cellName);if(this.showEdit){this.inputContainer.css("display","block");}else{this.inputContainer.css("display","none");}
if(this.showBorder){this.cell.css("border-top","1px #ccc solid");}else{this.cell.css("border-top","1px #fff solid");}},askDelete:function(){let _this=this;var menu="";menu+="You sure you want to delete this cell?<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","yes"],"Yes")+"<br>";menu+=HtmlUtils.div(["class","ramadda-link","what","cancel"],"No");var popup=_this.jq(ID_MENU);popup.html(HtmlUtils.div(["class","ramadda-popup-inner"],menu));popup.show();popup.position({of:this.menuButton,my:"left top",at:"right top",collision:"fit fit"});popup.find(".ramadda-link").click(function(){var what=$(this).attr("what");popup.hide();if(what=="yes"){_this.notebook.deleteCell(_this);}});},processCell:function(){var value=this.input.val();value=value.trim();var help="More information <a target=_help href='"+ramaddaBaseUrl+"/userguide/wikidisplay.html#notebook'>here</a>";value=value.replace(/{cellname}/g,this.cellName);value=value.replace(/{help}/g,help);var result="";var type="html";var blob="";var commands=value.split("\n");var types=["html","wiki","sh","js"];var ok=true;for(var i=0;i<commands.length;i++){if(!ok)break;value=commands[i].trim();var isType=false;for(var typeIdx=0;typeIdx<types.length;typeIdx++){var t=types[typeIdx];if(value.startsWith(t+":")){var r={ok:true}
var blobResult=this.processBlob(type,blob,r);if(blobResult){if(blobResult!=""){blobResult+="<br>";}
result+=blobResult;}
ok=r.ok;blob=value.substring((t+":").length);if(blob!="")blob+="\n";type=t;isType=true;break;}}
if(isType)continue;blob=blob+value+"\n";}
if(ok){var r={ok:true}
var blobResult=this.processBlob(type,blob,r);if(blobResult){if(blobResult!=""){blobResult+="<br>";}
result+=blobResult;}
ok=r.ok;}
this.outputHtml=result;this.output.html(result);return ok;},outputUpdated:function(){this.outputHtml=this.jq(ID_OUTPUT).html();},focus:function(){this.input.focus();},clearOutput:function(){this.output.html("");this.outputHtml="";},processHtml:function(blob,result){blob=blob.trim();blob=blob.replace(/\n/g,"<br>");return blob;},processWiki:function(blob,result){var id=this.notebook.getProperty("entryId","");var entry=this.getCurrentEntry(null);if(entry)id=entry.getId();let _this=this;let divId=HtmlUtils.getUniqueId();var callback=function(html){$("#"+divId).html(html);Utils.initContent("#"+_this.getDomId(ID_OUTPUT));_this.outputUpdated();}
blob="{{group showMenu=false}}\n"+blob;GuiUtils.loadHtml(ramaddaBaseUrl+"/wikify?doImports=false&entryid="+id+"&text="+encodeURIComponent(blob),callback);var h=HtmlUtils.div(["id",divId],"Processing...");return h;},processSh:function(blob,result){var r="";var lines=blob.split("\n");for(var i=0;i<lines.length;i++){var line=lines[i];if(line=="")continue;var toks=line.split(" ");var command=toks[0].trim();if(this["processCommand_"+command]){r+=this["processCommand_"+command](line,toks);r+="<br>";}else{r+=this.processCommand_help(line,toks,"Unknown command: <i>"+command+"</i>");r+="<br>";}}
return r;},processJs:function(blob,result){try{notebookState.stop=false;notebookState.cell=this;notebookState.displayNotebookResult=null;var js="function nbFunc() {\n"+blob+"\n}\nnotebookState.displayNotebookResult = nbFunc();";var r=eval(js);if(notebookState.stop){result.ok=false;}
if(!notebookState.displayNotebookResult)return"";return notebookState.displayNotebookResult;}catch(e){result.ok=false;var lines=blob.split("\n");var line=lines[e.lineNumber-2];return"Error: "+e.message+"<br>"+HtmlUtils.span(["class","display-notebook-error"]," &gt; "+line);}},processBlob:function(type,blob,result){if(blob==""){return"";}
if(type=="html"){return this.processHtml(blob,result);}else if(type=="wiki"){return this.processWiki(blob,result);}else if(type=="js"){return this.processJs(blob,result);}else{return this.processSh(blob,result);}
return"unknown:"+blob;},calculateInputHeight:function(){this.content=this.input.val();var lines=this.content.split("\n");if(lines.length!=this.inputRows){this.inputRows=lines.length;this.input.attr("rows",Math.max(1,this.inputRows));}},writeStatusMessage:function(v){var msg=this.jq(ID_MESSAGE);if(!v){msg.hide();msg.html("");}else{msg.show();msg.position({of:this.getOutput(),my:"left top",at:"left+4 top+4",collision:"none none"});msg.html(v);}},handleControlKey:function(event){var k=event.which;},writeInput:function(v){var input=this.getInput();if(input.val()==v){return;}
input.val(v).focus();this.currentInput=this.getInput().val();},getOutput:function(){return this.jq(ID_OUTPUT);},getInput:function(){return this.jq(ID_INPUT);},writeResult:function(html){this.writeStatusMessage(null);html=HtmlUtils.div([ATTR_CLASS,"display-notebook-result"],html);var output=this.jq(ID_OUTPUT);output.append(html);output.animate({scrollTop:output.prop("scrollHeight")},1000);this.currentOutput=output.html();this.currentInput=this.getInput().val();},writeError:function(msg){this.writeStatusMessage(msg);},header:function(msg){return HtmlUtils.div([ATTR_CLASS,"display-notebook-header"],msg);},processCommand_help:function(line,toks,prefix){var help="";if(prefix!=null)help+=prefix;help+="<pre>pwd, ls, cd</pre>";return help;},entries:{},cdEntry:function(entryId){this.currentEntry=this.entries[entryId];this.output.html(this.processCommand_pwd("pwd",[]));this.outputUpdated();},getEntryPrefix:function(id,entry){this.entries[entry.getId()]=entry;var call="getHandler('"+id+"').cdEntry('"+entry.getId()+"')";return HtmlUtils.div(["style","padding-right:4px;","title","cd to entry","onclick",call,"class","ramadda-link"],HtmlUtils.image(ramaddaBaseUrl+"/icons/go.png"));},displayEntries:function(entries,divId){this.currentEntries=entries;var handlerId=addHandler(this);var html=this.notebook.getEntriesTree(entries,{handlerId:handlerId,showIndex:false,suffix:"_shell_"+(this.uniqueCnt++)});$("#"+divId).html(html);this.outputUpdated();},getEntryFromArgs:function(args,dflt){var currentEntries=this.currentEntries;if(currentEntries==null){return dflt;}
...
return dflt;},getCurrentEntry:function(dflt){if(this.currentEntry==null){this.currentEntry=this.notebook.currentEntry;}
...
return this.currentEntry;},processCommand_pwd:function(line,toks){var entry=this.getCurrentEntry();var icon=entry.getIconImage([ATTR_TITLE,"View entry"]);var html=this.notebook.getEntriesTree([entry],{suffix:"_YYY"});return"Current entry:<br>"+html;},createPointDisplay:function(line,toks,displayType){var entry=this.getEntryFromArgs(toks,this.getCurrentEntry());console.log("createDisplay got:"+entry.getName());var jsonUrl=this.getPointUrl(entry);if(jsonUrl==null){this.writeError("Not a point type:"+entry.getName());return;}
this.createDisplay(entry.getId(),displayType,jsonUrl);},processCommand_table:function(line,toks){this.createPointDisplay(line,toks,DISPLAY_TABLE);},processCommand_linechart:function(line,toks){this.createPointDisplay(line,toks,DISPLAY_LINECHART);},processCommand_barchart:function(line,toks){this.createPointDisplay(line,toks,DISPLAY_BARCHART);},processCommand_bartable:function(line,toks){this.createPointDisplay(line,toks,DISPLAY_BARTABLE);},processCommand_hello:function(line,toks){this.writeResult("Hello, how are you?");},processCommand_scatterplot:function(line,toks){this.createPointDisplay(line,toks,DISPLAY_SCATTERPLOT)},processCommand_blog:function(line,toks){this.getLayoutManager().publish('blogentry');},processCommand_cd:function(line,toks){let _this=this;console.log("cd:"+toks);if(toks.length<=1){this.currentEntry=this.notebook.currentEntry;return this.processCommand_pwd("pwd",[]);}
if(toks[1]==".."){var entry=this.getCurrentEntry();this.currentEntry=entry.getParentEntry(entry=>{console.log(entry);if(entry){_this.currentEntry=entry;_this.processCommand_cd(line,toks)}});if(this.currentEntry){return this.processCommand_pwd("pwd",[]);}
return"Retrieving entry...";}
return"NA";},processCommand_ls:function(line,toks){let _this=this;let divId=HtmlUtils.getUniqueId();var callback=function(children){_this.displayEntries(children,divId);};var children=this.getCurrentEntry().getChildrenEntries(callback,"");if(children!=null){setTimeout(function(){_this.displayEntries(children,divId);},1);}
return HtmlUtils.div(["style","max-height:200px;overflow-y:auto;","id",divId],"Listing entries...");},entryListChanged:function(entryList){var entries=entryList.getEntries();if(entries.length==0){this.writeStatusMessage("Sorry, nothing found");}else{this.displayEntries(entries);}},processCommand_search:function(line,toks){var text="";for(var i=1;i<toks.length;i++)text+=toks[i]+" ";text=text.trim();var searchSettings=new EntrySearchSettings({text:text,});var repository=this.getRamadda();this.writeStatusMessage("Searching...");var jsonUrl=repository.getSearchUrl(searchSettings,OUTPUT_JSON);this.entryList=new EntryList(repository,jsonUrl,this,true);},processCommand_clear:function(toks){this.clearOutput();},});}
...
dataTable.addRows(justData);return dataTable;},makeChartOptions:function(dataList,props,selectedFields){var chartOptions={tooltip:{isHtml:true},};$.extend(chartOptions,{lineWidth:1,colors:this.colorList,curveType:this.curveType,vAxis:{}});if(this.lineWidth){chartOptions.lineWidth=this.lineWidth;}
if(this.fontSize>0){chartOptions.fontSize=this.fontSize;}
var defaultRange=this.getDisplayManager().getRange(selectedFields[0]);var range=[NaN,NaN];if(!isNaN(this.vAxisMinValue)){range[0]=parseFloat(this.vAxisMinValue);}else if(defaultRange!=null){range[0]=defaultRange[0];}
if(!isNaN(this.vAxisMaxValue)){range[1]=parseFloat(this.vAxisMaxValue);}else if(defaultRange!=null){range[1]=defaultRange[1];}
if(!isNaN(range[0])){chartOptions.vAxis.minValue=range[0];}
if(!isNaN(range[1])){chartOptions.vAxis.maxValue=range[1];}
this.chartDimensions={width:"90%",left:"10%"}
...
this.setContents(this.getChartDiv());return chartOptions;},getChartDiv:function(){var chartId=this.getDomId(ID_CHART);var divAttrs=[ATTR_ID,chartId];divAttrs.push("style");divAttrs.push("height:100%;width:100%;");return HtmlUtils.div(divAttrs,"");},makeGoogleChart:function(dataList,props,selectedFields){if(typeof google=='undefined'){this.setContents("No google");return;}
this.chartOptions=this.makeChartOptions(dataList,props,selectedFields);this.chart=this.doMakeGoogleChart(dataList,props,selectedFields,this.chartOptions);if(this.chart!=null){var dataTable=this.makeDataTable(dataList,props,selectedFields);if(!dataTable)return;if(!Utils.isDefined(this.chartOptions.height)){this.chartOptions.height="100%";}
...
if(!isNaN(this.vAxisMaxValue)){chartOptions.vAxis.maxValue=parseFloat(this.vAxisMaxValue);}
if(!isNaN(this.vAxisMinValue)){chartOptions.vAxis.minValue=parseFloat(this.vAxisMinValue);}
...
return this.groupBy;},getChartDiv:function(){var chartId=this.getDomId(ID_CHART);var divAttrs=[ATTR_ID,chartId];divAttrs.push("style");var style="";if(this.getProperty("width")){var width=this.getProperty("width");if(width>0)
style+="width:"+width+"px;";else if(width<0)
style+="width:"+(-width)+"%;";else
style+="width:"+width;}else{style+="width:"+"100%;";}
if(this.getProperty("height")){var height=this.getProperty("height");if(height>0)
...
return"<div class='display-treemap-tooltip-outer'><div class='display-treemap-tooltip''><i>left-click: go down<br>right-click: go up</i></div></div>";return null;};var chartOptions=SUPER.makeChartOptions.call(this,dataList,props,selectedFields);$.extend(chartOptions,{highlightOnMouseOver:true,generateTooltip:tooltip,maxDepth:parseInt(this.getProperty("maxDepth",2)),maxPostDepth:parseInt(this.getProperty("maxPostDepth",3)),});return chartOptions;},defaultSelectedToAll:function(){return true;},doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){var dataTable=this.makeDataTable(dataList,props,selectedFields);if(!dataTable)return;return new google.visualization.TreeMap(document.getElementById(this.getChartId()));},addTuple:function(data,colorField,seen,value,parent,n1,n2){var ovalue=value;var cnt=0;if(Utils.isDefined(seen[value])&&parent){value=parent+":"+value;}
...
dataTable.addRows(list);return dataTable;}});}
function GaugeDisplay(displayManager,id,properties){RamaddaUtil.inherit(this,new RamaddaGoogleChart(displayManager,id,DISPLAY_GAUGE,properties));addRamaddaDisplay(this);RamaddaUtil.inherit(this,{doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){this.dataList=dataList;this.chartOptions=chartOptions;var min=Number.MAX_VALUE;var max=Number.MIN_VALUE;var setMinMax=true;for(var row=1;row<dataList.length;row++){var tuple=this.getDataValues(dataList[row]);for(var col=0;col<tuple.length;col++){if(!Utils.isNumber(tuple[col])){continue;}
...
function ScatterplotDisplay(displayManager,id,properties){RamaddaUtil.inherit(this,new RamaddaGoogleChart(displayManager,id,DISPLAY_SCATTERPLOT,properties));$.extend(this,{doMakeGoogleChart:function(dataList,props,selectedFields,chartOptions){var height=400;if(Utils.isDefined(this.chartHeight)){height=this.chartHeight;}
var width="100%";if(Utils.isDefined(this.chartWidth)){width=this.chartWidth;}
var chartId=this.getChartId();$("#"+chartId).css("width",width);$("#"+chartId).css("height",height);this.chartOptions={title:'',tooltip:{isHtml:true},legend:'none',chartArea:{left:"10%",top:10,height:"80%",width:"90%"}};if(this.getShowTitle()){this.chartOptions.title=this.getTitle(true);}
if(dataList.length>0&&this.getDataValues(dataList[0]).length>1){this.chartOptions.hAxis={title:this.getDataValues(dataList[0])[0]};this.chartOptions.vAxis={title:this.getDataValues(dataList[0])[1]};if(!isNaN(this.vAxisMinValue)){chartOptions.hAxis.minValue=this.vAxisMinValue;chartOptions.vAxis.minValue=this.vAxisMinValue;}
if(!isNaN(this.vAxisMaxValue)){chartOptions.hAxis.maxValue=this.vAxisMaxValue;chartOptions.vAxis.maxValue=this.vAxisMaxValue;}}
return new google.visualization.ScatterChart(document.getElementById(this.getChartId()));},getDefaultSelectedFields:function(fields,dfltList){var f=[];for(i=0;i<fields.length;i++){var field=fields[i];if(field.isNumeric){f.push(field);if(f.length>=2)
break;}}
...
right=HtmlUtils.tds(["align","right"],values);var align=(justOne?"right":"left");var label=field.getLabel();var toks=label.split("!!");var title=field.getId();label=toks[toks.length-1];if(justOne){label+=":";}
label=label.replace(/ /g,"&nbsp;")
var row=HtmlUtils.tr([],HtmlUtils.td(["align",align],"<b>"+HtmlUtils.tag("div",["title",title],label)+"</b>")+right);if(justOne){html+=row;}else{html+=row;}}
...
var dataList=this.getStandardData(null,{includeIndex:false});var allFields=this.dataCollection.getList()[0].getRecordFields();var fields=this.getSelectedFields([]);if(fields.length==0)fields=allFields;var html=HtmlUtils.openTag("table",["border","0","class","display-correlation"]);html+="<tr valign=bottom><td class=display-heading>&nbsp;</td>";for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field1=fields[fieldIdx];if(!field1.isFieldNumeric()||field1.isFieldGeo())continue;html+="<td align=center class=top-heading>"+HtmlUtils.tag("div",["class","top-heading"],field1.getLabel())+"</td>";}
html+="</tr>";var colors=null;colorByMin=parseFloat(this.colorByMin);colorByMax=parseFloat(this.colorByMax);colors=this.getColorTable(true);var colCnt=0;for(var fieldIdx1=0;fieldIdx1<fields.length;fieldIdx1++){var field1=fields[fieldIdx1];if(!field1.isFieldNumeric()||field1.isFieldGeo())continue;colCnt++;html+="<tr><td>"+HtmlUtils.tag("div",["class","side-heading"],field1.getLabel().replace(/ /g,"&nbsp;"))+"</td>";var rowName=field1.getLabel();for(var fieldIdx2=0;fieldIdx2<fields.length;fieldIdx2++){var field2=fields[fieldIdx2];if(!field2.isFieldNumeric()||field2.isFieldGeo())continue;var colName=field2.getLabel();var t1=0;var t2=0;var cnt=0;for(var rowIdx=1;rowIdx<dataList.length;rowIdx++){var tuple=this.getDataValues(dataList[rowIdx]);var v1=tuple[field1.getIndex()];var v2=tuple[field2.getIndex()];t1+=v1;t2+=v2;cnt++;}
...
html+="</tr>";}
html+="<tr><td></td><td colspan = "+colCnt+">"+HtmlUtils.div(["id",this.getDomId(ID_BOTTOM)],"")+"</td></tr>";html+="</table>";this.setContents(html);this.displayColorTable(colors,ID_BOTTOM,colorByMin,colorByMax);this.initTooltip();this.displayManager.propagateEventRecordSelection(this,this.dataCollection.getList()[0],{index:0});},});}
...
html+=HtmlUtils.openTag("table",["border","0","class","display-heatmap"]);html+="<tr valign=bottom>";if(showIndex){html+="<td align=center class=top-heading>"+HtmlUtils.tag("div",["class","top-heading"],header[0])+"</td>";}
for(var fieldIdx=0;fieldIdx<fields.length;fieldIdx++){var field=fields[fieldIdx];if((!field.isFieldNumeric()||field.isFieldGeo()))continue;html+="<td align=center class=top-heading>"+HtmlUtils.tag("div",["class","top-heading"],field.getLabel())+"</td>";}
...
var rowLabel=index;html+="<tr valign='center'>\n";if(showIndex){html+=HtmlUtils.td(["class","side-heading","style",extraCellStyle],rowLabel);}
...
RamaddaUtil.defineMembers(this,{searchSettings:new EntrySearchSettings({parent:properties.entryParent,provider:properties.provider,text:properties.entryText,entryType:properties.entryType,orderBy:properties.orderBy}),entryList:properties.entryList,entryMap:{},getSearchSettings:function(){if(this.providers!=null){var provider=this.searchSettings.provider;var fromSelect=this.jq(ID_PROVIDERS).val();if(fromSelect!=null){provider=fromSelect;}
...
if(!this.haveSearched){if(this.doSearch){this.submitSearchForm();}}},showEntryDetails:function(event,entryId,src,leftAlign){if(true)return;var entry=this.getEntry(entryId);var popupId="#"+this.getDomId(ID_DETAILS+entryId);if(this.currentPopupEntry==entry){this.hideEntryDetails(entryId);return;}
...
var html="";html+=HtmlUtils.openTag(TAG_TABLE,[ATTR_CLASS,"display-metadata-table",ATTR_WIDTH,"100%","cellpadding","5","cellspacing","0"]);var type=this.findEntryType(this.searchSettings.entryType);var typeName="Entry";if(type!=null){typeName=type.getLabel();}
...
var skip={"content.pagestyle":true,"content.pagetemplate":true,"content.sort":true,"spatial.polygon":true,};var headerItems=[];headerItems.push(HtmlUtils.th([ATTR_CLASS,"display-metadata-table-cell"],HtmlUtils.b(typeName)));for(var i=0;i<mdts.length;i++){var type=mdts[i];if(skip[type]){continue;}
var label=mdtmap[mdts[i]];if(label==null)label=mdts[i];headerItems.push(HtmlUtils.th([ATTR_CLASS,"display-metadata-table-cell"],HtmlUtils.b(label)));}
var headerRow=HtmlUtils.tr(["valign","bottom"],HtmlUtils.join(headerItems,""));html+=headerRow;var divider="<div class=display-metadata-divider></div>";var missing=this.missingMessage;if(missing=null)missing="&nbsp;";for(var entryIdx=0;entryIdx<entries.length;entryIdx++){var entry=entries[entryIdx];var metadata=entry.getMetadata();var row=[];var buttonId=this.getDomId("entrylink"+entry.getIdForDom());var link=entry.getLink(entry.getIconImage()+" "+entry.getName());row.push(HtmlUtils.td([ATTR_CLASS,"display-metadata-table-cell"],HtmlUtils.div([ATTR_CLASS,"display-metadata-entrylink"],link)));for(var mdtIdx=0;mdtIdx<mdts.length;mdtIdx++){var mdt=mdts[mdtIdx];if(skip[mdt]){continue;}
...
html+=HtmlUtils.closeTag(TAG_TABLE);this.jq(ID_ENTRIES).html(html);},});}
...
function RamaddaEntrydisplayDisplay(displayManager,id,properties){var SUPER;$.extend(this,{sourceEntry:properties.sourceEntry});RamaddaUtil.inherit(this,SUPER=new RamaddaDisplay(displayManager,id,DISPLAY_ENTRYDISPLAY,properties));if(properties.sourceEntry==null&&properties.entryId!=null){let _this=this;var callback=function(entry){_this.sourceEntry=entry;_this.initDisplay();}
...
props.editMode=true;if(type==DISPLAY_LABEL&&props.text==null){var text=prompt("Text");if(text==null)return;props.text=text;}
...
return;}
if(entry.getType().getId()=="geo_shapefile"){var bounds=createBounds(entry.getWest(),entry.getSouth(),entry.getEast(),entry.getNorth());if(bounds.left<-180||bounds.right>180||bounds.bottom<-90||bounds.top>90){console.log("entry has bad bounds:"+entry.getName()+" "+bounds);return;}
var selectCallback=function(layer){_this.handleLayerSelect(layer);}
var unselectCallback=function(layer){_this.handleLayerUnselect(layer);}
var url=ramaddaBaseUrl+"/entry/show?output=shapefile.kml&entryid="+entry.getId();var layer=this.map.addKMLLayer(entry.getName(),url,true,selectCallback,unselectCallback);this.addedLayers[entry.getId()]=layer;bounds=this.map.transformLLBounds(bounds);this.map.map.zoomToExtent(bounds,true);return;}
...
return null;return[lat,lon];},setInitMapBounds:function(north,west,south,east){if(!this.map)return;this.map.centerOnMarkers(new OpenLayers.Bounds(west,south,east,north));},sourceToEntries:{},handleEventEntriesChanged:function(source,entries){if(!this.map){return;}
...
},addOrRemoveEntryMarker:function(id,entry,add,args){console.log("entry marker");if(!args){args={};}
...
return;var pointData=this.getPointData();var records=this.filterData();if(records==null){err=new Error();console.log("null records:"+err.stack);return;}
...
sizeBy.index=sizeBy.field!=null?sizeBy.field.getIndex():-1;colorBy.index=colorBy.field!=null?colorBy.field.getIndex():-1;var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);for(var i=0;i<points.length;i++){var pointRecord=records[i];var tuple=pointRecord.getData();var v=tuple[colorBy.index];if(excludeZero&&v==0){continue;}
