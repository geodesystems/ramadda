...
_this.displayColorTable(colors,ID_COLORTABLE,this.origMinValue,this.origMaxValue,{stringValues:this.colorByValues});}else{_this.displayColorTable(this.colors,ID_COLORTABLE,this.origMinValue,this.origMaxValue,{stringValues:this.colorByValues});}},getColor:function(value,pointRecord){var percent=0;if(this.showPercent){var total=0;var data=pointRecord.getData();for(var j=0;j<data.length;j++){var ok=this.fields[j].isNumeric&&!this.fields[j].isFieldGeo();if(ok&&this.pctFields!=null){ok=this.pctFields.indexOf(this.fields[j].getId())>=0||this.pctFields.indexOf("#"+(j+1))>=0;}
if(ok){total+=data[j];}}
if(total!=0){percent=percent=value/total*100;percent=(percent-this.minValue)/(this.maxValue-this.minValue);}}else{var v=value;if(this.isString){v=this.colorByMap[v];}
...
colorBy.index=colorBy.field!=null?colorBy.field.getIndex():-1;colorBy.stringMap=this.getColorByMap();if(colorBy.index>=0){records.map(record=>{var tuple=record.getData();var v=tuple[colorBy.index];if(colorBy.isString){if(!Utils.isDefined(colorBy.colorByMap[v])){colorBy.colorByValues.push(v);colorBy.colorByMap[v]=colorBy.colorByValues.length;colorBy.minValue=1;colorBy.maxValue=colorBy.colorByValues.length;}}
if(excludeZero&&v===0){return;}
if(!colorBy.isString){if(!isNaN(v)&&!(v===null)){if(i==0||v>colorBy.maxValue)colorBy.maxValue=v;if(i==0||v<colorBy.minValue)colorBy.minValue=v;}}});}
...
enums=toks;}
var includeAll=this.getProperty(filterField.getId()+".includeAll",true);if(enums==null){var allName=this.getProperty(filterField.getId()+".allName","All");enums=includeAll?[[FILTER_ALL,allName]]:[];var enumValues=[];var seen={};records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(!seen[value]){seen[value]=true;var label=value;if(label.length>20){label=label.substring(0,19)+"...";}
if(typeof value=="string")
value=value.replace(/\'/g,"&apos;");var tuple=[value,label];enumValues.push(tuple);}});enumValues.sort((a,b)=>{return(""+a[1]).localeCompare(""+b[1]);});for(var j=0;j<enumValues.length;j++){var v=enumValues[j];enums.push(v);}}
...
var record=selected[rowIdx];var row=this.getDataValues(record);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);s=this.getRecordTemplate(row,fields,s,props);s=s.replace(/\${recordIndex}/g,(rowIdx+1));var tag=HtmlUtils.openTag("div",["style","","id",this.getId()+"-"+record.getId(),"title","","class","display-template-record","recordIndex",rowIdx]);if(s.startsWith("<td")){s=s.replace(/<td([^>]*)>/,"<td $1>"+tag);s=s.replace(/<\/td>$/,"</div></td>");contents+=s;}else if(s.startsWith("<tr")){s=s.replace(/<td([^>]*)>/g,"<td $1>"+tag);s=s.replace(/<\/td>/g,"</div></td>");contents+=s;}else{contents+=tag+s+"</div>"}
