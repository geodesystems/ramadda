...
function RamaddaDisplay(argDisplayManager,argId,argType,argProperties){let ID_PAGE_COUNT="pagecount";let ID_PAGE_PREV="pageprev";let ID_PAGE_NEXT="pagenext";RamaddaUtil.initMembers(this,{});let SUPER=new DisplayThing(argId,argProperties);RamaddaUtil.inherit(this,SUPER);this.getSuper=function(){return SUPER;}
...
header2+=HtmlUtils.span(["xstyle","display:inline-block;","id",this.getDomId(ID_HEADER2_SUFFIX)]);this.jq(ID_HEADER2).html(header2);this.initHeader2();var theDisplay=this;let macroChange=(macro,value,what)=>{if(this.settingMacroValue)return;this.reloadData();this.settingMacroValue=true;var args={entryId:this.entryId,property:"macroValue",id:macro.name,what:what,value:value};this.propagateEvent("handleEventPropertyChanged",args);this.settingMacroValue=false;};macroDateIds.every(id=>{HtmlUtils.datePickerInit(id);return true;});macros.every(macro=>{this.jq(macro.getId()).change(function(e){macroChange(macro,$(this).val());});this.jq(macro.getId()+"_min").change(function(e){macroChange(macro,$(this).val(),"min");});this.jq(macro.getId()+"_max").change(function(e){macroChange(macro,$(this).val(),"max");});this.jq(macro.getId()+"_from").change(function(e){macroChange(macro,$(this).val(),"from");});this.jq(macro.getId()+"_to").change(function(e){macroChange(macro,$(this).val(),"to");});return true;});let inputFunc=function(input,input2,value){var id=input.attr("id");if(!input2){if(id.endsWith("_min")){input2=$("#"+id.replace(/_min$/,"_max"));}else if(id.endsWith("_max")){var tmp=input;input=$("#"+id.replace(/_max$/,"_min"));input2=tmp;}}
...
this.setContents(this.getLoadingMessage());},handleNoData:function(pointData,reload){if(!reload){if(debug)console.log("\tno reload");this.addData(pointData);this.checkSearchBar();}else{if(!this.dataCollection)
...
$(this).css("border","1px solid #aaa");_this.haveCalledUpdateUI=false;_this.clipBounds=true;_this.updateUI();});},handleNoData:function(pointData,reload){this.addPoints([],[],[]);if(this.map)
...
this.map.setProgress("");},updateUI:function(reload){this.lastUpdateTime=null;SUPER.updateUI.call(this,reload);if(!this.getDisplayReady()){return;}
...
this.map.setProgress(HtmlUtils.div([ATTR_CLASS,"display-map-message"],msg));setTimeout(()=>{try{this.updateUIInner(pointData,records);}catch(exc){console.log(exc)
