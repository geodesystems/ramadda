...
return true;},getDataFilters:function(){return DataUtils.getDataFilters(this,this.getProperty("dataFilters"));},filterData:function(dataList,fields,doGroup,skipFirst){var startDate=this.getProperty("startDate");var endDate=this.getProperty("endDate");if(startDate){this.startDateObject=new Date(startDate);}
...
let dataFilters=this.getDataFilters();if(dataFilters.length){dataList=dataList.filter((r,idx)=>{if(!this.checkDataFilters(dataFilters,r))return false;return true;});}
...
let dataFilterIds=[];this.getDataFilters().map(f=>{if(!f.label||!f.field)return;let cbxid=this.getDomId("datafilterenabled_"+f.field.getId());dataFilterIds.push(cbxid);header2+=HtmlUtils.checkbox("",["id",cbxid],f.enabled)+" "+f.label+"&nbsp;&nbsp;"});if(this.getProperty("filterDate")){let type=this.getProperty("filterDate");var enums=[];if(this.getProperty("filterDateIncudeAll")){enums.push(["all","All"]);}
...
var DataUtils={parseCommands:function(commands){let result=[];if(!commands)return result;commands.split(";").map(command=>{command=command.trim();let toks=command.match(/([^\(]+)\(([^\)]*)\)/);let rest="";if(toks){command=toks[1];rest=toks[2];}
let args={};rest.split(",").map(arg=>{arg=arg.trim();let value="";let atoks=arg.match(/(.*)=(.*)/);if(atoks){arg=atoks[1];value=atoks[2];}
arg=arg.trim();value=value.trim();value=value.replace(/^'/g,"").replace(/'$/g,"");if(arg!=""){args[arg]=value;}});result.push({command:command,args:args});});return result;},getDataFilters:function(display,prop){let filters=[];if(!prop)return filters;DataUtils.parseCommands(prop).map(cmd=>{[type,fieldId,value,enabled,label]=[cmd.command,cmd.args.field,cmd.args.value,cmd.args.enabled,cmd.args.label];if(!Utils.isDefined(enabled))
enabled=true;else
enabled=enabled=="true";if(label){var cbx=display.jq("datafilterenabled_"+fieldId);if(cbx.length){enabled=cbx.is(':checked');}}
if(type=="match"||type=="notmatch")
value=new RegExp(value);else
value=+value;let field=display.getFieldById(null,fieldId);if(!field){return;}
