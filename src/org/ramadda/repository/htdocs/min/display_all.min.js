...
contents+=headerTemplate;for(var rowIdx=0;rowIdx<selected.length;rowIdx++){var row=this.getDataValues(selected[rowIdx]);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);for(var col=0;col<fields.length;col++){var f=fields[col];var value=row[f.getIndex()];if(f.getType()=="image"){if(value&&value.trim().length>1){var attrs=[];if(this.getProperty("imageWidth","")!=""){attrs.push("width");attrs.push(this.getProperty("imageWidth",""));}
