...
this.headers.push("");this.step=0;this.showBlocks();},showBlocks:function(){var contents="";if(this.step>this.counts.length)this.step=0;contents+=HtmlUtils.div(["class","display-block-header"],this.getProperty("header",""));contents+=HtmlUtils.openDiv(["class","display-blocks"]);var limit=this.step<this.counts.length-1?this.counts[this.step]:0;var tmp=this.getProperty("colors","red,blue,gray,green",true);var ct=(typeof tmp)=="string"?tmp.split(","):tmp;var footer="";while(ct.length<this.counts.length){ct.push(ct[ct.length-1]);}
var multiplier=parseFloat(this.getProperty("multiplier","1",true));var dim=this.getProperty("blockDimensions","8",true);for(var i=0;i<this.counts2.length;i++){var style="background:"+ct[i]+";"+"width:"+dim+"px;height:"+dim+"px;";var label=this.footers[i].replace("${count}",multiplier*this.counts[i]);footer+=HtmlUtils.div(["class","display-block","style",style],"")+" "+label+"&nbsp;&nbsp;";var cnt=this.counts2[i];for(var j=0;j<this.counts2[i];j++){contents+=HtmlUtils.div(["class","display-block","style",style,"title",label],"");}}
contents+=HtmlUtils.closeDiv();contents+=HtmlUtils.div(["class","display-block-footer"],footer);this.writeHtml(ID_DISPLAY_CONTENTS,contents);}})}
