...
function NotebookState(cell,div){this.id=HtmlUtils.getUniqueId();this.cell=cell;this.notebook=cell.notebook;$.extend(this,{entries:{},div:div,stopFlag:false,result:null,getStop:function(){return this.stopFlag;},getCell:function(){return this.cell;},setValue:function(name,value){this.notebook.setCellValue(name,value);},makeData:async function(entry){if(!entry)
...
if(entry==null)
await this.cell.getCurrentEntry(e=>entry=e);if((typeof entry)!="string")entry=entry.getId();await GuiUtils.loadHtml(ramaddaBaseUrl+"/wikify?doImports=false&entryid="+entry+"&text="+encodeURIComponent(s),callback);},write:function(s){this.div.append(s);},linechart:async function(entry,props){if(!entry)
...
chunk.div.set(HtmlUtils.div(["class","display-notebook-md"],html));},processPy:async function(chunk){await Utils.importJS(ramaddaBaseUrl+"/lib/skulpt.min.js");await Utils.importJS(ramaddaBaseUrl+"/lib/skulpt-stdlib.js");var output="";var outf=function(t){if(t.trim()!=""){output+=t.trim()+"<br>"}}
...
await cmd.proc.call(_this,cmd.line,cmd.toks,cmd.div,cmd.extra);}},processJs:async function(chunk){var lines;var topLines=0;try{await this.getCurrentEntry(e=>{current=e});var state=new NotebookState(this,chunk.div);notebookStates[state.id]=state;var notebookEntries=this.notebook.getCurrentEntries();for(name in notebookEntries){state.entries[name]=notebookEntries[name].entry;}
var jsSet="";state.entries["current"]=current;state.entries["parent"]=this.parentEntry;state.entries["base"]=this.notebook.getBaseEntry();state.entries["root"]=this.notebook.getRootEntry();var stateJS="notebookStates['"+state.id+"']";topLines++;jsSet+="var notebook= "+stateJS+";\n";for(name in state.entries){var e=state.entries[name];topLines++;jsSet+="var "+name+"= notebook.entries['"+name+"'];\n"}
for(name in this.notebook.globals){topLines++;jsSet+="var "+name+"= notebook.getNotebook().globals['"+name+"'];\n";}
var js=chunk.content.trim();lines=js.split("\n");js=jsSet+"\n"+js;var result=eval.call(null,js);if(result!=null)
console.log(result+"\n"+js);if(state.getStop()){chunk.ok=false;}
var html="";if(result!=null){var type=typeof result;if(type!="object"&&type!="function"){html=result;this.rawOutput+=html+"\n";}}
chunk.div.append(html);}catch(e){chunk.ok=false;var line=lines[e.lineNumber-topLines-2];this.notebook.log("Error: "+e.message+"<br>&gt;"+(line?line:""),"error","js",chunk.div);}
