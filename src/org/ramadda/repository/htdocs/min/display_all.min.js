...
return true;},getDataFilters:function(prop){prop=prop||this.getProperty("dataFilters");if(!prop){return null;}
...
let dataFilterIds=[];if(this.getProperty("dataFilters")){this.getProperty("dataFilters").split(";").map(tok=>{[type,fieldId,value,enabled,label]=tok.split(",");if(!label)return;enabled=enabled=="true";let cbxid=this.getDomId("datafilterenabled_"+fieldId);dataFilterIds.push(cbxid);header2+=HtmlUtils.checkbox("",["id",cbxid],enabled)+" "+label+"&nbsp;&nbsp;"});}
