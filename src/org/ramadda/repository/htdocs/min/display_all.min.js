...
return titleToShow;},handleEventFieldValueSelected:function(source,args){this.setProperty("filterPattern",args.value);this.setProperty("patternFilterField",args.field.getId());this.updateUI();},handleEventPropertyChanged:function(source,prop){if(prop.property=="filterValue"){this.haveCalledUpdateUI=false;var widgetId="filterby_"+prop.fieldId;if(prop.id.endsWith("_min"))widgetId+="_min";if(prop.id.endsWith("_max"))widgetId+="_max";this.settingFilterValue=true;this.jq(widgetId).val(prop.value);this.settingFilterValue=false;this.dataFilterChanged();return;}
...
label=tuple[stringField.getIndex()];value=tuple[sortField.getIndex()];if(isNaN(value)||value===null)value="NA";html+="<tr valign=top class='display-ranking-row' what='"+obj.originalRow+"'><td> #"+(rowIdx+1)+"</td><td>&nbsp;"+label+"</td><td align=right>&nbsp;"+
value+"</td></tr>";}
...
if(this.getProperty("doAnimation",false)){var buttons=HtmlUtils.div(["id",this.getDomId(ID_START),"title","Previous"],HtmlUtils.getIconImage("fa-fast-backward"))+"&nbsp;"+
...
"";buttons=HtmlUtils.div(["xclass","display-map-toolbar"],buttons);this.jq(ID_TOP_LEFT).append(buttons);HtmlUtils.rangeInputInit(this.getDomId(ID_SLIDER));this.run=this.jq(ID_RUN);this.prev=this.jq(ID_PREV);this.next=this.jq(ID_NEXT);this.start=this.jq(ID_START);this.end=this.jq(ID_END);this.showAll=this.jq(ID_SHOWALL);this.animation.label=this.jq(ID_ANIMATION_LABEL);this.run.button().click(()=>{this.toggleAnimation();});this.start.button().click(()=>{this.run.html(HtmlUtils.getIconImage("fa-play"));this.animation.running=false;this.animation.begin=this.animation.dateMin;if(this.animation.mode=="sliding"){this.animation.end=new Date(this.animation.dateMin.getTime()+this.animation.window);}else{this.animation.end=this.animation.begin;}
this.applyAnimation();});this.end.button().click(()=>{this.run.html(HtmlUtils.getIconImage("fa-play"));this.animation.running=false;this.animation.end=this.animation.dateMax;if(this.animation.mode=="sliding"){this.animation.begin=new Date(this.animation.dateMax.getTime()-this.animation.window);}else{this.animation.end=this.animation.dateMin;}
this.applyAnimation();});this.prev.button().click(()=>{this.animation.running=false;this.run.html(HtmlUtils.getIconImage("fa-play"));this.startAnimation(true,true);});this.next.button().click(()=>{this.animation.running=false;this.run.html(HtmlUtils.getIconImage("fa-play"));this.startAnimation(true);});this.showAll.button().click(()=>{this.animation.inAnimation=false;this.animation.label.html("");this.animation.running=false;this.run.html(HtmlUtils.getIconImage("fa-play"));this.showAllPoints();});}
...
var matchedFeatures=[];var seen={};var maxCnt=-1;var minCnt=-1;for(var i=0;i<circles.length;i++){var circle=circles[i];if(circle.style&&circle.style.display=="none")continue;var center=circle.center;var tmp={index:-1,maxExtent:maxExtent};var matchedFeature=this.findContainingFeature(features,center,tmp);if(!matchedFeature)continue;maxExtent=tmp.maxExtent;if(!seen[matchedFeature.featureIndex]){seen[matchedFeature.featureIndex]=true;matchedFeatures.push(matchedFeature);}
...
return matchedFeature;},needsData:function(){return true;},animation:{running:false,inAnimation:false,begin:null,end:null,dateMin:null,dateMax:null,dateRange:0,dateFormat:this.getProperty("animationDateFormat","yyyyMMdd"),mode:this.getProperty("animationMode","cumulative"),steps:this.getProperty("animationSteps",60),windowUnit:this.getProperty("animationWindow",""),window:0,speed:parseInt(this.getProperty("animationSpeed",250)),},toggleAnimation:function(){this.animation.running=!this.animation.running;this.run.html(HtmlUtils.getIconImage(this.animation.running?"fa-stop":"fa-play"));if(this.animation.running)
this.startAnimation();},startAnimation:function(justOneStep,back){if(!this.points){return;}
if(!this.animation.dateMax)return;if(!justOneStep)
this.animation.running=true;if(!this.animation.inAnimation){this.animation.inAnimation=true;this.animation.label.html("");var date=this.animation.dateMin;this.animation.begin=date;var unit=this.animation.windowUnit;if(unit!=""){var tmp=0;var size=0;if(unit=="decade"){this.animation.begin=new Date(date.getUTCFullYear(),0);size=1000*60*60*24*365*10+1000*60*60*24*365;}else if(unit=="year"){this.animation.begin=new Date(date.getUTCFullYear(),0);size=1000*60*60*24*366;}else if(unit=="month"){this.animation.begin=new Date(date.getUTCFullYear(),date.getMonth());size=1000*60*60*24*32;}else if(unit=="day"){this.animation.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay());size=1000*60*60*25;}else if(unit=="hour"){this.animation.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours());size=1000*60*61;}else if(unit=="minute"){this.animation.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getMinutes());size=1000*61;}else{this.animation.begin=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getSeconds());size=1001;}
this.animation.window=size;}else{this.animation.window=this.animation.dateRange/this.animation.steps;}
this.animation.end=this.animation.begin;for(var i=0;i<this.points.length;i++){var point=this.points[i];point.style.display='none';}
if(this.map.circles)
this.map.circles.redraw();}
this.stepAnimation(back);},stepAnimation:function(back){if(!this.points)return;if(!this.animation.dateMax)return;var oldBegin=this.animation.begin;var oldEnd=this.animation.end;var unit=this.animation.windowUnit;var date=back?new Date(this.animation.begin.getTime()-this.animation.window):new Date(this.animation.end.getTime()+this.animation.window);var newDate;if(unit=="decade"){newDate=new Date(date.getUTCFullYear(),0);}else if(unit=="year"){newDate=new Date(date.getUTCFullYear(),0);}else if(unit=="month"){newDate=new Date(date.getUTCFullYear(),date.getMonth());}else if(unit=="day"){newDate=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay());}else if(unit=="hour"){newDate=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours());}else if(unit=="minute"){newDate=new Date(date.getUTCFullYear(),date.getMonth(),date.getDay(),date.getHours(),date.getMinutes());}else{newDate=new Date(this.animation.end.getTime()+this.animation.window);}
if(this.animation.mode=="sliding"){if(back){this.animation.begin=newDate;this.animation.end=oldBegin;}else{this.animation.begin=oldEnd;this.animation.end=newDate;}
if(this.animation.begin.getTime()<this.animation.dateMin.getTime()){this.animation.begin=this.animation.dateMin;this.animation.end=new Date(this.animation.dateMin.getTime()+this.animation.window);}
if(this.animation.begin.getTime()>this.animation.dateMax.getTime()){this.animation.end=this.animation.dateMax;this.animation.begin=new Date(this.animation.dateMax.getTime()-this.animation.window);}
if(this.animation.end.getTime()>this.animation.dateMax.getTime()){}}else{this.animation.end=newDate;}
this.applyAnimation();},applyAnimation:function(){var windowStart=this.animation.begin.getTime();var windowEnd=this.animation.end.getTime();var atLoc={};for(var i=0;i<this.lines.length;i++){var line=this.lines[i];if(line.date<windowStart||line.date>windowEnd){line.style.display='none';continue;}
...
this.map.lines.redraw();this.animation.label.html(this.formatAnimationDate(this.animation.begin)+" - "+this.formatAnimationDate(this.animation.end));if(windowEnd<=this.animation.dateMax.getTime()){if(this.animation.running){setTimeout(()=>{if(!this.animation.running)return;this.stepAnimation()},this.animation.speed);}}else{this.animation.running=false;this.animation.inAnimation=false;this.run.html(HtmlUtils.getIconImage("fa-play"));}
this.applyVectorMap(true);},formatAnimationDate:function(d){if(this.animation.dateFormat=="yyyy"){return Utils.formatDateYYYY(d);}else if(this.animation.dateFormat=="yyyyMMdd"){return Utils.formatDateYYYYMMDD(d);}else{return Utils.formatDate(d);}},showAllPoints:function(){if(this.lines){for(var i=0;i<this.lines.length;i++){var line=this.lines[i];line.style.display='inline';}
if(this.map.lines)
...
sizeBy.index=sizeBy.field!=null?sizeBy.field.getIndex():-1;colorBy.index=colorBy.field!=null?colorBy.field.getIndex():-1;shapeBy.index=shapeBy.field!=null?shapeBy.field.getIndex():-1;var excludeZero=this.getProperty(PROP_EXCLUDE_ZERO,false);this.animation.dateMin=null;this.animation.dateMax=null;var colorByMapProp=this.getProperty("colorByMap");if(colorByMapProp){var toks=colorByMapProp.split(",");colorBy.stringMap={};for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){colorBy.stringMap[toks2[0]]=toks2[1];}}}
var colorByMap={};var colorByValues=[];var colorByMinPerc=this.getDisplayProp(source,"colorByMinPercentile",-1);var colorByMaxPerc=this.getDisplayProp(source,"colorByMaxPercentile",-1);var justOneMarker=this.getProperty("justOneMarker",false);for(var i=0;i<points.length;i++){var pointRecord=records[i];if(this.animation.dateMin==null){this.animation.dateMin=pointRecord.getDate();this.animation.dateMax=pointRecord.getDate();}else{var date=pointRecord.getDate();if(date){if(date.getTime()<this.animation.dateMin.getTime())
this.animation.dateMin=date;if(date.getTime()>this.animation.dateMax.getTime())
...
sizeBy.range=sizeBy.maxValue-sizeBy.minValue;if(this.animation.dateMax){this.animation.dateRange=this.animation.dateMax.getTime()-this.animation.dateMin.getTime();}
...
if(isNaN(props.pointRadius)||props.pointRadius==0)props.pointRadius=radius;if(colorBy.index>=0){var value=pointRecord.getData()[colorBy.index];var percent=0;var msg="";var pctFields=null;if(this.percentFields!=null){pctFields=this.percentFields.split(",");}
...
var date=pointRecord.getDate();if(mapPoint){if(date){mapPoint.date=date.getTime();}
