...
return null;},addGlobal:function(name,value){name=name.trim().replace(/[ -]/g,"_");if(Utils.isDefined(window[name]))window[name]=value;this.globals[name]=value;},getBaseEntry:function(){return this.baseEntry;},getRootEntry:function(){return this.rootEntry;},getPopup:function(){return this.jq(ID_MENU);},loadJson:async function(data){if(data.error){this.setContents(_this.getMessage("Failed to load notebook: "+data.error));return;}
...
var pointData=this.dataCollection.getList();for(var i=0;i<pointData.length;i++){pointData[i].handleEventMapClick(this,source,args.lon,args.lat);}},handleEventPointDataLoaded:function(source,pointData){this.updateUI();},updateUI:async function(){if(!this.loadedResources){await Utils.importCSS(ramaddaBaseHtdocs+"/lib/skewt/sounding.css");await Utils.importJS(ramaddaBaseHtdocs+"/lib/skewt/d3skewt.js");this.loadedResources=true;}
...
var date=records[0].getDate();if(this.jq(ID_DATE_LABEL).size()==0){this.jq(ID_TOP_LEFT).append(HtmlUtils.div(["id",this.getDomId(ID_DATE_LABEL)]));}
if(date!=null){this.jq(ID_TOP_LEFT).html("Date: "+this.formatDate(date));}else{this.jq(ID_TOP_LEFT).html("");}
...
if(points.length==0){console.log("points.length==0");return;}
...
if(showPoints){if(!Utils.isDefined(seen[point]))seen[point]=0;if(seen[point]>500)continue;seen[point]++;var mapPoint=this.map.addPoint("pt-"+i,point,props,html,dontAddPoint);var date=pointRecord.getDate();if(date){mapPoint.date=date.getTime();}
