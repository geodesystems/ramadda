...
value=HtmlUtils.image(value,attrs);}}else if(f.getType()=="url"){if(value&&value.trim().length>1){value=HtmlUtils.href(value,value);}}else if(f.isDate){if(value){s=s.replace("${"+f.getId()+"}",value);s=s.replace("${"+f.getId()+"_yyyy}",Utils.formatDateYYYY(value));s=s.replace("${"+f.getId()+"_yyyymmdd}",Utils.formatDateYYYYMMDD(value));s=s.replace("${"+f.getId()+"_monthdayyear}",Utils.formatDateMonthDayYear(value));s=s.replace("${"+f.getId()+"_mdy}",Utils.formatDateMDY(value));}
continue;}
...
if(color){value=HtmlUtils.span(["style","color:"+color],value);}
...
return iconMap;},getColorByMap:function(){var colorByMapProp=this.getProperty("colorByMap");if(colorByMapProp){var toks=colorByMapProp.split(",");var stringMap={};for(var i=0;i<toks.length;i++){var toks2=toks[i].split(":");if(toks2.length>1){stringMap[toks2[0]]=toks2[1];}}
return stringMap;}
...
date2=null;value=[date1,date2];}else{value=$("#"+this.getDomId("filterby_"+filterField.getId())).val();if(!Array.isArray(value))value=[value];value.map(v=>_values.push((""+v).toLowerCase()));}
...
if(filterBy.length>0){var searchBar="";var dateIds=[];var hideFilterWidget=this.getProperty("hideFilterWidget",false,true);var widgetStyle="";if(hideFilterWidget)
...
widget=HtmlUtils.select("",attrs,enums,dfltValue);}else if(filterField.isNumeric){var min=0;var max=0;var cnt=0;records.map(record=>{var value=this.getDataValues(record)[filterField.getIndex()];if(isNaN(value))return;if(cnt==0){min=value;max=value;}
...
var style=(hideFilterWidget?"display:none;":"")+this.getProperty("filterByStyle","");header2+=HtmlUtils.span(["class","display-filterby","style",style,"id",this.getDomId(ID_FILTERBAR)],searchBar);}
...
var inputFunc=function(input){var id=input.attr("id");var value=input.val();var fieldId=input.attr("fieldId");_this.checkFilterField(input);_this.haveCalledUpdateUI=false;if(_this.settingFilterValue){return;}
_this.settingFilterValue=true;_this.dataFilterChanged();_this.propagateEvent("handleEventPropertyChanged",{property:"filterValue",id:id,fieldId:fieldId,value:value});_this.settingFilterValue=false;};this.jq(ID_FILTERBAR).find(".display-filter-input").keyup(function(e){var keyCode=e.keyCode||e.which;if(keyCode==13){return;}
...
if(template!=""){var props=this.getTemplateProps(fields);var max=parseFloat(this.getProperty("maxNumber",-1));for(var rowIdx=0;rowIdx<selected.length;rowIdx++){if(max!=-1&&rowIdx>=max)break;var record=selected[rowIdx];var row=this.getDataValues(record);var s=template;s=s.replace("${selectCount}",selected.length);s=s.replace("${totalCount}",records.length);s=this.getRecordTemplate(row,fields,s,props);contents+=HtmlUtils.div(["id",this.getId()+"-"+record.getId(),"title","","class","display-template-entry","recordIndex",rowIdx],s);}}
if(selected.length>0)
contents+=footerTemplate;this.writeHtml(ID_DISPLAY_CONTENTS,contents);this.makeTooltips(this.jq(ID_DISPLAY_CONTENTS).find(".display-template-entry"),selected);},highlightCount:0,handleEventRecordHighlight:function(source,args){let myCount=++this.highlightCount;var element=$("#"+this.getId()+"-"+args.record.getId());var container=this.jq(ID_DISPLAY_CONTENTS);if(args.highlight){setTimeout(()=>{if(myCount==this.highlightCount){element.addClass("display-template-record-highlight");container.scrollTop(element.offset().top-container.offset().top+container.scrollTop())}},500);}else{element.removeClass("display-template-record-highlight");}}})}
...
var polygonField=this.getFieldById(fields,this.getProperty("polygonField"));var polygonColorTable=this.getColorTable(true,"polygonColorTable",null);var latlon=this.getProperty("latlon",true);var source=this;var radius=parseFloat(this.getDisplayProp(source,"radius",8));var strokeWidth=parseFloat(this.getDisplayProp(source,"strokeWidth","1"));var strokeColor=this.getDisplayProp(source,"strokeColor","#000");var colorByAttr=this.getDisplayProp(source,"colorBy",null);var colors;if(colorByAttr){colors=this.getColorTable(true,colorByAttr+".colorTable");if(!colors){var c=this.getProperty(colorByAttr+".colors");if(c)
colors=c.split(",");}
if(!colors)
...
props.fillOpacity=0.8;didColorBy=true;}
...
this.labelLayer.removeFeatures(this.labelFeatures);this.labelLayer.addFeatures(features);this.labelFeatures=features;$("#"+this.labelLayer.id).css("z-index",1000);},handleEventRemoveDisplay:function(source,display){if(!this.map){return;}
...
console.log("Add");this.myMarkers[source]=this.map.addMarker(source.getId(),point,icon,"",args.html,null,24);}}});}
