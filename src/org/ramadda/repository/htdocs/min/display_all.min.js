...
var annotationsMap={};var annotations=this.getProperty("annotations");if(annotations){dataTable.addColumn({type:'string',role:'annotation','p':{'html':true}});dataTable.addColumn({type:'string',role:'annotationText','p':{'html':true}});var toks=annotations.split(",");for(var i=0;i<toks.length;i++){var toks2=toks[i].split(";");if(toks2.length!=3)continue;var index=toks2[0].trim();var label=toks2[1];var desc=toks2[2];if(index.match(/^[0-9]+$/)){index=parseFloat(index);}else{index=Utils.parseDate(index,false);}
annotationsMap[index]={label:label,description:desc}}}
for(var i=1;i<dataList.length;i++){var row=this.getDataValues(dataList[i]);row=row.slice(0);var label="";if(dataList[i].record){for(var j=0;j<tooltipFields.length;j++){label+="<b>"+tooltipFields[j].getLabel()+"</b>: "+
