...
var annotationsMap={};if(annotations){let legend="";dataTable.addColumn({type:'string',role:'annotation','p':{'html':true}});dataTable.addColumn({type:'string',role:'annotationText','p':{'html':true}});var labelCnt=0;var toks=annotations.split(",");for(var i=0;i<toks.length;i++){var toks2=toks[i].split(";");if(toks2.length<2)continue;var index=toks2[0].trim();var label=toks2[1].trim();if(label==""){labelCnt++;label=""+labelCnt;}
...
let annotation={label:label,description:desc};var legendLabel=desc;if(url!=null){legendLabel=HtmlUtils.href(url,legendLabel,["target","_annotation"]);}
legend+=HtmlUtils.b(label)+":"+legendLabel+" ";annotationsMap[index]=annotation;if(isDate){annotationsMap[Utils.formatDateYYYYMMDD(index)]=annotation;annotationsMap[Utils.formatDateYYYYWeek(index)]=annotation;}}
...
var colors=this.getColorTable(true);var colorBy=this.getColorByInfo(records);var didColorBy=false;for(var i=1;i<dataList.length;i++){var record=dataList[i];var theRecord=dataList[i].record;var row=this.getDataValues(record);if(i<2){}
var color="";if(colorBy.index>=0){var value=theRecord.getData()[colorBy.index];hasColorByValue=true;colorByValue=value;didColorBy=true;color=colorBy.getColor(value,theRecord);}
