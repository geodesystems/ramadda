...
if(points.length==0){}
this.addPoints(records,fields,points);this.addLabels(records,fields,points);this.applyVectorMap();},addPoints:function(records,fields,points){var polygonField=this.getFieldById(fields,this.getProperty("polygonField"));var source=this;var radius=parseFloat(this.getDisplayProp(source,"radius",8));var strokeWidth=parseFloat(this.getDisplayProp(source,"strokeWidth","1"));var strokeColor=this.getDisplayProp(source,"strokeColor","#000");var colorByAttr=this.getDisplayProp(source,"colorBy",null);var colors;if(colorByAttr){colors=this.getColorTable(true,colorByAttr+".colorTable");if(!colors){var c=this.getProperty(colorByAttr+".colors");if(c)
colors=c.split(",");}
if(!colors)
...
var html=this.getRecordHtml(pointRecord,fields);if(polygonField){var s=values[polygonField.getIndex()];var delimiter;[";",","].map(d=>{if(s.indexOf(d)>=0)delimiter=d;});var toks=s.split(delimiter);var p=[];for(var pIdx=0;pIdx<toks.length;pIdx+=2){var lat=parseFloat(toks[pIdx]);var lon=parseFloat(toks[pIdx+1]);p.push(new OpenLayers.Geometry.Point(lon,lat));}
var poly=this.map.addPolygon("polygon"+pIdx,"",p,props,html);if(date){poly.date=date.getTime();}
