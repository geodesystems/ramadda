...
console.log("\returning value:"+value);return value;},debugGetProperty:false,getPropertyInner:function(key,dflt,skipThis){let debug=this.debugGetProperty;if(debug)console.log("getProperty:"+key+" dflt:"+dflt);if(!skipThis&&Utils.isDefined(this[key])){if(debug)console.log("\tgetProperty-1");return this[key];}
...
return df;},getDefaultSelectedFields:function(fields,dfltList){let debug=false;if(this.defaultSelectedToAll()&&this.allFields!=null){var tmp=[];for(i=0;i<this.allFields.length;i++){var field=this.allFields[i];if(!field.isFieldGeo()){tmp.push(field);}}
if(debug)
...
this.checkSearchBar();this.updateUI();},getFilterId:function(id){return this.getDomId("filterby_"+id);},getHeader2:function(){return"";},initHeader2:function(){},getRequestMacros:function(){if(!this.requestMacros){this.requestMacros=this.getRequestMacrosInner();}
return this.requestMacros;},getRequestMacrosInner:function(){let macros=[];let p=this.getProperty("requestFields","");let e=this.getProperty("extraFields","");let list=Utils.mergeLists(e.split(","),p.split(","));list.every(macro=>{if(macro=="")return true;let values=null;let enums=this.getProperty("request."+macro+".values");if(enums){values=[]
if(this.getProperty("request."+macro+".includeAll",this.getProperty("request.includeAll",!enums.startsWith(":")))){values.push(["","All"]);}
...
let macroType=this.getProperty("request."+macro+".type",values!=null?"enumeration":"string");macros.push({display:this,name:macro,values:values,urlarg:this.getProperty("request."+macro+".urlarg",macro),type:macroType,triggerReload:this.getProperty("request."+macro+".triggerReload",true),dflt:this.getProperty("request."+macro+".default",""),dflt_from:this.getProperty("request."+macro+"_from.default",""),dflt_to:this.getProperty("request."+macro+"_to.default",""),dflt_min:this.getProperty("request."+macro+"_min.default",""),dflt_max:this.getProperty("request."+macro+"_max.default",""),label:this.getProperty("request."+macro+".label",Utils.makeLabel(macro)),multiple:this.getProperty("request."+macro+".multiple",false),rows:this.getProperty("request."+macro+".rows",3),getWidget:function(dateIds){let visible=this.display.getProperty("request."+this.name+".visible",this.display.getProperty("macros.visible",true));let style=visible?"":"display:none;";let widget;let label=this.label;if(this.type=="enumeration"){if(values){let attrs=["style",style,"id",this.display.getDomId(this.getId()),"class","display-filter-input"];if(this.multiple){attrs.push("multiple");attrs.push(null);attrs.push("size");attrs.push(Math.min(this.rows,values.length));}
widget=HtmlUtils.select("",attrs,this.values,this.dflt,20);}}else if(this.type=="numeric"){let minId=this.display.getDomId(this.getId()+"_min");let maxId=this.display.getDomId(this.getId()+"_max");widget=HtmlUtils.input("","",["style",style,"id",minId,"size","5","class","display-filter-input"],this.dflt_min)+
" - "+
...
if(!widget)return"";return(visible?this.display.makeFilterWidget(label,widget):widget);},getId:function(){return"macro_"+this.name;},getValue:function(){let widget=this.display.jq(this.getId());let value=this.dflt;if(widget.length!=0)value=widget.val();this.display.setProperty("request."+this.name+".default",value);return value;},apply:function(url){if(this.type=="numeric"){let min=this.display.jq(this.getId()+"_min").val()||"";let max=this.display.jq(this.getId()+"_max").val()||"";this.dflt_min=min;this.dflt_max=max;if(min!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg+"_from",min);if(max!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg+"_to",max);this.display.setProperty("request."+this.name+"_min.default",min);this.display.setProperty("request."+this.name+"_max.default",max);}else if(this.type=="date"){let from=this.display.jq(this.getId()+"_from").val()||"";let to=this.display.jq(this.getId()+"_to").val()||"";this.dflt_from=from;this.dflt_to=to;this.display.setProperty("request."+this.name+"_from.default",from);this.display.setProperty("request."+this.name+"_to.default",to);if(from!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg+"_fromdate",from);if(to!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg+"_todate",to);}else if(this.type=="enumeration"){let value=this.getValue();if(Array.isArray(value)){value.map(v=>{if(v!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg,v);});}else{if(value!="")
url=url+"&"+HtmlUtils.urlArg(this.urlarg,value);}}else{let value=this.getValue();this.dflt=value;if(value!=""){url=url+"&"+HtmlUtils.urlArg(this.urlarg,value);}}
return url;}});return true;});return macros;},makeFilterWidget:function(label,widget,title){if(!label)
return HtmlUtils.div(["class","display-filter-widget"],widget);return HtmlUtils.div(["class","display-filter-widget"],this.makeFilterLabel(label,title)+(label.trim().length==0?" ":": ")+
widget);},makeFilterLabel:function(label,tt){let attrs=["class","display-filter-label"];if(tt){attrs.push("title");attrs.push(tt);}
return HtmlUtils.span(attrs,label);},checkSearchBar:function(){let _this=this;this.filterFields=[];this.colorByFields=this.getFieldsByIds(null,this.getProperty("colorByFields","",true));this.sizeByFields=this.getFieldsByIds(null,this.getProperty("sizeByFields","",true));let pointData=this.getData();if(pointData==null)return;let fields=pointData.getRecordFields();let records=pointData.getRecords();let header2="";if(this.getProperty("showProgress",false)){header2+=HtmlUtils.div(["id",this.getDomId(ID_DISPLAY_PROGRESS),"style","display:inline-block;margin-right:4px;min-width:20px;"]);}
header2+=HtmlUtils.span(["id",this.getDomId(ID_HEADER2_PREFIX)]);header2+=this.getHeader2();if(this.getProperty("pageRequest",false)){header2+=HtmlUtils.span(["id",this.getDomId(ID_PAGE_COUNT)]);}
let macros=this.getRequestMacros();let macroDateIds=[];macros.every(macro=>{if(this.getProperty("request."+macro.name+".show",true)){header2+=macro.getWidget(macroDateIds)+"&nbsp;&nbsp;";}
...
if(entry){pointData.entry=entry;this.addEntry(entry);}},clearProgress:function(){this.jq(ID_DISPLAY_PROGRESS).html("");},startProgress:function(){if(this.jq(ID_DISPLAY_PROGRESS).length>0)
this.jq(ID_DISPLAY_PROGRESS).html(HtmlUtils.image(icon_progress));else
...
this.setContents(this.getMessage(msg));},clearCache:function(){},pointDataLoaded:function(pointData,url,reload){let debug=false;this.clearProgress();this.inError=false;this.clearCache();if(debug)console.log("pointDataLoad:"+this.getId()+" "+this.type+" #records:"+pointData.getRecords().length);if(debug)
console.log("\tclearing last selected fields");this.lastSelectedFields=null;this.allFields=null;if(!reload){if(debug)console.log("\tno reload");this.addData(pointData);this.checkSearchBar();}else{if(!this.dataCollection)
this.dataCollection=new DataCollection();this.dataCollection.setData(pointData);}
...
mapPoint.highlightTextGetter=highlightGetter;mapPoint.record=record;mapPoint.textGetter=textGetter;mapPoint.hasColorByValue=hasColorByValue;mapPoint.colorByValue=colorByValue;mapPoint.colorByColor=colorByColor;if(date){mapPoint.date=date.getTime();}
