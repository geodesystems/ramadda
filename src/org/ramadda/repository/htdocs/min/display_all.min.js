...
this.addLabels(records,fields,points);this.addPoints(records,fields,points);this.applyVectorMap();},addPoints:function(records,fields,points){var cidx=0
...
this.map.labelLayer.removeFeatures(this.labelFeatures);this.map.labelLayer.addFeatures(features);this.labelFeatures=features;},handleEventRemoveDisplay:function(source,display){if(!this.map){return;}
...
return icon;}
displayMapCurrentMarker++;if(displayMapCurrentMarker>=displayMapMarkers.length)displayMapCurrentMarker=0;return ramaddaBaseUrl+"/lib/openlayers/v2/img/"+displayMapMarkers[displayMapCurrentMarker];},handleEventRecordSelection:function(source,args){if(!this.getProperty("showRecordSelection",true))return;if(!this.map){return;}
var record=args.record;if(record.hasLocation()){var latitude=record.getLatitude();var longitude=record.getLongitude();if(latitude<-90||latitude>90||longitude<-180||longitude>180)return;var point=new OpenLayers.LonLat(longitude,latitude);var marker=this.myMarkers[source];if(marker!=null){this.map.removeMarker(marker);}
