...
editor.container.addEventListener("contextmenu",function(e){e.preventDefault();var cursor=editor.getCursorPosition();var t=editor.getValue();var s="";var lines=t.split("\n");for(i=0;i<lines.length;i++){if(i==cursor.row){s+=lines[i].substring(0,cursor.column);break;}
s+=lines[i]+"\n";}
var inBracket=false;var inRightBracket=false;var i;for(i=s.length-1;i>=0;i--){var c=s[i];if(c=="}"){if(inRightBracket)
return;inRightBracket=true;continue;}
inRightBracket=false;if(c=="{"){if(inBracket){break;}
inBracket=true;continue;}
inBracket=false;}
if(i<0)return;var tag=s.substring(i+2);var tmp=tag.match(/type *= *\"([^\"]+)\"?/);var type;if(tmp&&tmp.length>1)type=tmp[1];var idx=tag.indexOf(" ");if(idx<0)return;tag=tag.substring(0,idx);var tags=[];if(tag=="display"&&type){try{var display=(new DisplayManager()).createDisplay(type,{dummy:true});if(display){tags=display.getWikiEditorTags();}}catch(e){console.log("Error getting tags for:"+type+" error:"+e);}}
if(wikiAttributes[tag]){wikiAttributes[tag].map(a=>tags.push(a));}
if(tags.length==0)return;var menu="<div style=margin:5px;><table><tr valign=top><td><div>";tags.map(tag=>{if(tag.startsWith("label:")){if(menu!="")menu+="</div></td>";menu+="<td><b> "+tag.substring(6)+"</b><br><div style='margin-left:5px;max-width:400px;overflow-x:auto;max-height:400px;overflow-y:auto;'>";return;}
var t=" "+tag.replace(/\"/g,"&quot;")+" ";menu+=HtmlUtils.onClick("insertText('"+info.id+"','"+t+"')",tag)+"<br>\n";});menu+="</div></td></tr></table></div>";popupObject=getTooltip();popupObject.html(menu);popupObject.show();popupObject.position({of:$(window),my:"left top",at:"left+"+e.x+" top+"+(e.y),collision:"fit fit"});return false;},false);}
