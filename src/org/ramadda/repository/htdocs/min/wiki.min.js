...
var toolbar=$("#"+info.id+"_toolbar");editor.container.addEventListener("contextmenu",function(e){e.preventDefault();var cursor=editor.getCursorPosition();var menu=toolbar.html();menu=menu.replace(/(menulink_[0-9]+)/g,"$1_popup");menu=menu.replace(/(_entryid)/g,"popup_entryid");menu=menu.replace(/(_wikilink)/g,"popup_wikilink");menu=menu.replace(/(_fieldname)/g,"popup_fieldname");menu=HtmlUtils.div(["class","wiki-editor-popup-toolbar"],menu);var t=editor.getValue();var s="";var lines=t.split("\n");for(i=0;i<lines.length;i++){if(i==cursor.row){s+=lines[i].substring(0,cursor.column);break;}
s+=lines[i]+"\n";}
var inBracket=false;var inRightBracket=false;var i;for(i=s.length-1;i>=0;i--){var c=s[i];if(c=="}"){if(inRightBracket)
break;inRightBracket=true;continue;}
inRightBracket=false;if(c=="{"){if(inBracket){break;}
inBracket=true;continue;}
inBracket=false;}
if(i>=0){var tag=s.substring(i+2);var tmp=tag.match(/type *= *\"([^\"]+)\"?/);var type;if(tmp&&tmp.length>1)type=tmp[1];var idx=tag.indexOf(" ");if(idx>=0){tag=tag.substring(0,idx);var tags=[];if(tag=="display"&&type){try{var display=(new DisplayManager()).createDisplay(type,{dummy:true});if(display){tags=display.getWikiEditorTags();}}catch(e){console.log("Error getting tags for:"+type+" error:"+e);}}
