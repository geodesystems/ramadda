...
var wikiAttributes={map:["label:Map Attributes","icon=\"#/icons/dots/green.png\"","width=\"100%\"","height=\"400\"","listentries=\"true\"","details=\"false\"","showLocationSearch=\"true\"","showCheckbox=\"true\"","showSearch=\"false\"","icon=\"#/icons/dots/green.png\"","iconsonly=\"false\""],}
function wikiInitEditor(info){var editor=info.editor
var toolbar=$("#"+info.id+"_toolbar");editor.container.addEventListener("contextmenu",function(e){e.preventDefault();var cursor=editor.getCursorPosition();var menu=toolbar.html();menu=menu.replace(/(menulink_[0-9]+)/g,"$1_popup");menu=menu.replace(/(_entryid)/g,"popup_entryid");menu=menu.replace(/(_wikilink)/g,"popup_wikilink");menu=menu.replace(/(_fieldname)/g,"popup_fieldname");var t=editor.getValue();var s="";var lines=t.split("\n");for(i=0;i<lines.length;i++){if(i==cursor.row){s+=lines[i].substring(0,cursor.column);break;}
s+=lines[i]+"\n";}
...
menu+="<div style='border-top:1px solid #ccc; margin:5px;'><table><tr valign=top><td><div>";tags.map(tag=>{if(tag.startsWith("label:")){if(menu!="")menu+="</div></td>";menu+="<td><b> "+tag.substring(6)+"</b><br><div style='margin-left:5px;max-width:400px;overflow-x:auto;max-height:400px;overflow-y:auto;'>";return;}
var t=" "+tag.replace(/\"/g,"&quot;")+" ";menu+=HtmlUtils.onClick("insertText('"+info.id+"','"+t+"')",tag)+"<br>\n";});menu+="</div></td></tr></table></div>";}}
