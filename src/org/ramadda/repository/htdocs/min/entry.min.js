...
url+="&max="+settings.getMax();url+="&skip="+settings.getSkip();url+=settings.getExtra();return url;},addEntry:function(entry){this.entryCache[entry.getId()]=entry;},getEntry:function(id,callback){var entry=this.entryCache[id];if(entry!=null){return entry;}
if(window.globalRamaddas){for(var i=0;i<window.globalRamaddas.length;i++){var em=window.globalRamaddas[i];var entry=em.entryCache[id];if(entry!=null){return entry;}}}
if(callback==null){return null;}
var ramadda=this;var jsonUrl=this.getJsonUrl(id)+"&onlyentry=true";var jqxhr=$.getJSON(jsonUrl,function(data){if(GuiUtils.isJsonError(data)){return;}
var entryList=createEntriesFromJson(data,ramadda);var first=null;if(entryList.length>0)first=entryList[0];callback(first,entryList);}).fail(function(jqxhr,textStatus,error){var err=textStatus+", "+error;GuiUtils.handleError("Error getting entry information: "+err,jsonUrl,false);});return null;}});}
...
RamaddaUtil.defineMembers(this,{getId:function(){return this.id;},getIdForDom:function(){return this.domId;},getFullId:function(){return this.getRamadda().getRoot()+","+this.id;},getDisplayName:function(){if(this.displayName)return this.displayName;return this.getName();},getStartDate:function(){return this.startDate;},getEndDate:function(){return this.endDate;},getIsGroup:function(){return this.isGroup;},getParentEntry:function(callback,extraArgs){let _this=this;if(!this.parent)return null;if(this.parentEntry)return this.parentEntry;if(!callback)return null;this.parentEntry=this.getRamadda().getEntry(this.parent,entry=>{_this.parentEntry=entry;callback(entry);});return this.parentEntry;},getChildrenEntries:function(callback,extraArgs){if(this.childrenEntries!=null){return this.childrenEntries;}
var theEntry=this;var settings=new EntrySearchSettings({parent:this.getId()});var jsonUrl=this.getRamadda().getSearchUrl(settings,OUTPUT_JSON);var jsonUrl=this.getRamadda().getJsonUrl(this.getId())+"&justchildren=true";if(extraArgs!=null){jsonUrl+="&"+extraArgs;}
var myCallback={entryListChanged:function(list){callback(list.getEntries());}};var entryList=new EntryList(this.getRamadda(),jsonUrl,myCallback,true);return null;},getType:function(){if(this.typeObject!=null){}
...
function EntryList(repository,jsonUrl,listener,doSearch){$.extend(this,{repository:repository,url:jsonUrl,listener:listener,haveLoaded:false,entries:[],map:{},getRepository:function(){return this.repository;},setEntries:function(entries){this.entries=entries;this.haveLoaded=true;},getEntry:function(id){var entry=this.map[id];if(entry!=null)return entry;return this.getRepository().getEntry(id);},getEntries:function(){return this.entries;},createEntries:function(data,listener){this.entries=createEntriesFromJson(data,this.getRepository());for(var i=0;i<this.entries.length;i++){var entry=this.entries[i];this.map[entry.getId()]=entry;}
if(listener==null){listener=this.listener;}
if(listener&&listener.entryListChanged){listener.entryListChanged(this);}},doSearch:function(listener){if(listener==null){listener=this.listener;}
var _this=this;var jqxhr=$.getJSON(this.url,function(data,status,jqxhr){if(GuiUtils.isJsonError(data)){return;}
...
for(var i=0;i<this.entryLists.length;i++){var entryList=this.entryLists[i];if(!entryList.getRepository().canSearch())continue;var callback={entryListChanged:function(entryList){if(listener){listener.entryListChanged(_this,entryList);}}};entryList.doSearch(callback);}},getEntry:function(id){for(var i=0;i<this.entryLists.length;i++){var entryList=this.entryLists[i];var entry=entryList.getEntry(id);if(entry!=null){return entry;}}
