...
return this.type;},getThumbnail:function(){if(!this.metadata)return null;for(let i=0;i<this.metadata.length;i++){let metadata=this.metadata[i];if(metadata.type=="content.thumbnail"&&Utils.stringDefined(metadata.value.attr1)){if(metadata.value.attr1.startsWith("http"))return metadata.value.attr1;return this.getRamadda().getRoot()+"/metadata/view/"+metadata.value.attr1+"?element=1&entryid="+this.getId()+"&metadata_id="+metadata.id;}}
...
return null;},hasResource:function(){return this.getFilename()!=null;},getResourceUrl:function(){if(this.url){return this.url;}
