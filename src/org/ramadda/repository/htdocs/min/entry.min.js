...
url+="&maxlongitude="+settings.getEast();if(settings.entryRoot)
url+="&entryRoot="+encodeURIComponent(settings.entryRoot);for(var i=0;i<settings.metadata.length;i++){let metadata=settings.metadata[i];url+="&metadata_attr1_"+metadata.type+"="+metadata.value;}
...
var entry=this.entryCache[id];if(entry!=null){if(debug)console.log("getEntry:"+entry.getName());return Utils.call(callback,entry);}
...
let entryList=createEntriesFromJson(data,ramadda);let first=null;if(entryList.length>0)first=entryList[0];if(debug)console.log("\tramadda.getEntry: result:"+entryList.length+" "+first);Utils.call(callback,first,entryList);}).fail(function(jqxhr,textStatus,error){var err=textStatus+", "+error;GuiUtils.handleError("Error getting entry information: "+err,jsonUrl,false);});}});}
...
props.columns=myColumns;$.extend(this,props);RamaddaUtil.defineMembers(this,{getIsGroup:function(){return this.isgroup;},getIcon:function(){return this.icon;},getLabel:function(){return this.label||this.name;},getId:function(){if(this.type!=null)return this.type;return this.id;},getCategory:function(){return this.category;},getEntryCount:function(){return this.entryCount;},getColumns:function(){return this.columns;},});}
...
RamaddaUtil.defineMembers(this,{getId:function(){return this.id;},getIdForDom:function(){return this.domId;},getFullId:function(){return this.getRamadda().getRoot()+","+this.id;},getDisplayName:function(){if(this.displayName)return this.displayName;return this.getName();},getCreateDate:function(){return this.startCreate;},getStartDate:function(){return this.startDate;},getEndDate:function(){return this.endDate;},getIsGroup:function(){return this.isGroup;},getRoot:async function(callback,extraArgs){var parent=this;while(true){var tmp;await parent.getParentEntry(e=>tmp=e);if(!tmp){return Utils.call(callback,parent);}
...
var settings=new EntrySearchSettings({parent:this.getId()});var jsonUrl=this.getRamadda().getSearchUrl(settings,OUTPUT_JSON);var jsonUrl=this.getRamadda().getJsonUrl(this.getAbsoluteId())+"&justchildren=true";if(extraArgs!=null){jsonUrl+="&"+extraArgs;}
var myCallback={entryListChanged:function(list){callback(list.getEntries());}};var entryList=new EntryList(this.getRamadda(),jsonUrl,myCallback,false);await entryList.doSearch();},getType:function(){if(this.typeObject!=null){}
