...
var ID_SEARCH_ATTRIBUTES = 'searchattributes';
var ID_EXPAND_WIKI_MENU = 'expandwikimenu';
var ID_EXPANDED_WIKI_MENU = 'expandedwikimenu';
var ID_UNEXPANDED_WIKI_MENU = 'unexpandedwikimenu';
...
if(!window.WikiUtil) {
	...
	initWikiEditor(entry, wikiId, textId,cbxId) {
		...
		if(on) {
                    ...
                    let val = jqid(textId).val();
                    editor.getEditor().setValue(val,8);
                    jqid(wikiId).focus();
		} else {
                    let val = editor.getEditor().getValue();
                    jqid(textId).val(val);
                    jqid(textBlock).css(CSS_DISPLAY,DISPLAY_BLOCK);
                    jqid(wikiBlock).css(CSS_DISPLAY,DISPLAY_NONE);
                    jqid(textId).focus();
		}
            })
	},
	...
	chooseInsertText:function(id, attr,value,help) {
	    ...
	    let html = '';
	    values.forEach(v=>{
		let link =HU.div([ATTR_CLASS,HU.classes(CLASS_MENU_ITEM,CLASS_CLICKABLE),
				  'data-id',id,
				  'data-attr',attr,
				  ATTR_DATA_VALUE,v],v);
		html+=link;
...
WikiEditor.prototype = {
	...
	if(isNew) {
	    html +=HU.br('The new entry has been added. What do you want to insert into the document?');
	} else {
	...
	this.jq(this.ID_WIKI_MESSAGE).css(CSS_DISPLAY,DISPLAY_BLOCK).html(msg);
    },

    clearMessage:function(msg) {
	this.jq(this.ID_WIKI_MESSAGE).css(CSS_DISPLAY,DISPLAY_NONE);
	...
	return HU.findClass(this.getBlock(),'ace_scroller');
    },
    getSession:function() {
	...
	*/
	...
	html+=HU.textarea('',llmText,[ATTR_PLACEHOLDER,'Enter input or select text in editor',
				      ATTR_ID,this.domId(this.ID_LLM_INPUT),
				      ATTR_ROWS,6,
				      ATTR_COLS,80,
	...
	html+=HU.div([ATTR_STYLE,HU.css(CSS_POSITION,POSITION_RELATIVE)],
		     HU.textarea('','',
				 [ATTR_PLACEHOLDER,'Results',
				  ATTR_ID,this.domId('rewrite-results'),
				  ATTR_ROWS,6,
				  ATTR_COLS,80,
				  ATTR_STYLE,textAreaStyle])+
		     HU.div([ATTR_STYLE,
			     HU.css(CSS_DISPLAY,DISPLAY_NONE,
				    CSS_POSITION,POSITION_ABSOLUTE,
				    CSS_TOP, HU.perc(50),
				    CSS_LEFT,HU.perc(50),
				    '-ms-transform',   HU.translate('-50%', '-50%'),
				    CSS_TRANSFORM, HU.translate('-50%', '-50%')),
			     ATTR_ID,this.domId(ID_LLM_LOADING)],
	...
	html += HU.buttons([HU.span([ATTR_CLASS,CLASS_DIALOG_BUTTON,'replace','true'],'Replace'),
			    ...
			    HU.span([ATTR_CLASS,CLASS_DIALOG_BUTTON,
				     ATTR_CANCEL,true,
				     ATTR_ID,this.domId(ID_CANCEL)],LABEL_CANCEL)]);
	...
	dialog.find(HU.dotClass('wiki-llm-input')).keypress(function(e){
	    if(e.keyCode == 13) {
		call();
	    }
	});
	...
	let _this = this;
	dialog.find(HU.dotClass(CLASS_DIALOG_BUTTON)).button().click(function() {
	    _this.llmReplacing = true;	    
	    if ($(this).attr(ATTR_CANCEL)) {
		dialog.remove();
		return;
	    }
	...
	if(!inPlace) {
	    ...
	    jqid(this.domId(this.ID_WIKI_PREVIEW)).remove();
	    ...
	    preview.css(CSS_WIDTH,HU.px(width));
	...
	let wikiCallback = (html,status,xhr) =>{
		...
		let close  = HU.span([ATTR_CLASS, HU.classes(CLASS_BUTTON, CLASS_CLICKABLE),
				      ATTR_ID,this.domId(this.ID_WIKI_PREVIEW_CLOSE)],
				     "Close");
		let left = Utils.join([
		    close,
		    HU.span([ATTR_CLASS, CLASS_CLICKABLE,
			     ATTR_ID,this.domId(this.ID_WIKI_PREVIEW_OPEN)],
			    HU.getIconImage('fa-sync',[ATTR_TITLE,'Preview Again'])),
		    ...
		    HU.span([ATTR_CLASS, CLASS_CLICKABLE,
			     ...
			     ATTR_ID,this.domId(this.ID_WIKI_PREVIEW_WORDCOUNT)],
			    HU.getIconImage('fa-calculator')),

		    /*
		      HU.span([ATTR_CLASS, CLASS_CLICKABLE,ATTR_TITLE,'Copy',
		      ATTR_ID,this.domId(this.ID_WIKI_PREVIEW_COPY)], HU.getIconImage('fa-copy')),
		      HU.span([ATTR_CLASS, CLASS_CLICKABLE,ATTR_TITLE,'Download',
		      ATTR_ID,this.domId(this.ID_WIKI_PREVIEW_DOWNLOAD)], HU.getIconImage('fa-download')),
		    */
		],SPACE2);

		let right  = HU.span([ATTR_CLASS, CLASS_CLICKABLE,
				      ATTR_ID,this.domId(this.ID_WIKI_PREVIEW_CLOSE)],
				     HU.getIconImage('fa-window-close',[ATTR_TITLE,'Close Preview']));
		...
		jqid(this.domId(this.ID_WIKI_PREVIEW_CLOSE)).button().click(() =>{
		    this.previewShown = false;
		    jqid(this.domId(this.ID_WIKI_PREVIEW)).hide();
		});		
	    }
	}
	...
	try {
	    ...
	    //clean  up the text code points
	    text = 'base64:' + Utils.utf8ToB64(text);
	} catch(err) {
	...
	let popupColorTable=(dialog,source,block)=>{
	    ...
	    contents = HU.center(HU.div([ATTR_ID,guid,
					 ATTR_STYLE,HU.css(CSS_MIN_WIDTH,HU.px(400),
							   CSS_MARGIN,HU.px(5))])) + contents;
	    ...
	    HU.initPageSearch(HU.dotClass(CLASS_COLORTABLE_SELECT),
			      HU.dotClass(CLASS_COLORTABLE_CATEGORY),
			      'Find colortable',false,{target:jqid(guid)});

	    ctDialog.find(HU.dotClass(CLASS_COLORTABLE_SELECT)).click(function() {
		let colorTable =$(this).attr(ATTR_COLORTABLE);
	...
	let menu =  HU.open(TAG_DIV,[ATTR_CLASS,'wiki-editor-popup',
				     ATTR_STYLE,HU.css(CSS_MIN_WIDTH,HU.px(400),CSS_MAX_WIDTH,HU.px(600))]);
	if(!title) {
	    title = Utils.makeLabel(tagInfo.tag) +' Properties';
	}
	...
	menu+=header;
	menu+=HU.thinLine();
	menu+=HU.vspace(HU.em(0.5));
	menu+=HU.open(TAG_DIV,[ATTR_STYLE,HU.css(CSS_MARGIN,HU.px(5))]);
	...
	Utils.splitList(blocks,3).forEach(blocks=>{
		...
		menu+=title;
		//		menu +=HU.toggleBlock(title, contents);
	    });
	    menu += HU.close(TAG_DIV);
	});
	menu += HU.close(TAG_DIV,TAG_DIV);
	if(this.tagDialog) {
	    this.tagDialog.remove();
	}
	let dialog = this.tagDialog =
	...
	HU.findClass(dialog,'ramadda-block-link').click(function() {
	    let block = blockMap[$(this).attr('data-block')];
	    if(!block) {
		alert('Could not find block');
		return;
	...
	let html = HU.div([ATTR_ID,this.domId('wiki-editor-popup'),
			  ...
			  HU.div([ATTR_STYLE, HU.css(CSS_TEXT_ALIGN,ALIGN_CENTER,
						     CSS_PADDING,HU.px(4))],
				 HU.span([ATTR_TITLE,'Tidy the text',
					  ATTR_ID,this.domId(this.ID_WIKI_POPUP_TIDY)],
					 HU.getIconImage('fas fa-broom')) + SPACE1 +
				 HU.span([ATTR_TITLE,'Compact the text',
					  ATTR_ID,this.domId(this.ID_WIKI_POPUP_COMPACT)],
					 HU.getIconImage('fas fa-snowplow')) + SPACE1 +
				 HU.span([ATTR_ID,this.domId(this.ID_WIKI_POPUP_OK)],LABEL_OK) + SPACE1 +
				 HU.span([ATTR_ID,this.domId(this.ID_WIKI_POPUP_CANCEL)],LABEL_CANCEL)));
	...
	let dialog = HU.makeDialog({
	    ...
	    sticky:true,
	    draggable:true,
	    modalContentsCss:HU.css(CSS_LEFT,HU.px(50))});
	jqid(ID_SEARCH_ATTRIBUTES).click(function(){
	...
	blocks.forEach((block,idx)=>{
	    ...
	    let title = block.title;
	    all+=HU.div([ATTR_CLASS,'wiki-searchheader','data-block-index',block.index],
			HU.b(title));
	...
	top = 	HU.div([ATTR_STYLE,
			HU.css(CSS_PADDING,HU.px(5),
			       CSS_BORDER_BOTTOM,CSS_BASIC_BORDER)], top);
	all  = HU.div([ATTR_STYLE,
		       HU.css(CSS_MARGIN,HU.px(5))],all);
	all  = top +
	    HU.div([ATTR_ID,_this.domId('allsearch_corpus'),
		    ATTR_CLASS,'wikieditor-menu-popup',
		    ATTR_STYLE,HU.css(CSS_WIDTH,HU.px(500),
				      CSS_MAX_HEIGHT,HU.px(400),
				      CSS_OVERFLOW_Y,OVERFLOW_AUTO)], all);
	...
	let headers = jqid(_this.domId('allsearch_corpus')).find(HU.dotClass('wiki-searchheader'));
	...
	_this.jq('searchshowall').change(function() {
		...
		commands.each(function(){
		    ...
		    span.html(orig  +' - ' +
			      HU.span([ATTR_STYLE,HU.css(CSS_FONT_SIZE,HU.perc(70),
							 CSS_FONT_STYLE,FONT_ITALIC)],title));
		});
	    } else {
		commands.each(function(){
		    let span = $(this);
	...
	let message= HU.div([ATTR_ID,entryDiv,
	...
	if(c) {
	    scroller.css(CSS_CURSOR,CURSOR_CONTEXT_MENU);
	...
	try {
		...
		if(display.getTypeLabel) {
		    ...
		    if(url) title = HU.href(url, title + SPACE +
	...
	let id = this.getId();
	let button = jqid("displays_button"+ id);
	...
	let menuTags = '';
	let menu = HU.open(TAG_TABLE)+HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_TOP]);
        for (let i = 0; i < cats.length; i++) {
	    let cat = cats[i];
	    let menuItems = Utils.join(links[cat],"<div>\n");
	    menuTags+=  HU.div([ATTR_STYLE,HU.css(CSS_VERTICAL_ALIGN,ALIGN_TOP,
	    ...
	    menu += HU.td(['data-category',cat,ATTR_CLASS,'wiki-editor-display-category'],
			  HU.div([ATTR_STYLE,HU.css(CSS_MARGIN_RIGHT,HU.px(5))], HU.b(cat)) +
			  HU.open(TAG_DIV,[ATTR_STYLE,HU.css(CSS_MARGIN_RIGHT,HU.px(5),
							     CSS_MAX_HEIGHT,HU.px(250),
							     CSS_OVERFLOW_Y,OVERFLOW_AUTO)]) + menuItems);
        }
	menu = HU.div([ATTR_ID,"wiki-display-popup",
		       ATTR_STYLE,HU.css(CSS_FONT_SIZE,HU.pt(10),CSS_WIDTH,HU.px(800))], menu);
	...
	let contents = HU.div([ATTR_STYLE,HU.css(CSS_MARGIN,HU.px(10))],
			      header+ HU.div([ATTR_ID,this.domId(ID_EXPANDED_WIKI_MENU)]) +
			      HU.div([ATTR_ID,this.domId(ID_UNEXPANDED_WIKI_MENU)],menu));
	...
	button.click(() =>{
	    ...
	    this.jq(ID_EXPAND_WIKI_MENU).click(()=>{
		if(!exDiv) {
		    exDiv='';
		    popup.find(HU.dotClass('wiki-editor-display-category')).each(function() {
			let cat = $(this).attr('data-category');
			let inner = HU.open(TAG_TABLE,[ATTR_WIDTH,HU.perc(100),
						       ATTR_STYLE,
						       HU.css(CSS_WIDTH,HU.perc(100),
							      'table-layout','fixed',
							      CSS_BORDER_COLLAPSE,'separate',
			...
			$(this).find(HU.dotClass('display-link')).each(function() {
			    ...
			    if(cnt>3) {
				cnt=1;
				inner+=HU.close(TAG_TR)+HU.open(TAG_TR,[ATTR_VALIGN,ALIGN_TOP]);
			    }
			    let c = $(this).attr(ATTR_TITLE);
			    let div=HU.div([ATTR_CLASS,CLASS_CLICKABLE,
					    ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_INLINE_BLOCK)],
					   c);
		    ...
		    exDiv=HU.div([ATTR_STYLE,HU.css(CSS_MAX_HEIGHT,HU.px(500),
				 ...
				 exDiv);
		    let ex = $(exDiv).appendTo(_this.jq(ID_EXPANDED_WIKI_MENU));
		    ex.find(TAG_IMG).attr(ATTR_WIDTH,HU.px(200));
		}
		expanded = !expanded;
		if(expanded) {
		    _this.jq(ID_EXPAND_WIKI_MENU).html(HU.getIconImage("fas fa-minimize"));
		    _this.jq(ID_EXPANDED_WIKI_MENU).show();
		    _this.jq(ID_UNEXPANDED_WIKI_MENU).hide();
		} else {
		    _this.jq(ID_EXPAND_WIKI_MENU).html(HU.getIconImage("fas fa-maximize"));
		    _this.jq(ID_EXPANDED_WIKI_MENU).hide();
		    _this.jq(ID_UNEXPANDED_WIKI_MENU).show();
		}
	    });
		...
		let text = $(this).val().trim().toLowerCase();
		let seen = {};
		let displayLinks = popup.find(HU.dotClass('wiki-editor-popup-link'));
		let headers = popup.find(HU.dotClass('wiki-editor-display-category'));
		displayLinks.each(function() {
			...
			if(!corpus) {
			    console.log('no corpus',$(this).html());
			    return;
			}
			corpus =  corpus.toLowerCase();
	...
	btn.find(TAG_I).css(CSS_COLOR,'blue');
	btn.css(CSS_BACKGROUND,'#aaa');
	scroller.css(CSS_CURSOR,CURSOR_POINTER);
    },
	...
	popup =HU.center(HU.input('','',[ATTR_PLACEHOLDER,'Find Tag',
					 ...
					 ATTR_WIDTH,10])) + popup;
	popup = HU.div([ATTR_STYLE,HU.css(CSS_WIDTH,HU.px(600),
					  CSS_HEIGHT,HU.px(300),
	...
	jqid(this.id+'_toolbar_search').click(function(){
	    ...
	    this.tagSelectDialog.find(TAG_A).tooltip({
		classes: {"ui-tooltip": "wiki-editor-tooltip"},
		content: function () {
		    return $(this).prop(ATTR_TITLE);
		},
	    ...
	    HU.findClass(this.tagSelectDialog,'wiki-editor-popup-category').css(CSS_DISPLAY,DISPLAY_NONE);
	    let tags = this.tagSelectDialog.find(HU.dotClass('wiki-editor-popup-link'));
	    //	    let tags = this.tagSelectDialog.find(TAG_A);	    
	...
	let treeAttrs = Utils.mergeLists([
	    ...
	    {p:'metadataDisplay1',
	     ex:'<metadata type>:template=<b>{attr1}_colon_</b> {attr2}:separator=;',
	     tt:'Add metadata under the name. e.g.: type1:template={attr1},type2:template={attr1}_colon_ {attr2}'},
	...
	let mapProperties = [
	    ...
	    {p:'skipEntries',ex:true,tt:'Don\'t show the entries'},
	    {p:'addLabel',ex:true},
	    {p:'declutter',ex:true},
	    {p:'fontSize',ex:'10pt'},
	    {p:'textBackground',ex:'white'},
	    {p:'textPadding',ex:4},	    	    	    
	    {p:'labelMaxLength',ex:20},
...
function getWikiEditorMenuBlocks(attrs,forPopup,id) {
    ...
    let itemClass = HU.classes(CLASS_MENUITEM, (forPopup?HU.classes(CLASS_HOVERABLE,CLASS_HIGHLIGHTABLE):""));
    let currentLabel = '';
    let processAttr =(tag)=>{
	...
	if(tag.info) {
	    block.items.push(HU.div([ATTR_CLASS, itemClass],
				    HU.tag(TAG_I,[],tag.info)));
	    return;
	}
	if(!tag.p)  {
...
function getWikiEditorMenuBar(blocks,id, prefix) {
    let menu  = '';
    menu += HU.tag(TAG_LI, [],
		   HU.div([ATTR_ID,ID_SEARCH_ATTRIBUTES,
			   ATTR_CLASS,
	...
	if(Array.isArray(block.items)) {
	    sub = Utils.wrap(block.items,HU.open(TAG_LI),'');
	} else {
	    sub = block.items;
	}
	menu += HU.tag(TAG_LI, [],
    ...
    let menubar = HU.div([ATTR_CLASS,'wiki-popup-menubar',
			  ATTR_ID, id],
			 HU.tag(TAG_UL, [ATTR_ID, id+'_inner',
					 ATTR_CLASS, 'sf-menu'], menu))
    return menubar;
}
...
Transcriber.prototype = {
	...
	this.jq(ID_TRANSCRIBE_PLAY).html(HU.getIconImage('fa-solid fa-stop fa-gray'));
	let updateTime = ()=>{
	    let now = new Date();
	...
	this.jq(ID_TRANSCRIBE_PLAY).html(HU.getIconImage('fa-solid fa-microphone fa-gray'));
    },
	...
	navigator.mediaDevices.getUserMedia({audio: true,}).then((stream) => {
		...
		HU.span([ATTR_TITLE,'Start recording',
			 ATTR_CLASS,CLASS_CLICKABLE,
			 ATTR_ID,this.domId(ID_TRANSCRIBE_PLAY)],
		...
		HU.span([ATTR_TITLE,'Delete recording',
			...
			HU.getIconImage('fa-solid fa-delete-left fa-gray')),
	    ...
	    html+=HU.div([ATTR_STYLE,HU.css(CSS_POSITION,POSITION_RELATIVE)],
			 HU.textarea('','',[ATTR_PLACEHOLDER,'',
					    ATTR_ID,this.domId(ID_TRANSCRIBE_TEXT),
					    ATTR_ROWS,6,
					    ATTR_COLS,80,
					    ATTR_STYLE,
					    HU.css(CSS_BORDER,CSS_BASIC_BORDER,
						   CSS_PADDING,HU.px(4),
						   CSS_MARGIN,HU.px(4),
						   CSS_FONT_STYLE,FONT_ITALIC)])+
			 HU.div([ATTR_STYLE,HU.css(CSS_DISPLAY,DISPLAY_NONE,
						   CSS_POSITION,POSITION_ABSOLUTE,
						   CSS_TOP,HU.perc(50),
						   CSS_LEFT,HU.perc(50),
				...
				HU.image(RamaddaUtil.getCdnUrl('/icons/mapprogress.gif'),
					 [ATTR_STYLE,HU.css(CSS_WIDTH,HU.px(100))])));
	    html+=HU.buttons([HU.span([ATTR_CLASS,CLASS_DIALOG_BUTTON,'append','true'],
				      this.opts.appendLabel),
			      HU.span([ATTR_CLASS,CLASS_DIALOG_BUTTON,
				       ATTR_ID,this.domId(ID_CANCEL)],LABEL_CANCEL)]);
	    html = HU.div([ATTR_CLASS,CLASS_DIALOG],html);
	    let closeCallback =()=>{
	    ...
	    this.jq(ID_TRANSCRIBE_PLAY).button().click(()=>{
		if(this.transcribePlaying) {
		    this.transcribeStop();
		} else {
	    ...
	    this.jq(ID_TRANSCRIBE_DELETE).click(()=>{
	    ...
	    let _this = this;
	    dialog.find(HU.dotClass(CLASS_DIALOG_BUTTON)).button().click(function() {
		let val = _this.jq(ID_TRANSCRIBE_TEXT).val()??'';
		if ($(this).attr('append')) {
