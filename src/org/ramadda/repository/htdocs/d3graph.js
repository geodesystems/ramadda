...
//            var path = svg.append("svg:g").selectAll("path").data(force.links(),  function(d) { return d.source.id + "-" + d.target.id; });
            ...
            .attr("class", function(d) { return "graph-path"})
            .attr("marker-end", function(d) { return "url(#arrow)"; });
            path.exit().remove();       
        }
    ...
    force.on("tick", function() {
        ...
        allNodes.attr("class", function(d) {if(d.graphurl) return "graph-node-text-unvisited"; else return "graph-node-text-visited";});
