...
echo "Running RAMADDA: ${JAVA} -Xmx${JAVA_MEMORY}  -Dfile.encoding=utf-8 -jar ${RAMADDA_DIR}/lib/ramadda.jar -port ${RAMADDA_PORT} -Dramadda_home=${RAMADDA_HOME} $* " > ${RAMADDA_PARENT_DIR}/service.out

## Add -Djava.awt.headless=true if there are problems running remotely on a Mac

${JAVA} -Xmx${JAVA_MEMORY}  -Dfile.encoding=utf-8 -jar ${RAMADDA_DIR}/lib/ramadda.jar -port ${RAMADDA_PORT} -Dramadda_home=${RAMADDA_HOME} $* 
