...
//		if(typeHandler!=null) theTypeHandlers.add(typeHandler);
    ...
    */
	...
	Statement select = getDatabaseManager().select(connection,
						       Tables.ENTRIES.COL_TYPE,
						       Misc.newList(Tables.ENTRIES.NAME),
						       Clause.eq(Tables.ENTRIES.COL_ID,entryId),"",1);
    ...
    */
	...
	for(TypeHandler _typeHandler: getRepository().getTypes(type)) {
	    typeList.add(_typeHandler.getType());
	}
	getDatabaseManager().addTypeClause(getRepository(),request, typeList,ors);
