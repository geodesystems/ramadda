...
public class CsvOutputHandler extends OutputHandler {
	...
	boolean isCsv = what.equals(WHAT_CSVAPI);
	...
	for(Entry entry: entries) {
		...
		if(what.equals(WHAT_WGET)) {
			...
			url = request.getAbsoluteUrl(url);
			sb.append("wget --no-check-certificate  -O "+ HU.quote(file) +" " + HU.quote(url)+";\n");
		    } else   {
			throw new IllegalArgumentException("Unknown output format:" + what);
		    }
		}
		int cnt=0;
		file = file.trim();
