...
public class Seesv implements SeesvCommands {
    ...
    private static int uniqueCnt=0;
    public static List<File> applySeesv(File dir, String[]commands,List<File> inputFiles) throws Exception {
    ...
    */
	...
	if (file.startsWith(PREFIX_RESOURCE)) {
	    ...
	    try(InputStream is =  IOUtil.getInputStream(file, Seesv.class)) {
	...
	*/
	...
	new Cmd(CMD_ROWS_LAST, "Move rows to the end of list that match the pattern",
		...
		new Arg(ATTR_VALUES,
			"Comma separated list of new values. Can contain ${column} macros",
			ATTR_TYPE,
			TYPE_LIST)),
		...
		new Arg(ARG_NAME, "Comma separated list of column names"),		
		new Arg(ATTR_VALUES,
			"Comma separated list of new values. Can contain ${column} macros",
			ATTR_TYPE,TYPE_LIST)),
	new Cmd(CMD_CONCAT, "Create a new column from the given columns",
		...
		new Arg(ARG_COLUMN, "Value column", ATTR_TYPE, TYPE_COLUMN),
                new Arg("delimiter"),
	...
	new Cmd(CMD_CHECK, "Check that the values are numbers",
		...
		ARG_LABEL,"Reverse geocode",
                ...
                new Arg(ARG_LONGITUDE, "longitude column")),		
