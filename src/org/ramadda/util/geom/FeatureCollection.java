...
public class FeatureCollection {
    ...
    public static   DbaseDataWrapper  getNameField(List<DbaseDataWrapper> fieldDatum) throws Exception {
    ...
    public static FeatureCollection makeFeatureCollection(String fcname, String desc, EsriShapefile shapefile,
							  Hashtable extraProps,
							  Hashtable<String, Object> collectionProps)  throws Exception {

        DbaseFile              dbfile    = shapefile.getDbFile();
        collectionProps.put("dbfile", dbfile);
        collectionProps.put("shapefile", shapefile);	

	List<DbaseDataWrapper> fieldDatum = FeatureCollection.getDatum(dbfile, extraProps, collectionProps);
        DbaseDataWrapper       nameField = getNameField(fieldDatum);
	collectionProps.put(FeatureCollection.PROP_SCHEMA,DbaseDataWrapper.getSchema(fieldDatum));

        List          features   = shapefile.getFeatures();
        FeatureCollection fc = new FeatureCollection(fcname, desc, collectionProps, fieldDatum);
		...
		name = nameField.getString(i).trim();
            ...
            Hashtable<String, Object> featureProps = new Hashtable<String, Object>();
            if (fieldDatum != null) {
		String schemaName = (String)collectionProps.get(FeatureCollection.PROP_SCHEMANAME);
		String schemaId = (String)collectionProps.get(FeatureCollection.PROP_SCHEMAID);		
		if(schemaName ==null) schemaName = "";
		if(schemaId ==null) schemaId = "";		
		featureProps.put(FeatureCollection.PROP_SCHEMANAME, schemaName);
		featureProps.put(FeatureCollection.PROP_SCHEMAID, schemaId);
		    ...
		    // since shapefile parser makes no distinction between ints & doubles, this hack will fix that.
                ...
                featureProps.put(FeatureCollection.PROP_SCHEMADATA, schemaData);
	    }
            Feature feature = new Feature(name, geom, featureProps, collectionProps);
    ...
    public static List<DbaseDataWrapper> getDatum(DbaseFile dbfile, Hashtable properties, Hashtable pluginProperties)
	throws Exception {
        if (dbfile == null) {
	    return null;
	}
	...
	for (int j = 0; j < fieldNames.length; j++) {
	    ...
	    if (properties != null) {
		String v = (String) properties.get("map."+ dbd.getName() + ".drop");
	...
	for(Feature feature: features) {
	    if(feature.contains(lat,lon)) return feature;
	}
	return null;
    }
