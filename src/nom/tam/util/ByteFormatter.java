...
public final class ByteFormatter {
    ...
    static {  // Static initializer
        ...
        int min = (int) Math.floor((int) (Math.log(Double.MIN_VALUE)
                                          * ilog10));
        int max = (int) Math.floor((int) (Math.log(Double.MAX_VALUE)
                                          * ilog10));
        max    += 1;
     ...
     *
     * @throws TruncationException _more_
     */
    ...
    public int format(int val, byte[] buf, int off, int len)
        ...
        if (val == Integer.MIN_VALUE) {
            if ((len > 10)
                    || ( !truncateOnOverflow && (buf.length - off > 10))) {
        ...
        if ((truncateOnOverflow && (ndig > len))
                || (ndig > buf.length - off)) {
            truncationFiller(buf, off, len);
    ...
    public int format(long val, byte[] buf, int off, int len)
        ...
        if (val == Long.MIN_VALUE) {
            if ((len > 19)
                    || ( !truncateOnOverflow && (buf.length - off > 19))) {
    ...
    public int format(String val, byte[] array, int off, int len) {
        ...
        if ((truncateOnOverflow && (slen > len))
                || (slen > array.length - off)) {
     ...
     * @throws TruncationException _more_
     */
    int combineReal(double val, byte[] buf, int off, int len, byte[] mant,
                    int lmant, int shift)
            throws TruncationException {
        ...
        if (simple) {
            return Math.abs(mantissa(mant, lmant, exp, simple, buf, off,
                                     len));
        } else {
     ...
     * @return _more_
     */
    int mantissa(byte[] mant, int lmant, int exp, boolean simple, byte[] buf,
                 int off, int len) {
