...
public class BinaryTableHDU extends TableHDU {
    ...
    private String[] keyStems = {
        "TTYPE", "TFORM", "TUNIT", "TNULL", "TSCAL", "TZERO", "TDISP", "TDIM"
    };
    ...
    public static boolean isData(Object o) {
        ...
        if ((o instanceof nom.tam.util.ColumnTable)
                || (o instanceof Object[][]) || (o instanceof Object[])) {
            return true;
        } else {
            return false;
        }
    ...
    public void write(ArrayDataOutput ado) throws FitsException {
        ...
        if (oldSize != table.getHeapSize()) {
            myHeader.addValue("PCOUNT", table.getHeapSize(),
                              "ntf::binarytablehdu:pcount:1");
        }
    ...
    public boolean setComplexColumn(int index) throws FitsException {
        ...
        if (table.setComplexColumn(index)) {
            ...
            myHeader.addValue("TFORM" + (index + 1), dim + prefix + suffix,
                              oldComment);
            if (tdim.length() > 0) {
                myHeader.addValue("TDIM" + (index + 1), "(" + tdim + ")",
                                  "ntf::binarytablehdu:tdimN:1");
            } else {
