...
public class BinaryTable extends Data implements TableData {
    ...
    public BinaryTable() throws FitsException {
        ...
        try {
            ...
            System.err.println(
                "Impossible exception in BinaryTable() constructor" + e);
        }
    ...
    public BinaryTable(ColumnTable tab) {
        ...
        for (int col = 0; col < nCol; col += 1) {
            rowLen += sizes[col]
                      * ArrayFuncs.getBaseLength(tab.getColumn(col));
        }
        heapOffset = 0;
        nRow       = tab.getNRows();
    }
    ...
    private int processCol(Header header, int col) throws FitsException {
        ...
        if (tform == null) {
            throw new FitsException("Attempt to process column " + (col + 1)
                                    + " but no TFORMn found.");
        }
        tform = tform.trim();
    ...
    public static int[] getTDims(String tdims) {
        ...
        if ((first >= 0) && (last > first)) {
            ...
            java.util.StringTokenizer st =
                new java.util.StringTokenizer(tdims, ",");
    ...
    boolean setComplexColumn(int index) throws FitsException {
        ...
        if ((index >= 0) && (index < bases.length)
                && ((bases[index] == float.class)
                    || (bases[index]
                        == double.class)) && (dimens[index][dimens[index].length - 1]
                            == 2)) {
    ...
    public void fillHeader(Header h) throws FitsException {
        ...
        try {
            ...
            iter.next();
            iter.add("TFIELDS",
                     new HeaderCard("TFIELDS", modelRow.length,
                                    "ntf::binarytable:tfields:1"));
    ...
    void fillForColumn(Header h, int col, Cursor iter) throws FitsException {
        ...
        if (bases[col] == int.class) {
            tform += "J";
        } else if ((bases[col] == short.class)
                   || (bases[col] == char.class)) {
            tform += "I";
            ...
            throw new FitsException("Invalid column data class:"
                                    + bases[col]);
        }
        ...
        if ((dimens[col].length > 0) && !isVarCol(col)) {
            ...
            key = "TDIM" + (col + 1);
            iter.add(key,
                     new HeaderCard(key, new String(tdim),
                                    "ntf::headercard:tdimN:1"));
        }
    }
    ...
    private ColumnTable createTable() throws FitsException {
        ...
        for (int i = 0; i < nfields; i += 1) {
            arrCol[i] =
                ArrayFuncs.newInstance(ArrayFuncs.getBaseClass(modelRow[i]),
                                       sizes[i] * nRow);
        }
        ...
        for (int col = 0; col < row.length; col += 1) {
            ...
            if (row[col] instanceof String) {
                ...
                if ((dims.length > 1) || (dims[0] > 1)) {
                    ...
                    Object[] arr = (Object[]) ArrayFuncs.newInstance(base,
                                       xdims);
                    for (int irow = 0; irow < nrow; irow += 1) {
    ...
    private Object[] getFileRow(int row) throws FitsException {
        ...
        for (int col = 0; col < data.length; col += 1) {
            data[col] = ArrayFuncs.newInstance(
                ArrayFuncs.getBaseClass(modelRow[col]), sizes[col]);
        }
    ...
    private Object encurl(Object res, int col, int rows) {
        ...
        if (bases[col] != String.class) {
            ...
            if (dimens[col].length > 2) {
                ...
                System.arraycopy(dimens[col], 0, dims, 1,
                                 dimens[col].length - 1);
                dims[0] = rows;
    ...
    public void deleteRows(int row, int len) throws FitsException {
        try {
            ...
            throw new FitsException("Error deleting row block " + row
                                    + " to " + (row + len - 1)
                                    + " from table");
        }
    }
    ...
    private boolean isVarying(Object o) {
        ...
        if ((classname.length() != 3) || (classname.charAt(0) != '[')
                || (classname.charAt(1) != '[')) {
            return false;
        }
    ...
    public int addFlattenedColumn(Object o, int[] dims) throws FitsException {
        ...
        if ( !isVarCol(nCol)) {
            modelRow[nCol] =
                ArrayFuncs.newInstance(ArrayFuncs.getBaseClass(o), dims);
    ...
    public void read(ArrayDataInput i) throws FitsException {
        ...
        if (i instanceof RandomAccess) {
            ...
            try {
                ...
                throw new FitsException("Unable to skip binary table HDU:"
                                        + e);
            }
            try {
                ...
                throw new PaddingException(
                    "Missing padding after binary table:" + e, this);
            } catch (IOException e) {
                throw new FitsException(
                    "Error skipping padding after binary table:" + e);
            }
    ...
    protected void readTrueData(ArrayDataInput i) throws FitsException {
        ...
        try {
            ...
            throw new PaddingException(
                "Error skipping padding after binary table", this);
        } catch (IOException e) {
            throw new FitsException(
                "Error reading binary table data padding:" + e);
        }
    }
    ...
    public Object getData() throws FitsException {
        ...
        if (table == null) {
            ...
            if (currInput == null) {
                throw new FitsException(
                    "Cannot find input for deferred read");
            }
    ...
    private Object arrayToColumn(int col, Object o) throws FitsException {
        ...
        if ( !isVarCol(col)) {
            ...
            if (isString(col)) {
                ...
                o = FitsUtil.stringsToByteArray((String[]) o,
                        dims[dims.length - 1]);
        ...
        if (isVarCol(col)) {
            ...
            if (isLongVary(col)) {
                ...
                if ( !warnedOnVariableConversion) {
                    System.err.println(
                        "Warning: converting long variable array pointers to int's");
            ...
            if (isString(col)) {
                ...
                if (bytes.length > 0) {
                    if (dims.length > 0) {
                        o = FitsUtil.byteArrayToStrings(bytes,
                                dims[dims.length - 1]);
                    } else {
    ...
    public void deleteColumns(int start, int len) throws FitsException {
        ...
        try {
            ...
            throw new FitsException(
                "Error deleting columns from BinaryTable:" + e);
        }
    }
