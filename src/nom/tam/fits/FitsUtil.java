...
public class FitsUtil {
    ...
    public static void reposition(Object o, long offset)
        ...
        if ( !(o instanceof RandomAccess) || (offset < 0)) {
            throw new FitsException("Invalid attempt to reposition stream "
                                    + o + " of type "
     ...
     * @throws FitsException _more_
     */
    static InputStream decompress(InputStream compressed)
            throws FitsException {
        ...
        try {
            ...
            if ((mag1 == 0x1f) && (mag2 == 0x8b)) {
                ...
                try {
                    Constructor con = Class.forName(cname).getConstructor(
                                          InputStream.class);

                    return (InputStream) con.newInstance(pb);
                } catch (Exception e) {
                    System.err.println(
                        "Unable to find constructor for BZIP2 decompression.  Is the Apache BZIP jar in the classpath?");

                    throw new FitsException(
                        "No CBZip2InputStream class found for bzip2 compressed file");
                }
            } else {
                // Push the data back into the stream
                pb.unread(mag2);
     ...
     * @throws FitsException _more_
     */
    static InputStream compressInputStream(final InputStream compressed)
            throws FitsException {
        try {
            ...
            throw new FitsException(
                "Unable to read .Z compressed stream.\nIs `uncompress' in the path?\n:"
                + e);
        }
    }
    ...
    public static boolean isCompressed(String filename) {
        ...
        return (len > 2)
               && (filename.substring(len - 3).equalsIgnoreCase(".gz")
                   || filename.substring(len - 2).equals(".Z"));
    }
    ...
    public static String[] byteArrayToStrings(byte[] o, int maxLen) {
        ...
        for (int i = 0; i < res.length; i += 1) {
            ...
            if (errFound && !wroteCheckingError) {
                System.err.println(
                    "Warning: Invalid ASCII character[s] detected in string:"
                    + res[i]);
                System.err.println(
     ...
     * @throws FitsException _more_
     */
    public static void pad(ArrayDataOutput stream, long size)
            throws FitsException {
            ...
            t   = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        throw new Error(
                            "Error reading BZIP compression using: "
                            + System.getenv("BZIP_DECOMPRESSOR"), e);
                    }
                }
            });

        } catch (Exception e) {
            throw new FitsException("Error initiating BZIP decompression: "
                                    + e);
        }
        t.start();
